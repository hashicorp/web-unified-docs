...
created_at: 2026-04-29T14:16:58.000Z
last_modified: 2026-04-29T14:16:58.000Z
...
Each Terraform test lives in a test file. Terraform discovers test files based on their file extension: `.tftest.hcl` or `.tftest.json`.
...
Assertion arguments in Terraform `run` blocks help your [validate your configuration](/terraform/language/validate). Each `assert` block contains a [`condition` argument](/terraform/language/expressions/conditionals) and an [`error_message` argument](/terraform/language/validate#error-messages).
...
Assertions within tests can reference any existing [named values](/terraform/language/expressions/references) that are available within the main Terraform configuration.
...
You can provide values for [Input Variables](/terraform/language/block/variable) within your configuration directly from your test files.
...
You can specify values for variables across all tests with the [Command line](/terraform/language/values/variables#command-line-variables) and with [Variable definition files](/terraform/language/values/variables#variable-definition-files).
...
At the root level of a Terraform testing file, you can define [`provider` blocks](/terraform/language/block/provider) as if Terraform were creating them [within the main configuration](/terraform/language/providers). Terraform will then pass these provider blocks into its configuration as each `run` block executes.
...
By default, if any [validation](/terraform/language/validate), including `check` block assertions, fails during the execution of a Terraform test file then the overall command reports the test as a failure.
...
You can use assertions alongside an `expect_failures` block, but consider that all validation features, except `check` blocks, halt Terraform execution on failure. This applies during test execution, so your assertions should only evaluate values that are certain to be computed before the checkable object is due to fail. You can manage this using references, or the `depends_on` meta-argument within your main configuration.
...
- They do not share the same state file. The state file is determined by the state key or the module source when the state key is not set. If two `run` blocks share the same module configuration, they must specify different state keys to be executed in parallel.
...
Runs A and B execute simultaneously. Run C waits until both A and B are finished before starting. Finally, runs D and E run in parallel, but only after C completes.
