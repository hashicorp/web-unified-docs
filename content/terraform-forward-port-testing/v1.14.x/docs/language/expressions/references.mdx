...
created_at: 2026-04-29T14:11:19.000Z
last_modified: 2026-04-29T14:11:19.000Z
...
`var.<NAME>` is the value of the [input variable](/terraform/language/block/variable) of the given name.
...
`local.<NAME>` is the value of the [local value](/terraform/language/block/locals) of the given name.
...
[output values](/terraform/language/block/output), use `module.<MODULE NAME>.<OUTPUT NAME>`.
...
variables](/terraform/language/block/variable); they are just arbitrary names
...
When a resource has the
[`for_each`](/terraform/language/block/resource#for_each)
...
[an input variable declared as sensitive](/terraform/language/block/variable#sensitive),
...
[output value](/terraform/language/block/output) then Terraform will require
