...
assertions about what happens during those actions. When testing the happy
path, developers can make assertions about what data is stored in state after
successfully provisioning a resource. When testing unhappy paths, developers
are able to make assertions about errors returned by Terraform, non-empty
plans, and the actual cloud resources created by Terraform.

Because acceptance testing performs real actions with Terraform, acceptance
tests require interacting with real APIs. This has a number of consequences:
* Running acceptance tests will lead to cloud spend of some amount.  Running
* acceptance tests can cause cloud accounts to hit rate limits or quotas for
provisioned resources.
* The duration of a test can vary greatly depending on how the APIs it uses
  behave, resulting in 'long-running' tests.

Users are protected from accidental cloud spend by the `TF_ACC` environment
variable, which must be explicitly set to enable and opt into running
acceptance tests. However once users opt in to running a test there are no
methods of protecting against large versus small amounts of cloud spend.

The testing framework also provides a way to validate all resources created by
tests are destroyed.
...
~> **Note**: Acceptance tests typically create and destroy actual
infrastructure resources, possibly incurring expenses during or after the test
duration.
