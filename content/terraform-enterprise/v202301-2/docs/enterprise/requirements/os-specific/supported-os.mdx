...
- Red Hat Enterprise Linux 7.4 - 7.9 / 8.4 - 8.7 ([RedHat Linux Requirements](/enterprise/requirements/os-specific/rhel-requirements))
...
- Add the `container_file_t` type to the SELinux context for the mounted disk path and its subdirectories. The commands below update the mounted disk path `/opt/tfe` and its subdirectories to use the correct SELinux context.
  ...
  ```
-> **Note:** * Indicates a deprecated operating system that will no longer be supported following February 2023 (v202303-1).
