...
-   `terraform plan` starts a [speculative plan](/terraform/enterprise/run/remote-operations#speculative-plans) in a Terraform Cloud workspace, using configuration files from a local directory. You can quickly check the results of edits (including compliance with Sentinel policies) without needing to copy sensitive variables to your local machine.
...
To run a [speculative plan](/terraform/enterprise/run/remote-operations#speculative-plans) on your configuration, use the `terraform plan` command. The plan will run in Terraform Cloud, and the logs will stream back to the command line along with a URL to view the plan in the Terraform Cloud UI.
