...
[JSON API documents]: /enterprise/api-docs#json-api-documents

[JSON API error object]: https://jsonapi.org/format/#error-objects

[run]: /enterprise/run/states
...
Terraform Cloud uses run task stages and run task results to track [run task](/enterprise/workspaces/settings/run-tasks) execution.

When Terraform Cloud creates a [run][], it reads the run tasks associated to the workspace. Each run task in the workspace is configured to begin during a specific [run stage](/enterprise/run/states). Terraform Cloud creates a run task stage object for each run stage that triggers run tasks. You can configure run tasks during the [Pre-Plan Stage](/enterprise/run/states#the-pre-plan-stage), [Post-Plan Stage](/enterprise/run/states#the-post-plan-stage) and [Pre-Apply Stage](/enterprise/run/states#the-pre-apply-stage).
...
This page lists the endpoints to retrieve run task stages and run task results. Refer to the [Run Tasks API](/enterprise/api-docs/run-tasks/run-tasks) for endpoints to create and manage run tasks within Terraform Cloud. Refer to the [Run Tasks Integration API](/enterprise/api-docs/run-tasks/run-tasks-integration) for endpoints to build custom run tasks for the Terraform Cloud ecosystem.
...
| `awaiting_override` | The task stage is waiting for user input. Once a user manually overrides the failed policy checks, the run returns to the `running` state. |
