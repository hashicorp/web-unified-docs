...
These instructions are for using Bitbucket Cloud for Terraform Cloud's VCS features. Bitbucket Cloud is the cloud-hosted version of Bitbucket; [self-hosted Bitbucket Server instances have separate instructions,](/terraform/enterprise/vcs/bitbucket-server) as do the [other supported VCS providers.](/terraform/enterprise/vcs)
