...
HashiCorp provides a [stability policy](/terraform/enterprise/api-docs/stability-policy) for the Terraform Cloud API, ensuring backwards compatibility for stable endpoints. The [changelog](/terraform/enterprise/api-docs/changelog) tracks changes to the API for Terraform Cloud and Terraform Enterprise.
...
-   [User tokens](/terraform/enterprise/users-teams-organizations/users#api-tokens) — each Terraform Cloud user can have any number of API tokens, which can make requests on their behalf.
...
Terraform Cloud relies on a HashiCorp-developed blob storage service for storing statefiles and multiple other pieces of customer data, all of which are documented on our [data security page](/terraform/enterprise/replicated/architecture/system-overview/data-security).
