...
1. The demo [operational mode](/terraform/enterprise/replicated/install/pre-install-checklist#operational-mode-decision) has been removed. If you are currently running demo mode, we strongly suggest that you upgrade to Terraform Enterprise v202204-1. To do this, you must [migrate your application data](/terraform/enterprise/admin/infrastructure/demo-to-disk-migration).
...
1. Changed cost estimation so that it uses the [HTTP proxy settings](/terraform/enterprise/replicated/install/interactive/installer#proxy-usage) configured within Terraform Enterprise.
...
1. The [`iact_subnet_list` setting](/terraform/enterprise/replicated/install/automated/automating-the-installer#iact_subnet_list) now allows you to use `, ` to separate IPv4 addresses.
