...
1. Only applicable when using External Vault. You must update your [External Vault](/terraform/enterprise/requirements/data-storage/vault) policy to use specific API paths instead of wildcard matching. Skipping this step prevents Terraform Enterprise from starting.
...
1. Using the new `azure_use_msi` and `azure_client_id` [settings](/terraform/enterprise/install/automated/automating-the-installer#available-settings), it is now possible to authenticate to Azure Blob Storage with a system-assigned or user-assigned Azure managed identity.
...
1. The [Prometheus metrics endpoint](/terraform/enterprise/admin/infrastructure/monitoring#terraform-enterprise-metrics) now ships an additional metric `tfe_run_current_count`, which represents the current count of TFE runs in a given workspace, organization, and status.
...
1. The [External Vault](/terraform/enterprise/requirements/data-storage/vault) policy has been updated to use specific API paths instead of wildcard matching.
