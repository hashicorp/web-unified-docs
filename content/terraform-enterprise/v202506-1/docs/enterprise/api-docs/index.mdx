...
Some endpoints let you interact with Terraform Enterprise application resources such as workspaces, runs, and variables. Unless otherwise specified, all documentation on this site refers to application endpoints.
...
Terraform Enterprise relies on a HashiCorp-developed blob storage service for storing state files and other pieces of customer data. This service does not require you to submit a bearer token with each request. Instead, each URL includes a securely-generated secret and is only valid for 25 hours.
...
If an organization doesn't have the necessary entitlement to use a given feature, Terraform Enterprise returns a `404` error for API requests to any endpoints devoted to that feature.
