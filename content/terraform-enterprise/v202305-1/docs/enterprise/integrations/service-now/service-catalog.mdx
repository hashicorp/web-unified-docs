...
are covered in the [installation documentation](/terraform/enterprise/integrations/service-now).
...
If using v2.2.0 or above, the Service Catalog app allows you to set an [execution mode](/terraform/enterprise/workspaces/settings#execution-mode) for your Terraform workspaces. There are three modes to choose from:

-   The default value is "Remote", which instructs Terraform Cloud to perform runs on its disposable virtual machines.
-   Switching to "Local" mode allows Terraform to execute runs locally using the CLI workflow. 
...
-> **Note:** [Terraform Cloud Agents](/terraform/cloud-docs/agents) are available in the Terraform Cloud Business Tier.
