...
description: >-
  Use the `/state-version-outputs` endpoint to access output values from a
  Terraform state version. List and show state version outputs, and show current
