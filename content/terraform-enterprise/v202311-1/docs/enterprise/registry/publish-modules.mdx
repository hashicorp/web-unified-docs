...
4.  When prompted, choose either the **Tag** or **Branch** module publishing type.

5.  (Optional) If this module is a [no-code ready module](/terraform/enterprise/no-code-provisioning/module-design), select the **Add Module to no-code provision allowlist** checkbox.
...
To release a new version of a module using the branch-based publishing workflow, navigate to the module overview screen, then click the **Publish New Version** button. Select the commit SHA that the new version points to, and assign a new module version. You cannot re-use an existing module version.
...
-   To change from tag-based to branch-based publishing, you must configure a **Module branch** and [create a new module version](#branch-based-publishing-workflow). This is because Terraform Could does not automatically create new module versions.
