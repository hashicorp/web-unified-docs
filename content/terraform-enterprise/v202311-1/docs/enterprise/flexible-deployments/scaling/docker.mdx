---
page_title: Docker Scaling - Installation - Flexible Deployment Options - Terraform Enterprise
description: >-
...
- [CLI-based commands](/terraform/enterprise/flexible-deployments/admin/admin-cli/admin-cli) for administration
...
-> **Note**: Active/Active installations on VMware infrastructure require configuring a load balancer to route traffic across Terraform Enterprise servers. This documentation does not cover that setup. While auto-scaling groups are unavailable via native vCenter options, you must still configure a fully automated deployment. You must also reduce the available servers to one for upgrades, maintenance, and support.

Your Terraform Enterprise application must be using [External Services](/terraform/enterprise/flexible-deployments/install/operation-modes#external-services) mode to connect to an external PostgreSQL database and object storage.
...
Your existing Terraform Enterprise application settings are still necessary, but must be expanded. Refer to [Configuration Reference](/terraform/enterprise/flexible-deployments/install/configuration) for a full list of configuration options.

#### Enable Active/Active

Update the TFE configuration to reflect the `active-active` operational mode:
...
You _must_ add the encryption password value to your configuration,  and that password _must be identical between node instances_ for the Active/Active architecture to function:

| **Key**      | **Description**                           | **Value can change between deployments?**                       | **Specific Format Required** |
| ------------ | ----------------------------------------- | --------------------------------------------------------------- | ---------------------------- |
| TFE_ENCRYPTION_PASSWORD | Used to encrypt sensitive data | **No.** Changing will make decrypting existing data impossible. | No                           |
...
It can take up to 15 minutes for the node to provision and install the Terraform Enterprise application. You can monitor the provisioning status by watching your auto scaling group in your cloud’s web console. To confirm the successful implementation of the TFE application you can use the `tfectl` CLI tool in the TFE container to monitor the application status:

```bash
tfectl app status
```

Refer to the [Admin CLI Commands](/terraform/enterprise/flexible-deployments/administration/admin/admin-cli) documentation for more status and troubleshooting commands.
