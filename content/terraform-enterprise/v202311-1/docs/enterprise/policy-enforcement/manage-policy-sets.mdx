...
7.  **(Optional)** Add **Policy exclusions** for this policy set. Terraform does not enforce this policy on workspaces in the policy set's scope that you specify in the Policy exclusions field. 
...
Sentinel policy sets also have an **override** option. Failed policies stop the run, but any user with [Manage Policy Overrides permission](/terraform/enterprise/users-teams-organizations/permissions#manage-policy-overrides) can override these failures and allow the run to complete.
Both soft-mandatory and hard-mandatory policies can be overriden.
