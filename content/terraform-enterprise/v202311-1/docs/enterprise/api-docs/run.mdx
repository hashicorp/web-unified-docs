...
description: >-
  Use the `/runs` endpoint to manage Terraform runs. List, get, create, apply,
...
| `planned_and_saved`    | The run has finished its planning, checks, and estimates, and can be confirmed for apply. This status is only used for saved plan runs. Not yet supported in Terraform Enterprise.                                                                                                                                                                                                                                     |
...
The run operation specifies the Terraform execution mode. You can reference the following list of possible operation modes and use them as [runs list](/terraform/enterprise/api-docs/run#list-runs-in-a-workspace) query parameters.
...
| `save_plan`      | The run is a saved plan run. It can include both plan and apply phases, but only becomes the workspace's current run if a user chooses to apply it. Not yet supported in Terraform Enterprise. |
...
You can use the following sources as [runs list](/terraform/enterprise/api-docs/run#list-runs-in-a-workspace) query parameters.
...
| -------------------------------------------------- | -------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `data.attributes.allow-empty-apply`                | bool                 |                                                    | Specifies whether Terraform can apply the run even when the plan [contains no changes](/terraform/enterprise/run/modes-and-options#allow-empty-apply). Often used to [upgrade state](/terraform/enterprise/workspaces/state#upgrading-state) after upgrading a workspace to a new terraform version.                                                                                                                                                                     |
| `data.attributes.allow-config-generation`          | bool                 | false                                              | Specifies whether Terraform can [generate resource configuration](/terraform/language/import/generating-configuration) when planning to import new resources. If set to `false`, `import` configuration blocks that do not have a corresponding `resource` block result in an error.                                                                                                                                                                                                                             |
| `data.attributes.auto-apply`                       | bool                 | The current auto apply setting in the workspace    | Enables automatic apply changes when a Terraform plan is successful. Defaults to the [auto apply](/terraform/enterprise/workspaces/settings#auto-apply-and-manual-apply) workspace setting.                                                                                                                                                                                                                                                                       |
| `data.attributes.is-destroy`                       | bool                 | false                                              | Specifies that the plan is a destroy plan. When `true`, the plan destroys all provisioned resources. Mutually exclusive with `refresh-only`.                                                                                                                                                                                                                                                                                                                                       |
...
| `data.attributes.replace-addrs`                    | array\[string]       |                                                    | Specifies an optional list of resource addresses to pass to the `-replace` flag.                                                                                                                                                                                                                                                                                                                                                                                    |
| `data.attributes.target-addrs`                     | array\[string]       |                                                    | Specifies an optional list of resource addresses to pass to the `-target` flag.                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `data.attributes.save-plan`                        | bool                 | false                                              | Specifies if this should be a saved plan run. Terraform can apply saved plan runs, but they perform their plan and checks without becoming the workspace's current run (like a plan-only run), and will only become the current run if you confirm them for apply. When creating new [configuration versions](/terraform/enterprise/api-docs/configuration-versions) for saved plan runs, be sure to make them `provisional`. Not yet supported in Terraform Enterprise. |
...
| `data.relationships.workspace.data.id`             | string               |                                                    | Specifies the workspace ID where Terraform executes the run.                                                                                                                                                                                                                                                                                                                                                                                                               |
| `data.relationships.configuration-version.data.id` | string               |                                                    | Specifies the configuration version to use for this run. If the `configuration-version` object is omitted, Terraform creates the run using the workspace's latest configuration version.                                                                                                                                                                                                                                                                                   |
...
| `filter[operation]` | **Optional.** A comma-separated list of run operations; the result will only include runs that perform one of these operations. Options are listed in [Run Operations](/terraform/enterprise/api-docs/run#run-operations).   |
| `filter[status]`    | **Optional.** A comma-separated list of run statuses; the result will only include runs that are in one of these statuses. Options are listed in [Run States](/terraform/enterprise/api-docs/run#run-states).                |
| `filter[source]`    | **Optional.** A comma-separated list of run sources; the result will only include runs that came from one of these sources. Options are listed in [Run Sources](/terraform/enterprise/api-docs/run#run-sources).             |
