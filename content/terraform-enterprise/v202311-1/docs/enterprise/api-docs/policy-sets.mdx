...
description: >-
  Use the `/policy-sets` endpoint to manage groups of Sentinel and OPA policies.
  List, show, create, and update policy sets and policy set versions. Attach,
  detach, and exclude policies from workspaces, or attach and detach policies to
  projects.
...
[Policy Enforcement](/terraform/enterprise/policy-enforcement) lets you use the policy-as-code frameworks Sentinel and Open Policy Agent (OPA) to apply policy checks to Terraform Cloud workspaces.

[Policy sets](/terraform/enterprise/policy-enforcement/manage-policy-sets) are collections of policies that you can apply globally or to specific [projects](/terraform/enterprise/workspaces/organize-workspaces-with-projects) and workspaces. For each run in the selected workspaces, Terraform Cloud checks the Terraform plan against the policy set.
...
This API provides endpoints to create, read, update, and delete policy sets in a Terraform Cloud organization. To view and manage individual policies, use the [Policies API](/terraform/enterprise/api-docs/policies).
...
| `data.relationships.policy-tool-version`  **(beta)**  | string         | `latest`   | The policy tool version that the policies are evaluated against. For `sentinel` policy sets, you can only set the policy tool version when `agent-enabled` is set to `true`                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `data.attributes.vcs-repo`                           | object         | (previous value) | VCS repository information. When present, Terraform sources the policies and configuration from the specified VCS repository instead of using definitions from Terraform Cloud. Note that this option and `policies` relationships are mutually exclusive and may not be used simultaneously.                    |
| `data.attributes.vcs-repo.branch`                    | string         | (previous value) | The branch of the VCS repo. When empty, Terraform uses the VCS provider's default branch value.                                                                                                                                                                                                                  |
...
| `data.attributes.vcs-repo.ingress-submodules`        | boolean        | (previous value) | Determines whether Terraform instantiates repository submodules during the clone operation.                                                                                                                                                                                                                    |
| `data.attributes.policies-path`                      | boolean        | (previous value) | The subdirectory of the attached VCS repository that contains the policies for this policy set. Terraform ignores files and directories outside of the sub-path. Changes to the unrelated files do not update the policy set. You can only enable this option when a VCS repository is present. |
...
| `data.relationships.workspace-exclusions`            | array\[object] | (previous value) | An array of references to excluded workspaces that Terraform Cloud does not enforce this policy set on. Sending an empty array clears all exclusions assignments.                                                                                                                                          |
| `data.relationships.agent-enabled`  **(beta)**       | boolean        | `false`          | Only valid for `sentinel` policy sets. Whether this policy set should run as a policy evaluation in the terraform cloud agent                                                                                                                                                                                |
| `data.relationships.policy-tool-version`  **(beta)** | string         | `latest`         | The policy tool version that policies are evaluated against. For `sentinel` policy sets, the policy tool version can be set only if `agent-enabled` is set to `true`                                                                                                                                |
