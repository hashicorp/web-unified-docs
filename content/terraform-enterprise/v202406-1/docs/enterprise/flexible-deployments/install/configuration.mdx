...
last_modified: 2026-09-04T14:33:53.000Z
...
, in your [Helm chart](https://helm.sh/) (Kubernetes)
or in the `env` block of your [Nomad job](/nomad/docs/job-specification/env).
...
### `TFE_RUN_PIPELINE_NOMAD_ADDRESS`

Address of the Nomad server where Terraform Enterprise and Terraform Enterprise agents run. Defaults to `"http://localhost:4646"`.

### `TFE_RUN_PIPELINE_NOMAD_TLS_CONFIG_CA_CERT`

File path to CA certificate for mTLS/TLS communication with Nomad.

Required if Nomad has mTLS/TLS enabled.

### `TFE_RUN_PIPELINE_NOMAD_TLS_CONFIG_CLIENT_CERT`

File path to client certificate for mTLS/TLS communication with Nomad.

Required if Nomad has mTLS enabled.

### `TFE_RUN_PIPELINE_NOMAD_TLS_CONFIG_CLIENT_KEY`

File path to the client certificate key for mTLS/TLS communication with Nomad.
...
### `TFE_RUN_PIPELINE_NOMAD_TLS_CONFIG_INSECURE`

If set disables SSL verification while connecting with Nomad. Defaults to `true` when Nomad address schema is `http`. Defaults to `false` when Nomad address schema is `https`

## Proxy configuration

Terraform Enterprise relies on the native application runtime for setting proxy values. For configuring proxies in Docker,
refer to the offical [Docker proxy configuration guide](https://docs.docker.com/network/proxy/). For Kubernetes, refer to the
[Kubernetes proxy configuration guide](https://kubernetes.io/docs/concepts/cluster-administration/proxies/). For Nomad deployments, refer to the [reference architecture guide](/terraform/enterprise/replicated/architecture/reference-architecture)
