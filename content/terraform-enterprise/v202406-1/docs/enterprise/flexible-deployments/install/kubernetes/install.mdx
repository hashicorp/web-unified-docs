---
page_title: Installation on Kubernetes & OpenShift (Beta) - Flexible Deployment Options - Terraform Enterprise
description: >-
...
Kubernetes and OpenShift deployments have different operational and observability considerations than traditional deployments, and external service dependencies should be deployed outside the cluster and scale reliably to accommodate Terraform Enterprise workloads. Refer to [Architectures](/terraform/enterprise/replicated/architecture/reference-architecture) for other deployment options.


-> **Note:** For existing Terraform Enterprise users with [Replicated deployments](/terraform/enterprise/replicated), refer to [Migrating to Flexible Deployment Options](/terraform/enterprise/replicated/replicated-migration).

## 1. Requirements

  1. Before installing, ensure you meet the [shared requirements](/terraform/enterprise/flexible-deployments/install/requirements/) for all flexible deployment methods and the [requirements for Kubernetes and OpenShift (Beta) deployments](/terraform/enterprise/flexible-deployments/install/kubernetes/requirements).

## 2. Pull image

  <Note title="OpenShift kubectl configuration">
  The following instructions assume that you have kubectl available and successfully ran <code>oc login</code> when installing an OpenShift environment to establish a functioning Kubernetes context to authenticate and interact with OpenShift.
  </Note>
...
### Support for forking
The [Terraform Enterprise Helm Chart](https://github.com/hashicorp/terraform-enterprise-helm) aims to meet the needs of the majority of our users. You are welcome to fork our helm chart and adapt it to your organization’s requirements. 
