...
Metrics collection can be configured with the `metrics_endpoint_enabled` config flag in the [application config file](/terraform/enterprise/replicated/install/automated/automating-the-installer#application-settings). By default, `metrics_endpoint_enabled` is set to `"0"` (disabled). To enable metrics collection, set this value to `"1"`.
