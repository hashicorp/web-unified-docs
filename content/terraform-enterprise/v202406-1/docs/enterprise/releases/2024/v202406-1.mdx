...
1. [Updated September 6, 2024] This release contains a possible migration error due to orphaned team membership records referring to teams that have been deleted. If this issue affects you, the upgrade will fail. To avoid this, please refer to [our support article](https://www.ibm.com/support/pages/node/7265753) for steps to check if you have orphaned records and how to remove them before proceeding with the migration.
...
## Highlights
1. This Terraform Enterprise release (v202406-1) is required, because of an upgrade of the internal PostgreSQL service from v14 to v16. This database upgrade also enables using the backup and restore API endpoints against external PostgreSQL servers running versions 15 or 16. If your upgrade fails, [restore your PostgresQL v14 database](https://www.ibm.com/support/pages/node/7265173).
1. Terraform Enterprise now supports a new [Flexible deployment option](/terraform/enterprise/flexible-deployments). Terraform Enterprise can now be deployed to HashiCorp Nomad. The feature is still in Beta stage. To get started, check out the [Nomad Beta requirements](/terraform/enterprise/flexible-deployments/install/nomad/requirements).
1. Terraform Enterprise now supports a new [Flexible deployment option](/terraform/enterprise/flexible-deployments). Terraform Enterprise can now be deployed to Red Hat OpenShift. The feature is still in Beta stage. To get started, please read the updated [Kubernetes and OpenShift requirements](/terraform/enterprise/flexible-deployments/install/kubernetes/requirements) and check out [Operater Terraform Enterprise on Red Hat OpenShift](/terraform/enterprise/flexible-deployments/install/kubernetes/openshift).

## Features
1. You can now search for organizations by name on the main organizations page.
1. A new page has been added to allow administrators to view and cancel the current run for each workspace in an organization. For more information, refer to [Organizations](https://developer.hashicorp.com/terraform/enterprise/users-teams-organizations/organizations#runs).
