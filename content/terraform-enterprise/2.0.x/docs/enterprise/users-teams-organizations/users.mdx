...
last_modified: 2026-04-06T23:41:04.000Z
=======
...
=======
[invite]: /terraform/enterprise/users-teams-organizations/organizations#invite-users-to-the-organization
>>>>>>> tfe-release/2.0.0
...
User accounts belong to individual people. Each user can be part of one or more [teams](/terraform/enterprise/users-teams-organizations/teams), which are granted permissions on projects and projects and workspaces within an organization. A user can be a member of multiple [organizations][].
...
=======
After you create an account, you do not belong to any organizations. To begin using HCP Terraform, you can either [create an organization](/terraform/enterprise/users-teams-organizations/organizations#create-an-organization) or ask an organization owner to send you an invitation email to join their organization.
>>>>>>> tfe-release/2.0.0
...
<<<<<<< HEAD
An organization owner or a user with [**Manage Membership**](/terraform/enterprise/users-teams-organizations/permissions/organization#manage-membership) permissions enabled must [invite you to join their organization](/terraform/enterprise/users-teams-organizations/organizations#users) and [add you to one or more teams](/terraform/enterprise/users-teams-organizations/teams/manage#manage-team-membership).
=======
An organization owner or a user with [**Manage Membership**](/terraform/enterprise/users-teams-organizations/permissions/organization#manage-membership) permissions enabled must [invite you to join their organization](/terraform/enterprise/users-teams-organizations/organizations#invite-users-to-the-organization) and [add you to one or more teams](/terraform/enterprise/users-teams-organizations/teams/manage#manage-team-membership).
>>>>>>> tfe-release/2.0.0
...
<<<<<<< HEAD
Admin permissions are distinct from normal organization-level permissions and apply to a different set of UI controls and API endpoints. Admin users can administer any resource across the instance when using the site admin pages or the [admin API](/terraform/enterprise/api-docs/admin), but they have normal user permissions when using an organization's standard UI controls and API endpoints. These normal user permissions are determined by team membership.

Admin users can also [designate site auditors](/terraform/enteprise/application-administration/resources#assign-site-auditor-permission) for the entire instance. The site auditor permission gives users read-only access to all Terraform Enterprise resources so that they can ensure compliance with standards your company may be subject to.  
=======
...
=======
We recommend creating tokens that are valid for the shortest duration possible. Refer to [API Token Expiration](/terraform/enterprise/users-teams-organizations/api-tokens#token-expiration) for details.
>>>>>>> tfe-release/2.0.0
...
=======
3.  Choose the token's expiration date or time. The UI displays a token's expiration date and time in your current time zone.
>>>>>>> tfe-release/2.0.0
...
User tokens can be disabled at the organization level, for more information refer to [User API tokens](/terraform/enterprise/users-teams-organizations/api-tokens#user-api-tokens).
...
<<<<<<< HEAD
This does not grant HCP Terraform access to repositories.
=======
