...
last_modified: 2026-03-31T02:01:02.000Z
...
5.  You can enable the **Disable overrides** option to prevent project and workspace managers from overriding the key. Refer to [Disable overrides for project tags](#disable-overrides-for-project-tags) for additional information.<!-- BEGIN: TFC:only name:reg-artifact-tags -->
6.  Enable the **Registry environment tag** option if you want to define values  that you can use to tag module and provider versions in the private registry. You can only enable this option for one reserved tag key per organization. <!-- Controlling registry artifact usage is in private beta Uncomment when this functionality is available as public beta or GA Refer to [Control access to registry artifacts](#control-access-to-registry-artifacts) for more information.--> <!-- END: TFC:only name:reg-artifact-tags -->
7.  Click **Save** to finish adding the key.
