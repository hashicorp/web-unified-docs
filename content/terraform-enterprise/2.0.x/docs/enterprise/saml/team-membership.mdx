...
When [SCIM provisioning](/terraform/enterprise/scim) is enabled, Terraform Enterprise ignores the team membership information in SAML assertions. This page applies only when SAML, not SCIM, manages team membership. Refer to [Link SCIM groups to teams](/terraform/enterprise/scim/manage/team-mapping) for information on managing team membership with SCIM.

## Configure Team Membership Mapping

Enable the **Use SAML to manage team memberships** option on the site administration page to allow the SAML identity provider to control team membership. This option is effective only when SCIM is not enabled.
...
When SCIM is enabled, Terraform Enterprise does not use SAML attributes or the **Site Admin Role** setting to provision site administrators. Use the configured SCIM site admin group instead.

If the **Site Admin Role** setting in [the SAML settings](/terraform/enterprise/saml/configuration) is enabled and SCIM is not enabled, Terraform Enterprise grants [site administrator](/terraform/enterprise/application-administration) access to users whose team attribute contains that role name. The default role name is `site-admins`.
