...
Before working with the runs or configuration versions APIs, read the [API-driven run workflow](/terraform/enterprise/workspaces/run/api) page. The page provides a full overview of the workflow and walks through a simplified implementation.
...
You need read runs permission to list and view configuration versions for a workspace, and you need queue plans permission to create new configuration versions. Refer to the [permissions](/terraform/enterprise/users-teams-organizations/permissions/workspace) documentation for more details.
