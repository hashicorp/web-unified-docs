...
last_modified: 2026-04-10T18:35:29.000Z
...
To learn how to create and delete groups in HCP, refer to [Create a group](/hcp/docs/hcp/iam/groups#create-a-group). To assign groups permissions in HCP Terraform, refer to the [Member roles API](/terraform/cloud-docs/api-docs/group-member-roles).
...
Users must be invited to join organizations before they can be added to teams. See [the Organization Memberships API documentation](/terraform/enterprise/api-docs/organization-memberships) for more information. Invited users who have not yet accepted will not appear in Teams API responses.
...
The `scim-linked`, `scim-group-name`, `scim-sync-paused`, and `scim-updated-at` attributes are only returned when SCIM is enabled. Refer to [Team attributes for SCIM-linked teams](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#team-attributes-for-scim-linked-teams).
  ...
  "data": [
    {
      ...
      "attributes": {
        ...
        "scim-linked": true,
        "scim-group-name": "Engineering",
        "scim-sync-paused": false,
        "scim-updated-at": "2025-06-17T18:35:29.000Z",
        "permissions": {
        ...
        "organization-access": {
          ...
          "delegate-policy-overrides": false,
...
-> **Note:** In Terraform Enterprise, updating the `name` of a team that is linked to a SCIM group returns `422 Unprocessable Entity`. Requests that include `sso-team-id` return `200 OK`, but Terraform Enterprise ignores changes to `sso-team-id` while the team remains linked to a SCIM group. Refer to [Restrictions for SCIM-linked teams](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#restrictions-for-scim-linked-teams).
...
In Terraform Enterprise, deleting a team that is linked to a SCIM group returns `404 Not Found`. Refer to [Restrictions for SCIM-linked teams](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#restrictions-for-scim-linked-teams). To delete the team, first [remove its SCIM group mapping](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#remove-a-scim-group-mapping-from-a-team).
