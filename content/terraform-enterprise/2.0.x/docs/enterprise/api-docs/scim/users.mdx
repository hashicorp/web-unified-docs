...
- `Add` on single-valued supported attributes, which Terraform Enterprise treats the same as `Replace`.
- `Replace` without a `path`, where `value` is an object containing one or more supported attributes.
