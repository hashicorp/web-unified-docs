...
<<<<<<< HEAD
| `sanitized-state-download-url`   | A URL to which you can download state data with sensitive values redacted. This URL is only available for workspaces configured to encrypt HCP Terraform state and plan data with a key that you provide and control. This functionality is only available in HCP Terraform. Refer to  [Encrypt your state and plan files](/terraform/enterprise/hold-your-own-key).                                                                          |
=======
| `sanitized-state-download-url`   | A URL to which you can download state data with sensitive values redacted. This URL is only available for workspaces using [hold your own key](/terraform/enterprise/hold-your-own-key) encryption.                                                                          |
>>>>>>> tfe-release/2.0.0
...
| `hyok-encrypted-data-key`        | A reference to the HYOK encrypted data key used to secure this state version. Hold your own key is only available in HCP Terraform. Refer to [Encrypt your state and plan files](/terraform/enterprise/hold-your-own-key) for more information.                              |
