...
<<<<<<< HEAD
You can establish connections between one or more workspaces in your organization and use the `run-triggers` endpoint to allow Terraform Enterprise to automatically queue runs in your workspace when a successful run apply operation is performed in any of the source workspaces. You can connect each workspace to up to 20 source workspaces.
=======
You can establish connections between one or more workspaces in your organization and use the `run-triggers` endpoint to allow HCP Terraform to automatically queue runs in your workspace when a successful run apply operation is performed in any of the source workspaces. You can connect each workspace to up to 20 source workspaces.
>>>>>>> tfe-release/2.0.0
