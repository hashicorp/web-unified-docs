...
The no-code provisioning API allows you to create, configure, and deploy Terraform modules as workspaces in the no-code provisioning workflows within HCP Terraform. For more information on no-code modules, refer to [Designing No-Code Ready Modules](/terraform/enterprise/workspaces/no-code-provisioning/module-design).
...
2.  The module must meet the [design requirements](/terraform/enterprise/workspaces/no-code-provisioning/module-design#requirements) for a no-code module.
...
Returns the details of a no-code workspace update run, including the run's current state, such as `pending`, `fetching`, `planning`, `planned`, or `cost_estimated`. Refer to [Run States and Stages](/terraform/enterprise/workspaces/run/states) for more information on the states a run can return.
