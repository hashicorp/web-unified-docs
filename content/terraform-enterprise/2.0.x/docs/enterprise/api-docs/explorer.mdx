---
page_title: /explorer API reference for Terraform Enterprise
description: >-
  Use the Terraform Enterprise API's `/explorer` endpoint to query, filter,
  save, and sort data about resources, modules, and providers across your
  workspaces and projects.
source: terraform-docs-common
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-04-06T19:37:05Z
last_modified: 2026-06-09T18:37:13.000Z
...
[speculative plans]: /terraform/enterprise/workspaces/run/remote-operations#speculative-plans
...
workspaces" permission, teams with the "Read all projects" permission, and the [organization API token](/terraform/enterprise/users-teams-organizations/api-tokens#organization-api-tokens).

Explorer queries time out after 60 seconds. If the desired query is timing
out, try a simpler query that uses fewer filters.

Requests to the explorer are subject to [rate
limiting](/terraform/enterprise/api-docs#rate-limiting). Explorer returns a
...
| Parameter      | Description |
| -------------- | ----------------------------------------------------------------------------- |
| `type`         | **Required.** Must be one of the following available views: `workspaces`, `tf_versions`, `providers`, `modules`, or `resources`. See the [View Types](#view-types) section for more information about each. |
...
| `fields`       | Optionally limit the data returned only to the specified fields. The field names supplied must use snake case and be submitted in comma-separated format. If omitted, all available fields are returned for the requested view type. |
| `page[number]` | Optional. If omitted, the endpoint returns the first page. |
| `page[size]`   | Optional. If omitted, the endpoint uses a default page size. |
...
| `resources`   | Information about the Terraform resources tracked across the target organization. Each row returned represents a single resource. This type is in beta. See [View Type: resources](#view-type-resources) for available fields. |
...
| `resource_count`                  | `number`   | The total number of resources tracked in this workspace's current state. |
...
| `tags`                            | `string` or `null`   | A comma separated list of key-value pair tags applied to this workspace. Returns `null` when no tags are applied. |
...
| Field              | Type     | Description |
| ------------------ | -------- | ------------------------------------------------------------------ |
| `unique_identifier` | `string` | A unique string identifier for this Terraform version entry. |
...
| `registry_type`   | `string` | The registry type for this provider, such as `public` or `private`. |
| `curated`         | `boolean`| Returns `true` if this provider is curated and `false` otherwise. |
...
| `registry_type`   | `string`  | The registry type for this module, such as `public` or `private`. |
| `curated`         | `boolean` | Returns `true` if this module is curated and `false` otherwise. |
...
-> **Note**: The `resources` view type is in beta and subject to change without notice.

| Field                      | Type       | Description                                                               |
| -------------------------- | ---------- | ------------------------------------------------------------------------- |
| `name`                     | `string`   | The name of this resource. |
| `address`                  | `string`   | The address of this resource. |
| `provider_type`            | `string`   | The resource type identifier as defined by the provider (for example, `aws_instance`, `random_pet`). |
| `provider_name`            | `string`   | The source address of the provider for this resource. For providers from the public registry, the `registry.terraform.io/` hostname is omitted (for example, `hashicorp/aws`). For built-in providers, the full address is returned (for example, `terraform.io/builtin/terraform`). |
| `module_name`              | `string \| null`   | The name of the module for this resource, if available. Returns `null` for top-level resources not inside a module. |
| `terraform_version`        | `string`   | The Terraform version associated with this resource. |
| `billable_rum`             | `boolean`  | True if this resource is a billable resource under management (RUM), false otherwise. |
| `source_type`              | `string`   | The origin of the state data for this resource. `run` indicates state was produced by a HCP Terraform-managed run. `local` indicates the workspace uses [local execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode), where runs are managed outside of HCP Terraform. |
| `source_id`                | `string`   | The ID of the run that produced the current state for this resource. Returns `null` when `source_type` is `local`. |
| `source_updated_at`        | `datetime` | The time this resource's state was last updated. When `source_type` is `run`, this is the time the run was applied. When `source_type` is `local`, this is the time the state version was last updated. |
| `workspace_name`           | `string`   | The name of the workspace this resource belongs to. |
| `workspace_external_id`    | `string`   | The external ID of the workspace this resource belongs to. |
| `project_name`             | `string`   | The name of the project this resource belongs to. |
| `project_external_id`      | `string`   | The external ID of the project this resource belongs to. |
| `organization_external_id` | `string`   | The external ID of the organization this resource belongs to. |
...
| `is_before`        | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. You must use ISO 8601 formatted timestamps for filter values. If you do not provide a time zone offset, the platform interprets the filter value as UTC. |
| `is_after`         | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. You must use ISO 8601 formatted timestamps for filter values. If you do not provide a time zone offset, the platform interprets the filter value as UTC. |
...
The following example call returns data for all workspaces:
...
The following example call returns data for all workspaces with `test` in their name:
...
The following example call returns data for all workspaces with the tag key `billing` having value `tier2`:
...
The following example call returns modules used across all workspaces:
...
The following example call returns public modules only:
...
The following example call returns all resources:

```shell
curl \
  --header "Authorization: Bearer $TOKEN" \
  'https://app.terraform.io/api/v2/organizations/$ORG_NAME/explorer?type=resources'
```

The following example call returns only billable RUM resources:

```shell
curl \
  --header "Authorization: Bearer $TOKEN" \
  'https://app.terraform.io/api/v2/organizations/$ORG_NAME/explorer?type=resources&filter%5B0%5D%5Bbillable_rum%5D%5Bis%5D%5B0%5D=true'
```

**View Type: `provider`**

The following example call returns providers used across all workspaces:
...
The following example call returns the most used providers across all workspaces:
...
The following example call returns Terraform versions used across all workspaces:
...
The following example call returns all Terraform versions used in more than 10 workspaces:
...
The following example shows the response for all workspaces:
...
```csv
all_checks_succeeded,current_rum_count,checks_errored,checks_failed,checks_passed,checks_unknown,current_run_applied_at,current_run_external_id,current_run_status,drifted,external_id,module_count,modules,organization_name,project_external_id,project_name,provider_count,providers,resource_count,resources_drifted,resources_undrifted,state_version_terraform_version,vcs_repo_identifier,workspace_created_at,workspace_name,workspace_terraform_version,workspace_updated_at
"true","0","0","0","0","0","","run-rdoEKh2Gy3K6SbCZ","planned_and_finished","false","ws-j2sAeWRxou1b5HYf","0","","acme-corp","prj-V61QLE8tvs4NvVZG","Default Project","0","","0","0","0","1.14.4","","2023-10-17T21:56:45+00:00","payments-service","1.14.4","2023-12-13T15:48:16+00:00"
"true","0","0","0","0","0","2023-08-18T16:24:59+00:00","run-9MmJaoQTvDCh5wUa","applied","true","ws-igUVNQSYVXRkhYuo","0","","acme-corp","prj-uU2xNqGeG86U9WgT","web","0","","6","3","3","1.14.4","acmecorp/api","2023-04-25T17:09:14+00:00","api-service","1.14.4","2023-12-13T15:29:03+00:00"
```

## List saved views

Use this endpoint to display all of the explorer's [saved views](/terraform/enterprise/workspaces/explorer#saved-views) in an organization.
...
Use this endpoint to create a [saved view](/terraform/enterprise/workspaces/explorer#saved-views) in the explorer.
...
HCP Terraform and Terraform Enterprise do not save query results or query history.
Returning to them re-runs the query and shows current results only.
...
```csv
all_checks_succeeded,checks_errored,checks_failed,checks_passed,checks_unknown,current_run_applied_at,current_run_external_id,current_run_status,drifted,external_id,module_count,modules,organization_name,project_external_id,project_name,provider_count,providers,resource_count,resources_drifted,resources_undrifted,source_module_id,state_version_terraform_version,vcs_repo_identifier,workspace_created_at,workspace_name,workspace_terraform_version,workspace_updated_at
"true","0","0","0","0","","run-rdoEKh2Gy3K6SbCZ","planned_and_finished","false","ws-j2sAeWRxou1b5HYf","0","","acme-corp","prj-V61QLE8tvs4NvVZG","Default Project","0","","0","0","0","private/acmecorp/rds/aws/1.0.2","1.14.4","","2023-10-17T21:56:45+00:00","candy distribution service","1.14.4","2023-12-13T15:48:16+00:00"
"true","0","0","0","0","2023-08-18T16:24:59+00:00","run-9MmJaoQTvDCh5wUa","applied","true","ws-igUVNQSYVXRkhYuo","0","","acme-corp","prj-uU2xNqGeG86U9WgT","web","0","","6","3","3",null,"1.14.4","acmecorp/api","2023-04-25T17:09:14+00:00","api-service","1.14.4","2023-12-13T15:29:03+00:00"
```
