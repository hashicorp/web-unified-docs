...
last_modified: 2026-06-08T23:44:56.000Z
...
| `resources`   | **Beta.** Information about the Terraform resources tracked across the target organization. Each row returned represents a single resource. See [View Type: resources](#view-type-resources) for available fields. |
...
| `tags`                            | `string \| null`   | A comma separated list of tags (keys and key:value pairs) applied to this workspace. Returns `null` when no tags are applied. |
...
| `registry_type`   | `string` | The registry type for this provider (for example, `public` or `private`). |
| `curated`         | `boolean`| True if this provider is curated, false otherwise. |
...
| `registry_type`   | `string`  | The registry type for this module (for example, `public` or `private`). |
| `curated`         | `boolean` | True if this module is curated, false otherwise. |
...
| `is_before`        | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. ISO 8601 formatted timestamps are required for filter values. If no time zone offset is provided, the filter value will be interpreted as UTC. |
| `is_after`         | `datetime`                    | Filters data to rows where the field value is earlier than the filter value. ISO 8601 formatted timestamps are required for filter values. If no time zone offset is provided, the filter value will be interpreted as UTC. |
...
_Show only billable RUM resources_
