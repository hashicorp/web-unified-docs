...
last_modified: 2026-04-08T18:54:14.000Z
...
| `current_rum_count`               | `number` or `null` | The number of <!-- BEGIN: TFC:only name:rum-link -->[resources under management](/terraform/cloud-docs/overview#what-is-a-managed-resource) (RUM). HCP Terraform <!-- END: TFC:only name:rum-link --><!-- BEGIN: TFEnterprise:only name:rum-link -->resources under management (RUM). Terraform Enterprise <!-- END: TFEnterprise:only name:rum-link -->returns `null` when the count is unknown, including while it calculates the count. A workspace may contain resources even when `current_rum_count` returns `null`. |
...
The `current_rum_count` field returns empty when HCP Terraform has not yet
determined the workspace's RUM count. This can occur while HCP Terraform
calculates the count. The workspace may contain resources, even when the field shows zero
resources.
