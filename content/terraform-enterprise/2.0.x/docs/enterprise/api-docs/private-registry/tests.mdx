...
last_modified: 2026-04-08T18:54:26.000Z
>>>>>>> tfe-release/2.0.0
...
When [dynamic credentials](/terraform/enterprise/registry/test/dynamic-credentials) are enabled for a module's tests, test runs include OIDC token information. The following attributes track OIDC token usage:
...
<<<<<<< HEAD
The API never returns the actual OIDC token. These attributes only provide metadata about token generation and expiration for auditing purposes.
=======
<Note>
...
-   [Test Configuration API](/terraform/enterprise/api-docs/private-registry/test-configuration)
-   [Dynamic credentials for module testing](/terraform/enterprise/registry/test/dynamic-credentials)
