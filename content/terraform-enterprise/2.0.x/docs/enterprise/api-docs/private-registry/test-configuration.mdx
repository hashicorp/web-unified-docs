...
last_modified: 2026-04-08T18:54:25.000Z
...
| `oidc-provider`      | OIDC provider type. Accepted values: <ul><li>`aws`: Amazon Web Services</li><li>`gcp`: Google Cloud Platform</li><li>`azure`: Microsoft Azure</li><li>`vault`: HashiCorp Vault</li></ul>                                                                      | String    | Required when `oidc-enabled` is `true` |
| `oidc-configuration` | Provider-specific configuration. Refer to: <ul><li>[AWS configuration](#aws-configuration)</li><li>[GPC configuration](#gpc-configuration)</li><li>[Azure configuration](#azure-configuration)</li><li>[Vault configuration](#vault-configuration)</li></ul> | Object    | Required when `oidc-enabled` is `true` |
...
Configure the default token time-to-live (TTL) for module test runs at the organization level using the [Organizations API](/terraform/enterprise/api-docs/organizations).
