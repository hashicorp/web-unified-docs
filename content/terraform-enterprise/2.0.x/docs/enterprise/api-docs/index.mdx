...
last_modified: 2026-04-08T18:54:16.000Z
>>>>>>> tfe-release/2.0.0
...
-   [`/api/v1/support/bundle-requests`](/terraform/enterprise/api-docs/support-bundle). Call this endpoint to generate, retrieve, list and delete support bundles.
-   [`/api/v1/diagnostics`](/terraform/enterprise/api-docs/diagnostics). Call this endpoint to perform comprehensive health diagnostics of your Terraform Enterprise instance.
<<<<<<< HEAD
...
-   `policy-enforcement` — Allows an organization to use [Sentinel](/terraform/enterprise/policy-enforcement/define-policies/custom-sentinel). Affects the [policies][], [policy sets][], and [policy checks][] endpoints.
...
-   `private-policy-agents` - Allows an organization to ensure that HTTP enabled [Sentinel](/terraform/enterprise/policy-enforcement/define-policies/custom-sentinel) and OPA [policies][] can communicate with isolated, private, or on-premises infrastructure.
...
[member roles]: /terraform/enterprise/api-docs/member-roles
