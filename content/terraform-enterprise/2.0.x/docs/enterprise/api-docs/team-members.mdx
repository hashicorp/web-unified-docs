...
-> **Note:** Team management is available in HCP Terraform **Essentials**, **Standard**, and **Premium** editions. Free organizations can also use this API, but can only manage membership of their owners team. Refer to the [Terraform pricing page](https://www.hashicorp.com/en/pricing?tab=terraform) for more information.
...
In Terraform Enterprise, if a team is linked to a SCIM group, requests to the endpoints on this page return `403 Forbidden` because team membership is managed by SCIM. Refer to [Restrictions for SCIM-linked teams](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#restrictions-for-scim-linked-teams). To manage membership directly again, [remove the team's SCIM group mapping](/terraform/enterprise/api-docs/admin/team-scim-group-mapping#remove-a-scim-group-mapping-from-a-team).
