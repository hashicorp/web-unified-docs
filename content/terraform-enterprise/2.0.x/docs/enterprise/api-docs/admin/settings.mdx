...
- `/admin/saml-settings`
- [`/admin/scim-settings`](/terraform/enterprise/api-docs/admin/scim-settings)
- `/admin/smtp-settings`
...
Refer to [SCIM Settings API](/terraform/enterprise/api-docs/admin/scim-settings) for endpoints to enable, update, and disable SCIM provisioning settings.
...
This endpoint rotates the OIDC signing key used for signing tokens issued for [dynamic provider credentials](/terraform/enterprise/dynamic-provider-credentials). This key automatically rotates every 90 days, hitting this endpoint resets this timer. Hitting this endpoint has _no effect_ on the next trim time. This endpoint should only be used if a leak of the key is suspected.
