...
The Site Auditor role provides read-only, cross-organization visibility into all resources in a Terraform Enterprise instance, enabling users to perform comprehensive platform-wide reviews without requiring write access or organization-level membership.
Site Auditors can access read-only standard and admin API endpoints but cannot modify resources, access state files, or be service accounts. 
