...
last_modified: 2026-04-10T18:23:23.000Z
...
The Site Auditor role provides read-only, cross-organization visibility into all resources in a Terraform Enterprise instance, which lets users perform platform-wide reviews without organization-level membership.
The `is-site-auditor` attribute is separate from `is-admin`. A user with only the Site Auditor role has read-only access and cannot modify resources or access state files. A user can also hold admin privileges at the same time, so a sample response may show both attributes set to `true`.
Site Auditors cannot be service accounts.

| Status  | Response                                | Reason                                                                         |
|---------|-----------------------------------------|--------------------------------------------------------------------------------|
| [200][] | [JSON API document][] (`type: "users"`) | Successfully granted site auditor privileges.                                  |
| [400][] | [JSON API error object][]               | Service accounts cannot be granted the site auditor role.                      |
