...
last_modified: 2026-04-08T18:54:44.000Z
...
3.  Perform the following actions in your Microsoft Entra admin center:
    ...
    2.  Provide the HCP Terraform or Terraform Enterprise callback URL.
    3.  Retrieve the Microsoft Entra application ID, tenant ID, and tenant key.
...
3.  Complete the following fields and enable the following options with the corresponding values currently displayed in your HCP Terraform or Terraform Enterprise browser tab:
...
2.  Enter your Azure DevOps Services application's **Application (client) ID**, **Directory (tenant) ID** and **Client Secret**. Get these values from the application's details in the open browser tab described in [Create a new Microsoft Entra application](#create-a-new-microsoft-entra-application).
...
After configuring Azure DevOps Services access for HCP Terraform<!-- BEGIN: TFEnterprise:only name:product-name --> or Terraform Enterprise<!-- END: TFEnterprise:only name:product-name -->, you can create workspaces or Stacks based on your organization's repositories.
