...
-> **Note:** Alternately, you can skip the OAuth configuration process and authenticate with a personal access token. This requires using HCP Terraform's API. For details, see [the OAuth Clients API page](/terraform/enterprise/api-docs/oauth-clients). GitHub Enterprise Cloud uses this same GitHub.com connection, because it is hosted at `github.com`. Only self-hosted GitHub Enterprise Server uses the separate [GitHub Enterprise instructions](/terraform/enterprise/vcs/github-enterprise).
