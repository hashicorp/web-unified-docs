...
You can view all active SCIM tokens from the SCIM settings page:
...
Terraform Enterprise does not send alerts before a SCIM token expires. When a token has already expired, Terraform Enterprise surfaces that status in the following places:
...
The token list also shows an expiration badge for each token, including whether the token is expired or when it will expire.
