...
last_modified: 2026-09-04T14:33:49.000Z
...
### `TFE_RUN_PIPELINE_DOCKER_REGISTRY_AUTH_USERNAME`

If the container image used to execute Terraform runs is stored in a private registry,
provide the username to access the image using basic authentication.

### `TFE_RUN_PIPELINE_DOCKER_REGISTRY_AUTH_PASSWORD`

If the container image used to execute Terraform runs is stored in a private registry,
provide the password to access the image using basic authentication.
...
Can be used together with `TFE_DATABASE_USE_MTLS`. Not required when using passwordless authentication.
...
### `TFE_DATABASE_PASSWORDLESS_AWS_USE_INSTANCE_PROFILE`

Whether to use Identity and Access Management (IAM) for database authentication. 
Defaults to `false`.

### `TFE_DATABASE_PASSWORDLESS_AWS_REGION`

AWS Region of the RDS PostgreSQL resource. Defaults to `""`.
...
### `TFE_DATABASE_PASSWORDLESS_GOOGLE_USE_DEFAULT_CREDENTIALS`

Whether to use Default Credentials for database authentication.
Defaults to `false`.

### `TFE_DATABASE_EXTRA_SCHEMAS`

A comma-separated list of additional PostgreSQL schemas to append to the database search path. Use this setting when PostgreSQL extensions required by Terraform Enterprise are installed in a schema other than `rails`, `vault`, `registry`, `task_worker`, `terraform-enterprise` and `public` schemas.
...
The PostgreSQL server for the Explorer database in the format `HOST[:PORT]` (e.g.,
`db.example.com` or `db.example.com:5432`). If only `HOST` is provided then the
`:PORT` defaults to `:5432`.

Setting this variable enables the Explorer feature. Required to use Explorer.

### `TFE_EXPLORER_DATABASE_NAME`

Name of the PostgreSQL database to store Explorer data.

Required to use Explorer.

### `TFE_EXPLORER_DATABASE_USER`

PostgreSQL user for the Explorer database.
...
Not required when using passwordless authentication.

### `TFE_EXPLORER_DATABASE_PARAMETERS`

PostgreSQL server parameters for the Explorer connection URI. Used to configure the
PostgreSQL connection (e.g., `sslmode=require`).
...
Must be `true` or `false`. `true` indicates to use mutual TLS (mTLS) authentication for clients to access Redis with Redis standalone or Sentinel. Defaults to `false`. Can be used on its own or in combination with `TFE_REDIS_USER` and `TFE_REDIS_PASSWORD` when `TFE_REDIS_USE_AUTH` is `true`.
...
Redis server user. This value is required when `TFE_REDIS_USE_AUTH` is set to `true`, and Redis server is configured to use username for authentication. There is no need to set this config when running Redis server internally as credentials are not required. Can be used together with `TFE_REDIS_USE_MTLS` when `TFE_REDIS_USE_AUTH` is also `true`.

### `TFE_REDIS_PASSWORD`

Redis server password. This value is required when `TFE_REDIS_USE_AUTH` is set to `true`. There is no need to set this config when running Redis server internally as credentials are not required. Can be used together with `TFE_REDIS_USE_MTLS` when `TFE_REDIS_USE_AUTH` is also `true`.
...
Redis server user. This value is required when `TFE_REDIS_SIDEKIQ_USE_AUTH` is set to `true`, and Redis server is configured to use username for authentication. There is no need to set this config when running Redis server internally as credentials are not required.

### `TFE_REDIS_SIDEKIQ_PASSWORD`

Redis server password. This value is required when `TFE_REDIS_SIDEKIQ_USE_AUTH` is set to `true`. There is no need to set this config when running Redis server internally as credentials are not required.
...
### `TFE_REDIS_PASSWORDLESS_AZURE_CLIENT_ID`

Client ID of a user-assigned Managed Service Identity. Leave blank to use the system-assigned Managed Service Identity. Defaults to `""`.

### `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AZURE_CLIENT_ID`

Client ID of a user-assigned Managed Service Identity. Leave blank to use the system-assigned Managed Service Identity. Defaults to `""`.

### `TFE_REDIS_PASSWORDLESS_AWS_USE_INSTANCE_PROFILE`

Whether to use Identity and Access Management (IAM) for Redis authentication. Defaults to `false`.

### `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AWS_USE_INSTANCE_PROFILE`

Whether to use Identity and Access Management (IAM) for Redis authentication used by Sidekiq. Defaults to `false`.

### `TFE_REDIS_PASSWORDLESS_AWS_REGION`

AWS Region of the AWS ElastiCache resource. Defaults to `""`.

### `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AWS_REGION`

AWS Region of the AWS ElastiCache resource. Defaults to `""`.

### `TFE_REDIS_PASSWORDLESS_AWS_HOST_NAME`

The name of the Redis instance on AWS. Defaults to `""`.

### `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AWS_HOST_NAME`
...
Disables the admin console web-based administrative interface for managing and monitoring Terraform Enterprise deployments.
...
Sets Terraform Enterprise application log verbosity to `trace`, `debug`,
`info`, `warn`, `error`, or `off` (case-insensitive). Defaults to `info`.

For other logs, use [`TFC_AGENT_LOG_LEVEL`](/terraform/cloud-docs/agents/logging#log-verbosity-and-levels)
for HCP Terraform Agents or [`TF_LOG`, `TF_LOG_CORE`, and
`TF_LOG_PROVIDER`](/terraform/internals/debugging) for Terraform runs.
`TFE_LOG_FORWARDING_ENABLED` and `TFE_LOG_FORWARDING_CONFIG_PATH` configure log
forwarding, not verbosity.
...
#### `TFE_OBJECT_STORAGE_AZURE_UPLOAD_PART_SIZE`
Optional configuration of the size of the data packets sent to Azure Storage. Defaults to 1M if left blank.

#### `TFE_OBJECT_STORAGE_AZURE_UPLOAD_CONCURRENCY`
Optional configuration of the number of simultaneous uploads to Azure Storage. Defaults to 1 if left blank.
...
#### `TFE_OBJECT_STORAGE_S3_ENABLE_MD5_VALIDATION`

When set to `true`, enables MD5 checksums in place of more advanced data integrity options not yet supported by all 3rd party S3 storage solutions.


## Startup checks

### `TFE_STARTUP_CHECKS_IGNORE_FAILURES`

Comma-separated list of startup check groups whose failures should be ignored. When you include a check group in this list, Terraform Enterprise ignores any failures produced and continues startup.

Use this setting sparingly as ignoring the failure of a startup check may impact the normal operations of Terraform Enterprise. A common use case is ignoring the `tls` check group when rotating an expired CA certificate in a shared CA bundle.

Terraform Enterprise recognizes the following group names. Group names are not case-sensitive, and Terraform Enterprise removes surrounding whitespace and ignores unknown group names.
...
The following checks are only used for diagnostics and do not prevent Terraform Enterprise from starting:
...
This setting does not affect the `preupgrade-check` tool and runtime diagnostics. They continue to report failures normally.

Refer to the [Startup checks reference](/terraform/enterprise/deploy/reference/startup-checks) documentation for more information.
