...
**Not recommended for general use.** Comma-separated list of [startup check](/terraform/enterprise/deploy/reference/startup-checks) groups whose failures should be ignored. When a check group is listed here, any failure produced by that check at startup is logged at `WARN` and does **not** prevent Terraform Enterprise from starting. The check still ran and still failed; Terraform Enterprise simply continues startup instead of aborting.
...
- `redis`
- `cloud`
- `archivist` (only when `TFE_OBJECT_STORAGE_TYPE=s3`)
- `explorer_database` (only when an explorer database is configured)
...
Unknown values are silently ignored. Diagnostic-only checks (`atlas`, `vault`, `task-worker`, `fluent-bit`) are unaffected because they do not gate startup. This setting only affects the startup check phase — the `preupgrade-check` tool and runtime diagnostics continue to report failures normally.

**Risks of ignoring failures:**

- A missing or invalid `license` still prevents Terraform Enterprise from operating at runtime.
- `database` or `redis` failures typically cause downstream errors during startup or normal operation.

Use sparingly. A common case is an expired CA certificate in a shared CA bundle that you intend to rotate without taking the application offline.
