...
Must be `true` or `false`. `true` indicates to use mutual TLS (mTLS) authentication for clients to access Redis with Redis standalone or Sentinel. Defaults to `false`. Can not be `true` if `TFE_REDIS_USER` and `TFE_REDIS_PASSWORD` is specified.
...
Redis server user. This value is required when `TFE_REDIS_USE_AUTH` is set to `true`, and Redis server is configured to use username for authentication. There is no need to set this config when running Redis server internally as credential is not required. Can not be used if `TFE_REDIS_USE_MTLS` is set to `true`.

### `TFE_REDIS_PASSWORD`

Redis server password. This value is required when `TFE_REDIS_USE_AUTH` is set to `true`. There is no need to set this config when running Redis server internally as credential is not required. Can not be used if `TFE_REDIS_USE_MTLS` is set to `true`.
