...
last_modified: 2026-09-04T14:55:37.000Z
...
Controls Terraform Enterprise application log verbosity. Accepted values are
`trace`, `debug`, `info`, `warn`, `error`, and `off`. Values are
case-insensitive. Defaults to `info`.

For HCP Terraform Agent logs, use
[`TFC_AGENT_LOG_LEVEL`](/terraform/cloud-docs/agents/agents#log-level). For
Terraform run logs, use [`TF_LOG`, `TF_LOG_CORE`, and
`TF_LOG_PROVIDER`](/terraform/internals/debugging).
