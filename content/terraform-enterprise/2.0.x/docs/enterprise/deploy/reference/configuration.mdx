...
This setting should be used sparingly as ignoring the failure of a startup check may impact the normal operations of Terraform Enterprise. A common case to ignore the `tls` check group is an expired CA certificate in a shared CA bundle that you intend to rotate eventually.
