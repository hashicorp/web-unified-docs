...
By default, Terraform Enterprise aborts startup if any startup check fails. You can configure
Terraform Enterprise to continue startup when specific check groups fail by setting
...
to a comma-separated list of group names. Terraform Enterprise still runs each check. When a
check in a listed group fails, Terraform Enterprise:
...
For example, if you set the following configuration, Terraform Enterprise tolerates failures in
the `tls` check group:
...
When the `tls` startup check fails, Terraform Enterprise logs a warning similar to the following
message and continues startup:
