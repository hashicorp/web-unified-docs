...
You can configure Terraform Enterprise to tolerate failures from specific
check groups by setting [`TFE_STARTUP_CHECKS_IGNORE_FAILURES`](/terraform/enterprise/deploy/reference/configuration#tfe_startup_checks_ignore_failures).
When a group is listed there, its failures are logged at `WARN` (as `failed but ignored`)
and Terraform Enterprise continues startup. See [Tolerating startup check failures](#tolerating-startup-check-failures)
below for the recognized group names and operational guidance.
...
* The Postgres database version is a value other than `13`, `14`, `15`, `16`, or `17`.
...
## Tolerating startup check failures

By default, Terraform Enterprise aborts startup if any startup check fails. You can opt in to tolerating
failures from specific check groups by setting
[`TFE_STARTUP_CHECKS_IGNORE_FAILURES`](/terraform/enterprise/deploy/reference/configuration#tfe_startup_checks_ignore_failures)
to a comma-separated list of group names. When a check in a listed group fails, Terraform Enterprise:

- Still runs the check.
- Logs the failure at `WARN` with a `failed but ignored` message instead of aborting.
- Continues with the remaining startup sequence.

```
2023-06-30T18:14:45.792Z [WARN]  terraform-enterprise: check failed but ignored: name=tls duration="795.072µs" err="..."
```

The setting accepts the following group names (matched case-insensitively, surrounding whitespace ignored):
...
Unknown values in the list are silently ignored. Diagnostic-only checks (`atlas`, `vault`, `task-worker`,
`fluent-bit`) do not gate startup, so listing them has no effect. The `preupgrade-check` tool and runtime
diagnostics are not affected by this setting; they continue to report failures normally.

This setting is intended for narrow operational scenarios, such as rotating an expired CA certificate
in a shared CA bundle without taking the application offline. Tolerating other failures, such as
`license`, `database`, or `redis`, typically defers errors to runtime rather than resolving them, and
