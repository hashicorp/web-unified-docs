...
last_modified: 2026-06-03T08:15:56.000Z
...
By default, Terraform Enterprise cancels startup if any startup check fails. To configure
Terraform Enterprise to continue startup when specific check groups fail, set the
[`TFE_STARTUP_CHECKS_IGNORE_FAILURES`](/terraform/enterprise/deploy/reference/configuration#tfe_startup_checks_ignore_failures) environment variable
to a comma-separated list of group names.

Terraform Enterprise still runs each check, but when a check in a listed group fails, Terraform Enterprise logs a `failed but ignored` warning message and continues with the remaining startup sequence.

The following example configures Terraform Enterprise to tolerate failures in
the `tls` check group:

```shell-session
TFE_STARTUP_CHECKS_IGNORE_FAILURES=tls
```

If the `tls` startup check fails, Terraform Enterprise logs a warning and continues startup:

```
2023-06-30T18:14:45.792Z [WARN]  terraform-enterprise: check failed but ignored: name=tls duration="795.072µs" err="..."
```

Refer to the [`TFE_STARTUP_CHECKS_IGNORE_FAILURES` configuration reference](/terraform/enterprise/deploy/reference/configuration#tfe_startup_checks_ignore_failures) for more information on configuring startup check behavior.
...
When using username and password authentication, `TFE_DATABASE_PASSWORD` must also be set. The password is not required when using passwordless authentication, and is optional when using client certificate authentication.
...
* The database user does not have sufficient privileges. Refer to [User requirements](/terraform/enterprise/deploy/configuration/storage/connect-database/postgres#user) for the two supported permission models. The error log will identify the specific cause:
  * `database user does not have sufficient privileges: schemas not yet created: [<schemas>]` — the schemas do not exist and the user lacks database-level `CREATE` privilege.
  * `database user does not have sufficient privileges: missing USAGE or CREATE privilege on schemas: [<schemas>]` — the schemas exist but the user is missing `USAGE` or `CREATE` on one or more of them.
