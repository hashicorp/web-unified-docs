...
Complete the following prerequisites before installing Terraform Enterprise on [Nomad](/nomad). Refer to [Deploy Nomad in a production environment](/nomad/docs/deploy/production) for documentation about setting up a Nomad cluster.
...
1. A Redis data store. Refer to [Configure Redis data store connection](/terraform/enterprise/deploy/configuration/storage/connect-redis) for supported versions.

### Create the deployment configuration

Create a custom YAML configuration file, for example `/tmp/overrides.yaml`, to
override the default values. The file contains settings for the operational
mode, license, TLS certificates, and network configuration. Add any additional
configurations necessary for your environment. Refer to [Configuration file
overview](/terraform/enterprise/deploy/reference/configuration) for additional
information.
...
Refer to the [example Nomad job specification for TFE](#nomad-job-specification) for a template that you can copy and modify.
