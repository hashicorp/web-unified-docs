...
last_modified: 2026-04-10T08:51:00.000Z
...
### Using a non-default Docker Network

In case you are not using the default Docker network and you need to specify into which one to deploy Terraform Enterprise, you can utilise the `TFE_RUN_PIPELINE_DOCKER_NETWORK` variable. By default, this variable is empty and the default Docker network is targeted for the deployment.
...
services:
  tfe:
    image: images.releases.hashicorp.com/hashicorp/terraform-enterprise:<vYYYYMM-#>
    environment:
      TFE_RUN_PIPELINE_DOCKER_NETWORK: "<Docker network name>"
```

Refer to the [`TFE_RUN_PIPELINE_DOCKER_NETWORK` reference](terraform/enterprise/deploy/reference/configuration#tfe_run_pipeline_docker_network) for additional information.
