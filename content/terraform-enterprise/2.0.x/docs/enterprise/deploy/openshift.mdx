...
last_modified: 2026-06-17T01:09:33.000Z
...
1. Create an image pull secret in `<TFE_PROJECT>` to fetch the `terraform-enterprise` container from the `<DOCKER_REGISTRY_URL>`.
   - This URL can be `images.releases.hashicorp.com`, `cp.icr.io`, or your internal container registry.
   - If you are using `images.releases.hashicorp.com`, use `terraform` as the `<DOCKER_REGISTRY_USERNAME>` parameter in the following command with `--docker-password=$(cat /path/to/terraform.hclic)`
   - If you are using `cp.icr.io`, use `cp` as the `<DOCKER_REGISTRY_USERNAME>` parameter in the following command with `--docker-password=$(cat /path/to/access.key)`
...
1. Validate the readiness of the Terraform Enterprise application by querying the [readiness endpoint](/terraform/enterprise/api-docs/readiness). The endpoint returns `200 OK` when the node is ready to accept requests. The readiness endpoint reports the status of a single node, so in multi-replica deployments query an individual node directly rather than through a load balancer, which may route the request to any node.

  ```shell-session
  $ curl https://tfe.test.hashicorp.com/api/v1/health/readiness
  ```
