...
1. Ports for ingress traffic. The Terraform binary accepts traffic from the following sources:
    ...
    - metrics
    - TFE servers
1. Egress destination endpoints. To run Terraform Enterprise in an air-gapped environment, download the image from a connected system and then re-host it internally or load it directly onto the target host.
...
- You can fetch the image from a connected system, then perform one of the following tasks:
...
You should only add Amazon S3 endpoints to your allowlist when downloading the image directly from the public HashiCorp release service as part of your installation workflow. The Amazon-owned endpoints can change over time, so review this list before each direct download.
