...
1. Ports for ingress traffic. The Terraform binary accepts traffic from the following sources:
    ...
    - metrics
    - TFE servers
1. Egress destination endpoints. Airgapped deployments can download the Terraform Enterprise image from a connected system and then re-host it internally or load it directly onto the target host.
...
- `https://images.releases.hashicorp.com`: Hosts Terraform Enterprise release container images in the HashiCorp registry.
- `https://cp.icr.io`: Hosts Terraform Enterprise release container images in IBM Entitled Registry.
- `https://helm.releases.hashicorp.com`: Hosts the Helm chart for Kubernetes installation.

### Airgapped image delivery options

Use the following guidance when you install Terraform Enterprise in an airgapped environment:

- Terraform Enterprise does not require ongoing direct access from the application host to `images.releases.hashicorp.com`, `cp.icr.io`, or the Amazon S3 endpoints that back the public HashiCorp release service.
- If you pull the image directly from the public HashiCorp release registry, the download may be served from one of the following Amazon S3 endpoints:
...
- You can fetch the image from a connected system, then either:
  - re-host it in an internal container registry that the Terraform Enterprise host can access
  - load it directly into the local Docker image cache on the Terraform Enterprise host
- If your organization uses IBM Entitled Registry, use the applicable Public Availability Override (PAO) process and IBM Entitled Registry workflow to retrieve and mirror the image.
- If you need a fully offline installation workflow, use the Replicated airgap bundle described in [Interactive Terraform Enterprise Installation](/terraform/enterprise/deploy/replicated/install/interactive/installer#run-the-installer-airgapped).

Only allowlist the Amazon S3 endpoints when your installation workflow downloads the image directly from the public HashiCorp release service. The Amazon-owned endpoints can change over time, so review this list before each direct download.

### HashiCorp service APIs

Terraform Enterprise calls the following hostnames unless you have supplied a custom Terraform bundle. Refer to [Custom and Community Providers](/terraform/enterprise/workspaces/run/install-software#custom-and-community-providers)
...
- Any VCS servers and services that you plan to use.
...
- Third-party services that you intend to integrate with or manage from the Terraform Enterprise server.
...
Some systems do not support IPv6, which may affect your ability to comply with regulations required by your industry. IBM Entitled Registry, used to download images from `cp.icr.io`, does not support IPv6. The HashiCorp Registry at `images.releases.hashicorp.com` supports IPv6.

Refer to the [Terraform Enterprise deployment overview](/terraform/enterprise/deploy) for additional information.
