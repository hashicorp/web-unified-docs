...
- The `terraform_enterprise`, `rails`, `vault`, `task_worker`, and `registry` schemas are required. The database user must meet one of the following permission requirements:
  - The user has `CREATE` privilege on the database. Terraform Enterprise creates the required schemas automatically on startup.
  - All required schemas already exist and the user has `USAGE` and `CREATE` privileges on each schema. Terraform Enterprise does not need to create schemas in this case.

  `CREATE` privilege on the `public` schema alone is not sufficient and will cause the database startup check to fail.

  <Note>
  If you are changing the database user, ensure the new user is the owner of all
...
## Create extensions in a separate schema

By default, Terraform Enterprise expects PostgreSQL extensions to be available in one of these schemas `rails`, `vault`, `registry` and `task_worker`. If not found, Terraform Enterprise tries to create them in the `public` schema.
If your security posture or organizational policy requires extensions to be installed in a dedicated schema (for example, an `extensions` schema), you need to install them manually in the dedicated schema and then configure Terraform Enterprise to include that schema in the database search path.

Set the `TFE_DATABASE_EXTRA_SCHEMAS` environment variable to a comma-separated list of schema names to append to the search path. For example:

```
TFE_DATABASE_EXTRA_SCHEMAS=extensions
```

If you use a separate database for the Explorer feature and its extensions are in a different schema, set `TFE_EXPLORER_DATABASE_EXTRA_SCHEMAS` to override the Explorer search path independently. If `TFE_EXPLORER_DATABASE_EXTRA_SCHEMAS` is not set, the value of `TFE_DATABASE_EXTRA_SCHEMAS` is used for the Explorer database as well.
...
You can configure Terraform Enterprise to use PostgreSQL client certificates to authenticate with the server. When providing client certificates, the `sslmode` parameter is set to `verify-full`. You can optionally provide a database password together with client certificates by setting `TFE_DATABASE_PASSWORD`, but it is not required. 
