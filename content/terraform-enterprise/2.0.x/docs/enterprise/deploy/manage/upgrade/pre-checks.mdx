...
@include 'pull-tfe-image-docker.mdx'
...
| --- | --- | --- |
| `0` | All checks passed. The output may include warnings, such as a version-compatibility warning indicating that downtime is required to upgrade. | Clear to upgrade. Review any warnings before proceeding. |
...
Warnings do not cause a non-zero exit code. This ensures that orchestrated pre-upgrade runs, such as the Kubernetes pre-upgrade Job, complete successfully when the only findings are warnings. Review the warning text in the output to confirm that the conditions are acceptable before you upgrade.
