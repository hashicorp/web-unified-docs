...
Unless the execution mode of a workspace is set to "local", Terraform Enterprise performs [remote operations](/terraform/enterprise/workspaces/run/remote-operations), running Terraform in its own worker VMs.
