...
last_modified: 2026-04-10T08:51:01.000Z
...
Commands throughout this guide refer to `terraform-enterprise` as the container name, which is the default for Replicated installations. If your container has a different name, adjust the commands accordingly. Use the following command to find your container name:
...
1. Run the following commands to validate that the upgrade completed successfully:
   ...
   1. Run a health check.
      ...
      If this fails with an `sh: /root/ptfe-health-check: not found` message, run:
      ```shell-session
...
Creating your TLS certificates before stopping Replicated ensures a smooth migration without delays.
...
1. Copy your existing Replicated TLS certificates to the staging location:
   ...
   The output shows the certificate paths in the container. Depending on your setup, the file paths may differ from the following example:
   ```json
   {
   ...
   Use the paths from the previous step to copy your certificates from the container to the host:
   ```shell-session
...
1. Review the configuration file from the previous step. Update any values as needed before moving on to the next step. Pay special attention to placeholder values enclosed in `<>`, such as `image` and `TFE_LICENSE`, and replace them with your actual values.
   ...
   Edit the compose file using your preferred editor:
   ```shell-session
...
1. Use the `sudo tee` command to create the systemd service file. In the following example, Docker is installed at `/usr/bin/docker`. Adjust the paths in the configuration if your Docker installation differs:
...
1. Update this systemd unit file if any of the following are true:
...
1. Stop Terraform Enterprise (Replicated). Ensure the application has fully stopped before proceeding.
   ...
   ```
1. Back up your data. You can skip this step if you have already created a backup. Otherwise, complete the following steps.
   ...
   - Archive your mounted disk data, replacing `${DISK_PATH}` with the path from the previous command.
...
Finally, test that your new Terraform Enterprise installation works properly.

If you have an existing suite of release acceptance tests, you can use those instead of doing the following steps.

We recommend testing capabilities that you use in production, just as you would for a Terraform Enterprise upgrade. For example, if you are using Sentinel or run tasks in production, we recommend testing a run that includes these integrations in a non-production environment before deploying to production.
...
1. Stop and disable Terraform Enterprise (Docker).
   ```shell-session
   $ sudo systemctl disable --now terraform-enterprise
   ```
1. Start and enable Replicated.
   ```shell-session
   $ sudo systemctl enable --now replicated replicated-ui replicated-operator
   ```
...
If you have an existing suite of release acceptance tests, execute those instead of doing the following steps.
...
Redis is required to run Terraform Enterprise in Kubernetes. If your deployment is operating in [`external` mode](/terraform/enterprise/deploy/reference/configuration#external), you must deploy an external Redis server.
...
1. On the `env.secrets` and `env.variables` sections of the [overrides values file](https://github.com/hashicorp/terraform-enterprise-helm/blob/main/values.yaml), use the external services credentials from the Replicated installation for the Kubernetes installation. Specifically the following:
   ...
   1. The `TFE_ENCRYPTION_PASSWORD` value should match the Replicated installation value. You can get this from your Replicated instance by connecting over SSH and running the following command:
      ```shell-session
      $ replicatedctl app-config export --template '{{ .enc_password.Value }}'
      ```
      Refer to the [Replicated to flexible deployments configuration mapping](/terraform/enterprise/deploy/reference/configuration#replicated-to-flexible-deployments-configuration-mapping) for more information on how the Replicated configuration maps to the variables and secrets on the Terraform Enterprise Helm chart.
...
You should be able to log in to your new Terraform Enterprise installation with the credentials previously used for your Replicated installation.
...
1. Back up your Replicated configuration.
...
1. Create a yaml file based on the template for your current operational mode: 
     - [Mounted Disk operational mode Podman YAML example](/terraform/enterprise/deploy/podman#example-disk-mode-configuration).
     - [External operational mode Podman YAML example](/terraform/enterprise/deploy/podman#example-external-mode-configuration).
     - [Active/Active operational mode Podman YAML example](/terraform/enterprise/deploy/podman#example-active-active-mode-configuration).
...
Complete the following steps to verify that your new Terraform Enterprise installation works as expected.

Alternatively, you can execute your existing suite of release acceptance tests.
...
The existing version of Terraform Enterprise must be able to run on non-Replicated runtimes. Refer to [Upgrade your existing Terraform Enterprise installation](#upgrade-your-existing-terraform-enterprise-installation) for upgrade instructions.
...
1. In the Terraform Enterprise Nomad job specification, specify the following values:
   1. The `TFE_OBJECT_STORAGE_TYPE` and `TFE_OBJECT_STORAGE_*` variables should specify the object storage type and the container or bucket credentials from your Replicated installation.
   1. The `TFE_DATABASE_*` variables should specify database credentials from the Replicated installation.
   1. The `TFE_REDIS_*` values on the Nomad job specification should specify the same credentials from the external Redis in your Replicated installation.
   1. If Terraform Enterprise is connected to an external Vault server, the `TFE_VAULT_*` values on the Nomad job specification should specify the same credentials from the external Vault in your Replicated installation.
...
1. Install Terraform Enterprise on Nomad. Refer to [Deploy Terraform Enterprise to Nomad](/terraform/enterprise/deploy/nomad) for instructions.

#### Step 5: Validate migration success

Verify that your new Terraform Enterprise installation works properly. If you have an existing suite of release acceptance tests, you can use them instead of completing the following steps.
...
1. Stop the Terraform Enterprise job and purge it.

   ```shell-session
   $ nomad job stop -purge -namespace=$namespace <terraform enterprise job name>
   ```

1. Optionally, clean up Nomad variables, ACLs, and namespaces.
