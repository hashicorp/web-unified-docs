...
As an alternative to the steps outlined on this page, you can configure
dynamic credentials for AWS with the [quick setup](/terraform/enterprise/dynamic-provider-credentials/aws-configuration/quick-setup) method.
...
| `TFC_AWS_RUN_ROLE_ARN`<br />`TFC_AWS_RUN_ROLE_ARN[_TAG]`<br />`TFC_DEFAULT_AWS_RUN_ROLE_ARN`       | The ARN of the role to assume in AWS. | Requires **v1.7.0** or later if self-managing agents. Optional if `TFC_AWS_PLAN_ROLE_ARN` and `TFC_AWS_APPLY_ROLE_ARN` are both provided. Refer to [Optional environment variables](/terraform/enterprise/dynamic-provider-credentials/aws-configuration#optional-environment-variables) for more information. |
