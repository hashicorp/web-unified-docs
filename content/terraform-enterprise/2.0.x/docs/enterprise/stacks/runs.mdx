---
page_title: Stack runs in Terraform Enterprise
description: >-
  Learn how Terraform Enterprise executes Terraform runs to keep Stack
  deployments up to date with configuration versions over time.
...
Terraform Enterprise is the interface for keeping track of your Stack configuration over time and the corresponding deployments for each version of your configuration. Learn how Terraform Enterprise executes Terraform runs to keep Stack deployments up to date.

## Run environment

Terraform Enterprise is designed as an execution platform for Terraform, and executes runs on its own disposable virtual machines. Terraform runs managed by Terraform Enterprise are called remote operations. Terraform Enterprise executes Stack deployment runs remotely, and you cannot execute Stack runs locally.
...
[HCP Terraform agents](/terraform/cloud-docs/agents/) let Terraform Enterprise communicate with isolated, private, or on-premises infrastructure. The agent polls Terraform Enterprise for any changes to your configuration and executes the changes locally, so you do not need to allow public ingress traffic to your resources. Agents let you control infrastructure in private environments without modifying your network perimeter. Note that [agent hooks](/terraform/cloud-docs/agents/hooks) do not support Stack workflows.
...
Terraform Enterprise creates a new configuration version each time any of the following change:

-   Your configuration changes and VCS automatically fetches that change
-   Your configuration changes and you manually fetch that change in Terraform Enterprise
...
To learn more about interacting with your Stack's deployment runs, refer to [Review deployment runs](/terraform/enterprise/stacks/deploy/runs).

### Deployment run order

Each Stack in Terraform Enterprise maintains its own configuration versions, and each version has a queue of deployment runs. Terraform Enterprise executes deployment runs in the order that you approve them.

When you approve a deployment plan, Terraform Enterprise automatically dismisses all pending plans for that deployment from other configuration versions. Terraform Enterprise permanently discards plans from older configuration versions because you cannot revert your deployment to a previous configuration version.
...
Terraform Enterprise lets multiple deployment runs exist simultaneously for different configuration versions, but only one run can modify a deployment's state at a time. Deployment runs lock state while they are applying to ensure a run can finish without interference.

When you approve a deployment run, Terraform Enterprise locks the deployment's state before beginning execution. This lock remains in place throughout the entire run, because a run may require multiple plan-and-apply cycles to reach convergence. The lock prevents any other approved runs from starting their execution and potentially creating conflicts.
...
After the running deployment releases its lock, the next approved run in the queue can acquire the lock and begin its execution. To learn more about state for deployments in a Stack, refer to [State for Stacks](/terraform/enterprise/stacks/state).
...
-   The import run mode triggers when Terraform Enterprise converts a workspace into a Stack. 
...
If you remove a deployment block from your configuration, Terraform Enterprise starts a destroy run using the last configuration that included that deployment. If you plan on updating the provider configuration in your Stack and also destroying some deployments, we recommend using the destroy argument to remove deployments to ensure your configuration has the authentication necessary to destroy that deployment.
...
Each step in a deployment run matches a specific state as Terraform Enterprise executes a deployment run. A deployment run step exists in one of four primary states:
...
Running steps are when Terraform Enterprise actively executes part of the deployment run. During a running step Terraform is doing one of the following:
...
Runs that produce changes pause in the waiting step state to wait for operator approval. Terraform Enterprise requires manual confirmation before proceeding with infrastructure changes, unless you configure [auto-approval rules for your deployments](/terraform/language/stacks/deploy/conditions).
