...
description: >-
  Learn how to create and review configuration versions for your Stacks in
...
Each Stack deployment uses a configuration version to determine what infrastructure to create, update, or delete. You can rollout changes to your Stack deployments by creating new configuration versions. Terraform Enterprise is the interface for keeping track of your Stack configuration over time and the corresponding deployments for each version of your configuration.
...
-   [Organization-level **Manage all projects**](/terraform/enterprise/users-teams-organizations/permissions/organization) or higher
-   [Project-level **Read**](/terraform/enterprise/users-teams-organizations/permissions/project) or higher
...
Each configuration version creates a deployment run for every deployment of your Stack to implement the changes in that version. To learn more about how deployments and how they plan and apply infrastructure, refer to [Stack deployment runs](/terraform/enterprise/stacks/runs).

### VCS-linked repositories

Stacks automatically detect when you push changes to your configuration from a VCS-linked repository. Every time you push changes to your repository, Terraform Enterprise automatically fetches your configuration and creates a new configuration version.

Whether automatic or manual, every time Terraform Enterprise fetches a new version of your configuration file, it creates a new configuration version, whether you made changes to that file or not.

Each configuration version creates a deployment run for every deployment of your Stack in order to implement the changes in that version. Learn more about [reviewing and approving deployment runs](/terraform/enterprise/stacks/deploy/runs).
...
2.  In the navigation menu, Select **Projects** under **Manage**.
3.  Select the project containing your Stack.
4.  Select the Stack you want to review.
5.  Click **Fetch configurations from VCS**.
...
You can also create a configuration version using the API. To learn more, refer to the [Stacks API](/terraform/enterprise/api-docs/stacks/configurations#create-a-stack-configuration).

## Review configuration versions

The **Configurations** page contains a numbered sequential list of your configuration versions.

Configuration versions create deployment runs for each of your Stack's deployments. Terraform Enterprise marks each configuration version as **Completed** if it created deployment runs for all of your Stack's deployments, or **Failed** if it failed out and could not create the necessary deployment runs.

Clicking on an individual configuration version reveals the following:

-   How long ago the configuration version was created.
-   The latest commit that Terraform Enterprise based this configuration version on and who made that commit.
...
Learn about [deployment runs](/terraform/enterprise/stacks/deploy/runs) or how to [approve or discard deployment run plans](/terraform/enterprise/stacks/deploy/runs) to manage your deployments.
