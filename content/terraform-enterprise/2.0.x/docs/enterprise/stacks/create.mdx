...
Stacks are not available for users on legacy HCP Terraform team plans. Learn more about [migrating to current HCP Terraform plans](/terraform/enterprise/overview/migrate-teams-standard).
...
-   The [HCP Terraform API](/terraform/enterprise/api-docs/stacks)
...
9.  By default, HCP Terraform fetches your Stack's configuration after creating your Stack. To fetch your Stack configuration manually, uncheck **Fetch configuration after HCP Terraform creates stack**.
...
4.  After running the command, you can view your new Stack in the HCP Terraform UI, or by running the `terraform stacks list` command:
...
5.  After creating your Stack, you can push up your Stack component and deployment configuration files to create a new configuration version. Use the `terraform stacks configuration upload` command to manually upload your configuration files:
    ...
    Note that though you can create a Stack from the Terraform CLI, you cannot deploy a Stack locally. You can only deploy a Stack remotely in HCP Terraform, or by running your a Stack on a custom HCP Terraform agent. To learn more, refer to [Stack deployment runs](/terraform/enterprise/stacks/runs).
