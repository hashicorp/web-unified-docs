...
Configuring automatic infrastructure destruction for a workspace requires [admin permissions](/terraform/enterprise/users-teams-organizations/permissions/workspace#workspace-admin) for that workspace.
...
<<<<<<< HEAD
By default, [workspace administrators](/terraform/enterprise/users-teams-organizations/permissions/workspace#workspace-admin) can only delete unlocked workspaces that are not managing any infrastructure. Organization owners can force delete a workspace to override these protections. Organization owners can also configure the [organization's settings](/terraform/enterprise/users-teams-organizations/organizations#general) to let workspace administrators force delete their own workspaces.
=======
By default, [workspace administrators](/terraform/enterprise/users-teams-organizations/permissions/workspace#workspace-admin) can only delete unlocked workspaces that are not managing any infrastructure. Organization owners can force delete a workspace to override these protections. Organization owners can also configure the [organization's settings](/terraform/enterprise/users-teams-organizations/organizations/settings#general) to let workspace administrators force delete their own workspaces.
>>>>>>> tfe-release/2.0.0
