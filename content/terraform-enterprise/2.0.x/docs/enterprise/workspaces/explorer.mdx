---
page_title: Terraform Enterprise explorer for workspace visibility
description: >-
...
- [Organization owner](/terraform/enterprise/users-teams-organizations/permissions/organization#organization-owners)
- [View all workspaces](/terraform/enterprise/users-teams-organizations/permissions/organization#view-all-workspaces) or greater

## Types

The explorer for workspace visibility supports the following types:

- [Workspaces](/terraform/enterprise/workspaces)
- [Modules](/terraform/language/modules)
- [Providers](/terraform/language/providers)
- [**Resources**](/terraform/cloud-docs/workspaces/resources) shows individual resources tracked across the organization.
...
- **Workspaces by run status** provides a run-focused view by sorting workspaces by their current run status.
...
- **Workspaces with zero resources** lists workspaces where the resource count is `0`. This use case is in beta and may not be visible to all users.
- **Workspaces with the most resources** lists workspaces with at least one [resource under management](/terraform/enterprise/overview/estimate-hcp-terraform-cost#what-is-a-managed-resource), sorted by resources under management (RUM) count descending. This use case is in beta and may not be visible to all users.
...
Each filter condition appears as a row of inputs consisting of a target field, an operator, and a value.

1. Choose a target field from the first dropdown to select the field that the explorer runs the query against. The available options vary based on the target field's data type.

1. Choose an operator from the second dropdown. The available options vary based on the target field's data type.
...
Inspect the filter conditions used by the various predefined [**Use Cases**](#use-cases) to learn how they are constructed.
...
1. Click a tile in the **Types** or **Use cases** section.
...
- The type of data you are querying, either module, provider, workspace, resources, or Terraform versions.
...
The **Actions** dropdown on every query results and saved view page also includes **Give feedback**, which opens a form where you can share feedback about the explorer.
...
1. Enter a name for this new saved view.
1. Click **Save**.

Complete the following steps to delete a saved view:

1. Open a saved view in the explorer.
1. Choose **Delete view** from the **Actions** dropdown menu.
