...
-   Sentinel policy checks occur after Terraform completes the plan and after both [run tasks](/terraform/enterprise/workspaces/settings/run-tasks) and [cost estimation](/terraform/cloud-docs/cost-estimation). This order lets you write Sentinel policies to restrict costs based on the data in the cost estimates.
...
Refer to [Run States and Stages](/terraform/enterprise/workspaces/run/states) for more details.
...
-> **Note:** For Sentinel, the Terraform CLI also prints policy results for [CLI-driven runs](/terraform/enterprise/workspaces/run/cli). CLI support for policy results is not available for OPA.

## Override Policies

You need [manage policy overrides](/terraform/enterprise/users-teams-organizations/permissions/organization#manage-policy-overrides) permissions to override failed Sentinel and OPA policies.
...
-> **Note:** HCP Terraform does not allow policy overrides for [no-operation plans containing no infrastructure changes](/terraform/enterprise/workspaces/run/modes-and-options#allow-empty-apply), unless you choose the **Allow empty apply** option when starting the run.
