...
Be sure that the user's identity and the validity of their request have been verified according to appropriate security procedures before disabling their configured 2FA.
 ...
 If the user belongs to an organization that requires 2FA, upon login, they will be redirected to [set it up again](/terraform/enterprise/users-teams-organizations/2fa) before they can view any other part of Terraform Enterprise.

### Assign site auditor permission

Click on a user and enable the Enable Site Auditor permission to grant them read-only access to Terraform Enterprise resources. Auditors can navigate the platform to verify resources, but they can't perform operations that modify data, such as starting Terraform runs. When logged in as a site auditor, a banner appears at the top of the page to indicate that you have read-only access to data.

Site auditors can access all areas of the UI except for some of the settings pages for platform resources. The following table describes the settings pages that are available to site auditors:
...
Permissions with greater access have precedence over site auditor permissions so that you can continue to perform tasks as normal team member. For example, if you are on a team that has write permissions to one or more projects, workspaces, or Stacks, the site auditor permission doesn't prevent you from performing operations that modify data in those entities. You can continue to perform tasks granted to your team in addition to viewing data data in other organizations.
