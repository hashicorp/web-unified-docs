...
-> **Note:** You cannot generate authentication tokens while impersonating a user.
...
For organizations that require 2FA, Terraform Enterprise redirects users to the [2FA set up page](/terraform/enterprise/users-teams-organizations/2fa) at login before they can view any other part of Terraform Enterprise.

### Assign site auditor permission

Click on a user and enable the **Site Auditor** permission to grant them read-only access to Terraform Enterprise resources. Auditors can navigate the platform to verify resources, but they can't perform operations that modify data, such as starting Terraform runs. When logged in as a site auditor, a banner appears at the top of the page to indicate that you have read-only access to data.

Site auditors have read-only access to most areas of the UI. Access to platform resource settings is limited. The following table describes the settings pages that are available to site auditors:


| Resource | Title | Path | Notes |
| --- | --- | --- | --- |
| Organization | General settings | `/app/{org}/settings/profile` | Subset that only shows ID, name, notification email, default execution mode |
| Organization | Teams | `/app/{org}/settings/teams` | |
| Organization  | Variable sets | `/app/{org}/settings/varsets` | |
| Organization  | Runs | `/app/{org}/settings/runs` | |
| Organization  | API tokens | `/app/{org}/settings/authentication-tokens` | Only tokens for teams you're a member of are accessible |
| Project  | Team access | `app/{org}/projects/{project}/access` | |
| Project  | Variable sets | `/app/{org}/projects/{project}/settings/varsets)` | |
| Workspace | n/a | n/a | Workspace settings are inaccessible to site auditors |

Call the API to audit data that isn't accessible in the UI under the site auditor permission. Refer to the [API reference documentation](/terraform/enterprise/api-docs) for information about available endpoints.

Permissions with greater access have precedence over site auditor permissions so that you can continue to perform tasks as normal team member. For example, if you are on a team that has write permissions to one or more projects, workspaces, or Stacks, the site auditor permission doesn't prevent you from performing operations that modify data in those entities. You can continue to perform tasks granted to your team in addition to viewing data in other organizations.
