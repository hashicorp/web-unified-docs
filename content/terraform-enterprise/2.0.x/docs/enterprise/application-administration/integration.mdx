...
The SCIM provisioning settings allow you to enable site-level SCIM provisioning for Terraform Enterprise. To access the SCIM settings, click **SCIM**.

Use the **Enable SCIM provisioning** setting to allow Terraform Enterprise to process SCIM provisioning requests from your identity provider. Terraform Enterprise requires SAML to already be configured before you enable SCIM.

Refer to [Configure SCIM provisioning](/terraform/enterprise/scim/configure) for the setup workflow and identity provider configuration. Refer to [Manage SCIM provisioning](/terraform/enterprise/scim/manage) for token management, site admin group behavior, and operational guidance.
