...
These instructions are for using an on-premises installation of Azure DevOps Server 2019 for Terraform Cloud's VCS features. [Azure DevOps Services has separate instructions,](/enterprise/vcs/azure-devops-services) as do the [other supported VCS providers.](/enterprise/vcs)
...
2.  The next page has several steps to guide you through adding a new VCS provider.
...
3.  On the "Set up provider" step there are three textboxes. Enter an optional **Name** for this VCS connection. Enter the instance URL for your Azure DevOps Server in **HTTP URL** and **API URL** textboxes. Click the "Continue" button to continue to the next step.
