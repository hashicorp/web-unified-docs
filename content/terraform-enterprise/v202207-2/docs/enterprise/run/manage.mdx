---
page_title: Viewing and Managing Runs - Runs - Terraform Enterprise
description: >-
...
-> **API:** See the [Runs API](/enterprise/api-docs/run) and [lock a Workspace endpoint](/enterprise/api-docs/workspaces#lock-a-workspace).
...
In workspaces where you have permission to apply runs, run pages include controls for interacting with the run at the bottom of the page. ([More about permissions.](/enterprise/users-teams-organizations/permissions)) Depending on the state of the run, the following buttons might be available:

| Button              | Available when:                                                                                                                                                                                                                    |
| ------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Add Comment         | Always.                                                                                                                                                                                                                            |
| Confirm & Apply     | A plan needs confirmation.                                                                                                                                                                                                         |
| Override & Continue | A soft-mandatory policy failed. Requires permission to manage policy overrides for the organization. ([More about permissions.](/enterprise/users-teams-organizations/permissions))                                                |
| Discard Run         | A plan needs confirmation or a soft-mandatory policy failed.                                                                                                                                                                       |
| Cancel Run          | A plan or apply is currently running.                                                                                                                                                                                              |
| Force Cancel Run    | A plan or apply was canceled, but something went wrong and Terraform Cloud couldn't end the run gracefully. Requires admin access to the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions)) |
...
If a plan needs confirmation (with [manual apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) enabled) or a soft-mandatory policy failed, the run will remain paused until a user with appropriate permissions uses these buttons to continue or discard the run. For more details, see [Run States and Stages](/enterprise/run/states).

### Canceling Runs

If a run is currently planning or applying, users with permission to apply runs for the workspace can cancel the run before it finishes, using the "Cancel Run" button on the run's page. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
Since force-canceling can have dangerous side-effects (including loss of state and orphaned resources), it requires admin access to the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions)) Additionally, the "Force Cancel Run" button only appears after the normal cancel button has been used and a cool-off period has elapsed, to ensure Terraform Cloud has a chance to terminate the run safely.
...
You can find the lock button in the "Actions" menu in a workspace's header, or in [the workspace settings page](/enterprise/workspaces/settings). Locking a workspace requires permission to lock and unlock the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
