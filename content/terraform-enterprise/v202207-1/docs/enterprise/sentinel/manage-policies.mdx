...
-> **API:** See the [Policy Sets API](/enterprise/api-docs/policy-sets).<br/>
...
Policy sets are managed at an organization level; viewing and modifying them requires permission to manage policies. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
-   `soft-mandatory` is much like `hard-mandatory`, but allows any user with the [Manage Policy Overrides](/enterprise/users-teams-organizations/permissions#manage-policy-overrides) permission to override policy failures on a case-by-case basis.
