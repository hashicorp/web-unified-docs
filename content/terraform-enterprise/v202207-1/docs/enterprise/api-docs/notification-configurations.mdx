...
Interacting with notification configurations requires admin access to the relevant workspace. ([More about permissions](/enterprise/users-teams-organizations/permissions).)
...
| ------------------ | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Created            | `"run:created"`         | A run is created and enters the [Pending stage](/enterprise/run/states#1-the-pending-stage)                                                                                             |
| Planning           | `"run:planning"`        | A run acquires the lock and starts to execute.                                                                                                                                          |
| Needs Attention    | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This may include approving the plan or a [policy override](/enterprise/run/states#5-the-policy-check-stage).          |
| Applying           | `"run:applying"`        | A run enters the [Apply stage](/enterprise/run/states#6-the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                                       |
...
| Drifted            | `"assessment:drifted"`  | Terraform Cloud detected configuration drift. This is only available if drift detection is enabled for the workspace. Not available in Terraform Enterprise.               |
| Drift Check Failed | `"assessment:failed`"   | A drift detection assessment failed. This is only available if drift detection is enabled for the workspace. Not available for Terraform Enterprise. |

## Notification Payload

The notification is an HTTP POST request with a detailed payload. The content depends on the type of notification.

For Slack and Microsoft Teams notifications, the payload conforms to the respective webhook API and results in a notification message with informational attachments. Refer to [Slack Notification Payloads](/enterprise/workspaces/settings/notifications#slack) and [Microsoft Teams Notification Payloads](/enterprise/workspaces/settings/notifications#microsoft-teams) for examples. For generic notifications, the payload varies based on whether the notification contains information about run events or workspace events.
...
Workspace events include detailed information about workspace-level validation events like [drift detection](/enterprise/workspaces/settings/drift-detection) if it is enabled for the workspace. Much of the information provides details about the associated [assessment result](/enterprise/api-docs/assessment-results), which Terraform Cloud uses to track instances of continuous validation. Generic notifications for workspace events contain the following information:
...
| `details.new_assessment_result.succeeded`    | bool   | Whether or not assessment succeeded.                                                |
| `details.new_assessment_result.drifted`      | bool   | Whether or not assessment detected drift.                                           |
| `details.new_assessment_result.created_at`   | string | Timestamp of the assessment result                                                  |
...
| `details.prior_assessment_result.succeeded`  | bool   | Whether or not assessment succeeded.                                                |
| `details.prior_assessment_result.drifted`    | bool   | Whether or not assessment detected drift.                                           |
...
| --------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `:workspace_id` | The ID of the workspace to list configurations for. Obtain this from the [workspace settings](/enterprise/workspaces/settings) or the [Show Workspace](/enterprise/api-docs/workspaces#show-workspace) endpoint. |
...
| Parameter       | Description                                                                                                                                                                                                                                                                                                                             |
| --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `:workspace_id` | The ID of the workspace to list configurations from. Obtain this from the [workspace settings](/enterprise/workspaces/settings) or the [Show Workspace](/enterprise/api-docs/workspaces#show-workspace) endpoint.  If neither pagination query parameters are provided, the endpoint will not be paginated and will return all results. |
