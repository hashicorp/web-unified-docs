...
4.  Click **Create run task**. The run task is now available within the organization. You can now associate it with one or more workspaces.
...
5.  Click **Create**. Your run task is now configured.

## Understanding Run Tasks Within a Run

Run tasks perform actions between the [plan](/enterprise/run/states#2-the-plan-stage) and [apply](/enterprise/run/states#6-the-apply-stage) stages of a [Terraform run](/enterprise/run). Once all run tasks have completed, the run will end based on the most restrictive enforcement level in each associated run task.
