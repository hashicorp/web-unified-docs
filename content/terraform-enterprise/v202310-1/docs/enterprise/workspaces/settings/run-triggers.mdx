...
description: >-
  Learn how to use run triggers to connect workspaces within your organization.
...
-> **Note:** Runs initiated in your workspace as the result of a run trigger connection to a source workspace will not auto-apply, regardless of your [auto-apply](/terraform/enterprise/workspaces/settings#auto-apply-and-manual-apply) setting selection. You will need to manually apply these runs.
...
Before other workspaces can read the outputs of a workspace, it must be configured to allow access. For more information about cross-workspace state access in Terraform Cloud, see [Terraform State in Terraform Cloud](/terraform/enterprise/workspaces/state).
