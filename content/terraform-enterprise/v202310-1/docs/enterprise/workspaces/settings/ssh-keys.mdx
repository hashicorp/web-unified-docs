...
description: >-
  Learn how to configure the SSH keys Terraform uses to pull modules from
...
-> **Note:** The [private module registry](/terraform/enterprise/registry) is an easier way to manage private Terraform modules in Terraform Cloud, and doesn't require setting SSH keys for workspaces. The rest of this page only applies to configurations that fetch modules directly from a private Git repository.
...
-> **Note:** SSH keys for cloning Terraform modules from Git repos are only used during Terraform runs. They are managed separately from any [keys used for bringing VCS content into Terraform Cloud](/terraform/enterprise/vcs#ssh-keys).
