...
required, using [policy sets](/enterprise/sentinel/manage-policies)
...
Specifies whether the plan associated with the run is a [speculative plan](/enterprise/run/remote-operations#speculative-plans) only.
...
Provides the names of the variables that are configured within the run and the [sensitivity](/enterprise/workspaces/variables/managing-variables#sensitive-values) state of the value.
...
The **organization namespace** contains data associated with the current run's Terraform Cloud [organization](/enterprise/users-teams-organizations/organizations).
...
Contains the workspace's [auto-apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) setting.
...
Contains the configured [Terraform working directory](/enterprise/workspaces/settings#terraform-working-directory) of the workspace.
...
Details regarding each attribute can be found in the documentation for the Terraform Cloud [Workspaces API](/enterprise/api-docs/workspaces).
