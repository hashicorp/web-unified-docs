...
installations can be found [here](/enterprise/integrations/service-now/service-now-v1)
...
- One or more [supported version control
  systems](/enterprise/vcs/#supported-vcs-providers) (VCSs) with read access
...
1.  Either [create an
    organization](/enterprise/users-teams-organizations/organizations#creating-organizations)
    or choose an existing organization where ServiceNow will create new
    workspaces.
    **Save the organization name for later.**
2.  [Create a team](/enterprise/users-teams-organizations/teams) for that
    organization called "ServiceNow", and ensure that it has [permission to
    manage
    workspaces](/enterprise/users-teams-organizations/permissions#manage-workspaces).
    You do not need to add any users to this team.
    [permissions-citation]: #intentionally-unused---keep-for-maintainers
3.  On the "ServiceNow" team's settings page, generate a [team API
    token](/enterprise/users-teams-organizations/api-tokens#team-api-tokens).
    **Save the team API token for later.**
4.  If you haven't yet done so, [connect a VCS provider](/enterprise/vcs) to
...
5.  On your organization's VCS provider settings page (Settings > VCS Providers),
...
6.  Identify one or more VCS repositories in the VCS provider containing
...
8.  The organization name
9.  The team API token
10. The hostname of your Terraform Enterprise instance, or of Terraform Cloud.
    The hostname of Terraform Cloud is `app.terraform.io`.
11. The OAuth token ID(s) of your VCS provider(s), and the repository
    identifier for each VCS repository containing Terraform configurations that
    will be used by the integration.
12. Any Terraform or environment variables required by the configurations in the
...
For instance, if you are configuring this integration for your company, `Example Corp`, using two GitHub repositories, you would share values like the following
...
2.  Using the left-hand navigation, open the configuration table for the
    ...
    - Terraform > Configs
3.  Click on "New" to create a new Terraform Cloud connection.
...
Navigate to the [Service Catalog documentation](/enterprise/integrations/service-now/service-catalog) to
...
to the [developer documentation](/enterprise/integrations/service-now/developer-reference).

### ServiceNow Administrator's Guide.

Refer to the [ServiceNow Administrator documentation](/enterprise/integrations/service-now/admin-guide) for
...
customizations documentation](/enterprise/integrations/service-now/example-customizations).
