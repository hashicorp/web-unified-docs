...
## Specify OIDC integration settings

You can configure Terraform Enterprise to be accessible over a primary and a secondary hostname. When both hostnames are configured, set the [`TFE_OIDC_HOSTNAME_CHOICE`](/terraform/enterprise/deploy/reference/configuration#tfe_oidc_hostname_choice) setting and specify either `primary` or `secondary` if you are federating workloads for OIDC-related integrations.  
