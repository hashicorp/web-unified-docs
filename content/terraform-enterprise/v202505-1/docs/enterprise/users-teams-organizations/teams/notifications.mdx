...
You can only enable Slack webhook notifications for HCP Terraform or Terraform Enterprise that uses IPv4 addresses. This is because Slack webhooks does not support IPv6-only networks. You can use other notification methods, such as email, Microsoft Teams, and custom webhooks instead.
