...
1. Changed cost estimation so that it uses the [HTTP proxy settings](/terraform/enterprise/install/interactive/installer#proxy-usage) configured within Terraform Enterprise.
...
1. The [`iact_subnet_list` setting](/terraform/enterprise/install/automated/automating-the-installer#iact_subnet_list) now allows you to use `, ` to separate IPv4 addresses.
