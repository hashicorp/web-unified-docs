...
To modify VCS Status Checks settings, you must have [**Manage VCS Settings**](/terraform/enterprise/users-teams-organizations/permissions/organization#manage-vcs-settings) permissions.
...
Workspaces that use part of a shared repository do not typically run plans for changes that do not affect their files. This includes [speculative plans](/terraform/enterprise/workspaces/run/remote-operations#speculative-plans) on pull requests. Since **pending** VCS status checks can block pull requests, workspaces automatically send passing commit statuses for any PRs that do not affect their files.
