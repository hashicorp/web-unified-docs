...
last_modified: 2026-01-29T18:27:21.000Z
...
~> **Note**: The `terraform_full_workspace` claim is only included in workspace-scoped tokens. Organization-level operations, such as [module test runs](/terraform/enterprise/registry/test/dynamic-credentials), do not include this claim.

## Module test run tokens

When using [dynamic credentials for module testing](/terraform/enterprise/registry/test/dynamic-credentials), HCP Terraform generates workload identity tokens specifically for test runs. These tokens have a different structure and custom claims compared to workspace run tokens.
...
-   **Configuration**: Organization owners can adjust the TTL using the Organizations API by setting the `module_test_token_ttl` attribute
...
For more information, refer to [Dynamic credentials for module testing](/terraform/enterprise/registry/test/dynamic-credentials).
