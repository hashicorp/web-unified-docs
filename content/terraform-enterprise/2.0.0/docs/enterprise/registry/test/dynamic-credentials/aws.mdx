...
4.  Choose the OIDC provider you created in the previous step.
5.  For **Audience**, select `aws.workload.identity`.
6.  Click **Next**.
7.  Attach permissions policies required for your module tests. Refer to [permissions recommendations](#permissions-recommendations) for more information.
8.  Specify a name for the role, for example `terraform-module-test-role`.
9.  Click **Create role**.
...
1.  Increase the organization's `module_test_token_ttl` using the [Organizations API](/terraform/enterprise/api-docs/organizations)
