...
This option instructs Terraform to begin new runs only for changes that affect specified files and directories. This behavior also applies to [speculative plans](/terraform/enterprise/workspaces/run/remote-operations#speculative-plans) on pull requests.
...
Whether to perform [speculative plans on pull requests](/terraform/enterprise/workspaces/run/ui#speculative-plans-on-pull-requests) to the connected repository, to assist in reviewing proposed changes. Automatic speculative plans are enabled by default, but you can disable them for any workspace.
