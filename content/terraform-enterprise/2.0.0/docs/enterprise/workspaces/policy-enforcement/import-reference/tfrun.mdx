...
Specifies whether the plan associated with the run is a [speculative plan](/terraform/enterprise/workspaces/run/remote-operations#speculative-plans) only.
...
Provides the names of the variables that are configured within the run and the [sensitivity](/terraform/enterprise/variables/managing-variables#sensitive-values) state of the value.
