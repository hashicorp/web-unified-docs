...
You must be a member of a team with [manage all projects & workspaces permissions](/terraform/enterprise/users-teams-organizations/permissions#manage-all-projects-workspaces), [manage workspaces only permissions](/terraform/enterprise/users-teams-organizations/permissions#manage-workspaces-only), or [admin permissions for a project](/terraform/enterprise/users-teams-organizations/permissions#project-admins) to create a workspace.
...
4.  (Version control only) Choose an existing version control provider or configure a new one. Refer to [Connecting VCS Providers](/terraform/enterprise/vcs) for more details.
...
After you create a new workspace from a version control repository, Terraform Cloud scans its configuration files for [Terraform variables](/terraform/enterprise/workspaces/variables#terraform-variables) and displays any that do not have a default value and do not have a definition in an existing global [variable set](/terraform/enterprise/api-docs/variable-sets). Terraform cannot perform successful runs in the workspace until you set values for these variables.
...
If you connected a VCS repository to the workspace, Terraform Cloud automatically registers a webhook with your VCS provider. A workspace with no runs will not accept new runs from a VCS webhook, so you must [manually start at least one run](/terraform/enterprise/run/ui#manually-starting-runs).

After you manually start a run, Terraform Cloud automatically queues a plan when new commits appear in the selected branch of the linked repository or someone opens a pull request on that branch. Refer to [Webhooks](/terraform/enterprise/vcs#webhooks) for more details.
