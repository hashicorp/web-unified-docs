...
| `TFC_AWS_RUN_ROLE_ARN`  | The arn of the AWS role arn to authenticate against | Optional if `TFC_AWS_PLAN_ROLE_ARN` and `TFC_AWS_APPLY_ROLE_ARN` are both provided. These variables are described [below](/terraform/enterprise/workspaces/dynamic-provider-credentials/aws-configuration#optional-environment-variables) |
