...
mode](/terraform/enterprise/install/pre-install-checklist#operational-mode-decision),
...
- [Log Forwarding](/terraform/enterprise/admin/infrastructure/logging)
...
See [the Upgrades section](/terraform/enterprise/admin/infrastructure/upgrades) of the documentation.
...
[Determine Your Requirements](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/cluster-create-determine-requirements.html) provides a high level description of the implementation options for the memory cache. Primary differentiators are Cluster Mode and Multi-AZ. The primary difference is how far high availability might extend. Cluster Mode extends the cluster to multiple Node Groups and will benefit larger scale, multi-client implementations. The only clients for Terraform Enterprise are the compute instances in a single region, so this is not required or supported. Here is  additional information on [Managing Your ElastiCache Clusters](https://docs.aws.amazon.com/AmazonElastiCache/latest/red-ug/Clusters.html).
