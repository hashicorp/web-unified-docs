...
| Needs Attention          | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This input may include approving the plan or a [policy override](/enterprise/run/states#7-the-policy-check-stage). |
| Applying                 | `"run:applying"`        | A run enters the [Apply stage](/enterprise/run/states#9-the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                                    |
...
| Health Assessment Failed | `"assessment:failed`"   | A health assessment failed. This option is only available if you enable health assessments for the workspace.  Not available in Terraform Enterprise.                                |
...
Workspace events include detailed information about workspace-level validation events like [health assessments](/enterprise/workspaces/settings/health-assessments) if you enable them for the workspace. Much of the information provides details about the associated [assessment result](/enterprise/api-docs/assessment-results), which Terraform Cloud uses to track instances of continuous validation. Generic notifications for workspace events contain the following information:
