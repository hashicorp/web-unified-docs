...
for the workspace. All runs can enter this phase, including [speculative plans](/enterprise/run#speculative-plans). During this phase, Terraform Cloud sends information about the run to the configured external system and waits for a `passed` or `failed` response to determine whether the run can continue.
