...
2.  Click **Create**. Your run task is now configured.

## Understanding Run Tasks Within a Run

Run tasks perform actions between the [plan](/enterprise/run/states#4-the-plan-stage) and [apply](/enterprise/run/states#9-the-apply-stage) stages of a [Terraform run](/enterprise/run). Once all run tasks complete, the run ends based on the most restrictive enforcement level in each associated run task.
