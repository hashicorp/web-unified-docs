...
Docker logs using the `json-file` logging driver (the default driver) will no longer be automatically rotated. Please refer to the [log rotation documentation](/enterprise/install/pre-install-checklist#log-rotation) for details on how to configure log rotation.
