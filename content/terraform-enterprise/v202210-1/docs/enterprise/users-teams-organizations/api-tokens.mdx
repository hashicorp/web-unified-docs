...
-> **API:** See the [Team Token API](/enterprise/api-docs/team-tokens) and [Organization Token API](/enterprise/api-docs/organization-tokens).

## User API Tokens

API tokens may belong directly to a user. User tokens are the most flexible token type because they inherit permissions from the user they are associated with. For more information on user tokens and how to generate them, see the [Users](/enterprise/users-teams-organizations/users#api-tokens) documentation.
...
Each organization can have **one** valid API token at a time. Only [organization owners](/enterprise/users-teams-organizations/teams#the-owners-team) can generate or revoke an organization's token.
...
-> **Note:** This endpoint cannot be accessed with [organization tokens](#organization-api-tokens). You must access it with a [user token](/enterprise/users-teams-organizations/users#api-tokens) or [team token](#team-api-tokens).

## Agent API Tokens

[Agent pools](/enterprise/agents) have their own set of API tokens which allow agents to communicate with Terraform Cloud, scoped to an organization. These tokens are not valid for direct usage in the Terraform Cloud API and are only used by agents.
