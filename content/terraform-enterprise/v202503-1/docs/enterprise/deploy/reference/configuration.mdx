...
last_modified: 2026-09-04T14:33:56.000Z
...
### `http_proxy`
Configures the proxy address to use for HTTP requests, for example`http://proxy.example.com:8080`. When using a proxy with username and password, special characters in these fields will need to be manually HTML encoded. Do not set a value  blank to disable using a proxy server. Defaults to `""`.

### `https_proxy`
Configures the proxy address to use for HTTPS requests, for example `http://proxy.example.com:8080`. Leave blank to disable using a proxy server. When using a proxy with username and password, special characters in these fields will need to be manually HTML encoded. Defaults to `""`.
