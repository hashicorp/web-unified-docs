...
HCP Terraform manages infrastructure collections with _workspaces_ instead of directories. A workspace contains everything Terraform needs to manage a given collection of infrastructure, and separate workspaces function like completely separate working directories.
...
-> **Note:**  The explorer for workspace visibility surfaces a wider range of valuable information from across your workspaces. [Learn more](/terraform/enterprise/workspaces/explorer).

<!-- END: TFC:only name:explorer-callout -->

Click **Projects & workspaces**. HCP Terraform displays the workspaces the current user account has permission to read runs for. The project drawer lists any projects the user has access to. ([More about permissions.](/terraform/enterprise/users-teams-organizations/permissions))
...
Refer to [Organize Workspaces with Projects](/terraform/enterprise/projects/managing) for more details.

## Creating Workspaces

You can create workspaces through the [HCP Terraform UI](/terraform/enterprise/workspaces/creating), the [Workspaces API](/terraform/enterprise/api-docs/workspaces), or the [HCP Terraform CLI integration](/terraform/cli/cloud).
