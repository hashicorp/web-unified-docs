...
-   **Team management:**
    ...
    -   **Manage:** — _Implies permission to read._ Allows users to set or remove project permissions for visible teams. Project admins can not view or manage [secret teams](/terraform/enterprise/users-teams-organizations/teams#team-visibility) unless they are also organization owners.
...
HCP Terraform has three levels of team management permissions: manage membership, manage teams, and manage organization access. Each permission level grants users the ability to perform specific actions and progressively requires prerequisite permissions. 

For example, to grant a user the manage teams permission, that user must already have the manage membership level. To grant the manage organization, a user must have both the manage teams and manage membership permissions.
...
Allows members to create, update, and delete teams.

This permission grants the ability to update a team's names and SSO IDs, but does not allow access to organization settings. On its own, this permission does not allow users to create, update, delete, or otherwise access secret teams.

The manage teams permission confers all permissions granted by the manage membership permission.

We designed this permission to allow owners of large organizations to delegate team management to another trusted team, and you should only grant it to teams of trusted users. 
...
On its own, this permission does not allow users to create, update, delete, or otherwise access secret teams.

This permission confers all of the permissions granted by the manage teams and manage membership permissions.
