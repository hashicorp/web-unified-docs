...
An organization can subscribe to different [feature sets](/enterprise/api-docs/feature-sets), which represent the [pricing plans](/enterprise/paid) available in Terraform Cloud. An organization's [entitlement set](/enterprise/api-docs/#feature-entitlements) is calculated using its subscription and feature set.
