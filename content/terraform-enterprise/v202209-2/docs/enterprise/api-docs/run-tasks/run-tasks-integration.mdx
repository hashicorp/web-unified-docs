...
| `stage`                         | string  | `pre_plan`, `post_plan`, `pre_apply` | The [run stage](/enterprise/run/states) when Terraform Cloud triggers the run task.  The `stage` property is in beta. |
