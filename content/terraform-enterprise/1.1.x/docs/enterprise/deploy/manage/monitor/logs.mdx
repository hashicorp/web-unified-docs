---
page_title: Enable service logs and audit logs
description: Learn how to enable service logs and audit logging in Terraform Enterprise to monitor security events and system performance.
---

# Enable logs 

You can enable service and audit logs in Terraform Enterprise to help you monitor your deployment. Audit logs in Terraform Enterprise to help you monitor security-related events, such as when someone accesses the admin console or calls system API endpoints.

Terraform Enterprise writes service logs directly to standard output and standard error, which allows you to forward logs using native tooling for your deployment platform.

## Audit logs

To enable audit logging, enable the admin console in your deployment configuration. Refer to [Enable the admin console](/terraform/enterprise/deploy/configuration/admin-console) for instructions. 

Audit log availability depends on system configuration and operational status. The system might not capture audit events during maintenance, failures, or misconfigurations.
