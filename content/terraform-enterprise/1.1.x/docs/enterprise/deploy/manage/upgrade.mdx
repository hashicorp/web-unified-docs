...
- New container images downloaded for every hop
...
- **Required — Prepare the maintenance window**: Freeze automation that could start runs, notify stakeholders about the planned downtime, and document the rollback criteria you intend to follow if validation fails.
- **Recommended — Rehearse the upgrade path**: Run through the full sequence in a test environment that mirrors production. Validate that required releases install cleanly, dependencies meet the target version requirements, automation resumes after the upgrade, and your rollback procedure works as expected.
