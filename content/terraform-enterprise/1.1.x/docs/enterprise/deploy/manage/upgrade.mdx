...
We recommend following the checklist for every environment where you run Terraform Enterprise.

<Tip title="Planning checklist">

1. Select a target version and confirm its compatibility with your environments
2. Identify pre-requisite intermediate releases
3. Schedule maintenance window
4. Capture configuration
5. Backup PostgreSQL, Redis, and object storage
6. Download the container images downloaded for each intermediate release
7. Define rollback criteria and rehearse the rollback procedure
8. Upgrade test environment
...
2. **Map required releases**: Use the "Last required release" marker on the target version page (for example, [Terraform Enterprise 1.1.x](/terraform/enterprise/releases/1.1.x)) to determine which previous releases you must install first. Repeat this comparison for each required release until you reach your current running version. Document the sequence so you can stage artifacts for every hop. See the [Example upgrade path](#example-upgrade-path) for a sample workflow.
...
5. **Rehearse the upgrade path**: Run through the full sequence in a test environment that mirrors production. Validate that required releases install cleanly, dependencies meet the target version requirements, automation resumes after the upgrade, and your rollback procedure works as expected.
...
1. Stage the images and release notes for each hop. Use the planning checklist at the beginning of this topic to coordinate backups, notifications, and testing before you promote the upgrade to production.
...
1. Update your configuration to reference the staged Terraform Enterprise image for the target [release](/terraform/enterprise/releases): `images.releases.hashicorp.com/hashicorp/terraform-enterprise:vYYYYMM-#`.
