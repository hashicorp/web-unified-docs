...
3. In the sidebar navigation, select **Support Bundle**.
4. Click **Generate Bundle**.
5. Wait for the bundle generation to complete.
6. Download the generated bundle for your active nodes.

The console generates a separate support bundle for each actively running node in your deployment. You can download bundles for individual nodes or all nodes as needed. 
