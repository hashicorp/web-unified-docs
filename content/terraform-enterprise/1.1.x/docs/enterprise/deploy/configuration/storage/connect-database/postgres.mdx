...
Use AWS IAM to [authenticate](#aws-iam-authentication) with AWS RDS Database for PostgreSQL. Here are a few requirements:
...
You can use Service Accounts to authenticate with DB
Use Google Default Credentials to [authenticate](#google-default-credentials) with SQL Database for PostgreSQL. Here are a few requirements:
