...
Your managed Redis server may require additional configuration. For example, Azure Managed Redis is based on Redis Enteprise, which has additional authentication configuration requirements. Refer to the documentation for your distribution for more information.

### Secure Redis servers

For secure Redis servers, create a user with read and write access. Redis users must have sufficient permissions to authenticate with Terraform Enterprise. 
...
You can connect to either a Redis standalone instance, or a Redis Enterprise instance in non-clustering mode. You can configure authentication for all Redis configurations.
...
- Set the `TFE_REDIS_SIDEKIQ_PASSWORDLESS_AZURE_USE_MSI` variable to `true` if your Sidekiq Redis instance requires connecting using Azure Entra as well.
