...
PostgreSQL 13 reached end of life on November 14, 2025 and is no longer supported.  

</Note>

### Server distribution

You can connect to several PostgreSQL and PostgreSQL-compatible databases. We have tested and verified Terraform Enterprise with the following services: 

- AlloyDB 16. Supported for Terraform Enterprise deployments in GCP only
- [Amazon Aurora PostgreSQL](/terraform/enterprise/deploy/configuration/storage/connect-database/aurora)
- [Patroni PostgreSQL](/terraform/enterprise/deploy/configuration/storage/connect-database/patroni)
