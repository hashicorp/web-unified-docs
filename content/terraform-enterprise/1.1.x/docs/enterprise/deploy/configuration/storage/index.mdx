...
- **Redis data store**: You can configure Terraform to cache application data in a Redis data store. This system is required for some operational modes. When recommend using a Redis data store, even in modes where it is optional, to improve performance. 
