...
| `data.attributes.global-remote-state`                   | boolean         | (previous value) | Enables all workspaces in the organization to [access this workspace's state data](/terraform/enterprise/workspaces/state) during runs. <br/><!-- BEGIN: TFC:only -->When set to `false`, the `project-remote-state` attribute determines if all other workspaces can access the state data. This attribute does not affect access for workspaces that are explicitly granted access the state data.<!-- END: TFC:only --><!-- BEGIN: TFEnterprise:only name:project-remote-state -->Do not set to `true` if `project-remote-state` is also `true`. <!-- END: TFEnterprise:only name:project-remote-state  --> Refer to [Remote State Consumers](/terraform/enterprise/api-docs/workspaces#get-remote-state-consumers) for information about managing allowed workspaces. |
