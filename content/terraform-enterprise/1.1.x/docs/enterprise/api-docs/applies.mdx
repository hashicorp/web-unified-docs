...
An apply represents the results of applying a Terraform workspace execution plan. <!-- BEGIN: TFC:only name:stacks-tfe --> If you are using a Stack and want to interact with Stack deployment runs, refer to [deployment runs](/terraform/enterprise/stacks/runs). <!-- END: TFC:only name:stacks-tfe -->
