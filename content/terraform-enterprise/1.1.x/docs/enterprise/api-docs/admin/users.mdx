...
description: >-
  Use the `/admin/users` endpoint to manage users. Learn how to list, delete, suspend, re-activate, and impersonate users, grant and revoke administrative and site auditor privileges, and disable 2FA.
...
## Grant a user site auditor privileges

`POST /admin/users/:id/actions/grant_site_auditor`

| Parameter | Description                                  |
| --------- | -------------------------------------------- |
| `:id`     | The ID of the user to make a site auditor. |

The Site Auditor role grants read-only, cross-organization visibility to all resources in a Terraform Enterprise instance. This capability enables auditors and security teams to perform comprehensive platform-wide reviews without requiring write access or organization-level membership.

Site Auditors can:
- View all resources across all organizations (workspaces, projects, policy sets, agent pools, variables, etc.)
- Access read-only standard and admin-level API endpoints

Site Auditors cannot:
- Modify any resources (no POST, PUT, PATCH, or DELETE operations)
- Access state files (excluded for security reasons)
- Be service accounts

-> **Note:** This endpoint is only available in Terraform Enterprise and requires the authenticated user to be a site administrator.

| Status  | Response                                | Reason                                             |
| ------- | --------------------------------------- | -------------------------------------------------- |
| [200][] | [JSON API document][] (`type: "users"`) | Successfully granted site auditor privileges.       |
| [400][] | [JSON API error object][]               | Service accounts cannot be granted site auditor role. |
| [404][] | [JSON API error object][]               | User not found, not an enterprise instance, or client is not an administrator. |
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request POST \
  "https://app.terraform.io/api/v2/admin/users/user-ZL4MsEKnd6iTigTb/actions/grant_site_auditor"
```
...
## Revoke a user's site auditor privileges

`POST /admin/users/:id/actions/revoke_site_auditor`

| Parameter | Description                            |
| --------- | -------------------------------------- |
| `:id`     | The ID of the site auditor to revoke privileges from. |

This endpoint revokes a user's site auditor privileges, removing their read-only, cross-organization access.

-> **Note:** This endpoint is only available in Terraform Enterprise and requires the authenticated user to be a site administrator.

| Status  | Response                                | Reason                                             |
| ------- | --------------------------------------- | -------------------------------------------------- |
| [200][] | [JSON API document][] (`type: "users"`) | Successfully revoked site auditor privileges.       |
| [404][] | [JSON API error object][]               | User not found, not an enterprise instance, or client is not an administrator. |
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request POST \
  "https://app.terraform.io/api/v2/admin/users/user-ZL4MsEKnd6iTigTb/actions/revoke_site_auditor"
```
