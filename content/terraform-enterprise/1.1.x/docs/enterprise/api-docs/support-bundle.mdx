...
A support bundle contains diagnostic information that helps troubleshoot issues with a TFE installation. Refer to the [tfectl documentation](/terraform/enterprise/deploy/reference/cli#support-bundle) to see how to generate a support bundle using `tfectl`.
...
> **Note:** You can only create one support bundle request per second otherwise Terraform Enterprise returns a `429 Too Many Requests` error.
...
| `page[number]` | integer | Page number for pagination (defaults to 1) |
| `page[size]` | integer | Number of items per page (defaults to 20, maximum 100) |
...
| `completed_at` | datetime | When the bundle generation completed (only for finished/errored bundles) |
| `size_bytes` | integer | Total size of the bundle in bytes (only for finished bundles) |
...
| `nodes[].size_bytes` | integer | Size of the bundle from this node in bytes (only for finished nodes) |
| `nodes[].error` | string | Error message for this node (only for errored nodes) |
| `nodes[].completed_at` | datetime | When bundle generation completed for this node (only for finished/errored nodes) |
| `error` | string | Error message (only for errored bundles) |
...
| --- | --- | --- |
| `id` | string | Unique identifier for the support bundle request |
| `status` | string | Overall status of the bundle request (`generating`, `finished`, or `errored`). Priority order: if one or more nodes are errored, the overall status is `errored`. If one or more nodes are generating, the overall status is `generating`. Otherwise, the status is `finished`. |
| `created_at` | datetime | When the bundle request was created |
| `completed_at` | datetime | When all bundle generation completed across all nodes (only for finished/errored bundles) |
| `size_bytes` | integer | Total size of all bundles from all nodes in bytes (only for finished bundles) |
| `error` | string | Overall error message if any node failed during generation (only for errored bundles) |
| `nodes` | array of objects | Detailed status information for each node in the bundle |
| `nodes[].node` | string | Node identifier |
| `nodes[].status` | string | Status for this specific node (`generating`, `finished`, or `errored`) |
| `nodes[].size_bytes` | integer | Size of the bundle from this node in bytes (only for finished nodes) |
...
- Body: Binary tar.gz (gzip-compressed tarball) containing the requested support bundle data.
