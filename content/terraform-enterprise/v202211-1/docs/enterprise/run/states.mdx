...
All runs can enter this phase, including [speculative plans](/enterprise/run#speculative-plans).
...
[Sentinel policies]: /enterprise/sentinel
...
The pre-apply phase only occurs if there are enabled [run tasks](/enterprise/workspaces/settings/run-tasks) in the workspace that are configured to begin before Terraform creates the apply. Terraform Cloud sends information about the run to the configured external system and waits for a `passed` or `failed` response to determine whether the run can continue. The information sent to the external system includes the configuration version of the run.
