...
Feature sets represent the different [pricing plans](/enterprise/paid) available to Terraform Cloud organizations. An organization's [entitlement set](/enterprise/api-docs#feature-entitlements) is calculated using its [subscription](/enterprise/api-docs/subscriptions) and feature set.
