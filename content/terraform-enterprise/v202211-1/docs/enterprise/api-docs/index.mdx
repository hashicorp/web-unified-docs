...
Terraform Cloud relies on a HashiCorp-developed blob storage service for storing statefiles and multiple other pieces of customer data, all of which are documented on our [data security page](/terraform/enterprise/system-overview/data-security).
...
-   `private-module-registry` — Allows an organization to publish and use modules with the [private module registry][]. Affects the [registry modules](/terraform/enterprise/api-docs/private-registry/modules) endpoints.
-   `run-tasks` — Allows an organization to use [run tasks](/enterprise/workspaces/settings/run-tasks). Affects the [run tasks](/terraform/enterprise/api-docs/run-tasks/run-tasks) endpoints.
...
[registry modules]: /enterprise/api-docs/modules

[registry providers]: /enterprise/api-docs/providers

[runs]: /enterprise/api-docs/run

[run tasks]: /enterprise/api-docs/run-tasks
