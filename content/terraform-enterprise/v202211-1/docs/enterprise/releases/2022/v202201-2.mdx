...
1. The setting to [require site admins to enable two-factor authentication](/enterprise/admin/application/general#require-site-admins-to-enable-two-factor-authentication) now applies when SAML is enabled.
1. Users who are logged in can now view and manage their active sessions within their user settings.
1. The [CLI integration](/cli/cloud) is now available for using Terraform Enterprise from the command line. We recommend using this native integration for Terraform versions 1.1 or later, as it provides an improved user experience and various enhancements.
...
1. The [new `tfe-admin rotate-encryption-password` command](/enterprise/admin/infrastructure/admin-cli) rotates the encryption password on active/active installations of Terraform Enterprise.
1. A workspace's ID can now be copied from the workspace overview page.
1. New speculative attribute has been added to the [Configuration Versions API](/cloud-docs/api-docs/configuration-versions) response.
