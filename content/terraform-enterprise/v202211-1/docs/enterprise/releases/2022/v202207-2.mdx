...
# TFE Release v202207-2 (642 Required)

### KNOWN ISSUES:

1. Only applicable when using External Vault. You must update your [External Vault](/enterprise/requirements/data-storage/vault) policy to use specific API paths instead of wildcard matching. Skipping this step prevents Terraform Enterprise from starting.
...
1. Using the new `azure_use_msi` and `azure_client_id` [settings](/enterprise/install/automated/automating-the-installer#available-settings), it is now possible to authenticate to Azure Blob Storage with a system-assigned or user-assigned Azure managed identity.
...
1. You can now [scope agent pools](/cloud-docs/agents#scope-an-agent-pool-to-specific-workspaces) to specific workspaces from the Agent Pool settings page. This will allow you to protect sensitive workspaces by restricting which workspaces can target each agent pool.
1. The [Prometheus metrics endpoint](/enterprise/admin/infrastructure/monitoring#terraform-enterprise-metrics) now ships an additional metric `tfe_run_current_count`, which represents the current count of TFE runs in a given workspace, organization, and status.
1. Administrators can use [Admin Settings](/enterprise/api-docs/admin/settings) to set the maximum number of workspaces for any single organization.

### Improvements

1. When [listing workspaces](/enterprise/api-docs/workspaces#list-workspaces), you can now use the `exclude-tags` parameter to exclude workspaces with specific tags.
...
1. The [External Vault](/enterprise/requirements/data-storage/vault) policy has been updated to use specific API paths instead of wildcard matching.
