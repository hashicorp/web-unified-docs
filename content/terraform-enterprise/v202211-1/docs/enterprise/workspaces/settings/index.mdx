...
Workspace settings are separated into several pages, which are listed in the workspace's "Settings" menu. The following groups of settings are available:
...
-   "Health", for enforcing health assessments across eligible workspaces.
...
Every workspace has a unique ID that cannot be changed. Workspace IDs are sometimes necessary when working with [Terraform Cloud's API](/enterprise/api-docs).
...
2.  Complete any runs that are no longer in the [pending stage](/enterprise/run/states#1-the-pending-stage).
...
Which other workspaces within the organization can access the state of the workspace during [runs managed by Terraform Cloud](/enterprise/run#remote-operations). The [`terraform_remote_state` data source](/language/state/remote-state-data) relies on state sharing to access workspace outputs.
...
Refer to [Health Assessments](/enterprise/workspaces/settings/health-assessments) for details.
