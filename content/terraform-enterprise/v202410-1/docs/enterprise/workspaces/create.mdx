...
This topic describes how to create and manage workspaces in Terraform Enteprise and Terraform Enterprise UI. A workspace is a group of infrastructure resources managed by Terraform. Refer to [Workspaces overview](/terraform/enterprise/workspaces) for additional information.

> **Hands-on:** Try the [Get Started - Terraform Enteprise](/terraform/tutorials/cloud-get-started?utm_source=WEBSITE&utm_medium=WEB_IO&utm_offer=ARTICLE_PAGE&utm_content=DOCS) tutorials.
...
-   Terraform Enteprise UI: Refer to [Create a workspace](#create-a-workspace) for instructions.
...
Complete the following steps to create a workspace from the UI:

1.  Log in and choose your organization.
2.  Click **New** and choose **Workspace** from the drop-down menu.
3.  If you have multiple projects, Terraform Enteprise may prompt you to choose the project to create the workspace in. Only users on teams with permissions for the entire project or the specific workspace can access the workspace. Refer to [Manage projects](/terraform/enterprise/projects/manage) for additional information.
...
7.  Add an optional description for the workspace. The description appears at the top of the workspace in the Terraform Enteprise UI.
...
If you connected a VCS repository to the workspace, Terraform Enteprise automatically registers a webhook with your VCS provider. A workspace with no runs will not accept new runs from a VCS webhook, so you must [manually start at least one run](/terraform/enterprise/run/ui#manually-starting-runs).

After you manually start a run, Terraform Enteprise automatically queues a plan when new commits appear in the selected branch of the linked repository or someone opens a pull request on that branch. Refer to [Webhooks](/terraform/enterprise/vcs#webhooks) for more details.
