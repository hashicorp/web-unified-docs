---
page_title: Manage projects in Terraform Enterprise and Terraform Enterprise
description: |-
...
This topic describes how to create and manage projects in Terraform Enterprise. A project is a folder containing one or more workspaces.
...
-   You must be a member of a team with the **Visible** option enabled under **Visibility** in the organization settings to configure a new team's access to the project. Refer to [Team Visibility](/terraform/enterprise/users-teams-organizations/teams#team-visibility) for additional information.
...
2.  Search for a project that you want to view. You can use the following methods:
    ...
    -   Use the search bar to search on the name of a project.
...
You can configure Terraform Enterprise to automatically destroy each workspace's
...
!> **Warning:** Automatic destroy plans _do not_ prompt you for apply approval in the Terraform Enterprise user interface. We recommend only using this setting for development environments.
