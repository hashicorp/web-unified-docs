...
| --- | --- | 
| `POST` `/organizations/:organization_name/workspaces` | Call this endpoint to [create a workspace](#create-a-workspace). |
...
| `GET` `/organizations/:organization_name/workspaces` | Call this endpoint to [list existing workspaces](#list-workspaces). |
...
| `PATCH` `/workspaces/:workspace_id` | Call this endpoint to [update a workspace](#update-a-workspace). |
...
Call the following endpoint to list the tags attached to a workspace:
...
The flat string tags endpoint supports pagination [with standard URL query parameters](/terraform/enterprise/api-docs#query-parameters). Remember to percent-encode `[` as `%5B` and `]` as `%5D` if your tooling doesn't automatically encode URLs.
