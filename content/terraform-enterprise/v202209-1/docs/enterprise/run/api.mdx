...
description: >-
  External tools communicate with Terraform Enterprise to upload configurations
  and trigger runs.
...
~> **Important:** The script below is provided to illustrate the run process, and is not intended for production use. If you want to drive Terraform Cloud runs from the command line, please see the [CLI-driven run workflow](/enterprise/run/cli).
...
You need queue plans permission to create new configuration versions for the workspace. Refer to the [permissions](/enterprise/users-teams-organizations/permissions#general-workspace-permissions) documentation for more details.
...
-   **$TOKEN** is the API Token used for [authenticating with the Terraform Cloud API](/enterprise/api-docs/#authentication).
...
The [configuration version API](/enterprise/api-docs/configuration-versions) requires a `tar.gz` file to use the configuration version for a run, so you must package the directory containing the Terraform configuration into a `tar.gz` file.
...
Terraform Cloud automatically creates a new run with a plan once the new file is uploaded. If the workspace is configured to auto-apply, it will also apply if the plan succeeds; otherwise, an apply can be triggered via the [Run Apply API](/enterprise/api-docs/run#apply). If the API token used for the upload lacks permission to apply runs for the workspace, the run can't be auto-applied. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
In addition to uploading configurations and starting runs, you can use Terraform Cloud's APIs to create and modify workspaces, edit variable values, and more. See the [API documentation](/enterprise/api-docs) for more details.
