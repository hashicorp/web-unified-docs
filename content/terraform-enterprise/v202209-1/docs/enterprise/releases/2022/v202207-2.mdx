...
1. Only applicable when using External Vault. You must update your [External Vault](https://www.terraform.io/enterprise/requirements/data-storage/vault) policy to use specific API paths instead of wildcard matching. Skipping this step prevents Terraform Enterprise from starting.
...
1. You can now [scope agent pools](https://www.terraform.io/enterprise/agents#scope-an-agent-pool-to-specific-workspaces) to specific workspaces from the Agent Pool settings page. This will allow you to protect sensitive workspaces by restricting which workspaces can target each agent pool.
...
1. When [listing workspaces](https://www.terraform.io/enterprise/api-docs/workspaces#list-workspaces), you can now use the `exclude-tags` parameter to exclude workspaces with specific tags.
