...
Account represents the current user interacting with Terraform. It returns the same type of object as the [Users](/enterprise/api-docs/users) API, but also includes an email address, which is hidden when viewing info about other users.
