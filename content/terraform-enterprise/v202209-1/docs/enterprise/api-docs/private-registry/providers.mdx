...
All members of an organization can view and use both public and private providers, but you need [owners team](/enterprise/users-teams-organizations/permissions#organization-owners) or [Manage Private Registry](/enterprise/users-teams-organizations/permissions#manage-private-registry) permissions to add, update, or delete them them in private registry.
...
-   [Authentication](/enterprise/api-docs/#authentication) is handled the same as all other Terraform Cloud endpoints.
...
-   **Private providers:** The private provider record will be available in the organization's registry provider list immediately after creation, but you must [create a version and upload release assets](/enterprise/registry/publish-providers#publishing-a-provider-and-creating-a-version) before consumers can use it. The private registry does not automatically update private providers when you release new versions. You must add each new version with the [Create a Provider Version](/enterprise/api-docs/private-registry/provider-versions-platforms#create-a-provider-version) endpoint.
...
~> **Important:** For private providers, you must also create a version, a platform, and upload release assets before consumers can use the provider. Refer to [Publishing a Private Provider](/enterprise/registry/publish-providers) for more details.
