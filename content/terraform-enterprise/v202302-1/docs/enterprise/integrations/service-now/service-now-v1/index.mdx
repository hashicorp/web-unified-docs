...
-> **\[DEPRECATED] Integration version:** v1.1.0 (Find the latest version [here](/enterprise/integrations/service-now))
...
-   A [supported version control system](/enterprise/vcs#supported-vcs-providers) (VCS) with read access to repositories with Terraform configuration.
...
1.  [Create an organization](/enterprise/users-teams-organizations/organizations) (or choose an existing organization) where ServiceNow will create new workspaces.
2.  [Create a team](/enterprise/users-teams-organizations/teams) for that organization called "ServiceNow", and ensure that it has the organization-level ["Manage workspaces only" permission](/enterprise/users-teams-organizations/permissions#manage-workspaces-only). You do not need to add any users to this team.
3.  On the "ServiceNow" team's settings page, generate a [team API token](/enterprise/users-teams-organizations/api-tokens#team-api-tokens). **Save this API token for later.**
4.  If you haven't yet done so, [connect a VCS provider](/enterprise/vcs) for this Terraform organization.
...
3.  Click on "New" to create a new Terraform Enterprise connection:
    -   Set API Team Token to the Terraform Enterprise [Team Token](/enterprise/users-teams-organizations/api-tokens#team-api-tokens) you created earlier.
...
-> **Note:** Currently, the integration defaults to creating workspaces with [auto-apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) enabled. Since VCS-backed workspaces [start Terraform runs when changes are merged](/enterprise/run/ui), changes to a workspace template repository may cause new runs in any Terraform workspaces created from it.

## Terraform Variables and ServiceNow Variable Sets

ServiceNow has the concept of a Variable Set which is a collection of ServiceNow Variables that can be referenced in a workflow from a Service Catalog item. The Terraform Integration codebase can create [Terraform Variables and Terraform Environment Variables](/enterprise/workspaces/variables) via the API using the `tf_variable.createVariablesFromSet()` function.
