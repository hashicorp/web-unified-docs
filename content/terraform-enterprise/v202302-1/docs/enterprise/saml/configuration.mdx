...
-> **Note:** For instructions for specific IdPs, see [Identity Provider Configuration](/enterprise/saml/idp-configuration).
...
Prior to activating SAML, we recommend that you create a [non-SSO admin account for recovery](/enterprise/saml/troubleshooting#create-a-non-sso-admin-account-for-recovery).
...
- **Team Attribute Name**: (default: `MemberOf`) The name of the SAML attribute that determines [team membership](/enterprise/saml/team-membership). The value of this attribute in the SAML assertion must be either a string containing a comma-separated list of team names or separate [AttributeValue items](/enterprise/saml/attributes#memberof). Team membership mapping is case-sensitive.
