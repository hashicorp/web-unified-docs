...
Changing settings requires admin access to the relevant workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))

[permissions-citation]: #intentionally-unused---keep-for-maintainers

-> **API:** See the [Update a Workspace endpoint](/enterprise/api-docs/workspaces#update-a-workspace) (`PATCH /organizations/:organization_name/workspaces/:name`).
...
Every workspace has a unique ID that you cannot change. You may need to reference the workspace's ID when using the [Terraform Cloud API](/enterprise/api-docs).
...
The [project](/enterprise/workspaces/organize-workspaces-with-projects) that this workspace belongs to. Changing the workspace’s project can change the read and write permissions for the workspace and which users can access it. Refer to [Project Permissions](/enterprise/users-teams-organizations/permissions#project-permissions) for  more detail on workspace access. 
...
To disable remote execution for a workspace, change its execution mode to "Local". This mode lets you perform Terraform runs locally with the [CLI-driven run workflow](/enterprise/run/cli). The workspace will store state and variables, which Terraform can access with the [CLI integration](/cli/cloud).

If you instead need to allow Terraform Cloud to communicate with isolated, private, or on-premises infrastructure, consider using [Terraform Cloud Agents](/cloud-docs/agents). By deploying a lightweight agent, you can establish a simple connection between your environment and Terraform Cloud.
...
1.  Disable [auto-apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) if you have it enabled.
2.  Complete any runs that are no longer in the [pending stage](/enterprise/run/states#the-pending-stage).
3.  [Lock](/enterprise/workspaces/settings#locking) your workspace to prevent any new runs.
4.  Change the execution mode.
5.  Enable [auto-apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply), if you had it enabled before changing your execution mode.
6.  [Unlock](/enterprise/workspaces/settings#locking) your workspace.
...
-   Plans queued by users without permission to apply runs for the workspace must be approved by a user who does have permission. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
-   Plans queued due to [run triggers](/enterprise/workspaces/settings/run-triggers) from another workspace must always be manually applied.
...
> **Hands-on:** Try the [Upgrade Terraform Version in Terraform Cloud](https://learn.hashicorp.com/tutorials/terraform/cloud-versions) tutorial.

-> **API:** You can specify a Terraform version when [creating a workspace](/enterprise/api-docs/workspaces#create-a-workspace) via the API. The API also supports setting a valid [version constraint](/language/expressions/version-constraints) as the Terraform version.
...
In VCS-backed workspaces that specify a working directory, Terraform Cloud assumes that only changes within that working directory should trigger a run. You can override this behavior with the [Automatic Run Triggering](/enterprise/workspaces/settings/vcs#automatic-run-triggering) settings.
...
In [runs triggered by VCS commits](/enterprise/run/ui), this is automatic. In [CLI-driven runs](/enterprise/run/cli), Terraform's CLI sometimes uploads additional content:
...
Which other workspaces within the organization can access the state of the workspace during [runs managed by Terraform Cloud](/enterprise/run/remote-operations#remote-operations). The [`terraform_remote_state` data source](/language/state/remote-state-data) relies on state sharing to access workspace outputs.
...
~> **Important:** Unlike other settings, locks can also be managed by users with permission to lock and unlock the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
See [Run Notifications](/enterprise/workspaces/settings/notifications) for detailed information about configuring notifications.
...
See [Run Triggers](/enterprise/workspaces/settings/run-triggers) for detailed information about configuring run triggers.
...
See [Using SSH Keys for Cloning Modules](/enterprise/workspaces/settings/ssh-keys) for detailed information about this page.

## Team Access

The "Team Access" page configures which teams can perform which actions on a workspace.

See [Managing Access to Workspaces](/enterprise/workspaces/settings/access) for detailed information.
...
See [VCS Connections](/enterprise/workspaces/settings/vcs) for detailed information about this page.
...
After you delete the workspace and its state file, Terraform can no longer track or manage the infrastructure. You must manually delete any remaining resources or [import](/cli/commands/import) them into another Terraform workspace.

By default, [workspace administrators](/enterprise/users-teams-organizations/permissions#workspace-admins) can only delete unlocked workspaces that are not managing any infrastructure. Organization owners can force delete a workspace to override these protections. Organization owners can also configure the [organization's settings](/enterprise/users-teams-organizations/organizations#general) to let workspace administrators force delete their own workspaces.
