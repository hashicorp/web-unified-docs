...
State version output IDs must be obtained from a [state version object](/enterprise/api-docs/state-versions). When requesting a state version, you can optionally add `?include=outputs` to include full details for all of that state version's outputs.
...
This endpoint allows organization users, who do not have permissions to read state versions, to fetch the latest [output values](/language/values/outputs) for a workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))

-> **Note:** Sensitive values are not revealed and will be returned as `null`. To fetch an output including sensitive values see [Show a State Version Output](/enterprise/api-docs/state-version-outputs#show-a-state-version-output).
