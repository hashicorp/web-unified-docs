...
[Run tasks](/terraform/enterprise/workspaces/settings/run-tasks) allow Terraform Cloud to interact with external systems at specific points in the Terraform Cloud run lifecycle.
This page lists the API endpoints used to trigger a run task and the expected response from the integration.

Refer to [run tasks](/terraform/enterprise/api-docs/run-tasks/run-tasks) for the API endpoints to create and manage run tasks within Terraform Cloud. You can also access a complete list of all run tasks in the [Terraform Registry](https://registry.terraform.io/browse/run-tasks). 
...
| `stage`                         | string  | `pre_plan`, `post_plan`, `pre_apply` | The [run stage](/enterprise/run/states) when Terraform Cloud triggers the run task.                                 |
| `is_speculative`                | bool    |                                      | Whether the task is part of a [speculative run](/enterprise/run/remote-operations#speculative-plans).               |
...
| Key path                             | Type   | Values | Description                                                                                                                                                                                         |
| ------------------------------------ | ------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `configuration_version_id`           | string |        | The ID of the [configuration version](/enterprise/api-docs/configuration-versions) for the run.                                                                                                     |
| `configuration_version_download_url` | string |        | The URL to [download the configuration version](cloud-docs/api-docs/configuration-versions#download-configuration-files). This is `null` if the configuration version is not available to download. |
| `workspace_working_directory`        | string |        | The working directory specified in the run's [workspace settings](/enterprise/workspaces/settings#terraform-working-directory).                                                                     |
