...
-> **Note:** Before working with the runs or configuration versions APIs, read the [API-driven run workflow](/enterprise/run/api) page, which includes both a full overview of this workflow and a walkthrough of a simple implementation of it.

Performing a run on a new configuration is a multi-step process.

1.  [Create a configuration version on the workspace](/enterprise/api-docs/configuration-versions#create-a-configuration-version).
2.  [Upload configuration files to the configuration version](/enterprise/api-docs/configuration-versions#upload-configuration-files).
...
| `fetching_completed`   | Terraform Cloud has fetched the configuration from VCS and the run will continue.                                                                                                                                                                                                                                                                                                                                      |
...
| `queuing`              | Terraform Cloud is queuing the run to start the planning phase.                                                                                                                                                                                                                                                                                                                                                        |
| `plan_queued`          | Terraform Cloud is waiting for its backend services to start the plan.                                                                                                                                                                                                                                                                                                                                                 |
...
| `apply_queued`         | Once the changes in the plan have been confirmed, the run will transition to `apply_queued`. This status indicates that the run should start as soon as the backend services that run terraform have available capacity. In Terraform Cloud, you should seldom see this status, as our aim is to always have capacity. However, in Terraform Enterprise this status will be more common due to the self-hosted nature. |
...
The run operation specifies the Terraform execution mode. You can reference the following list of possible operation modes and use them as [runs list](/enterprise/api-docs/run#list-runs-in-a-workspace) query parameters.

| Operation        | Description                                                                                                                                                                        |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `plan_only`      | Indicates the run completed without an apply phase. This is also called a [speculative plan](/enterprise/run/modes-and-options#plan-only-speculative-plan).                        |
| `plan_and_apply` | Indicates the run completed with both plan and apply phases.                                                                                                                       |
| `refresh_only`   | Specifies the run was instructed to execute a plan to update Terraform state.                                                                                                      |
| `destroy`        | Specifies the run destroyed all objects, regardless of configuration changes.                                                                                                      |
| `empty_apply`    | Specifies the run performed an apply with no changes to resources. This is most commonly used to [upgrade terraform state versions](/enterprise/workspaces/state#upgrading-state). |

### Run Sources

You can use the following sources as [runs list](/enterprise/api-docs/run#list-runs-in-a-workspace) query parameters.
...
Creating a run requires permission to queue plans for the specified workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))

When creating a run, you may optionally provide a list of variable objects containing key and value attributes. These values apply to that run specifically and take precedence over variables with the same key applied to the workspace(e.g., variable sets). Refer to [Variable Precedence](/enterprise/workspaces/variables#precedence) for more information. All values must be expressed as an HCL literal in the same syntax you would use when writing Terraform code. Refer to [Types](/language/expressions/types#types) in the Terraform documentation for more details.
...
-> **Note:** This endpoint cannot be accessed with [organization tokens](/enterprise/users-teams-organizations/api-tokens#organization-api-tokens). You must access it with a [user token](/enterprise/users-teams-organizations/users#api-tokens) or [team token](/enterprise/users-teams-organizations/api-tokens#team-api-tokens).
...
| -------------------------------------------------- | -------------------- | -------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `data.attributes.auto-apply`                       | bool                 | The current Auto Apply setting in the workspace    | Whether to automatically apply changes when a Terraform plan is successful. Defaults to the [Auto Apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) workspace setting.                                                                                         |
...
| `data.attributes.refresh-only`                     | bool                 | false                                              | Whether this run should refresh the state without modifying any resources. Mutually exclusive with `is-destroy`.                                                                                                                                                                 |
...
| `data.attributes.variables`                        | array\[{key, value}] | (empty array)                                      | Specifies an optional list of run-specific variable values. Refer to [Run-Specific Variables](/enterprise/workspaces/variables/managing-variables#run-specific-variables) for details.                                                                                           |
| `data.attributes.plan-only`                        | bool                 | (from configuration version)                       | Specifies if this is a [speculative, plan-only](/enterprise/run/modes-and-options#plan-only-speculative-plan) run that Terraform cannot apply. Often used in conjunction with terraform-version in order to test whether an upgrade would succeed.                               |
...
| `data.attributes.allow-empty-apply`                | bool                 |                                                    | Specifies whether Terraform can apply the run even when the plan [contains no changes](/enterprise/run/modes-and-options#allow-empty-apply). Often used to [upgrade state](/enterprise/workspaces/state#upgrading-state) after upgrading a workspace to a new terraform version. |
...
-> **Note:** If the run has a soft failed sentinel policy, you will need to [override the policy check](/enterprise/api-docs/policy-checks#override-policy) before Terraform can apply the run. You can find policy check details in the `relationships` section of the [run details endpoint](#get-run-details) response.

Applying a run requires permission to apply runs for the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
| `filter[operation]` | **Optional.** A comma-separated list of run operations; the result will only include runs that perform one of these operations. Options are listed in [Run Operations](/enterprise/api-docs/run#run-operations).             |
| `filter[status]`    | **Optional.** A comma-separated list of run statuses; the result will only include runs that are in one of these statuses. Options are listed in [Run States](/enterprise/api-docs/run#run-states).                          |
| `filter[source]`    | **Optional.** A comma-separated list of run sources; the result will only include runs that came from one of these sources. Options are listed in [Run Sources](/enterprise/api-docs/run#run-sources).                       |
| `search[user]`      | **Optional.** Searches runs that matches the supplied VCS username.                                                                                                                                                          |
| `search[commit]`    | **Optional.** Searches runs that matches the supplied commit sha.                                                                                                                                                            |
| `search[basic]`     | **Optional.** Searches runs that matches the supplied VCS username, commit sha, run_id, and run message. The presence of search[commit] or search[user] takes priority over this parameter and will cause it to not be used. |
...
Discarding a run requires permission to apply runs for the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
Canceling a run requires permission to apply runs for the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
The `force-cancel` action is like [cancel](#cancel-a-run), but ends the run immediately. Once invoked, the run is placed into a `canceled` state, and the running Terraform process is terminated. The workspace is immediately unlocked, allowing further runs to be queued. The `force-cancel` operation requires admin access to the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
Force-executing a run requires permission to apply runs for the workspace. ([More about permissions.](/enterprise/users-teams-organizations/permissions))
...
The GET endpoints above can optionally return related resources, if requested with [the `include` query parameter](/enterprise/api-docs#inclusion-of-related-resources). The following resource types are available:
