...
[organization settings]: /enterprise/users-teams-organizations/organizations#organization-settings
...
-   [Teams API](/enterprise/api-docs/teams) to list, create, update, and delete teams
-   [Team Members API](/enterprise/api-docs/team-members) to add and delete users from teams
-   [Team Tokens API](/enterprise/api-docs/team-tokens) to generate and delete tokens
-   [Team Access API](/enterprise/api-docs/team-access) to manage team access to one or more workspaces
...
Refer to [Organization Owners](/enterprise/users-teams-organizations/permissions#organization-owners) for more details about owners team permissions.
...
are members of. Refer to [Team Visibility](/enterprise/users-teams-organizations/teams#team-visibility) for details.
...
2.  Click a team to go to its settings page, which lists the team's settings and current members. Members that have [two-factor authentication](/enterprise/users-teams-organizations/2fa) enabled have a **2FA** badge.
...
If the user is not yet in the organization, [invite them to join the organization](/enterprise/users-teams-organizations/organizations#users) and include a list of teams they should belong to in the invitation. Once the user accepts the invitation, Terraform Cloud automatically adds them to those teams.
...
[organization-level permissions](/enterprise/users-teams-organizations/permissions#organization-permissions), since workspace admins cannot manage permissions for teams they cannot view.

[permissions-citation]: #intentionally-unused---keep-for-maintainers

### API Tokens

Each team can have an API token that is not associated with a specific user. You can manage this API token from the team's settings page under **Team API Token** . Refer to [Team API Tokens](/enterprise/users-teams-organizations/api-tokens#team-api-tokens) for details.

## Managing Workspace Access

You can grant teams various permissions on workspaces. Refer to [Workspace Permissions](/enterprise/users-teams-organizations/permissions#workspace-permissions) for details.
...
You can grant teams permissions to manage a project and the workspaces that belong to it. Refer to [Project Permissions](/enterprise/users-teams-organizations/permissions#project-permissions) for details.

## Managing Organization Access

Organization owners can grant teams permissions to manage policies, projects and workspaces, VCS settings, private registry providers and modules, and policy overrides across an organization. Refer to [Organization Permissions](/enterprise/users-teams-organizations/permissions#organization-permissions) for details.

## Managing Organization Access

Organization owners can grant teams permissions to manage policies, workspaces, VCS settings, private registry providers and modules, and policy overrides across an organization. Refer to [Organization Permissions](/enterprise/users-teams-organizations/permissions#organization-permissions) for details.
