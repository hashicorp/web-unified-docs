...
* [CLI-based commands](/enterprise/admin/infrastructure/admin-cli) for administration instead of the Replicated Admin Console
...
As mentioned above, the Active/Active architecture requires an existing [automated installation](/enterprise/install/automated/automating-the-installer) of Terraform Enterprise that follows our [best practices for deployment](/enterprise/reference-architecture).
...
The application itself must be using [External Services](/enterprise/install/pre-install-checklist#operational-mode-decision) mode to connect to an external PostgreSQL database and object storage.
...
* **Port 8201** must be open between the nodes to allow Vault to run in [High Availability](https://developer.hashicorp.com/vault/docs/internals/high-availability) mode
...
Refer to the [Admin CLI Commands](/enterprise/admin/infrastructure/admin-cli) documentation for more status and troubleshooting commands.
...
Redis on VMware was tested with a virtual machine with 2 CPUs and 8 GB of memory running Ubuntu 20.04. Both Redis v5 and v6 are supported. A full list of supported Operating Systems can be found on the [Pre-Install Checklist](/enterprise/install/pre-install-checklist#operating-system-requirements).
