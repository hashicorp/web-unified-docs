...
Before you begin, consult the [Pre-Install Checklist](/enterprise/install/pre-install-checklist) for prerequisites. You'll need to prepare data files and a Linux instance.

-> **Compatibility warning**: Installation requires the `docker-version=20.10.17` flag as of July 5th, 2023. If you do not provide this flag, Docker v24 is installed, which is _not_ compatible with this version of Terraform Enterprise.
...
Unless the execution mode of a workspace is set to "local", Terraform Enterprise performs [remote operations](/enterprise/run/remote-operations), running Terraform in its own worker VMs.

When running within Terraform Enterprise's worker VMs, Terraform uses [service discovery](/internals/remote-service-discovery) to find the Terraform Enterprise service itself. Depending on your infrastructure setup, you may need to tell Terraform Enterprise not to access its own hostname via the proxy, so that Terraform can communicate with the Terraform Enterprise services.
...
You can use the equivalent setting, [extra_no_proxy](/enterprise/install/automated/automating-the-installer#available-settings), to automate this step.
...
built-in container image by default. To add custom tools or logic to your
Terraform run environment, you must build a custom image and configure
Terraform Enterprise to use it.

The process for building a custom image differs depending on whether your
`run_pipeline_mode` setting is set to `agent` or `legacy`. As of v202302-1, the
default `run_pipeline_mode` is `agent`.

See the
[release notes](/terraform/enterprise/releases/2023/v202302-1)
and the
[worker to agent migration guide](/terraform/enterprise/admin/infrastructure/worker-to-agent-migration)
for more information about the run pipeline mode change.
...
-> **Note:** Custom scripts are for the `legacy` run pipeline mode only. As of Terraform Enterprise v202302-1, the default run pipeline mode is `agent`. If you wish to use a custom image for the `legacy` run pipeline mode, you must [build a legacy custom image](#legacy). See the [release notes](/terraform/enterprise/releases/2023/v202302-1) and the [worker to agent migration guide](/terraform/enterprise/admin/infrastructure/worker-to-agent-migration) for more information about the run pipeline mode change. Support for the `legacy` run pipeline mode and legacy custom images will end in v202305-1.
...
[recovering from failures](/enterprise/admin/infrastructure/automated-recovery).
The mode should be selected based on your organization's needs. See
[Pre-Install Checklist: Operational Mode Decision](/enterprise/install/pre-install-checklist#operational-mode-decision)
for more details.
...
1. From a shell on your instance:
   - To execute the installer directly, run `curl https://install.terraform.io/ptfe/stable | sudo bash docker-version=20.10.17`.
   - To inspect the script locally before running, run `curl https://install.terraform.io/ptfe/stable > install.sh docker-version=20.10.17` and, once you are satisfied with the script's content, execute it with `sudo bash install.sh`.
...
1. Airgap installations require Docker to be pre-installed. Double-check that your instance has a supported version of Docker (see [Pre-Install Checklist: Software Requirements](/enterprise/install/pre-install-checklist#software-requirements) for details).
...
1. Run `sudo ./install.sh airgap docker-version=20.10.17` or `sudo ./install.sh airgap` if you have pre-installed Docker.
...
1. Set an encryption password used to encrypt the sensitive information at rest. The default value is auto-generated,
   ...
   password to restore access to the data in the event of a reinstall. See [Encryption Password](/enterprise/install/automated/encryption-password) for more information.
1. Configure the operational mode for this installation. See
   [Pre-Install Checklist: Operational Modes](/enterprise/install/pre-install-checklist#operational-mode-decision) for information on what the different values are. Ensure that you've met the relevant pre-install requirements for the mode you chose.
...
1. _Optional:_ Configure the product to use an externally managed Vault cluster.
   See [Externally Managed Vault Cluster](/enterprise/requirements/data-storage/vault) for details about the required Vault configuration before using this option.
...
page](/enterprise/install/interactive/config) to continue setting up Terraform Enterprise.
