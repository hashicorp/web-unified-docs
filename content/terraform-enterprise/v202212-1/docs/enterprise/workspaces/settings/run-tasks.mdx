...
Run tasks perform actions between the [plan](/enterprise/run/states#the-plan-stage) and [apply](/enterprise/run/states#the-apply-stage) stages of a [Terraform run](/enterprise/run/remote-operations). Once all run tasks complete, the run ends based on the most restrictive enforcement level in each associated run task.
