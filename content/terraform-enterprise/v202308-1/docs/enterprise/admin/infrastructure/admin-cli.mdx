...
This command rotates the [encryption password](/terraform/enterprise/install/automated/encryption-password) in use by Terraform Enterprise.
...
* Follow the instructions in [Terraform Enterprise Active/Active](/terraform/enterprise/install/automated/active-active#scale-down-to-zero-nodes) to scale down to zero nodes and proceed through scaling up to one node, validating success, and then scaling additional nodes.
