...
- [Data Security](/terraform/enterprise/system-overview/data-security) for details about the contents of Terraform Enterprise's blob storage and PostgreSQL database.
...
If the Replicated console dashboard is unavailable, execute `replcatedctl app-config export --template '{{back_uptoken.Value}}'` from the TFE node shell to get the backup API authorization token.
