...
1. The `install.sh` script and method used must be from the Replicated airgap installer boostrapper, using the process described for [airgap installation](/terraform/enterprise/install/interactive/installer#run-the-installer-airgapped).
