...
An example set of log entries emitted by a TFE container would appear as follows:
...
This means it is the responsibility of the TFE operator to mount the config
...
Exposing FluentBit configuration to TFE operators is provided as a convenience to
...
The FluentBit solution provided in [legacy Replicated TFE deployments](/terraform/enterprise/replicated/monitoring/logging)
