...
description: >-
  Use the `/account` endpoint to manage the current user. Get user details,
...
For internal reasons, Terraform Cloud associates team and organization tokens with a synthetic user account called _service user_. Terraform Cloud returns the associated service user for account requests authenticated by a team or organization token. Use the `authenticated-resource` relationship to access the underlying team or organization associated with a token. For user tokens, you can use the user, itself.
