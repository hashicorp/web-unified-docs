---
page_title: Terraform Enterprise Operator for Kubernetes
description: >-
  Terraform Enterprise Operator allows you to provision infrastructure directly
...
The [Terraform Cloud Operator for Kubernetes](https://github.com/hashicorp/terraform-cloud-operator) allows you to manage Terraform Cloud resources with Kubernetes custom resources. You can provision infrastructure internal or external to your Kubernetes cluster directly from the Kubernetes control plane. 

The operator's CustomResourceDefinitions (CRD) let you dynamically create Terraform Cloud workspaces with Terraform modules, populate workspace variables, and provision infrastructure with Terraform runs.

## Key benefits

The Terraform Cloud Operator v2 offers several improvements over v1:

-   **Flexible resource management**: The operator now features multiple custom resources, each with separate controllers for different Terraform Cloud resources. This provides additional flexibility and the ability to manage more custom resources concurrently, significantly improving performance for large-scale deployments.
...
-   **Configurable synchronization**: The `--sync-period` option allows you to configure the synchronization frequency between custom resources and Terraform Cloud, ensuring timely updates and smoother operations.

## Supported Terraform Cloud features

The Terraform Cloud Operator for Kubernetes allows you to create agent pools, deploy modules, and manage workspaces through Kubernetes controllers. These controllers enable you to automate and manage Terraform Cloud resources using custom resources in Kubernetes.

### Agent pools

Agent pools in Terraform Cloud manage the execution environment for Terraform runs. The Terraform Cloud Operator allows you to create and manage agent pools as part of your Kubernetes infrastructure.
...
<CodeBlockConfig highlight="17-23">
...
Terraform Cloud workspaces organize and manage Terraform configurations. The Terraform Cloud Operator for Kubernetes allows you to create, configure, and manage workspaces directly from Kubernetes.
...
In the above example, the `applyMethod` has the value of `auto`, so Terraform Cloud automatically applies any changes to this workspace. The specification also configures the workspace to use the `ap-us-west-development` agent pool and run the `rt-us-west-development` run task at the `pre_plan` stage.
