...
This document expects that the user is already familiar with how to do a [manual install](/terraform/enterprise/replicated/install/interactive/installer#installation).
...
- `enc_password` — (Required) The [password](/terraform/enterprise/replicated/install/automated/encryption-password) used to encrypt and decrypt the internally-managed Vault unseal key and root token. Not required only when opting out of internally-managed Vault.
...
- `iact_subnet_list` - A comma-separated list of CIDR masks that configure the ability to retrieve the [IACT](/terraform/enterprise/replicated/install/automated/automating-initial-user) from outside the host. For example: "10.0.0.0/24, 10.0.1.0/24". If not set, no subnets can retrieve the IACT.

- `iact_subnet_time_limit` - The time limit that requests from the subnets listed can request the [IACT](/terraform/enterprise/replicated/install/automated/automating-initial-user), as measured from the instance creation in minutes; defaults to 60.
...
- `log_forwarding_enabled` - (Optional) Whether or not to enable [log forwarding](/terraform/enterprise/replicated/monitoring/logging) for Terraform Enterprise. Set to `1` to enable log forwarding and `0` to disable log forwarding. Defaults to `0`.

- `log_forwarding_config` - (Optional) Valid [log forwarding](/terraform/enterprise/replicated/monitoring/logging) configuration specifying external destination(s) to forward logs.

- `metrics_endpoint_enabled` - (Optional) Enable an endpoint to [expose container metrics](/terraform/enterprise/replicated/monitoring/monitoring#terraform-enterprise-metrics). Defaults to `0`.
...
If you want to use a [custom image](/terraform/enterprise/replicated/install/interactive/installer#custom-image), the following settings apply:

For a [legacy custom image](/terraform/enterprise/replicated/install/interactive/installer#legacy):
...
For an [agent custom image](/terraform/enterprise/replicated/install/interactive/installer#agent):
