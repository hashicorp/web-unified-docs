...
description: >-
  Use the `/oauth-clients` endpoint to manage OAuth connections between your
  organization and a VCS provider. List, show, create, update, and destroy
...
An OAuth client represents the connection between an organization and a VCS provider. By default, you can globally access an OAuth client throughout the organization, or you can have scope access to one or more [projects](/terraform/enterprise/projects/managing).
...
~> **Note:** You cannot call this endpoint to create a Bitbucket Cloud, Bitbucket Data Center, or Azure DevOps Services OAuth Client.
