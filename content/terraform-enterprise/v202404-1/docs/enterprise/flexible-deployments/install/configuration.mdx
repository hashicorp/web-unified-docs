---
page_title: Configuration Reference - Installation - Flexible Deployment Options - Terraform Enterprise
description: >-
...
last_modified: 2026-09-04T14:33:52.000Z
...
variables. Specify these environment variables in your
[Compose file](https://docs.docker.com/compose/compose-file/03-compose-file/) (Docker)
or in your [Helm chart](https://helm.sh/) (Kubernetes).
...
Redis server user. This value is reuqired when `TFE_REDIS_USE_AUTH` is set to `true`, and Redis server is configured to use username for authentication. There is no need to set this config when running Redis server internally as credential is not required.
...
Refer to [the logging documentation](/terraform/enterprise/flexible-deployments-beta/monitoring/observability/logs)
...
This is a base64-encoded custom pod template in JSON format, equivalent to Kubernetes' `corev1.PodTemplateSpec`, used in a job. Terraform Enterprise configuration values will override corresponding values in the pod template:

* **Restart Policy**: Always set to `RestartPolicyNever` to prevent the Pod from restarting on failure or completion.
* **Labels**: Added if provided. Terraform Enterprise settings take precedence if a label key exists.
* **Image Pull Secret**: If `TFE_RUN_PIPELINE_KUBERNETES_IMAGE_PULL_SECRET_NAME` is provided, it is used.
* **Containers**: Only one `corev1.Container` is allowed.
  * Container name is set from Terraform Enterprise internal configuration.
  * Image is either the default Terraform Enterprise image or from `TFE_RUN_PIPELINE_IMAGE`.
  * Work directory is from Terraform Enterprise configuration.
* **Environment Variables**: Added if provided. Terraform Enterprise settings take precedence if a key exists.
* **Requests and Limits**: `TFE_CAPACITY_CPU` and `TFE_CAPACITY_MEMORY` values are used for the pod requests and limits, if present.


## Proxy configuration

Flexible Deployment Options now relies on the native application runtime for setting proxy values. For configuration in Docker
refer to the offical [Docker proxy configuration guide](https://docs.docker.com/network/proxy/); in the case of Kubernetes, rely on the
[Kubernetes proxy configuration guide](https://kubernetes.io/docs/concepts/cluster-administration/proxies/).
...
Refer to [settings](/terraform/enterprise/flexible-deployments/install/configuration#application-settings) for more information about the variables.

| Replicated                  | Flexible Deployment Options                                                                                         |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------- |
