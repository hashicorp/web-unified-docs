...
<Note>
The `tfectl app status` command is marked for deprecation as of 1.2.0, and will be removed in the upcoming major release. Use `tfectl app diagnostics` instead.
</Note>
...
Set a custom timeout (in seconds, 1-300, default 30):

```shell-session
$ tfectl app diagnostics --timeout 60
```

Filter checks to run (format: group or group.subcheck, comma-separated):
...
Note: When the database is unavailable, diagnostics can only be fetched from the current node. Running diagnostics on remote nodes (--node, --all) requires database connectivity.
...
Set a custom timeout (in seconds, 1-30, default 1):
