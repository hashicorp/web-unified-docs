...
## Explorer settings for passworless IAM authentication
...
200 OK with a JSONAPI structure having the backfill status:
...
The `STATUS` value is one of `"not started"`, `"in progress"`, or `"done"`.

The endpoint will return a 4xx error code on invalid requests (non-GET method, missing or non-administrator authentication token, explorer not enabled).
