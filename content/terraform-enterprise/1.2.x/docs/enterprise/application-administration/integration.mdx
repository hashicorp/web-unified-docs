...
[flow](https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth#use-client-credentials-grant-flow-to-authenticate-smtp-imap-and-pop-connections).

-> **Note:** Refer to [Microsoft's documentation](https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth) for instructions on registering the application in Microsoft Entra and configuring the necessary mailbox settings.
...
* **Sender Email**: The address that system mails should come from. A plain email address; do not include a display name.
* **Send test email to**: A sample address to send a test email to. Used to validate the settings when configuring SMTP; not stored.
* **Host** and **Port**: The host is `smtp.office365.com`, and port is `587`.
* **Client ID**: The application ID that authenticates with the SMTP server. Register the Terraform
Enterprise application in Microsoft Entra and enter the generated Client ID. Refer to [Microsoft's
documentation](https://learn.microsoft.com/en-us/exchange/client-developer/legacy-protocols/how-to-authenticate-an-imap-pop-smtp-application-by-using-oauth) for registration instructions.
