---
page_title: Kubernetes and OpenShift - Requirements - Flexible Deployment Options - Terraform Enterprise
description: >-
...
Place your custom `tfc-agent` image in a container registry accessible from the OpenShift nodes that will host Terraform Enterprise.
Automatic CA certificate injection is not available for custom `tfc-agent` sourced images and may be required as an additional configuration in this Dockerfile. 
