...
Flexible Deployment Options supports migrations between instances of the application on the same version. We do not support combining an application upgrade and migrating from Replicated to Flexible Deployment Options. Upgrade your current installation of TFE to the latest version before proceeding with your Flexible Deployment Options migration.
...
1. Upgrade your current Terraform Enterprise (Replicated) installation to a FDO-compatible Terraform Enterprise version - `v202309-1` or later. Migrations from Replicated to Flexible Deployment Options are only supported between instances on the same version.
...
In order to meet the [Terraform Enterprise Nomad Installation Requirements](/terraform/enterprise/flexible-deployments/install/nomad/requirements), you must provide an external Postgres server, external object storage, and external Redis storage.

You must complete additional steps to migrate a Terraform Enterprise deployment on Replicated in [`disk` mode](/terraform/enterprise/operational-modes#operational-modes): 

1. [Migrate your Replicated deployment to `external` mode](/terraform/enterprise/replicated/administration/infrastructure/mounted-to-external-migration) 
...
1. Complete the steps for migrating to Nomad in `external` mode. Refer to [External Services to Nomad migration](/terraform/enterprise/replicated/replicated-migration#external-services-or-active-active-to-nomad) for instructions.
...
If you are migrating a Replicated deployment in `active-active` operational mode, you should already have all the required service dependencies. Refer to [Terraform Enterprise Nomad Installation Requirements](/terraform/enterprise/flexible-deployments/install/nomad/requirements) for more details on the requirements for running Terraform Enterprise on Nomad.
...
If you need to revert at any point, refer to [Nomad rollback steps](/terraform/enterprise/replicated/replicated-migration#nomad-rollback-steps) for instructions.
...
1. Create a Nomad job file for Terraform Enterprise using [Terraform Enterprise Nomad job specification](/terraform/enterprise/flexible-deployments/install/nomad/install#1-requirements).

1. In the [Terraform Enterprise Nomad job specification](/terraform/enterprise/flexible-deployments/install/nomad/install#1-requirements), specify the following values:
   1. The `TFE_OBJECT_STORAGE_TYPE` and `TFE_OBJECT_STORAGE_*` variables should specify the object storage type and the container or bucket credentials from your Replicated installation.
   1. The `TFE_DATABASE_*` variables should specify database credentials from the Replicated installation.
   1. The `TFE_REDIS_*` values on the Helm chart should specify the same credentials from the external Redis in your Replicated installation.
   ...
   1. The `TFE_ENCRYPTION_PASSWORD` value should match the Replicated installation value. You can get this from your Replicated instance by connecting to the instance over SSH and running the following command:
      ...
      $ replicatedctl app-config export --template '{{ .enc_password.Value }}'
      ```
      Refer to the [Replicated to Flexible Deployment Options configurations mapping](/terraform/enterprise/flexible-deployments/install/configuration#replicated-to-flexible-deployment-options-configuration-mapping) for more information on how the Replicated configuration maps to the variables and secrets on Terraform Enterprise job specification.
...
1. Install Terraform Enterprise on Nomad. Refer to [Nomad installation guide](/terraform/enterprise/flexible-deployments/install/nomad/install) for instructions.
