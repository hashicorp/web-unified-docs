...
In addition to [adding modules from the Terraform Registry](/enterprise/registry/add), you can publish private modules to an organization's Terraform Cloud private registry. The registry handles downloads and controls access with Terraform Cloud API tokens, so consumers do not need access to the module's source repository, even when running Terraform from the command line.
...
Members of the [owners team](/enterprise/users-teams-organizations/permissions#organization-owners) and teams with [Manage Private Registry permissions](/enterprise/users-teams-organizations/permissions#manage-private-registry) can publish and delete modules from the private registry.
...
You can publish modules through the UI as shown below or with the [Registry Modules API](/enterprise/api-docs/private-registry/modules). The API also supports publishing modules without a VCS repo as the source, which is not possible in the UI.
...
1.  (Optional) If this module is a [no-code ready module](/enterprise/no-code-provisioning/module-design), select the **Add Module to no-code provision allowlist** checkbox.
