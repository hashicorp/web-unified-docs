...
When you enable it, you must specify the name of a SAML attribute in the [Team Attribute Name](/enterprise/saml/configuration#attributes) setting, and make sure the AttributeStatement in the SAMLResponse contains a list of AttributeValue items in the correct format.
...
If the "Site Admin Role" setting (in [the SAML settings](/enterprise/saml/configuration)) is enabled, the selected special team name (default: `site-admins`) will add a user as a [site admin](/enterprise/admin) for the Terraform Enterprise instance.
