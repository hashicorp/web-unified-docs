...
-> **Note:** Workspaces configured with `Local` [execution mode](/enterprise/workspaces/settings#execution-mode) do not support notifications.
...
-   **Workspace Events**: Terraform Cloud can send notifications for all events or only for specific events. The following events are available:
    -   **Drift**: Terraform Cloud detected configuration drift. This notification is only available if you enable [health assessments](/enterprise/workspaces/health) for the workspace.
...
-   **Run Events:** Terraform Cloud can send notifications for all events or only for specific events. The following events are available:
    -   **Created**: A run begins and enters the [Pending stage](/enterprise/run/states#the-pending-stage).
    -   **Planning**: A run acquires the lock and starts to execute.
    -   **Needs Attention**: A plan has changes and Terraform requires user input to continue. This event may include approving the plan or a [policy override](/enterprise/run/states#the-policy-check-stage).
    -   **Applying**: A run enters the [Apply stage](/enterprise/run/states#the-apply-stage), where Terraform makes the infrastructure changes described in the plan.
