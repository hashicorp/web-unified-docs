...
The [Vault Transit Secret Engine](https://www.vaultproject.io/docs/secrets/transit/index.html)
...
through the [Vault transit secret engine](https://www.vaultproject.io/docs/secrets/transit/index.html),
...
[datakey generation](https://www.vaultproject.io/api-docs/secret/transit#generate-data-key) creates the encryption key material using bit material from the kernel's cryptographically secure pseudo-random
number generator (CSPRNG) as the `context` value. Blob storage encryption generates a unique key for each object and relies on envelope encryption, so Vault does not rotate the encryption key material for individual objects.
