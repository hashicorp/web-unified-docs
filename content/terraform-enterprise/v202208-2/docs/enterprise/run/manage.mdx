...
Refer to the [Runs API](/enterprise/api-docs/run) and [lock a Workspace endpoint](/enterprise/api-docs/workspaces#lock-a-workspace).
...
In workspaces where you have [permission to apply runs](/enterprise/users-teams-organizations/permissions#general-workspace-permissions), you can interact with a run at the bottom of its details page.
...
If a plan needs confirmation (with [manual apply](/enterprise/workspaces/settings#auto-apply-and-manual-apply) enabled) or a soft-mandatory policy failed, the run remains paused until a user with appropriate permissions uses these buttons to continue or discard the run. Refer to [Run States and Stages](/enterprise/run/states) for more details.

### Canceling Runs

If a run is currently planning or applying, users with [permission to apply runs](/enterprise/users-teams-organizations/permissions#general-workspace-permissions) for the workspace can click **Cancel Run** to stop the run before it finishes.
...
You can lock the workspace to temporarily stop runs from being queued. Locking a workspace requires [permission to lock and unlock the workspace](/enterprise/users-teams-organizations/permissions#general-workspace-permissions).
