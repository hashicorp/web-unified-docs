...
| Needs Attention    | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This may include approving the plan or a [policy override](/enterprise/run/states#7-the-policy-check-stage).          |
| Applying           | `"run:applying"`        | A run enters the [Apply stage](/enterprise/run/states#8-the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                                       |
