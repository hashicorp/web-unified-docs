...
1. The setting to [require site admins to enable two-factor authentication](/terraform/enterprise/admin/application/general#require-site-admins-to-enable-two-factor-authentication) now applies when SAML is enabled.
...
1. The [new `tfe-admin rotate-encryption-password` command](/terraform/enterprise/admin/infrastructure/admin-cli) rotates the encryption password on active/active installations of Terraform Enterprise.
