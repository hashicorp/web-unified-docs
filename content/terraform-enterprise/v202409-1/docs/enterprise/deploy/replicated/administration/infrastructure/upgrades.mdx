...
version. [Learn more about availability during upgrades here](/terraform/enterprise/deploy/replicated/architecture/system-overview/reliability-availability#availability-during-upgrades).

## Before Upgrade

We strongly recommend the following:

- Create a backup copy of the storage prior to upgrading your instance. Backup and restore responsibility varies depending on your Terraform Enterprise [operation mode](/terraform/enterprise/deploy/replicated/architecture/system-overview/reliability-availability#operation-modes).
- Review the general [Terraform Enterprise requirements](/terraform/enterprise/deploy/replicated/requirements/credentials), including credentials, hardware, operating system, data storage, and network.
...
- Review the [Upgrading Terraform Enterprise or Patching Terraform Enterprise Node Instances](/terraform/enterprise/deploy/replicated/administration/infrastructure/admin-cli#upgrading-tfe-or-patching-tfe-node-instances) for your Terraform Enterprise active/active architecture.
