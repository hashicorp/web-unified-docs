...
description: >-
  Learn how to create the initial admin user in Terraform Enterprise.
...
After installing Terraform Enterprise, you must create an admin user so that you can manage the Terraform Enterprise application and run workloads.
...
Navigate to `https://${TFE_HOSTNAME}/admin/retrieve-iact` in your browser from a workstation within the [subnet list](/terraform/enterprise/deploy/reference/configuration#tfe_iact_subnets). You can access your token on this webpage.
...
Navigate to `https://${TFE_HOSTNAME}/admin/account/new?token=${IACT_TOKEN}` in your browser from a workstation within the [subnet list](/terraform/enterprise/deploy/reference/configuration#tfe_iact_subnets). Follow the prompts to create the admin user.
