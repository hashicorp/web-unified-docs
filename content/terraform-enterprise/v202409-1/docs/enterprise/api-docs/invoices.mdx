...
description: >-
  Use the `invoices` endpoint to access an organization's invoices. List
...
It uses a pagination scheme that's somewhat different from [our standard pagination](/terraform/enterprise/api-docs#pagination). The page size is always 10 items and is not configurable; if there are no more items, `meta.continuation` will be null. The current page is controlled by the `cursor` parameter, described below.
