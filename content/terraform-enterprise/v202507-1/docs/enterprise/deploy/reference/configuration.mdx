...
last_modified: 2026-09-04T14:33:58.000Z
...
Must be `true` or `false`. `true` indicates to use mutual TLS (mTLS) authentication for clients to access Redis with Redis standalone. Defaults to `false`. mTLS is not currently supported in Sentinel mode. Can not be `true` if `TFE_REDIS_USER` and `TFE_REDIS_PASSWORD` is specified.
