...
| `data.attributes.default-agent-pool-id`                                   | string  | (previous value) | Required when `default-execution-mode` is set to `agent`. The ID of the agent pool belo  nging to the organization. Do _not_ specify this value if you set `execution-mode` to `remote` or `local`.                                                                                                                                                                                |
...
This endpoint shows organizations that are configured to share modules with an organization through module sharing. Refer to the [`module-consumers` endpoint](/terraform/enterprise/api-docs/admin/module-sharing) for more information.
