...
For plans created under HYOK enabled workspaces, a sanitized version of the plan file with sensitive information redacted will be available for download.
These endpoints generate a temporary authenticated URL to the location of the sanitized execution plan. When successful, this endpoint responds with a temporary redirect that should be followed.  If using a client that can follow redirects, you can use this endpoint to save the `.json` file locally without needing to save the temporary URL.
...
| [422][] | [JSON API error object][] | Sanitized plan does not use a supported version of terraform (&lt; 0.12.X) |
