...
All runs can enter this phase, including [speculative plans](/enterprise/run/modes-and-options#plan-only-speculative-plan).
...
for the workspace. All runs can enter this phase, including [speculative plans](/enterprise/run/modes-and-options#plan-only-speculative-plan). During this phase, Terraform Cloud sends information about the run to the configured external system and waits for a `passed` or `failed` response to determine whether the run can continue.
