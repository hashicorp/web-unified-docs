...
description: >-
  Use run tasks to make requests when a run reaches a specific phase. Learn
...
| `stage`                              | string  | `pre_plan`, `post_plan`, `pre_apply`, `post_apply` | The [run stage](/terraform/enterprise/run/states) when Terraform Enterprise triggers the run task.                                                                                                                  |
...
The PATCH request submits a JSON object with the following properties as a request payload. This payload is also described in the [JSON schema for run task results](https://github.com/hashicorp/web-unified-docs-internal/blob/main/content/terraform-docs-common/public/schema/run-tasks/runtask-result.json).
...
```json
{
  ...
  "attributes": {
    ...
    "tags": {
      "Status":  [{"label": "Denied", "level": "error" }],
      "Severity": [
        {"label": "High", "level": "error" },
        {"label": "Recoverable", "level": "info" },
      ],
      "Cost Centre": [{"label": "IT-OPS"}]
    },
