...
description: >-
  Use the `/registry-modules` endpoint to manage modules published to an
  organization's private registry. List, get, create, publish, and delete
...
The Terraform Cloud Module Registry implements the [Registry standard API](/terraform/registry/api-docs) for consuming/exposing private modules. Refer to the [Module Registry HTTP API](/terraform/registry/api-docs) to perform the following:
...
-   [Authentication](/terraform/enterprise/api-docs#authentication) is handled the same as all other Terraform Cloud endpoints.
