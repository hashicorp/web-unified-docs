...
description: >-
  Use the `notification-configurations` endpoint to manage notification
  configurations. List, show, create, update, verify, and delete notification
...
| Auto Destroy Results     | `"workspace:auto_destroy_run_results"` | Terraform Cloud attempted an automated workspace destroy run.                                                                                                                                |
...
Workspace events include detailed information about workspace-level validation events like [health assessments](/terraform/enterprise/workspaces/health) if you enable them for the workspace. Much of the information provides details about the associated [assessment result](/terraform/enterprise/api-docs/assessment-results), which Terraform Cloud uses to track instances of continuous validation.

Terraform Cloud returns different types of attributes returned in the payload details, depending on the type of `trigger_scope`. There are two main values for `trigger_scope`: `assessment` and `workspace`, examples of which you can see below.
...
| `notification_configuration_url`   | string | The URL to get the notification's configuration from the Terraform Cloud API.                                |
| `trigger_scope`                    | string | Always "workspace" for ephemeral workspace notifications                                                     |
| `trigger`                          | string | Value of the trigger that caused Terraform Cloud to send the notification.                                   |
| `message`                          | string | Human-readable reason for the notification.                                                                  |
| `details`                          | object | Object containing details specific to the notification.                                                      |
| `details.auto_destroy_at`          | string | Timestamp when Terraform Cloud will schedule the next destroy run. Only applies to reminder notifications.   |
| `details.run_created_at`           | string | Timestamp of when Terraform Cloud successfully created a destroy run. Only applies to results notifications. |
...
The shape of data in auto destroy notification payloads may differ depending on the success of the run Terraform Cloud created. Refer to the specific examples below.

###### Reminder

Reminders that Terraform Cloud will trigger a destroy run at some point in the future.
...
Run-specific values are empty when Terraform Cloud was unable to create an auto destroy run.
