...
| ----------------------------------- | ------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TFC_GCP_PROVIDER_AUTH`             | `true`                                                                         | Requires **v1.7.0** or later if self-managing agents. Must be present and set to `true`, or Terraform Cloud will not attempt to use dynamic credentials to authenticate to GCP.                                                                                                                                           |
| `TFC_GCP_RUN_SERVICE_ACCOUNT_EMAIL` | The service account email Terraform Cloud will use when authenticating to GCP. | Requires **v1.7.0** or later if self-managing agents. Optional if `TFC_GCP_PLAN_SERVICE_ACCOUNT_EMAIL` and `TFC_GCP_APPLY_SERVICE_ACCOUNT_EMAIL` are both provided. These variables are described [below](/terraform/enterprise/workspaces/dynamic-provider-credentials/gcp-configuration#optional-environment-variables) |
