...
The `created_by` value within the [root namespace](#namespace-root) is string that specifies the user name of the Terraform Cloud user for the specific run.
...
Provides the targets specified with the [`-replace`](/terraform/cli/commands/plan#resource-targeting) flag in the CLI or the `replace-addrs` attribute in the API. Is `null` if no resource targets are specified.
...
Provides the targets specified with the [`-target`](/terraform/cli/commands/plan#resource-targeting) flag in the CLI or the `target-addrs` attribute in the API. Is `null` if no resource targets are specified.
...
Specifies the name assigned to the Terraform Cloud project.

## Namespace: organization

The **organization namespace** contains data associated with the current run's Terraform Cloud [organization](/terraform/enterprise/users-teams-organizations/organizations).
...
Details regarding each attribute can be found in the documentation for the Terraform Cloud [Workspaces API](/terraform/enterprise/api-docs/workspaces).
