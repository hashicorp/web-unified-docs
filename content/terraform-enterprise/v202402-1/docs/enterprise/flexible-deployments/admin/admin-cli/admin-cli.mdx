...
Terraform Enterprise provides CLI commands via `tfectl` to change its configuration, stop the application safely, and produce support bundles. You must have access into the Docker container or Kubernetes pod to run these commands. For instructions on connecting to the command line, refer to [Access the Terraform Enterprise command line](/terraform/enterprise/flexible-deployments/admin/admin-cli/cli-access).
...
Multi node commands are only relevant and possible if you are running TFE on Kubernetes, or on another deployment option using the 'active-active' operational mode.
</Note>
...
The `support bundle` command generates support bundle on a TFE installation.
...
The `support bundle-download` command enables the administrator of a mounted disk installation to access support bundles via https in the TFE node.
...
If the command is executed in a TFE installation that is not running on a mounted disk, it will have no effect and perform no action.

## Connect to the internal Rails Console

At times, it may become necessary to modify the internal state of the TFE application. An example is when you have lost credentials
...
This command displays the status of the license installed on your TFE application.
...
To create the first admin user in your TFE installation, you will need the **Initial Admin Creation Token** or **IACT**.
You can retrieve this from any TFE node using:
...
You can also view the health on other nodes in the TFE installation. To see the status of a specific node, use the `--node` flag, e.g.:
...
For a health check on every node in the TFE installation, you could run:
...
To read the configuration from all the nodes in the TFE installation, you can use:
...
The `TFE_ENCRYPTION_PASSWORD` is used to encrypt all TFE data (e.g., state files, plans). To change it, use:
...
**WARNING**: After changing the encryption password, restart your TFE nodes with the new password as the value of the `TFE_ENCRYPTION_PASSWORD` environment variable.
...
When using `TFE_OPERATIONAL_MODE: disk`, TFE runs its Postgres instance managed through the container.
...
* The execution of this command will **momentarily shut down your TFE instance**. Please make sure that no work
