...
| `data.relationships.agent-enabled`  **(beta)**       | boolean        | `false`          | Only valid for `sentinel` policy sets. Whether this policy set should run as a policy evaluation in the terraform cloud agent                                                                                                                                                                                |
|`data.relationships.policy-tool-version`  **(beta)** | string         | `latest`         | The version of the policy that Terraform Cloud uses to evaluate policies. You can only set a policy tool version for 'sentinel' policy sets if `agent-enabled` is `true`.                                                                                                                                |
