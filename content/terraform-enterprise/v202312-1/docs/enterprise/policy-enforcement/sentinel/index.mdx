...
Policies are rules that Terraform Cloud enforces on runs. You use the [Sentinel policy language](https://docs.hashicorp.com/sentinel/language) to define Sentinel policies. After you define policies, you must add them to [policy sets](/terraform/enterprise/policy-enforcement/manage-policy-sets) that Terraform Cloud can enforce globally or on specific [projects](/terraform/enterprise/workspaces/organize-workspaces-with-projects) and workspaces.
...
-   [tfrun](/terraform/enterprise/policy-enforcement/sentinel/import/tfrun) - Access data associated with a [run in Terraform Cloud](/terraform/enterprise/run/remote-operations). For example, you could retrieve the run's workspace.
...
-> **Note:** Terraform Cloud does not support custom imports.
