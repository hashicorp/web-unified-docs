...
- A [supported version](/terraform/enterprise/replicated/requirements/os-specific/supported-os) of RedHat Enterprise Linux.
- A [supported Docker Engine](/terraform/enterprise/replicated/requirements/docker_engine) configuration.
...
Yes, if you are running Red Hat Enterprise Linux v7. If you are running v7 and choose to run Docker from the Extra Packages for Enterprise Linux (EPEL) repository, you must [modify the default `libseccomp` profile](/terraform/enterprise/replicated/requirements/docker_engine#docker-engine-with-a-compatible-libseccomp-version).
