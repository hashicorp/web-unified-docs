...
expectation is that the Terraform Enterprise application data is backedup via the [Backup and Restore API](/terraform/enterprise/replicated/administration/infrastructure/backup-restore), or the entire virtual machine is backed up via snapshot (with file-quiescence enabled), and then replicated or backed up
