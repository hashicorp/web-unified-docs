...
HCP Terraform, this integration lets ServiceNow users order Service
...
installations can be found [here](/terraform/enterprise/integrations/service-now/service-catalog-terraform/service-now-v1)
...
First, the Terraform Admin will configure your HCP Terraform organization with
...
token, the hostname of your HCP Terraform instance, and details about version
...
|                                                                              | Retrieve the OAuth token IDs and repository identifiers that HCP Terraform uses to identify your VCS repositories.                       |
...
-   One or more [supported version control
    systems](/terraform/enterprise/vcs#supported-vcs-providers) (VCSs) with read access
...
2.  [Create a team](/terraform/enterprise/users-teams-organizations/teams) for that
...
4.  If you haven't yet done so, [connect a VCS provider](/terraform/enterprise/vcs) to
    ...
    connected to a manually-created HCP Terraform workspace can also be
...
5.  On your organization's VCS provider settings page (Settings > VCS Providers),
    ...
    with the ServiceNow integration. HCP Terraform uses the OAuth token ID to
...
10. The hostname of your Terraform Enterprise instance, or of HCP Terraform.
    The hostname of HCP Terraform is `app.terraform.io`.
