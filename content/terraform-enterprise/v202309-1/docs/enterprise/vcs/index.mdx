...
description: >-
  Learn how to connect a version control system (VCS) to your organization to
  integrate Terraform Enterprise into your development workflow and access
  additional features.
...
- When workspaces are linked to a VCS repository, Terraform Cloud can [automatically initiate Terraform runs](/terraform/enterprise/run/ui) when changes are committed to the specified branch.
- Terraform Cloud makes code review easier by [automatically predicting](/terraform/enterprise/run/ui#speculative-plans-on-pull-requests) how pull requests will affect infrastructure.
...
- [GitHub.com](/terraform/enterprise/application-administration/github-app-integration)

<!-- END: TFC:only -->

- [GitHub App for TFE](/terraform/enterprise/application-administration/github-app-integration)
...
- [Bitbucket Server](/terraform/enterprise/vcs/bitbucket-server)
...
For most supported VCS providers, Terraform Cloud does not need an SSH key — it can do everything it needs with the provider's API and an OAuth token. The exceptions are Azure DevOps Server and Bitbucket Server, which require an SSH key for downloading repository contents. The setup instructions for [Azure DevOps Server](/terraform/enterprise/vcs/azure-devops-server) and [Bitbucket Server](/terraform/enterprise/vcs/bitbucket-server) include this step.
...
-> **Note:** Alternatively, you can skip the OAuth configuration process and authenticate with a personal access token. This requires using Terraform Cloud's API. For details, see [the OAuth Clients API page](/terraform/enterprise/api-docs/oauth-clients).
