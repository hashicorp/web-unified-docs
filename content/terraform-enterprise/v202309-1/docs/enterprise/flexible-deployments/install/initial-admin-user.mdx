...
### Retrieve IACT with container or pod
...
Run the following command to retrieve your IACT token from a Docker container.
  ```shell-session
  $ docker exec <CONTAINER_NAME> curl http://localhost:80/admin/retrieve-iact
  ```

Run the following command to retrieve your IACT token from a Kubernetes pod.
  ```shell-session
  $ kubectl exec -it <POD_NAME> -- retrieve-iact
  ```
...
### Create initial admin user with browser
...
### Create initial admin user with container or pod
