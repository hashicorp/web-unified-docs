...
description: >-
  Use the `/policy-sets` endpoint to manage groupings of Sentinel and OPA
  policies. List, show, create, and update policy sets and policy set versions,
  and attach and detach policies from workspaces and projects using the HTTP
  API.
...
| `include`           | **Optional.** Enables you to include related resource data. Value must be a comma-separated list containing one or more of `projects`, `workspaces`, `policies`, `newest_version`, or `current_version`. See the [relationships section](#relationships) for details.                                                                            |   |
...
-> **Note:** The `data.relationships.projects` and `data.relationships.workspaces` objects refer to projects and workspaces directly attached to the policy set. These keys are omitted for policy sets marked as global, which are implicitly related to all of the organization's workspaces.
...
| `policies`        | Individually managed policies which are associated with the policy set.                                                                                                                        |
| `projects`        | The project this policy set applies to.                                                                                                                                                        |
| `workspaces`      | The workspaces to which the policy set applies.                                                                                                                                                |
