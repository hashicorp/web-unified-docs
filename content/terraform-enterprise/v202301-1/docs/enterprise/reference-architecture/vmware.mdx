...
[pre-install checklist](/enterprise/install/pre-install-checklist)
...
The [operational
mode](/enterprise/install/pre-install-checklist#operational-mode-decision) determines the
...
For more information about Terraform Enterprise's disk requirements, see [Before Installing: Disk Requirements](/enterprise/requirements/data-storage/operational-mode-requirements).
...
- Both Terraform Enterprise virtual machines can access an external Redis server, a PostgreSQL database, and an S3-compatible blob storage bucket. Terraform Enterprise will use an internal Vault server by default. Optionally, you can configure Terraform Enterprise to use an [existing Vault cluster](/enterprise/requirements/data-storage/vault).
