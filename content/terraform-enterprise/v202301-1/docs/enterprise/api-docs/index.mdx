...
Terraform Cloud provides an API for a subset of its features. If you have any questions or want to request new API features, please email [support@hashicorp.com](mailto:support@hashicorp.com).
...
HashiCorp provides a [stability policy](/enterprise/api-docs/stability-policy) for the Terraform Cloud API, ensuring backwards compatibility for stable endpoints. The [changelog](/enterprise/api-docs/changelog) tracks changes to the API for Terraform Cloud and Terraform Enterprise.
...
-   [User tokens](/enterprise/users-teams-organizations/users#api-tokens) — each Terraform Cloud user can have any number of API tokens, which can make requests on their behalf.
-   [Team tokens](/enterprise/users-teams-organizations/api-tokens#team-api-tokens) — each team can have one API token at a time. This is intended for performing plans and applies via a CI/CD pipeline.
-   [Organization tokens](/enterprise/users-teams-organizations/api-tokens#organization-api-tokens) — each organization can have one API token at a time. This is intended for automating the management of teams, team membership, and workspaces. The organization token cannot perform plans and applies.

### Blob Storage Authentication

Terraform Cloud relies on a HashiCorp-developed blob storage service for storing statefiles and multiple other pieces of customer data, all of which are documented on our [data security page](/enterprise/architectural-details/data-security).
...
For example, the [state versions api](/enterprise/api-docs/state-versions) returns a field named `hosted-state-download`, which is a URL of this form:
...
The [show entitlement set](/enterprise/api-docs/organizations#show-the-entitlement-set) endpoint can return information about an organization's current entitlements, which is useful if your client needs to change its interface when a given feature isn't available.
...
-   `run-tasks` — Allows an organization to use [run tasks](/enterprise/workspaces/settings/run-tasks). Affects the [run tasks][] endpoints.
...
-   `teams` — Allows an organization to manage access to its workspaces with [teams](/enterprise/users-teams-organizations/teams). Without this entitlement, an organization only has an owners team. Affects the [teams][], [team members][], [team access][], and [team tokens][] endpoints.
...
[agents]: /enterprise/api-docs/agents

[agent pools]: /enterprise/api-docs/agents

[agent tokens]: /enterprise/api-docs/agent-tokens

[applies]: /enterprise/api-docs/applies

[audit trails]: /enterprise/api-docs/audit-trails

[Configuration Designer]: /enterprise/registry/design

[cost estimation]: /enterprise/cost-estimation

[o-clients]: /enterprise/api-docs/oauth-clients

[o-tokens]: /enterprise/api-docs/oauth-tokens

[plans]: /enterprise/api-docs/plans

[policies]: /enterprise/api-docs/policies

[policy checks]: /enterprise/api-docs/policy-checks

[policy sets]: /enterprise/api-docs/policy-sets

[private module registry]: /enterprise/registry

[registry modules]: /enterprise/api-docs/private-registry/modules

[registry providers]: /enterprise/api-docs/private-registry/providers

[runs]: /enterprise/api-docs/run

[run tasks]: /enterprise/api-docs/run-tasks/run-tasks

[Sentinel]: /enterprise/policy-enforcement/sentinel

[single sign on]: /enterprise/users-teams-organizations/single-sign-on

[state versions]: /enterprise/api-docs/state-versions

[teams]: /enterprise/api-docs/teams

[team access]: /enterprise/api-docs/team-access

[team members]: /enterprise/api-docs/team-members

[team tokens]: /enterprise/api-docs/team-tokens

[vcs integrations]: /enterprise/vcs
