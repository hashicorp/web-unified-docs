...
-> **Note:** The [private module registry](/enterprise/registry) is an easier way to manage private Terraform modules in Terraform Cloud, and doesn't require setting SSH keys for workspaces. The rest of this page only applies to configurations that fetch modules directly from a private Git repository.
...
-> **Note:** SSH keys for cloning Terraform modules from Git repos are only used during Terraform runs. They are managed separately from any [keys used for bringing VCS content into Terraform Cloud](/enterprise/vcs#ssh-keys).
...
-> **API:** See the [SSH Keys API](/enterprise/api-docs/ssh-keys) and [Assign an SSH Key to a Workspace endpoint](/enterprise/api-docs/workspaces#assign-an-ssh-key-to-a-workspace). <br/>
...
2.  Obtain a PEM formatted SSH keypair that Terraform Cloud can use to download modules during a Terraform run. You might already have an appropriate key. If not, create one on a secure workstation and distribute the public key to your VCS provider(s). Do not use or generate a key that has a passphrase. Git is running non-interactively and cannot prompt for it.
    ...
    ```
3.  Enter a name for the key in the **Name** field. Choose something identifiable. Keys are only listed by name. Terraform Cloud retains the text of each private key, but never displays it for any purpose.
4.  Paste the text of the private key in the **Private SSH Key** field.
5.  Click **Add Private SSH Key**.
