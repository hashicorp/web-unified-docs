...
1. The demo [operational mode](/enterprise/install/pre-install-checklist#operational-mode-decision) has been removed. If you are currently running demo mode, we strongly suggest that you upgrade to Terraform Enterprise v202204-1. To do this, you must [migrate your application data](/enterprise/admin/infrastructure/demo-to-disk-migration).
...
1. Changed cost estimation so that it uses the [HTTP proxy settings](/enterprise/install/interactive/installer#proxy-usage) configured within Terraform Enterprise.
1. Added the ability to specify an ['SSO team ID'](/cloud-docs/users-teams-organizations/single-sign-on#team-names-and-sso-team-ids) for teams that Terraform Enterprise can use to map teams to non-human readable 'MemberOf' values in SAML assertions.
1. Updated display of download count metrics for modules in the private registry
1. Added an API endpoint to fetch a workspace's current state version outputs. Refer to [Show Current State Version Outputs For a Workspace](/cloud-docs/api-docs/state-version-outputs#show-current-state-version-outputs-for-a-workspace) for details.
