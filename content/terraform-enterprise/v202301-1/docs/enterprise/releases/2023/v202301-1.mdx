...
1. The private registery will now validate providers are supported by the Terraform SDK. [supported by Golang](https://github.com/golang/go/blob/master/src/go/build/syslist.go).
...
1. Outputs view, available upon a successful apply in the workspaces, set to Structured Run Output mode, is shown expanded by default, instead of being in a collapsed state.
...
1. Fixed an 500 response error on the teams/organization-memberships endpoint when trying to delete a member whose user record could not be found.
...
1. Prevent VCS runs from being triggered on discarded workspaces.
