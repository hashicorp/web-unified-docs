...
~> **Important:** Integration with a SaaS VCS provider (GitHub.com, GitLab.com, Bitbucket Cloud, or Azure DevOps Services) requires ingress from the public internet. This lets the [inbound web hooks](/enterprise/vcs#webhooks) reach Terraform Enterprise. You should also configure appropriate security controls, such as a Web Application Firewall (WAF).
...
The metrics endpoints are optional. You can enable them in the [application settings](/enterprise/install/automated/automating-the-installer#available-settings) when you install Terraform Enterprise.
...
[custom Terraform bundle](/enterprise/run/install-software#custom-and-community-providers)
is supplied:
