...
HCP Terraform can generate [test files](/terraform/language/tests) for any private modules that use the [branch-based publishing workflow](/terraform/enterprise/registry/publish-modules#branch-based-publishing) and do not already contain tests in their source code. 
