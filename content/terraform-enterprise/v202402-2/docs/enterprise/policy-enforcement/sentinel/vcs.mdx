---
page_title: Policy Set VCS Repositories - Sentinel - Terraform Enterprise
description: >-
  Configure a Sentinel policy set version control repository to use in Terraform
  Enterprise. A policy set repository has a configuration file, policy files,
  and module files. 
...
To enable policy enforcement, you must group Sentinel policies into policy sets. You can then apply those policy sets globally or to specific [projects](/terraform/enterprise/workspaces/organize-workspaces-with-projects) and workspaces.
