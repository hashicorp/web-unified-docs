---
page_title: >-
  Service Catalog - ServiceNow Service Catalog Integration - Terraform
  Enterprise
description: |-
...
infrastructure from the service catalog, and HCP Terraform will fulfill the
...
1.  The OAuth token ID and repository identifier for each VCS repository that
    HCP Terraform will use to provision infrastructure. Your Terraform Admin
...
2.  Configure at least one one VCS repository in ServiceNow.
...
You can set custom names for your Terraform workspaces by choosing a prefix for your workspace name. The service catalog app appends the ServiceNow RITM number to the name. If you do not define a workspace prefix, ServiceNow uses the RITM number as the workspace name.

You can use the following characters for workspace names: 
...
Names must not exceed 90 characters.
Refer to the [workspace naming recommendations](/terraform/enterprise/workspaces/creating#workspace-naming) for best practices.

## Workspace Tags

You can add custom tags to your Terraform workspaces by specifying a comma-separated list of values in the **Workspace Tags** field. ServiceNow parses the values and attaches them to the workspace in Terraform Enterprise. 

Tags enable you to categorize, filter, and manage workspaces provisioned through the service catalog for Terraform.
...
You can use the following characters for workspace tags:
...
Workspace tags are limited to 255 characters. Refer to the [workspace tagging rules](/terraform/enterprise/workspaces/creating#workspace-tags) for more details. 
