...
- `disable_credential_rotation` - (Optional) Controls whether the plugin rotates the incoming credentials and manages a new service account. If you set the `disable_credential_rotation` attribute to `false` or omit it, the plugin rotates the incoming credentials to create a new service account, then deletes the incoming credentials.
  ...
  The `disable_credential_rotation` option must be set to `true` if you use dynamic credentials.

  Credential rotation is not supported for Hitachi Content Platform and may not function for other S3-compliant providers.

### Secrets

MinIO and S3-compliant buckets can have the following secrets:
