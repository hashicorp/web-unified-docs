...
1. Check the location of your controller configuration file on disk, such as `/etc/boundary.d/controller.hcl` in the example on this page.
...
Create a new unit file, such as `/etc/systemd/system/boundary-controller.service`. Add the following code to the file. Update the path to your `controller.hcl` file on the `ExecStart` and `ConditionFileNotEmpty` lines, and the user and group as needed.
