...
Create a new unit file, such as `/etc/systemd/system/boundary-worker.service`. Add the following code to the file. Update the path to your worker config file on the `ExecStart` and `ConditionFileNotEmpty` lines, and the user and group as needed.
...
ConditionFileNotEmpty=/etc/boundary.d/egress-worker.hcl
