...
In the image above, the tag type is `config`, which means the tags were defined in the worker config file. Config tags cannot be modified within the Admin UI, you must update them in the config file itself.
