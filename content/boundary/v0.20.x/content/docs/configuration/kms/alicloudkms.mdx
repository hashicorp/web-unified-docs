...
- `purpose` - Purpose of this KMS, acceptable values are: `bsr`, `config`, `downstream-worker-auth`, `previous-root`, `recovery`, `root`, `worker-auth`, and `worker-auth-storage`.
  ...
  For more information about the supported KMS purposes, refer to [Data encryption in Boundary](/boundary/docs/secure/encryption/data-encryption).
