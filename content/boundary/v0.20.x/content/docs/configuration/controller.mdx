...
- `public_cluster_addr` - Specifies the public host or IP address, and
  ...
  Workers use this address after their initial connection through the worker's
  ...
  This value can be a direct address string, can refer to a file on disk
  (file://) from which an address will be read; an env var (env://) from which
