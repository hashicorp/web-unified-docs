...
<table>
  ...
  <tr>
    ...
    <br /><br />
    This known issue was resolved in Client Agent version 0.20.1. The Client Agent now tracks the operational state in a file so that it persists across restarts. You can configure where the file resides in the <code>boundary-client-agent.hcl</code> configuration. Now, if you pause the Client Agent and then restart the system, the Client Agent remains paused after the restart.
    <br /><br />
