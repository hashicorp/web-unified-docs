...
Lastly, for instructions on how to upgrade an existing Boundary deployment to v0.7.0, please review Boundary’s [general upgrade guide](https://learn.hashicorp.com/tutorials/boundary/upgrade-version).
...
**Managed group configuration in the Admin UI**: Managed groups are a special type of IAM group which populate users based on administrator-defined filters of external identity provider (IdP) metadata.
Configuration of managed groups is now available in the Boundary Admin UI.

**UI Support for Resource Filtering:** Users may now filter auth method and session resources in the Admin UI and Boundary Desktop Client.
