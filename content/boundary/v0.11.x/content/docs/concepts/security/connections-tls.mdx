...
There may be various permutations of this mechanism in the future; there is only
one mechanism now, referred to as _worker-led_ registration because the Worker
produces the initial key material.
...
5. A Controller receiving the request validates that the provided nonce matches
   ...
   Worker certificate, its CA certifciate, and the nonce, and returns it to the
...
Workers can take advantage of the [KMS](/docs/concepts/security/data-encryption)
