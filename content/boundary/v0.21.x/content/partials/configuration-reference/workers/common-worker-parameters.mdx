...
- `ssh_known_hosts_path` - Specifies the path of the `known_hosts` file to be used by the worker for
  SSH host key verification of an SSH target. The path must already exist. If not provided the
  worker will skip host key verification. On `SIGHUP`, the `known_hosts` file is re-parsed and new values used.
