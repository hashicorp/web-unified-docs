...
Boundary Enterprise version 0.21.3 introduced support for IBM Passport Advantage Online license keys that contain a valid Boundary entitlement.
To use an IBM Passport Advantage Online license key, you must configure the the [`license`](/boundary/docs/configuration/controller#license) attribute in the `controller` block to select the desired entitlement from the license.
Differences from using a HashiCorp license key include:

- When you view the license information using CLI or API commands, IBM Passport Advantage Online licenses show additional `edition` and `add_ons` fields corresponding to the selected entitlement.
- IBM Passport Advantage Online licenses always enforce the termination date, following the behavior described in the [License expiration](#license-expiration) section.
