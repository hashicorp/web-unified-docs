...
1. Create an authorized user:
  1. Navigate back to the **Users** page. Click **New User**.
  1. Complete the following fields:
     - **Name**: (Required) A name for identification purposes.
...
1. Check the box next to the `unauthorized` user in the `global` scope. Click **Add Principals**.
...
1. Complete the following fields:
   ...
   Click **Save**.
1. Select the **Principals** tab for the new role. Click **Add Principals**.
1. Check the box next to the `authorized` user in the `global` scope. Click **Add Principals**.
1. Select the **Grants** tab for the role.
...
First, create an `unauthorized` user and limit its permissions.
...
Grant the authorized user full access in the org and project scopes by creating an `authorized_org_role` and `authorized_project_role` with full access grants.
