...
Tracks the complete lifecycle of user sessions from pending to termination. Uses the **accumulating snapshot** pattern: a single row is inserted on session creation and updated as the session progresses through state transitions.
...
Tracks individual connections within sessions, providing per-connection network metrics.
...
Tracks authentication token lifecycle: issuance, access, and deletion.
...
Stores user identity with auth method, account, and organizational context.
...
Denormalized host dimension capturing the full target hierarchy.
...
Tracks credential libraries and stores with Vault integration metadata.
...
Classifies network addresses by type and address family.
...
Pre-populated conformed dimensions for time-series analysis. `wh_date_dimension` contains ~18,265 rows (2019-10-09 onward). `wh_time_of_day_dimension` contains 86,401 rows (one per second in a day). These enable efficient date/time grouping without runtime calculations.
...
Generates a comprehensive audit trail showing who accessed what system, when, from where, with what credentials, and how much data moved. This query satisfies the core requirement of every major compliance framework.
...
Identifies users who have never accessed a target or whose last access exceeds a configurable threshold. This is a cornerstone of quarterly access certification reviews.
...
Surfaces all access to targets or credentials with administrative or production markings, supporting least-privilege audits and privileged access reviews.
...
Detects users who access both development and production systems — a common separation-of-duties violation. The query groups targets by environment based on naming conventions and flags users spanning multiple environments.
...
Surfaces sessions that were authorized but never established a connection. These represent either abandoned access attempts, potential authorization failures, or reconnaissance activity.
...
Detects users accessing an unusually high number of distinct targets in a short time window. This pattern is a strong indicator of lateral movement during an active compromise.
...
Identifies authentication tokens used from multiple distinct client IP addresses — a potential indicator of token theft, session hijacking, or credential sharing.
...
Surfaces connections with byte volumes significantly above baseline, a potential indicator of data exfiltration. Uses a self-baselining approach comparing each user's recent connections against their own 30-day average.
...
Reconstructs a complete chronological timeline of all access events for a specific user during an incident window. Use this to understand the full scope of a compromise.
...
- `wh_date_dimension` and `wh_time_of_day_dimension` are pre-populated reference tables.
