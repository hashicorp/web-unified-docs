---
layout: docs
page_title: SSH host key validation
description: >-
...
This page describes how to configure SSH host key validation on workers to verify host identity when connecting to targets.
...
To configure host key validation, add the `ssh_known_hosts_path` to your [worker config file](/boundary/docs/workers/create). This should point to a system-wide known hosts config file, such as the `/etc/ssh/ssh_known_hosts` on Linux or `%ProgramData%\ssh\ssh_known_hosts` on Windows. This will vary depending on how you have configured SSH on the worker.
...
To enable SSH host key validation, you need to reference the location of the known hosts file using the `ssh_known_hosts_path` parameter in the Boundary config file.
...
When the known hosts configuration changes, you need to restart the worker.
...
If the content of either of these files changes, you need to either restart the worker service or SIGHUP the worker.
...
To SIGHUP the worker instead, locate the PID of the worker service:
