...
To configure host key validation, add the `ssh_known_hosts_path` parameter to your [worker config file](/boundary/docs/workers/create). The path should point to a system-wide known hosts config file, such as the `/etc/ssh/ssh_known_hosts` on Linux or `%ProgramData%\ssh\ssh_known_hosts` on Windows. It will vary depending on how you have configured SSH on the worker.
...
When the known hosts configuration changes, you must restart the worker.
...
If the content of either of these files changes, you must either restart the worker service or SIGHUP the worker.
