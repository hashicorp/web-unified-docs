...
@include 'configuration-reference/domain-model/host-catalog-structure'
...
In this example you will create three hosts for separate databases. One for hypothetical mysql database running on `192.168.0.10` at port `3306`, a second for another mysql database running on `192.168.0.11` at port `3306`, and a third for a postgres database running on `192.168.0.20` at port `5432`.
...
The `databases` host catalog will contain the `mysql-hosts` host set and the `postgres-hosts` host set. The `mysql-hosts` contains the `mysql1` and `mysql2` hosts, and the `postgres-hosts` contains the `postgres` host.
...
Hosts can belong to a host catalog without being assigned to a host set, but you must add hosts to a host set before you a target can connect to it.
...
1. Click the **Hosts** tab. Click the **Manage** drop-down and select **Add Existing Host**.
