...
last_modified: 2026-05-14T17:45:01.000Z
...
Dynamic host catalogs use filters to select which hosts are added to any host sets you create. For example, you can create a host catalog to sync hosts from GCP, and then create a host set named `production` that selects all GCP Compute Engine VM instances tagged with `labels.env:prod`.

To set up a dynamic host catalog, you should:

1. Authenticate the host catalog to your GCP account using a supported authentication method.
1. Create a host set with a filter to select the hosts you want to sync.
...
You can use the `jq` utility to remove the extra `\n` characters. For example:
...
If you run your Boundary controller or Boundary workers on a GCP VM instance, you can use Application Default Credentials (ADC) to authenticate with GCP.
Boundary uses the service account associated with the VM instance to authenticate with GCP.

ADC is commonly used in scenarios where workers are running on GCP, but your controller is deployed somewhere else.
...
1. Complete the following fields:
    ...
    It is required when you authenticate using the service account.
      ...
      If you downloaded the private key from GCP, it may contain extra `\n` characters that cause an error.
      You can use `jq` to remove the extra `\n` characters.
...
1. Use the following command to create a dynamic host catalog for GCP:
  ...
  Refer to the [host-catalogs create](/boundary/docs/commands/host-catalogs/create) command documentation for additional fields that you can use when you create host catalogs.
...
Refer to the [boundary_host_catalog_plugin](https://registry.terraform.io/providers/hashicorp/boundary/latest/docs/resources/host_catalog_plugin) topic in the Boundary Terraform provider documentation for additional fields that you can use when you create host catalogs.
...
1. Complete the following fields:
   ...
   If you enter a name, it must be unique.
   - **Description**: (Optional) An optional description of the host catalog for identification purposes.
   - **Filter**: (Optional) An array of string filters in the format `key=val`. The key corresponds to a filter option like `status`, where the value could be `running`. A full example would be `labels.env:dev`.
     ...
     We recommend using labels as filters because they are more flexible and can filter on multiple values as in the following examples:
       ...
       - `labels.env:dev`
       - `labels.env:prod AND labels.app:web`
1. Click **Save**.
...
1. Use the following command to create a host set:
  ...
  Refer to the [host-sets create](/boundary/docs/commands/host-sets/create) command documentation for additional fields that you can use when you create host sets.
...
Refer to the [boundary_host_set_plugin](https://registry.terraform.io/providers/hashicorp/boundary/latest/docs/resources/host_set_plugin) topic in the Boundary Terraform provider documentation for additional fields that you can use when you create host sets.
...
Refer to the domain model documentation for [host catalogs](/boundary/docs/domain-model/host-catalogs) and [host sets](/boundary/docs/domain-model/host-sets) to better understand those Boundary resources.
