...
last_modified: 2026-09-11T15:59:29.000Z
...
This attribute is required when you use dynamic credentials.
...
You can use this value to control how IAM principals and applications name their role sessions when they assume an IAM role.
...
- `disable_credential_rotation` - (Optional) Controls whether the plugin rotates the incoming credentials and manages a new service account. If you set this attribute to `false` or omit it, the plugin rotates the incoming credentials to create a new service account, then deletes the incoming credentials.
