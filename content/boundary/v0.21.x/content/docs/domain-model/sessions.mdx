...
### Security rationale  
Ending all active sessions when controllers are down makes sure that no one keeps access without proper checks. This protects against unauthorized access if the system that manages permissions goes offline.
