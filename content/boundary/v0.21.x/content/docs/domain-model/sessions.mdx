...
### Security rationale  
Ending all active sessions when controllers are down ensures that no user retains access without proper checks. This approach protects against unauthorized access if the system that manages permissions goes offline.

If a worker cannot connect to the controller, it cannot determine if an active session was canceled.
