...
Because of the way the Client Agent manipulates DNS records, it can trigger warnings from some operating systems, third-party tools, or firewalls.
VPN and EDR software, in particular, may create conflicts with the Client Agent and result in loss of network connectivity, or other issues.
It may be mistaken for a man-in-the-middle attack on some Linux systems.

Refer to the following topics for general troubleshooting steps and a list of third-party products that have been tested and confirmed to work with the Client Agent.

- [Troubleshoot the Client Agent](/boundary/docs/client-agent/troubleshoot)
