...
The Client Agent does not have a standalone binary, so you must use the Boundary installer to [install or update](/boundary/docs/deploy/self-managed/install-clients) it.
...
The Boundary Client Agent daemon and Boundary search functionality require the default anonymous login role so users can add auth tokens to the client cache and search cached resources.
