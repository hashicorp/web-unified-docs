...
# Connecting business intelligence tools to the Boundary data warehouse

**How to connect free BI tools to your Boundary database and run access analytics queries.**
Validated against Boundary v0.21.0. Last updated: May 22, 2026.

Your Boundary deployment has a PostgreSQL database. Inside it is a built-in analytics schema (`wh_*` tables) that tracks every session, connection, user authentication, and credential usage your Boundary instance handles. This data is updated automatically — no ETL pipeline needed.
...
Do this once, from a PostgreSQL client connected to your Boundary database.
...
2. Go to **Settings** (gear icon) → **Admin settings** → **Databases** → **Add database**
3. Fill in:
...
In Metabase, go to **Browse Data** → select **Boundary Analytics**. You should see tables like `wh_session_accumulating_fact`, `wh_user_dimension`, and `wh_host_dimension`.
...
Paste these into Metabase's **SQL Editor** (the native query editor, not the visual builder).
...
This query is your go-to for any compliance audit. It returns every access event with who, what, when, from where, and how much data moved — all the fields auditors ask for.
...
Finds accounts that have never been used or haven't been used in 90+ days — a core requirement for quarterly access certification reviews.
...
**Review cadence:** Run this quarterly. Deprovision "NEVER USED" accounts. Flag "INACTIVE >90d" for manager confirmation.

### 4.10 Compliance: Separation of duties check

Detects users who access both production and non-production environments — a violation of basic separation of duties. Adapt the `ILIKE` patterns to match your naming conventions.
...
**Any rows returned = potential violation.** Review whether prod+non-prod access is authorized for that user.

### 4.11 SecOps: Lateral movement detection

Finds users hitting multiple different targets in a short time window — a strong indicator of lateral movement during a compromise.
...
**Triage:** 3+ targets in 24h → investigate. 5+ targets in under an hour → escalate to incident response.

### 4.12 SecOps: Data exfiltration detection

Compares each user's recent connection volumes against their own 30-day baseline. Connections 3+ standard deviations above normal are flagged.
...
**Alert thresholds:** 3+ std deviations → unusual, investigate. 5+ → critical, escalate immediately.

### 4.13 SecOps: Incident timeline reconstruction

When investigating a security incident, this query reconstructs the complete chronological timeline for a specific user. Replace `'user'` with the user's name and adjust the date range.
...
**Output:** A chronological table of every AUTHORIZED → CONNECTED → TERMINATED event for the user, ready to paste into an incident report.
...
Perfect for ad-hoc exploration. Connect directly and run any of the queries above.

## Step 6: What to do when...
...
If both are 0, use Boundary to authorize a session and connect to a target. Then re-check.
...
1. Use date filters (`WHERE ... >= CURRENT_DATE - INTERVAL 'X days'`)
2. Set a statement timeout: `ALTER ROLE boundary_bi_readonly SET statement_timeout = '30s';`
3. Use a read replica for BI queries if available
...
You now have a working read-only BI connection to Boundary's built-in data warehouse. The key workflow:
