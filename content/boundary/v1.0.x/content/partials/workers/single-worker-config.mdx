Boundary workers use an HCL configuration file to define how they proxy sessions, where they store credentials, and which KMS key encrypts their authentication material. Create the `egress-worker.hcl` file with the following configuration information:
...
- `kms` - Configures KMS (key management system) blocks for [various purposes](/boundary/docs/secure/encryption/data-encryption).
