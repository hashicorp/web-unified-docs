...
- `cors_enabled` `(boolean: true)` - Specifies if CORS (Cross-Origin Resource Sharing) should be enabled, which
  allows Boundary to support external browser-based clients (not including Admin
  UI), such as Boundary Desktop Client. If not specified, CORS will be enabled with an
  allowed origin that grants access to Boundary Desktop Client, unless overridden with
...
- `cors_disable_default_allowed_origin_values` `(boolean: false)` - Specifies
  ...
  in `cors_allowed_origins`, such as Boundary Desktop Client's origin. This also
...
- `cors_allowed_origins` `(array(string): ["serve://boundary"])` - An array of
  ...
  origins, set to `['*']`. By default, Boundary Desktop Client's origin
