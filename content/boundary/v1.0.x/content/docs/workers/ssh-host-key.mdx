---
layout: docs
page_title: Configure SSH host key validation for Boundary workers
description: >-
  Configure SSH host key validation on Boundary workers to verify host identity and reduce the risk of man-in-the-middle attacks.
...
# Configure SSH host key validation for Boundary workers

Boundary workers can validate SSH host keys before connecting to a target, so clients only reach the intended server and are protected against man-in-the-middle attacks.
...
Refer to [Configure worker tags in Boundary](/boundary/docs/workers/worker-tags) for tag syntax.
...
To SIGHUP the worker instead, locate the PID (process ID) of the worker service. For example:
...
Then reload the configuration file for the process using `SIGHUP` (a Unix signal that tells a running process to reload its configuration):
