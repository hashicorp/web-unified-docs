---
layout: docs
page_title: Boundary worker configuration overview
description: >-
  Configure Boundary workers as network proxies for sessions, set up storage for session recording, and chain workers with multi-hop configurations across private networks.
...
# Overview of Boundary workers

Boundary workers act as network proxies for sessions, creating a direct tunnel between users and private targets instead of exposing an entire private network. Instead of exposing a private network to the public, or allowing users to have access to entire private networks, workers create a direct network tunnel between users and targets.
...
In HCP (HashiCorp Cloud Platform) Boundary, HCP-managed workers are automatically deployed with the cluster.
...
You must register workers using the worker-led or controller-led methods in the system with an API call. These workers require storage on disk to store the current set of credentials. Workers using an external KMS (key management system) auto-register after authenticating. This makes them an easy mechanism to use for automatic scaling.
...
## Reload and shut down workers with signals
...
![Diagram showing Worker 1 connected upstream to a controller, with Worker 2 downstream of Worker 1 and Worker 3 downstream of Worker 2](/img/multi-hop-workers.png)
...
## Related worker configuration topics
