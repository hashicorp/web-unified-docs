...
To recover, free disk space on the worker.
...
<Note>
  ...
  Boundary requires the extra headroom so that a worker near the threshold does not flip back and forth between states.
