---
layout: docs
page_title: Register Boundary workers
description: >-
  Register Boundary workers with the controller using controller-led, worker-led, or KMS-led authorization and the required activation token.
...
# Register Boundary workers

Boundary workers register with the controller using an activation token, secured over PKI (public key infrastructure), and stored in an `auth_storage_path` directory. Choose from controller-led, worker-led, or KMS-led authorization depending on how you deploy and scale workers.
...
- Key rotation - Rotating a `downstream-worker-auth` key requires coordinating the changes on both the upstream and all affected downstream workers.
...
## KMS (key management system) registration
...
You can only set the `name` and `description` fields for workers that use the [KMS-led authorization and authentication method](/boundary/docs/workers/registration#kms-led-authorization-and-authentication).
