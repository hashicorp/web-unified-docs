...
Workers that use an external KMS re-authenticate each time they connect, so they do not store credentials on disk and you can replace them freely.
...
Workers that register through an external KMS do not require an `auth_storage_path`, because the KMS re-authenticates them each time they connect.
