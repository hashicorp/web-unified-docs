---
layout: docs
page_title: Configure a Boundary worker filter
description: >-
  Configure a Boundary worker filter to control which workers can manage a session, relay Vault requests, or store session recordings.
...
# Configure a Boundary worker filter

Boundary worker filters use tags to control which workers are allowed to perform specific functions — such as accessing a target, relaying Vault requests, or storing session recordings — so you can restrict sensitive operations to trusted workers.
...
Refer to [Configure worker tags in Boundary](/boundary/docs/workers/worker-tags) to learn how to tag workers before writing a filter.

| Attribute | Applies to | Controls |
|---|---|---|
| `egress_worker_filter` | Targets | Which worker accesses the target |
| `ingress_worker_filter` | Targets | Which worker a client connects to first |
| `worker_filter` | Vault credential stores, storage buckets | Which worker relays Vault requests or stores recordings |
