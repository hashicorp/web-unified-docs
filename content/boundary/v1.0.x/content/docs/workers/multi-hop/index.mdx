...
description: >-
  Configure Boundary multi-hop sessions to chain two or more workers across networks and reach private targets without exposing internal network access.
...
Reaching a target behind multiple private network enclaves without opening inbound firewall rules is difficult with traditional bastion hosts. Boundary multi-hop sessions solve this by chaining two or more workers together into a reverse proxy chain between the user and the target, so every worker in the chain needs only outbound connectivity.
...
Multi-hop capabilities, including multi-hop sessions and [Vault private access](/boundary/docs/vault),
...
![Diagram showing Worker 1 connected upstream to Worker 2, which connects downstream to Worker 3 in a multi-hop chain](/img/multi-hop-workers.png)
