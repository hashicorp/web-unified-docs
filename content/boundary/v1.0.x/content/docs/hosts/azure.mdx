...
description: >-
  Use dynamic host catalogs to discover Azure resources using Azure Resource Manager and sync them as Boundary hosts. Create a host catalog and host set for Azure.
...
1. Authenticate the host catalog to your Microsoft Azure account using a supported authentication method.
...
1. Complete the following fields:
    ...
    If you enter a name, it must be unique.
    - **Description**: (Optional) An optional description of the host catalog for identification purposes.
    - **Filter**: (Optional) An array of string filters in the format `key eq 'value'`. The key corresponds to a filter option like `tagName`. A full example would be `tagName eq 'application'`.
      ...
      This means it is useful to filter directly on tag names or values as in the following examples:
