...
1. Complete the fields related to your Vault instance:
    ...
    - **Worker Filter** (optional) - If your Vault instance does not have a publicly accessible address and instead is proxied through a Boundary worker, enter the worker filter. This should be a boolean expression. To have Boundary format the filter for you, toggle the **Show filter generator** switch. Refer to [Generate a filter in the Admin UI](/boundary/docs/workers/filters#generate-a-filter-in-the-admin-ui) for the procedure, or the examples in the [Worker tags](/boundary/docs/workers/worker-tags) documentation.
