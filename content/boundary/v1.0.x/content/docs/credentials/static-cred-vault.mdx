...
last_modified: 2026-08-10T22:18:11.000Z
...
1. Complete the fields related to your Vault instance:
    ...
    - **Worker Filter** (optional) - If your Vault instance does not have a publicly accessible address and instead is proxied through a Boundary worker, enter the worker filter. The filter should be a boolean expression. To have Boundary format the filter for you, click the **Show filter generator** toggle. Refer to [Generate a filter in the Admin UI](/boundary/docs/workers/filters#generate-a-filter-in-the-admin-ui) for the procedure, or the examples in the [Worker tags](/boundary/docs/workers/worker-tags) documentation.
...
resource "boundary_credential_store_vault" "example" {
  name        = "example_vault_credential_store"
