---
layout: docs
page_title: Boundary workers
description: >-
  Workers let you proxy traffic to private targets. Learn about worker capabilities, multi-hop sessions, tags for filtering, and monitoring status and health.
...
such as the database, KMS (Key Management System), Vault, identity providers, and plugins.

![Diagram showing Boundary's three-part architecture: the control plane (controllers), data plane (workers), and client, with workers proxying traffic between users and private targets.](/img/how-boundary-works_light.png#light-theme-only)
![Diagram showing Boundary's three-part architecture: the control plane (controllers), data plane (workers), and client, with workers proxying traffic between users and private targets.](/img/how-boundary-works_dark.png#dark-theme-only)
...
![A user connecting through three networks, each containing a worker tagged A, B, or C and the target that worker reaches](/img/worker-tags_light.png#light-theme-only)
![A user connecting through three networks, each containing a worker tagged A, B, or C and the target that worker reaches](/img/worker-tags_dark.png#dark-theme-only)
