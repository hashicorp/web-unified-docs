---
layout: docs
page_title: Credential management in Boundary
description: >-
  Use credential brokering and credential injection to authenticate Boundary users to targets, and compare the security considerations of each method.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-05-11T10:18:43-04:00
last_modified: 2026-08-11T01:39:37.000Z
# END AUTO GENERATED METADATA
---

# Credential management

Sharing static credentials with everyone who needs target access is a
security liability. Boundary solves this with two paradigms: credential
brokering, which returns credentials to the user, and credential injection,
which authenticates on the user's behalf without ever exposing the
credential.
...
![Credential brokering: Vault passes credentials to Boundary, which returns them to the user, who then opens a session through the worker to the server](/img/credential-brokering_light.png#light-theme-only)
![Credential brokering: Vault passes credentials to Boundary, which returns them to the user, who then opens a session through the worker to the server](/img/credential-brokering_dark.png#dark-theme-only)
...
Credential injection is required for the RDP (Remote Desktop Protocol) and SSH target types.
...
![Credential injection: Vault passes credentials to Boundary, which sends them to the worker, and the worker authenticates to the server on the user's behalf](/img/credential-injection_light.png#light-theme-only)
![Credential injection: Vault passes credentials to Boundary, which sends them to the worker, and the worker authenticates to the server on the user's behalf](/img/credential-injection_dark.png#dark-theme-only)
...
## Related credential documentation
