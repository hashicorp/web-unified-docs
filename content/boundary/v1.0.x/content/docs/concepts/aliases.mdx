...
You can create aliases in the `global` or project scopes, but if you create them for project scopes, you must also create suffixes for the project scope and its containing org scope. Once you create the suffixes, Boundary appends them to any aliases you create in the project scope. The suffix enables reuse across scopes, while still ensuring that aliases are globally unique values.
