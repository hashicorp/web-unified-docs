...
You can create aliases in any scope, but if you create them for org or project scopes, you must also create a suffix for the scope. Once you create the suffix, Boundary appends the suffix to any aliases you create in that scope. The suffix enables reuse across org and project scopes, while still ensuring that aliases are globally unique values.

Creating aliases at the org and project scope level allows scope administrators to granularly manage their own aliases, seeing only the resources that belong to their scope. Organizations can also reuse alias values across org and project scopes where it makes sense.
