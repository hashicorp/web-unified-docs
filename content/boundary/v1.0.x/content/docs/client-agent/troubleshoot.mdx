...
| Transparent sessions fail on Windows when you use the default port <br /><br /> (Fixed in Client Agent version 0.21.2) | On Client Agent versions prior to 0.21.2, a port conflict with the Windows Remote Desktop Connection app on modern Windows operating systems (Windows 11+, Windows Server 2025) caused transparent sessions to fail when you used the default port (`3389`) to establish an RDP connection. This issue is resolved in Client Agent version 0.21.2.<br /><br />If you experience this issue, we recommend that you upgrade the Client Agent to the latest Client Agent version. Refer to [Install Boundary clients](/boundary/docs/deploy/self-managed/install-clients).<br /><br />For Client Agent versions prior to 0.21.2, refer to [RDP targets](/boundary/docs/targets/configuration/configure-transparent-sessions#rdp-targets) for 2 possible workarounds.|
