...
When you use the Client Agent to connect to an RDP target from a Windows machine running Parallels, Boundary may create multiple sessions. One session is **Active**, while several others show a **Pending** state. When you disconnect from the target, Boundary creates a new session.
