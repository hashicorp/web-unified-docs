...
Refer to the following sections for specific conflicts with Prallels Desktop.
...
When you disconnect from the target, Boundary creates a new session.

If session recording is enabled for the target, Boundary records all of the **Pending** sessions as well as the **Active** one.
If you cancel the **Pending** sessions, they transition to a **Completed** state, but there is no recording data.
The **Active** session transitions to a **Failed** state.
