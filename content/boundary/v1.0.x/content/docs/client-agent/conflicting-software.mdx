...
If you are unable to establish a transparent session while using the Palo Alto Networking GlobalProtect VPN, you may need to explicitly specify a network interface and the upstream DNS server(s) to use.

You may also experience conflicts with some versions of the Client Agent on macOS.
...
The Palo Alto Networking GlobalProtect VPN client for Mac v6.3.3-828 conflicts with version 0.21.x of the Boundary Client Agent.
This conflict is a known issue, and there is no supported workaround at this time.

If the Client Agent is running, and you connect to the GlobalProtect VPN, the Client Agent may stop working.
If you are already connected to the GlobalProtect VPN, and you start the Client Agent, you may lose internet connectivity.

## Parallels Desktop (macOS VM on macOS host)

Refer to the following sections for specific conflicts with Parallels Desktop.
...
When you use the Client Agent on a Windows VM running in Parallels Desktop on macOS to connect to an RDP target, Boundary may create multiple sessions. One session is **Active**, while several others show a **Pending** state. When you disconnect from the target, Boundary creates a new session.

If session recording is enabled for the target, Boundary records all of the **Pending** sessions as well as the **Active** one. If you cancel the **Pending** sessions, they transition to a **Completed** state, but there is no recording data. The **Active** session transitions to a **Failed** state.

This is a known issue. There is no workaround at this time.
