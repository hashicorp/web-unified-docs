...
# Configure a controller using a Helm chart
...
The `controller.config` contains the Boundary controller HCL. The chart writes the HCL value into a ConfigMap and mounts it at `/etc/boundary/controller.hcl`.
