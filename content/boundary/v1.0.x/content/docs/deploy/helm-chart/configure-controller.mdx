---
layout: docs
page_title: Configure a controller using a Helm chart
description: >-
...
When you enable TLS, keep the listener `tls_cert_file` and `tls_key_file` paths in `controller.config` aligned with `tls.mountPath`, and set `controller.livenessProbe.scheme` and `controller.readinessProbe.scheme` to `HTTPS`.
...
If the `controller.config` references `migration_url = "env://BOUNDARY_PG_MIGRATION_URL"`, the chart injects that environment variable into the controller pods and database Jobs.
...
The default `controller.config` includes `disable_mlock = true`. Keep this setting according to the chart's default security model, which drops all Linux capabilities and disallows privilege escalation.
...
Refer to the following topic for next steps:

To deploy workers, refer to [Deploy workers using a Helm chart](/boundary/docs/deploy/helm-chart/deploy-worker).

## More information

To view example controller configurations, refer to [Example controller configurations](/boundary/docs/deploy/helm-chart/controller-examples).
