---
layout: docs
page_title: Deploy a Boundary controller using a Helm chart
description: >-
...
The chart reads sensitive values from an existing Kubernetes Secret referenced by `secretRefs.secretName`. At a minimum, you must create a Secret with the database URL and Boundary Enterprise license. Include bootstrap admin credentials when bootstrap admin creation is enabled.
...
Keep `disable_mlock = true` with the chart's default security model. The containers run without the elevated privileges required for memory locking.
