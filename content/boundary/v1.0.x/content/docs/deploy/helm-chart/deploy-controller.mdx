...
When `bootstrapAdmin.enabled=true`, the bootstrap admin Job connects to the controller API Service and verifies the TLS certificate. The certificate must include the following SAN (adjust if `nameOverride` or `fullnameOverride` is set):
