---
layout: docs
page_title: Deploy a worker using a Helm chart
description: >-
 Discover resources for deploying Boundary workers using a Helm chart.
---
...
Start from the worker configuration guidance in [Create the worker configuration](https://developer.hashicorp.com/boundary/docs/workers/create) if you want a base worker configuration.
...
This inline AEAD KMS example is mainly suitable for development or testing. The key is visible to anyone who can read the configuration file. For production use, you should use an external KMS as described in [Configure KMS](https://developer.hashicorp.com/boundary/docs/configuration/kms).
