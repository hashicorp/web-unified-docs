...
The following examples show common controller variations that build on the baseline installation in [Deploy controllers](/boundary/docs/deploy/helm-chart/deploy-controller). They focus on the controller chart and assume PostgreSQL, KMS access, and any required Secrets already exist.
...
- You handle database initialization outside Helm.
...
Use this pattern if you handle Boundary identity bootstrap through another workflow.
...
After the migration completes, reset `database.migrate.enabled` to prevent the migration Job from triggering on the next upgrade:
