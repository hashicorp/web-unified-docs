---
layout: docs
page_title: Boundary controller Helm chart examples
description: >-
 Configure common Boundary controller deployment scenarios with the Helm chart,
 including TLS setup, database migration, cluster exposure, and bootstrap admin.
---

# Boundary controller Helm chart examples

Use these controller configuration variations — TLS setup, database migration, bootstrap admin creation, and cluster service exposure — to adapt the baseline controller installation in [Deploy controllers](/boundary/docs/deploy/helm-chart/deploy-controller) to your environment. They assume PostgreSQL and KMS (key management system) access already exist.
...
The TLS certificate must include the following SAN (Subject Alternative Name) (adjust if `nameOverride` or `fullnameOverride` is set):
...
If the migration Job started by `database.migrate.enabled=true` fails, the controller pods remain scaled to zero until the migration succeeds. Refer to the PostgreSQL backup you took before running migrations or repairs to restore the database, then correct the underlying issue (for example, an invalid `database.repair.version`) before setting `database.migrate.enabled=true` again and re-running `helm upgrade`.
