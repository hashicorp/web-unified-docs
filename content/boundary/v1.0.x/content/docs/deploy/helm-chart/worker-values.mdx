---
layout: docs
page_title: Boundary worker Helm chart values
description: >-
 Configure the Boundary worker Helm chart using this reference for runtime
 configuration, persistence, proxy and ops services, security, and
 scheduling.
---

# Boundary worker values

You configure the Boundary worker Helm chart with Helm values and a Boundary HCL configuration; the chart is the source of truth for supported values.
...
| `serviceAccount.annotations` | `{}` | Annotations added to the ServiceAccount. Use to attach IRSA (IAM Roles for Service Accounts, `eks.amazonaws.com/role-arn`) or Workload Identity (`iam.gke.io/gcp-service-account`) annotations for cloud-provider KMS (key management system) access. |
...
### Worker crash-loops after enabling auth storage

If the worker pod crash-loops after you enable auth storage, confirm that `auth_storage_path` in `worker.config` matches the mounted PVC path set by `worker.persistence.authStorage.path`. A mismatch between the two means the worker cannot read or write its identity material at the expected location.
...
To install a worker using these values, refer to [Deploy workers using a Helm chart](/boundary/docs/deploy/helm-chart/deploy-worker).
