---
layout: docs
page_title: Boundary controller Helm chart values
description: >-
 Configure the Boundary controller Helm chart using this reference for runtime
 configuration, Kubernetes infrastructure, security, and database job values.
---

# Boundary controller values

You configure the Boundary controller Helm chart with Helm values and a Boundary HCL configuration; the chart is the source of truth for supported values.
...
- AEAD (Authenticated Encryption with Associated Data) KMS (key management system) keys are not referenced through `env://BOUNDARY_KMS_*` inside `kms` blocks.
...
### Controller pods fail readiness or liveness probes

If controller pods fail their readiness or liveness probes after you change TLS settings, check whether `controller.livenessProbe.scheme` or `controller.readinessProbe.scheme` matches the `tls_disable` setting on the ops listener in `controller.config`. When a probe scheme is not explicitly set, the chart derives it from `tls_disable` (`HTTPS` when `tls_disable=false`, `HTTP` when `tls_disable=true`). If you override the probe scheme manually, make sure it stays aligned with the ops listener whenever you change `tls_disable`.
...
To install a controller using these values, refer to [Deploy controllers using a Helm chart](/boundary/docs/deploy/helm-chart/deploy-controller).
