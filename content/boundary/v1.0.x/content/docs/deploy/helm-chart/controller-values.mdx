...
| `image.tag` | `1.0.0-ent` | Controller image tag |
...
The chart automatically adds liveness and readiness probes when an ops TCP listener is defined in `controller.config`. You do not need to set these values unless you want to adjust probe timing. The chart automatically sets the probe scheme to `HTTPS` when the ops listener in `controller.config` has TLS enabled.

| Key | Default | Description |
| --- | --- | --- |
| `controller.livenessProbe.scheme` | `HTTP` | Probe scheme for `/health` on the ops listener. The chart sets this to `HTTPS` automatically when the ops listener has TLS enabled in `controller.config`. |
...
| `controller.readinessProbe.scheme` | `HTTP` | Readiness probe scheme for `/health` on the ops listener. The chart sets this to `HTTPS` automatically when the ops listener has TLS enabled in `controller.config`. |
