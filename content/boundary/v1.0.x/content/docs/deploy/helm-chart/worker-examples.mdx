...
- [Worker with ingress capabilities](#worker-with-ingress-capabilities)
- [Worker with intermediate capabilities](#worker-with-intermediate-capabilities)
- [Worker with egress capabilities](#worker-with-egress-capabilities)
- [Worker with a pre-configured DNS address](#worker-with-a-pre-configured-dns-address)
- [Worker-led registration](#worker-led-registration)
- [Controller-led registration](#controller-led-registration)
- [KMS-backed worker authentication](#kms-backed-worker-authentication)
- [Enable session recording storage](#enable-session-recording-storage)
...
### Token from a Kubernetes Secret using secretRefs

Store the activation token in a Kubernetes Secret and inject it as an environment variable. The chart maps the Secret key to `BOUNDARY_WORKER_CONTROLLER_GENERATED_ACTIVATION_TOKEN`, which Boundary reads at startup via the `env://` prefix.
...
### Token via extraEnv in a values file

Use `extraEnv` to inject the token directly as an environment variable without creating a Secret. This is useful when the token is managed by an external secrets operator or CI pipeline that injects values at install time.
...
### Token via extraEnv at install time

Pass the token at install time using `--set` without adding it to a values file. This avoids storing the token on disk and is useful for one-time installs or scripted pipelines.

Set `controller_generated_activation_token` to the `env://` reference in `worker.config` as shown above, then run:
...
$ helm install boundary-worker hashicorp/boundary-worker \
  ...
  --set 'extraEnv[0].name=BOUNDARY_WORKER_CONTROLLER_GENERATED_ACTIVATION_TOKEN' \
  --set 'extraEnv[0].value=<activation-token>'
```
