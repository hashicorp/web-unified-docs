---
layout: docs
page_title: Deploy Boundary workers
description: >-
...
"Ingress," "intermediate," and "egress" are general ways to describe how the respective worker interacts with resources.
A worker can serve more than one of those roles at a time.
Refer to [Multi-hop sessions](/boundary/docs/workers/multi-hop) for more information.
...
- **Expired `auth_request_token`**: The Worker Auth Registration Request token is time-limited. If registration fails because the token has expired, restart the worker to generate a new token and repeat the registration steps.
- **Worker fails to register**: Confirm that the worker's configuration file references the correct KMS key or authorization method for its registration flow (worker-led, controller-led, or KMS), and that the controller's cluster listener is reachable from the worker's network boundary.
