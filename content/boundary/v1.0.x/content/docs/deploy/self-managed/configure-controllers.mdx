---
layout: docs
page_title: Configure Boundary controllers
description: >-
...
last_modified: 2025-12-03T09:28:40-05:00
# END AUTO GENERATED METADATA
---

# Configure Boundary controllers

Configuring a Boundary controller means preparing TLS certificates, generating root and recovery KMS keys, connecting a PostgreSQL database, and starting the controller as a systemd service so it can accept client connections and coordinate workers.
This page assumes you have already installed Boundary on at least three controller nodes, as described in [Install Boundary](/boundary/docs/deploy/self-managed/install).
...
## What KMS keys does a Boundary controller need?
...
One worker uses a PKI (public key infrastructure) exchange to authenticate with the controllers. The other uses a KMS key for authentication by both the worker and the controller.
...
Refer to the table below for explanations of the parameters used in the example above:

| Parameter | Type | Description |
|-----------|------|-------------|
| `disable_mlock` | bool: false | Disables the server from executing the `mlock` syscall, which prevents memory swap to the disk. Preventing memory swap is fine for local development and testing. However, HashiCorp does not recommend using it for production unless the systems running Boundary use encrypted swap or do not use swap at all. Boundary only supports memory locking on UNIX-like systems that support the `mlock()` syscall, like Linux and FreeBSD. |
| `listener` | block | Configures the listeners on which Boundary serves traffic (API cluster and proxy). |
| `controller` | block | Configures the controller. If present, `boundary server` starts a controller subprocess. |
| `events` | block | Configures event-specific parameters. |
| `kms` | block | Configures KMS blocks for [various purposes](/boundary/docs/secure/encryption/data-encryption). |

On Linux, to give the Boundary executable the ability to use the `mlock` syscall without running the process as root, run the following command:
...
Common issues when configuring a Boundary controller include:

- **TLS certificate and key mismatch**: Confirm that the certificate and key files at `/etc/boundary.d/tls/boundary-cert.pem` and `/etc/boundary.d/tls/boundary-key.pem` are a matching pair, and that the certificate's CN or SAN matches your controller's DNS record.
- **KMS permission errors**: Confirm that the IAM roles or permissions associated with your KMS credentials (such as `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`) grant access to the specific KMS keys referenced in your `controller.hcl` file.
- **Database connection failures**: Confirm that the PostgreSQL connection string in `boundary.env` is correct and that the database is reachable from each Boundary controller node.
