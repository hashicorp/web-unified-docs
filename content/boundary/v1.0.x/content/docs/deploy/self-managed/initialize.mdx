...
Initializing Boundary creates the first database resources — the default scope, auth method, admin account, and role — that let you log in and manage the cluster for the first time, either automatically during database setup or manually using the recovery KMS.

Before you begin, you should have already initialized the Boundary database, as described in [Configure controllers](/boundary/docs/deploy/self-managed/configure-controllers#initialize-the-database).
...
To access a new Boundary instance for the first time, you must use the recovery KMS (Key Management System) defined in the
...
1. Create org and project scopes.
1. Create an auth method.
1. Create a login account.
1. Create a user.
1. Create roles to manage scopes.
...
| Role | Scope | Purpose |
|------|-------|---------|
| `global_anon_listing` | Global | Allows anonymous (unauthenticated) users the ability to list scopes and auth methods in the global scope. |
| `org_anon_listing` | Organization | Allows anonymous (unauthenticated) users the ability to list scopes and auth methods in the organization scope. |
| `org_admin` | Organization | Allows `myuser` user administration grants at the org scope. |
| `project_admin` | Project | Allows `myuser` user administration grants at the project scope. |
...
Do not delete the default anonymous login role or remove its auth token grant.
The Boundary Client Agent daemon and Boundary search functionality require this role so users can add auth tokens to the client cache and search cached resources.

Boundary initialization paths may expose this role as `Login Grants`, `Login and Default Grants`, or, in HCP Boundary, `global_anon_listing`.
The role is associated with the `u_anon` principal in the global scope and should apply to descendant scopes.
...
Verify the role name and deployment-specific behavior for your Boundary deployment before you modify or remove the role.
...
Common issues when initializing Boundary include:

- **Incorrect `-recovery-config` path**: Confirm the path passed to `-recovery-config` (or the `BOUNDARY_RECOVERY_CONFIG` environment variable) points to a valid KMS configuration file containing a `recovery` purpose block that matches the controller configuration.
- **Grant string typos**: Confirm each `-grant` value follows the `ids=<id>;type=<type>;actions=<action>` syntax exactly. A malformed grant string causes the role to grant unintended (or no) permissions.

## Next steps

After you initialize Boundary, you should [Install the Boundary Clients](/boundary/docs/deploy/install-clients).