...
Use Terraform patterns to create Boundary password and LDAP auth methods, then add accounts and associate them with users — the building blocks for assigning group membership and role-based access. Boundary supports password, OIDC (OpenID Connect), and LDAP (Lightweight Directory Access Protocol) auth methods.

This page covers creating auth methods, accounts, and users — for grouping users and assigning roles, refer to the groups and RBAC page.

| Auth method | Best for |
|---|---|
| Password | Simple, self-managed credentials |
| LDAP | Existing directory-based identity providers |
| OIDC | Federated identity providers supporting OpenID Connect |
...
The next example demonstrates how to configure an LDAP auth method to connect Boundary to your existing identity provider.
...
## Related auth method and account documentation
