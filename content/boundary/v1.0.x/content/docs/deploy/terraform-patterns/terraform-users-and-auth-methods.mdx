...
Use Terraform patterns to create Boundary password and LDAP auth methods, then add accounts and associate them with users for assigning group membership and role-based access. Boundary supports password, OIDC (OpenID Connect), and LDAP (Lightweight Directory Access Protocol) auth methods.
