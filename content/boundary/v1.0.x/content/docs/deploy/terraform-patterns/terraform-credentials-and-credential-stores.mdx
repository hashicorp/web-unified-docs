...
Boundary supports static credentials (username/password, SSH keys, JSON, SSH certificates) and Vault dynamic credentials. Use Terraform patterns to create credential stores and credentials, or credential libraries that generate dynamic Vault credentials on demand for hosts.

This page covers creating credentials and credential stores — for attaching credentials to a target, refer to the targets page.

| Credential store type | Credential type | Best for |
|---|---|---|
| Static | Username/password, SSH key, JSON, SSH certificate | Fixed, manually managed credentials |
| Vault | Dynamic, generated per session | Rotating, short-lived credentials via Vault |
...
## Related credential and credential store documentation
