...
Use Terraform patterns to create Boundary hosts and host catalogs, then attach them to host sets that targets connect to.

This page covers creating hosts and host catalogs — for connecting them to targets, refer to the targets page.
...
| Host catalog type | Best for |
|---|---|
| Static | Small, fixed, manually managed host lists |
| AWS plugin | Auto-discovering EC2 hosts by tag/region |
| Azure plugin | Auto-discovering Azure hosts by tag/region |
...
## Related host, host set, and host catalog documentation
