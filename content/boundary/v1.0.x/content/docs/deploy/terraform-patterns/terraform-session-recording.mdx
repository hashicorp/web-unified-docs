...
Use Terraform patterns to enable Boundary session recording. Create a storage policy that defines retention rules for a scope, then attach a storage bucket that holds the AWS (Amazon Web Services) S3-backed recordings — the last prerequisite before enabling recording on a target. This example does not demonstrate creating the S3 bucket or the associated IAM (Identity and Access Management) policies. Refer to the [create storage buckets](/boundary/docs/session-recording/configuration/create-storage-bucket) documentation to learn how to configure AWS.
