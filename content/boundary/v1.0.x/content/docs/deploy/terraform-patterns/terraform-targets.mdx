...
Once you have defined a host, a host catalog, and a credential store, use Terraform patterns to create Boundary [targets](/boundary/docs/domain-model/targets) — the resource users connect to, with support for SSH and TCP protocols, injected or brokered credentials, and optional session recording.
