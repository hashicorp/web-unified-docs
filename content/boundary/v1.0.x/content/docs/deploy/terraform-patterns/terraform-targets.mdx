...
Once you have defined a host, a host catalog, and a credential store, use Terraform patterns to create Boundary [targets](/boundary/docs/domain-model/targets). Targets define the resources that users connect to and support SSH and TCP protocols, injected or brokered credentials, and optional session recording.
