...
last_modified: 2026-08-10T22:29:42.000Z
...
- [RBAC (Role-Based Access Control)](/boundary/docs/rbac) - Boundary's allow-only permissions model enables authorization for authenticated connections by granting capabilities to roles which are then assigned to users, groups, or managed groups.
...
Boundary integrates with systems like HashiCorp Vault, cloud IAM (Identity and Access Management) services, and other credential stores, but cannot protect against exploits targeting these external services.
...
![HCP Boundary deployed in a single AWS region across three availability zones, with Boundary workers and controllers running as Nomad jobs and an Aurora Postgres cluster outside the VPC](/img/security-model/boundary-architecture_light.png#light-theme-only)
![HCP Boundary deployed in a single AWS region across three availability zones, with Boundary workers and controllers running as Nomad jobs and an Aurora Postgres cluster outside the VPC](/img/security-model/boundary-architecture_dark.png#dark-theme-only)
...
![A user reaching the Vault database engine, which issues dynamic credentials to an Aurora Postgres cluster under IAM controls](/img/security-model/data-at-rest_light.png#light-theme-only)
![A user reaching the Vault database engine, which issues dynamic credentials to an Aurora Postgres cluster under IAM controls](/img/security-model/data-at-rest_dark.png#dark-theme-only)
...
![Users connecting to the Boundary control plane, which brokers credentials from Vault and routes the session through a worker to the target system](/img/security-model/data-in-transit_light.png#light-theme-only)
![Users connecting to the Boundary control plane, which brokers credentials from Vault and routes the session through a worker to the target system](/img/security-model/data-in-transit_dark.png#dark-theme-only)

### Identity and access in HCP Boundary
