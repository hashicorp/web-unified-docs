...
Boundary secures all three of its connection types — client-to-controller, client-to-worker, and worker-to-upstream — with TLS or mutually authenticated TLS (mTLS), and for client-to-worker connections it establishes certificates dynamically so workers require no manual TLS configuration.
...
It is possible to require client certificates; see the [configuration for `listener` blocks](/boundary/docs/configuration/listeners/tcp-listeners#tls) for the available TLS parameters.
...
determined dynamically via SNI (Server Name Indication), and the session is then mutually authenticated.
Here's how it works:
...
takes place can happen in one of two ways: by a built-in X.509-based PKI (public key infrastructure) system,
...
3. The Worker establishes a TLS 1.3 connection to the Controller. The encrypted
   value is transmitted to the Controller via the TLS ALPN (Application-Layer Protocol Negotiation) field as numbered
   chunks.
