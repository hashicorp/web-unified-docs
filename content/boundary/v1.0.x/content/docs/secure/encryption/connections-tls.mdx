...
Boundary secures all three of its connection types, client-to-controller, client-to-worker, and worker-to-upstream, with TLS or mutually authenticated TLS (mTLS). For client-to-worker connections, it establishes certificates dynamically so workers require no manual TLS configuration.
