...
last_modified: 2026-09-10T20:01:54.000Z
...
## Vendor-specific attributes and secrets

There are specific attributes and secrets for the Amazon S3, MinIO, and S3-compliant cloud vendors:

<Tabs>

<Tab heading="AWS S3" group="AWS">
...
<Tab heading="MinIO" group="MinIO">

A MinIO storage bucket contains the bucket name, endpoint URL, and an optional region, as well as any credentials needed to access the bucket, such as the access key ID and secret access key.

<Tabs>

<Tab heading="Attributes">

MinIO buckets have the following attributes:
...
MinIO buckets can have the following secrets:
...
<Tab heading="S3-compliant" group="S3">

An S3-compliant storage bucket contains the bucket name, endpoint URL, and an optional region, as well as any credentials needed to access the bucket, such as the access key ID and secret access key.

<Tabs>

<Tab heading="Attributes">

S3-compliant buckets have the following attributes:
...
- `disable_credential_rotation`: (Optional) Controls whether the plugin will rotate the incoming credentials and manage a new storage service account. If this attribute is set to `false`, or not provided, the plugin will rotate the incoming credentials to create a new storage service account, then delete the incoming credentials.
  ...
  Note that credential rotation is not supported for Hitachi Content Platform, and it may not function for other S3-compliant providers.

</Tab>

<Tab heading="Secrets">

S3-compliant buckets can have the following secrets:
