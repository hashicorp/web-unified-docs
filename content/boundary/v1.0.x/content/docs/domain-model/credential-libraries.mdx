...
last_modified: 2026-09-02T13:22:03.000Z
...
Note that all SSH certificates issued by a Vault SSH certificate credential library use the `SSH_CERT_TYPE_USER` certificate type mentioned in the OpenSSH definition link.
   ...
   Starting in Boundary version 1.0.2, the `additional_valid_principals` attribute supports parameter templating using session and account variables, for example `{{.AccountSubject}}`, `{{.Account.Email}}`, and `{{.User.Name}}`.
