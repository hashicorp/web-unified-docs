...
last_modified: 2026-09-03T12:57:09.000Z
...
### Vault LDAP credential library attributes

You can configure a credential library using [Vault's LDAP secrets engine](/vault/docs/secrets/ldap).

The Vault LDAP credential library has the following additional attribute:
...
Note that all SSH certificates issued by a Vault SSH certificate credential library use the `SSH_CERT_TYPE_USER` certificate type mentioned in the OpenSSH definition link.
   ...
   Starting in Boundary version 1.0.2, the `additional_valid_principals` attribute supports parameter templating using session and account variables, for example `{{.Account.Subject}}`, `{{.Account.Email}}`, and `{{.User.Name}}`.

   Static values continue to work unchanged. You can include both a static principal such as `ec2-user` and a dynamically resolved principal such as `{{.Account.Subject}}` in the same list.
...
Starting in Boundary version 1.0.2, the `additional_valid_principals` attribute supports parameter templating using session and account variables, for example `{{.Account.Subject}}`, `{{.Account.Email}}`, and `{{.User.Name}}`. Static values continue to work unchanged.
...
The `username` and `key_id` fields support the following variables:

- `{{.User.Name}}` - The user's name from the user resource.
- `{{.Account.Id}}` - The account's ID.

Starting in Boundary version 1.0.2, the `additional_valid_principals` field also supports the following variables:
...
- `{{.Account.Id}}` - The account's ID.
- `{{.Account.Name}}` - The name of the account from the account resource.
- `{{.Account.LoginName}}` - The account's login name, if the account uses a login name.
- `{{.Account.Subject}}` - The account's subject, if the account uses a subject.
- `{{.Account.Email}}` - The account's email, if the account uses an email.
