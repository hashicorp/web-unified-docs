...
- `-scope-id=<string>` - Scope in which to create the alias. The default is `global`.
  ...
  You must create org and project suffixes before you can create an alias in a project scope. Otherwise, you can only create the alias in the `global` scope. Refer to [Create a suffix for a scope](/boundary/docs/targets/configuration/create-suffix) for more information.
