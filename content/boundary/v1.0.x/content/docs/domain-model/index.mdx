...
The resources described so far make up Boundary's control plane. [Workers][] make up its data plane. A worker is a resource that represents a service that proxies [sessions][] between [users][] and [targets][], which lets you provide access to private resources without exposing the networks they run on. Workers can also be setup to reach Vault through multihop, and provide access to storage buckets for session recording.
