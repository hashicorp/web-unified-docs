...
![Boundary scope hierarchy showing the global scope containing an org scope, which contains projects that hold roles, targets, and host catalogs](/img/boundary-domain-model-overview_light.png#light-theme-only)
![Boundary scope hierarchy showing the global scope containing an org scope, which contains projects that hold roles, targets, and host catalogs](/img/boundary-domain-model-overview_dark.png#dark-theme-only)
...
![Relationship between IAM components: users and groups in an org are assigned to roles in each project, and each role holds grants](/img/component-relationship_light.png#light-theme-only)
![Relationship between IAM components: users and groups in an org are assigned to roles in each project, and each role holds grants](/img/component-relationship_dark.png#dark-theme-only)

## Workers

The resources described so far make up Boundary's control plane. [Workers][] make up its data plane. A worker is a resource that represents a service that proxies [sessions][] between [users][] and [targets][], which lets you provide access to private resources without exposing the networks they run on. Workers can also be set up to reach Vault through multi-hop configurations, and provide access to storage buckets for session recording.
...
- **[Auth token][]** :
  ...
  through an [authentication method][].
...
[auth token]: /boundary/docs/domain-model/auth-tokens
[auth tokens]: /boundary/docs/domain-model/auth-tokens
