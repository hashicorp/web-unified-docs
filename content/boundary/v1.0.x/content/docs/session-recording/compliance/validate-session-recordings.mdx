...
description: >-
  Verify Boundary session recording integrity using ed25519 signatures and SHA256 checksums. Unwrap BSR keys from your KMS and confirm file checksums in six steps.
...
-  The BSR key is a plaintext AES-GCM (Advanced Encryption Standard in Galois/Counter Mode) key.
...
For information about how Boundary validates BSR files during upload to S3, refer to [Validate data integrity in the external object store](/boundary/docs/session-recording/compliance/validate-data-store).
...
For more information about the BSR file structure, refer to [BSR file data structure](/boundary/docs/session-recording/data/bsr-file-structure).
...
If verification errors occur during recovery, refer to [Session recording recovery](/boundary/docs/session-recording/recovery).
