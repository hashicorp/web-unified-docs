...
description: >-
  The Boundary BSR file stores session recordings in hierarchical session, connection, and channel folders with metadata JSON files and SHA256 checksums.
...
A BSR connection directory contains a summary of connections, as well as inbound and outbound requests.
If you use a multiplexed protocol, there are subdirectories for the channels.

Every directory contains a SHA256SUM and SHA256SUM.sig file, to be used for cryptographically verifying the contents of
that directory. The SHA256SUM file contains rows of file names paired with a checksum for file contents. The
SHA256SUM.sig file is a copy of the SHA256SUM file, signed with the BSR’s private key. For more information on verifying a
...
The following example BSR structure represents a multiplexed session recording with the ID `sr_iNCdGSREeX`. The session recording contains one connection,
...
- `SHA256SUM` is a plaintext file that contains rows of file names paired with a checksum for file contents.
...
- `wrappedBsrKey` is the BSR key wrapped by the external KMS AES-GCM (Advanced Encryption Standard in Galois/Counter Mode) key that you configure.
...
and the connection ids. For each connection id listed, there should be a corresponding connection directory in the session directory.
...
- `requests-inbound.data` is a binary file containing all inbound protocol request messages transmitted for the connection.
- `requests-outbound.data` is a binary file containing all outbound protocol request messages transmitted for the connection.
...
error: error message would appear here
```
...
A BSR channel folder contains the following files:
...
- `requests-inbound.data` is a binary file containing all inbound protocol request messages transmitted for the channel.
- `requests-outbound.data` is a binary file containing all outbound protocol request messages transmitted for the channel.
- `messages-inbound.data` is a binary file containing all inbound protocol data transmitted for the channel.
- `messages-outbound.data` is a binary file containing all outbound protocol data transmitted for the channel.
...
For reading BSR `.data` files, refer to [Read a BSR file](/boundary/docs/session-recording/data/read-bsr-file).
