...
description: >-
  Boundary BSR data files store session recording data as binary chunks. Understand the chunk structure and how SSH recordings become asciinema playback files.
...
BSR (Boundary Session Recording) data files are where all the data transmitted during a session is stored. BSR data files
...
chunk in a BSR file is the header chunk, and the last chunk in a BSR file is the EOF (end-of-file) chunk. The remaining
...
using a protobuf (Protocol Buffers) definition provided here: [SSH Chunks](https://github.com/hashicorp/boundary/blob/main/internal/bsr/proto/ssh/v1/ssh_chunks.proto).
...
For the directory structure that contains BSR `.data` files, refer to [BSR file data structure](/boundary/docs/session-recording/data/bsr-file-structure).
