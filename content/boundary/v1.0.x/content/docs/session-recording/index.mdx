...
description: >-
  Boundary session recording captures every SSH and RDP user session as a tamper-proof BSR file, giving administrators a verifiable audit trail for compliance.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-05-11T10:18:43-04:00
last_modified: 2026-09-04T13:15:34.000Z
...
This limitation applies to Boundary versions prior to 1.0.1 only. Starting in version 1.0.1, the 5 GB limit is removed.

If you use Amazon S3 with a version prior to 1.0.1, recorded session files cannot exceed 5 GB due to Amazon's PUT operation size limit. If a file exceeds 5 GB, the session details show the recording state as `Failed` and playback may not be available. To avoid large recordings, limit the use of Secure File Copy (SCP) to transfer large files during recorded sessions.

</Warning>

## SSH recording format

When you view recorded SSH sessions using the CLI or Admin UI, Boundary can convert the recording of an individual SSH channel into an asciicast format to play back an interactive SSH session.
...
Starting in Boundary version 1.0.2, if an SSH session uses the `RemoteCommand` option or executes a non-interactive command, Boundary displays the command alongside its recorded output.
The additional context helps you determine what produced the output in the recording.
...
An attacker could change the `path` variable to point to a malicious program or change the behavior of a normal program so that it performs a malicious action.

- asciicast silently ignores any requests that do not have an explicit handler, even though they may cause significant changes to code execution during the SSH session.
An attacker could execute malicious code using a request without an explicit handler.

## RDP recording format
...
Boundary does not record RDP sessions directly as video. Instead, Boundary first records the session in BSR format. You export the data when you want a playable/downloadable video artifact for a recorded RDP connection. An export is a Boundary resource that tracks the conversion of a recorded RDP connection into video.

When you create an export, Boundary uses the stored RDP connection recording to create a video. This video file is encoded using AV1 and containerized into the WebM format. The result is a `video/webm` file.

For more information about how session recording exports work, refer to [RDP session recording exports](/boundary/docs/session-recording/data/exports).
...
A session recording can contain one or more connection recordings. Boundary creates an export from an RDP connection recording. When the export finishes successfully, Boundary produces a `video/webm` artifact for that connection recording.
...
Once you enable session recording, you can:

- [Find and view SSH sessions](/boundary/docs/session-recording/configuration/manage-recorded-sessions)
- [Export and view RDP sessions](/boundary/docs/session-recording/configuration/export-rdp)
...
- [RDP session recording exports](/boundary/docs/session-recording/data/exports)
- [Delete a session recording export](/boundary/docs/session-recording/data/delete-exports)

### Session recording recovery

If an SSH session recording fails, Boundary may be able to recover it.
Refer to [Session recording recovery](/boundary/docs/session-recording/recovery) for more information.
