...
description: >-
  Configure MinIO as a Boundary session recording storage provider. Set up a service account, apply the required S3 IAM policy, and enable credential rotation.
...
- A MinIO storage bucket must be configured with read/write (R/W) access. If you use a
