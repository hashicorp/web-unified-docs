...
description: >-
  Configure Amazon S3 for Boundary session recordings. Choose between static IAM credentials or dynamic AssumeRole, with optional KMS encryption for stored recordings.
...
- An AWS IAM (Identity and Access Management) role policy with the following statement:
   ...
   If you use dynamic credentials, after you create an IAM role with this policy attached, attach the role to the instance that the self-managed worker runs on.

- If you apply KMS (Key Management Service) encryption to the storage bucket, you must add these additional permissions to the role policy for the storage bucket's IAM user:
