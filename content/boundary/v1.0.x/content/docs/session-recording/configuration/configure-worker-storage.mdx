...
description: >-
  Configure Boundary workers for session recording storage. Set the recording path and minimum capacity thresholds, and understand the storage states that control worker availability.
...
<Note>
  ...
  You cannot use an HCP (HashiCorp Cloud Platform) managed worker for the local storage. HCP Boundary users must configure a self-managed worker to enable session recording.
...
- Have an accessible directory defined by `recording_storage_path` for storing session recordings while they are in progress. On session closure, Boundary moves the local session recording to remote storage and deletes the local copy.
   ...
   - For Boundary Enterprise, refer to the [worker configuration](/boundary/docs/workers#session-recording) documentation.
...
- `Out of storage`: The worker is out of local disk space. It is not available to record new sessions or play back existing recordings. An administrator must free disk space on the worker to remedy the issue. Do not delete session recordings that are mid-recovery to free up disk space.
...
To recover a worker from an unhealthy state, free disk space in the `recording_storage_path` directory.
The worker returns to the `available` state on its own, and does not require a restart.

A worker in an unhealthy state does not return to `Available` as soon as it crosses back over the threshold.
...
## Example worker HCL configuration
...
The Boundary service should also have access to the `auth_storage_path` defined in the worker configuration file.

For example,

```shell-session
$ sudo chown boundary:boundary /local/storage/directory
```

To learn more about running the Boundary service under systemd, refer to the [Deploy workers: Manually configure systemd](/boundary/docs/deploy/self-managed/deploy-workers#manually-configure-systemd-optional) documentation.
