...
description: >-
  Troubleshoot Boundary session recording issues: stale recordings after AWS credential rotation, unsupported RDP recovery, and S3 access debugging tips.
...
Use this page to diagnose and resolve common Boundary session recording issues,
including stale recordings after AWS credential rotation, unsupported RDP
recovery workflows, and S3 storage access failures.
...
**Unsupported RDP recovery workflow during worker failure:**

If a worker fails during an RDP recording, there is no way to recover the recording.
...
- Crashes and restarts while servicing connections and channels for session to an RDP target
...
Interrupted SSH recordings may be able to be recovered.
