---
layout: docs
page_title: Boundary connection workflows
description: >-
  Compare Boundary's target connection workflows, including transparent sessions, connect helpers, the -exec flag, and SSH ProxyCommand, to pick the right fit.
...
# Boundary connection workflows

Boundary offers several workflows for connecting to a target, including the `boundary connect` command, transparent sessions, connect helpers, the `-exec` flag, SSH ProxyCommand, and multi-hop sessions. Each workflow trades setup effort against how transparent the connection feels to end users. This page compares the workflows so you can choose one; for setup steps, refer to each workflow's own page.
