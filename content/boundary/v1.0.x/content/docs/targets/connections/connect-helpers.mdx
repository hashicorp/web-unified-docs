---
layout: docs
page_title: Boundary connect helpers
description: >-
  Use Boundary connect helpers to automatically accept host key prompts and connect to Cassandra, HTTP, Kubernetes, MySQL, PostgreSQL, RDP, and SSH targets.
...
Boundary connect helpers automatically accept host key prompts and invoke the right client — `psql`, `mongosh`, `kubectl`, and others — with the correct connection parameters, so you can connect to Cassandra, HTTP, Kubernetes, MySQL, PostgreSQL, RDP, Redis, and SSH targets without manual setup.
Refer to [Connection workflows](/boundary/docs/targets/connections) for an overview of all the ways to connect to a target.
...
Refer to [Exec flag](/boundary/docs/targets/connections/exec-flag) for more information about the `-exec` flag.
...
The RDP (Remote Desktop Protocol) connect helper determines which RDP client to use depending on your operating system.
