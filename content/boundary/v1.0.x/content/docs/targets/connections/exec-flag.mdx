---
layout: docs
page_title: Boundary -exec flag
description: >-
  Use the Boundary -exec flag to wrap TCP sessions in your preferred client and create an authenticated proxy for tools without built-in connect helper support.
...
The Boundary `-exec` flag lets [`boundary connect`](/boundary/docs/commands/connect) wrap a TCP session in any client, even one without a [built-in connect helper](/boundary/docs/targets/connections/connect-helpers), so you can create an authenticated proxy to almost any tool.
...
_Output: PuTTY opens a new session window connected through the authenticated proxy. (Verify against a live run.)_
...
## Change the target's default port
