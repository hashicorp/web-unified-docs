---
layout: docs
page_title: Boundary SSH ProxyCommand
description: >-
  Configure SSH ProxyCommand to proxy an SSH connection through Boundary using a target's ID or domain, without running boundary connect ssh directly each time.
...
# Boundary SSH ProxyCommand

Boundary lets you proxy an SSH connection to a target using the SSH `ProxyCommand` option, so you can run `ssh <target>` instead of `boundary connect ssh` directly.
You can configure `.ssh/ssh_config` to reference a target by its ID or its domain.

Refer to [Connection workflows](/boundary/docs/targets/connections) for other ways to connect to a target.

## Configure a host using target ID

1. Add a `Host` entry to `.ssh/ssh_config` that matches your target IDs.
1. Set `ProxyCommand` to invoke `boundary connect` with the `-exec nc` flag, passing `{{boundary.ip}}` and `{{boundary.port}}` to `nc`.
1. Run `ssh <target-id>` to start the session.

To use SSH `ProxyCommand`, you must configure a `Host` entry in `.ssh/ssh_config` for `localhost`.

In the following example, `ProxyCommand` configures the SSH client to invoke [`boundary connect`](/boundary/docs/commands/connect).
...
### SSH to target using the target ID
...
1. Authorize a session to the target and store the token in an environment variable.
1. Configure a `Host` entry in `.ssh/ssh_config` to invoke `boundary connect` using the stored token.
1. Run `ssh user@localhost` to start the session.
...
### SSH to target using the target domain
...
If `ssh` doesn't invoke `boundary connect`, confirm the `Host` pattern in `.ssh/ssh_config` matches your target ID or domain exactly.
