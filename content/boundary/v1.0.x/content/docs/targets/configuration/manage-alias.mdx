...
description: >-
  Edit or delete existing Boundary target aliases, including changing which target or host an alias points to, from the Admin Console or CLI.
...
You can edit or delete existing Boundary target aliases at any time — change which target or host an alias points to, or update its name, description, or value.
...
1. Use the following command to delete an alias:
   ...
   The output confirms the deletion (verify against a live run):
...
If you haven't created an alias yet, refer to [Create target aliases](/boundary/docs/targets/configuration/create-target-alias).
