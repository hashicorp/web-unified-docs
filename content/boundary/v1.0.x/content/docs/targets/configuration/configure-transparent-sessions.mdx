...
Ensure that you have authorization to establish a session to the target.
  ...
  On Client Agent versions prior to 0.21.2, RDP targets require a custom port for transparent sessions.
...
The port conflict has been resolved in Client Agent version 0.21.2.
To upgrade to the latest Client Agent version, refer to [Install Boundary clients](/boundary/docs/deploy/self-managed/install-clients).

</Note>

On Client Agent versions prior to 0.21.2, a port conflict with the Windows Remote Desktop Connection app on modern Windows operating systems (Windows 11+, Windows Server 2025) caused transparent sessions to fail when you used the default port (`3389`) to establish an RDP connection.
...
If you are on a Client Agent version lower than 0.21.2, you must use a custom port to connect to an RDP target using a transparent session from a Windows client.
Two primary workarounds are available:
