---
layout: docs
page_title: Overview of target aliases in Boundary
description: >-
  Configure Boundary target aliases and transparent sessions to simplify connection workflows and let end users skip target IDs and ephemeral ports entirely.
...
# Overview of target aliases in Boundary

Boundary target aliases let end users connect to targets using a short, memorable, DNS-like string instead of a target ID or name-and-scope combination — simplifying connection workflows and enabling transparent sessions.
...
You may receive connection errors if the alias does not match the website's hostname exactly. The alias is used during TLS negotiation.
If the alias does not match the certificate, the TLS handshake fails.
...
Aliases created in project scopes automatically include project and org suffixes in the following format:

`alias.projectsuffix.orgsuffix`

The suffixes can cause end-to-end TLS to fail if the full alias value does not match the server certificate's Common Name or Subject Alternative Name.
To maintain end-to-end TLS for a website resource, create the alias in the `global` scope and ensure it matches the hostname.
You can create aliases in project scopes if you control the certificate and can configure it to match the full alias value, including suffixes.

For more information, refer to [Suffixes](#suffixes).
