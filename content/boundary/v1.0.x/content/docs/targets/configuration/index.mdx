...
You can create aliases in the `global` or project scopes. However, you can associate aliases with targets or hosts from any scope. Aliases created in project scopes require a suffix.
...
Before you can create an alias for a project scope, you must create a suffix for the project scope. Then, when you create the alias in the project scope, Boundary automatically appends the suffix to it.

Suffixes ensure that aliases are globally unique across scopes, even when it makes sense to use the same alias across multiple project scopes.
