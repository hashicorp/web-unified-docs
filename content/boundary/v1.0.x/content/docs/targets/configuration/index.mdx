...
You can create aliases in the `global`or project scopes. Aliases created in project scopes require a suffix.
...
Creating aliases at the project scope level gives your organization increased granularity and control over scope resources. Project-level administrators can manage their scope's aliases without relying on a global administrator.

Aliases created in project scopes require a suffix. Boundary automatically appends the suffix to any aliases created in the scope. You also must create a suffix for the containing org scope. Suffixes ensure that aliases are globally unique across scopes, even when you use the same alias value across multiple org or project scopes.

For example, your organization might use an alias value `postgres-db` and differentiate between different departments using a suffixes such as `postgres-db-eng` and `postgres-db-qa`. You could combine org and project scopes to create aliases such as `postgres-db-eng-frontend` and `postgres-db-eng-backend`.
