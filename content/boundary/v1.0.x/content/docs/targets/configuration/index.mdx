...
For example, your organization might use a global alias value `postgres-db` and differentiate between different departments using suffixes:
...
Suffixes ensure that aliases are globally unique across scopes, even when you use the same alias value across multiple org or project scopes. You can use them to create a natural DNS-style hierarchy for your access paths that mimics the Boundary deployment structure.

To read more about how you can use suffixes to scale your Boundary deployment, refer to the blog post [Scaling without friction](https://www.hashicorp.com/en/blog/scaling-without-friction-aliases-at-project-scope-in-boundary).
