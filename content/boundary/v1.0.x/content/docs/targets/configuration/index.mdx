...
You may receive connection errors if the alias does not match the website's hostname exactly since the alias is used during TLS negotiation.
...
Aliases that are created in project scopes automatically include project and org suffixes in the following format:

`alias.projectsuffix.orgsuffix`

The suffixes can cause end-to-end TLS to fail if they do not match the certificate.
If you want to maintain end-to-end TLS for a global resource, you should create the alias in the `global` scope and ensure it matches the hostname you are targeting.
You can create aliases in project scopes when you control the certificate and can ensure it matches the alias.
