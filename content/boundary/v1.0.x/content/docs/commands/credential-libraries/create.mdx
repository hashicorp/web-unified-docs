...
description: >-
  The "credential-libraries create" command creates new credential library resources. You can create a Vault, Vault LDAP, or Vault SSH certificate credential library.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-05-11T10:18:43-04:00
last_modified: 2026-09-03T12:57:08.000Z
...
The available types are `vault-generic`, `vault-ldap`, and `vault-ssh-certificate`.
...
<Tab heading="Vault LDAP">

The `credential-libraries create vault-ldap` command lets you create a Vault LDAP credential library.

#### Example

The following example creates a Vault LDAP credential library using a credential store with the ID `csvlt_1234567890`:
...
#### Vault LDAP credential library options

The following is a specific Vault LDAP credential library option in addition to the command options:
...
Use this option when you want to configure additional users or groups as valid principals that the certificate should be signed for in Vault.
   ...
   Starting in Boundary version 1.0.2, each value can use parameter templating with session and account variables (for example, `{{.Account.Subject}}`). Refer to [Vault SSH certificate credential library parameter templating](/boundary/docs/domain-model/credential-libraries#vault-certificates-library-parameter-templating) for supported variables. Static values continue to work unchanged.
