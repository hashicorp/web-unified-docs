...
The `credential-libraries update vault-ldap` command lets you update a Vault LDAP credential library.

#### Example

The following example updates a Vault LDAP credential library with the ID `clvlt_1234567890` to add the name `devops` and the description `For DevOps usage`:

```shell-session
$ boundary credential-libraries update vault-ldap \
   -id clvlt_1234567890 \
   -name devops \
   -description "For DevOps usage"
```
