...
This option supports referencing values from files using `file://` and environment variables using `env://`.
  ...
  You can append the following field to `-attr` to force Boundary to select a host's primary ENI:
  - `-instance_addresses_only=true` - If added, Boundary limits the synching of instance addresses and automatically selects the host's primary ENI (Elastic Network Interface). To unset the value later, change `true` to `false`.
