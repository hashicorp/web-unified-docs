...
last_modified: 2026-04-28T00:04:28.000Z
...
- `-debug` `(string: "")` - Boundary includes a small number of runtime tracing
   user regions, which you can use to see where Boundary spends its time during execution. To create a trace, you need to expose the pprof endpoint, which is disabled by default. To expose the pprof endpoint you must enable the runtime `-debug` flag on a process with an `ops` listener.

   For example:

   `$ boundary dev -debug -ops-listen-address=127.0.0.1:9203`

   Refer to the [Enable debug mode](/boundary/docs/monitor/debug) page to learn more.
...
- `-worker-auth-storage-skip-cleanup`  - If set, prevents deletion of
   ...
   you specify a `worker-auth-storage-dir` value. The default value is `false`.
...
- `-worker-recording-storage-dir` `(string: "")` - The directory in which to
   store worker session recordings when in dev mode. If you do not specify a directory, Boundary creates a temporary directory.