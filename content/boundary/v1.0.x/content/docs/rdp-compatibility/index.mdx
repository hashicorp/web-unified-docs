...
A: Some modern Windows versions block local applications from using the default RDP port (3389).
On Client Agent versions prior to 0.21.2, you must use a workaround, such as connecting to a custom port or using a pre-configured `.rdp` file.
Refer to [RDP targets](/boundary/docs/targets/configuration/configure-transparent-sessions#rdp-targets) for more information.

This limitation no longer exists since Client Agent version 0.21.2.
Refer to [Install Boundary clients](/boundary/docs/deploy/self-managed/install-clients) to upgrade the Client Agent to the latest version.
