...
You can inject generic secrets, LDAP secrets, and SSH certificates directly into Boundary sessions.
...
The LDAP secrets engine lets you configure static or dynamic credentials in Vault and integrate them with your LDAP server. You can create roles that map to specific LDAP groups or users to control access and permissions.
...
Refer to [Manage static credentials with Vault](/boundary/docs/credentials/static-cred-vault) to configure credential brokering or credential injection with static credentials on Vault.