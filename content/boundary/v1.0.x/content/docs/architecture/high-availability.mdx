...
# Boundary high availability installation
...
The general architecture for the server infrastructure requires 3 controllers and 3 workers. Note that it is possible to run a controller and worker within the same process, but the guide here assumes separate deployments. For more on how Boundary controllers and workers are recommended to be deployed, refer to the [recommended architecture](/boundary/docs/architecture/recommended-architecture) page. The documentation here uses virtual machines running on Amazon EC2 as the example environment, but this use case can be extrapolated to almost any cloud platform to suit operator needs:
...
### Why can't my worker reach the controller cluster port?
...
### What happens if a node fails in this architecture?

Because this architecture runs 3 controllers and 3 workers behind a load balancer, the loss of a single node doesn't take down the control or data plane. Refer to [fault tolerance recommendations](/boundary/docs/architecture/fault-tolerance) for details on node, availability zone, and regional failure handling.
