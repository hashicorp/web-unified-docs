---
layout: docs
page_title: Boundary system requirements
description: >-
  Size Boundary controller and worker nodes correctly, configure network connectivity and encryption, and choose compatible databases and load balancers for production deployments.
...
Boundary's system requirements define minimum CPU, memory, disk, and network specifications for controller and worker nodes, along with PostgreSQL database, load balancer, and TLS encryption recommendations. Scale up based on your cluster's actual usage and load test results. This page covers hardware, network, database, and load balancer requirements.
...
file descriptor usage and the memory consumption of each Boundary worker node, and size the worker according to the [hardware sizing](#hardware-sizing-for-boundary-servers) recommendations for your expected usage.

If you want to [export a BSR (Boundary Session Recording) file](/boundary/docs/session-recording/data/exports), we recommend that you have a worker that supports hardware accelerated AV1 encoding,
...
![Diagram showing network traffic flow between client machines, load balancers, Boundary controllers, workers, and the PostgreSQL database](/img/boundary-network_light.png#light-theme-only)
![Diagram showing network traffic flow between client machines, load balancers, Boundary controllers, workers, and the PostgreSQL database](/img/boundary-network_dark.png#dark-theme-only)
...
nodes using standard PKI (Public Key Infrastructure) HTTPS TLS.
...
## Admin UI communication

The Admin UI uses secure cookies and therefore requires HTTPS.
...
We recommend that you use load balancing technology to distribute requests to
your Boundary controller nodes. The API listener and the cluster listener have
different requirements, and round-robin load balancing across the cluster
listener breaks the client-side load balancing that workers perform.

Refer to
[Load balancing Boundary controllers](/boundary/docs/architecture/load-balancing)
for the recommendations for each listener, including managed load balancing
services by cloud provider and how to configure `public_cluster_addr` and
`initial_upstreams`.
