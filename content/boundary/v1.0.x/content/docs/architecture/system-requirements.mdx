...
### Why are my Boundary workers running out of file descriptors?

As users create new sessions, a Boundary worker node opens more sockets and creates additional file descriptors. If a worker becomes memory or file descriptor constrained, monitor both file descriptor usage and memory consumption for that worker node, and size the worker according to the [hardware sizing](#hardware-sizing-for-boundary-servers) recommendations for your expected usage.