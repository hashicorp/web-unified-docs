...
When a Boundary worker node fails, the controller assigns new proxy session requests to another active worker node.
It is a best practice to deploy at least three Boundary workers per network boundary, per type (ingress and egress).
