...
- Run in TCP mode.
- Forward from one controller's `public_cluster_addr` to that same
...
- Change the set of controllers without editing every worker's
  `initial_upstreams`. Workers reach the load balancer and receive the current 
  controller set after connecting.
