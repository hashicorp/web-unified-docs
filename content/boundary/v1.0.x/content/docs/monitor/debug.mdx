...
description: >-
  Enable Boundary's debug flag to expose pprof endpoints and collect runtime traces that help you diagnose latency bottlenecks on controllers and workers.
...
Enabling Boundary's `-debug` flag exposes `pprof` endpoints on the `ops` listener, letting you collect runtime traces to identify latency bottlenecks and correlate application behavior with low-level runtime activity.
...
## Enable debug mode with `boundary dev` and `boundary server`
