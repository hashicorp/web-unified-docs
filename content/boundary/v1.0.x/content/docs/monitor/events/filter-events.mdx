...
description: >-
  Filter audit, observation, system, and telemetry events using `allow_filters` and `deny_filters` predicates to find information written to any Boundary event sink.
...
Boundary lets you filter which events — error, observation, system, or telemetry — are written to a given event sink using `allow_filters` and `deny_filters` predicates in the sink configuration. Refer to [Filtering and listing resources](/boundary/docs/filtering) for more on Boundary's filter syntax.
