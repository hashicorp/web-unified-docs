...
Boundary lets you filter which error, observation, system, or telemetry events are written to a given event sink using `allow_filters` and `deny_filters` predicates in the sink configuration. Refer to [Filtering and listing resources](/boundary/docs/filtering) for more on Boundary's filter syntax.
