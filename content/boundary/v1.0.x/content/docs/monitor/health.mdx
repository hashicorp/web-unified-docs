...
description: >-
  Verify Boundary controller and worker status using the `/health` endpoint, check health with `wget` or `curl`, and view an example response and configuration.
...
Boundary provides health monitoring through the `/health` path using a listener with the `"ops"` purpose. By default, a listener with that purpose runs on port `9203`, returning `200 OK` when the controller's API is healthy.
