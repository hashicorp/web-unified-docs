...
This reduces the likelihood that the instance receives a request during shutdown.
...
Because the worker endpoint always returns `200 OK`, you must inspect the response body like this rather than rely on the HTTP status code.
