...
This FAQ answers Boundary's most common questions about networking requirements, Vault integration, identity provider support, and session monitoring — find the details you need in the sections below.
...
2. Boundary can use Vault as an OpenID Connect (OIDC) provider to enable sign-in with Vault's supported auth methods (even non-OIDC auth methods like Active Directory kerberos/LDAP).
This scenario is walked through in this [tutorial](/vault/tutorials/auth-methods/oidc-identity-provider).
3. Boundary can use Vault as the external key management system (KMS) that serves as Boundary's root of trust. More information on this use case can be found
...
Boundary supports [OpenID Connect (OIDC) authentication](/boundary/tutorials/identity-management/oidc-auth0), which allows support for many popular identity providers (IdPs)
...
At this time, it is not possible to configure Boundary's dynamic host catalog using Identity and Access Management (IAM) roles.
