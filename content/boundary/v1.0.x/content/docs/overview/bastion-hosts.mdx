...
description: >-
    Replace bastion hosts with Boundary's just-in-time, role-based access controls (RBAC) that eliminate privileged jump servers and per-user security group sprawl.
...
Boundary replaces traditional bastion host architectures with just-in-time, role-based access (RBAC) to infrastructure — without exposing a privileged jump server to your network. This page compares Boundary's access model to bastion hosts and where each approach fits.
...
Maintaining security groups, network ACLs, and identity and access management (IAM) controls on a bastion host at a per-user level is nearly impossible, unless you create and maintain multiple bastion hosts per user or group.
...
## How Boundary differs from bastion hosts
...
Refer to also [Boundary vs. VPNs](/boundary/docs/overview/vpn) and [Boundary vs. privileged access management](/boundary/docs/overview/pam).
