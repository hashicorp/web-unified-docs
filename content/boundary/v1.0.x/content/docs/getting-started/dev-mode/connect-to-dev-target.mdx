...
description: >-
  Connect to a target using `boundary connect`, then explore connect helpers, the exec flag, SSH ProxyCommand, and the Desktop Client for different workflows.
...
Running `boundary connect` against your first target establishes a
local authenticated proxy you can use like a normal SSH connection, no
shared credentials or exposed network required. This topic walks through connecting
