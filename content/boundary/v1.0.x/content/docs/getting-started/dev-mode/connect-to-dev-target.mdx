---
layout: docs
page_title: Connect to your first Boundary target
description: >-
  Connect to your first Boundary target using `boundary connect`, then explore connect helpers, the exec flag, SSH ProxyCommand, and the Desktop Client for different connection workflows.
...
# Connect to your first Boundary target

Running `boundary connect` against your first Boundary target establishes a
local authenticated proxy you can use like a normal SSH connection — no
shared credentials or exposed network required. This walks through connecting
to the pre-configured dev-mode target, selecting a specific target explicitly,
and choosing a connect helper for SSH, RDP (Remote Desktop Protocol),
Kubernetes, or database clients.

## Step 1: Connect to the default target
...
## Step 2: Select targets (optional)
...
## Step 3: Choose a connect helper
...
The following video shows selecting a target and connecting to it from the
