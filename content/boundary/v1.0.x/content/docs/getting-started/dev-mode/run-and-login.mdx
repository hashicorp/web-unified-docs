---
layout: docs
page_title: Log in to Boundary in dev mode
description: >-
...
# Log in to Boundary in dev mode

Starting dev mode and logging in gives you a temporary, pre-configured
Boundary environment, accessible via the CLI, Admin Console, Desktop Client,
or Terraform, using default sample credentials so you can start exploring
immediately.

## Start Boundary in dev mode

Use the following command to start Boundary in dev mode:
...
`boundary dev`. The [auth method](/boundary/docs/domain-model/auth-methods) resource in the [global scope](/boundary/docs/domain-model/scopes) is set as the primary auth method for that scope. This allows you to omit the auth method ID when authenticating, enabling the short-form `boundary authenticate`:
...
The following video shows logging in to the Boundary Admin Console at
127.0.0.1:9200 with the default admin credentials.
...
The following video shows logging in to the Boundary Desktop Client at
127.0.0.1:9200 with the default admin credentials.
