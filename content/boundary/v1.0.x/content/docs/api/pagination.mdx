...
![Flowchart of the API pagination loop: make the initial request, and while response_type is delta, reuse the list_token in the next request until the response is complete](/img/api-pagination/pagination-workflow_light.png#light-theme-only)
![Flowchart of the API pagination loop: make the initial request, and while response_type is delta, reuse the list_token in the next request until the response is complete](/img/api-pagination/pagination-workflow_dark.png#dark-theme-only)
