...
The Boundary cache daemon and search functionality require the role so users can add auth tokens to the client cache and search cached resources.
...
The role is associated with the `u_anon` principal in the global scope, should apply to descendant scopes, and includes the following grant:
