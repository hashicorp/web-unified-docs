...
## S-3 compliant attributes and secrets

An S-3 compliant storage bucket contains the bucket name, endpoint URL, and an optional region, as well as any credentials needed to access the bucket, such as the access key ID and secret access key.

### Attributes

S-3 compliant buckets have the following attributes:
...
S-3 compliant buckets can have the following secrets:
