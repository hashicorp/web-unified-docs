...
1. Convert the loaded time with the correct offset using the [WorldTimeAPI](http://worldtimeapi.org/)
...
supported_maintenance_day = rule when tfrun.workspace.auto_apply is true {
  tfrun_created_at.add(time.hour * timezone.offset(timezone_id)).weekday_name in maintenance_days
}
...
httpGet = func(id){
  # Using WorldTimeAPI - no authentication required
  uri = "http://worldtimeapi.org/api/timezone/" + id
...
offset = func(id) {
  tz = httpGet(id)
  # WorldTimeAPI returns offset in seconds, convert to hours
  offset = decimal.new(tz.raw_offset / 3600).int
  return offset
}
```