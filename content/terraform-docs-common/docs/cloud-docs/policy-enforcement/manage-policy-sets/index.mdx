...
last_modified: 2026-08-20T14:26:16.000Z
# END AUTO GENERATED METADATA
---

# Manage policies and policy sets in HCP Terraform

Policies are rules that HCP Terraform enforces on Terraform runs. You can define policies using the [Terraform policy](/terraform/cloud-docs/policy-enforcement/define-policies/terraform-policy), [Sentinel](/terraform/cloud-docs/policy-enforcement/define-policies/custom-sentinel), or [Open Policy Agent (OPA)](/terraform/cloud-docs/policy-enforcement/opa) policy-as-code frameworks.

~> **Note:** Only Terraform policy supports both workspaces and Stacks. Sentinel and OPA policies are workspace-only. Refer to [Define policies overview](/terraform/cloud-docs/policy-enforcement/define-policies) for details.
...
Policy sets are collections of policies you can apply globally, to specific [projects](/terraform/cloud-docs/projects/manage), workspaces, and Stacks, or to workspaces with specific tags in your organization. For each run with a policy set applied, HCP Terraform checks the Terraform plan against the policy set. Depending on the [enforcement level](#policy-enforcement-levels), failed policies can stop a run. If you do not want to enforce a policy set on a specific project, workspace, Stack, or tag, you can exclude it from that set.
...
HCP Terraform runs a policy evaluation in your self-managed agent pool if you meet the following requirements:

- You are on the HCP Terraform **Premium** edition.
- You configure the workspace or Stack to run Terraform operations in your self-managed agent pool. Refer to [Manage agent pools](/terraform/cloud-docs/agents/agent-pools) for more information.
...
- Mandatory overridable: Stops the operation if the conditions of the policy aren't met, but allows a user with appropriate permissions to override the policy and continue. Failed policies stop the run, but organization admins can configure the platform to allow team members to override failures so that runs can continue. Organization admins can either let all team members override **Mandatory overridable** policy failures or delegate authority to specified teams. Refer to [Policy overrides](/terraform/cloud-docs/users-teams-organizations/permissions/organization#policy-overrides) for more information.
...
- Advisory: Failed policies never interrupt the run. They provide information about policy check failures in the UI.
- Soft mandatory: Failed policies stop the run, but organization admins can configure the platform to allow team members to override failures so that runs can continue. Organization admins can either let all team members override **Soft mandatory** policy failures or delegate authority to specified teams. Refer to [Policy overrides](/terraform/cloud-docs/users-teams-organizations/permissions/organization#policy-overrides) for more information.
