...
- **Policies enforced globally**: Automatically enforces the policy set on all of the organization's existing and future workspaces and Stacks. It is optional, but you can also specify workspaces or Stacks to exclude from the policy set when this scope is enabled. Enable one of the following settings to configure exclusions:
   - **Names of projects, workspaces, Stacks, and deployments**: Choose one or more projects, workspaces, Stacks, or deployments to exclude. Selecting a project exempts all workspaces and Stacks in the project from the policies. You can also choose specific workspaces, Stacks, or deployments to exempt from the policy set. Note that the **Projects**, **Workspaces**, **Stacks**, and **Deployments** selectors function independently so that you can exclude entire projects as well as specific workspaces, Stacks, or deployments in other projects.
...
For more information about policy enforcement for Stacks, including Phase 1 capabilities and limitations, refer to [Policy enforcement for Stacks](/terraform/cloud-docs/stacks/policy-enforcement).
