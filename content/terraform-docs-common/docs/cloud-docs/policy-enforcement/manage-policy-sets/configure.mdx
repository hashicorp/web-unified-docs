---
page_title: Configure policy sets
description: >-
...
last_modified: 2026-08-20T14:26:14.000Z
# END AUTO GENERATED METADATA
---

# Configure policy sets

Policy sets are collections of policies that you can apply to an entire organization, specific projects, workspaces and Stacks, or workspaces with matching tags. For more information about defining policies, refer to [Define policies](/terraform/cloud-docs/policy-enforcement/define-policies). For information about using policies created and maintained by HashiCorp, refer to [Run pre-written Sentinel policies](/terraform/cloud-docs/policy-enforcement/prewritten-sentinel).
...
- HashiCorp managed pre-written policies [BETA]: Create a policy set using the HashiCorp Managed Policies provided by HCP Terraform. 
- Managed policies: Create a policy set and add policies to it in the HCP Terraform or Terraform Enterprise interface.
- Policy sets hosted in a version control system: Create a policy set in the HCP Terraform or Terraform Enterprise interface and then connect it to a repository. HCP Terraform automatically refreshes the policy set when you change relevant files. Version control policy sets have specific organization and formatting requirements. Refer to [Sentinel VCS Repositories](/terraform/cloud-docs/policy-enforcement/manage-policy-sets/sentinel-vcs) and [OPA VCS Repositories](/terraform/cloud-docs/policy-enforcement/manage-policy-sets/opa-vcs) for details.
...
1. Choose one of the following sources:
   - **Pre-written policies [BETA]**: Choose this option to run pre-written policies provided by HCP Terraform. 
...
1. Click **Next**. 
   - **Pre-written policies [BETA]**: choose either **Terraform policy** or **Sentinel** as the policy framework.  
   - **All other options**: choose **Terraform policy**, **Sentinel**, or **Open policy agent (OPA)** as the policy framework.
...
1. For **Pre-written policies [BETA]**: from the policy table, select all policies you want to add to the policy set. 
   ...
   - Click on the **Next** button when all policies required have been selected.
1. For **Pre-written policies [BETA]**:
   - Use the **...** button for each policy or the **Actions** dropdown to define the enforcement level for the selected policies. Refer to [Policy enforcement levels](/terraform/cloud-docs/workspaces/policy-enforcement/manage-policy-sets#policy-enforcement-levels) for more information. 
...
1. For Sentinel policy sets, choose a policy execution mode. Refer to [Policy set execution modes](#policy-set-execution-modes) for details about each option. Pre-written policy sets only support agent mode. 
1. For OPA policy sets, configure the following settings:
   - Choose an OPA version from the **Runtime version** drop-down menu.
   - Enable the **This policy set can be overridden in the event of mandatory failures** option to let users with override policy permissions apply plans, even when a mandatory policy fails. Refer to [Policy enforcement levels](/terraform/cloud-docs/policy-enforcement/manage-policy-sets#policy-enforcement-levels) for more information.
1. Click **Next**, then configure the following settings depending on the source you chose in step 3:
   ...
   - **Automated**: Specify a list of key-value pairs that HCP Terraform or Terraform Enterprise sends to the Sentinel runtime when performing policy checks. Refer to [Sentinel parameters](#sentinel-parameters) for more information.
...
HCP Terraform and Terraform Enterprise provide several options for fine tuning the scope of your policy sets. Terraform policy sets can be applied to either workspaces or Stacks, while OPA and Sentinel policies can only be applied to workspaces. You can enable one of the following scope settings when configuring your policy set connection:

- **Policies enforced globally**: Automatically enforces the policy set on all of the organization's existing and future workspaces and Stacks. It is optional, but you can also specify workspaces or Stacks to exclude from the policy set when this scope is enabled. Enable one of the following settings to configure exclusions:
   - **Names of projects, workspaces, and Stacks**: Choose one or more projects, workspaces, or Stacks to exclude. Selecting a project exempts all workspaces and Stacks in the project from the policies. You can also choose specific workspaces, or Stacks to exempt from the policy set. Note that the **Projects**, **Workspaces**, and **Stacks** selectors function independently so that you can exclude entire projects as well as specific workspaces or Stacks in other projects.
   - **Tags attached to workspaces**: Choose tags within the organization. Workspaces with matching tags are exempt from the policies. Refer to [Create workspace tags](/terraform/cloud-docs/workspaces/tags) for more information about tags.
- **Policies enforced on selected projects, workspaces, and Stacks**: Enforces the policy set on specific workspaces, Stacks, or all workspaces and Stacks in a project. This affects all current and future workspaces and Stacks for any projects you choose. Note that the **Projects**, **Workspaces**, and **Stacks** selectors function independently so that you can enforce the policies on entire projects as well as specific workspaces, or Stacks in other projects.
- **Policies applied to tags**: Enforces the policy set on all workspaces that match the selected tags. You must select at least one tag and choose a tag match logic:
   - **And**: All selected tags must be present on a workspace for the policy set to apply.
   - **Or**: At least one of the selected tags must be present on a workspace for the policy set to apply.

Only Terraform policy sets can be applied to Stacks. For more information about policy enforcement for Stacks, refer to [Policy enforcement for Stacks](/terraform/cloud-docs/stacks/policy-enforcement).
...
When the HCP Terraform execution mode is set to **Agent**, you must connect to HCP Terraform agents v1.28.10 or newer to evaluate policies written for Terraform policy.

### Edit policy sets

1. Sign in to [HCP Terraform](https://app.terraform.io/login) or Terraform Enterprise and navigate to the organization you want to edit a policy set in.
...
1. Sign in to [HCP Terraform](https://app.terraform.io/login) or Terraform Enterprise and navigate to the organization you want to delete a policy set in.
1. Choose **Settings** from the sidebar, then **Policies** in your organization’s settings.
...
[Terraform policy inputs](/terraform/policy/write/inputs-and-locals) are a list of key-value pairs that HCP Terraform and Terraform Enterprise send to the Terraform policy runtime as parameters when performing policy checks on workspaces. If the value parses as JSON, HCP Terraform and Terraform Enterprise send it to Terraform policy as the corresponding type, such as string, Boolean, integer, map, or list. If the value fails JSON validation, the value is sent as a string.

You can set Terraform policy parameters when you [edit a policy set](#edit-policy-sets). Terraform policy also loads values for inputs from workspace environment variables with a matching name. For example, a workspace with an environment variable of `instance_type` sets the value of the `instance_type` input.

When Terraform policy evaluates your Terraform runs, it sets values for inputs in your policies through the following methods in order of precedence:

1. From the policy set parameters.
