...
HCP Terraform and Terraform Enterprise provide several options for fine tuning the scope of your policy sets. Terraform policy sets can be applies to either workspaces or Stacks, while OPA and Sentinel policies can only be applied to workspaces. You can enable one of the following scope settings when configuring your policy set connection:
...
Only Terraform policy sets can be applies to Stacks. For more information about policy enforcement for Stacks, refer to [Policy enforcement for Stacks](/terraform/cloud-docs/stacks/policy-enforcement).
