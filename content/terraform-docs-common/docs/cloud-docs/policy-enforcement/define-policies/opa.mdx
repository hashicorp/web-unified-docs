...
last_modified: 2025-09-05T20:13:15-07:00
...
- [Copy pre-written Sentinel policies](/terraform/cloud-docs/policy-enforcement/prewritten-sentinel)

~> **Note:** OPA policies are only supported for workspaces. For Stacks, use Terraform policy. Refer to [Define policies overview](/terraform/cloud-docs/policy-enforcement/define-policies) for framework support details.
...
The following test validates `block_auto_apply_runs`. The test is written in rego and uses the OPA [test format](https://www.openpolicyagent.org/docs/policy-testing/#test-format) to check that the workspace [apply method](/terraform/cloud-docs/workspaces/settings#apply-method) is not configured to auto apply. You can run this test with the `opa test` CLI command. Refer to [Policy Testing](https://www.openpolicyagent.org/docs/policy-testing) in the OPA documentation for more details.
...
|  `speculative` | Boolean | Whether the plan associated with the run is a [speculative plan](/terraform/cloud-docs/workspaces/run/remote-operations#speculative-plans) only |
...
|   `variables` |  A string-keyed map of values. | Provides the variables configured within the run. Each variable `name` maps to two properties: `category` and `sensitive`.  The `category` property is a string indicating the variable type, either "input" or "environment". The `sensitive` property is a boolean, indicating whether the variable is a [sensitive value](/terraform/cloud-docs/variables/managing-variables#sensitive-values).  |
