...
~> **Note:** Sentinel policies are only supported for workspaces. For Stacks, use Terraform policy. Refer to [Define policies overview](/terraform/cloud-docs/policy-enforcement/define-policies) for framework support details.
