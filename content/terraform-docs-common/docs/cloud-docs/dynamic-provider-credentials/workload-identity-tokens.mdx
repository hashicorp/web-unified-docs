...
~> **Note**: The `terraform_full_workspace` claim is only included in workspace-scoped tokens. Organization-level operations, such as [module test runs](/terraform/cloud-docs/registry/test-dynamic-credentials), do not include this claim.

## Module test run tokens

When using [dynamic credentials for module testing](/terraform/cloud-docs/registry/test-dynamic-credentials), HCP Terraform generates workload identity tokens specifically for test runs. These tokens have a different structure and custom claims compared to workspace run tokens.
...
For more information, refer to [Dynamic credentials for module testing](/terraform/cloud-docs/registry/test-dynamic-credentials).
