...
| `TFC_VAULT_CLIENT_TIMEOUT`<br />`TFC_VAULT_CLIENT_TIMEOUT[_TAG]`<br />`TFC_DEFAULT_VAULT_CLIENT_TIMEOUT`                                     | An integer value representing the number of seconds the Vault Client should wait before timing out when requesting cloud provider credentials. If this value is not set, the default of 60 seconds is used. | Requires **v1.9.0** or later if self-managing agents. |
