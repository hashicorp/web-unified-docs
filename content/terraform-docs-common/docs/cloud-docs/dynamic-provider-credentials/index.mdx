...
Dynamic credentials for queries do not support run phase specific environment variables (plan/apply). Must use the requried Run-specific environment varaibles for the corresponding cloud platform.

AWS: `TFC_AWS_RUN_ROLE_ARN`
Azure: `TFC_AZURE_RUN_CLIENT_ID`
GCP: `TFC_GCP_RUN_SERVICE_ACCOUNT_EMAIL`
HCP: `TFC_HCP_RUN_PROVIDER_RESOURCE_NAME`
Kubernetes: `TFC_KUBERNETES_WORKLOAD_IDENTITY_AUDIENCE`
Vault: `TFC_VAULT_RUN_ROLE`
