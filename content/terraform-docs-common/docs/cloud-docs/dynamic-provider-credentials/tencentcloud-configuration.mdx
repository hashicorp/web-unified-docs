...
If the same permissions will be used regardless of run phase, then the condition can be modified like the below to use `string_like` for the sub and include a `*` after `run_phase:` to perform a wildcard match:
