...
1. You configure your cloud provider (AWS, GCP, Azure, or Vault) to trust tokens from HCP Terraform's OIDC issuer
2. You configure your registry module's test configuration to use dynamic credentials
3. When a test run starts, HCP Terraform generates a unique OIDC token specific to that test
4. HCP Terraform provides the token to your test run through a secure mechanism
5. Your cloud provider verifies the token and grants temporary credentials based on your trust configuration
6. The token automatically expires after the configured time-to-live (TTL)
...
- The HCP Terraform UI (module **Tests** → **Configuration** page)
