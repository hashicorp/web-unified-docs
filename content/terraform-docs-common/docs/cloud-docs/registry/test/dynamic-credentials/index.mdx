...
last_modified: 2026-02-02T16:32:55-08:00
...
- `terraform_run_phase`: Always `"plan"` for module test runs because test runs are read-only operations
...
1. Click **Tests**, then **Configuration**.

For instructions on how to use the API, refer to [Test Configuration API](/terraform/cloud-docs/api-docs/private-registry/modules/test-configuration).

For instructions on using environment variables in the test configuration, refer to [Configure environment variables](/terraform/cloud-docs/registry/test#configure-environment-variables).
