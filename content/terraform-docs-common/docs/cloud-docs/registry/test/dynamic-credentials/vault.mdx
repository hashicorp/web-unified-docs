...
description: >-
  Configure HashiCorp Vault JWT auth to trust HCP Terraform's OIDC tokens for module test runs,
  enabling dynamic secret generation without static credentials.
---
...
- A HashiCorp Vault instance (Vault 1.8.0 or later) with permissions to configure auth methods and policies
...
- The `vault` CLI (optional, for command-line configuration)
...
For Terraform Enterprise, replace `app.terraform.io` with your TFE hostname:
...
1. The role exists and is configured correctly
1. The `bound_audiences` includes `vault.workload.identity`
1. The `bound_claims` match your test run's token claims
1. The subject claim matches the `bound_subject` if configured

### Error: "Invalid JWT"

The token validation failed. Verify:
...
HCP Terraform couldn't reach Vault. Check:

1. The Vault URL in the test configuration is correct
2. The URL includes the protocol (`https://`) and port
3. Your network allows HCP Terraform to reach Vault
4. Vault's TLS certificate is valid (or Vault is configured to allow insecure connections)

### Error: "Role not found"

The specified role doesn't exist. Verify:

1. The role name in HCP Terraform matches the role in Vault exactly (case-sensitive)
2. The role exists in the correct namespace (if using Vault Enterprise)
3. The auth path is correct (default is `jwt`)
