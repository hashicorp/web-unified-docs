...
last_modified: 2026-01-30T09:59:34-08:00
...
You can also configure dynamic credentials using the [Test Configuration API](/terraform/cloud-docs/api-docs/private-registry/modules/test-configuration):
...
1. The URL includes the `https://` protocol  and port number
1. Your network allows HCP Terraform to reach Vault
1. Either Vault's TLS certificate is valid or Vault is configured to allow insecure connections
...
1. If using Vault Enterprise, the role exists in the correct namespace.
1. The auth path is correct. The default path is `jwt`.
