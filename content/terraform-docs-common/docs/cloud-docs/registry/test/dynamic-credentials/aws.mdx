...
If you haven't already configured an OIDC identity provider for HCP Terraform in your AWS account, create one:
...
4. Enter the provider URL:
   ...
   - For Terraform Enterprise: `https://<TFE_HOSTNAME>` (replace with your TFE hostname)
5. Click **Get thumbprint** to automatically retrieve the thumbprint
6. For **Audience**, enter: `aws.workload.identity`
7. Click **Add provider**

~> **Note**: You only need to create one OIDC provider per AWS account. You can reuse this provider for all HCP Terraform workspaces and module tests.

### Create an IAM role for module tests

Create a dedicated IAM role that module tests will assume:

1. In the AWS Management Console, navigate to **IAM > Roles**
2. Click **Create role**
3. Select **Web identity** as the trusted entity type
4. Choose the OIDC provider you created above
5. For **Audience**, select `aws.workload.identity`
6. Click **Next**
7. Attach permissions policies required for your module tests (see [permissions recommendations](#permissions-recommendations))
8. Name the role (e.g., `terraform-module-test-role`)
9. Click **Create role**
...
- `my-org`: Your organization's name (found in organization settings)
...
6. Enter the IAM role ARN (e.g., `arn:aws:iam::123456789012:role/terraform-module-test-role`)
