...
Instead of using static credentials stored as environment variables to test modules, we recommend configuring HCP Terraform to use OpenID Connect (OIDC) to generate dynamic, short-lived credentials. 

Dynamic credentials authenticate to your cloud provider during test runs without exposing long-lived credentials. This improves security and eliminates the need to manage and rotate cloud provider credentials.
