...
HCP Terraform will list any sets of variables configured on the workspace for
...
credentails** button to configure dynamic credentials for your workspace. Refer
to the [AWS quick setup](/terraform/cloud-docs/dynamic-provider-credentials/aws-configuration/quick-setup) documentation for more information.
...
You can then reference a variable set in your deployment configuration, `<NAME>.tfdeploy.hcl`, using the `store` block. In the following example, the `store` block can access [Terraform variable values](/terraform/cloud-docs/variables#terraform-variables) in a variable set named `"Database passwords"`:
