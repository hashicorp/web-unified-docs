...
Both Workspaces and Stacks can now connect to a private VCS provider. 

We recommend creating a separate agent pool to test your Stacks private VCS operations. Stacks Private VCS requires HCP Terraform agent v1.25.2. Refer to [software requirements](/terraform/cloud-docs/agents/requirements#software) for more information.
</Note>
...
- HCP Terraform agent v1.15.0 or newer for Workspaces and v1.25.2 for Stacks.
...
| `TFC_AGENT_ACCEPT` | A comma-separated list of values that must include `ingress` for Workspaces and `source_bundle` for Stacks. Refer to [Install and Run Agents](/terraform/cloud-docs/agents/agents#accept) for more information. | `-accept` |

Below is an example command to start the HCP Terraform agent for Workspaces:
...
Below is an example command to start the HCP Terraform agent for Stacks:
