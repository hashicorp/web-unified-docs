...
| `TFC_AGENT_ACCEPT` | A comma-separated list of values that must include `ingress` for workspaces or `source_bundle` for Stacks. Refer to [Install and Run Agents](/terraform/cloud-docs/agents/agents#accept) to learn more. | `-accept` |

Below is an example command to start the HCP Terraform agent for a workspace:
...
Below is an example command to start the HCP Terraform agent for a Stack:
