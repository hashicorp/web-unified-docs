...
HCP Terraform returns a "doesn't exist or isn't accessible" error from the workspace, Stack, and module creation endpoints when it cannot confirm that the calling identity has access to the requested repository. The message has several variants, each pointing at a different layer of the check. Identify the exact wording you received, then apply the matching remediation:

- **`The authenticated user has not linked a GitHub account to HCP Terraform via the GitHub App.`** The user behind the API token has never authorized the Terraform Cloud GitHub App on their personal GitHub account, so HCP Terraform has no per-user token with which to check repository access. To resolve, have that user sign in to HCP Terraform and complete the steps in [Authorizing](#authorizing).

- **`Team and organization API tokens cannot currently be used to create or update workspaces connected to GitHub App-backed repositories.`** Team and organization tokens authenticate as a synthetic service-account user that cannot complete a personal GitHub OAuth flow. To resolve, authenticate with a user API token whose owner has authorized the Terraform Cloud GitHub App. See [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the full constraint.

- **`The specified repository "<owner>/<repo>" doesn't exist or isn't accessible to the authenticated user's linked GitHub account.`** The user is linked but their personal GitHub account does not have at least read access to the repository. To resolve, grant the user GitHub access to the repository.

- **`The specified repository "<owner>/<repo>" doesn't exist or isn't accessible.`** (no trailing qualifier) The GitHub App installation itself cannot see the repository — either it is not included in the installation's allowed repository list, or the repository identifier is wrong, or the GitHub App's installation token has been revoked. To resolve, adjust the GitHub App installation's repository selection on GitHub (see [Configuring and Uninstalling](#configuring-and-uninstalling)), or confirm the `:owner/:repo` identifier matches the repository on GitHub.

The variants exist because GitHub App-backed access is enforced at two layers — the installation's repository scope, and (for the API and TFE provider paths) the calling user's personal GitHub access — and either layer can fail independently.
