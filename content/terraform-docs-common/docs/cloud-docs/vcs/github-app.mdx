...
HCP Terraform returns this error from the workspace, Stack, and module creation endpoints when it can resolve the GitHub App installation but cannot confirm that the calling user has access to the requested repository on GitHub. The message has two distinct variants, each with a different remediation:

- **`The authenticated user has not linked a GitHub account to HCP Terraform via the GitHub App.`** The user behind the API token has never authorized the Terraform Cloud GitHub App on their personal GitHub account, so HCP Terraform has no per-user token with which to check repository access. To resolve, either have that user sign in to HCP Terraform and complete the steps in [Authorizing](#authorizing), or authenticate with a [team token](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) that has permission to manage VCS settings.

- **`The specified repository "<owner>/<repo>" doesn't exist or isn't accessible to the authenticated user's linked GitHub account.`** The user is linked but their GitHub account does not have at least read access to the repository, or the repository is not included in the GitHub App installation's allowed repository list. To resolve, grant the user GitHub access to the repository, or adjust the GitHub App installation's repository selection on GitHub.

The two variants exist because per-repository access on the GitHub App is enforced at the user level on top of the installation-level scope; both checks must pass.
