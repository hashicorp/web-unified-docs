...
The API and the `tfe` provider do not trigger the browser-based GitHub authorization pop-up described in [Authorizing](#authorizing). Any user whose API token connects workspaces, Stacks, or modules to a GitHub App-backed repository must complete that browser-based authorization at least once before their token works for that operation. Refer to [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for how each token type interacts with this requirement.

</Note>

## Troubleshooting

Use the following guidance to resolve common errors when you connect HCP Terraform to a GitHub App-backed repository.

### `Repository doesn't exist or isn't accessible`

HCP Terraform returns a `422` "doesn't exist or isn't accessible" error from the workspace, Stack, and registry module create and update endpoints when it cannot confirm that the calling identity has access to the requested repository. The exact wording depends on the endpoint. The workspace endpoints return short messages that do not distinguish the underlying cause. The Stack and registry module endpoints return more specific messages, and Stack errors begin with the prefix `VCS Connection Validation failed:`. Match the message you received to one of the following causes, then apply the remediation:

- **The calling user has not linked a GitHub account.** The workspace endpoints return `Repository requires the authenticated user to link a GitHub account via the GitHub App`. The Stack and registry module endpoints return `The authenticated user has not linked a GitHub account to HCP Terraform via the GitHub App. Link your GitHub account in user settings.` The user who owns the API token has never authorized the Terraform Cloud GitHub App on their personal GitHub account, so HCP Terraform has no per-user token with which to check repository access. To resolve the error, have that user sign in to HCP Terraform and complete the steps in [Authorizing](#authorizing).

- **The API token is a team or organization token.** The workspace endpoints return the same `Repository requires the authenticated user to link a GitHub account via the GitHub App` message for this cause. The Stack and registry module endpoints return `Team and organization API tokens cannot currently be used to create or update workspaces connected to GitHub App-backed repositories.` Team and organization tokens authenticate as a synthetic service-account user that cannot complete a personal GitHub OAuth flow. To resolve the error, authenticate with a user API token whose owner has authorized the Terraform Cloud GitHub App. Refer to [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the full constraint.

- **The user's linked GitHub account cannot access the repository.** The workspace endpoints return `Repository doesn't exist or isn't accessible`. The Stack and registry module endpoints return `The specified repository "<owner>/<repo>" doesn't exist or isn't accessible to the authenticated user's linked GitHub account.` The user is linked but their personal GitHub account does not have at least read access to the repository. To resolve the error, grant the user GitHub access to the repository.

- **The GitHub App installation cannot see the repository.** The workspace endpoints return the same `Repository doesn't exist or isn't accessible` message for this cause. The Stack and registry module endpoints return `The specified repository "<owner>/<repo>" doesn't exist or isn't accessible.` without a qualifier about the linked GitHub account. This error occurs when the installation's allowed repository list does not include the repository, the repository identifier is wrong, or GitHub has revoked the App's installation token. To resolve the error, adjust the GitHub App installation's repository selection on GitHub or confirm that the `:owner/:repo` identifier matches the repository on GitHub. Refer to [Configuring and Uninstalling](#configuring-and-uninstalling) for instructions on changing the installation's repository selection.

These causes exist because HCP Terraform enforces GitHub App-backed access at two layers: the installation's repository scope, and the calling user's personal GitHub access for the API and `tfe` provider paths. Either layer can fail independently. The workspace endpoints collapse each pair of causes into one message, so work through both causes that match your message when you troubleshoot a workspace error.
