...
1. Click the **New Token** button to generate a new personal access token with **Code (Read)** and **Code (Status)** scopes. Under **Organization**, select the specific Azure DevOps organization you want to connect to HCP Terraform, and set an expiration that meets your requirements.

   <Note>

   Microsoft is [retiring globally-scoped personal access tokens](https://devblogs.microsoft.com/devops/retirement-of-global-personal-access-tokens-in-azure-devops/) that use the **All accessible organizations** scope. Scope the token to a specific organization and provide that organization name in [Step 3](#step-3-add-the-personal-access-token-on-hcp-terraform).
...
1. In the **Azure DevOps Organization Name** field, enter the name of the organization your personal access token is scoped to. This is the organization segment of your Azure DevOps URL, for example `my-company` in `https://dev.azure.com/my-company`. Provide this value when your token is scoped to a specific organization. Leave the field blank only if you are connecting with a globally-scoped token.
