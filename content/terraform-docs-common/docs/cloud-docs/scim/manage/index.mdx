...
- Configure or update the side auditor group
...
1. Log in to your HCP Terraform.
1. Click **Organization settings** and then **SCIM provisioning**.
1. Click the **Manage** drop-down menu and choose **Disable SCIM**.
...
<!-- BEGIN: TFEnterprise:only name:site-auditor -->
## Configure site auditor group

Use a SCIM group to grant the [site auditor role](/terraform/enterprise/api-docs/admin/users#grant-a-user-site-auditor-privileges) automatically. The site auditor role provides read-only, cross-organization visibility into all resources in the instance. 

1. Open **Site Admin** > **SCIM**.
1. In **Site Auditor IdP Group**, select the SCIM group to use for site auditors.
1. Click **Update site auditor provisioning**.

### Site auditor group behavior

- Setting or changing the selected SCIM group updates site auditor access immediately for the current members of the mapped group.
- Members added to the selected SCIM group through later SCIM group updates also gain the site auditor role.
- Suspended users in the selected SCIM group do not receive the site auditor role until they are reactivated. When a suspended user is reactivated, Terraform Enterprise re-grants the role if the user is still a member of the configured site auditor group.
- When you change the site auditor group, Terraform Enterprise revokes the site auditor role from members of the previous mapped group, then grants the role to the current members of the new mapped group.
- Clearing the site auditor group removes the mapping. Existing site auditor grants are not automatically revoked.
- Service accounts cannot be granted the site auditor role.
- A SCIM group used for the site auditor mapping cannot also be the site admin group or be linked to a Terraform Enterprise team. Terraform Enterprise rejects a mapping that conflicts with either.
- When SCIM is enabled, use this setting to manage site auditor access. Terraform Enterprise does not use SAML site auditor attributes for site auditor provisioning.
<!-- BEGIN: TFEnterprise:only name:site-auditor -->
...
For detailed token lifecycle guidance, refer to [Tokens](/terraform/cloud-docs/scim/manage/tokens).
...
- [Manage SCIM tokens](/terraform/cloud-docs/scim/manage/tokens)
