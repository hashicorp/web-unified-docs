...
last_modified: 2026-09-08T16:31:21.000Z
...
Disabling SCIM stops all provisioning requests for your organization and removes the SCIM configuration. When you disable SCIM, HCP Terraform performs the following actions:

- Stops synchronizing users and groups from your IdP
- Removes all SCIM-managed team memberships
- Revokes all SCIM tokens

HCP Terraform retains existing users, existing teams and their permissions, and organization memberships. Users can also continue logging in through SSO after SCIM is disabled.
...
Re-enabling SCIM does not automatically replay changes that your identity provider made while SCIM was disabled. You must restart provisioning in Entra ID or remove and re-add group assignments in Okta to synchronize any missed changes.
