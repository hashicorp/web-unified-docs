...
For provider-specific setup, assignment, scope, and attribute-mapping checks, refer to the [configure guides](/terraform/cloud-docs/scim/configure).
...
Refer to [Owners team and emergency access](/terraform/cloud-docs/scim/users#owners-team-and-emergency-access) for more information.
...
| `409` | Duplicate `userName` or `displayName` | [Users not syncing](#users-not-syncing), [Duplicate users](#duplicate-users), [Groups not syncing](#groups-not-syncing) |
...
<!-- BEGIN: TFEnterprise:only name:403-admin-steps -->
...
<!-- END: TFEnterprise:only name:403-admin-steps -->
...
1. Verify that the primary email address in your IdP matches the email address of the existing HCP Terraform account exactly, including letter case.
1. Remove the duplicate account and re-sync.

If the organization has two accounts for the same person where one is SCIM-managed and one is manually managed, the SAML `nameID` and the SCIM `userName` are using different IdP attributes. When the stored `scim_username` does not match the `nameID` sent at login, HCP Terraform cannot find the SCIM-managed user and triggers the SSO account-linking flow, which creates a second manually managed account. Refer to [User lifecycle](/terraform/cloud-docs/scim/users) for more information. 

Complete the following steps to resolve this issue:

1. Confirm that the SAML `nameID` and the SCIM `userName` mapping use the same IdP attribute. For Entra ID, both the **Unique User Identifier** in the SAML configuration and the `userName` attribute mapping must be `userPrincipalName`.
...
Terraform Enterprise handles between SCIM deactivation and deletion differently:
...
- When your IdP sends a `DELETE` request, HCP Terraform either removes the user from the organization or deletes the user from the system depending on the type of account. Users that own and manually manage their accounts are removed from the organization, whereas users provisioned and managed in the IdP are deleted.
...
The <!-- BEGIN: TFEnterprise:only name:scim-api-handle -->public SCIM endpoint returns<!-- END: TFEnterprise:only name:scim-api-handle --><!-- BEGIN: TFC:only name:scim-api-handle -->SCIM provisioning endpoints return<!-- END: TFC:only name:scim-api-handle --> an HTTP `413` error when one of the following events occur:
...
<!-- BEGIN: TFEnterprise:only name:scim-api-handle -->
...
<!-- END: TFEnterprise:only name:scim-api-handle -->

<!-- BEGIN: TFC:only name:scim-api-handle -->
For details about sending requests to the SCIM provisioning endpoints, such as request-size and per-endpoint request limits, refer to [SCIM provisioning API endpoints](/terraform/enterprise/api-docs/scim) and [SCIM Groups API](/terraform/enterprise/api-docs/scim/groups).
<!-- END: TFC:only name:scim-api-handle -->
...
All SCIM provisioning endpoints have a rate limit of 30 requests per second. Refer to the [SCIM provisioning API endpoints](/terraform/cloud-docs/api-docs/scim) for rate limit values, response codes, and details.
...
<!-- BEGIN: TFEnterprise:only name:500-retry -->
For public SCIM `/scim/v2/*`, retry once after confirming the IdP request is valid. If the error repeats, inspect Terraform Enterprise logs and diagnostics.
<!-- END: TFEnterprise:only name:500-retry -->

<!-- BEGIN: TFC:only name:500-retry -->
...
If you cannot resolve your SCIM issue using the troubleshooting steps in this topic, contact HashiCorp support. <!-- BEGIN: TFEnterprise:only name:get-support -->Refer to [Get support for Terraform Enterprise](/terraform/enterprise/deploy/troubleshoot/contact-support) for instructions on opening a case, creating diagnostic support bundles, and other information.<!-- END: TFEnterprise:only name:get-support --><!-- BEGIN: TFC:only name:get-support -->Visit the [IBM support page](https://www.ibm.com/mysupport/s/?language=en_US) for support articles and to open a case. <!-- END: TFC:only name:get-support -->
