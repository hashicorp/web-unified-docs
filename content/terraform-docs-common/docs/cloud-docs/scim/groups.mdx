...
last_modified: 2026-08-20T23:03:05.000Z
...
1. Adds users to the organization if they are not already members.
...
- Group display name changes. <!-- BEGIN: TFC:only name:product -->HCP Terraform<!-- END: TFC:only name:product --><!-- BEGIN: TFEnterprise:only name:product -->Terraform Enterprise<!-- END: TFEnterprise:only name:product --> stores the updates uisng the letter case provided by the IdP.
...
For example, Okta sends `PUT` requests with a complete list of all group members, whereas Microsoft Entra ID sends `PATCH` requests with lists of members to add and remove.
