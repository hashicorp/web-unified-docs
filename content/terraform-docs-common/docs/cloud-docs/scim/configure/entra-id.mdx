...
last_modified: 2026-08-26T23:38:38.000Z
...
Before you configure Microsoft Entra ID as the SCIM identity provider, you must enable SCIM in Terraform Enterprise and generate a SCIM token. Refer to [Configure SCIM provisioning](/terraform/cloud-docs/scim/configure) for instructions.
...
Configure which users and groups Microsoft Entra ID synchronizes to <!-- BEGIN: TFEnterprise:only name:product -->
Terraform Enterprise<!-- END: TFEnterprise:only name:product --><!-- BEGIN: TFC:only name:product -->HCP Terraform<!-- END: TFC:only name:product -->:
