...
Before you configure Microsoft Entra ID as the SCIM identity provider, you must enable SCIM in <!-- BEGIN: TFEnterprise:only name:product -->Terraform Enterprise<!-- END: TFEnterprise:only name:product --><!-- BEGIN: TFC:only name:product -->HCP Terraform<!-- END: TFC:only name:product --> and generate a SCIM token. Refer to [Configure SCIM provisioning](/terraform/cloud-docs/scim/configure) for instructions.
...
- Site auditor group: If you configure a site auditor group in Terraform Enterprise, do not also use that group as the site admin group or link it to a Terraform Enterprise team. Terraform Enterprise uses the site auditor group exclusively for [Site Auditor](/terraform/enterprise/scim/manage#configure-site-auditor-group) provisioning.
...
1. Assign a group in your identity provider.
1. Wait for synchronization and confirm that the user appears in HCP Terraform with a status of **Synced**.
...
<!-- BEGIN: TFEnterprise:only name:troubleshooting -->
For broader Terraform Enterprise-side diagnosis, refer to [Troubleshoot SCIM provisioning](/terraform/enterprise/scim/troubleshooting).
