...
1. Copy and store the token securely before closing the dialog. The token is displayed only once. If you close the the dialog before saving the token, you must generate a new token. You need the token to configure the integration your identity provider.
1. Copy and save the URL from the **Base URL** field. You need the base URL to configure the integration with your identity provider.
...
1. It is optional, but we recommend configuring a token expiration.
1. Click **Create Token**.
