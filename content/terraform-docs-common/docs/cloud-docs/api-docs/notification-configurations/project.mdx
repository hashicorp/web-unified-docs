...
| Policy override required | `"run:policy_override_required"` | A policy check failed and requires a policy override to continue. |
...
| Needs attention <sup>DEPRECATED</sup> | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This input may include approving the plan or a policy override. Replaced by `"run:pending_apply_approval"`, `"run:cost_estimated"`, `"run:policy_override_required"`, and `"run:policies_checked"`. |
