---
page_title: Workspace notification configurations API reference for HCP Terraform
description: >-
 Use the HCP Terraform API's `/notification-configurations` endpoint to read, create, update, verify, and delete workspace configurations and create team configurations.
...
# Workspace notification configurations API reference

Use the workspace notifications API endpoints to send send messages to external applications when a run state transition or workspace event occurs. You can specify a destination URL, request type, and what events will trigger the notification. Each workspace can have up to 20 notification configurations, and they apply to all runs for that workspace.

## Overview

The following table lists the available API endpoints for managing workspace notification configurations:

| Method | Endpoint | Description |
| --- | --- | --- |
| `POST` | `/workspaces/:workspace_id/notification-configurations` | [Create a new notification configuration for a workspace](#create-a-notification-configuration) |
| `GET` | `/workspaces/:workspace_id/notification-configurations` | [List all notification configurations for a workspace](#list-notification-configurations) |
| `GET` | `/notification-configurations/:notification-configuration-id` | [Retrieve details of a specific notification configuration](#show-a-notification-configuration) |
| `PATCH` | `/notification-configurations/:notification-configuration-id` | [Update an existing notification configuration](#update-a-notification-configuration) |
| `POST` | `/notification-configurations/:notification-configuration-id/actions/verify` | [Send a verification request to test a notification configuration](#verify-a-notification-configuration) |
| `DELETE` | `/notification-configurations/:notification-configuration-id` | [Delete a notification configuration](#delete-a-notification-configuration) |


For project notification configurations, refer to [Project notification configurations API reference](/terraform/cloud-docs/api-docs/notification-configurations/project). 
For team notification configurations, refer to [Team notification configurations API reference](/terraform/cloud-docs/api-docs/notification-configurations/team).

Interacting with notification configurations requires admin access to the relevant workspace. Refer to [Permissions overview](/terraform/cloud-docs/users-teams-organizations/permissions) for more information.

[Speculative plans](/terraform/cloud-docs/workspaces/run/modes-and-options#plan-only-speculative-plan) and workspaces configured with `Local` [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) do not support notifications.

[permissions-citation]: #intentionally-unused---keep-for-maintainers

## Notification triggers

Notifications are sent in response to workspace triggers. You can specify workspace events in the `triggers` array attribute.
...
| ------------------ | ----------------------- | -------------------------------------------------------------------------------------------------------- |
| Created            | `"run:created"`         | A run is created and enters the [Pending stage](/terraform/cloud-docs/workspaces/run/states#the-pending-stage)                                                    |
| Planning           | `"run:planning"`        | A run acquires the lock and starts to execute.                                                      |
| Pending apply approval | `"run:pending_apply_approval"` | A plan has changes and requires user approval before Terraform can apply it.               |
| Cost estimated     | `"run:cost_estimated"`  | A cost estimate for the run is available.                                                            |
| Policy override required | `"run:policy_override_required"` | A policy check failed and requires a [policy override](/terraform/cloud-docs/workspaces/run/states#the-policy-check-stage) to continue. |
| Policies checked   | `"run:policies_checked"` | Policy validation completed.                                                                        |
| Applying           | `"run:applying"`        | A run enters the [Apply stage](/terraform/cloud-docs/workspaces/run/states#the-apply-stage), where Terraform makes the infrastructure changes described in the plan.                            |
| Completed          | `"run:completed"`       | A run completes successfully.                                     |
| Run errored        | `"run:run_errored"`     | The run ended due to an error.                                                                       |
| Run canceled       | `"run:run_canceled"`    | The run was canceled before completion.                                                              |
| Policy soft failed | `"run:policy_soft_failed"` | A policy check produced a warning (soft failure).                                                 |
| Needs Attention <sup>DEPRECATED</sup> | `"run:needs_attention"` | A plan has changes and Terraform requires user input to continue. This input may include approving the plan or a [policy override](/terraform/cloud-docs/workspaces/run/states#the-policy-check-stage). Replaced by `"run:pending_apply_approval"`, `"run:cost_estimated"`, `"run:policy_override_required"`, and `"run:policies_checked"`. |
| Errored <sup>DEPRECATED</sup> | `"run:errored"`  | A run terminates early due to error or cancellation. Replaced by `"run:run_errored"`, `"run:run_canceled"`, and `"run:policy_soft_failed"`. |
...
| Workspace Deleted | `"workspace:deleted"` | A workspace is deleted. |

The `"run:needs_attention"` and `"run:errored"` triggers are deprecated. Use their granular replacements when you create notification configurations. Existing configurations that subscribe to a deprecated trigger continue to receive notifications, and the payload's `trigger` field reports the deprecated value. Configurations that subscribe to a granular trigger receive the granular value in the `trigger` field. The HCP Terraform UI does not offer deprecated triggers when you create a configuration, but the API continues to accept them for backward compatibility. When you edit and save an existing configuration in the UI, HCP Terraform replaces deprecated triggers with their granular replacements.
...
The `type` and `id` attributes in `relationships.subscribable` may reference `"workspaces"`, `"projects"`, or `"teams"`.
