...
Refer to [tasks](/terraform/cloud-docs/api-docs/tasks/tasks) for the API endpoints to create and manage run tasks within HCP Terraform. You can also access a complete list of all run tasks in the [Terraform Registry](https://registry.terraform.io/browse/run-tasks).
...
| `stage`                              | string  | `pre_plan`, `post_plan`, `pre_apply`, `post_apply` | The [run stage](/terraform/cloud-docs/workspaces/run/states) when HCP Terraform triggers the run task.                                                                                                                    |
...
| `is_speculative`                     | bool    |                                                    | Whether the task is part of a [speculative run](/terraform/cloud-docs/workspaces/run/remote-operations#speculative-plans).                                                                                                |
...
| `vcs_commit_hash`                    | string  |                                                    | The commit hash that triggered this run. This is `null` if the workspace does not have a VCS repository or if the commit hash is unavailable.                                                                     |
...
```json
{
  ...
  "vcs_commit_hash": "7d8fb2a2d601edebdb7a59ad2088a96673637d22",
