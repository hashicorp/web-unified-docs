...
| `filter[task_category]` | **Optional.** Filters task configs to only include the task category. Either `hook` or `native`. |
...
curl \
  --header "Authorization: Bearer $TOKEN" \ 
  https://app.terraform.io/api/v2/organizations/my-organization/task-configs?
  filter%5Btarget_id%5D%3Dprj-WsVcWRr7SfxRci1v%26filter%5Btarget_type%5D%3Dproject
```
