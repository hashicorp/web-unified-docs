---
page_title: /tasks API reference for HCP Terraform
description: >-
  Use HCP Terraform API's `/tasks` endpoint to read, create, update, and delete tasks, and read, update, delete and associate tasks to workspaces.
...
Refer to [tasks Integration](/terraform/cloud-docs/api-docs/tasks/run-tasks-integration) for the API endpoints related triggering tasks and the expected integration response.
...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `:organization_name` | The organization to create a task in. The organization must already exist in HCP Terraform, and the token authenticating the API request must have [owner permission](/terraform/cloud-docs/users-teams-organizations/permissions/organization#organization-owners). |
...
| `data.attributes.global-configuration | object |  | (Optional) The global configuration object. Is ignored if a `task-configs` relationship is present. If a `task-configs` relationship is not present, it will create a Task configuration owned by the organization. |
...
| `data.relationships.agent-pool.data.id`                  | string |         | (Optional) The agent pool that HCP Terraform uses to make requests for the task. Requires [HCP Terraform Premium plan](https://www.hashicorp.com/en/pricing?tab=terraform), the [`private_run_tasks` feature entitlement](/terraform/cloud-docs/api-docs#feature-entitlements), and a self-hosted HCP Terraform agent with [request forwarding](/terraform/cloud-docs/agents/request-forwarding). |
| `data.relationships.task-configs.data` | list of objects |  | (Optional) The [task configurations](/terraform/cloud-docs/api-docs/tasks/task-configs) for the task.
| `data.relationships.task-configs.data.type` | string |  | Must be `"task-configs"`. |
| `data.relationships.task-configs.data.attributes.global` | string |  | Is this task configuration globally applied. See: [Global vs Targeted Configurations](/terraform/cloud-docs/api-docs/tasks/task-configs#global-vs-targeted-configurations) |
...
| `data.relationships.task-configs.data.relationships.owner.data.type` | string |  | The owner type of the task configuration. See: [Task Configuration Owners](/terraform/cloud-docs/api-docs/tasks/task-configs#task-configuration-owners) |
| `data.relationships.task-configs.data.relationships.projects.data` | list of objects |  | Specifies a list of projects to associated to the task configuration. See: [Global vs Targeted Configurations](/terraform/cloud-docs/api-docs/tasks/task-configs#global-vs-targeted-configurations) |
...
| Parameter | Description                                                                                   |
| --------- | --------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the task to show. Use the ["List Tasks"](#list-run-tasks) endpoint to find IDs. |
...
| Parameter | Description                                                                                     |
| --------- | ----------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the task to update. Use the ["List Tasks"](#list-run-tasks) endpoint to find IDs. |
...
| Parameter | Description                                                                                         |
| --------- | --------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the task to delete. Use the ["List Tasks"](#list-run-tasks) endpoint to find IDs. |
...
- Advisory tasks can not block a run from completing. If the task fails, the run will proceed with a warning.
...
You may also configure the task to begin during specific [run stages](/terraform/cloud-docs/workspaces/run/states). Tasks use the [Post-Plan Stage](/terraform/cloud-docs/workspaces/run/states#the-post-plan-stage) by default.
...
| Parameter | Description                                                                                                                 |
| --------- | --------------------------------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the workspace task to show. Use the ["List Workspace Tasks"](#list-workspace-run-tasks) endpoint to find IDs. |
...
| Parameter | Description                                                                                                         |
| --------- | ------------------------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the task to update. Use the ["List Workspace Tasks"](#list-workspace-run-tasks) endpoint to find IDs. |
...
| Parameter | Description                                                                                                                       |
| --------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `:id`     | The ID of the Workspace task to delete. Use the ["List Workspace Tasks"](#list-workspace-run-tasks) endpoint to find IDs. |
