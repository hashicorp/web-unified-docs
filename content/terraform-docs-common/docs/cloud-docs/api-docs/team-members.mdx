...
In HCP Europe organizations, you use HCP [groups](https://docs.hashicorp.com/hcp/docs/hcp/iam/groups) to manage permissions instead of teams. If your URL includes `portal.cloud.eu.hashicorp` or `app.eu.terraform.io`, then you are in an HCP Europe organization.

The following team membership APIs are not available for HCP Europe organizations. You can only manage user membership to a group on the [HCP platform](https://portal.cloud.eu.hashicorp.com).
