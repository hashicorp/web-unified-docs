...
To manage user memberships in a [group](https://docs.hashicorp.com/hcp/docs/hcp/iam/groups) for a HCP Europe organizations, you must use the HashiCorp Cloud Platform [(HCP)](https://portal.cloud.hashicorp.com). The Team membership APIs are not available for use with HCP Europe organizations.
