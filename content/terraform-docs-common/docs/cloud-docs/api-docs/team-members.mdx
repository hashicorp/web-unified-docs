...
For teams managed by your identity provider (IdP) through SCIM, HCP Terraform rejects calls to update the team membership. Use your identity provider to make changes, which HCP Terraform applies through the [SCIM provisioning endpoints](/terraform/cloud-docs/api-docs/scim).
