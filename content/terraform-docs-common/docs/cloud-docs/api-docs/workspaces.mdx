...
| `data.attributes.vcs-repo.github-app-installation-id` | string | none | Specifies the [GitHub App](/terraform/cloud-docs/vcs/github-app) installation to use. Find this ID on the account settings page. You can not specify this value if `oauth-token-id` is specified. The authenticated user must have [authorized HCP Terraform on their GitHub account](/terraform/cloud-docs/vcs/github-app#authorizing) so HCP Terraform can resolve their access to the repository. If they have not, this endpoint returns a `422` error with the description `Repository requires the authenticated user to link a GitHub account via the GitHub App`. Refer to [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the constraints on each HCP Terraform token type. |
...
| `data.attributes.vcs-repo.github-app-installation-id` | string |  | The VCS Connection GitHub App Installation to use. Find this ID on the account settings page. You can not specify this value if `oauth-token-id` is specified. The authenticated user must have [authorized HCP Terraform on their GitHub account](/terraform/cloud-docs/vcs/github-app#authorizing) so HCP Terraform can resolve their access to the repository. If they have not, this endpoint returns a `422` error with the description `Repository requires the authenticated user to link a GitHub account via the GitHub App`. Manage the token from **Account Settings** within HCP Terraform. Refer to [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the constraints on each HCP Terraform token type.                                                                                                                                                                               |
