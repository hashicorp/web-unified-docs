...
last_modified: 2025-10-24T16:20:42-04:00
...
Refer to [Workspace Permissions](/terraform/cloud-docs/users-teams-organizations/permissions/workspace) for additional information.
...
When you use `vcs-repo.github-app-installation-id` to connect a workspace to a [GitHub App](/terraform/cloud-docs/vcs/github-app)-backed repository, the user who owns the API token must have already authorized HCP Terraform on their GitHub account. Without that authorization, this endpoint returns a `422` with the error `Repository requires the authenticated user to link a GitHub account via the GitHub App`. To resolve the error, sign in to HCP Terraform as that user and follow the steps in [Authorizing the GitHub App](/terraform/cloud-docs/vcs/github-app#authorizing). Refer to [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the constraints on each token type.
...
| `data.attributes.global-remote-state`                   | boolean         | `false`                     | Enables all workspaces in the organization to [access this workspace's state data](/terraform/enterprise/workspaces/state) during runs. <p>On HCP Terraform, if it's set to `false`, then only specifically approved workspaces can access its state.</p><p> On Terraform Enterprise, do not set to `true` if `project-remote-state` is also `true`.</p>  Refer to [Remote State Consumers](/terraform/enterprise/api-docs/workspaces#get-remote-state-consumers) for information about managing allowed workspaces.      |
...
| `data.attributes.hyok-enabled`       | boolean | `false` | Whether this workspace secures HCP Terraform artifacts using hold your own key encryption. Hold your own key is only available in HCP Terraform, [learn more](/terraform/cloud-docs/hold-your-own-key).   |
...
_Using a project-level default execution mode_
...
| `data.attributes.global-remote-state`         | boolean        | (previous value) | Enables all workspaces in the organization to [access this workspace's state data](/terraform/enterprise/workspaces/state) during runs. <p>On HCP Terraform, if it's set to `false`, then only specifically approved workspaces can access its state.</p><p> On Terraform Enterprise, do not set to `true` if `project-remote-state` is also `true`.</p>  Refer to [Remote State Consumers](/terraform/enterprise/api-docs/workspaces#get-remote-state-consumers) for information about managing allowed workspaces.      |
...
| `data.attributes.vcs-repo.github-app-installation-id` | string |  | The VCS Connection GitHub App Installation to use. Find this ID on the account settings page. The authenticated user must have [authorized HCP Terraform on their GitHub account](/terraform/cloud-docs/vcs/github-app#authorizing) so HCP Terraform can resolve their access to the repository. If they have not, this endpoint returns the error `Repository requires the authenticated user to link a GitHub account via the GitHub App`. Manage the token from **Account Settings** within HCP Terraform. You can not specify this value if `oauth-token-id` is specified.                                                                                                                                                                               |
...
| `data.attributes.setting-overwrites` | object | | The keys in this object are attributes that have project-level defaults. Each attribute key stores a boolean value which is `false` by default. To overwrite the default inherited value, set an attribute's value to `true`. For example, to set `execution-mode` as the project default, you set `setting-overwrites.execution-mode = true`. |
...
This endpoint retrieves the list of other workspaces that are allowed to access the given workspace's state during runs according to the workspace's state-sharing configuration. The following table describes how Terraform Enterprise determines access to the state data:

| Authorization type | Attributes | Results |
| --- | --- | --- |
| Explicit authorization | `global-remote-state` is `false` <br/><nobr>`project-remote-state` is `false`</nobr> | The endpoint returns a list of only the workspaces that have been specifically authorized to access this workspace's state. |
| Project-wide access | `project-remote-state` is `true` | The endpoint returns a list of all other workspace in the same project. |
| Organization-wide access | `global-remote-state` is `true` | The endpoint returns a list of all other workspaces in the organization. |
...
-   If `global-remote-state` is set to `false` for the workspace, the endpoint returns the list of other workspaces that are specifically authorized to access the workspace's state.
-   If `global-remote-state` is set to `true`, the endpoint returns a list of all other workspace in the organization.
...
<!-- BEGIN: TFC:only name:project-remote-state -->

This endpoint updates the workspace's remote state consumers to be exactly the list of workspaces specified in the payload. You can only use this endpoint for workspaces where `global-remote-state` is `false`.

<!-- END: TFC:only name:project-remote-state -->

<!-- BEGIN: TFEnterprise:only name:project-remote-state -->

This endpoint updates the workspace's remote state consumers to be exactly the list of workspaces specified in the payload. You can only use this endpoint for workspaces where `global-remote-state` and `project-remote-state` are `false`.
...
This endpoint adds one or more remote state consumers to the workspace. You can only use this endpoint for workspaces where `global-remote-state` is `false`.

<!-- END: TFC:only name:project-remote-state -->

<!-- BEGIN: TFEnterprise:only name:project-remote-state -->

This endpoint adds one or more remote state consumers to the workspace. You can only use this endpoint for workspaces where `global-remote-state` and `project-remote-state` are `false`.
...
This endpoint removes one or more remote state consumers from a workspace, according to the contents of the payload. You can only use this endpoint for workspaces where `global-remote-state` is `false`.

<!-- END: TFC:only name:project-remote-state -->

<!-- BEGIN: TFEnterprise:only name:project-remote-state -->

This endpoint removes one or more remote state consumers from a workspace, according to the contents of the payload. You can only use this endpoint for workspaces where `global-remote-state` and `project-remote-state` are `false`.
