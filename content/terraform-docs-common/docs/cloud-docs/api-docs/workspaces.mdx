...
| `data.attributes.hyok-enabled`       | boolean | `false` | Whether this workspace will secure HCP Terraform artifacts using Hold Your Own Key (HYOK) encryption. Once enabled, **this cannot be disabled**. HCP Terraform only, requires the [HCP Terraform Premium plan](https://www.hashicorp.com/en/pricing?tab=terraform).   |
