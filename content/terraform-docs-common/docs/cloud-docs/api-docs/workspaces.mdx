...
When `vcs-repo.github-app-installation-id` is used to connect a workspace to a [GitHub App](/terraform/cloud-docs/vcs/github-app)-backed repository, the authenticated user (the owner of the API token) must have already authorized HCP Terraform on their GitHub account. Without that linkage, this endpoint returns a `422` with the error `The authenticated user has not linked a GitHub account to HCP Terraform via the GitHub App.` To resolve, sign in to HCP Terraform as that user and follow the steps in [Authorizing the GitHub App](/terraform/cloud-docs/vcs/github-app#authorizing). See [GitHub App-backed VCS and API token types](/terraform/cloud-docs/users-teams-organizations/api-tokens#github-app-backed-vcs-and-api-token-types) for the constraints on each token type.
