...
| `data.attributes.invoke-action-addrs`              | array containing a string      |                                                    | Specifies the action address to invoke for an action-only run. You can specify one address. Mutually exclusive with `target-addrs` and `is-destroy`. Empty arrays are ignored. Requires a Terraform version that supports action invocations and access to actions in HCP Terraform or Terraform Enterprise.                                                                                                                                                             |
...
```json
{
  "data": {
    "attributes": {
      ...
      "invoke-action-addrs": ["action.aws_lambda_invoke.example"]
    },
    "type":"runs",
    "relationships": {
      "workspace": {
