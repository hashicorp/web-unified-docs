---
page_title: /organizations API reference for HCP Terraform
description: >-
   Use the HCP Terraform API's `/organizations` endpoint to create, update, and destroy organizations, and read their entitlement sets, module producers, and data retention policies.
...
**Note:** Only organizations entitled to `Stacks` return the `stacks-default-execution-mode` property and `stacks-default-agent-pool` relationship.
  ...
  "data": [
    ...
    },
    {
      ...
      "attributes": {
        ...
        "assessments-enforced": false,
        "default-execution-mode": "remote",
        "stacks-default-execution-mode": "remote",
        "user-tokens-enabled": true
      },
      "relationships": {
        "default-agent-pool": {
...
| `data.attributes.allow-force-delete-workspaces`                           | boolean | (false)   | Whether workspace administrators can [delete workspaces with resources under management](/terraform/cloud-docs/users-teams-organizations/organizations/settings#workspace-administrators-can-force-delete-workspaces). If false, only organization owners may delete these workspaces.                                                                                                                                                                              |
...
| `data.attributes.assessments-enforced`                                    | boolean | false     | Whether or not to compel health assessments for all eligible workspaces. When true, health assessments occur on all compatible workspaces, regardless of the value of the workspace setting `assessments-enabled`. When false, health assessments only occur for workspaces that opt in by setting `assessments-enabled: true`.             |
| `data.attributes.allow-force-delete-workspaces`                           | boolean | false     | Whether workspace administrators can [delete workspaces with resources under management](/terraform/cloud-docs/users-teams-organizations/organizations/settings#workspace-administrators-can-force-delete-workspaces). If false, only organization owners may delete these workspaces.                                                                                                                                                                              |
| `data.attributes.default-execution-mode`                                  | string  | `remote`  | Which [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) to use by default for workspaces. Valid values are `default`, `remote`, `local`, and `agent`.                                                                                                                                                                           |
| `data.attributes.stacks-default-execution-mode`                           | string  | `remote`  | Which [execution mode](/terraform/cloud-docs/stacks/configure#change-execution-mode) to use by default for stacks. Valid values are `default`, `remote`, and `agent`.                                                                                                                                                                           |
| `data.attributes.default-agent-pool-id`                                   | string  | (previous value) | Required when `default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the organization. Do _not_ specify this value if you set `execution-mode` to `default`, `remote` or `local`.                                                                                                                      |
| `data.attributes.stacks-default-agent-pool-id`                            | string  | (previous value) | Required when `stacks-default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the organization. Do _not_ specify this value if you set `execution-mode` to `default` or `remote`.                                                                                                                      |
...
**Note:** The `stacks-default-execution-mode` property and `stacks-default-agent-pool` relationship are only returned from organizations entitled to Stacks.
...
```
<!-- BEGIN: TFEnterprise:only name:module-producers-endpoint -->
## Show Module Producers

This endpoint shows organizations that are configured to share modules with an organization through [module sharing](/terraform/enterprise/api-docs/admin/registry-sharing).
...
```
<!-- END: TFEnterprise:only name:module-producers-endpoint -->
<!-- BEGIN: TFEnterprise:only name:module-data-retention-policy-endpoint -->
...
<!-- END: TFEnterprise:only name:module-data-retention-policy-endpoint -->
