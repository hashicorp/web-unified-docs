...
| `data.attributes.execution-mode`                      | string  | (previous value) | The execution mode that Terraform uses for this Stack. It can be only `remote` or `agent`.                                                                                                                            |
...
Each configuration summary is associated with a single Stack configuration. The summary includes counts of how many of its deployment groups and deployment runs are in each available status.

There is no endpoint for showing a single configuration summary by its ID. Instead, sideload the configuration's associated summary by including the `stack_configuration_summary` query parameter when calling the `/stack-configurations` or `/:stack_configuration_id` endpoints.
...
```json
{
  "data": [
    {
      "id": "stc-EGBivHirPGeTRoeL-summary",
  ...
  "links": {
    "self": "https://app.terraform.io/api/v2/stacks/st-ejrPFX2wkdYJzAdp/stack-configuration-summaries?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "first": "https://app.terraform.io/api/v2/stacks/st-ejrPFX2wkdYJzAdp/stack-configuration-summaries?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "prev": null,
    "next": null,
    "last": "https://app.terraform.io/api/v2/stacks/st-ejrPFX2wkdYJzAdp/stack-configuration-summaries?page%5Bnumber%5D=1&page%5Bsize%5D=20"
  },
  "meta": {
    "pagination": {
      "current-page": 1,
