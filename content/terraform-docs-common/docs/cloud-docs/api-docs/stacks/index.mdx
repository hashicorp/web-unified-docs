...
There is no endpoint for showing a single configuration summary by its ID, but you can use the configuration endpoint's `stack_configuration_summary` include parameter to sideload the configuration's associated summary.
