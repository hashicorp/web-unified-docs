...
| `GET`    | `/stack-configurations/:stack_configuration_id/stack-deployment-runs`                                | Call this endpoint to [list Stack deployment runs in a configuration](#list-stack-deployment-runs-in-a-configuration). |
| `GET`    | `/stack-deployment-groups/:stack_deployment_group_id/stack-deployment-runs`                          | Call this endpoint to [list Stack deployment runs in a group](#list-stack-deployment-runs-in-a-group).                 |
...
| `GET`    | `/stack-deployment-steps/:stack_deployment_step_id/artifacts`                                        | Call this endpoint to [retrieve a Stack deployment step artifact](#show-a-stack-deployment-step-artifact).             |
...
| `POST`   | `/stack-deployment-groups/:stack_deployment_group_id/start`                                          | Call this endpoint to [start a Stack deployment group](#start-a-stack-deployment-group).                               |
| `POST`   | `/stack-deployment-groups/:stack_deployment_group_id/cancel`                                         | Call this endpoint to [cancel a Stack deployment group](#cancel-a-stack-deployment-group).                             |
...
| `q`            | **Optional.** Search by deployment name. This search is case-insensitive.                                           |
| `sort`         | **Optional.** Sort by deployment name. Accepted values are "deployment" (ascending) and "-deployment" (descending). |
...
| Parameter        | Description                                                                                                                                                                                                         |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filter[status]` | **Optional.** Filters results to groups matching one or more of the specified statuses. Accepts a comma-separated list.                                                                                             |
| `page[number]`   | **Optional.** If omitted, the endpoint will return the first page.                                                                                                                                                  |
| `page[size]`     | **Optional.** If omitted, the endpoint will return 20 Stack deployment groups per page.                                                                                                                             |
| `q`              | **Optional.** Search by group name. This search is case-insensitive.                                                                                                                                                |
| `sort`           | **Optional.** Sort by group `name` or `status-priority`. Prefix with `-` to reverse order. `status-priority` orders groups from highest to lowest: failed, deploying, pre-deploying, pending, abandoned, succeeded. |

### Available Related Resources

To retrieve related resources, you can optionally add [the `include` query parameter](/terraform/cloud-docs/api-docs#inclusion-of-related-resources) when making `GET` requests. The following resource types are available:

| Resource Name                    | Description                                                        |
| -------------------------------- | ------------------------------------------------------------------ |
| `stack_deployment_group_summary` | A summary of the current statuses for the group's deployment runs. |
...
```json
{
  "data": [
    {
      "id": "sdg-56aAuyFSwfD12Wna",
      "type": "stack-deployment-groups",
      "attributes": {
        "status": "succeeded",
        "created-at": "2026-07-09T19:10:00.513Z",
        "updated-at": "2026-07-09T19:10:18.917Z",
        "deploying-at": "2026-07-09T19:10:11.083Z",
        "succeeded-at": "2026-07-09T19:10:18.915Z",
        "failed-at": null,
        "abandoned-at": null,
      ...
      "relationships": {
        ...
        "stack-deployment-runs": {
          "links": {
            "related": "/api/v2/stack-deployment-groups/sdg-56aAuyFSwfD12Wna/stack-deployment-runs"
          }
        },
        "stack-deployment-group-summary": {
          "data": {
            "id": "sdg-56aAuyFSwfD12Wna-summary",
      ...
      "links": {
        "self": "/api/v2/stack-deployment-groups/sdg-56aAuyFSwfD12Wna",
        "stack-deployment-group-summaries": "/api/v2/stack-configurations/stc-EGBivHirPGeTRoeL/stack-deployment-group-summaries"
      }
    }
  ],
  "links": {
    "self": "https://app.terraform.io/api/v2/stack-configurations/stc-EGBivHirPGeTRoeL/stack-deployment-groups?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "first": "https://app.terraform.io/api/v2/stack-configurations/stc-EGBivHirPGeTRoeL/stack-deployment-groups?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "prev": null,
    "next": null,
    "last": "https://app.terraform.io/api/v2/stack-configurations/stc-EGBivHirPGeTRoeL/stack-deployment-groups?page%5Bnumber%5D=1&page%5Bsize%5D=20"
  },
  "meta": {
    "pagination": {
      "current-page": 1,
...
| `:stack_deployment_group_name` | The Stack deployment group name.                                      |
...
| Parameter             | Description                                                                                                                                                                                                                                                                                                                                                                |
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filter[latest_only]` | **Optional.** When set to `true`, returns only the latest run for each deployment within the configuration.                                                                                                                                                                                                                                                                |
| `filter[status]`      | **Optional.** When set to `true`, returns only runs matching one or more of the specified statuses. Accepts a comma-separated list. The following statuses each map to multiple underlying run states: `pending` (pending, acquiring-lock, pending-capacity), `running` (pre-deploying, deploying), and `pending-operator` (pre-deploying-pending-operator, deploying-pending-operator). |
| `page[number]`        | **Optional.** If omitted, the endpoint will return the first page.                                                                                                                                                                                                                                                                                                         |
| `page[size]`          | **Optional.** If omitted, the endpoint will return 20 Stack deployment runs per page.                                                                                                                                                                                                                                                                                      |
| `q`                   | **Optional.** Search by deployment name or group name. This search is case-insensitive.                                                                                                                                                                                                                                                                                    |
| `sort`                | **Optional.** Sort by `deployment` (deployment name) or `deployment-group-name`. Prefix with `-` to reverse order.                                                                                                                                                                                                                                                         |
...
| Parameter             | Description                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `filter[latest_only]` | **Optional.** When set to `true`, returns only the latest run for each deployment within the group.                                                                                                                                                                                                                                                                                                        |
| `filter[status]`      | **Optional.** When set to `true`, returns only runs matching one or more of the specified statuses. Accepts a comma-separated list. The following statuses each map to multiple underlying run states: `failed`, `pending-operator` (pre-deploying-pending-operator, deploying-pending-operator), `running` (pre-deploying, deploying), `pending` (pending, acquiring-lock, pending-capacity), `abandoned`, `succeeded`. |
| `page[number]`        | **Optional.** If omitted, the endpoint will return the first page.                                                                                                                                                                                                                                                                                                                                         |
| `page[size]`          | **Optional.** If omitted, the endpoint will return 20 Stack deployment runs per page.                                                                                                                                                                                                                                                                                                                      |
| `q`                   | **Optional.** Search by deployment name. This search is case-insensitive.                                                                                                                                                                                                                                                                                                                                  |
| `sort`                | **Optional.** Sort by group `name` or `status-priority`. Prefix with `-` to reverse order. `status-priority` orders groups from highest to lowest: failed, deploying, deploying-pending-operator, pre-deploying, pre-deploying-pending-operator, acquiring-lock, pending-capacity, pending, abandoned, succeeded.                                                                                          |
...
| `sort`         | **Optional.** Sort by step order. Prefix with `-` to reverse order.                         |
...
| Parameter                    | Description                                      |
| -----------------------------| ------------------------------------------------ |
| `:stack_deployment_group_id` | The id of the Stack deployment group to approve. |

| Status  | Response                  | Reason                                                                              |
| ------- | ------------------------- | ----------------------------------------------------------------------------------- |
| [204][] | No Content                | Successfully approved all plans for the Stack deployment group.                     |
...
| Parameter                    | Description                                    |
| ---------------------------- | ---------------------------------------------- |
| `:stack_deployment_group_id` | The id of the Stack deployment group to rerun. |

| Status  | Response                  | Reason                                                                           |
| ------- | ------------------------- |--------------------------------------------------------------------------------- |
| [204][] | No Content                | Successfully reran the Stack deployment group.                                   |
| [404][] | [JSON API error object][] | Stack deployment group not found, or user unauthorized to perform action.        |
| [409][] | [JSON API error object][] | The `deployments` query parameter is present but empty.                          |
| [422][] | [JSON API error object][] | One or more of the specified deployments are not in a failed or abandoned state. |
...
| Parameter     | Description                                                                                                                                                                                                                                           |
| ------------- |------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `deployments` | **Optional.** A comma-separated list specifying the names of the deployments within the group that are to be rerun. If omitted, all failed or abandoned deployments will be rerun. If present, all requested deployments must be failed or abandoned. |
...
## Start a Stack deployment group

This endpoint starts planning all runs in a Stack deployment group that has eager plan disabled.

`POST /stack-deployment-groups/:stack_deployment_group_id/start`

| Parameter                    | Description                                    |
| ---------------------------- |----------------------------------------------- |
| `:stack_deployment_group_id` | The id of the Stack deployment group to start. |

| Status  | Response                  | Reason                                                                    |
| ------- | ------------------------- | ------------------------------------------------------------------------- |
| [202][] | None                      | Successfully started planning the Stack deployment group.                 |
| [404][] | [JSON API error object][] | Stack deployment group not found, or user unauthorized to perform action. |
...
$ curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/stack-deployment-groups/stg-LSqektntTNJrAhCt/start
```

## Cancel a Stack deployment group

This endpoint cancels a Stack deployment group.

`POST /stack-deployment-groups/:stack_deployment_group_id/cancel`

| Parameter                    | Description                                     |
| ---------------------------- | ----------------------------------------------- |
| `:stack_deployment_group_id` | The id of the Stack deployment group to cancel. |

| Status  | Response                  | Reason                                                                    |
| ------- | ------------------------- | ------------------------------------------------------------------------- |
| [202][] | None                      | Successfully cancels the Stack deployment group.                          |
| [404][] | [JSON API error object][] | Stack deployment group not found, or user unauthorized to perform action. |
| [409][] | [JSON API error object][] | Stack deployment group is in a terminal state and cannot be canceled.     |
...
$ curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/stack-deployment-groups/stg-LSqektntTNJrAhCt/cancel
```
...
| Parameter                  | Description                                    |
| -------------------------- | ---------------------------------------------- |
| `:stack_deployment_run_id` | The id of the Stack deployment run to approve. |

| Status  | Response                  | Reason                                                                  |
| ------- | ------------------------- | ----------------------------------------------------------------------- |
| [204][] | No Content                | Successfully approved all plans for the Stack deployment run.           |
...
| Parameter                  | Description                                    |
| -------------------------- | ---------------------------------------------- |
| `:stack_deployment_run_id` | The id of the Stack deployment run to cancel.  |

| Status  | Response                  | Reason                                                                  |
| ------- | ------------------------- | ----------------------------------------------------------------------- |
| [202][] | None                      | Successfully canceled the Stack deployment run.                         |
| [404][] | [JSON API error object][] | Stack deployment run not found, or user unauthorized to perform action. |
...
| Parameter                   | Description                                      |
| --------------------------- | ------------------------------------------------ |
| `:stack_deployment_step_id` | The id of the Stack deployment step to advance. |

| Status  | Response                  | Reason                                                                   |
| ------- | ------------------------- | ------------------------------------------------------------------------ |
| [204][] | No Content                | Successfully advanced the Stack deployment step.                         |
