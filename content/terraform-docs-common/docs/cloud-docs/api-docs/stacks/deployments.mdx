...
| Parameter                   | Description                                                                                       |
|-----------------------------|---------------------------------------------------------------------------------------------------|
| `stack_deployment_step_id`  | **Required.** The id of the Stack deployment step. Must match the step ID in the URL path.        |
| `page[number]`              | **Optional.** If omitted, the endpoint will return the first page.                                |
...
$ curl\
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/stack-deployment-steps/sds-LSqektntTNJrAhCt/stack-diagnostics?stack_deployment_step_id=sds-LSqektntTNJrAhCt"
```
