  ...
  "data": [
    {
      ...
      "attributes": {
        ...
        "detail": "While preparing the configuration, HCP Terraform reported 1 warning.",
        "diags": [
          {
...
This endpoint returns an HTTP 307 redirect to the actual artifact URL. Use `curl -L` or configure your HTTP client to follow redirects automatically.
...
$ curl -L \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/stack-deployment-steps/sds-LSqektntTNJrAhCt/artifacts?name=plan-description
```

### Retrieving Stack Outputs

The `apply-description` artifact contains Stack outputs from a completed apply operation. This is the programmatic method for retrieving output values after a successful deployment.

To retrieve Stack outputs, request the `apply-description` artifact from the final apply step of a successful deployment run:

```shell
$ curl -L \
  --header "Authorization: Bearer $TOKEN" \
  https://app.terraform.io/api/v2/stack-deployment-steps/sds-ApplyStepId/artifacts?name=apply-description
```
