...
There is no endpoint for showing a single deployment group summary by its ID, but you can use the deployment group endpoint's `stack_deployment_group_summary` include parameter to sideload the group's associated summary.
