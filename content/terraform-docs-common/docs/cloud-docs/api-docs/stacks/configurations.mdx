...
## Fetch a Stack configuration upload URL
...
The following example demonstrates the full workflow for manually uploading a Stack configuration.

1. Create a Stack configuration.
...
2. Fetch the upload URL using the Stack configuration ID from the response.

   ```shell
   $ curl\
     --header "Authorization: Bearer $TOKEN" \
     --header "Content-Type: application/vnd.api+json" \
     https://app.terraform.io/api/v2/stack-configurations/stc-4EWffseuNa1FpfDH/upload-url
   ```

3. Upload the archive to the `source-upload-url` from the response.
...
After uploading, HCP Terraform will begin preparing the configuration. 
