...
| `stack_configuration_summary` | A summary of the current statuses for the configurations deployment groups and deployment runs. |
...
Each deployment group summary is associated with a single deployment group, and includes counts of how many of its deployment runs are in each available status.
...
-> **Note:** The `status-counts` attribute is a deprecated alias for the `stack-deployment-run-status-summary` attribute.
