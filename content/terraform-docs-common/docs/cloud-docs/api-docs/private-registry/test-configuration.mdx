...
Configure dynamic credentials for registry module tests using the Test Configuration API. This API allows you to enable and manage OIDC authentication for module test runs, eliminating the need for static credentials.
...
- `registry_name` (required): The registry name (usually `private`)
- `namespace` (required): The namespace (organization name)
- `name` (required): The module name
- `provider` (required): The provider name (e.g., `aws`, `gcp`, `azurerm`)
...
- `oidc-enabled` (boolean, required): Set to `true` to enable dynamic credentials
- `oidc-provider` (string, required when `oidc-enabled` is `true`): The OIDC provider type. Must be one of:
  - `aws` - Amazon Web Services
  - `gcp` - Google Cloud Platform
  - `azure` - Microsoft Azure
  - `vault` - HashiCorp Vault
- `oidc-configuration` (object, required when `oidc-enabled` is `true`): Provider-specific configuration. See provider sections below.
...
- `role-arn` (string, required): The ARN of the IAM role to assume
- `audience` (string, optional): The audience claim for the OIDC token. Defaults to `aws.workload.identity`
...
- Configure provider-specific dynamic credentials:
...
- Review the [Test Runs API](/terraform/cloud-docs/api-docs/private-registry/tests) to see OIDC token information in test run responses
