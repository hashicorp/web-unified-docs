...
Call the following endpoint to list all tags assigned to a module version.
...
Send a `PATCH` request to the following endpoint to add, remove, or update tags for a module version in your organization's registry.
...
`PATCH` request must include a JSON object with the following properties as a request payload.
