...
~> **Security Note**: The actual OIDC token is never returned by way of the API. These attributes only provide metadata about token generation and expiration for auditing purposes.

For information about configuring dynamic credentials for module tests, refer to:
- [Test Configuration API](/terraform/cloud-docs/api-docs/private-registry/test-configuration)
