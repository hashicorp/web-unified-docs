...
Send a `PATCH` request to the following endpoint to adds or update tags for a registry provider in your organization's registry.

To remove tags from a registry module, omit the tags from the request body.
...
| `:namespace`         | The namespace of the provider. For private providers, the namespace is typically your organization name .                                                                          |
...
To send a `PATCH` request to this endpoint, you must include the following properties as a request payload.
...
The following endpoint returns all published versions for a registry provider including tags attached to each version.
...
The following request filters provider versions where the key is `environment` and the value is `testing`.
