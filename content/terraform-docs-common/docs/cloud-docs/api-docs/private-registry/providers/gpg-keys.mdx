...
Uploads a GPG Key to a private registry scoped with a namespace. The response will provide a "key-id", which is required to [Create a Provider Version](/terraform/cloud-docs/api-docs/private-registry/providers/versions-platforms#create-a-provider-version).
