...
For public modules, you can only send `DELETE` requests when specifying the module namespace and name. Sending a `DELETE` request to a public module and specifying additional parameters returns a 404.
...
| [404][] | [JSON API error object][] | Module not found |
...
To remove tags from a registry module, send a `PATCH` request and omit the tags from the request body.
...
| [404][] | [JSON API error object][] | Registry module not found|
...
To send a `PATCH` request, you must include a JSON object with the following properties as a request payload.  
...
The following endpoint returns all published versions for a registry module including tags attached to each version.
