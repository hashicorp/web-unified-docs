---
page_title: /tests API reference for HCP Terraform
description: >-
  Use the HCP Terraform API's `/tests` endpoint to list, get, create, and cancel Terraform test runs.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2025-05-27T14:28:51-04:00
last_modified: 2026-02-02T15:47:39-08:00
...
When [dynamic credentials](/terraform/cloud-docs/registry/test/dynamic-credentials) are enabled for a module's tests, test runs include OIDC token information. The following attributes track OIDC token usage:

| Attribute                                  | Description                                                                 |
| ------------------------------------------ | --------------------------------------------------------------------------- |
| `data.attributes.oidc-token-generated-at`  | The timestamp when the OIDC token was generated (ISO 8601 format).          |
| `data.attributes.oidc-token-expires-at`    | The timestamp when the OIDC token expires (ISO 8601 format).                |

<Note>

The actual OIDC token is never returned by way of the API. These attributes only provide metadata about token generation and expiration for auditing purposes.

</Note>

For information about configuring dynamic credentials for module tests, refer to:
- [Test Configuration API](/terraform/cloud-docs/api-docs/private-registry/modules/test-configuration)
- [Dynamic credentials for module testing](/terraform/cloud-docs/registry/test/dynamic-credentials)

### Test Sources
List tests for a module. You can use the following sources as [tests list](/terraform/cloud-docs/api-docs/private-registry/modules/tests#list-tests-for-a-module) query parameters.
...
```json
{
  "data": {
    ...
    "attributes": {
      ...
      "oidc-token-generated-at": "2023-10-03T18:27:39.500Z",
      "oidc-token-expires-at": "2023-10-03T18:37:39.500Z"
    },
    "relationships": {
...
| `filter[source]`    | **Optional.** A comma-separated list of test sources; the result will only include tests that came from one of these sources. Options are listed in [Test Sources](/terraform/cloud-docs/api-docs/private-registry/modules/tests#test-sources).                                                                                                                                       |
