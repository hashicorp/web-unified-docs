...
Send a `PATCH` request to the following endpoint to add or update tags for a Stack component configuration in your organization's registry.

To remove tags from a Stack component configuration, omit the tag key and values from the request body when sending the `PATCH` request.
...
| [404][] | [JSON API error object][] | Component configuration not found                            |
| [415][] | [JSON API error object][] | Invalid `Content- Type` header. The `Content-Type` must be `application/vnd.api+json` |

### Request body

[](#request-body)

To send a `PATCH` request,  the request body must contain a JSON object with the following properties as a request payload.

You must include the `Content-Type` header set to `application/vnd.api+json`, which is not a requirement for other registry artifacts.
...
The following endpoint returns all published versions for a Stack component configuration in the registry including the tags attached to each version.
...
The following request filters component configuration versions where the key is `environment` and the value is `staging`.
