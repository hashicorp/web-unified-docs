---
page_title: /registry-modules API reference for HCP Terraform
description: >-
  Use the HCP Terraform API's `/registry-modules` endpoint to read, publish, update, delete, and add versions to modules in your organization's private registry.
  ...
  "data": [
    {
      ...
      "relationships": {
        ...
        "tag-bindings": {
          "links": {
            "related": "/api/v2/registry-modules/mod-kwt1cBiX2SdDz38w/tag-bindings"
          }
				}
      },
      "links": {
    ...
    },
    {
      ...
      "relationships": {
        ...
        "tag-bindings": {
          "links": {
            "related": "/api/v2/registry-modules/mod-PopQnMtYDCcd3PRX/tag-bindings"
          }
				}
      },
      "links": {
...
| `data.attributes.test-config.agent-execution-mode`    | string  | `remote`  | The execution mode for test runs. Must be either `remote` or `agent`. When set to `agent`, tests will run on agents in the specified agent pool. Only available when the enterprise-testing feature is enabled.                                                                                                      |
| `data.attributes.test-config.agent-pool-id`           | string  |           | The ID of the agent pool to use for test runs. Required when `agent-execution-mode` is set to `agent`. Must not be set when `agent-execution-mode` is `remote`. Only available when the enterprise-testing feature is enabled.                                                                                      |
...
| `data.attributes.commit-sha` | string | `null`  | If applicable, the commit SHA you use when creating the module version archive you are uploading. |
...
```json
{
  "data": {
    ...
    "relationships": {
      ...
      "tag-bindings": {
        "links": {
          "related": "/api/v2/registry-modules/mod-fZn7uHu99ZCpAKZJ/tag-bindings"
        }
      }
    },
    "links": {
...
| `data.attributes.test-config.agent-execution-mode` | string  | (previous value) | The execution mode for test runs. Must be either `remote` or `agent`. When set to `agent`, tests will run on agents in the specified agent pool. Only available when the enterprise-testing feature is enabled.                       |
| `data.attributes.test-config.agent-pool-id`        | string  | (previous value) | The ID of the agent pool to use for test runs. Required when `agent-execution-mode` is set to `agent`. Must not be set when `agent-execution-mode` is `remote`. Only available when the enterprise-testing feature is enabled.        |
...
```json
{
  "data": {
    "attributes": {
      ...
      "test-config": {
        "tests-enabled": true,
        "agent-execution-mode": "agent",
        "agent-pool-id": "apool-yoGUFz5zcRMMz53i"
      }
    },
    "type": "registry-modules"
  }
...
For public modules, only the endpoint specifying the module namespace and name is valid. The other DELETE endpoints will 404.
...
## List All Tags for a Registry Module

[](#list-tags-for-a-registry-module)

Lists all tags assigned to a registry module.

`GET /api/v2/registry-modules/:module-id/tag-bindings`

| Parameter | Description |
|---|---|
| `:module-id` | The id of the module to be listed. |
...
| [404](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/404) | [JSON API error object](http://jsonapi.org/format/#error-objects) | Module not found |
...
curl \
  --request GET \
  --header "Authorization: Bearer $TOKEN" \
  https://app.terraform.io/api/v2/registry-modules/mod-fZn7uHu99ZCpAKZJ/tag-bindings
```
...
```json
{
  "data": [
    {
      "id": "tb-yPdfdsfWQryyJ7X",
      "type": "tag-bindings",
      "attributes": {
        "key": "env",
        "value": "staging",
        "created-at": "2026-06-11T01:39:16.287Z"
      }
    },
    {
      "id": "tb-VebM7JLKPIynQ5Qx",
      "type": "tag-bindings",
      "attributes": {
        "key": "DEPT",
        "value": "ENGINEERING",
        "created-at": "2026-06-11T01:45:23.923Z"
      }
    },
    {
      "id": "tb-BN9cq80ofmJKL8973o",
      "type": "tag-bindings",
      "attributes": {
        "key": "billingID",
        "value": "1234",
        "created-at": "2026-06-09T23:20:06.883Z"
      }
    }
  ],
  "links": {
    "target": "/api/v2/organizations/my-organization/registry-modules/private/my-organization/my-module/aws"
  }
}
```

## Add or Update Tags to a Registry Module

[](#add-tags-to-a-registry-module)

Adds or updates tags for a registry module in your organization's registry.
-> **Note:** Removing tags from a registry module is made via PATCH requests, where the request body will contain only the tag key and values that are currently needed.

`PATCH /api/v2/organizations/:organization-name/registry-modules/:registry-name/:namespace/:module-name/:provider`

| Parameter | Description |
|---|---|
| `:organization_name` | The name of the organization that owns the registry module. |
| `:registry_name` | The name of the registry. For private modules, this should be `private` and for publicly curated modules, this should be `public`. |
...
| Status | Response | Reason                                      |
|---|---|---------------------------------------------|
| [200](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/200) | [JSON API document](/hashicorp/web-unified-docs/blob/main/terraform/cloud-docs/api-docs#json-api-documents) | Successfully added/updated the tag binding  |
...
| [422](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status/422) | [JSON API error object](http://jsonapi.org/format/#error-objects) | Unprocessable content - Tag key or value have a wrong format|
...
curl \
  ...
  --request PATCH \
  --data @payload.json \
  https://app.terraform.io/api/v2/organizations/my-organization/registry-modules/public/terraform-aws-modules/iam/aws
```
...
```json
{
  "data": {
    ...
    "attributes": {
      ...
      "provider": "aws",
      "version-statuses": [],
      "created-at": "2026-02-24T02:29:29.881Z",
      "updated-at": "2026-02-24T02:29:29.892Z",
      "registry-name": "public",
      "version-tags": [],
      "permissions": {
        "can-delete": true,
    ...
    "relationships": {
      ...
      "tag-bindings": {
        "links": {
          "related": "/api/v2/registry-modules/mod-e2EX43902FYe/tag-bindings"
        }
      }
    },
    "links": {
      "self": "/api/v2/organizations/organization-name/registry-modules/public/terraform-aws-modules/iam/aws"
    }
  }
}
```
<!-- BEGIN: TFC:only -->
## Versions List for a Registry Module

[](#versions-list-for-a-registry-module)

This endpoint returns all the published versions for a registry module with its tags' information.

`GET api/v2/organizations/:organization-name/registry-modules/:registry-name/:namespace/:module-name/:provider-name/versions`
...
curl \
  --request GET \
  --header "Authorization: Bearer $TOKEN" \
  https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions
```
...
```json
{
  ...
  "links": {
    "self": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "first": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "prev": null,
    "next": null,
    "last": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?page%5Bnumber%5D=1&page%5Bsize%5D=20"
  },
  "meta": {
    "pagination": {
      "current-page": 1,
...
The following request is very similar to the previous one, but it filters Module Versions where the key is `environment` and the value is `staging`.

```shell
$ curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?page[number]=1&page[size]=20&filter[tagged][0][key]=environment&filter[tagged][0][value]=staging"
```
...
```json
{
  ...
  "links": {
    "self": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?filter%5Btagged%5D%5B0%5D%5Bkey%5D=environment&filter%5Btagged%5D%5B0%5D%5Bvalue%5D=staging&page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "first": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?filter%5Btagged%5D%5B0%5D%5Bkey%5D=environment&filter%5Btagged%5D%5B0%5D%5Bvalue%5D=staging&page%5Bnumber%5D=1&page%5Bsize%5D=20",
    "prev": null,
    "next": null,
    "last": "https://app.terraform.io/api/v2/organizations/hashicorp/registry-modules/private/hashicorp/test-module/terraform/versions?filter%5Btagged%5D%5B0%5D%5Bkey%5D=environment&filter%5Btagged%5D%5B0%5D%5Bvalue%5D=staging&page%5Bnumber%5D=1&page%5Bsize%5D=20"
  },
  "meta": {
    "pagination": {
      "current-page": 1,
