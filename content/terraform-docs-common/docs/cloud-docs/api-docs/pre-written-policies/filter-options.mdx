...
description: >-
  Use the HCP Terraform API's `/ootb-policies/filter-options` endpoint to retrieve the available filter values for the HashiCorp managed pre-written policy catalog, including providers, governed resource types, compliance framework libraries, and services.
tfc_only: true
...
# Policy filter options API reference
...
## Get pre-written policy filter options

`GET /ootb-policies/filter-options`

Returns the set of values available for filtering the HashiCorp managed pre-written policy catalog: providers, governed resource types, libraries (compliance frameworks), and services. Each library slug is a valid value for the collection endpoint's `filter[library]` parameter. Optionally scoped to a single policy kind via `filter[kind]`.
