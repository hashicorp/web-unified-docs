...
GCP OIDC configurations are models that represent the necessary information for HCP Terraform to request the use of your Google Cloud Platform (GCP) key for Hold Your Own Key (HYOK) functionality.

Hold Your Own Key (HYOK) is only available on the [HCP Terraform Premium plan](https://www.hashicorp.com/en/pricing?tab=terraform).
This includes the management of OIDC configurations. To learn more about hold your own key, refer to the feature [Overview](/terraform/cloud-docs/hold-your-own-key).
