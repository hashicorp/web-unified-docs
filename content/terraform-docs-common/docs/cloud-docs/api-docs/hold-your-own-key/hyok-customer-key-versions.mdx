...
An HYOK customer key version represents a specific version of your key management service key.
While an HYOK configuration is associated with a specific key, that key can have multiple versions.
This allows for rotation and management of keys in your key management service without needing to create a new HYOK configuration.
