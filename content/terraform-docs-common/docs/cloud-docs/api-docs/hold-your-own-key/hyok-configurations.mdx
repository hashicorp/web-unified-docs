...
| `kms-options`                    | Optional object used to specify [additional fields](/terraform/cloud-docs/api-docs/hold-your-own-key/hyok-configurations#kms-options) for some key management services. Fields include `key_region`, `key_location`, and `key_ring_id`.  |
...
| `status`                         | Indicates a HYOK configuration [status](/terraform/cloud-docs/api-docs/hold-your-own-key/hyok-configurations#hyok-configuration-status). This status can be `active`, `testing`, `test_failed`, `available`, `revoking`, or `revoked`. |
