...
AWS OIDC Configurations are models that represent the necessary information for HCP Terraform to request the use of your AWS Key Management Service keys for Hold Your Own Key (HYOK) functionality.
This includes the AWS role ARN that HCP Terraform will assume in order to encrypt a data encryption key used within HCP Terraform.
