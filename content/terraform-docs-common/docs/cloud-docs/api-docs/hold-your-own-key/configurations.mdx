...
last_modified: 2026-09-17T18:17:19.000Z
...
| `kms-options`                    | Optional object used to specify additional fields for some key management services. Fields include `key-region`, `multi-region`, `key-location`, `key-ring-id`, and `transit-path`. See table below for more details.  |
...
| Option | Description |
| --- | --- |
| `key-region` | The AWS region where your key is located. Required for AWS. When `multi-region` is `true`, the agent resolves the region at runtime and uses this value as a fallback. |
...
| `key-location` | The Google Cloud location that contains your key ring, such as `global` or `us-central1`. Required for GCP. |
| `key-ring-id` | The name of the Google Cloud KMS key ring that contains your key. A key ring is the root resource for Google Cloud KMS keys and key versions. Required for GCP. |
| `transit-path` | The path where Vault's transit secrets engine is mounted. Only supported for Vault. Defaults to `transit`. |

Setting an option on a key management service that does not support it returns a `422` response. For example, setting `transit-path` on an AWS configuration fails.
...
| `data.attributes.kms-options`        | object  | `{}`      | Additional options for your key management service, such as `transit-path` for Vault or `key-region` for AWS. Refer to [KMS options](#kms-options).                                                                                                                                          |
