...
| `kms-options`                    | Optional object used to specify [additional fields](/terraform/cloud-docs/api-docs/hyok-configurations#kms-options) for AWS KMS. Fields include `key_region`, `key_location`, and `key_ring_id`.  |
...
| `revoked`  | Indicates that the HYOK configuration was successfully revoked. If you wish to stop using a key or a specific key version, you may revoke it within HCP Terraform. |
| `revoking`  | Indicates that the HYOK configuration is in the process of being revoked. |

### KMS Options
The HYOK KMS Options are found in `data.attributes.kms-options`. The following list describes the possible options for KMS options.
