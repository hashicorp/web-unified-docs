...
| `kek-id`                         | Reference to                                                                                                 |
| `kms-options`                    | Optional object used to specify [additional fields](/terraform/enterprise/api-docs/state-versions#kms-options) for some key management services. Fields include `key_region`, `key_location`, and `key_ring_id`.  |
...
| `status`                         | Indicates a HYOK configuration [status](/terraform/enterprise/api-docs/state-versions#hyok-configuration-status). This status can be `active`, `testing`, `test_failed`, `available`, `revoking`, or `revoked`. |
...
`POST /workspaces/:workspace_id/hyok-configurations`
...
curl \
  ...
  --request POST \
  --data @payload.json \
  https://app.terraform.io/api/v2/workspaces/ws-6fHMCom98SDXSQUv/hyok-configurations
```
...
```json
{
    "data": {
        ...
        "links": {
            "self": "/api/v2/hyok-configurations/sv-DmoXecHePnNznaA4"
        }
    }
}
```
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/hyok-configurations?filter%5Bworkspace%5D%5Bname%5D=my-workspace&filter%5Borganization%5D%5Bname%5D=my-organization"
```
    ...
    "data": [
        {
            ...
            "links": {
                "self": "/api/v2/hyok-configurations/sv-g4rqST72reoHMM5a"
            }
        },
        {
            ...
            "links": {
                "self": "/api/v2/hyok-configurations/sv-QYKf6GvNv75ZPTBr"
            }
        }
    ],
    "links": {
        "self": "https://app.terraform.io/api/v2/hyok-configurations?filter%5Borganization%5D%5Bname%5D=hashicorp&filter%5Bworkspace%5D%5Bname%5D=my-workspace&page%5Bnumber%5D=1&page%5Bsize%5D=20",
        "first": "https://app.terraform.io/api/v2/hyok-configurations?filter%5Borganization%5D%5Bname%5D=hashicorp&filter%5Bworkspace%5D%5Bname%5D=my-workspace&page%5Bnumber%5D=1&page%5Bsize%5D=20",
        "prev": null,
        "next": null,
        "last": "https://app.terraform.io.io/api/v2/hyok-configurations?filter%5Borganization%5D%5Bname%5D=hashicorp&filter%5Bworkspace%5D%5Bname%5D=my-workspace&page%5Bnumber%5D=1&page%5Bsize%5D=20"
    },
    "meta": {
        "pagination": {
            "current-page": 1,
...
`GET /hyok-configurations/:state_version_id`
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/hyok-configurations/sv-SDboVZC8TCxXEneJ
```
...
`PATCH /workspaces/:workspace_id/hyok-configurations`
...
`POST /api/v2/hyok-configurations/:state_version_id/actions/soft_delete_backing_data`
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request POST \
  https://app.terraform.io/api/v2/hyok-configurations/sv-ntv3HbhJqvFzamy7/actions/soft_delete_backing_data
...
`POST /api/v2/hyok-configurations/:state_version_id/actions/restore_backing_data`
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request POST \
  https://app.terraform.io/api/v2/hyok-configurations/sv-ntv3HbhJqvFzamy7/actions/restore_backing_data
```
...
`POST /api/v2/hyok-configurations/:state_version_id/actions/permanently_delete_backing_data`
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request POST \
  https://app.terraform.io/api/v2/hyok-configurations/sv-ntv3HbhJqvFzamy7/actions/permanently_delete_backing_data
```
