...
Terraform actions are provider-defined operations that HCP Terraform can invoke during runs. Use these endpoints to list the actions available in a workspace and review action invocations after a run. Refer to [Terraform actions](/terraform/plugin/framework/actions) for more information about defining actions in providers.
...
```json
{
  "data": [
    {
      "id": "act-ryh1uSaExmQqK72V",
      "type": "actions",
      "attributes": {
        "address": "terraform_data.example.action.restart",
...
## List Action Providers in a Workspace

`GET /workspaces/:workspace_id/terraform-actions/providers`

| Parameter       | Description                                                                                                                                                                                                                    |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `:workspace_id` | The ID of the workspace to list action providers from. Obtain this from the [workspace settings](/terraform/cloud-docs/workspaces/settings) or the [show workspace](/terraform/cloud-docs/api-docs/workspaces#show-workspace) endpoint. |

| Status  | Response                                           | Reason                                                      |
| ------- | -------------------------------------------------- | ----------------------------------------------------------- |
| [200][] | [JSON API document][] (`type: "action-provider"`) | Request was successful.                                     |
| [404][] | [JSON API error object][]                          | Workspace not found or user unauthorized to perform action. |
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/workspaces/ws-4j8p6jX1w33MiDC7/terraform-actions/providers
```
...
```json
{
  "data": [
    {
      "id": "terraform.io/builtin/terraform",
      "type": "action-provider",
      "attributes": {
        "name": "terraform.io/builtin/terraform"
      }
    }
  ]
}
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/terraform-actions/act-ryh1uSaExmQqK72V
```
...
This endpoint lists action invocations for an action, grouped and aggregated by run.
...
| `filter[invoked_by]`                    | **Optional.** Return run summaries for one or more comma-separated invoker values. Use the [invoked-by values endpoint](#list-action-invoked-by-values) to discover values. |
| `filter[source]`                        | **Optional.** Return run summaries for one or more comma-separated run sources. Valid values are `tfe-api`, `terraform`, `terraform+cloud`, `tfe-configuration-version`, and `tfe-ui`. |
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/terraform-actions/act-ryh1uSaExmQqK72V/invocations/run-summary?filter%5Binvocation_method%5D=direct&page%5Bsize%5D=20"
```
...
```json
{
  "data": [
    {
      "id": "act-ryh1uSaExmQqK72V:run-KTuq99JSzgmDSvYj",
...
## List Action Invoked By Values

`GET /terraform-actions/:action_id/invocations/invoked-by`

This endpoint lists distinct `invoked_by` filter values available for an action. The response can include `system:not_a_user`, which represents invocations without a human actor.
...
| Status  | Response                                            | Reason                                                   |
| ------- | --------------------------------------------------- | -------------------------------------------------------- |
| [200][] | [JSON API document][] (`type: "action-invoked-by"`) | Request was successful.                                  |
| [404][] | [JSON API error object][]                           | Action not found or user unauthorized to perform action. |
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://app.terraform.io/api/v2/terraform-actions/act-ryh1uSaExmQqK72V/invocations/invoked-by
```
...
```json
{
  "data": [
    {
      "id": "example-user",
      "type": "action-invoked-by",
      "attributes": {
        "name": "example-user"
      }
    },
    {
      "id": "system:not_a_user",
      "type": "action-invoked-by",
      "attributes": {
        "name": "system:not_a_user"
      }
    }
  ]
}
...
This endpoint lists individual action invocations for a specific action and run, ordered by execution order.
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/terraform-actions/act-ryh1uSaExmQqK72V/runs/run-KTuq99JSzgmDSvYj/invocations?filter%5Bstatus%5D=successful&page%5Bsize%5D=20"
```
...
This endpoint lists lifecycle action invocations for a specific action and run, grouped by triggering resource address. The endpoint paginates by resource group, not by individual invocation.
...
curl \
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  "https://app.terraform.io/api/v2/terraform-actions/act-ryh1uSaExmQqK72V/runs/run-KTuq99JSzgmDSvYj/invocations/lifecycle-resource-groups?filter%5Baggregate_status%5D=successful&page%5Bsize%5D=20"
```
