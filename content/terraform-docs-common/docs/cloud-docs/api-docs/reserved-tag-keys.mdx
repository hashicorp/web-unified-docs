...
last_modified: 2026-07-14T18:37:53.000Z
...
Each reserved tag key includes an `is-registry-tag` attribute. This read-only boolean indicates whether the reserved tag key is related to a registry artifacts, such as modules and providers. When `true`, the reserved tag key applies to registry artifacts.
### Sample payload
