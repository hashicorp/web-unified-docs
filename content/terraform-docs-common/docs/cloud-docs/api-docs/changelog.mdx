...
- Notification configurations support the following granular run triggers: `run:pending_apply_approval`, `run:cost_estimated`, `run:policy_override_required`, `run:policies_checked`, `run:run_errored`, `run:run_canceled`, and `run:policy_soft_failed`. Generic notification payloads report the subscribed trigger in the `trigger` field. The `run:needs_attention` and `run:errored` triggers are deprecated, but the API continues to accept them for backward compatibility. Refer to the [workspace notification configurations API reference](/terraform/cloud-docs/api-docs/notification-configurations/workspace#notification-triggers) for more information.
