---
page_title: API Changelog for HCP Terraform
page_id: api-changelog
description: >-
  Use this log of HCP Terraform API changes to track new features and evolving functionality over time.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2025-05-27T14:28:51-04:00
last_modified: 2026-04-24T15:19:47.000Z
...
- Fix the API path for listing Stack deployment runs in a group.
- Add Stack deployment endpoints for starting and cancelling a Stack deployment group.
- Add missing `q`, `sort` and `filter[status]` query params to the endpoint listing all Stack deployment groups.
- Add missing status responses to endpoint that reruns a failed Stack deployment group.
- Add missing `q` and `sort` query params to the endpoint listing all Stack deployments.
- Add missing `q`, `sort`, `filter['latest_only']` and `filter[status]` query params to the endpoint listing all Stack deployment runs in a configuration.
- Add missing `q`, `sort`, `filter['latest_only']` and `filter[status]` query params and `current_step` related resource to the endpoint listing all Stack deployment runs in a group.
- Add missing `current_step` related resource to the endpoint showing Stack deployment run details.
- Update status code returned when a Stack deployment run is successfully canceled the Stack deployment run.
- Add missing `sort` query param to endpoint listing Stack deployment steps a run.

## 2026-09-22

- Add the `sort` query parameter to the [list agent tokens endpoint](/terraform/cloud-docs/api-docs/agent-tokens#list-agent-tokens) to sort tokens by creation time.

## 2026-08-10

- [Stack configuration summary](/terraform/cloud-docs/api-docs/stacks#list-stack-configuration-summaries) records now include a summary of run statuses in addition to group statuses.
- The main inner attribute for [Stack deployment group summary](/terraform/cloud-docs/api-docs/stacks/configurations#list-deployment-group-summaries) records has been renamed to `stack-deployment-run-status-summary`. The new name matches their configuration summaries attributes. This attribute name replaces the deprecated `status-counts` alias.
- The Stack configuration endpoints for listing and showing data can now sideload configuration summaries with `?include=stack_configuration_summary`.
- The Stack deployment endpoints for listing groups and showing a group can sideload group summaries with `?include=stack_deployment_group_summary`.

## 2026-07-28

- Add the [Task configurations API reference](/terraform/cloud-docs/api-docs/tasks/task-configs),
which documents endpoints for adding configurations to [Tasks](/terraform/cloud-docs/api-docs/tasks/tasks).

## 2026-05-13

- Add the [Terraform actions API reference](/terraform/cloud-docs/api-docs/terraform-actions), which documents endpoints for listing Terraform actions, retrieving action filter options, and inspecting action invocation history by run.

## 2026-05-14

- The `concurrency-override` attribute has been removed from the [feature sets API](/terraform/cloud-docs/api-docs/feature-sets) response.

<!-- BEGIN: TFC:only name:metrics-service-tokens -->

## 2026-04-24

- Add API documentation for the new [Metrics Service tokens](/terraform/cloud-docs/api-docs/metrics-service-tokens).

<!-- END: TFC:only name:metrics-service-tokens -->
...
## 2025-12-18
* Document the new `/registry-components` endpoints that let you read, publish, update, and delete Stack component configurations in your organization's private registry. For more details, refer to the [Stack component configurations API reference](/terraform/cloud-docs/api-docs/private-registry/stack-components).
...
* Update existing [agent pools API documentation](/terraform/cloud-docs/api-docs/agents) to include new agent pool scoping options, including adding allowed projects and adding exclusions for workspaces.
...
* You can now revoke, and revert the revocation of, module versions. Learn more about [Managing module versions](/terraform/cloud-docs/api-docs/private-registry/modules/versions).
...
## 2024-10-15
* Add new documentation for the ability to deprecate, and revert the deprecation of, module versions. Learn more about [Managing module versions](/terraform/cloud-docs/api-docs/private-registry/modules/versions).
...
* Add support for [streamlined run task reviews](/terraform/cloud-docs/integrations/run-tasks), enabling run task integrations to return high fidelity results.
  ...
  * Updated the run task payload [JSON schema](https://github.com/hashicorp/web-unified-docs/blob/main/content/terraform-docs-common/public/schema/run-tasks/runtask-result.json).
