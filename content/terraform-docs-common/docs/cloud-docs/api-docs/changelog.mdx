...
- [Stack deployment group summary](/terraform/cloud-docs/api-docs/stacks/configurations#list-deployment-group-summaries) records now use the same name for their main inner attribute as the configuration summaries do (`stack-deployment-run-status-summary`). This is wordier than the (now deprecated) `status-counts` alias, but is pleasantly unambiguous.
- The Stack configuration list and show endpoints can sideload configuration summaries with `?include=stack_configuration_summary`.
