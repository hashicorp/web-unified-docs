...
HCP Terraform workspaces that have enabled hold your own key (HYOK) encryption create sanitized plans that omit sensitive encrypted data. You can download the sanitized version of the plan file with your sensitive information redacted. Learn more about [hold your own key encryption](/terraform/cloud-docs/encryption/hold-your-own-key).
