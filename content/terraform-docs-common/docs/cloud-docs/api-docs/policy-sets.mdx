---
page_title: /policy-sets API reference for HCP Terraform
description: >-
  Use the HCP Terraform API's `/policy-sets` endpoint to read, create, delete, update and version Sentinel and OPA policy sets. Also, attach, exclude, and detach policy sets to workspaces and projects.
...
[Policy Enforcement](/terraform/cloud-docs/policy-enforcement) lets you use the policy-as-code frameworks Terraform policy, Sentinel, and Open Policy Agent (OPA) to apply policy checks to HCP Terraform workspaces.
...
| `data.attributes.kind`                                | string         | `sentinel` | The policy-as-code framework associated with the policy. Valid values are `tfpolicy`, `sentinel`, and `opa`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
### Sample payload with HashiCorp managed pre-written policies (OOTB)
...
### Sample response with HashiCorp managed pre-written policies (OOTB)

```json
{
  "data": {
    "id": "polset-xxxxxxxxxxxxxxxx",
    "type": "policy-sets",
    "relationships": {
      "policies": {
        "data": [{ "id": "pol-yyyyyyyyyyyyyyyy", "type": "policies" }]
      }
    }
  }
}
...
Use [`GET /api/v2/policy-sets/<returned-id>?include=policies`](/terraform/cloud-docs/api-docs/policy-sets#show-a-policy-set) to retrieve the policy details.
