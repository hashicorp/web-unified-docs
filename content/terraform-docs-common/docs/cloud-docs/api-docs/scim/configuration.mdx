...
This endpoint sets `enabled` to `false` on the organization's SCIM configuration, which temporarily pauses SCIM provisioning. Requests to the SCIM provisioning endpoints for the organization return an error while SCIM is disabled.
