...
In organizations in HCP Europe, you use HCP [groups](https://docs.hashicorp.com/hcp/docs/hcp/iam/groups) to manage permissions instead of teams. All of the `team-workspace` API endpoints function with groups as well. The API responses will return information about groups and their permissions on workspaces.
