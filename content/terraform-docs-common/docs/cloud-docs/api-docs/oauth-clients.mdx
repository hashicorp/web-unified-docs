---
page_title: /oauth-clients API reference for HCP Terraform
description: >-
  Use the HCP Terraform API's `/oauth-clients` endpoint to manage connections between VCS providers and organizations and projects. Learn how to read, create, update, and destroy clients.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2025-05-27T14:28:51-04:00
last_modified: 2025-11-05T14:01:41+05:30
...
| `data.attributes.service-provider`   | string |         | The VCS provider being connected with. Valid options are `"github"`, `"github_enterprise"`, `"gitlab_hosted"`, `"gitlab_community_edition"`, `"gitlab_enterprise_edition"`, or `"ado_server"`. Use `"github"` for GitHub Enterprise Cloud, because it is hosted at `github.com`. Only use `"github_enterprise"` for a self-hosted GitHub Enterprise Server instance with its own hostname. |
...
| `data.attributes.ado-org-name`       | string | `null`  | **Only used for Azure DevOps Services connections that authenticate with an org-scoped personal access token (PAT).** The Azure DevOps organization name (for example, `my-company` from `https://dev.azure.com/my-company`). The value must start and end with a letter or number and may contain only letters, numbers, and hyphens. Required when using an org-scoped PAT; omit or set to `null` when using a globally-scoped PAT. |
...
For GitHub Enterprise Cloud, set `service-provider` to `"github"` with `http-url` of `"https://github.com"` and `api-url` of `"https://api.github.com"`, because GitHub Enterprise Cloud is hosted at `github.com`. The `"github_enterprise"` option is reserved for self-hosted GitHub Enterprise Server and rejects URLs that point to `github.com`.
...
| `data.attributes.ado-org-name`   | string | (previous value) |  The Azure DevOps organization name, for example, `my-company` from `https://dev.azure.com/my-company`. Set this value when migrating a connection to an org-scoped PAT. The value must start and end with a letter or number and may contain only letters, numbers, and hyphens. Omit or set to `null` when using a globally-scoped PAT. This value is only used for Azure DevOps Services connections that authenticate with an organization-scoped personal access token (PAT).  |
