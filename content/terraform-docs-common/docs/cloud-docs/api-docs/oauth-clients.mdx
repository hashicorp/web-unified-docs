...
| `data.attributes.ado-org-name`   | string | (previous value) | **Only used for Azure DevOps Services connections that authenticate with an org-scoped personal access token (PAT).** The Azure DevOps organization name (for example, `my-company` from `https://dev.azure.com/my-company`). Set this value when migrating a connection to an org-scoped PAT. The value must start and end with a letter or number and may contain only letters, numbers, and hyphens. Omit or set to `null` when using a globally-scoped PAT. |
