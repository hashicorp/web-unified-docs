...
| `data.attributes.stacks-default-execution-mode` | string | `remote` | Specifies which [execution mode](/terraform/cloud-docs/stacks/configure#change-execution-mode) to use by default for stacks in this project. Valid values are `remote`, and `agent`. |
| `data.attributes.default-agent-pool` | string | (previous value) | Required when `default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the project. Do _not_ specify this value if you set `execution-mode` to `remote` or `local`. |
| `data.attributes.stacks-default-agent-pool` | string | (previous value) | Required when `stacks-default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the project. Do _not_ specify this value if you set `execution-mode` to `remote`. |
...
| `data.attributes.stacks-default-agent-pool` | string | (previous value) | Required when `stacks-default-execution-mode` is set to `agent`. The ID of the agent pool belonging to the project. Don't specify this value if you set `execution-mode` to `remote`. |
| `data.attributes.setting-overwrites` | object |      |  The keys in this object are attributes that have project-level defaults. Each attribute key stores a boolean value which is `false` by default. To overwrite the default inherited value, set an attribute's value to `true`. For example, to unset `execution-mode` as the project-level default, you set `setting-overwrites.execution-mode = true`.  |
