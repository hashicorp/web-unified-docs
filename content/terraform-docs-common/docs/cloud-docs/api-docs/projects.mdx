...
| `data.attributes.default-execution-mode` | boolean | `remote` | Specifies which [execution mode](/terraform/cloud-docs/workspaces/settings#execution-mode) to use by default. Valid values are `remote`, `local`, and `agent`. |
