...
created_at: 2026-02-17T14:53:31-08:00
last_modified: 2026-08-31T22:02:46.000Z
# END AUTO GENERATED METADATA
---
...
- **`all_agent_pools`**: Applies to all agent pools in the organization. You can set `enforcement-scope` to `all_agent_pools` for multiple IP allowlists per organization.
- **`selected_agent_pools`**: Applies only to specifically assigned agent pools. You can set `enforcement-scope` to `selected_agent_pools` for multiple IP allowlists per organization.
...
| `data.attributes.enforcement-scope` | string |                          | The scope of enforcement for this IP allowlist. Must be one of the following values: <ul><li>`"organization"`</li><li>`"all_agent_pools"`</li><li>`"selected_agent_pools"`</li></ul> Only one `"organization"` is allowed per organization. |
  ...
  "data": [
    {
      ...
      "attributes": {
        ...
        "enabled": true,
        "updated-at": "2025-05-27T14:28:51Z"
      },
      "links": {
...
| `data.attributes.range`        | string  |         | Specifies the CIDR range. Must be a valid IPv4 CIDR range, for example `"192.168.1.0/24"`.  |
| `data.attributes.enabled`      | boolean |         | Whether the CIDR range is enforced.                             |
| `data.attributes.description`  | string  | none    | A description of the CIDR range.                                |
...
| ----------------------------- | ------- | ---------------- | --------------------------------------------------------------------------- |
| `data.type`                   | string  |                  | Must be `"cidr-ranges"`.                                                    |
| `data.attributes.range`       | string  | (previous value) | The CIDR range. If omitted, the existing range is preserved. Must be a valid IPv4 CIDR range. |
| `data.attributes.enabled`     | boolean | (previous value) | Whether the CIDR range is enforced. If omitted, the existing value is preserved. |
| `data.attributes.description` | string  | (previous value) | A description of the CIDR range. If omitted, the existing description is preserved. |
...
This endpoint associates agent pools with an IP allowlist, restricting which IP addresses can access those agent pools. You can call this endpoint only when the `enforcement-scope` attribute for IP allowlist is set to  `"selected_agent_pools"`.
...
| Key path        | Type   | Default | Description                                          |
| --------------- | ------ | ------- | ---------------------------------------------------- |
| `data`          | array  |         | A list of the agent pools to assign to the IP allowlist. |
| `data[].type`   | string |         | Must be `"agent-pools"`.                             |
| `data[].id`     | string |         | The ID of the agent pool to assign.                  |
...
| Key path        | Type   | Default | Description                                              |
| --------------- | ------ | ------- | ------------------------------------------------------- |
| `data`          | array  |         | A list of the agent pools to unassign from the IP allowlist. |
| `data[].type`   | string |         | Must be `"agent-pools"`.                                 |
| `data[].id`     | string |         | The ID of the agent pool to unassign.                    |
