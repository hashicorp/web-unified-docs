...
This endpoint associates agent pools with an IP allowlist, restricting which IP addresses can access those agent pools. This endpoint applies only to IP allowlists whose `enforcement-scope` is `"selected_agent_pools"`.
