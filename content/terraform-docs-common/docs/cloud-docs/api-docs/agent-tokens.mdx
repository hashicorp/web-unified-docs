...
| [400][] | [JSON API error object][]                               | Invalid `sort` parameter                                      |
...
| `sort`        | **Optional.** Sorts tokens by creation time. Valid values are `"created-at"` (oldest first) and `"-created-at"` (newest first). If omitted, the endpoint does not apply an explicit sort order. |

When you specify `sort`, tokens with the same creation time are sorted by ID in the same direction.
...
To list the newest tokens first, set `sort` to `-created-at`:
...
curl \
  ...
  --header "Content-Type: application/vnd.api+json" \
  --request GET \
  "https://app.terraform.io/api/v2/agent-pools/apool-MCf6kkxu5FyHbqhd/authentication-tokens?sort=-created-at"
```
