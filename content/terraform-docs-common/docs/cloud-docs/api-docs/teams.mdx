...
In HCP Europe organizations, you use HCP groups to manage permissions instead of teams. If your URL includes `portal.cloud.eu.hashicorp` or `app.eu.terraform.io`, then you are in an HCP Europe organization.

To learn how to create and delete groups in HCP, refer to [Create a group](/hcp/docs/hcp/iam/groups#create-a-group). To learn how to assign groups permissions in HCP Terraform, refer to the [Member roles API](/terraform/cloud-docs/api-docs/member-roles).
