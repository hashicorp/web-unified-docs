...
~> **Important:** Starting November 10, 2026, this endpoint will no longer return plaintext values for sensitive outputs. The `value` field will be `null` for any output where `sensitive` is `true`, matching the behavior of [Show Current State Version Outputs for a Workspace](#show-current-state-version-outputs-for-a-workspace). To retrieve the value of a sensitive output, use [Show a State Version Output](#show-a-state-version-output).
