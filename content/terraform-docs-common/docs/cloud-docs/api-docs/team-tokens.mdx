...
description: >-
  Use the HCP Terraform API's `/teams/:team_id/authentication-tokens` endpoint to generate, delete, list, and bulk-delete a team's API tokens.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2025-05-27T14:28:51-04:00
last_modified: 2026-08-28T06:01:57.000Z
...
Teams relying on the [legacy `/teams/:team_id/authentication-token`](/terraform/cloud-docs/api-docs/team-tokens#legacy-team-tokens-api-reference) endpoint can only create a single valid token at a time. Generating a new legacy token when a team token already exists revokes the existing legacy token and replaces it with a new team token.
...
| `data.attributes.expired-at`  | string | `null`  | The UTC date and time that the team token will expire in ISO 8601 format. If omitted or set to `null`, the token will expire two years from the date of creation. |
...
## Delete team tokens in bulk

Deletes one or more team tokens from an organization in a single request.

| Method | Path                                          |
| :----- | :-------------------------------------------- |
| DELETE | /organizations/:organization_name/team-tokens |

| Status  | Response                  | Reason                                                          |
| ------- | ------------------------- | --------------------------------------------------------------- |
| [204][] | No Content                | Successfully deleted the specified team tokens.                 |
| [403][] | [JSON API error object][] | Forbidden. The user is not authorized to delete team tokens.    |
| [404][] | [JSON API error object][] | Organization not found.                                         |

### Parameters

- `:organization_name` (`string: <required>`) - specifies the name of the organization containing the team tokens to delete

### Request body

The `DELETE` endpoint requires a JSON object with the following properties as a request payload.

| Key path      | Type   | Default | Description                         |
| ------------- | ------ | ------- | ----------------------------------- |
| `data[].type` | string |         | Must be `"authentication-tokens"`.  |
| `data[].id`   | string |         | The ID of the team token to delete. |
...
curl \
  ...
  --request DELETE \
  --data @payload.json \
  https://app.terraform.io/api/v2/organizations/hashicorp/team-tokens
```
...
| `page[size]`      | If omitted, the endpoint returns 20 tokens per page.                                                                                                                                                                          | Optional |
| `q`               | A search query string. You can search for a team authentication token using the team name.                                                                                                                                    | Optional |
| `sort`            | Allows you to sort the team tokens by `"created-by"`, `"expired-at"`, and `"last-used-at"`. Prepending a hyphen to the sort parameter reverses the order. For example, `"-expired-at"` sorts by expiration date in descending order. If omitted, the default sort order is ascending. | Optional |
| `filter[status]`  | A comma-separated list of token statuses. The result lists tokens that match one of the statuses you specify. Valid values are `active`, `expiring_in_7_days`, `expiring_in_30_days`, `expired`, and `no_expiration`.         | Optional |
...
| `data.attributes.expired-at`  | string | `null`  | The UTC date and time that the team token will expire in ISO 8601 format. If omitted or set to `null`, the token expires in two years from the date and time of creation. |
...
| `page[size]`     | If omitted, the endpoint returns 20 tokens per page.                                                                                                                                                                                                                                                     | Optional |
| `q`              | A search query string. You can search for a team authentication token using the team name.                                                                                                                                                                                                               | Optional |
| `sort`           | Allows you to sort the team tokens by `"team-name"`, `"created-by"`, `"expired-at"`, and `"last-used-at"`. Prepending a hyphen to the sort parameter reverses the order. For example, `"-team-name"` sorts by name in reverse alphabetical order. If omitted, the default sort order is ascending.            | Optional |
| `filter[status]` | A comma-separated list of token statuses. The result lists tokens that match one of the statuses you specify. Valid values are `active`, `expiring_in_7_days`, `expiring_in_30_days`, `expired`, and `no_expiration`.                                                                                    | Optional |
