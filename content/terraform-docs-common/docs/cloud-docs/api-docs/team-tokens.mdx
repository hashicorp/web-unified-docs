...
last_modified: 2026-07-29T13:53:20.000Z
...
| `sort`            | Allows sorting the team tokens by `"created-by"`, `"expired-at"`, and `"last-used-at"`. Prepending a hyphen to the sort parameter reverses the order. For example, `"-expired-at"` sorts by expiration date in descending order. If omitted, the default sort order is ascending. | Optional |
...
| `page[size]`     | If omitted, the endpoint returns 20 tokens per page.                                                                                                                                                                                                                                                     | Optional |
| `q`              | A search query string. You can search for a team authentication token using the team name.                                                                                                                                                                                                               | Optional |
| `sort`           | Allows sorting the team tokens by `"team-name"`, `"created-by"`, `"expired-at"`, and `"last-used-at"`. Prepending a hyphen to the sort parameter reverses the order. For example, `"-team-name"` sorts by name in reverse alphabetical order. If omitted, the default sort order is ascending.            | Optional |
| `filter[status]` | A comma-separated list of token statuses. The result lists tokens that match one of the statuses you specify. Valid values are `active`, `expiring_in_7_days`, `expiring_in_30_days`, `expired`, and `no_expiration`.                                                                                    | Optional |
