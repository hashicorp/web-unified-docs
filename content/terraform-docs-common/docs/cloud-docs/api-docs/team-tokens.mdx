...
For a HCP Europe organizations, team tokes APIs can also be used for [groups](https://docs.hashicorp.com/hcp/docs/hcp/iam/groups). The team_id parameter should be replaced with a group_id for these organizations, and API responses will return group-related information. Additionally, the manage group API tokens permission is managed on the [HCP](https://portal.cloud.hashicorp.com).
