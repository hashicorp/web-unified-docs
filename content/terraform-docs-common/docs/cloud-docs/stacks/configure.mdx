...
1. Update any of the following settings:
   ...
   - Version control settings, including:
        ...
        - With the branch-based workflow, you can configure **Trigger patterns** to control which file changes trigger new runs. With the tag-based workflow, you can configure a **Custom regular expression** to control which tags trigger new runs.
...
The execution mode determines the Terraform execution platform for this Stack. You can choose one of the following modes:

- **Project Default**: The Stack inherits the project's [default execution mode](/terraform/cloud-docs/projects#execution-mode). This is the default setting.

- **Remote (custom)**: The Stack runs on disposable virtual machines launched by HCP Terraform or Terraform Enterprise.

- **Agent (custom)**: The Stack runs on [HCP Terraform agents](/terraform/cloud-docs/agents/). Agents let HCP Terraform or Terraform Enterprise communicate with isolated, private, or on-premises infrastructure.
