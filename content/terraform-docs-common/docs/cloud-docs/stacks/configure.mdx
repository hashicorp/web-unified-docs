...
- Project Default: HCP Terraform uses a project's [default execution mode](/terraform/cloud-docs/projects#execution-mode) to choose the execution platform for a stack. This is the default setting.

- Remote (custom): HCP Terraform performs Terraform runs on its own disposable virtual machines.

- Agent (custom): [HCP Terraform agents](/terraform/cloud-docs/agents/) let HCP Terraform communicate with isolated, private, or on-premises infrastructure.
