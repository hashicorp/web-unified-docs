...
- Your component configuration, in one or more `tfcomponent.hcl` files.
- Your deployment configuration, in one or more `tfdeploy.hcl` files.
...
TODO: introduce groups and explain that they exist to configure runs for deployments that should be handled almost identically.
...
Each Stack in HCP Terraform maintains its own configuration versions, and each version has a collection of deployment runs. HCP Terraform starts applying deployment runs as they become approved, but the order is otherwise unspecified.

TODO: This whole section must be split into multiple sections.
TODO: Explain eager_plan = "off" and how it allows you to wait to plan a large group until you've seen what a smaller canary group is doing. Such groups currently must be manually started via UI or API.
TODO: Explain "overlapping groups" as they relate to locking and abandonment. (Overlapping groups are any past or future group containing any deployment from the group in question. This allows deployments to change groups while preserving lock integrity.)
TODO: Explain the group-based lock, which allows all deployments in a group to finish rolling out one config before members of the group start rolling out another one. This helps keep a group's deployments almost identical. Explain that the lock gets released when the group succeeds or fails.
TODO: explain that failure is a non-terminal state for groups and allows reruns, but it releases the lock, which must be re-acquired. (This might become impossible if a future config starts applying.)
TODO: explain how failure_tolerance limits concurrency for safety purposes, further reduces concurrency on receiving failure signals, and automatically halts rollout when acceptable failures are exceeded. Explain pending_capacity run status at this point.

When you approve a deployment plan, HCP Terraform automatically dismisses all pending plans for that deployment from other configuration versions.

For older configurations, HCP Terraform permanently abandons any runs, because you cannot revert your deployment to a previous configuration version.
...
TODO: Integrate this into the rearranged content around group-based locking.
...
ASK: Do we need to define pending capacity and acquiring lock?
