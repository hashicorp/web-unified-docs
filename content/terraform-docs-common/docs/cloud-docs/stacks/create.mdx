...
## Requirements
<!-- BEGIN: TFC:only name:no-stacks-on-hcp-legacy -->
...
<!-- END: TFC:only name:no-stacks-on-hcp-legacy -->
...
If you want to migrate existing workspaces to a Stack, refer to [Migrate HCP Terraform workspaces to Terraform Stacks](/terraform/cloud-docs/stacks/migrate).
...
1. Optionally, click **Advanced options** to control the following behavior of your Stack:
    * If the repository contains multiple Stack configurations, specify the relative path from your root directory that contains your Stack configuration in the **Stack working directory** field. By default, HCP Terraform uses the repository's root directory.
    * By default, HCP Terraform uses a project's default execution mode setting to run a Stack, but you can enable a different mode in the **Execution Mode** field. Refer to [Change execution mode](/terraform/cloud-docs/stacks/configure#change-execution-mode) for more information.
    * By default, HCP Terraform automatically triggers new Stack runs when you push a configuration change to the linked repository. You can enable **VCS Trigger enabled** to control how HCP Terraform applies new versions of your stack.
      * Choose either **Branch-based** or **Tag-based** publishing. With the branch-based workflow, you can configure **Trigger patterns** to control which file changes trigger new runs and choose to enable **Automatic speculative plans** to run a plan when you open pull requests against the linked repository. With the tag-based workflow, you can configure a **Custom regular expression** to control which tags trigger new runs.
...
Stack configuration files (`.tfstack.hcl` and `.tfdeploy.hcl`) do not support filesystem operations available in standard Terraform configurations:

- **`file()` function is not available**: You cannot read external files using `file()` in stack configuration files
- **`path.root` and `path.module` are not supported**

### Workarounds

If moving file loading to component configurations is not feasible, use these alternatives:

#### Quick Fixes (For Small Configurations)

1. **Inline content using heredoc strings:**
   ```hcl
   deployment "prod" {
     inputs = {
       user_data = <<-EOT
   ...
   > **Note:** Large heredocs in deployment files can make the HCP Terraform UI difficult to read and audit. Use this only for small configurations. For complex scripts, consider using `templatefile()` inside your component configuration instead.

2. **Pass structured data directly:**
   ```hcl
   deployment "prod" {
     inputs = {
       app_config = {
...
#### Enterprise Patterns (For Large-Scale Operations)

3. **Use HCP Terraform Variable Sets:**
   - Store large configurations as Terraform variables in HCP Terraform
   - Mark sensitive values as sensitive
   - Reference variables in your deployments
   - Manage centrally across multiple Stacks and projects
