...
- Each Stack supports a maximum of 500 deployments, which may limit scalability for large environments.
...
Stack configuration files (`.tfstack.hcl` and `.tfdeploy.hcl`) do not support filesystem operations:

- The `file()` function is not available in stack configuration files.
- `path.root` and `path.module` are not available.

The recommended pattern is to load external files inside your component configurations, where standard Terraform functions and filesystem symbols behave as they do in non-stack configurations. See the [Configuration Limitations](/terraform/cloud-docs/stacks/create#configuration-limitations) section in the Create a Stack guide for migration patterns and workarounds.
