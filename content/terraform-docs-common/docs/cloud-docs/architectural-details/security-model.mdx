...
Additionally, you can use [IP allowlists](/terraform/cloud-docs/users-teams-organizations/ip-allowlists) to restrict which IP addresses can access specific agent pools or your entire organization. This ensures that requests only come from approved IP addresses.
