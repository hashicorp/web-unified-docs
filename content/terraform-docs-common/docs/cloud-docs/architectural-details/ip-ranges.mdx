...
The IP ranges API provides information about HCP Terraform's outbound IP addresses, but you can also use [IP allowlists](/terraform/cloud-docs/users-teams-organizations/ip-allowlists) to restrict which IP addresses can access your HCP Terraform resources. IP allowlists let you define CIDR ranges that are permitted to connect to your organization, agent pools, or specific workspaces.
