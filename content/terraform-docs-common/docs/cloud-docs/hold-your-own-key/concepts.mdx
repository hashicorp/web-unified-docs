...
Hold your own key (HYOK) gives you control over your sensitive data by letting you provide your own encryption key to safeguard that data. Hold your own key lets you configure HCP Terraform artifact encryption using a key from a key management system (KMS).

When you enable hold your own key, the HCP Terraform agent encrypts specific Terraform CLI artifacts using a data encryption key (DEK) before uploading those files to HCP Terraform storage. To ensure you retain control of the DEK, the HCP Terraform agent authenticates with your key management service (KMS), and uses a key encryption key (KEK) to encrypt the DEK.
...
The above features do not have access to the sensitive information in your state and plan files in workspaces you enable with HYOK.
...
During an HCP Terraform run, the agent downloads the DEK and calls your KMS to decrypt it using the KEK. Then, when Terraform creates artifacts during the run, the agent uses the DEK to encrypt those artifacts before uploading them to HCP Terraform.
...
HCP Terraform stores the encrypted DEK and the encrypted artifacts. For HCP Terraform to read the artifacts and any secrets they may contain, your KMS needs to decrypt the DEK to decrypt the associated artifacts. Access to Terraform artifacts are controlled by the KEK on the KMS without needing to directly send artifacts to the KMS for encryption or decryption.

### JWT Claims

The JWT issued by HCP Terraform and used to authenticate to the KMS, have the following format:
...
When you configure your KMS's trust policy, match the following claims:
...
The DEK is a random 256-bit string. HCP Terraform sends the DEK to the associated KMS for that key configuration. Encryption is delegated to the KMS, so details vary depending on the key configuration:
...
Reference the following diagram for a visual representation of key handling and communication between HCP Terraform services, and services inside the customer network during an HCP Terraform apply:

![Diagram: HYOK apply workflow](/img/docs/tfc-hyok-apply-workflow.png)

HCP Terraform plans follow the same flow, except they decrypt a state file and produce and encrypt a plan file instead of the usual workflow.

### Download decrypted files

You can download decrypted state files from the HCP Terraform API or using the Terraform CLI.

When you attempt to download a decrypted specific state version in an HYOK-enabled workspace, HCP Terraform creates a temporary download link for its artifact storage service, archivist, then returns that link with an HTTP redirect response.

To access the link, follow the redirect through your browser or your CLI tool. After the redirect, archivist requests a decrypted data encryption key from HCP Terraform for artifact decryption. HCP Terraform uses a short-lived JWT that it issues during the link generation process and validates only for the specific artifact you request.

HCP Terraform then requests the decryption of the relevant data key for the artifact, using a [request forwarding agent](/terraform/cloud-docs/agents/request-forwarding) running inside your network. The agent decrypts the provided DEK using your KMS, and returns the decrypted DEK to HCP Terraform, and then to archivist.
...
Refer to the following diagram for a visual representation of the decrypted download flow during a `terraform init` using the Terraform CLI and a request forwarding agent within the customer network :
...
Before using HYOK, you establish a trust relationship between HCP Terraform and your chosen KMS. You must configure your KMS to accept HCP Terraform's identity tokens, telling your KMS to allow token bearers signed by HCP Terraform to encrypt and decrypt data with a specified key.

HCP Terraform's identity tokens are in the "JWT" (JSON Web Token) format.Learn more about JWT format in [JWT Claims](#jwt-claims).
