...
As part of setting up HYOK, you designate one configuration as your organization's **primary** configuration. When you enable HYOK on a workspace, HCP Terraform automatically secures that workspace's artifacts using an organization's primary configuration.
...
To use the hold your own key (HYOK) feature, ensure you meet the following requirements:
...
Take note of the key URI, which is in the format: `https://<KEY_VAULT_NAME>.vault.azure.net/keys/<KEY_NAME>`. You use this URI later to configure
...
Make a note of the HYOK config name you choose here, you use it later
...
`organization:<ORG_NAME>:hyok_config:<HYOK_CONFIG_NAME>`. The
hyok configuration name can contain letters, numbers, hyphens, and underscores.
You use this hyok configuration name when you configure it in HCP
Terraform.

<Note>

You must use a **Custom Trust Policy** rather than using
the **Web Identity** trusted entity type. We are working with AWS to improve this experience.
...
Note the **HYOK_CONFIG_NAME** value you choose here, as well as the **pool name** and **provider name**. You use these values later.

<Note>

GCP has a limit of 127 characters for subject claims. This means
`organization:<ORG_NAME>:hyok_config:<HYOK_CONFIG_NAME>` must be under
...
Ensure you exclude any trailing slashes from the HCP Terraform URL.

### Create a key

Set up the resource you want to access, which is the key encryption key you
...
Next, create the key which is used to encrypt and decrypt your data
...
JWT authentication the ability to encrypt and decrypt with your newly created key
...
```hcl
{
 ...
 "bound_claims": {
   "sub": "organization:<ORG_NAME>:hyok_config:*"
 },
 "user_claim": "sub",
 "role_type": "jwt",
 "token_ttl": "20m"
...
For additional security we recommend that you choose a configuration name now,
so that only the specified HYOK configuration may assume this role:
`organization:<ORG_NAME>:hyok_config:<HYOK_CONFIG_NAME>`. The HYOK configuration name can contain letters, numbers, hyphens, and
underscores.
...
when performing runs. The agents running your workspace's operations do not require request forwarding, but you should update them to version 1.23.0 or later to get the latest HYOK features.
...
- The **JWT auth path** where your JWT authentication backend is mounted. By default this is `admin`, but in the [Vault configuration example](#create-key-configuration) it was `jwt-prod`.
...
Once a configuration is saved, you can also access the  **Test Connection** button on the settings page for a HYOK configuration.

If HCP Terraform cannot successfully connect to your KMS and use your key to
encrypt and decrypt data, an error message appears. The most likely cause of
failures are:
...
- The required resources are not fully provisioned in your KMS. In rare cases, it can take up to 15 minutes for keys to be accessible to HCP Terraform after creation, particularly for Azure or GCP.

## Use a key configuration to encrypt a workspace

After setting up an HYOK configuration, you can select your primary configuration and use it to encrypt a workspace's artifacts.
...
To encrypt a workspace's artifacts using HYOK, go to a workspace's settings, and navigate to **HYOK Encryption**. Check **Enable HYOK for this workspace** to enable HYOK and encrypt data for the state and plan files in that workspace.
...
Any future runs on your workspace now produce encrypted state and plan files.
...
specific key version, you can revoke that key configuration within HCP Terraform. For example, if a key version is old or leaked, revoking it in HCP Terraform instructs the platform to stop using that key for encryption and to migrate all existing data off of it.
...
When you revoke a key configuration, HCP Terraform migrates all of the workspaces that used that configuration to the primary configuration. If you have not set a primary configuration, HCP Terraform automatically promotes the first available HYOK configuration to primary.
