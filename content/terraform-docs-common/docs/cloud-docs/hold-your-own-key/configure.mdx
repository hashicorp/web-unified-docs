...
last_modified: 2026-09-17T18:17:22.000Z
...
HYOK requires HCP Terraform agents on version `1.23.0` or later. Using a multi-region key with AWS KMS requires HCP Terraform agents on version `1.30.0` or later. Using a custom transit path with Vault requires HCP Terraform agents on version `1.31.2` or later.
...
You can mount the transit secrets engine at a path other than `transit`:

```shell-session
$ vault secrets enable -path=prod/transit transit
Success! Enabled the transit secrets engine at: prod/transit/
```

If you use a custom mount path, set the **Transit path** field when you
[configure key settings](#configure-key-settings), and replace `transit` with
your mount path in the commands and policies that follow.
...
If you mounted the transit secrets engine at a custom path, replace `transit` in
both policy paths with that mount path, such as `prod/transit/encrypt/<your key>`.
...
To configure your key in HCP Terraform, you need the following values from Vault:

- The **Name** of the key you created in the transit secrets engine. In the Vault
  configuration example above, this was `unique-prod`.
- The **Transit path** where you mounted the transit secrets engine. This field is
  optional and defaults to `transit`. Set it when you mounted the engine at a
  custom path, such as `prod/transit`. This option requires agents on version
  `1.31.2` or later.
...
- Single-region keys: When `multi-region` is disabled, the agent always uses the
  region you set in the HYOK configuration.
- Multi-region keys: When `multi-region` is enabled, the agent uses the AWS SDK's
...
Any future runs on your workspace will now produce encrypted state and plan files. Files from previous runs, or previously uploaded state versions, will not be re-encrypted under your key.
