...
- The Application ID for your Entra ID Application. This is also called an Application (client) ID on the EntraID Application
  overview page.
...
- The JWT auth path where your JWT authentication backend is mounted. By default this will
...
**Configure a new key** wizard during set up. Clicking **Test Connection** tells HCP
Terraform to attempt to authenticate to your KMS and test that basic encryption
and decryption works.

Once a configuration is saved, you can also access the button on the
...
Once you have followed the steps laid out in the previous
