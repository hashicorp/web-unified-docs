...
To learn more about configuring hold your own key, refer to [Configure and manage keys](/terraform/cloud-docs/hold-your-own-key/configure).

Hold your own key also produces sanitized versions of artifacts which redact secrets from the artifacts it encrypts. Sanitized state and plan files let HCP Terraform continue running policy checks, run tasks, cost estimation, and assessments without using sensitive data.
...
1. Enable hold your own key on a workspace.

### Configure your KMS and create a key

Begin by [configuring your KMS](/terraform/cloud-docs/hold-your-own-key/configure#create-key-configuration) to accept OIDC requests from HCP Terraform. Then, set up your keys and grant the necessary roles and permissions in your KMS. Specific configuration instructions differ between cloud providers.
...
- The name to identify this configuration within HCP Terraform.

After configuring a key, HCP Terraform automatically tests the key to ensure it can use that key to secure your data.
...
Choose one configuration to act as your primary configuration. HCP Terraform automatically uses the primary HYOK configuration to automatically encrypt all sensitive Terraform artifacts for that workspace.
