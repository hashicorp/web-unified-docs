...
To move a workspace, you must have the "Manage all Projects" organization permission or explicit team admin privileges on both the source and destination projects. Remember that moving a workspace to another project may affect user visibility for that project's workspaces. Refer to [Project Permissions](/terraform/cloud-docs/users-teams-organizations/permissions/project) for details on workspace access.
...
Determines the Terraform execution platform for this workspace. You can choose one of the following modes:

-  **Project Default**: HCP Terraform uses a project's [default execution mode](/terraform/cloud-docs/projects#execution-mode) to choose the execution platform for a workspace. This is the default setting.

- **Remote (custom)**: HCP Terraform performs Terraform runs on its own disposable virtual machines. This provides a consistent and reliable run environment and enables advanced features, such as Sentinel policy enforcement, cost estimation, notifications, and version control integration.

- **Local (custom)**: Lets you perform Terraform runs on your local workstation using the [CLI-driven run workflow](/terraform/cloud-docs/workspaces/run/cli). Your workspace still stores your state, which Terraform can access with the [CLI integration](/terraform/cli/cloud). HCP Terraform does not evaluate workspace variables or variable sets in local execution mode. This mode disables remote execution.

- **Agent (custom)**: Lets HCP Terraform communicate with lightweight [HCP Terraform agents](/terraform/cloud-docs/agents). Use this mode to run Terraform in isolated, private, or on-premises infrastructure. If you also specify **Agent** as your [policy execution mode](/terraform/cloud-docs/workspaces/policy-enforcement/manage-policy-sets/configure#policy-set-exeuction-modes), HCP Terraform and Terraform Enterprise use the same agent instance to evaluate policies and run Terraform operations.

#### Change the execution mode 

Changing your workspace's execution mode after a run has already been planned causes the run to error when it is applied. Complete the following steps to minimize the number of runs that error when changing your workspace's execution mode:

1. Disable [auto-apply](/terraform/cloud-docs/workspaces/settings#auto-apply) if you have it enabled.
1. Complete any runs that are no longer in the [pending stage](/terraform/cloud-docs/workspaces/run/states#the-pending-stage).
...
In [runs triggered by VCS commits](/terraform/cloud-docs/workspaces/run/ui), this is automatic. In [CLI-driven runs](/terraform/cloud-docs/workspaces/run/cli), Terraform's CLI sometimes uploads additional content:
...
By default, the **Share with specific workspaces** option is enabled for new workspaces in HCP Terraform. The workspace selector is also empty. As a result, other workspaces can't access the new workpaces's state.
<!-- END: TFC:only name:share-remote-state-projects-default -->

Administrators can enable remote state sharing between workspaces globally so that new workspaces share state during runs by default. The option you choose in the workspace settings overrides the global default. Refer to <!-- BEGIN: TFEnterprise:only name:share-remote-state-projects-defaults -->[Remote State Sharing](/terraform/enterprise/application-administration/general#remote-state-sharing) in the administration documentation <!-- END: TFEnterprise:only name:share-remote-state-projects-defaults --><!-- BEGIN: TFC:only name:share-remote-state-projects-default -->[Remote State Access Controls](/terraform/cloud-docs/workspaces/state#remote-state-access-controls) <!-- END: TFC:only name:share-remote-state-projects-default -->for more information. 
...
<!-- BEGIN: TFC:only name:share-remote-state-projects-default-change -->

<Note>

The default access permissions for new workspaces in HCP Terraform changed in April 2021. Workspaces created before this change default to allowing global access within their organization. You can change workspaces to more restrictive access at any time.  

</Note>

<!-- END: TFC:only name:share-remote-state-projects-default-change -->
...
- Plan-only runs.
- The planning stages of [saved plan runs](/terraform/cloud-docs/workspaces/run/modes-and-options.mdx#saved-plans). You can only _apply_ a saved plan if the workspace is unlocked, and applying that plan locks the workspace as usual. Terraform Enterprise does not yet support this workflow.
