...
- **Local (custom)**: Lets you perform Terraform runs on the local workstation per the [CLI-driven run workflow](/terraform/cloud-docs/workspaces/run/cli). The workspace stores state, which Terraform can access with the [CLI integration](/terraform/cli/cloud). HCP Terraform does not evaluate workspace variables or variable sets in local execution mode. This mode disables remote execution.

- **Agent (custom)**: Allows HCP Terraform to communicate with lightweight [HCP Terraform agents](/terraform/cloud-docs/agents). Use this mode to run Terraform in isolated, private, or on-premises infrastructure. When the policy execution mode is also set to **Agent**, HCP Terraform and Terraform Enterprise evaluate policies on an agent instance from the specified agent pool. Refer to [Configure connections to policy sets](/terraform/cloud-docs/workspaces/policy-enforcement/manage-policy-sets/configure) for more information.    
