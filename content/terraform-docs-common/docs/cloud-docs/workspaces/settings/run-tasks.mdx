...
## Scope a run task the the organization or project

<Note>

Project-scope run tasks are currently available as a beta feature.

</Note>

You can choose to apply a run task to individual workspaces or projects, or you can apply it globally to every workspace in an organization. To apply run tasks to individual workspaces, refer to [Associate run tasks with a single workspace](#associate-run-tasks-with-a-single-workspace).
...
1. Under **Enforce on**, choose one or more project to associate the run task with.
1. Click **Save**.

<!-- END: TFC:only name:tfc-project-run-task -->

## Associate run tasks with a single workspace
