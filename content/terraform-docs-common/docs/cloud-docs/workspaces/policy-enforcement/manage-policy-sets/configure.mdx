...
When a policy run starts, the job is executed internally by the HCP Terraform or Terraform Enterprise container.In this mode, HCP Terraform and Terraform Enterprise use Sentinel 0.40.x to evaluate policies. The latest language features are not supported. 
...
Platform environment mode may lead to slower overall application performance during high policy run volumes. This is because the policy run shares CPU and memory allocations with the HCP Terraform or Terraform Enterprise container.
...
HCP Terraform Agent mode can result in slower startup times because of the time necessary to dispatch an agent or start up the container. This mode cannot access cost estimation data.
