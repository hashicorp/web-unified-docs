...
You can set Terraform policy parameters when you [edit a policy set](#edit-policy-sets). Terraform policy will also load values for inputs from workspace environment variables prefixed with `TF_VAR_`. For example, a workspace with an environment variable of `TF_VAR_instance_type` would set the value of the `instance_type` input.
