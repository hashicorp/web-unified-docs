...
Legacy mode uses Sentinel 0.40.x to evaluate policies and does not support the latest language features. Use this mode when your Sentinel policies require access to cost estimation data. 
...
If you also specify **Agent** as your workspaces execution mode, HCP Terraform and Terraform Enterprise use the same agent instance to evaluate policies and run Terraform operations. You can explicitly set your workspace execution mode to **Agent**, or your workspace can inherit an execution mode from [project settings](/terraform/cloud-docs/projects#execution-mode) or [organization settings](/terraform/cloud-docs/users-teams-organizations/organizations#organization-settings).    
...
Use agent mode to execute your policy set using a specific version of Sentinel. You can also use agent mode to offload resource consumption from HCP Terraform or Terraform Enterprise, because your policies run in an isolated environment. 

Agent mode can result in slower startup times because of the time necessary to dispatch an agent or start up the container. Agent mode also cannot access cost estimation data.
