...
To evaluate Terraform policy policies, workspaces running in "Agent" execution mode must be of a minimum version 1.28.10.
...
You can set Terraform policy parameters when you [edit a policy set](#edit-policy-sets). Terraform policy will also load values for inputs from workspace environment variables with a matching name. For example, a workspace with an environment variable of `instance_type` would set the value of the `instance_type` input. You can also prefix environment variables with `TFPOLICY_INPUT_`, for example `TFPOLICY_INPUT_instance_type`.

When Terraform policy evaluates your Terraform runs, it will set values for inputs in your policies through the following methods, in order of precendence:
