...
- Terraform policy evaluations can occur at setup time, plan time, or apply time depending on the policy configuration. Setup evaluations can occur before providers and modules are downloaded during init. Plan time evaluations occur after the plan is complete. Apply time evaluations occur after the apply is complete.
...
When policies with a `mandatory_overridable` enforcement level fail, HCP Terraform will halt the run. Users with the manage policy overrides permission can override these policy failures and continue applying the run.
