...
If you use GitHub as your VCS provider and merge a PR changing 3000 or more files, HCP Terraform automatically triggers runs for every workspace connected to that repository. The GitHub API has a limit of 3000 reported changed files for a PR merge. To address this, HCP Terraform initiates workspace runs proactively, preventing oversight of file changes beyond this limit.
