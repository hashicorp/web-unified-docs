...
created_at: 2025-09-05T12:41:13-07:00
...
The details page for a run indicates when the Terraform invoked actions. Actions are preset operations built into providers that you can invoke to trigger automations outside of Terraform, such as Ansible playbooks and Lambda jobs. 

If the run invoked one or more actions, the **Action:** label and number of actions in the run appears in the operation output heading. For example, if the workspace contains two action configurations, the **Plan finished** operation heading includes **Actions: 2 to invoke**. 

You can invoke actions from the Terraform CLI or from the HCP Terraform UI. For information about invoking actions from the CLI, refer to [Invoke an action](/terraform/language/invoke-actions) in the Terraform configuration language documentation. For information about invoking actions and viewing additional action details in HCP Terraform, refer to [Manage Terraform actions](/terraform/cloud-docs/actions).

<Note>

An IBM HCP Terraform Standard tier subscription is required to use the `-invoke` flag to execute an action and to view details about action runs that were invoked from the command line using the `-invoke` flag. Refer to the [pricing page](https://www.hashicorp.com/en/pricing?tab=terraform) for more information.

</Note>

#### Search for action runs

In the **Run List**, choose either **Direct action invocation** or **Lifecycle action invocation** from the **Type** list and click **Apply** to filter for runs that include action invocations. 
...
You can also open the [Action page](/terraform/cloud-docs/actions) to find information about actions invoked in the workspace.  

<!-- END: TFC:only name:actions -->

## Interacting with Runs

In workspaces where you have [permission to apply runs](/terraform/cloud-docs/users-teams-organizations/permissions/workspace), you can interact with a run at the bottom of its details page.
...
If a run is currently planning or applying, users with [permission to apply runs](/terraform/cloud-docs/users-teams-organizations/permissions/workspace) for the workspace can click **Cancel Run** to stop the run before it finishes.
...
You can lock the workspace to temporarily stop runs from proceeding. Locking a workspace requires [permission to lock and unlock the workspace](/terraform/cloud-docs/users-teams-organizations/permissions/workspace), and requires that the workspace is not currently locked by an in-progress run.
