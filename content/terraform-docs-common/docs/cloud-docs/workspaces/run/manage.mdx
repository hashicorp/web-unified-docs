...
You can invoke actions either as part of a resource lifecycle configuration or manually target a specific action using the `-invoke` flag to a `terraformm apply` or `terraform plan` command.  

<Note>

A Standard tier subscription is required to view details about actions that were invoked from the command line using the `-invoked` flag. Refer to the [Terraform pricing page](https://www.hashicorp.com/en/pricing?tab=terraform) for more information.
