...
last_modified: 2026-03-12T03:10:34.000Z
...
- To change organization health settings, you must be an [organization owner](/terraform/cloud-docs/users-teams-organizations/permissions/organization#organization-owners).
- To change a workspace’s health settings, you must be an [administrator for that workspace](/terraform/cloud-docs/users-teams-organizations/permissions/workspace#workspace-admin).
<!-- BEGIN: TFC:only name:health-assessments -->
- To trigger [on-demand health assessments](/terraform/cloud-docs/workspaces/health#on-demand-assessments) for a workspace, you must be an [administrator for that workspace](/terraform/cloud-docs/users-teams-organizations/permissions/workspace#workspace-admin).
...
You can enforce health assessments across all eligible workspaces in an organization within the [organization settings](/terraform/cloud-docs/users-teams-organizations/organizations/settings#health). Enforcing health assessments at an organization-level overrides workspace-level settings. You can only enable health assessments within a specific workspace when HCP Terraform is not enforcing health assessments at the organization level.
...
When you enable health assessments for a workspace, HCP Terraform schedules the first health assessment based on whether there are active Terraform runs for the workspace:

- **No active runs:** During the next assesment period after you enable the feature.
- **Active speculative plan:** During the next assesment period after that plan is complete.
- **Other active runs:** During the next assessment period.

HCP Terraform schedules the first assessment during the next scheduled assessment period. To view when HCP Terraform expects to run the next assessment, refer to the workspace's **Drift** page in the UI.

Assuming the workspace can be assessed, the next health assessment is always scheduled a set period of time out from the more recent of last apply or assessment.

For example, if automatic health assessments are set to run on a 24-hour cadence, a workspace that last applied a run 4 hours ago and has not been assessed yet receives its next assessment in 20 hours.
...
Configuration drift happens when external changes affecting remote objects invalidate your infrastructure configuration. State drift occurs when external changes affecting remote objects _do not_ invalidate your infrastructure configuration. Refer to [Refresh-Only Mode](/terraform/cloud-docs/workspaces/run/modes-and-options#refresh-only-mode)  to learn more about remediating state drift.
