...
-> **Note:** Enabling health assessment does not guarantee that an assessment runs within minutes of enabling the feature. HCP Terraform schedules assessments according to the assessment cadence, so the first assessment starts during the next scheduled assessment period. To view information about when HCP Terraform expects to run the next assessment, refer to the workspace's **Drift** page in the UI.
