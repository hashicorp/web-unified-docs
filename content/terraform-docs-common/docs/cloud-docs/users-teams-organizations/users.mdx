...
User accounts belong to individual people. Each user can be part of one or more [teams](/terraform/cloud-docs/users-teams-organizations/teams), which are granted permissions on projects and workspaces within an organization. A user can be a member of multiple [organizations][].
