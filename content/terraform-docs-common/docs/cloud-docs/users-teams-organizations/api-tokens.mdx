...
The [GitHub App VCS provider](/terraform/cloud-docs/vcs/github-app) checks repository access at the level of the calling user's linked GitHub account, in addition to the GitHub App installation's repository scope. Connecting a workspace or Stack to a GitHub App-backed repository (`vcs-repo.github-app-installation-id`) currently requires the calling identity to have authorized HCP Terraform on a personal GitHub account through the [GitHub App OAuth flow](/terraform/cloud-docs/vcs/github-app#authorizing).

- **User API token.** The user who owns the token must have completed the GitHub App authorization at least once. If they have not, requests fail with `The authenticated user has not linked a GitHub account to HCP Terraform via the GitHub App.` Have the user sign in to HCP Terraform and follow [Authorizing](/terraform/cloud-docs/vcs/github-app#authorizing) to resolve.

- **Team and organization API tokens.** Team and organization tokens authenticate as a synthetic service account that cannot complete a personal GitHub OAuth flow, so today they cannot be used to *create* a workspace or Stack against a GitHub App-backed repository. They can still manage other workspace settings, and they remain the recommended pattern for VCS-managing automation against OAuth-backed VCS connections, because they are not tied to an individual GitHub account and survive user offboarding.
