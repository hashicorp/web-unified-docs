...
To connect a repository to a workspace, Stack, or registry module, you must use a user API token and complete the GitHub App OAuth workflow at least once. You cannot use a team or organization API token. If you do not complete the OAuth workflow, requests fail with a `422` error stating that the authenticated user must link a GitHub account through the GitHub App. To resolve the error, have the user sign in to HCP Terraform and follow the steps in [Authorizing](/terraform/cloud-docs/vcs/github-app#authorizing). 

You cannot use a team or organization API token to connect a repository to a workspace, Stack, or registry module, but you can use these tokens to manage workspace settings. For automation that manages OAuth-backed VCS connections, we recommend team and organization tokens because they are not tied to an individual GitHub account.
