...
last_modified: 2026-04-24T14:48:23.000Z
...
The [GitHub App VCS provider](/terraform/cloud-docs/vcs/github-app) checks repository access at the level of the calling user's linked GitHub account, in addition to the GitHub App installation's repository scope. Connecting a workspace, Stack, or registry module to a GitHub App-backed repository with `vcs-repo.github-app-installation-id` requires the calling identity to have authorized HCP Terraform on a personal GitHub account through the [GitHub App OAuth flow](/terraform/cloud-docs/vcs/github-app#authorizing).

- **User API token.** The user who owns the token must have completed the GitHub App authorization at least once. If they have not, requests fail with a `422` error stating that the authenticated user must link a GitHub account through the GitHub App. The exact wording varies by endpoint. Refer to [Troubleshooting](/terraform/cloud-docs/vcs/github-app#troubleshooting) for the message each endpoint returns. To resolve the error, have the user sign in to HCP Terraform and follow the steps in [Authorizing](/terraform/cloud-docs/vcs/github-app#authorizing).

- **Team and organization API tokens.** Team and organization tokens authenticate as a synthetic service account that cannot complete a personal GitHub OAuth flow, so you cannot use them to create or update a workspace, Stack, or registry module connected to a GitHub App-backed repository. They can still manage other workspace settings. For automation that manages OAuth-backed VCS connections, we recommend team and organization tokens because they are not tied to an individual GitHub account and keep working after a user leaves the organization.
