...
IP allowlists cannot govern agent pools that are configured for enhanced agents, such as pools used for [request forwarding](/terraform/cloud-docs/agents/request-forwarding) or [Hold Your Own Key (HYOK)](/terraform/cloud-docs/hold-your-own-key). HCP Terraform does not enforce IP restrictions on requests to these agent pools, even when they are assigned to an IP allowlist or fall within an **Apply to all agent pools** scope.
...
- IP allowlists cannot govern agent pools that are configured for enhanced agents, such as pools used for [request forwarding](/terraform/cloud-docs/agents/request-forwarding) or [Hold Your Own Key (HYOK)](/terraform/cloud-docs/hold-your-own-key).
