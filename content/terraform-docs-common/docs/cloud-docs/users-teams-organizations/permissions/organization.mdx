...
last_modified: 2026-03-31T03:10:29.000Z
...
- [Project permissions reference](/terraform/cloud-docs/users-teams-organizations/permissions/project)
...
Members do not have access to projects or workspaces and Stacks. You can grant permissions to individual projects or workspaces and Stacks through [Project Permissions](/terraform/cloud-docs/users-teams-organizations/permissions/set-permissions#set-project-level-roles) or [Workspace Permissions](/terraform/cloud-docs/users-teams-organizations/permissions/set-permissions#set-workspace-level-roles).
...
Members can create and manage all projects and workspaces or Stacks within the organization. In addition to the permissions granted when enabling the [**Manage all workspaces**](#manage-all-workspaces) permission, this also lets users perform the following actions:
...
In HashiCorp Cloud Platform (HCP) Europe organizations, you manage user access through HCP groups, and use group permissions instead of team permissions. To learn more, refer to [Use HCP Terraform in Europe](/terraform/cloud-docs/europe).
...
| [Manage teams](#manage-teams) | Create, update, delete teams and [generate tokens](/terraform/cloud-docs/users-teams-organizations/api-tokens#team-api-tokens) |
...
In order to modify the membership of a team, the user must be a member of a team with the **Manage membership** permissions enabled and the [**Visible**](/terraform/cloud-docs/users-teams-organizations/teams/manage#team-visibility) setting must be enabled for the team. The user can also be a member of the team if the **Visible** setting is disabled.
...
| [Manage run tasks](#manage-run-tasks) | Create, edit, and delete both native and run tasks |
...
Allows members to create, edit, and delete both native and run tasks on the organization.
...
<!-- BEGIN: TFC:only name:reg-artifact-copy -->
...
| [Assign project tags to modules, providers, and Stack component configurations](#assign-project-tags-to-modules-providers-and-stack-component-configurations) | Lets team members assign project tags to modules, providers, and Stacks components in the private registry |
| [Assign reserved keys to module, provider, and Stack component configuration versions](#assign-reserved-keys-to-module-provider-and-stack-component-configuration-versions) | Lets team members assign reserved keys to module, provider, and Stacks components versions in the private registry |

<!-- END: TFC:only name:reg-artifact-copy -->

<!-- BEGIN: TFEnterprise:only name:no-reg-artifact-copy -->
...
<!-- END: TFEnterprise:only name:no-reg-artifact-copy -->
...
Enable this setting to allow team members to attach existing project tags to modules, providers, and Stacks components in the registry. Use these tags to organize registry artifacts. Refer to [Define project tags](/terraform/cloud-docs/projects/manage#define-project-tags) for more information.

### Assign reserved keys to module, provider, and Stack component configuration versions

Enable this setting to allow team members to assign reserved tag keys to module and provider versions in the registry. The  **Registry environment tag** option must be enabled on the reserved tag keys to attach them to registry artifact versions. 
...
-->
Reserved keys apply to projects, workspaces, and registry artifact versions, such as the specific version of a module or provider. Refer to [Create and manage reserved tag keys](/terraform/cloud-docs/users-teams-organizations/organizations/manage-reserved-tags) for more information.

<!-- Not in the public beta, expose for GA or for public beta 
Use these tags to gate access to module and provider versions based on tag. Refer to [Use artifacts from the registry](/terraform/cloud-docs/registry/using#find-artifacts) for more information.-->
...
| [Delegate policy overrides](#delegate-policy-overrides) | Allow project and workspace managers to grant override permissions for **Soft mandatory** policy evaluations. When this setting is enabled for a team, its members can override failed policy evaluations on projects and workspaces that they manage. |
...
| [Manage run tasks](#settings-permissions) | Create, edit, delete both native and run tasks. |
...
| [Manage IP allowlists](#settings-permissions) | Create, edit, delete IP allowlists. |
...
- [HCP roles](/terraform/cloud-docs/users-teams-organizations/permissions/set-permissions#set-organization-level-roles-for-hcp-europe-organizations) - You can assign HCP roles to groups in the HashiCorp Cloud Platform (HCP), and these roles automatically grant permissions in HCP Terraform.
