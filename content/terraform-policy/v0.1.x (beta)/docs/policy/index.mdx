...
- Setup: HCP Terraform executes provider and module policies that evaluate arguments available at setup time, such as their source and version, during workspace initialization and before Terraform installs the providers or modules. A policy violation prevents Terraform from downloading the providers or modules.
...
- Learn how to write policies and tests for Terraform policy. Refer to [Write Terraform policies](/terraform/policy/write).
  - For complete examples, including tests, refer to [Terraform policy examples](/terraform/policy/write/examples).
