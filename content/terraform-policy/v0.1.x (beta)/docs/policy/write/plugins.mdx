...
Specify plugins by using the `plugins {}` block within the `policy {}` block of your `.policy.hcl` files. Refer to [The `policy {}` block](/terraform/policy/reference/policy/policy) for more information.
...
Reference your functions with the `plugin::` prefix in your policies.
