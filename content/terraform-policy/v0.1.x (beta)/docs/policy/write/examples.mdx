...
To test your plugin on your local machine, build it.

```shell-session
$ go build -o ../../bin/cidr_utils main.go
```

When you deploy your policies to an HCP Terraform policy set, you must build your plugins for the correct architecture and commit them to version control. For more information about writing and building plugins, refer to [Write policy plugins](/terraform/policy/write/plugins).
