...
```shell-session
$ CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ../../bin/cidr_utils main.go
```

Run the following command for `linux/arm64` agents:

```shell-session
$ CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o ../../bin/cidr_utils main.go
```

For more information about writing and building plugins, refer to [Write policy plugins](terraform/policy/write/plugins).
