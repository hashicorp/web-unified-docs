...
The policy uses an input variable to make the encryption requirement configurable and sets `enforcement_level = "mandatory_overridable"` to allow users with appropriate permissions to override the policy when needed (e.g., for development environments).
...
The policy recommends that VPC modules are sourced from an internal private registry and use version 2.0.0 or higher. The advisory enforcement level means violations will generate warnings but won't block operations.
...
This example demonstrates using `core::getresources()` to validate relationships between resources. It uses `enforcement_level = "mandatory_overridable"` to allow exceptions for special cases (e.g., public audit trails) with appropriate approval.

### Policy

The policy ensures that CloudTrail resources are associated with S3 buckets that have private ACLs. The mandatory_overridable enforcement level allows users with appropriate permissions to override this requirement when necessary.
...
This example demonstrates using the `operations` parameter to control when policies are enforced and the `prior_attrs` attribute to access previous resource state. This is useful for preventing dangerous operations like reducing volume sizes that could cause data loss.

### Policy

The policy uses `operations = ["update"]` to only enforce during update operations (not create or delete) and compares `prior_attrs.size` with `attrs.size` to prevent downsizing.
...
This test demonstrates using `prior_attrs` in test resources to simulate update operations. It includes three scenarios: increasing size (pass), keeping same size (pass), and decreasing size (fail).
