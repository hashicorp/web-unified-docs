...
The module source (For example: `"app.terraform.io/my-org/vpc/aws"`, `"./modules/networking"`).
...
In the following example, mock `module` blocks test a policy that enforces minimum module versions — one with a current version and one with an outdated version.
...
In the following example, mock `module` blocks test a policy that validates module sources — one from an approved source and one from an unapproved source.
...
In the following example, mock `module` blocks test a policy that enforces version constraints across different module versions—two that pass and one that fails.
