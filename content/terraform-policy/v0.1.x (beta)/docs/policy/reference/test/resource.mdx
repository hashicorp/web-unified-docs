...
- `resource` "[`<resource_type>`](#resource-type)" &nbsp "[`<test_label>`](#test-label)" &nbsp block
  ...
  - [`meta`](#meta) &nbsp object | optional
    ...
    - `type` &nbsp string
  - [`prior_attrs`](#prior-attrs) &nbsp object | optional
    - `<attribute_name>` &nbsp any | repeatable
...
The resource type (For example: `aws_instance`, `google_compute_instance`). Refer to the provider documentation for a list of supported resources.
...
The resource attributes before the operation (for `update` or `delete` operations). Each provider defines the value attributes for each of its resources. Refer to the provider documentation for more information.
