...
You can set values for policy inputs either per test case, or for the entire test file with the `inputs {}` block. The inputs block is allowed within provider, resource, data, and module blocks, or as a top-level block within policy test files. Inputs set on a specific block override inputs set at the top level of the test file.
