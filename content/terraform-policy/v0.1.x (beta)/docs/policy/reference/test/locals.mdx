...
Each `locals {}` block contains one or more local value assignments. The name of each local value must be unique within the test file.
...
locals {
  env_prefix = input.environment
  vpc_name   = "${local.env_prefix}-vpc"
  
  instance_type = input.environment == "production" ? "t3.medium" : "t3.micro"
}
