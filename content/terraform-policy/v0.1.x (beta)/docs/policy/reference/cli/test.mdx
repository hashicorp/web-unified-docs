...
1. In an [`inputs {}` block](/terraform/policy/reference/test/input) inside a provider, resource, or module block.
2. In an [`inputs {}` block](/terraform/policy/reference/test/input) at the top level of each test file.
