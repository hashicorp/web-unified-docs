...
description: >-
  Reference for the core::contains_substring function used to check if a string contains a substring in policy files.
...
In the following example, the `core::contains_substring` function performs case-insensitive substring matching by converting both strings to lowercase.

```hcl
resource_policy "aws_s3_bucket" "case_insensitive_check" {
  locals {
    bucket_lower = core::lower(attrs.bucket)
    contains_backup = core::contains_substring(bucket_lower, "backup")
  }
