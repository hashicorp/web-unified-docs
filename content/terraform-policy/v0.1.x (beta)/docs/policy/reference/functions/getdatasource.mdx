...
The `core::getdatasource` function returns a data source of the specified type that matches the given configuration. Terraform policy passes the data source configuration to the provider, and returns a matching data source object. Since this function returns a single data source, you must specify configuration as needed to specify a single data source. Refer to the provider documentation for the specified data source type to learn more.
...
| `config` | Yes | Map | A map representing the data source configuration, which Terraform policy validates against the data source schema and uses to fetch the data source from the provider. |
