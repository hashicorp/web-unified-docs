...
Data source arguments in Terraform configuration can contain one or more nested blocks of arguments. When creating Terraform policy code against data source arguments, you must represent the nested blocks according to the provider's schema.

In Terraform configuration, you can nest a single block, a list of blocks, or a set of blocks. Each block can contain its own arguments. Assign Terraform policy arguments according to the nested block's data type:
|------------|-------------------------------|
...
For example, the AWS `aws_ami` data source lets you define a set of nested blocks to filter responses.  In Terraform configuration, you could write the following `filter` blocks to filter AMIs by name and virtualization type:
...
In Terraform policy, represent the `filter` blocks as a list of objects:
...
In the following example, the `core::getdatasource` function retrieves an AWS AMI that matches the filter and validates both the AMI ID and name pattern.
