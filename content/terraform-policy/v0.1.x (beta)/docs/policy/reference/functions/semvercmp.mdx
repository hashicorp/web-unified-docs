...
description: >-
  Reference for the core::semvercmp function used to compare semantic versions in policy files.
...
The `core::semvercmp` function compares two semantic version strings. It returns `-1` if the first version is less than the second, `0` if they are equal, and `1` if the first version is greater than the second.
...
- `0` if `version_a` equals `version_b`
...
In the following example, the `core::semvercmp` function sorts a list of version strings.

```hcl
locals {
  versions = ["1.2.0", "1.10.0", "1.1.0", "2.0.0", "1.9.0"]
...
provider_policy "kubernetes" "use_latest" {
  locals {
    is_latest = core::semvercmp(meta.version, local.latest_version) == 0
  }

  enforce {
    condition     = local.is_latest
    error_message = "Kubernetes provider should use the latest approved version: ${local.latest_version}. Current: ${meta.version}"
  }
}
```
...
provider_policy "azurerm" "stable_versions_only" {
  locals {
    ...
    # If comparing pre-release versions
    min_stable = "3.0.0"
