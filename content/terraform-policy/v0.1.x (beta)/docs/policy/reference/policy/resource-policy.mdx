...
The `resource_policy` block defines policies for Terraform resources. The first label of the block identifies the resource type, such as `aws_instance`, `google_compute_instance`, or `azurerm_windows_virtual_machine`, and the second label is the policy name. You can use a wildcard (*) for all or part of the resource name to apply a policy to every resource defined in the configuration, or every resource starting with a given prefix.
