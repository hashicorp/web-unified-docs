...
You can declare multiple `enforce` blocks in each `resource_policy` block. If a condition in any `enforce` block fails during evaluation, Terraform policy fails the entire policy and returns the corresponding error messages.
