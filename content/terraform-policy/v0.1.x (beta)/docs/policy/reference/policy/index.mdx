...
Each policy block must contain one or more `enforce {}` blocks that define conditions that must evaluate to `true` for the policy to pass.
...
|------|-------------|------|---------|
| `operations` | Set the lifecycle operations to enforce the policy against. If ommitted, Terraform policy will enforce your policy during "create" and "update" operations. | String | `"create"`, `"update"`, `"delete"` |
