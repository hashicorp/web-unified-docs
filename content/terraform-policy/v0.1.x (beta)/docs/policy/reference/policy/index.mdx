...
By default, Terraform policy treats policies as mandatory. You can set the enforcement level with the optional `enforcement_level` argument. The available enforcement levels are `"advisory"`, `"mandatory_overridable"`, and `"mandatory"`.

By default, Terraform policy evaluates resource policies during create and update operations. You can select the lifecycle operations for a `resource_policy` with the optional `operations` argument.

Each policy block can contain `enforce {}` blocks that define conditions that must evaluate to `true` for the policy to pass.
...
|------|-------------|------|---------|
| `operations` | Set the lifecycle operations for a resource policy. If omitted, the policy applies to create and update operations. | List of strings | `["create", "update"]` |
...
Attribute values can be complex types such as maps and lists, which you can reference with dot notation. For example, use `attrs.tags.environment`. Use `attrs` to reference the target provider, resource, or module values. Resource policies can also use `prior_attrs` to reference previous resource state.
