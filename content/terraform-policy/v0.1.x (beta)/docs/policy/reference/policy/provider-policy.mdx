...
Each `provider_policy {}` block can contain `enforce {}` blocks that define conditions that must evaluate to `true` for the policy to pass.
...
You can include multiple `enforce {}` blocks in each policy block. If any enforce condition fails during evaluation, Terraform policy fails the policy and returns the corresponding error messages.

### The `locals {}` block

Each `provider_policy {}` block may contain a `locals {}` block that defines values local to the policy. These local values override values defined in root-level `locals {}` blocks. Refer to [The `locals {}` block](/terraform/policy/reference/policy/locals) for more information.
