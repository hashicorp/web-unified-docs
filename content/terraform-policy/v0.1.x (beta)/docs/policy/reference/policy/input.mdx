...
- `input` &nbsp "[`<name>`](#name)" &nbsp block
...
The following example demonstrates an input block with all configuration options specified, a second input block, and an example policy block that references inputs:
...
The name of the input variable. Must be unique within the policy file.

### Arguments

| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `type` | Yes | Type constraint | The type constraint for the input variable. |
| `description` | No | String | A description of the input variable's purpose. |
| `default` | No | Any | A default value for the input variable. If omitted and no value is provided, the value is null. Terraform policy returns an error unless `nullable` is `true`. |
| `nullable` | No | Boolean | Whether the input accepts a null value. Defaults to `false`. |
