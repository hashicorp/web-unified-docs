...
- `input` &nbsp "[`<label>`](#label)" &nbsp block
...
Whether the input accepts a null value. If set to `false`, either the input block must provide a default value, or the value must be set by the policy configuration, or Terraform policy will error.
...
The type constraint for the input variable. Input variables support the [same type constraints](/terraform/language/expressions/types#types) as Terraform. For example:
