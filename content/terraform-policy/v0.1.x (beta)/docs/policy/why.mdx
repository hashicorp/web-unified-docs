...
Terraform policy provides the following benefits.
### Native HCL syntax
...
Each framework enforces the following policies using a different mechanism:
...
<Tab heading="Terraform policy" group="tfp">

Uses the `core::getresources(type, filter)` function to fetch related resources by type and attribute. The filter is applied at evaluation time, returning only matching candidates. This makes relationship checks concise and readable.

</Tab>

<Tab heading="Sentinel" group="sentinel">

Iterates the [`tfplan/v2` import](/sentinel/docs/features/terraform/tfplan-v2) to find resource changes. Sentinel filters by type, but there is no structured helper API. Correlating two resource types by a shared attribute requires manually iterating both collections and matching by value. Deeply nested relationships across multiple resource types requires additional effort to write and maintain. Results can also be unpredictable when the linking attribute is unknown at plan time.

</Tab>

<Tab heading="OPA" group="opt">
