...
The `policy` block configures Terraform policy itself. You must include a `policy` block with a `required_providers` nested block in any policy file that contains resource or provider policies. Refer to [`policy`](/terraform/policy/reference/policy/policy) for more information.
