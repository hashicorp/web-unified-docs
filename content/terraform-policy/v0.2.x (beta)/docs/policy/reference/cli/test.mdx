...
if you specify target policy files or directories in your test files with the [`targets`](/terraform/policy/reference/test/policytest#targets) block, Terraform policy will error if a target does not match at least one policy file.
...
The `tfpolicy test` command will exit with a non-zero exit code when any tests fail, or an exit code of `0` if all tests pass.
