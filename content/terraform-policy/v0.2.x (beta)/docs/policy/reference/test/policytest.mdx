...
If you do not specify targets in your `policytest {}` block, Terraform policy will evaluate the policy files in the path you provide when you run your tests using the `--policies` flag. If you specify a target that does not match at least one policy, `tfpolicy test` will error.
