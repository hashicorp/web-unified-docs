...
The `core::alltrue` function evaluates a list and returns `true` if all elements evaluate to `true`. If any element does not evaluate to a boolean value, the function returns an error. If any element evaluates to `false`, the function returns `false`.
...
|----------|----------|------|-------------|
| `list` | Yes | List of booleans | The list of to evaluate. |

## Return value

Returns `true` if all elements in the list evaluate to `true`. Otherwise, returns `unknown` if any elements are unknown, or `false` if any element evaluates to `false`. Raises an error if any element is not a boolean.
...
resource_policy "aws_instance" "required_tags" {
  ...
  enforce {
    ...
    error_message = "Instance must have all required tags: ${join(", ", local.required_tags)}"
  }
}
```
