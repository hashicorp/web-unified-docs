...
resource_policy "aws_instance" "check_bucket_count" {
  enforce {
    condition     = core::length(local.all_s3_buckets) <= 10
    error_message = "Too many S3 buckets. Maximum allowed: 10, current: ${core::length(local.all_s3_buckets)}"
  }
}
```
...
resource_policy "aws_s3_bucket_policy" "check_bucket_exists" {
  enforce {
    condition     = core::length(local.specific_bucket) > 0
...
resource_policy "aws_cloudtrail" "s3_bucket_private" {
  locals {
    ...
    is_private = core::length(local.s3_bucket_acl) > 0 && local.s3_bucket_acl[0].acl == "private"
  }
...
resource_policy "aws_instance" "production_instance_check" {
  ...
  enforce {
    condition     = core::length(local.tagged_instances) <= 5
    error_message = "Too many production instances in us-east-1a. Maximum: 5, current: ${core::length(local.tagged_instances)}"
  }
}
```
