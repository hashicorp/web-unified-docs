...
Data source arguments use various types, including nested block arguments. For most argument types, you will assign a value of the matching type within the `config` object. However, when a data source uses nested blocks in Terraform configuration, you must represent them differently in Terraform policy.
...
| Terraform block type | Terraform policy config value |
|------------|-------------------------------|
| Single nested | Object |
| List nested | List of objects |
| Set nested  | List of objects |

For example, the AWS `aws_ami` data source uses a set nested block for `filter`. In Terraform configuration, you would write:
...
In the following example, the `core::getdatasource` function retrieves an AWS AMI that matches the filter and validates the AMI ID.
