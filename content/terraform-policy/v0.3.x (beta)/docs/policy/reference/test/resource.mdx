...
To test a resource policy, add a mock resource block to your test file. Use the `attrs` or `prior_attrs` attribute to set values for resource attributes defined by the provider. Use the `meta` attribute to set meta-attributes for the resource, if required. When you run your tests, Terraform policy will validate your resource block attributes using the providers defined in your policy block's [`required_providers`](/terraform/policy/reference/policy/policy#required_providers) attribute.
...
- `resource` "[`<resource_type>`](#resource-type)" &nbsp "[`<test_label>`](#test-label)" &nbsp block
  - [`attrs`](#attrs) &nbsp object | optional
    - [`<attribute_name>`](#attrs) &nbsp any | repeatable
  ...
  - [`meta`](#meta) &nbsp object | optional
    - [`module_path`](#meta) &nbsp string
    - [`operation`](#meta) &nbsp string
    - [`provider_type`](#meta) &nbsp string
    - [`tfe_stack`](#meta) &nbsp map
    - [`tfe_workspace`](#meta) &nbsp map
    - [`type`](#meta) &nbsp string
  - [`prior_attrs`](#prior_attrs) &nbsp object | optional
    - [`<attribute_name>`](#prior_attrs) &nbsp any | repeatable
  - [`skip`](#skip) &nbsp boolean | optional

While both `attrs` and `prior_attrs` are optional, one or the other must be present and non-empty. For [delete operations](#test-delete-operation), use the minimal fixture: `prior_attrs = { id = null }`.
...
Either `attrs` or [`prior_attrs`](#prior_attrs) must exist and be non-empty.
...
| `tfe_stack.deployment_group` | String | The deployment group for Terraform Stacks. Empty string when evaluating workspaces. |
| `tfe_stack.deployment_name` | String | The deployment name for Terraform Stacks. Empty string when evaluating workspaces. |
| `tfe_stack.stack_name` | String | The stack name for Terraform Stacks. Empty string when evaluating workspaces. |
| `tfe_workspace.tags` | Map(String) | Tags for the current workspace. Empty map when evaluating Stacks or when the workspace has no tags. |
...
Either `prior_attrs` or [`attrs`](#attrs) must exist and be non-empty.
...
In the following example, the mock `resource` block simulates a delete operation using `prior_attrs`.

```hcl
resource "aws_s3_bucket" "delete_test" {
  meta = {
    operation = "delete"
  }
