...
While both `attrs` and `prior_attrs` are optional, one or the other must be present and non-empty. If both evaluate to empty when Terraform policy evaluates your tests, it will skip the test case.
