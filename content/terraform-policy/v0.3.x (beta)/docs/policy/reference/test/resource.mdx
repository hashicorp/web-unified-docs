...
To test a resource policy, add a mock resource block to your test file. Use the `attrs` or `prior_attrs` attribute to set values for resource attributes defined by the provider. Use the `meta` attribute to set meta-attributes for the resource, if required.
