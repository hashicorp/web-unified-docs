...
created_at: 2026-08-24T00:55:04.000Z
last_modified: 2026-08-24T00:55:04.000Z
...
The admin token for HCP Vault is similar to the root token for self-managed
Vault, with some key differences:

| Token feature                               | Admin token                                  | Root token                      |
| ------------------------------------------- | -------------------------------------------- | ------------------------------- |
| Attached policy                             | hcp-root                                     | root                            |
| Expiration (TTL)                            | 6 hours                                      | No expiration (manually revoke) |
| Permissions                                 | Full access to admin and child namespaces    | Full access to entire cluster   |
| Provides access to root namespace           | No                                           | Yes                             |

You should limit the use of the admin token and generate other tokens through
supported auth methods with policies that follow [least privilege
principles](/well-architected-framework/secure-systems/identity-access-management/grant-least-privilege)
for regular use.
...
Workload Identity Federation (WIF) is not supported in HCP Vault Dedicated. This includes all Vault WIF functionality, including auth
methods, secrets engines, and Secrets Sync.
