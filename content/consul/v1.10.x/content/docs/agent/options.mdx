...
last_modified: 2026-07-14T23:21:38.000Z
...
- `-bind` ((#\_bind)) - The address that should be bound to for internal
  ...
  all addresses on the local machine and will [advertise](/consul/docs/agent/options#_advertise)
  ...
  `"[::]"`, Consul will [advertise](/consul/docs/agent/options#_advertise) the public
...
- `-join` ((#\_join)) - Address of another agent to join upon starting up.
  ...
  joining a client to a segment](/consul/docs/enterprise/network-segments#join_a_client_to_a_segment).
...
- `-protocol` ((#\_protocol)) - The Consul protocol version to use. Consul
  agents speak protocol 2 by default, however agents will automatically use protocol > 2 when speaking to compatible agents. This should be set only when [upgrading](/consul/docs/upgrading). You can view the protocol versions supported by Consul by running `consul -v`.
...
- `-segment` ((#\_segment)) <EnterpriseAlert inline /> - This flag is used to set
  ...
  Review the [Network Segments documentation](/consul/docs/enterprise/network-segments)
...
They are documented separately under [check configuration](/consul/docs/discovery/checks) and
[service configuration](/consul/docs/agent/services) respectively. The service and check
...
- `acl` ((#acl)) - This object allows a number of sub-keys to be set which
  ...
  - `tokens` ((#acl_tokens)) - This object holds all of the configured
    ...
    - `agent_master` ((#acl_tokens_agent_master)) - Used to access [agent endpoints](/consul/api-docs/agent)
...
- `acl_replication_token` ((#acl_replication_token_legacy)) - **Deprecated
  ...
  [`acl_down_policy`](/consul/docs/agent/options#acl_down_policy) is set to "extend-cache", tokens not
...
- `dns_config` This object allows a number of sub-keys
  ...
  - `prefer_namespace` ((#dns_prefer_namespace)) <EnterpriseAlert inline /> -
    ...
    versions and will assume the label is the datacenter. See: [this section](/consul/docs/discovery/dns#namespaced-services)
    for more details.
...
- `segments` <EnterpriseAlert inline /> - (Server agents only) This is a list of nested objects
  ...
  created automatically. Review the [Network Segments documentation](/consul/docs/enterprise/network-segments)
  for more details.
