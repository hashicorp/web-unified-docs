...
1. Specify the `partition-exports` configuration in the agent configuration file (see [`config_entries`](/consul/docs/agent/options#config_entries)) as described in [Configuration](#configuration).
...
See [Health HTTP Endpoint](/consul/api-docs/health) for additional information.