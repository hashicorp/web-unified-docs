...
last_modified: 2026-07-14T23:22:29.000Z
...
- Consul [Connect](/consul/docs/agent/options#connect) must be enabled in both datacenters.
- Each of your [datacenters](/consul/docs/agent/options#datacenter) must have a unique name.
- Your datacenters must be [WAN joined](https://learn.hashicorp.com/tutorials/consul/federarion-gossip-wan).
- The [primary datacenter](/consul/docs/agent/options#primary_datacenter) must be set to the same value in both datacenters. This specifies which datacenter is the authority for Connect certificates and is required for services in all datacenters to establish mutual TLS with each other.
- [gRPC](/consul/docs/agent/options#grpc_port) must be enabled.
- If you want to [enable gateways globally](/consul/docs/connect/gateways/mesh-gateway#enabling-gateways-globally) you must enable [centralized configuration](/consul/docs/agent/options#enable_central_service_config).
...
The first is that the [service kind](/consul/api-docs/agent/service#kind) must be "mesh-gateway". Second,
