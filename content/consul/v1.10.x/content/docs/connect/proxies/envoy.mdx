...
last_modified: 2026-07-14T23:22:49.000Z
...
| ------------------- | ------------------------------------------------------ |
| 1.10.x              | 1.18.6, 1.17.4, 1.16.5, 1.15.5                         |
...
1. Envoy 1.10.0 requires setting [`-envoy-version`](/consul/commands/connect/envoy#envoy-version) in the `consul connect envoy` command. This was introduced in Consul 1.7.0.
...
command](/consul/commands/connect/envoy). The command can either output the
...
configuration entry](/consul/docs/connect/config-entries/proxy-defaults) or override them directly in the `proxy.config` field
of a [proxy service
...
[`enable_central_service_config`](/consul/docs/agent/options#enable_central_service_config)
