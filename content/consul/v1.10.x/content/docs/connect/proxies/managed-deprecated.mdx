...
last_modified: 2026-07-14T23:22:55.000Z
...
After transitioning away from all managed proxy usage, the `proxy` subdirectory inside [`data_dir`](/consul/docs/agent/options#_data_dir) (specified in Consul config) can be deleted to remove extraneous configuration files and free up disk space.
...
[per-agent](/consul/docs/agent/options).
...
[service definition](/consul/docs/agent/services). The simplest possible
...
[`-data-dir`](/consul/docs/agent/options#_data_dir) CLI option. If a data dir is
