...
last_modified: 2026-07-14T23:23:07.000Z
...
specifically [`client_addr`](/consul/docs/agent/options#client_addr),
[`ports.dns`](/consul/docs/agent/options#dns_port), [`recursors`](/consul/docs/agent/options#recursors),
[`domain`](/consul/docs/agent/options#domain), and [`dns_config`](/consul/docs/agent/options#dns_config).
...
[documentation on configuration options](/consul/docs/agent/options),
...
[`recursors`](/consul/docs/agent/options#recursors) configuration so that non-Consul queries
...
but you can set a specific domain for responding to DNS queries by configuring the [`domain`](/consul/docs/agent/options#domain) parameter.
...
through the [`alt_domain`](/consul/docs/agent/options#alt_domain) agent configuration
option. As of Consul versions 1.11.0+, Consul's DNS response will use the same domain as was used in the query;
in prior versions, the response may use the primary [`domain`](/consul/docs/agent/options#domain) no matter which
...
[primary domain](/consul/docs/agent/options#domain) (not the alternative domain),
...
using the [`advertise-wan`](/consul/docs/agent/options#_advertise-wan) and
[`translate_wan_addrs`](/consul/docs/agent/options#translate_wan_addrs) configuration
options.
...
[`prefer_namespace` configuration](/consul/docs/agent/options#dns_prefer_namespace).
...
1. The agent's [`default` token](/consul/docs/agent/options#acl_tokens_default).
2. The built-in [`anonymous` token](/consul/docs/security/acl#builtin-tokens).
