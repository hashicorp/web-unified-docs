...
* `task_summary` - Summary of task information. See [Task Status API](/consul/docs/nia/api#task-status) to learn more about how health status is determined.
...
* `events` - [(list[Event])](/consul/docs/nia/api#event) - List of stored events that inform the task's status. See section below for information on event data. This field is only included in the response upon request by setting the `?include=events` parameter. The relative URL for the request to include events can be retrieved from the `events_url` field.
