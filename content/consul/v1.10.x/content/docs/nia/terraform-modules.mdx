...
last_modified: 2026-07-14T23:25:05.000Z
...
- [**`services` list**](/consul/docs/nia/configuration#services) - The list of services to act as a source input.
- **`source_includes_var` condition field** - If the condition supports this field, and it is set to true, then the condition’s objects will be used as a source input. For example, if a module is defined supporting [`catalog_services` input variable](#catalog-services-variable) then, this field can be set to true in the [catalog-services condition](/consul/docs/nia/tasks#catalog-services-condition).
- [**`source_input` block**](/consul/docs/nia/configuration#source-input) - This block specifically defines a source input.
...
- the task's [`services`](/consul/docs/nia/configuration#services)
- a [`condition "services"` block](/consul/docs/nia/configuration#services-condition) configured with `regexp` and `source_includes_var` set to true
- a [`source_input "services"` block](/consul/docs/nia/configuration#services-source-input) configured with `regexp`

The services source input operates by monitoring the [Health List Nodes For Service API](/consul/api-docs/health#list-nodes-for-service) and provides the latest service information to the input variables. A complete list of service information that would be provided to the module is expanded below:
...
Tasks configured with a Consul KV source input monitor Consul KV for changes to KV pairs that satisfy the provided configuration. The Consul KV source input operates by monitoring the [Consul KV API](/consul/api-docs/kv#read-key) and provides these key values to the task’s module.
...
- a [`condition "consul-kv"` block](/consul/docs/nia/configuration#consul-kv-condition) configured with the `source_includes_var` set to true.
- a [`source_input "consul-kv"` block](/consul/docs/nia/configuration#consul-kv-source-input).
...
Tasks configured with a Catalog Services source input monitors for service and tag information provided by the [Catalog List Services API](/consul/api-docs/catalog#list-services). The source input is a map of service names to a list of tags.
...
- a [`condition "catalog-services"` block](/consul/docs/nia/configuration#consul-kv-condition) configured with `source_includes_var` set to true.
...
Keys of the `services` map are unique identifiers of the service across Consul agents and data centers. Keys follow the format `service-id.node.datacenter` (or `service-id.node.namespace.datacenter` for Consul Enterprise). A complete list of attributes available for the `services` variable is included in the [documentation for Consul-Terraform-Sync tasks](/consul/docs/nia/tasks#services-condition).
