...
last_modified: 2026-07-14T23:24:35.000Z
...
See the [Helm configuration](/consul/docs/k8s/helm#v-synccatalog)
...
will need to be provided. Review the [ACL rules](/consul/docs/security/acl/acl-rules)
...
set specifically via the `consul.hashicorp.com/service-port` annotation (see [Service Ports](/consul/docs/k8s/service-sync#service-ports)).
...
service unless set specifically via the `consul.hashicorp.com/service-port` annotation (see [Service Ports](/consul/docs/k8s/service-sync#service-ports)).
...
The service instance's port can be overridden via the `consul.hashicorp.com/service-port` annotation (see [Service Ports](/consul/docs/k8s/service-sync#service-ports)).
...
-> **Note:** Consul to Kubernetes syncing **isn't required** if you've enabled [Consul DNS on Kubernetes](/consul/docs/k8s/dns)
