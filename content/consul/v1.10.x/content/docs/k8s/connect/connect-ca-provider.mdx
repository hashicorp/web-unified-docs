...
last_modified: 2026-07-14T23:23:59.000Z
...
please use the [Update CA Configuration Endpoint](/consul/api-docs/connect/ca#update-ca-configuration).

Consul has support for different certificate authority (CA) providers to be used with the Consul Service Mesh.
Please see [Connect Certificate Management](/consul/docs/connect/ca) for the information on the providers
...
such as shown in [Standalone Server with TLS](https://www.vaultproject.io/docs/k8s/helm/examples/standalone-tls),
...
To update any settings under this key, you must use Consul's [Update CA Configuration](/consul/api-docs/connect/ca#update-ca-configuration) API or the [`consul connect ca set-config`](/consul/commands/connect/ca#set-config) command.
