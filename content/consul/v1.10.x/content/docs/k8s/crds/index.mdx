...
last_modified: 2026-07-14T23:24:11.000Z
...
- [`Mesh`](/consul/docs/connect/config-entries/mesh) (requires Consul 1.10.0+)
- [`PartitionExports`](/consul/docs/connect/config-entries/partition-exports) 
...
- [`ServiceIntentions`](/consul/docs/connect/config-entries/service-intentions) (requires Consul 1.9.0+)
...
See [Install with Helm Chart](/consul/docs/k8s/installation/install) for further installation
instructions.
...
extra steps to migrate to CRDs. See [Upgrade An Existing Cluster to CRDs](/consul/docs/k8s/crds/upgrade-to-crds) for full instructions.
...
See [Configuration Entries](/consul/docs/agent/config-entries) for detailed schema documentation.
...
1. **Mirroring** - The Kubernetes namespace will be "mirrored" into Consul, i.e.
   ...
   This is configured via [`connectInject.consulNamespaces`](/consul/docs/k8s/helm#v-connectinject-consulnamespaces):
