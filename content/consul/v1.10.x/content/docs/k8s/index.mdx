...
last_modified: 2026-07-14T23:24:17.000Z
...
to Kubernetes using the [Helm chart](/consul/docs/k8s/installation/install#helm-chart-installation) or [Consul K8s CLI<sup>BETA</sup>](/consul/docs/k8s/installation/install#consul-k8s-cli-installation), sync services between Consul and
...
Consul can automatically inject the [Consul Connect](/consul/docs/connect)
...
[node ID](/consul/docs/agent/options#_node_id) is persisted so that servers
...
[accessing the Consul HTTP API](/consul/docs/k8s/installation/install#accessing-the-consul-http-api)
for an example.
