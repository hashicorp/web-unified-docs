...
last_modified: 2026-07-14T23:24:21.000Z
...
[`-retry-join` values](/consul/docs/agent/options#retry-join). In the
example above, a fake [cloud auto-join](/consul/docs/install/cloud-auto-join)
...
to all other server and client agents inside and _outside_ of Kubernetes over [LAN](/consul/docs/glossary#lan-gossip).
...
As noted in the [Security Model](/consul/docs/security/security-models#secure-configuration),
...
Please see the [reference documentation](/consul/docs/k8s/helm#v-externalservers-hosts)
...
If you would like to call the [ACL bootstrapping API](/consul/api-docs/acl/acl#bootstrap-acls) yourself or if your cluster has already been bootstrapped with ACLs,
