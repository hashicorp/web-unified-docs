...
last_modified: 2026-07-14T23:24:22.000Z
...
[Helm chart](/consul/docs/k8s/helm). This will install and configure
...
cluster with default configurations. We strongly recommend [learning about the configuration options](/consul/docs/k8s/helm#configuration-values) prior to going to production.
...
the [recommended security features](/consul/docs/security/security-models). Currently,
...
or by reading the [Helm Chart Reference](/consul/docs/k8s/helm).

For example, if you want to enable the [Consul Connect](/consul/docs/k8s/connect) feature,
...
`helm upgrade`. See [Upgrading](/consul/docs/k8s/upgrade) for more details.
...
1. Download and build the CLI as described in the [Consul K8s CLI reference](/consul/docs/k8s/k8s-cli).
1. Issue the `install` subcommand to install Consul on Kubernetes:
   ...
   Refer to the [Consul K8s CLI reference](/consul/docs/k8s/k8s-cli) for details about all commands and available options.
...
1. Enter `y` to proceed. The pre-install checks may fail if existing `PersistentVolumeClaims` (PVC) are detected. Refer to the [uninstall instructions](/consul/docs/k8s/operations/uninstall#uninstall-consul) for information about removing PVCs.
...
[`/agent` endpoints for services and checks](/consul/api-docs/agent).

For Consul installed via the Helm chart, a client agent is installed on
each Kubernetes node. This is explained in the [architecture](/consul/docs/k8s/installation/install#client-agents)
