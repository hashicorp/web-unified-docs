...
last_modified: 2026-07-14T23:24:25.000Z
...
This page describes how to federate multiple Kubernetes clusters. See [Multi-Cluster Overview](/consul/docs/k8s/installation/multi-cluster)
...
1. Gossip encryption is enabled in the above config file. To disable it, comment
   ...
   **NOTE:** This config assumes you've already
   ...
   [the docs for this setting](/consul/docs/k8s/helm#v-global-gossipencryption) for
...
With your `config.yaml` ready to go, follow our [Installation Guide](/consul/docs/k8s/installation/install)
...
- **Consul server config** - This is a JSON snippet that must be used as part of the server config for secondary datacenters.
  ...
  - [`primary_datacenter`](/consul/docs/agent/options#primary_datacenter) to the name of the primary datacenter.
  - [`primary_gateways`](/consul/docs/agent/options#primary_gateways) to an array of IPs or hostnames
