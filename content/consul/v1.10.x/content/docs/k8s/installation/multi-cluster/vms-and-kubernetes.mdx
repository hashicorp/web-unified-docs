...
last_modified: 2026-07-14T23:24:27.000Z
...
~> This topic requires familiarity with [Mesh Gateways](/consul/docs/connect/gateways/mesh-gateway) and [WAN Federation Via Mesh Gateways](/consul/docs/connect/gateways/mesh-gateway/wan-federation-via-mesh-gateways).
...
must be the [primary](/consul/docs/k8s/installation/multi-cluster/kubernetes#primary-datacenter).

## Kubernetes as the Primary

If your primary datacenter is running on Kubernetes, use the Helm config from the
[Primary Datacenter](/consul/docs/k8s/installation/multi-cluster/kubernetes#primary-datacenter) section to install Consul.

Once installed on Kubernetes, and with the `ProxyDefaults` [resource created](/consul/docs/k8s/installation/multi-cluster/kubernetes#proxydefaults),
...
1. For clients, you can generate TLS certs with:
   ...
   Or use the [auto_encrypt](/consul/docs/agent/options#auto_encrypt) feature.
...
construct the [Federation Secret](/consul/docs/k8s/installation/multi-cluster/kubernetes#federation-secret) in order to federate
...
With your config file ready to go, follow our [Installation Guide](/consul/docs/k8s/installation/install)
to install Consul on your secondary cluster(s).

After installation, if you're using consul-helm 0.30.0+, [create the
`ProxyDefaults` resource](/consul/docs/k8s/installation/multi-cluster/kubernetes#proxydefaults)
to allow traffic between datacenters.

## Next Steps

In both cases (Kubernetes as primary or secondary), after installation, follow the [Verifying Federation](/consul/docs/k8s/installation/multi-cluster/kubernetes#verifying-federation)
