...
last_modified: 2026-07-14T23:23:56.000Z
...
used on its own. Consul [integrates with Envoy](/consul/docs/connect/proxies/envoy) to simplify its configuration.
...
The [Consul Envoy integration](/consul/docs/connect/proxies/envoy)
...
that utilize layer 7 functionality, Envoy can be used. For non-performance critical layer 4 applications, you can use Consul's [built-in proxy](/consul/docs/connect/proxies/built-in) for convenience.
...
alongside other proxies, assuming they integrate with the [necessary APIs](/consul/docs/connect/proxies/integrate)
for Connect.
