...
last_modified: 2026-07-14T23:25:20.000Z
...
[ACL token replication](/consul/docs/agent/options#acl_enable_token_replication)
...
linking [roles](/consul/docs/security/acl/acl-system#acl-roles) and [service
identities](/consul/docs/security/acl/acl-system#acl-service-identities) to newly created
...
- **Bind Type and Name** - A binding rule can bind a token to a
  [role](/consul/docs/security/acl/acl-system#acl-roles) or to a [service
  identity](/consul/docs/security/acl/acl-system#acl-service-identities) by name. The name
...
1. Applications use the `consul login` subcommand or the [login API
   endpoint](/consul/api-docs/acl/acl#login-to-auth-method) to authenticate to a
...
9. (later) Applications SHOULD use the `consul logout` subcommand or the
   [logout API endpoint](/consul/api-docs/acl/acl#logout-from-auth-method) to destroy
