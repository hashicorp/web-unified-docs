...
last_modified: 2026-07-14T23:25:17.000Z
# END AUTO GENERATED METADATA
---

# ACL System

-> **1.4.0 and later:** This guide only applies in Consul versions 1.4.0 and later. The documentation for the legacy ACL system is [here](/consul/docs/security/acl/acl-legacy).
...
- **ACL Service Identities** - Service identities are a policy template for
  ...
  Connect](/consul/docs/connect). At authorization time this acts like an
...
- **ACL Node Identities** - Node identities are a policy template for
  ...
  ](/consul/docs/agent/options#acl_tokens_agent). At authorization time this acts like an
  additional policy was attached, the contents of which are described further
  below. These are directly attached to tokens and roles and are not
  independently configured. (Added in Consul 1.8.1)

- **ACL Auth Methods and Binding Rules** - To learn more about these topics,
  see the [auth methods documentation page](/consul/docs/security/acl/auth-methods).

ACL tokens, policies, roles, auth methods, and binding rules are managed by
Consul operators via Consul's [ACL API](/consul/api-docs/acl/acl),
[ACL CLI](/consul/commands/acl), or systems like
...
- **Rules** - Set of rules granting or denying permissions. See the [Rule Specification](/consul/docs/security/acl/acl-rules#rule-specification) documentation for more details.
...
-> **Consul Enterprise Namespacing** - Rules defined in a policy in any namespace other than `default` will be [restricted](/consul/docs/security/acl/acl-rules#namespace-rules) to being able to grant a subset of the overall privileges and only affecting that single namespace.
...
An ACL service identity is an [ACL policy](/consul/docs/security/acl/acl-system#acl-policies) template for expressing a link to a policy
suitable for use in [Consul Connect](/consul/docs/connect). They are usable
...
rules](/consul/docs/security/acl/acl-system#acl-rules-and-scope):
...
The [API documentation for roles](/consul/api-docs/acl/roles#sample-payload) has some
...
An ACL node identity is an [ACL policy](/consul/docs/security/acl/acl-system#acl-policies) template for expressing a link to a policy
suitable for use as an [Consul `agent` token](/consul/docs/agent/options#acl_tokens_agent). They are usable
on both tokens and roles and are composed of the following elements:
...
mode depending on the configuration of [`acl_default_policy`](/consul/docs/agent/options#acl_default_policy).
...
| Resource                                            | Scope                                                                                                                                                                                                    |
| --------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`acl`](/consul/docs/security/acl/acl-rules#acl-resource-rules)     | Operations for managing the ACL system [ACL API](/consul/api-docs/acl/acl)                                                                                                                                           |
| [`agent`](/consul/docs/security/acl/acl-rules#agent-rules)          | Utility operations in the [Agent API](/consul/api-docs/agent), other than service and check registration                                                                                                             |
| [`event`](/consul/docs/security/acl/acl-rules#event-rules)          | Listing and firing events in the [Event API](/consul/api-docs/event)                                                                                                                                                 |
| [`key`](/consul/docs/security/acl/acl-rules#key-value-rules)        | Key/value store operations in the [KV Store API](/consul/api-docs/kv)                                                                                                                                                |
| [`keyring`](/consul/docs/security/acl/acl-rules#keyring-rules)      | Keyring operations in the [Keyring API](/consul/api-docs/operator/keyring)                                                                                                                                           |
| [`node`](/consul/docs/security/acl/acl-rules#node-rules)            | Node-level catalog operations in the [Catalog API](/consul/api-docs/catalog), [Health API](/consul/api-docs/health), [Prepared Query API](/consul/api-docs/query), [Network Coordinate API](/consul/api-docs/coordinate), and [Agent API](/consul/api-docs/agent)    |
| [`operator`](/consul/docs/security/acl/acl-rules#operator-rules)    | Cluster-level operations in the [Operator API](/consul/api-docs/operator), other than the [Keyring API](/consul/api-docs/operator/keyring)                                                                                       |
| [`query`](/consul/docs/security/acl/acl-rules#prepared-query-rules) | Prepared query operations in the [Prepared Query API](/consul/api-docs/query)                                                                                                                                        |
| [`service`](/consul/docs/security/acl/acl-rules#service-rules)      | Service-level catalog operations in the [Catalog API](/consul/api-docs/catalog), [Health API](/consul/api-docs/health), [Intentions API](/consul/api-docs/connect/intentions), [Prepared Query API](/consul/api-docs/query), and [Agent API](/consul/api-docs/agent) |
| [`session`](/consul/docs/security/acl/acl-rules#session-rules)      | Session operations in the [Session API](/consul/api-docs/session)                                                                                                                                                    |

Since Consul snapshots actually contain ACL tokens, the [Snapshot API](/consul/api-docs/snapshot)
...
3. The [connect CA roots endpoint](/consul/api-docs/connect/ca#list-ca-root-certificates) exposes just the public TLS certificate which other systems can use to verify the TLS connection with Consul.

Constructing rules from these policies is covered in detail on the
[ACL Rules](/consul/docs/security/acl/acl-rules) page.

-> **Consul Enterprise Namespacing** - In addition to directly linked policies, roles and service identities, Consul Enterprise
will include the ACL policies and roles defined in the [Namespaces definition](/consul/docs/enterprise/namespaces#namespace-definition). (Added in Consul Enterprise 1.7.0)
...
| -------------------------------------------------------------- | ---------- | ---------- | ---------------------------------------------------------------------- |
| [`acl.enabled`](/consul/docs/agent/options#acl_enabled)               | `REQUIRED` | `REQUIRED` | Controls whether ACLs are enabled                                      |
| [`acl.default_policy`](/consul/docs/agent/options#acl_default_policy) | `OPTIONAL` | `N/A`      | Determines allowlist or denylist mode                                  |
| [`acl.down_policy`](/consul/docs/agent/options#acl_down_policy)       | `OPTIONAL` | `OPTIONAL` | Determines what to do when the remote token or policy resolution fails |
| [`acl.role_ttl`](/consul/docs/agent/options#acl_role_ttl)             | `OPTIONAL` | `OPTIONAL` | Determines time-to-live for cached ACL Roles                           |
| [`acl.policy_ttl`](/consul/docs/agent/options#acl_policy_ttl)         | `OPTIONAL` | `OPTIONAL` | Determines time-to-live for cached ACL Policies                        |
| [`acl.token_ttl`](/consul/docs/agent/options#acl_token_ttl)           | `OPTIONAL` | `OPTIONAL` | Determines time-to-live for cached ACL Tokens                          |
...
| ------------------------------------------------------------------------ | ---------- | ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`acl.tokens.agent_master`](/consul/docs/agent/options#acl_tokens_agent_master) | `OPTIONAL` | `OPTIONAL` | Special token that can be used to access [Agent API](/consul/api-docs/agent) when remote bearer token resolution fails; used for setting up the cluster such as doing initial join operations, see the [ACL Agent Master Token](#acl-agent-master-token) section for more details |
| [`acl.tokens.agent`](/consul/docs/agent/options#acl_tokens_agent)               | `OPTIONAL` | `OPTIONAL` | Special token that is used for an agent's internal operations, see the [ACL Agent Token](#acl-agent-token) section for more details                                                                                                                                   |
| [`acl.tokens.master`](/consul/docs/agent/options#acl_tokens_master)             | `OPTIONAL` | `N/A`      | Special token used to bootstrap the ACL system, check the [Bootstrapping ACLs](https://learn.hashicorp.com/tutorials/consul/access-control-setup-production) tutorial for more details                                                                                |
| [`acl.tokens.default`](/consul/docs/agent/options#acl_tokens_default)           | `OPTIONAL` | `OPTIONAL` | Default token to use for client requests where no token is supplied; this is often configured with read-only access to services to enable DNS service discovery on agents                                                                                             |

All of these tokens except the `master` token can all be introduced or updated via the [/v1/agent/token API](/consul/api-docs/agent#update-acl-tokens).

#### ACL Agent Master Token

Since the [`acl.tokens.agent_master`](/consul/docs/agent/options#acl_tokens_agent_master) is designed to be used when the Consul servers are not available, its policy is managed locally on the agent and does not need to have a token defined on the Consul servers via the ACL API. Once set, it implicitly has the following policy associated with it
...
The [`acl.tokens.agent`](/consul/docs/agent/options#acl_tokens_agent) is a special token that is used for an agent's internal operations. It isn't used directly for any user-initiated operations like the [`acl.tokens.default`](/consul/docs/agent/options#acl_tokens_default), though if the `acl.tokens.agent` isn't configured the `acl.tokens.default` will be used. The ACL agent token is used for the following operations by the agent:
...
The `service_prefix` policy needs read access for any services that can be registered on the agent. If [remote exec is disabled](/consul/docs/agent/options#disable_remote_exec), the default, then the `key_prefix` policy can be omitted.
...
[ACL rules](/consul/docs/security/acl/acl-rules).
