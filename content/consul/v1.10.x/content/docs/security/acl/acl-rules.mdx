...
last_modified: 2026-07-14T23:25:15.000Z
# END AUTO GENERATED METADATA
---

# ACL Rules

-> **1.4.0 and later:** This document only applies in Consul versions 1.4.0 and later. The documentation for the legacy ACL system is [here](/consul/docs/security/acl/acl-legacy).

Consul provides an optional Access Control List (ACL) system which can be used
to control access to data and APIs. To learn more about Consul's ACL review the
[ACL system documentation](/consul/docs/security/acl/acl-system)
...
- `list`: allows access to all the keys under a segment in the Consul KV. Note, this policy can only be used with the `key_prefix` resource and [`acl.enable_key_list_policy`](/consul/docs/agent/options#acl_enable_key_list_policy) must be set to true.
...
There is only one acl rule allowed per policy and its value is set to one of the [policy dispositions](/consul/docs/security/acl/acl-rules#rule-specification). In the example
...
configured with [`acl.tokens.agent_master`](/consul/docs/agent/options#acl_tokens_agent_master) to allow
...
The `key` and `key_prefix` resources control access to key/value store operations in the [KV API](/consul/api-docs/kv). Key
...
The `node` and `node_prefix` resources controls node-level registration and read access to the [Catalog API](/consul/api-docs/catalog),
...
Agents need to be configured with an [`acl.tokens.agent`](/consul/docs/agent/options#acl_tokens_agent)
...
[`acl.token.default`](/consul/docs/agent/options#acl_tokens_default) used by the agent does not have "read" access to a
...
1. Using the [acl.tokens.default](/consul/docs/agent/options#acl_tokens_default) configuration
...
[Prepared Query API](/consul/api-docs/query). Executing queries is subject to `node`/`node_prefix` and `service`/`service_prefix`
...
The `service` and `service_prefix` resources control service-level registration and read access to the [Catalog API](/consul/api-docs/catalog)
...
[`acl.tokens.default`](/consul/docs/agent/options#acl_tokens_default) used by the agent does not have "read" access to a
