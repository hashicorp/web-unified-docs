...
last_modified: 2026-07-14T23:25:30.000Z
...
- **mTLS** - Mutual authentication of both the TLS server and client x509 certificates prevents internal abuse through
  ...
  - [`verify_incoming`](/consul/docs/agent/options#verify_incoming) - By default this is false, and should almost always be set
    to true to require TLS verification for incoming client connections. This applies to both server RPC and to the
    HTTPS API.

  - [`verify_incoming_https`](/consul/docs/agent/options#verify_incoming_https) - By default this is false, and should be set
  ...
  - [`verify_incoming_rpc`](/consul/docs/agent/options#verify_incoming_rpc) - By default this is false, and should almost
    always be set to true to require clients to provide a valid TLS certificate for Consul agent RPCs.

  - [`verify_outgoing`](/consul/docs/agent/options#verify_outgoing) - By default this is false, and should be set to true to
  ...
  - [`enable_agent_tls_for_checks`](/consul/docs/agent/options#enable_agent_tls_for_checks) - By default this is false, and
  ...
  - [`verify_server_hostname`](/consul/docs/agent/options#verify_server_hostname) - By default this is false, and should be
  ...
  - [`auto_encrypt`](/consul/docs/agent/options#auto_encrypt) - Enables automated TLS certificate distribution for client
    agent RPC communication using the Connect CA. Using this configuration a [`ca_file`](/consul/docs/agent/options#ca_file)
    and ACL token would still need to be distributed to client agents.

    - [`allow_tls`](/consul/docs/agent/options#allow_tls) - By default this is false, and should be set to true on server
      agents to allow certificates to be automatically generated and distributed from the Connect CA to client agents.

    - [`tls`](/consul/docs/agent/options#tls) - By default this is false, and should be set to true on client agents to
    ...
    **Example Client Agent TLS Configuration**
    ...
    -> The client agent TLS configuration from above sets [`verify_incoming`](/consul/docs/agent/options#verify_incoming) to
...
- **ACLs** - The access control list (ACL) system provides a security mechanism for Consul administrators to grant
  ...
  administrators should configure the [`default_policy`](/consul/docs/agent/options#acl_default_policy) to "deny".

  The [system](/consul/docs/security/acl/acl-system) is comprised of five major components:
...
- **Gossip Encryption** - A shared, base64-encoded 32-byte symmetric key is required to [encrypt Serf gossip
  ...
  [regularly rotated](https://developer.hashicorp.com/consul/docs/secure/encryption/gossip/enable#rotate-the-gossip-encryption-key) using
  ...
  - [`encrypt_verify_incoming`](/consul/docs/agent/options#encrypt_verify_incoming) - By default this is true to enforce
    encryption on _incoming_ gossip communications.

  - [`encrypt_verify_outgoing`](/consul/docs/agent/options#encrypt_verify_outgoing) - By default this is true to enforce
...
- **Customize TLS Settings** - TLS settings such as the [available cipher suites](/consul/docs/agent/options#tls_cipher_suites),
  should be tuned to fit the needs of your environment.

  - [`tls_min_version`](/consul/docs/agent/options#tls_min_version) - Used to specify the minimum TLS version to use.

  - [`tls_cipher_suites`](/consul/docs/agent/options#tls_cipher_suites) - Used to specify which TLS cipher suites are allowed.

  - [`tls_prefer_server_cipher_suites`](/consul/docs/agent/options#tls_prefer_server_cipher_suites) - Used to specify which TLS
...
- **Customize Default Limits** - Consul has a number of builtin features with default connection limits that should be
  ...
  - [`http_max_conns_per_client`](/consul/docs/agent/options#http_max_conns_per_client) - Used to limit concurrent access from
    a single client to the HTTP(S) endpoint on Consul agents.

  - [`https_handshake_timeout`](/consul/docs/agent/options#https_handshake_timeout) - Used to timeout TLS connection for the
    HTTP(S) endpoint for Consul agents.

  - [`rpc_handshake_timeout`](/consul/docs/agent/options#rpc_handshake_timeout) - Used to timeout TLS connections for the RPC
    endpoint for Consul agents.

  - [`rpc_max_conns_per_client`](/consul/docs/agent/options#rpc_max_conns_per_client) - Used to limit concurrent access from a
    single client to the RPC endpoint on Consul agents.

  - [`rpc_rate`](/consul/docs/agent/options#rpc_rate) - Disabled by default, this is used to limit (requests/second) for client
    agents making RPC calls to server agents.

  - [`rpc_max_burst`](/consul/docs/agent/options#rpc_max_burst) - Used as the token bucket size for client agents making RPC
    calls to server agents.

  - [`kv_max_value_size`](/consul/docs/agent/options#kv_max_value_size) - Used to configure the max number of bytes in a
    key-value API request.

  - [`txn_max_req_len`](/consul/docs/agent/options#txn_max_req_len) - Used to configure the max number of bytes in a
...
- **Secure UI Access** - Access to Consul’s builtin UI can be secured in various ways:
  ...
  - **Restrict HTTP Writes** - Using the [`allow_write_http_from`](/consul/docs/agent/options#allow_write_http_from)
