...
last_modified: 2026-07-14T23:23:33.000Z
...
[datacenter](/consul/docs/agent/options#_datacenter) within a given
...
[datacenter](/consul/docs/agent/options#_datacenter), and joined together to form
...
[`segments`](/consul/docs/agent/options#segments) server agent configuration option.
...
The server [agent configuration](/consul/docs/agent/options) options relevant to network
segments are:

- [`ports.serf_lan`](/consul/docs/agent/options#serf_lan_port): The Serf LAN port on this server
  for the `<default>` network segment's gossip pool.
- [`segments`](/consul/docs/agent/options#segments): A list of user-defined network segments
...
[`segment`](/consul/docs/agent/options#_segment) agent configuration option.

### Join a Client to a Segment ((#join_a_client_to_a_segment))

For a client agent to [join](/consul/commands/join) the Consul
...
must connect to Client B's configured [Serf LAN port](/consul/docs/agent/options#serf_lan_port).
...
for segment S](/consul/docs/agent/options#segment_port).
...
1. **Specify an explicit port in the join address**. This can be done at the CLI when starting
   ...
   configuration using the [retry-join option](/consul/docs/agent/options#retry_join). This method
   is not compatible with [cloud auto-join](/consul/docs/install/cloud-auto-join#auto-join-with-network-segments).
...
2. **Specify an alternate Serf LAN port for the agent**. This can be done at the CLI when starting
   ...
   the agent's configuration using the [serf_lan](/consul/docs/agent/options#serf_lan_port) option.
...
The client [agent configuration](/consul/docs/agent/options) options relevant to network
segments are:

- [`segment`](/consul/docs/agent/options#segment-2): The name of the network segment this
  client agent belongs to.
- [`ports.serf_lan`](/consul/docs/agent/options#serf_lan_port):
...
- [`retry_join`](/consul/docs/agent/options#retry_join) or
  [`start_join`](/consul/docs/agent/options#start_join): A list of agent addresses to join
