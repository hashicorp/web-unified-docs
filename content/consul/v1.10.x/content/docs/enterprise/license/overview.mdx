...
last_modified: 2026-07-14T23:23:30.000Z
...
For server agents this means that they must either have the [`license_path`](/consul/docs/agent/options#license_path)
...
actually joins the cluster, where to make those RPC requests to is inferred from the [`start_join`](/consul/docs/agent/options#start_join)
or [`retry_join`](/consul/docs/agent/options#retry_join) configurations. If those are both unset or no
[`agent` token](/consul/docs/agent/options#acl_tokens_agent) is set then the client agent will immediately shut itself down.
