...
last_modified: 2026-07-14T23:23:46.000Z
...
In Consul 0.7, the default server [performance parameters](/consul/docs/agent/options#performance)
...
[performance parameters](/consul/docs/agent/options#performance) back to Consul's original
...
- Spurious leader elections can be caused by networking
  ...
  parameters](/consul/docs/agent/options#performance) configuration now gives you tools
...
- For DNS-heavy workloads, configuring all Consul agents in a cluster with the
  [`allow_stale`](/consul/docs/agent/options#allow_stale) configuration option will allow reads to
...
- In Consul 0.9.3 and later, a new [`limits`](/consul/docs/agent/options#limits) configuration is
...
Since writes must be synced to disk (persistent storage) on a quorum of servers before they are committed, deploying a disk with high write throughput (or an SSD) will enhance performance on the write side. ([Documentation](/consul/docs/agent/options#_data_dir))
...
If the leader server is out of memory or the disk is full, the server eventually stops responding, loses its election and cannot move past its last commit time. However, by configuring `max_stale` and setting it to a large value, Consul will continue to respond to queries during such outage scenarios. ([max_stale documentation](/consul/docs/agent/options#max_stale)).
...
To prevent any CPU spikes from a misconfigured client, RPC requests to the server should be [rate limited](/consul/docs/agent/options#limits)
...
[`csr_max_per_second`](/consul/docs/agent/options#ca_csr_max_per_second) and
[`csr_max_concurrent`](/consul/docs/agent/options#ca_csr_max_concurrent).
