...
last_modified: 2026-07-14T23:19:52.000Z
...
definition](/consul/docs/agent/services) with one extra field `ContentHash` which
...
Note that this endpoint, unlike most also [supports `snake_case`](/consul/docs/agent/services#service-definition-parameter-case)
...
- `Name` `(string: <required>)` - Specifies the logical name of the service.
  ...
  for [compatibility with external DNS](/consul/docs/agent/services#service-and-tag-names-with-dns).
...
- `Kind` `(string: "")` - The kind of service. Defaults to "" which is a
  ...
  "mesh-gateway" for instances of a [mesh gateway](/consul/docs/connect/gateways/mesh-gateway),
...
- `Weights` `(Weights: nil)` - Specifies weights for the service. Please see the
  [service documentation](/consul/docs/agent/services) for more information about
