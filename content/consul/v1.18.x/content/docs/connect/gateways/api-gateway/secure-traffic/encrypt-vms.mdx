...
1. Create an [`inline-certificate` configuration entry](/consul/docs/connect/config-entries/inline-certificate) and specify the following fields:
   - `Kind`: Specifies the type of configuration entry. This must be set to `inline-certificate`.
   - `Name`: Specify the name in the [API gateway listener configuration](/consul/docs/connect/config-entries/api-gateway#listeners) to bind the certificate to that listener.  
...
1. Configure any additional fields necessary for your use case, such as the namespace or admin partition. Refer to the [`inline-certificate` configuration entry](/consul/docs/connect/config-entries/inline-certificate) reference for additional information.
