...
Refer to the [`consul resource` CLI command reference](/consul/commands/resource) to learn about using the Consul CLI to interact with resources.
...
You can also use the `consul resource` command to return information about Consul resources in each group using a `group.groupVersion.kind` syntax. Refer to [`consul resource`](/consul/commands/resource) for more information.
