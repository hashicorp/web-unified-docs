...
last_modified: 2026-04-21T13:06:30.000Z
...
You may set limits on the rate of read and write requests that affect individual
servers in the datacenter. You can set limits for all source IP addresses, which
lets you specify a budget for read and write requests to prevent any single
source IP from overwhelming the Consul server and negatively affecting the
network. Enterprise users may also set up cluster-wide rate limits for Consul to
apply in case of intense traffic peak periods. Cluster-wide rate limits help to
avoid server malfunctions and DoS attacks in order to keep your datacenter
operational.
...
![Flowchart that describes the order of operations for determining request rate limits.](/img/rate-limiting-ops-order.png#light-theme-only)
![Flowchart that describes the order of operations for determining request rate limits.](/img/rate-limiting-ops-order-dark.png#dark-theme-only)
...
- The [Helm chart reference](/consul/docs/reference/k8s/helm) for additional information. 

- [Control plane request limit configuration entry
reference](/consul/docs/reference/config-entry/control-plane-request-limit) for
information about applying a CRD for limiting traffic rates from source IPs
- [Global rate limit configuration entry
  reference](/consul/docs/reference/config-entry/rate-limit) for information
  about applying a CRD for limiting traffic rates cluster-wide
