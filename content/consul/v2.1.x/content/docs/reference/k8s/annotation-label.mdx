...
created_at: 2026-03-31T20:45:50.000Z
last_modified: 2026-03-31T20:45:50.000Z
...
- `consul.hashicorp.com/connect-service-port` - For pods that accept inbound
  connections, this specifies the port or ports to route inbound connections to. If not specified, the behavior depends on your Consul edition:

  - **Consul Enterprise**: Defaults to all exposed container ports and registers a multiport service. Multiport service registration is a Consul Enterprise feature.
  - **Consul CE**: Defaults to all exposed container ports but registers only the first port. To register a specific port, you must set this annotation to that port.

  If specified, the value can be the _name_ of a configured port, such as `http`, or a direct port value such as `8080`. This is the port of the _service_; the proxy public listener listens on a dynamic port.

- `consul.hashicorp.com/connect-service-default-port` - For pods that register multiple ports, this specifies the default port for the Consul service registration. The default port is the first port from the `consul.hashicorp.com/connect-service-port` annotation, or the first exposed container port if that annotation is not set.

  When multiple ports are registered, Consul routes virtual address traffic without a port prefix to this default port. For example, `<service_name>.virtual.consul` routes to the same port as `<default_port>.<service_name>.virtual.consul`.
...
- `consul.hashicorp.com/connect-service-upstreams` - The list of upstream
  ...
  this annotation is optional. This annotation can be either _labeled_ or _unlabeled_. We recommend the labeled format because it has a more consistent syntax and can be used to reference cluster peers as upstreams.
    ...
    When transparent proxy is disabled, you must use this annotation to define all upstream services, including multi-port services. To configure a multi-port upstream, specify each destination port using the `destination_port` parameter. The following example configures `service-response` as an upstream with three named ports:
    ...
    **Note:** Kubernetes annotations have a character limit(256KB). This limits the number of upstreams you can specify for a service using this annotation. Consul does not support a combination of protocols for a multi-port service. All ports of a service must use the same protocol.
