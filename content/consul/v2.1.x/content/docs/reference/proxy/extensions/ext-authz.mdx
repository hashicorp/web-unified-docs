...
created_at: 2026-03-31T20:45:53.000Z
last_modified: 2026-03-31T20:45:53.000Z
# END AUTO GENERATED METADATA
---

# External authorization extension configuration reference

This topic describes how to configure the external authorization Envoy extension, which configures Envoy proxies to request authorization from an external service. 

Refer to [Delegate authorization to an external service](/consul/docs/envoy-extension/ext) for usage information.
...
- [`Arguments`](#arguments): map | required
   ...
   - [`Config`](#arguments-config): map | required
      ...
      - [`AllowedHeaders`](#arguments-config-allowedheaders): list
         - [`Contains`](#arguments-config-allowedheaders): string
         - [`Exact`](#arguments-config-allowedheaders): string
         - [`IgnoreCase`](#arguments-config-allowedheaders): boolean
         - [`Prefix`](#arguments-config-allowedheaders): string
         - [`SafeRegex`](#arguments-config-allowedheaders): string
         - [`Suffix`](#arguments-config-allowedheaders): string
      - [`DisallowedHeaders`](#arguments-config-disallowedheaders): list
         - [`Contains`](#arguments-config-disallowedheaders): string
         - [`Exact`](#arguments-config-disallowedheaders): string
         - [`IgnoreCase`](#arguments-config-disallowedheaders): boolean
         - [`Prefix`](#arguments-config-disallowedheaders): string
         - [`SafeRegex`](#arguments-config-disallowedheaders): string
         - [`Suffix`](#arguments-config-disallowedheaders): string
...
Specifies the type of Envoy proxy that this extension applies to. The extension only applies to proxies that match this type and is ignored for all other proxy types. Supported values are `connect-proxy` and, on Consul Enterprise, `api-gateway`.

When set to `api-gateway`, Consul applies external authorization to requests that the API gateway routes to upstream services. <EnterpriseAlert inline /> Support for the `api-gateway` proxy type was introduced in Consul Enterprise 2.0.2. The `api-gateway` proxy type also relaxes the [`Target.URI`](#arguments-config-grpcservice-target-uri) localhost restriction so that the authorization service can run on a remote host. Refer to [Delegate authorization for API gateways to an external service](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/vm) for usage information.
...
- Data type is one of the following string values:
  - `connect-proxy`
  - `api-gateway` <EnterpriseAlert inline />
...
Restricts which client request headers Envoy forwards to the external authorization service in the check request. When set, only client request headers whose names match one of the configured [string matchers](#string-matcher-configuration) are sent to the authorization service. This field applies to both the gRPC and HTTP transports.

When unset, Envoy keeps its default behavior: all client request headers are sent to a gRPC authorization service, whereas only a small default set (plus any [`HttpService.AuthorizationRequest.AllowedHeaders`](#arguments-config-httpservice-authorizationrequest)) are sent to an HTTP authorization service. This field is the only way to restrict the request headers sent to a gRPC authorization service.

#### Values

- Default: None
- Data type: List of [string matchers](#string-matcher-configuration)

### `Arguments.Config.DisallowedHeaders`

Prevents Envoy from forwarding the listed client request headers to the external authorization service. Client request headers whose names match one of the configured [string matchers](#string-matcher-configuration) are never sent to the authorization service, even when they also match [`AllowedHeaders`](#arguments-config-allowedheaders). This field applies to both the gRPC and HTTP transports and takes precedence over `AllowedHeaders` when a header matches both.
...
Each entry in `AllowedHeaders` and `DisallowedHeaders` matches a client request header name using exactly one of the following fields:

| Parameter | Description | Data type | Default |
| ---       | ---         | ---       | ---     |
| `Contains` | Matches when the header name contains this substring. | String | None |
| `Exact` | Matches when the header name equals this value exactly. | String | None |
| `Prefix` | Matches when the header name begins with this value. | String | None |
| `SafeRegex` | Matches when the header name matches this RE2 regular expression. | String | None |
| `Suffix` | Matches when the header name ends with this value. | String | None |
| `IgnoreCase` | When `true`, performs the match case-insensitively. | Boolean | `false` |
...
Specifies the URI of the external authorization service. Configure this field when you must provide an explicit URI to the external authorization service, such as cases in which the authorization service is running on the same host or pod. When [`ProxyType`](#arguments-proxytype) is `connect-proxy`, the value of this field must be one of `localhost:<port>`, `127.0.0.1:<port>`, or `::1:<port>`. When `ProxyType` is `api-gateway`, this restriction is relaxed and the URI may point to a remote host. <EnterpriseAlert inline />
...
### Authorize API gateway requests to a remote authorization service <EnterpriseAlert inline />
...
In the following example for Consul Enterprise, a service defaults configuration entry applies the extension to an API gateway named `api-gateway`. Because [`ProxyType`](#arguments-proxytype) is `api-gateway`, the [`Target.URI`](#arguments-config-grpcservice-target-uri) can point to a remote authorization service. The configuration also restricts which client request headers Envoy forwards to the authorization service:
...
Refer to [Delegate API gateway authorization to an external service on virtual machines](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/vm) and [on Kubernetes](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/k8s) for complete workflows, including how to set the gateway-wide and per-route authorization behavior.
