...
In order to configure an Auth0 application for Consul, you will first need to take note of your Consul agent address. This is the address that you use to access Consul, either via the UI or the CLI. To fetch it, run the following command:
...
In order to enable the OIDC auth method for Consul, you need to create a new auth method configuration that points to your Auth0 application. The contents of this configuration file depend on which way you supply it to Consul. You can either use the Consul CLI or the HTTP API.
...
After you create the configuration file, apply it to Consul for the new auth method to take effect. Note that the configuration file contents differ slightly depending on whether you use the CLI or the HTTP API.
