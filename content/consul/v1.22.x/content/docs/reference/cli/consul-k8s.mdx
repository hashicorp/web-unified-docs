...
last_modified: 2026-08-27T21:30:07.000Z
...
Supported values are: `all`, `helm`, `crds`, `sidecar`, `logs`, `proxy`. By default, the `-capture` flag captures `all` components.
...
| <nobr>`logs`</nobr>     | Captures logs from key Consul component pods: consul-server, consul-client, connect-injector, and gateways. <br/> It also records each pod’s log-capture status in an audit file. <br/> If multiple errors occur during capture, this target writes them to a single error file.|
...
| <nobr>`logs`</nobr>     | pattern: <br/>`debugfile/logs/<componentType>/<componentName>/<podName>/<containerName.log>` <br/> example: <br/>`debugfile/logs/statefulsets/consul-server/consul-server-0/consul.log` <br/><br/>`debugfile/logCaptureAudit.txt` <br/> `debugfile/logCaptureErrors.txt`        |
...
- `API Gateway`: These pods run a proxy to manage connections with networks
  outside of the Consul cluster. Read more about [API gateways](/consul/docs/north-south/api-gateway).
...
Consul, as in the [service mesh multi-port configuration](/consul/docs/k8s/connect#kubernetes-pods-with-multiple-ports),
