...
These requirements will vary based on the size of the Consul cluster, the number of client agents, and the workload characteristics (e.g. read-heavy vs write-heavy). While Consul may run on smaller hardware configurations, the above specifications are recommended to ensure optimal performance and reliability in production environments, and will help avoid having to upgrade hardware during a capacity outage.

The following table gives a recommended starting point for hardware specs for each major cloud infrastructure provider.
...
[RAFT](/consul/docs/concept/consensus) is the consensus algorithm that Consul nodes use. The default server [performance parameters](/consul/docs/reference/agent/configuration-file/general#performance) are tuned to allow Consul to run reliably (but relatively slowly) on a server cluster of three [AWS t2.micro](https://aws.amazon.com/ec2/instance-types/) instances. These thresholds were determined empirically using a leader instance that was under sufficient read, write, and network load to cause it to permanently be at zero CPU credits, forcing it to the baseline performance mode for that instance type. Real-world workloads typically have more bursts of activity, so this is a conservative and pessimistic tuning strategy.
...
The default RAFT performance configuration is equivalent to this:
...
When running Consul in production, we recommend configuring the server [performance parameters](/consul/docs/reference/agent/configuration-file/general#performance) with the lowest possible RAFT multiplier setting. This will let Consul servers detect a failed leader and complete leader elections quicker than the default configuration, at the expense of higher resource usage. We recommend the following high-performance configuration for Consul clusters, unless there is a specific reason to use the slower default configuration - for example, running on low-end hardware, on a high-latency network, or on instances sharing workloads.

The recommended high performance configuration looks like this:
