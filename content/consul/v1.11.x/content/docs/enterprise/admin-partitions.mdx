...
1. Create a server configuration values file to override the default Consul Helm chart settings:
  ...
  global:
    ...
    tls:
      enabled: true
    image: hashicorp/consul-enterprise:1.11.5-ent
    adminPartitions:
      enabled: true
    acls:
...
1. Install the Consul server(s) using the values file created in the previous step:
  ...
  ```shell-session
  $ helm install ${HELM_RELEASE_SERVER} hashicorp/consul --version "0.42.0" --create-namespace --namespace consul --values server.yaml
  ```
...
1. Install the workload client clusters:
  ...
  ```shell-session
  $ helm install ${HELM_RELEASE_CLIENT} hashicorp/consul --version "0.42.0" --create-namespace --namespace consul --values client.yaml 
  ```
