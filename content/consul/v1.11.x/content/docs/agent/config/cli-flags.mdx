...
-> **Note:** Some CLI arguments may be different from HCL keys. See [Configuration Key Reference](/docs/agent/options#config_key_reference) for equivalent HCL Keys.
...
- `-check_output_max_size` - Override the default
  ...
  capture, it is possible to use [`discard_check_output`](/docs/agent/options#discard_check_output).
...
- `-raft-protocol` ((#\_raft_protocol)) - This controls the internal version
  ...
  to 3 in order to gain access to Autopilot features, with the exception of [`cleanup_dead_servers`](/docs/agent/options#cleanup_dead_servers). Defaults to 3 in Consul 1.0.0 and later (defaulted to 2 previously). See [Raft Protocol Version Compatibility](/docs/upgrading/upgrade-specific#raft-protocol-version-compatibility) for more details.
...
- `-segment` ((#\_segment)) <EnterpriseAlert inline /> - This flag is used to set
  ...
  ~> **Warning:** The `segment` flag cannot be used with the [`partition`](/docs/agent/options#partition-1) option.
...
- `-advertise-wan` ((#\_advertise-wan)) - The advertise WAN address is used
  ...
  This can also be set on client agents when used in combination with the [`translate_wan_addrs`](/docs/agent/options#translate_wan_addrs) configuration option. By default, the [`-advertise`](#_advertise) address
  ...
  accessed from a remote datacenter if the remote datacenter is configured with [`translate_wan_addrs`](/docs/agent/options#translate_wan_addrs). In Consul 1.1.0 and later this can be dynamically defined with a [go-sockaddr]
  template that is resolved at runtime.
...
- `-config-file` ((#\_config_file)) - A configuration file to load. For
  more information on the format of this file, read the [Configuration Files](/docs/agent/options)
...
- `-config-dir` ((#\_config_dir)) - A directory of configuration files to
  ...
  the [Configuration Files](/docs/agent/options) section.
...
- `-recursor` ((#\_recursor)) - Specifies the address of an upstream DNS
  ...
  to the [`recursors` configuration option](/docs/agent/options#recursors).
...
- `-retry-join` ((#\_retry_join)) - Similar to [`-join`](#_join) but allows retrying a join until
  ...
  using the [`ports.serf_lan`](/docs/agent/options#serf_lan_port) configuration option or
