...
that does not require direct interaction with Helm, the [Consul CLI](/commands),
or `kubectl`.
...
- `Mesh Gateway`: These pods run a proxy to manage connections between
  Consul clusters connected using mesh federation. Read more about [Consul Mesh Federation](/docs/k8s/deployment-configurations/multi-cluster/kubernetes).
