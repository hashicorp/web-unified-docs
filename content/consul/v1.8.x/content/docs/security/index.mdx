...
The [gossip protocol](/docs/architecture/gossip) is powered by [Serf](https://www.serf.io/),
which uses a symmetric key, or shared secret, cryptosystem. There are more
details on the security of [Serf here](https://www.serf.io/docs/security/security-models.html).
For details on how to enable Serf's gossip encryption in Consul, see the
[encryption doc here](/docs/security/encryption).
...
- **Script checks enabled with network-exposed API.** If a Consul agent (client
  ...
  [`enable_script_checks`](/docs/agent/options#_enable_script_checks) must
...
For configuring network rules to support Consul, please see [Ports Used](/docs/agent/options#ports)
