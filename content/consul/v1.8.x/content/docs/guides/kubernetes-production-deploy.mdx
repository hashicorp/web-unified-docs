...
- Enable
  [encryption](/docs/security/encryption#gossip-encryption) to secure gossip traffic within the Consul cluster.
...
system](/docs/security/acl/acl-system).

Read the Consul Helm chart documentation to review all the [global
parameters](/docs/k8s/helm#v-global).
...
servers](/docs/architecture/consensus#deployment-table)
...
parameters](/docs/k8s/helm#v-server)
...
parameters](/docs/k8s/helm#v-client)
...
parameters](/docs/k8s/helm#v-connectinject).
