...
[Helm chart
values](/docs/k8s/helm#configuration-values)
...
documentation](/docs/k8s/helm) to learn more
about the chart.
...
balancer service for the UI, and enables [affinity](/docs/k8s/helm#v-server-affinity) to allow only one
...
process. The [catalog sync](/docs/k8s/helm#v-synccatalog) process will sync
