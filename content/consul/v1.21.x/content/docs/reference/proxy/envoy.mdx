...
<Warning>
  ...
  The `envoy_extra_static_listeners_json`, `envoy_extra_static_clusters_json`,
  `envoy_extra_stats_sinks_json`, `envoy_stats_config_json`, and `envoy_tracing_json`
  keys accept arbitrary Envoy protobuf JSON that is spliced verbatim into the sidecar
  bootstrap configuration. They can introduce code-executing HTTP filters — including
  [Lua](https://www.envoyproxy.io/docs/envoy/latest/configuration/http/http_filters/lua_filter)
  and [Wasm](https://www.envoyproxy.io/docs/envoy/latest/configuration/http/http_filters/wasm_filter)
  — that run as the Envoy process user on every proxied request, with access to the
  sidecar's mTLS private keys and the host filesystem.

  **Required ACL grants:** A token must hold both `service:write` on the proxy service
  and `mesh:write` to set any of these keys on a service registration or config entry.
...
<Warning>
  ...
  The `envoy_bootstrap_json_tpl`, `envoy_public_listener_json`,
  `envoy_listener_tracing_json`, `envoy_local_cluster_json`, `envoy_listener_json`,
  and `envoy_cluster_json` keys accept arbitrary
  Envoy protobuf JSON delivered verbatim to the sidecar. They can introduce
  code-executing HTTP filters that run as the Envoy process user on every proxied
  request, with access to the sidecar's mTLS private keys and the host filesystem.
