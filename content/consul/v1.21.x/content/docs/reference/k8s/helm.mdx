...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `tls` ((#v-global-tls)) - Enables [TLS](/consul/tutorials/security/tls-encryption-secure)
    ...
    - `verify` ((#v-global-tls-verify)) (`boolean: true`) - If true, `verify_outgoing`, `verify_server_hostname`,
      ...
      Please refer to [TLS on existing clusters](/consul/docs/secure/encryption/tls/enable/existing/k8s)
      for more details.
...
- `server` ((#v-server)) - Server, when enabled, configures a server cluster to run. This should
  ...
  - `extraConfig` ((#v-server-extraconfig)) (`string: {}`) - A raw string of extra [JSON configuration](/consul/docs/reference/agent/configuration-file) for Consul
  ...
  - `limits` ((#v-server-limits)) - Settings for potentially limiting timeouts, rate limiting on clients as well
    ...
    - `requestLimits` ((#v-server-limits-requestlimits)) - This object specifies configurations that limit the rate of RPC and gRPC
      ...
      also limits HTTP requests to the Consul server. Refer to [Consul agent
      configuration `request_limits`](/consul/docs/reference/agent/configuration-file/general#request_limits)
...
- `meshGateway` ((#v-meshgateway)) - [Mesh Gateways](/consul/docs/east-west/mesh-gateway) enable Consul Connect to work across Consul datacenters.
