...
You can safely store the license for Consul enterprise in Vault. This allows Consul agents to retrieve the license from Vault during initialization. Store the license from the `consul.hclic` local file in Vault using the `consul/` instance of the `kv` secrets engine.
