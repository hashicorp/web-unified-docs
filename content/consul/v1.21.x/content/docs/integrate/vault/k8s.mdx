...
This page describes how to securely deploy Consul on Kubernetes using Vault as a secrets manager. Using Vault, you can securely store and centrally manage the secrets you will need to provide to your Consul agents. These include, but are not limited to, gossip encryption keys, TLS certificates, service mesh certificates. Furthermore, you can also [store and manage your Consul Enterprise license](/consul/docs/deploy/server/k8s/vault/data/enterprise-license).
...
1. Store enterprise license in Vault
1. Store Consul gossip key in Vault
1. Setup PKI secrets engine for TLS and service mesh CA
1. Authenticate using a Kubernetes service account using the Vault Kubernetes authentication engine
...
First, if key/value v2 secrets engine `kv-v2` is not already enabled, enable it in Vault. In this example, we enable it at the `consul/` path.
...
You can safely store the license for Consul enterprise in Vault. This allows Consul agents to retrieve the license from Vault during initialization. In this example, the license is called `consul.hclic`. Store the license in Vault using the `consul/` instance of the `kv` secrets engine.
...
After you just stored the Consul license in the Vault `kv` secrets engine, you will need to define a policy that grants access to the path where you stored the license.
...
In this example, the `RootPKIPath` is `connect-root` and the `IntermediatePKIPath` is `connect-intermediate-dc1`. Update these values to reflect your Vault environment.
...
The chart configures a Kubernetes service account named `vault` that you will use to enable Vault communication with Kubernetes. Retrieve the JSON Web Token (JWT) for the `vault` service account and set it to the `token_reviewer_jwt` environment variable.
...
You can read more on Consul Helm chart configuration value, to tune your Consul installation at [Helm Chart Configuration](/consul/docs/reference/k8s/helm).
