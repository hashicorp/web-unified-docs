...
The `http://<consul_agent_address>:8550/oidc/callback` address is for logins with the CLI `consul login -type=oidc -method=oidc` command.

The `http://<consul_agent_address>:8500/ui/oidc/callback` is for logins with the Consul UI.
...
Retrieve the following application parameters from Auth0:
