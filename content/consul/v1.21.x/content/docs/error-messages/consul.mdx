...
last_modified: 2026-08-28T14:20:49.000Z
...
[Half-open TCP connections shut down completely](https://github.com/moby/libnetwork/issues/1204) in Docker, but may manifest in other environments as well.
...
1. Perform a [`consul leave`](/consul/commands/leave) on each server and then start the server again. Do this one server at a time, and make sure to leave the leader server for last so that you minimise the downtime incurred. Once all servers are restarted, the node ids will be set to the expected value, resolving the errors in the logs.
