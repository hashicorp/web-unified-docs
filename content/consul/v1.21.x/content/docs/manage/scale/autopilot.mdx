...
| :------------------------ | :------- | :------------ | :-------------------------------------------------------------------------------------------------------------------------------- | 
| `CleanupDeadServers`      | Boolean  | `true`        | Flag to enable periodic dead servers removal from the Raft peer set.                                                              |
| `LastContactThreshold`    | Duration | `200ms`       | Time duration that defines the maximum time since the last contact with the current leader for a server to be considered healthy. |
