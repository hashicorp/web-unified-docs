...
- Helm version `3.6+`. Visit the [Helm website](https://helm.sh/docs/intro/install/) to download the latest version.
- [`kubectl`](https://kubernetes.io/docs/reference/kubectl/) configured to point to the Kubernetes cluster you want to install Consul on.

Depending on your requirements you can:

- Perform a [default installation](#install-consul) by using the default Helm chart configuration. Use this option if you are testing Consul or setting up a test environment that does not have security requirements.
- Perform a [custom installation](#custom-installation) by modifying Helm chart configuration parameters. This is the recommended approach if you are installing Consul in production.
...
1. Verify that you have access to the Consul chart. As of this writing, Helm chart version `1.8.16` is the latest that supports Consul `v1.21.x` on Kubernetes:

   ```shell-session
   $ helm search repo hashicorp/consul --versions | egrep "NAME|1.21." | head -n2
   NAME            	CHART VERSION	APP VERSION	DESCRIPTION
   hashicorp/consul	1.8.16       	1.21.5     	Official HashiCorp Consul Chart
   ```
...
1. Run the following Helm command to install the latest `v1.21.x` version of Consul on Kubernetes with its default configuration, using the `1.8.16` Helm chart version. You can also select the Kubernetes namespace of your choosing by modifying the value of the `--namespace` flag for the Helm install.
...
- [Minimal Consul service mesh installation](#minimal-values-yaml-for-consul-service-mesh)
- [Select API gateway configuration](#select-api-gateway-configuration)
- [Install Consul on OpenShift clusters](#install-consul-on-openshift-clusters)
- [Enable Consul CNI plugin](#enable-the-consul-cni-plugin)
- [Enable Consul service mesh on selected namespaces](#enable-consul-service-mesh-on-select-namespaces)
...
1. Check if your cluster uses `TCPRoute`.
...
- If the OCP version is `4.19` or later and no `TCPRoute` resources are present, use the **OpenShift >= 4.19, no TCPRoute required** tab.
- If the OCP version is `4.19` or later and `TCPRoute` resources are present, use the **OpenShift >= 4.19, TCPRoute required** tab.
- If the OCP version is `4.18` or earlier, use the **OpenShift <= 4.18** tab.

<Tabs>
  <Tab heading="OpenShift <= 4.18 (Standard API gateway)">
...
```
  ...
  <Tab heading="OpenShift >= 4.19, no TCPRoute required (Standard API gateway)">
...
```
  ...
  <Tab heading="OpenShift >= 4.19, TCPRoute required (Consul API gateway)">
...
If you already installed Consul and want to make changes, you need to run `helm upgrade`. Refer to [Upgrading Consul on Kubernetes components](/consul/docs/upgrade/k8s) for more details.
...
@include 'alerts/gke-autopilot-cni-install.mdx'
