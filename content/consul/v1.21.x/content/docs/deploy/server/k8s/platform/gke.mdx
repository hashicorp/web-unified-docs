...
When using Workload Identity Federation (WIF) for GKE, the `consul-server-acl-init` container needs to have a custom service account that has the necessary permissions to query Google Compute resources. First, generate a GCP IAM service account definition using Helm template and your Consul values file. 
...
Then create your Consul namespace and make sure your namespace name matches the one you will use to deploy Consul.
