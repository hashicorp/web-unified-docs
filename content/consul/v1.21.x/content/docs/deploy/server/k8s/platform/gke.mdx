...
The content of the file differs based on whether you are using Workload Identity Federation (WIF) for GKE or not.

<Tabs>

<Tab heading="GKE (default)" group="wi-disabled">
...
<Tab heading="GKE using Workload Identity Federation" group="wi-enabled">

If you do use Workload Identity Federation (WIF) for GKE to manage access between your GKE cluster and GCP resources, you need to add an annotation to the Consul client service account. Replace `GSA_NAME` and `PROJECT_ID` with your GCP service account name and project ID.
...
When using Workload Identity Federation (WIF) for GKE, the `consul-server-acl-init` container requires a custom service account that has the necessary permissions to query Google Compute resources. 
