...
When you use Consul's service discovery features to return a registered service instance, Consul's forwards the request to the cluster's leader by default. That way, Consul returns the most recent authoritative results from the catalog.

You can change a Consul agent's consistency mode so that they return services with a greater or lower degree of accuracy, depending on the needs of the workloads in your service networking environment. 
