...
Before Consul 1.21.x, API gateways were managed by the API group `gateway.networking.k8s.io`.  This configuration is referred to as *Standard API gateway*.

Consul 1.21.x introduces the Consul API gateway controller and the new API gateway CRDs under the API Group `consul.hashicorp.com`. This configuration is referred to as *Consul API gateway*.
...
<Tabs>
  ...
  The following working example enables both Consul service mesh and the Consul API gateway on GKE Autopilot. Refer to [`connectInject.agiGateway` in the Helm chart reference](https://developer.hashicorp.com/consul/docs/reference/k8s/helm#v-connectinject-apigateway) for additional information.
...
1. Specify the correct controller name that matches the gateway type.
