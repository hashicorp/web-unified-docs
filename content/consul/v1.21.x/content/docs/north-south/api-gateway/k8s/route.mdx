...
1. Create a configuration file and specify the following fields:
  ...
  - `spec. rules`: Specifies a list of routing rules for constructing a routing table that maps listeners to services.
    ...
    Refer to the [`Routes` configuration reference](/consul/docs/reference/k8s/api-gateway/routes) for details about configuring route rules, and review [the Enable API gateway for Kubernetes guide](/consul/docs/north-south/api-gateway/k8s/enable) for guidance on selecting the right gateway type.
