...
<Warning>
  ...
  The `builtin/wasm` extension loads and executes a WebAssembly module inside Envoy's
  HTTP filter chain on every proxied request. This constitutes **arbitrary code
  execution** as the Envoy process user, with access to request and response bodies,
  headers, the sidecar's mTLS private keys, and the host filesystem.
  ...
  **Required ACL grants to attach this extension:**
  ...
  Attaching `builtin/wasm` to `proxy-defaults` requires `mesh:write` only, since
