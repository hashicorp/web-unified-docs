...
<Warning>
  ...
  The `builtin/lua` extension compiles and executes the `Script` argument as a Lua
  program inside Envoy's HTTP filter chain on every proxied request. This constitutes
  **arbitrary code execution** as the Envoy process user, with access to request and
  response bodies, headers, the sidecar's mTLS private keys, and the host filesystem.

  **Required ACL grants to attach this extension:**

  - `service:write` on the target service (to write the `service-defaults` config entry), **and**
  - `mesh:write` (to authorize code-executing extensions)

  Attaching `builtin/lua` to `proxy-defaults` requires `mesh:write` only, since
  `ProxyConfigEntry.CanWrite` already enforces that rule.

  **Residual risk — `operator:write` tokens:** By default, a policy that grants
  `operator:write` but does not explicitly set a `mesh` rule will implicitly receive
  `mesh:write` capability. Operators should audit any token that holds `operator:write`
  and, for tokens that should not have mesh-write capability (such as backup automation
  or force-leave responders), explicitly add `mesh = "deny"` to the policy.
