...
created_at: 2026-03-31T20:43:37.000Z
last_modified: 2026-03-31T20:43:37.000Z
...
- `TokenNameFormat` `(string <optional>)` - Defines the token name format for
   the generated tokens. This can be templated using HashiCorp Interpolation
   Language. Defaults to `${auth_method_type}-${auth_method_name}`.

  For all auth_method_type values, these variables are available for
  interpolation:
  
  - `auth_method_type`
  - `auth_method_name`

  Use `value.<claim_mapping_value>` for OIDC and JWT claims mapping values.
  
  For an OIDC auth method, `given_name` and `family_name` claim mapping are
  available if sent by the auth provider.

  Example OIDC `TokenNameFormat`:

  `'${auth_method_type}-${auth_method_name}-${value.given_name}-${value.family_name}'`

  For a JWT auth method, `preferred_username` and `sub` claim
  mappings are available if sent by the auth provider. 
  
  Example JWT `TokenNameFormat`:

  `'${auth_method_type}-${auth_method_name}-${value.given_name}-${value.family_name}-${value.preferred_username}-${value.sub}'`
...
- `TokenNameFormat` `(string <optional>)` - Defines the token name format for the generated
  tokens. This can be lightly templated using HIL `${foo}` syntax.
