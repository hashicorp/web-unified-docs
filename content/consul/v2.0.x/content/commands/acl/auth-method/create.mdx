...
created_at: 2026-03-31T20:43:50.000Z
last_modified: 2026-03-31T20:43:50.000Z
...
- `-token-name-format=<string>` - Defines the token name format for the generated
   tokens This can be lightly templated using HIL ${foo} syntax. 
   Defaults to ${auth_method_type}-${auth_method_name}.

  For all auth_method_type values, by default following variables are available for interpolation:
    - `auth_method_type`
    - `auth_method_name`

  Additional for OIDC and JWT claims mapping values are also available in the form of `value.<claim_mapping_value>`.
  For an OIDC auth method - `given_name` and `family_name` claim mapping are available if sent by the auth provider.
  Example OIDC TokenNameFormat
  `'${auth_method_type}-${auth_method_name}-${value.given_name}-${value.family_name}'`
  Additionally for an JWT auth method `preferred_username` and `sub` claim mapping are available if sent by the auth provider.
