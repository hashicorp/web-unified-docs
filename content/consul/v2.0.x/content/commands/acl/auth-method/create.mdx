...
- `-token-name-format=<string>` - Defines the token name format for the
   generated tokens. This can be templated using HashiCorp Interpolation
   Language. Defaults to '${auth_method_type}-${auth_method_name}'.

  The variables `auth_method_type` and `auth_method_name` are always available
  for interpolation.
