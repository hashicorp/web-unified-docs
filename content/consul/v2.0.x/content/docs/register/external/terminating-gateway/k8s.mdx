...
created_at: 2026-03-31T20:45:56.000Z
last_modified: 2026-03-31T20:45:56.000Z
...
You can also specify a [`secretRef`](/consul/docs/reference/config-entry/terminating-gateway#secretref) along with the file path fields (`caFile`, `certFile`, `keyFile`) to enable TLS via Envoy's Secret Discovery Service (SDS). Both the `secretRef` name and the corresponding file paths must be specified — the file paths indicate where the Kubernetes secret is mounted on the terminating gateway pod's filesystem, and the `secretRef` name identifies the Kubernetes secret so that certificate updates are dynamically delivered to Envoy via SDS without requiring a gateway restart.

<CodeBlockConfig filename="terminating-gateway-tls.yaml">
