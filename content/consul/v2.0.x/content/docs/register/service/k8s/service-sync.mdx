...
created_at: 2026-03-31T20:45:59.000Z
last_modified: 2026-03-31T20:45:59.000Z
...
When using service discovery features, Consul automatically syncs its catalog with the Kubernetes service catalog. Both single and multi-port services are available. If a Kubernetes service has multiple ports, Consul registers the first port in the series as the service's default unless you specify a default with the `consul.hashicorp.com/service-port` annotation.

For Consul service mesh, the catalog must register the service at a single port. Consul uses the service's default port for service mesh operations.

The following example demonstrates how to override the default service port with the `consul.hashicorp.com/service-port` annotation. It sets the default port for the `my-service` Service to `http`.
...
The annotation value may be a name of a port or an exact port value. We recommend using ports with unique names.
