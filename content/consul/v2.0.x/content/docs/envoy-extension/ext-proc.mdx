...
Consul applies Envoy extensions configured in proxy defaults before it applies extensions in service defaults. As a result, the Envoy extension configuration in service defaults will override configurations in proxy defaults.
...
- Review the [external processing extension configuration reference](/consul/docs/reference/proxy/extensions/ext-proc) for all available fields and defaults.
