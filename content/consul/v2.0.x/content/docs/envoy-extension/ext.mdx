...
After you apply the extension, use the [API gateway configuration entry](/consul/docs/reference/config-entry/api-gateway#extauthz) to set the gateway-wide authorization behavior and the [HTTP route `ExtAuthz` filter](/consul/docs/reference/config-entry/http-route#rules-filters-extauthz) to override it for specific routes. Refer to [Delegate API gateway authorization to an external service on virtual machines](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/vm) and [on Kubernetes](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/k8s) for the complete workflows.
