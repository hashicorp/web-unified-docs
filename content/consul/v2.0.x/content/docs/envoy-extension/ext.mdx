...
last_modified: 2026-07-22T06:29:33.000Z
...
The service defaults extension enables external authorization for all routes behind the gateway by default. If you want to enable external authorization only for specific routes, use the [API gateway configuration entry](/consul/docs/reference/config-entry/api-gateway#extauthz) to disable the gateway-wide default and the [HTTP route `ExtAuthz` filter](/consul/docs/reference/config-entry/http-route#rules-filters-extauthz) to override it for specific routes.
