...
last_modified: 2026-07-14T16:53:58.000Z
...
The service defaults extension sets the authorization behavior for the whole cluster. If you want to enable external authorization only for specific routes, use the [API gateway configuration entry](/consul/docs/reference/config-entry/api-gateway#extauthz) to disable the global external authorization for the gateway and the [HTTP route `ExtAuthz` filter](/consul/docs/reference/config-entry/http-route#rules-filters-extauthz) to override it for specific routes. 

Refer to [Delegate API gateway authorization to an external service on virtual machines](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/vm) and [on Kubernetes](/consul/docs/north-south/api-gateway/secure-traffic/ext-authz/k8s) for the complete workflows.

<Warning>
  Adding Envoy extensions in proxy defaults configuration entries may have unintended consequences. We recommend configuring `EnvoyExtensions` in service defaults configuration entries in most cases.
</Warning>
