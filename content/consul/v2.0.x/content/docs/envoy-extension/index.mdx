...
The `ext-proc` extension lets you configure external processing filters for Envoy proxy so that you can send requests and responses to an external service for inspection and mutation. Refer to the [external processing documentation](/consul/docs/envoy-extension/ext-proc) for more information.
