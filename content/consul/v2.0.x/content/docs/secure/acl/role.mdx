...
1. Assemble rules into [policies](/consul/docs/secure/acl/policy) and register them in Consul.
...
Use the Consul CLI or API endpoint to create roles.
...
Refer to the [CLI documentation](/consul/commands/acl/role) for details.
...
Refer to the [rules reference](/consul/docs/secure/acl/rule) for information about the rules in the policy.

### Example role configuration with templated policies
...
<Tip title="Scope for namespace and admin partition">
...
Consul generates the following policy for each service when you declare a service identity:
...
### Example role configuration with service identities
...
During the authorization process, Consul generates the following policies for the `web` and `db` services and links them to the token:
...
<Tip title="Consul Enterprise Namespacing">
...
Consul generates the following policy for each node when you declare a node identity:
...
### Example role configuration with node identities
...
During the authorization process, Consul generates the following policy andl links it to the token:
