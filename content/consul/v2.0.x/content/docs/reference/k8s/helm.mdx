...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `crds` ((#v-global-crds)) - This flag has different behaviors during the installation process and during the upgrade process.
    ...
    - `crds.consulapi.enabled` installs the CRDs under `consul.hashicorp.com` API group and enables the new custom gateway controller to watch for the resources.
  ...
  - `openshift` ((#v-global-openshift)) - Configuration for running this Helm chart on the Red Hat OpenShift platform.
    ...
    - `isOcpGreaterthan4_18` ((#v-global-openshift-isocpgreaterthan4_18)) (`boolean: false`) - This flag has different behaviors during the installation process and during the upgrade process.  
      During the installation,  if the installation is performed on an OCP instance 4.19+ the flag is ignored. We recommend to perform new installations on an OCP instance 4.19+.
      - If `crds.enableTcpRoute` is `true`, the standard gateway controller watches `TCPRoute` resources.
      - If `crds.enableTcpRoute` is `false`, the standard gateway controller does not watch TCPRoute resources.
      - if `crds.consulapi.enabled` is `true`, controller watches for resources under the API group `consul.hashicorp.com`.

      During the upgrade:
      - If `isOcpGreaterthan4_18` is `false` and `enableTcpRoute` is `false`, generates the manifests only for `gateway.networking.k8s.io` and ignores the TCP watch.
      - If `isOcpGreaterthan4_18` is `false` and `enableTcpRoute` is `true`, generates the manifests only for `gateway.networking.k8s.io` and watches for the TCP.
      - If `isOcpGreaterthan4_18` is `true` and `enableTcpRoute` is `false`, generates the manifests only for `gateway.networking.k8s.io` and ignores the TCP watch
      - If `isOcpGreaterthan4_18` is true and `enableTcpRoute` is true, generates the manifests for both `consul.hashicorp.com` ( all current GVK ) and `gateway.networking.k8s.io` ( no TCP )
      - In any case, `crds.consulapi.enabled` installs the CRDs under `consul.hashicorp.com` API group and enables the new custom gateway controller to watch for the resources.
  ...
  - `apiGateway` ((#v-connectinject-apigateway)) - Configuration settings for the Consul API Gateway integration.
    ...
    - `managedGatewayClass` ((#v-connectinject-apigateway-managedgatewayclass)) - Configuration settings for the GatewayClass installed by Consul on Kubernetes.
      ...
      - `deployment` ((#v-connectinject-apigateway-managedgatewayclass-deployment)) - DEPRECATED: These values are deprecated and will be removed in a future release.
        ...
        See: [`API gateway scaling`](/consul/docs/k8s/api-gateway/scaling).
