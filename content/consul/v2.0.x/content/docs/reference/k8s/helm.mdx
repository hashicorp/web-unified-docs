...
last_modified: 2026-07-22T07:16:43.000Z
...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `installK8sNetworkingCRDs` ((#v-global-installk8snetworkingcrds)) (`boolean: true`) - Enable this flag to install the CRDs under the `gateway.networking.k8s.io` API group. You can set this value to `false` if CRDs are externally installed before Consul installation or if CRDs are managed by the platform.
  ...
  - `domain` ((#v-global-domain)) (`string: consul`) - The domain Consul will answer DNS queries for
    (Refer to [`-domain`](/consul/commands/agent#_domain)) and the domain services synced from
  ...
  - `secretsBackend` ((#v-global-secretsbackend)) - secretsBackend is used to configure Vault as the secrets backend for the Consul on Kubernetes installation.
    ...
    Refer to [Vault as the Secrets Backend](/consul/docs/deploy/server/k8s/vault)
    ...
    - `vault` ((#v-global-secretsbackend-vault))
      ...
      - `vaultNamespace` ((#v-global-secretsbackend-vault-vaultnamespace)) (`string: ""`) - Vault namespace (optional). This sets the Vault namespace for the `vault.hashicorp.com/namespace`
        agent annotation and [Vault Connect CA namespace](/consul/docs/secure-mesh/certificate/vault#namespace).
      ...
      - `connectCA` ((#v-global-secretsbackend-vault-connectca)) - Configuration for the Vault Connect CA provider.
        ...
        Please refer to [Vault ACL policies](/consul/docs/secure-mesh/certificate/vault#vault-acl-policies)
        ...
        - `rootPKIPath` ((#v-global-secretsbackend-vault-connectca-rootpkipath)) (`string: ""`) - The path to a PKI secrets engine for the root certificate.
          For more details, please refer to [Vault Connect CA configuration](/consul/docs/secure-mesh/certificate/vault#rootpkipath).
        ...
        - `intermediatePKIPath` ((#v-global-secretsbackend-vault-connectca-intermediatepkipath)) (`string: ""`) - The path to a PKI secrets engine for the generated intermediate certificate.
          For more details, please refer to [Vault Connect CA configuration](/consul/docs/secure-mesh/certificate/vault#intermediatepkipath).
        ...
        - `additionalConfig` ((#v-global-secretsbackend-vault-connectca-additionalconfig)) (`string: {}`) - Additional Connect CA configuration in JSON format.
          Please refer to [Vault Connect CA configuration](/consul/docs/secure-mesh/certificate/vault#configuration)
  ...
  - `gossipEncryption` ((#v-global-gossipencryption)) - Configures Consul's gossip encryption key.
    (Refer to [`-encrypt`](/consul/commands/agent#_encrypt)).
  ...
  - `recursors` ((#v-global-recursors)) (`array<string>: []`) - A list of addresses of upstream DNS servers that are used to recursively resolve DNS queries.
    ...
    Refer to [`-recursor`](/consul/commands/agent#_recursor) for more details.
  ...
  - `tls` ((#v-global-tls)) - Enables [TLS](/consul/docs/secure/encryption/tls/mtls)
    ...
    - `enabled` ((#v-global-tls-enabled)) (`boolean: false`) - If true, the Helm chart will enable TLS for Consul
      ...
      This setting is required for [Cluster Peering](/consul/docs/east-west/cluster-peering/tech-specs/k8s).
    ...
    - `serverAdditionalIPSANs` ((#v-global-tls-serveradditionalipsans)) (`array<string>: ['::1']`) - A list of additional IP addresses to set as Subject Alternative Names (SANs)
      in the server certificate. This is useful when you need to access the
      Consul server(s) externally, for example, if you're using the UI.
    ...
    - `verify` ((#v-global-tls-verify)) (`boolean: true`) - If true, `verify_outgoing`, `verify_server_hostname`,
      ...
      Please refer to [TLS on existing clusters](/consul/docs/secure/encryption/tls/rotate/k8s)
      for more details.
  ...
  - `federation` ((#v-global-federation)) - Configure federation.
    ...
    - `k8sAuthMethodHost` ((#v-global-federation-k8sauthmethodhost)) (`string: null`) - If you are setting `global.federation.enabled` to true and are in a secondary datacenter,
      ...
      Please refer to the [Kubernetes Auth Method documentation](/consul/docs/secure/acl/auth-method/k8s).
  ...
  - `generateManifests` ((#v-global-generatemanifests)) (`boolean: false`) - Set to `true` to generate the manifests during an upgrade. After you generate the manifests, set this to `false` for subsequent Helm upgrades. Refer to the [API gateway resource upgrade guide](/consul/docs/north-south/api-gateway/k8s/resource-upgrade) for details on when to use this flag.

  - `crds` ((#v-global-crds)) - This flag has different behaviors during installation and upgrade.

    During the installation, custom CRDs and gateway TCP CRDs are enabled:
    - `enableTcpRoute` installs the TCP GVK under `gateway.networking.k8s.io` and enables the standard gateway controller to watch for those resources.
    - `crds.consulapi.enabled` installs the CRDs under the `consul.hashicorp.com` API group and enables the new custom gateway controller to watch for those resources.

    During the upgrade, manifests are generated:
    - If `crds.consulapi.enabled` is set to `false`, the chart does not generate manifests under the `consul.hashicorp.com` API group.
    - If `crds.consulapi.enabled` is set to `true`, the chart generates and applies manifests under the `consul.hashicorp.com` API group.
  ...
  - `openshift` ((#v-global-openshift)) - Configuration for running this Helm chart on the Red Hat OpenShift platform.
    ...
    - `upgradeTo419From418` ((#v-global-openshift-upgradeto419from418)) (`boolean: false`) - This flag has different behaviors during installation and upgrade.

      During the installation: if the installation is performed on an OCP instance 4.19+, this flag is ignored. We recommend performing new installations on OCP 4.19+.

      During the upgrade: enable this flag when upgrading OCP from 4.18 to 4.19+. When enabled, it generates manifests based on the value of `openshift.crds.enableTcpRoute`:
      - Case 1: when `upgradeTo419From418` is `true` and `openshift.crds.enableTcpRoute` is `false`, the upgrade generates manifests only for `gateway.networking.k8s.io` and ignores the TCP watch.
      - Case 2: when `upgradeTo419From418` is `true` and `openshift.crds.enableTcpRoute` is `true`, the upgrade generates manifests for both `consul.hashicorp.com` and `gateway.networking.k8s.io`. Manual deletion of the TCP GVK is required.

    - `isOcpGreaterthan4_18` ((#v-global-openshift-isocpgreaterthan4_18)) (`boolean: false`) - This flag has different behaviors during installation and upgrade.

      During the installation: if the installation is performed on an OCP instance 4.19+, this flag is ignored. We recommend performing new installations on OCP 4.19+.
      - If `openshift.crds.enableTcpRoute` is `true`, the standard gateway controller watches for TCPRoute resources.
      - If `openshift.crds.enableTcpRoute` is `false`, the standard gateway controller does not watch for TCPRoute resources.
      - If `openshift.crds.consulapi.enabled` is `true`, the custom gateway controller watches for resources under the `consul.hashicorp.com` API group.

      During the upgrade:
      - If `isOcpGreaterthan4_18` is `false` and `openshift.crds.enableTcpRoute` is `false`, the upgrade generates manifests only for `gateway.networking.k8s.io` and ignores the TCP watch.
      - If `isOcpGreaterthan4_18` is `false` and `openshift.crds.enableTcpRoute` is `true`, the upgrade generates manifests only for `gateway.networking.k8s.io` and watches for TCPRoute resources.
      - If `isOcpGreaterthan4_18` is `true` and `openshift.crds.enableTcpRoute` is `false`, the upgrade generates manifests only for `gateway.networking.k8s.io` and ignores the TCP watch.
      - If `isOcpGreaterthan4_18` is `true` and `openshift.crds.enableTcpRoute` is `true`, the upgrade generates manifests for both `consul.hashicorp.com` (all current GVKs) and `gateway.networking.k8s.io` (no TCP).

      In all cases, `openshift.crds.consulapi.enabled` installs the CRDs under the `consul.hashicorp.com` API group and enables the new custom gateway controller to watch for those resources.
  ...
  - `dualStack` ((#v-global-dualstack))

    - `enabled` ((#v-global-dualstack-enabled)) (`boolean: false`)
...
- `server` ((#v-server)) - Server, when enabled, configures a server cluster to run. This should
  ...
  - `replicas` ((#v-server-replicas)) (`integer: 1`) - The number of server agents to run. This determines the fault tolerance of
    the cluster. Please refer to the [deployment table](/consul/docs/concept/consensus#deployment-table)
  ...
  - `serverCert` ((#v-server-servercert)) - A secret containing a certificate & key for the server agents to use
    ...
    Refer to [Generate mTLS Certificates for Consul with Vault and Consul Template](/consul/docs/automate/consul-template/vault/mtls)
  ...
  - `storageClass` ((#v-server-storageclass)) (`string: null`) - The StorageClass to use for the servers' StatefulSet storage. It must be
    ...
    Refer to the [Read/Write Tuning](/consul/docs/reference/architecture/server#read-write-tuning)
  ...
  - `connect` ((#v-server-connect)) (`boolean: true`) - This will enable/disable [service mesh](/consul/docs/connect/enable). Setting this to true
    ...
    by setting the `server.extraConfig` value or by applying [configuration entries](/consul/docs/fundamentals/config-entry).
  ...
  - `updatePartition` ((#v-server-updatepartition)) (`integer: 0`) - This value is used to carefully
    ...
    and [Upgrade Consul](/consul/docs/upgrade/k8s#upgrading-consul-servers)
  ...
  - `extraConfig` ((#v-server-extraconfig)) (`string: {}`) - A raw string of extra [JSON configuration](/consul/docs/reference/agent/configuration-file/general) for Consul
  ...
  - `snapshotAgent` ((#v-server-snapshotagent)) - <EnterpriseAlert inline /> Values for setting up and running
    ...
    - `extraEnvironmentVars` ((#v-server-snapshotagent-extraenvironmentvars)) (`map`) - A list of extra environment variables to set on the snapshot agent specifically.
    ...
    - `extraVolumes` ((#v-server-snapshotagent-extravolumes)) (`array<map>`) - A list of extra volumes to mount onto the snapshot agent. Use this block
  ...
  - `auditLogs` ((#v-server-auditlogs)) - <EnterpriseAlert inline /> Added in Consul 1.8, the audit object allow users to enable auditing
    ...
    [audit logs](/consul/docs/monitor/log/audit) documentation
  ...
  - `limits` ((#v-server-limits)) - Settings for potentially limiting timeouts, rate limiting on clients as well
    ...
    - `requestLimits` ((#v-server-limits-requestlimits)) - This object specifies configurations that limit the rate of RPC and gRPC
      ...
      /consul/docs/reference/agent/configuration-file/general#request_limits
...
- `client` ((#v-client)) - Values that configure running a Consul client on Kubernetes nodes.
  ...
  - `join` ((#v-client-join)) (`array<string>: null`) - A list of valid [`-retry-join` values](/consul/commands/agent#_retry_join).
  ...
  - `nodeMeta` ((#v-client-nodemeta)) - nodeMeta specifies an arbitrary metadata key/value pair to associate with the node
    (refer to [`-node-meta`](/consul/commands/agent#_node_meta))
  ...
  - `aclInit` ((#v-client-aclinit))

    - `startupStagger` ((#v-client-aclinit-startupstagger))

      - `enabled` ((#v-client-aclinit-startupstagger-enabled)) (`boolean: false`) - When true, adds a randomized sleep before running acl-init to avoid login storms
        against the Consul servers in large clusters. Defaults set to 0 for no sleep.

      - `minSeconds` ((#v-client-aclinit-startupstagger-minseconds)) (`integer: 0`) - Minimum seconds to sleep before attempting the ACL login.

      - `maxSeconds` ((#v-client-aclinit-startupstagger-maxseconds)) (`integer: 0`) - Maximum seconds to sleep before attempting the ACL login.

  - `extraConfig` ((#v-client-extraconfig)) (`string: {}`) - A raw string of extra [JSON configuration](/consul/docs/reference/agent/configuration-file) for Consul
  ...
  - `livenessProbe` ((#v-client-livenessprobe)) (`string: ""`) - Override the default livenessProbe and readinessProbe for the DaemonSet consul-client container.
  ...
  - `readinessProbe` ((#v-client-readinessprobe))
...
- `dns` ((#v-dns)) - Configuration for DNS configuration within the Kubernetes cluster.
  ...
  - `proxy` ((#v-dns-proxy)) - Configures dns-proxy deployment.

    - `enabled` ((#v-dns-proxy-enabled)) (`boolean: false`) - True if you want to enable dns-proxy

    - `replicas` ((#v-dns-proxy-replicas)) (`integer: 1`) - The number of deployment replicas.

    - `port` ((#v-dns-proxy-port)) (`integer: 53`)

    - `aclToken` ((#v-dns-proxy-acltoken)) - Refers to an existing Kubernetes secret that contains an ACL token
      ...
      - `secretName` ((#v-dns-proxy-acltoken-secretname)) (`string: null`) - The name of the Kubernetes secret that holds the ACL token.

      - `secretKey` ((#v-dns-proxy-acltoken-secretkey)) (`string: null`) - The key within the Kubernetes secret that holds the ACL token.
...
- `ui` ((#v-ui)) - Values that configure the Consul UI.
  ...
  - `metrics` ((#v-ui-metrics)) - Configurations for displaying metrics in the UI.
    ...
    - `provider` ((#v-ui-metrics-provider)) (`string: prometheus`) - Provider for metrics. Refer to
      [`metrics_provider`](/consul/docs/reference/agent/configuration-file/ui#ui_config_metrics_provider)
  ...
  - `dashboardURLTemplates` ((#v-ui-dashboardurltemplates)) - Corresponds to [`dashboard_url_templates`](/consul/docs/reference/agent/configuration-file/ui#ui_config_dashboard_url_templates)
    configuration.

    - `service` ((#v-ui-dashboardurltemplates-service)) (`string: ""`) - Sets [`dashboardURLTemplates.service`](/consul/docs/reference/agent/configuration-file/ui#ui_config_dashboard_url_templates_service).
...
- `syncCatalog` ((#v-synccatalog)) - Configure the catalog sync process to sync K8S with Consul
  ...
  - `cleanupNodeOnRemoval` ((#v-synccatalog-cleanupnodeonremoval)) (`boolean: false`) - True if you want to deregister all services in this cluster from consul that have been registered by the catalog sync when the `enabled` flag is set to false.
  ...
  - `default` ((#v-synccatalog-default)) (`boolean: true`) - If true, all valid services in K8S are
    synced by default. If false, the service must be [annotated](/consul/docs/register/service/k8s/service-sync#enable-and-disable-sync)
    properly to sync.
  ...
  - `metrics` ((#v-synccatalog-metrics)) - Metrics settings for syncCatalog

    - `enabled` ((#v-synccatalog-metrics-enabled)) (`boolean: false`) - This value enables or disables metrics collection for registered services, overriding the global metrics collection settings.

    - `port` ((#v-synccatalog-metrics-port)) (`int: null`) - This value sets the port to use for scraping syncCatalog metrics via prometheus, defaults to 20300 if not set. Must be in the port
      range of 1024-65535.

    - `path` ((#v-synccatalog-metrics-path)) (`string: null`) - This value sets the path to use for scraping syncCatalog metrics via prometheus, defaults to /metrics if not set.
...
- `connectInject` ((#v-connectinject)) - Configures the automatic Connect sidecar injector.
  ...
  - `default` ((#v-connectinject-default)) (`boolean: false`) - If true, the injector will inject the
    ...
    [injection annotation](/consul/docs/connect/k8s#consul-hashicorp-com-connect-inject)
  ...
  - `apiGateway` ((#v-connectinject-apigateway)) - Configuration settings for the Consul API Gateway integration.
    ...
    - `managedGatewayClass` ((#v-connectinject-apigateway-managedgatewayclass)) - Configuration settings for the GatewayClass installed by Consul on Kubernetes.
      ...
      - `scaling` ((#v-connectinject-apigateway-managedgatewayclass-scaling)) - <EnterpriseAlert inline /> These settings enable per-Gateway scaling behavior for
        ...
        - `enabled` ((#v-connectinject-apigateway-managedgatewayclass-scaling-enabled)) (`boolean: false`) - Set to `true` to enable per-Gateway scaling behavior, including support for Gateway annotations for static replica counts and controller-managed HPA resources. Requires Consul Enterprise with a valid license.
      ...
      - `deployment` ((#v-connectinject-apigateway-managedgatewayclass-deployment)) - DEPRECATED: These values are deprecated and will be removed in a future release.
        ...
        For HPA, use: `consul.hashicorp.com/hpa-enabled: "true"` with `HPA minReplicas` and `HPA maxReplicas` annotations.
        ...
        - `defaultInstances` ((#v-connectinject-apigateway-managedgatewayclass-deployment-defaultinstances)) (`integer: 1`) - Default number of gateway instances.
          DEPRECATED: Use the `consul.hashicorp.com/default-replicas` annotation on the Gateway resource instead.
        ...
        - `maxInstances` ((#v-connectinject-apigateway-managedgatewayclass-deployment-maxinstances)) (`integer: 1`) - Maximum number of gateway instances.
          DEPRECATED: No longer enforced. Use `HPA maxReplicas` via annotations instead.
        ...
        - `minInstances` ((#v-connectinject-apigateway-managedgatewayclass-deployment-mininstances)) (`integer: 1`) - Minimum number of gateway instances.
          DEPRECATED: Use `HPA minReplicas` via annotations instead.
  ...
  - `cni` ((#v-connectinject-cni)) - Configures consul-cni plugin for Consul Service mesh services
    ...
    - `autorotateToken` ((#v-connectinject-cni-autorotatetoken)) (`boolean: false`) - If true, then use projected service account tokens for the CNI plugin and
      Uses auto-rotated projected token everytime when interacting with Kubernetes API.

    - `tokenExpiration` ((#v-connectinject-cni-tokenexpiration)) (`Integer: 3600`) - Set the CNI plugin ServiceAccount token validity period. Applicable only when autorotateToken is true.
    ...
    - `logLevel` ((#v-connectinject-cni-loglevel)) (`string: null`) - Log level for the installer and plugin. Overrides global.logLevel. One of "trace", "debug", "info", "warn", or "error".
  ...
  - `aclBindingRuleSelector` ((#v-connectinject-aclbindingruleselector)) (`string: serviceaccount.name!=default`) - Query that defines which Service Accounts
    ...
    Refer to Auth methods [Binding rules](/consul/docs/secure/acl/auth-method#binding-rules)
    and [Trusted identiy attributes](/consul/docs/secure/acl/auth-method/k8s#trusted-identity-attributes)
    for more details.
  ...
  - `sidecarProxy` ((#v-connectinject-sidecarproxy))
    ...
    - `concurrency` ((#v-connectinject-sidecarproxy-concurrency)) (`string: 2`) - The number of worker threads to be used by the Envoy proxy.
      ...
      For additional information, refer to the [Envoy threading model](https://blog.envoyproxy.io/envoy-threading-model-a8d44b922310).
  ...
  - `dataplaneAsSidecarInitContainer` ((#v-connectinject-dataplaneassidecarinitcontainer))

    - `defaultEnabled` ((#v-connectinject-dataplaneassidecarinitcontainer-defaultenabled)) (`boolean: false`) - If true, the consul-dataplane will run as an init container with the restart policy set to "Always" and using startup probe to determine readiness.
      Also, Kubelet will wait for the consul-dataplane init container to be ready before starting the application container.

    - `initialProbeCheckDelaySeconds` ((#v-connectinject-dataplaneassidecarinitcontainer-initialprobecheckdelayseconds)) (`integer: 1`) - Configures how long the k8s startup probe will wait initially before performing the first check.

    - `probeCheckPeriodSeconds` ((#v-connectinject-dataplaneassidecarinitcontainer-probecheckperiodseconds)) (`integer: 1`) - Configures how often the k8s startup probe will check the readiness of the consul-dataplane init container.

    - `probeCheckFailureThreshold` ((#v-connectinject-dataplaneassidecarinitcontainer-probecheckfailurethreshold)) (`integer: 10`) - Configures the failure threshold for the k8s startup probe.

    - `probeCheckTimeoutSeconds` ((#v-connectinject-dataplaneassidecarinitcontainer-probechecktimeoutseconds)) (`integer: 5`) - Configures the timeout for each k8s startup probe check.
...
- `meshGateway` ((#v-meshgateway)) - [Mesh Gateways](/consul/docs/east-west/mesh-gateway) enable Consul to work across Consul datacenters.

  - `enabled` ((#v-meshgateway-enabled)) (`boolean: false`) - If [mesh gateways](/consul/docs/east-west/mesh-gateway) are enabled, a Deployment will be created that runs
