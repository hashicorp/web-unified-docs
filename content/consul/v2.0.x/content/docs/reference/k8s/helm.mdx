...
last_modified: 2026-07-10T16:01:41.000Z
...
- `global` ((#v-global)) - Holds values that affect multiple components of the chart.
  ...
  - `installK8sNetworkingCRDs` ((#v-global-installk8snetworkingcrds)) (`boolean: true`) - Enable this flag to install the CRDs under `gateway.networking.k8s.io` API group. You can set this value to false if CRDs are externally installed before Consul installation or if CRDs are managed by the platform.
  ...
  - `generateManifests` ((#v-global-generateManifests)) (`boolean: false`) - Set to `true` to generate the manifests. After you generate the manifests, set this to `false` for subsequent Helm upgrades.
  
  - `generateManifestsStorageClassName` ((#v-global-generateManifestsStorageClassName)) (`string: ""`) - The storage class name to use for the PersistentVolumeClaim that stores generated manifests. We recommend using a storage class with `volumeBindingMode: Immediate`. If not set, the cluster's default storage class is used.
  
  - `generateManifestsStorage` ((#v-global-generateManifestsStorage)) (`string: "1Gi"`) - The storage size for the PersistentVolumeClaim used to store generated manifests.

  - `crds` ((#v-global-crds)) - This flag has different behaviors during the installation process and during the upgrade process.
   
    During the installation, custom CRDs and gateway TCP CRD are enabled.
    ...
    - `consulapi.enabled` installs the CRDs under `consul.hashicorp.com` API group and enables the new custom gateway controller to watch for the resources.

    During the upgrade, manifests are generated.
    - If `consulapi.enabled` is set to `false`, the chart does not generate manifests under the `consul.hashicorp.com` API group.
    - If `consulapi.enabled` is set to `true`, the chart generates and applies manifests under the `consul.hashicorp.com` API group.
  ...
  - `openshift` ((#v-global-openshift)) - Configuration for running this Helm chart on the Red Hat OpenShift platform.
    ...
    - `upgradeTo419From418` ((#v-global-openshift-upgradeto419from418)) (`boolean: false`) - This flag has different behaviors during the installation process and during the upgrade process. 
      During the installation: if the installation is performed on an OCP instance 4.19+ the flag is ignored. We recommend to perform new installations on an OCP instance 4.19+.

      During the upgrade: This flag should be enabled when upgrading the OCP from 4.18 to 4.19+ versions.
      When enabled, it will generate the manifests. The resources in the manifest depend on the value of `enableTcpRoute`:
      - case 1: when `upgradeTo419From418` is `true` and `enableTcpRoute` is `false`, the upgrade generates the manifests only for `gateway.networking.k8s.io` and ignores the TCP watch.
      - case 2: when `upgradeTo419From418` is `true` and `enableTcpRoute` is `true`, the upgrade generates the manifests for both `consul.hashicorp.com` and `gateway.networking.k8s.io` ( this process requeres the manual deletion of TCP GVK )

    - `isOcpGreaterthan4_18` ((#v-global-openshift-isocpgreaterthan4_18)) (`boolean: false`) - During install on OpenShift 4.19+, you typically deploy Consul directly on 4.19+, so this flag is not used.

      - If `global.openshift.crds.enableTcpRoute` is `true`, the standard gateway controller watches TCPRoute resources.
      - If `global.openshift.crds.enableTcpRoute` is `false`, the standard gateway controller does not watch TCPRoute resources.

      for `global.openshift.crds.consulapi.enabled` set to `true`; controller watches for resources under the API group - consul.hashicorp.com

      During the upgrade:
      If isOcpGreaterthan4_18 is false and `global.openshift.crds.enableTcpRoute` is `false`:
      generate the manifests only for gateway.networking.k8s.io and ignores the TCP watch
      If isOcpGreaterthan4_18 is false and `global.openshift.crds.enableTcpRoute` is `true`:
      generate the manifests only for gateway.networking.k8s.io and watches for the TCP
      If isOcpGreaterthan4_18 is true and `global.openshift.crds.enableTcpRoute` is `false`:
      generate the manifests only for gateway.networking.k8s.io and ignores the TCP watch
      If isOcpGreaterthan4_18 is true and `global.openshift.crds.enableTcpRoute` is `true`:
      generate the manifests for both consul.hashicorp.com ( all current GVK ) and gateway.networking.k8s.io ( no TCP )

      in any case, `global.openshift.crds.consulapi.enabled` install the CRDs under consul.hashicorp.com API group and enables the new custom gateway controller to watch for the resources.
  ...
  - `apiGateway` ((#v-connectinject-apigateway)) - Configuration settings for the Consul API Gateway integration.
    ...
    - `managedGatewayClass` ((#v-connectinject-apigateway-managedgatewayclass)) - Configuration settings for the GatewayClass installed by Consul on Kubernetes.
      ...
      - `scaling` ((#v-connectinject-apigateway-managedgatewayclass-scaling)) - <EnterpriseAlert inline /> These settings enable per-Gateway scaling behavior for
        ...
        - `enabled` ((#v-connectinject-apigateway-managedgatewayclass-scaling-enabled)) (`boolean: false`) - This value will enable Autoscaling.
      ...
      - `deployment` ((#v-connectinject-apigateway-managedgatewayclass-deployment)) - DEPRECATED: These values are deprecated and will be removed in a future release.
        ...
        - `defaultInstances` ((#v-connectinject-apigateway-managedgatewayclass-deployment-defaultinstances)) (`integer: 1`) - Default number of gateway instances
          DEPRECATED: Use consul.hashicorp.com/default-replicas annotation on Gateway resource
