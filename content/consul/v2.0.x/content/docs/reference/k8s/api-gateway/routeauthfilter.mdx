...
Map that overrides the gateway-wide external authorization (ext_authz) posture for the HTTP routes that reference this filter. It maps to the Consul [`http-route` `ExtAuthz` filter](/consul/docs/reference/config-entry/http-route#rules-filters-extauthz) and takes precedence over the gateway-wide toggle. This field only takes effect when a [`builtin/ext-authz` Envoy extension](/consul/docs/reference/proxy/extensions/ext-authz) with `ProxyType = "api-gateway"` is applied to the gateway.
...
Specifies whether Envoy runs the external authorization check for requests to HTTP routes that reference this filter. Set to `true` to force external authorization on for these routes, or `false` to skip it, regardless of the gateway-wide posture.
...
In the following example, the filter disables external authorization for the HTTP routes that reference it, overriding the gateway-wide posture:
