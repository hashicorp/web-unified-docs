...
last_modified: 2026-07-22T06:29:40.000Z
...
Map that overrides the gateway-wide external authorization (`ext_authz`) default behavior for the HTTP routes that reference this filter. It maps to the Consul [`http-route` `ExtAuthz` filter](/consul/docs/reference/config-entry/http-route#rules-filters-extauthz) and takes precedence over the gateway-wide toggle. This field only takes effect when a [`builtin/ext-authz` Envoy extension](/consul/docs/reference/proxy/extensions/ext-authz) with `ProxyType = "api-gateway"` is applied to the gateway.
