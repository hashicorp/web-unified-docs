...
created_at: 2026-03-31T20:45:51.000Z
last_modified: 2026-03-31T20:45:51.000Z
...
- [`rules`](#rules): list of objects | optional
  ...
  - [`filters`](#rules-filters): list of objects | optional
    ...
    - [`extensionRef`](#rules-filters-extensionref): map
      ...
      - [`kind`](#rultes-filters-extensionref): string | `RouteAuthFilter`, `RouteRetryFilter`, `RouteTimeoutFilter`, or `RouteExtProc`
      - [`name`](#rultes-filters-extensionref): string
...
When an `HTTPRoute` rule has one `backendRef`, Consul API gateway evaluates the backend service's Consul discovery chain. When the backend service uses a `ServiceRouter` to route requests to a `ServiceResolver` subset, Consul preserves the subset destination when it synthesizes the API gateway discovery chain. Consul combines the gateway route match and service-router match, so requests must satisfy both sets of criteria to use the subset route.
...
- [`HTTPRoute` in Kubernetes documentation](https://gateway-api.sigs.k8s.io/api-types/httproute/)
- [`TCPRoute`  in Kubernetes documentation](https://gateway-api.sigs.k8s.io/reference/spec/?h=tcproute#tcproute)
...
The following example creates `stable` and `canary` subsets for the `api` service. The `ServiceRouter` routes requests with the `x-version: canary` header to the `canary` subset. The `HTTPRoute` exposes the `api` service through `example-gateway`; Consul API gateway applies the service-router rule when routing matching requests to the backend service.

<CodeBlockConfig filename="route_to_service_subset.yaml">
