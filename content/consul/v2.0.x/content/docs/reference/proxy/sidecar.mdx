...
created_at: 2026-03-31T20:45:54.000Z
last_modified: 2026-03-31T20:45:54.000Z
...
- `proxy.local_service_ports` - Defaults to the parent service's named ports when the parent is a multi-port service. `proxy.local_service_port` remains the default port used when the proxy needs a single local service port.
...
### Example with multiport configuration

The following example configures a `web` service with a multiport upstream `api` service. For more information about multiport services, refer to the [multiport documentation](/consul/docs/connect/proxy/multiport).
