...
* `local_service_ports`: Array value that specifies the named ports exposed by a local multi-port service instance. Refer to the [proxy parameters reference](#local-service-ports) for details.
...
| `local_service_ports` <a name="local-service-ports"/>| Array value that specifies the named ports on the local service instance for multi-port services. `local_service_port` remains the default port used when the proxy needs a single local service port. | Optional | None |
...
For multi-port service mesh destinations, set `destination_port` to the named port you want the upstream listener to route to. When transparent proxy mode is enabled, applications can dial the named virtual address in the format `<port-name>.<service>.virtual.consul`.
...
| `destination_port` | String value that specifies the named port to target on a multiport upstream service. The value must match one of the named ports registered for the destination service. | Optional | None |
...
<CodeTabs heading="Example multiport service destination upstream for a named port">
