...
| `destination_port` | String value that specifies the named port to target on a multi-port upstream service. The value must match one of the named ports registered for the destination service. | Optional | None |
...
<CodeTabs heading="Example service destination upstream for a named port">
