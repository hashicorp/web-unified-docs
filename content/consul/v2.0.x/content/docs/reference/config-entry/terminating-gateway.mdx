...
TLS via Envoy's Secret Discovery Service (SDS). Both the `secretRef` name and the corresponding file paths must be specified — the file paths
indicate where the Kubernetes secret is mounted on the terminating gateway pod's filesystem, and the `secretRef` name identifies the Kubernetes
secret so that certificate updates are dynamically delivered to Envoy via SDS without requiring a proxy restart.
...
spec:
  services:
    - name: billing
      caFile: /etc/certs/ca-chain.cert.pem
      sni: billing.service.com
      # Kubernetes only: specify secretRef along with file paths to enable SDS
      # The file paths point to where the K8s secret is mounted on the gateway pod
      secretRef:
        name: billing-tls-cert
```
    ...
    },
    {
      ...
      children: [
        ...
        },
        {
          ...
          hcl: false,
          description:
            '`(Kubernetes only)` References a Kubernetes secret containing TLS certificates to enable Envoy SDS (Secret Discovery Service) for the linked service. Must be specified **together with** the corresponding file path fields (`caFile`, `certFile`, `keyFile`) — the file paths point to where the Kubernetes secret is mounted on the terminating gateway pod, and the secret name enables dynamic certificate delivery to Envoy via SDS.',
          children: [
            {
              name: 'name',
              type: 'string: ""',
              description:
                'The name of the Kubernetes secret containing the TLS certificates. Must be at least 1 character long.',
            },
          ],
        },
      ],
