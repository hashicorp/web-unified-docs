...
Overrides the gateway-wide external authorization (ext_authz) posture for the routes matched by this rule. This filter only takes effect when a [`builtin/ext-authz` Envoy extension](/consul/docs/reference/proxy/extensions/ext-authz) with `ProxyType = "api-gateway"` is applied to the gateway. It takes precedence over the [gateway-wide `ExtAuthz` toggle](/consul/docs/reference/config-entry/api-gateway#extauthz).
...
Specifies whether Envoy runs the external authorization check for requests matched by this rule. Set to `true` to force external authorization on for these routes, or `false` to skip it, regardless of the gateway-wide posture.
