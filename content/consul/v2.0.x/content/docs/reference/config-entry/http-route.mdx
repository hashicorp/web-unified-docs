...
last_modified: 2026-07-14T16:54:01.000Z
...
Overrides the gateway-wide external authorization (`ext_authz`) behavior for the routes matched by this rule. This filter only takes effect when a [`builtin/ext-authz` Envoy extension](/consul/docs/reference/proxy/extensions/ext-authz) with `ProxyType = "api-gateway"` is applied to the gateway. It takes precedence over the [gateway-wide `ExtAuthz` toggle](/consul/docs/reference/config-entry/api-gateway#extauthz).
