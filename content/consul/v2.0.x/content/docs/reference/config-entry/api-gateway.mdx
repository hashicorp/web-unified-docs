...
Sets the gateway-wide external authorization (ext_authz) posture for all routes attached to the API gateway. This toggle only takes effect when a [`builtin/ext-authz` Envoy extension](/consul/docs/reference/proxy/extensions/ext-authz) with `ProxyType = "api-gateway"` is applied to the gateway.
...
Specifies the default external authorization posture for routes behind the gateway. When set to `true`, Envoy runs the external authorization check for every route unless a route opts out with an `http-route` `ExtAuthz` filter. When set to `false`, Envoy skips the external authorization check by default and routes must opt in.
