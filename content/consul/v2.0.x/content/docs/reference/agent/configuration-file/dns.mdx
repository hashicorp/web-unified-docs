...
created_at: 2026-03-31T20:45:35.000Z
last_modified: 2026-03-31T20:45:35.000Z
...
- `dns_config` This object allows a number of sub-keys to be set which can tune how DNS queries are serviced. Refer to [DNS caching](/consul/docs/discover/dns/scale) for more information.
  ...
  - `recursor_strategy` - If set to `sequential`, Consul will query recursors in the
    order listed in the [`recursors`](/consul/docs/reference/agent/configuration-file/dns#recursors) option. If set to `random`,
    Consul will query an upstream DNS resolvers in a random order. Defaults to
    `sequential`.

  - `recursor_timeout` - Timeout used by Consul when
    recursively querying an upstream DNS server. See [`recursors`](/consul/docs/reference/agent/configuration-file/dns#recursors) for more details. Default is 2s. This is available in Consul 0.7 and later.
