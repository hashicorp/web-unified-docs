...
Consul includes certificate expiration telemetry for monitoring certificate
health across your Consul infrastructure. You can monitor the following
certificates:
...
Define monitoring behavior for an agent in the `telemetry` block of the agent
configuration file.

- `telemetry.certificate` - Configuration block for certificate expiration monitoring

  - `enabled` `(bool: true)` - Enables the root CA, signing CA, and agent TLS
    certificate expiration monitors.

  - `cache_duration` `(string: "5m")` - Duration value stored in the runtime
    telemetry configuration for certificate telemetry consumers. Valid time
    units are `ns`, `us`, `ms`, `s`, `m`, and `h`.

  - `critical_threshold_days` `(int: 7)` - Number of days before expiration to
    emit `CRITICAL`-severity certificate expiration and renewal-failure logs.

  - `warning_threshold_days` `(int: 30)` - Number of days before expiration to
    emit `WARNING`-severity certificate expiration logs.

  - `info_threshold_days` `(int: 90)` - Integer value stored in the runtime
    telemetry configuration for certificate telemetry consumers.

  - `exclude_auto_renewable` `(bool: false)` - Boolean value stored in the
    runtime telemetry configuration for certificate telemetry consumers.

The `enabled` setting applies to the root CA, signing CA, and agent TLS expiry
monitors. Leaf certificate expiry and renewal metrics are emitted by the leaf
certificate manager independently, but the warning and critical threshold
values still affect leaf renewal failure log severity.
