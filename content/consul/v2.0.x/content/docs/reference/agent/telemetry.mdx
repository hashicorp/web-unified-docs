...
The following metrics are emitted for certificate expiration and renewal health:

| Metric | Description | Unit | Type |
|--------|-------------|------|------|
| `consul.mesh.active-root-ca.expiry` | Leader-only gauge reporting the number of seconds until the active service mesh root CA expires. Includes the `datacenter` label. | seconds | gauge |
| `consul.mesh.active-signing-ca.expiry` | Leader-only gauge reporting the number of seconds until the active service mesh signing CA expires. Includes the `datacenter` label. | seconds | gauge |
| `consul.agent.tls.cert.expiry` | Gauge reporting the number of seconds until the local agent TLS certificate expires. Includes the `datacenter`, `partition`, and `node` labels. | seconds | gauge |
| `consul.leaf-certs.cert_expiry` | Gauge reporting the number of seconds until a cached service leaf certificate expires. Includes the `datacenter`, `partition`, `namespace`, `service`, and `kind` labels. | seconds | gauge |
| `consul.leaf-certs.cert_renewal_failure` | Gauge set to `1` while renewal of a cached service leaf certificate is failing and `0` after recovery. Active failures also include a `reason` label. | 0 or 1 | gauge |
| `consul.leaf-certs.renewal_success` | Counter incremented when a leaf certificate renewal succeeds. | renewals | counter |
| `consul.leaf-certs.renewal_failed` | Counter incremented when a leaf certificate renewal fails for non-rate-limit reasons. | failures | counter |
| `consul.leaf-certs.renewal_failed.rate_limited` | Counter incremented when a leaf certificate renewal is rate limited. | failures | counter |
| `consul.leaf-certs.consecutive_rate_limit_errors` | Gauge tracking consecutive rate-limit failures for leaf certificate renewal. | errors | gauge |
