...
created_at: 2026-03-31T20:45:55.000Z
last_modified: 2026-03-31T20:45:55.000Z
...
- [`port`](#port): number
- [`ports`](#ports): list of maps (service discovery only)
  - [`name`](#ports): string
  - [`port`](#ports): number 
  - [`default`](#ports): boolean
...
You can use this field for both service discovery and service mesh use cases. You cannot configure both this parameter and the [`ports` parameter](#ports) in the same service definition.
...
Specifies a series of maps that identify services with multiple ports for the Consul catalog.

This field supports service discovery use cases only. You cannot configure both this parameter and the [`ports` parameter](#ports) in the same service definition.

- Type: map

Define the following parameters for each port you want to specify:

| Parameter | Description                                                                                                      | Type    | Default |
| :-------- | :--------------------------------------------------------------------------------------------------------------- | :------ | :------ |
| `name`    | Specifies a name for the port. This value is addressable with Consul DNS. Each name should be unique.            | string  | None    |
| `port`    | Specifies a numeric port where the service is available.                                                         | number  | None    |
| `default` | Sets a port as the default when one is not included. Do not set more than one port as the default for a service. | boolean | `false` |

When the `tagged_address` parameter is not set, Consul uses the service's `address` and the `default` port defined for the service.

If you set multiple ports as the default port, Consul returns an error when you attempt to register the service.

For an example service definition, refer to [Multi-port service definition](#multi-port-service-definition).
...
When `tagged_address` is not configured in a service definition, Consul uses the `address` and `port` properties in the definition. When you define a multi-port service with the `ports` parameter, Consul uses the port configured as the default in the `ports.default` parameter.
...
## Multiport service definition

The following example defines a service with multiple ports using the [`ports`
parameter](#ports). This definition registers the `orders` service with an
`http` port and a `metrics` port. Refer to the [Consul
Multiport overview](/consul/docs/connect/proxy/multiport) for more information.
