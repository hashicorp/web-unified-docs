...
created_at: 2026-03-31T20:45:01.000Z
last_modified: 2026-03-31T20:45:01.000Z
...
Make sure that your network allows HTTPS egress on port 443 to `https://reporting.hashicorp.services` by adding the following IP addresses to your allow-list:
...
1. Go to [https://portal.cloud.hashicorp.com/license-utilization/reports/create](https://portal.cloud.hashicorp.com/license-utilization/reports/create).

1. Click **Choose files**, or drag and drop your files into the container.

If the upload succeeded, the HCP user interface displaus the file status `Uploaded` in green text. If the upload failed, the file status displays `Failed` in red text, and includes error information.

If the upload fails make sure you have not modified the file signature. If the error persists, contact your account representative.
