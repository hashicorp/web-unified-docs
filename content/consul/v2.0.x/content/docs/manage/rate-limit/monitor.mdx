...
The value of the `limit_type` label depends on the rate limit configuration:

- **Per-server rate limit**: `limit_type` uses the format `global/<operation>`, for example `global/read`.
- **Global rate limit** (config entry): `limit_type` uses the format `global.configentry/<operation>`, for example `global.configentry/read`.
