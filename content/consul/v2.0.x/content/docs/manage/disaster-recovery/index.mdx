...
created_at: 2026-03-31T20:45:13.000Z
last_modified: 2026-03-31T20:45:13.000Z
...
DRPs should also include specific steps that are necessary for the environment at hand. For example, when performing disaster recovery in an environment with a non-trivial amount of CustomResourceDefinitions. In such an environment, the saving or restoring of Consul snapshots may take an extraordinary amount of time because of the sheer number of CRDs being synced from, and later to the Kubernetes cluster. If, when testing the backup procedure, CRD redeployment time turns out to be unacceptably long and you are willing to have a stopgap solution, it may be foresighted to think ahead of a situation where you need to first restore the basic functionality of the Consul cluster without CRDs. This means that the DRP should have a section outlining how to first manually back up the CRDs, then remove them from the cluster, and finally proceed with standard procedure of Consul snapshot taking, environment restore, and Consul snapshot restore.
