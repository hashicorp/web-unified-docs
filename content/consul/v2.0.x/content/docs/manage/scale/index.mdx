...
Using auto scaling groups with automatically removed Consul server nodes may cause severe outages in your Consul datacenter. To mitigate the risks associated with automatic scaling of Consul nodes, refer to [Prevent 'No cluster leader' issues](#prevent-no-cluster-leader-issues).
...
## Prevent Raft quorum issues

Consul servers rely on the Raft consensus algorithm to [ensure consistency of the cluster's state](/consul/docs/concept/consensus). 

As a result, you must reconfigure the server agents one node at a time, in a rolling fashion. That means after every node is permanently removed or added, you should allow the cluster to return to a healthy state before you continue. Ideally all nodes should be healthy, but a valid quorum is the minimum requirement before you take any other action.


Unexpected events or automations can break this rule and lead to different issues, including temporary unavailability, quorum loss that requires manual recovery, or even data loss. Consul outputs the error 'No cluster leader' when this occurs.

Example events and automations that can produce this 'No cluster leader' error include:

- VM failure in your datacenter
- Node failure in K8s cluster
- [AWS Auto Scaling Group](https://docs.aws.amazon.com/autoscaling/ec2/userguide/auto-scaling-groups.html) and [Spot Instances](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-spot-instances.html)
- [Azure VM Scale Sets](https://learn.microsoft.com/en-us/azure/virtual-machine-scale-sets/) and [Spot VMs](https://learn.microsoft.com/en-us/azure/virtual-machines/spot-vms)
- [GCP Managed Instance Group](https://cloud.google.com/compute/docs/instance-groups#managed_instance_groups), [SpotVms](https://cloud.google.com/compute/docs/instances/spot), and [Preemptible VMs](https://cloud.google.com/compute/docs/instances/preemptible)

In each situation, the sudden removal of Consul server agents may result in the servers moving into `failed` states, which jeopardizes the Raft peer set.

To make your Consul datacenter more resilient against this error, we recommend the following best practices.

### Create regular backups

Having a recent backup of your data is always the best situation to be in, even when no outages occur. Refer to [Backup and restore a Consul datacenter](/consul/docs/manage/disaster-recovery/backup-restore) to learn more. 

<EnterpriseAlert> 

Enterprise customers might also rely on [automated backups](/consul/docs/manage/scale/automated-backup) to establish a recurrent backup plan.

</EnterpriseAlert>


### Use Consul Autopilot <EnterpriseAlert inline /> 
...
When Consul server agents have [`leave_on_terminate`](/consul/docs/reference/agent/configuration-file/general#leave_on_terminate) set to `true`, agents that receive a `TERM` signal will send a leave message to the rest of the cluster and gracefully leave.

This setting does not fully shield your cluster from outages because nodes can always fail suddenly, without the opportunity to leave gracefully. However, it makes the automated scaling workflows less impactful.

<Note>

If you set `leave_on_terminate=true`, all server instances must leave the cluster, rejoin it, and download the snapshot from leader before functioning again. That includes nodes restarting the Consul agent process to apply the configuration change. Slower restarts may result.

</Note>

### Set `min_quorum`

In production environments, always set a Consul datacenter's [`min_quorum`](/consul/docs/reference/agent/configuration-file/bootstrap#min_quorum) so that Consul Autopilot stops pruning dead servers when it reaches this minimum.

### Recover from outages

Even with every mitigation setting in place, it is always possible to end up in a quorum loss situation where manual intervention is required.

To learn how to recover a Consul datacenter from an inconsistent state, refer to [manual recovery using peers.json](/consul/tutorials/operate-consul/recovery-outage#manual-recovery-using-peers-json).