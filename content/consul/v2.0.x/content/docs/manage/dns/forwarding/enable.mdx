...
created_at: 2026-03-31T20:45:14.000Z
last_modified: 2026-03-31T20:45:14.000Z
...
This configuration realizes full DNS forwarding, which means that all DNS queries for the host are forwarded to Consul, not just the ones for the `.consul` top level domain. Consul's default configuration resolves only the `.consul` top level domain, so you must set the [`recursors`](/consul/docs/reference/agent/configuration-file/dns#recursors) flag if you want your node to be able to resolve other domains when using `iptables` configuration.
