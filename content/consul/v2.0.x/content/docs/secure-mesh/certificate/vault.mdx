...
created_at: 2026-03-31T20:46:12.000Z
last_modified: 2026-03-31T20:46:12.000Z
...
- `AuthMethod` / `auth_method` (`map: nil`) - Vault auth method to use for logging in to Vault.
   ...
   **Security (CVE-2026-2808):** As of Consul 1.22.x, credential files referenced by auth method
   parameters (such as `path`, `token_path`, `role_id_file_path`, and `secret_id_file_path`) must
   reside within specific allowed directories. Consul uses OS-level path traversal protection
   (`os.OpenRoot()`) to prevent arbitrary file reads from credential file parameters.

   The allowed directories for each auth method are:

   | Auth Method  | Allowed Directories |
   |-------------|-------------------|
   | **Kubernetes** | `/var/run/secrets/kubernetes.io/serviceaccount`, `/run/secrets/kubernetes.io/serviceaccount` |
   | **JWT**        | `/var/run/secrets/kubernetes.io/serviceaccount`, `/var/run/secrets/vault`, `/run/secrets/vault`, `/var/run/secrets`, `/run/secrets` |
   | **AppRole**    | `/var/run/secrets/vault`, `/run/secrets/vault`, `/var/run/secrets`, `/run/secrets` |

   Credential files outside these directories are rejected. Symlinks that escape
   these directories are also blocked. Additionally, credential files are limited to a
   maximum size of **5 MB** to prevent denial-of-service attacks. If your deployment
   stores credential files in non-standard paths, you must relocate them to one of the
   listed allowed directories before upgrading.
