...
created_at: 2026-03-31T20:46:11.000Z
last_modified: 2026-03-31T20:46:11.000Z
...
To enable the Vault service mesh provider on a new installation, instead of enabling it on an existing Consul installation, refer to [Vault as the Service Mesh Certificate Provider on Kubernetes](/consul/docs/deploy/server/k8s/vault/data).
...
To renew the Vault token, use the [`vault token renew`](/vault/commands/token/renew) CLI command
or API.
