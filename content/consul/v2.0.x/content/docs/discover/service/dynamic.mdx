...
Consul supports prepared queries for services registered with a single port, which excludes [multiport services](/consul/docs/connect/proxy/multiport). You cannot filter for [a service's `ports` parameter](/consul/docs/reference/service#port) when using prepared queries.
