...
Consul supports prepared queries for services registered with a single port. You cannot filter for [a service's `ports` parameter](/consul/docs/reference/service#port) when using prepared queries.
