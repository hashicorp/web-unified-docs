...
To query multiport services, the virtual DNS entries are generated based on the service name and port name, following the naming convention `<port-name>.<service-name>.virtual.consul`. Consul's internal DNS returns a Virtual IP (VIP) unique to that specific service-port combination. For example, if you have a service named `api` with two ports named `http` and `grpc`, the virtual DNS entries would be `http.api.virtual.consul` and `grpc.api.virtual.consul`. For more information about multiport services, refer to the [multiport documentation](/consul/docs/connect/proxy/multiport).
