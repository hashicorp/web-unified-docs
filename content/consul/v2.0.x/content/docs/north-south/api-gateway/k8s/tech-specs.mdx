...
You must specify the `"hashicorp.com/consul-api-gateway-controller"` controller so that Consul can manage gateways generated by the `GatewayClass`. Refer to the [Kubernetes `GatewayClass` documentation](https://gateway-api.sigs.k8s.io/reference/spec/?h=gatewayclass#gatewayclass) for additional information.
...
You can apply filters to add, remove, and set header values on incoming requests. Gateways support the `terminate` TLS mode and `core/v1/Secret` TLS certificates. Extended option support includes TLS version and cipher constraints. Refer to [Kubernetes `Gateway` resource configuration reference](/consul/docs/reference/k8s/api-gateway/gateway) for more information.
...
Refer to [Kubernetes `TCPRoute` documentation](https://gateway-api.sigs.k8s.io/reference/spec/?h=tcproute#tcproute) for additional information.
...
Refer to the [Kubernetes `ReferenceGrant` documentation](https://gateway-api.sigs.k8s.io/reference/spec/?h=tcproute#referencegrant) for additional information.

## Consul server deployments

Both Consul Enterprise and the community edition are supported.
