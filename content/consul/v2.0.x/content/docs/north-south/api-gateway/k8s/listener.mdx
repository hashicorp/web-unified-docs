...
## Define the standard gateway and listeners.

Create an standard API gateway values file that defines the gateway and listeners.

1. Specify the following fields:
    - `apiVersion`: Specifies the Kubernetes gateway API version. Must be `gateway.networking.k8s.io/v1`.
...
spec:
  gatewayClassName: consul-custom-consul
  listeners:
  - protocol: HTTP
    port: 80
    name: http
...
## Deploy the Consul API Gateway and listeners.

Create a Consul API gateway values file that defines the gateway and listeners using Consul's native API.

1. Specify the following fields:
    - `apiVersion`: Specifies the Consul API version. Must be `consul.hashicorp.com/v1beta1`.
    - `kind`: Specifies the type of configuration entry to implement. This must be `Gateway`. 
    - `metadata.name`:  Specify a name for the gateway configuration. The name is metadata that you can use to reference the configuration when performing Consul operations.
    - `spec.gatewayClassName`:  Specify the name of a `gatewayClass` configuration. Gateway classes are template-like resources in Kubernetes for instantiating gateway services. Specify `consul` to use the default gateway class shipped with Consul. Refer to the [GatewayClass configuration reference](/consul/docs/reference/k8s/api-gateway/gatewayclass) for additional information.
    - `spec.listeners`: Specify a list of listener configurations. Each listener is map containing the following fields:
...
In the following example, the Consul API gateway specifies an HTTP listener on port `80`:
...
## Kubernetes SDS implementation

<Tabs>
<Tab heading="Standard Gateway (gateway.networking.k8s.io/v1)">
...
<Tab heading="Consul Gateway (consul.hashicorp.com/v1beta1)">

This section reflects the SDS behavior for Consul API gateways using `consul.hashicorp.com/v1beta1`:

- Listener-level SDS from Gateway annotations and listener `tls.options`
- Backend-level SDS override from `RouteTLSSDSFilter`

### Consul API gateway listener SDS source of truth
...
### Consul API gateway SDS resolution behavior
...
### Consul API gateway listener validation behavior

- If only one SDS field is provided, listener validation fails.
- SDS and `tls.certificateRefs` are validated independently and can both be configured on the same listener.

### Translation to Consul entries for Consul API gateway

- Resolved listener SDS is mapped to `APIGatewayListener.TLS.SDS`.
- Listener certificate refs are mapped to `APIGatewayListener.TLS.Certificates`.
- When both are configured, translated listener TLS includes both SDS and Certificates.

### Example: Consul API gateway listener SDS via annotations and listener `tls.options`

```yaml
apiVersion: consul.hashicorp.com/v1beta1
kind: Gateway
metadata:
  name: custom-sds-gateway
  namespace: default
  annotations:
    api-gateway.consul.hashicorp.com/tls_sds_cluster_name: sds-cluster
    api-gateway.consul.hashicorp.com/tls_sds_cert_resource: wildcard.ingress.consul
...
### Consul API gateway TLS SDS precedence
...
## Route service SDS overrides for Consul API gateway (Kubernetes)

`RouteTLSSDSFilter` remains the backend-level SDS override mechanism for HTTP routes with Consul API gateways.
...
### Example: HTTPRoute backendRef override using RouteTLSSDSFilter with Consul API gateway

```yaml
apiVersion: consul.hashicorp.com/v1beta1
kind: HTTPRoute
metadata:
  name: custom-api-route-override
  namespace: default
spec:
  parentRefs:
  - name: custom-sds-gateway
    ...
    - name: svc-b
      ...
      - type: ExtensionRef
        extensionRef:
          group: consul.hashicorp.com
          kind: RouteTLSSDSFilter
          name: custom-route-sds-override-http
---
apiVersion: consul.hashicorp.com/v1alpha1
kind: RouteTLSSDSFilter
metadata:
...
### Custom gateway troubleshooting checklist
