...
## Deploy the custom gateway and listeners.

Create a custom API gateway values file that defines the gateway and listeners using Consul's native API.
...
In the following example, the custom API gateway specifies an HTTP listener on port `80`:
...
<Tab heading="Custom Gateway (consul.hashicorp.com/v1beta1)">

This section reflects the SDS behavior for custom gateways using `consul.hashicorp.com/v1beta1`:

- Listener-level SDS from Gateway annotations and listener `tls.options`
- Backend-level SDS override from `RouteTLSSDSFilter`

### Custom gateway listener SDS source of truth
...
### Custom gateway SDS resolution behavior
...
### Custom gateway listener validation behavior

- If only one SDS field is provided, listener validation fails.
- SDS and `tls.certificateRefs` are validated independently and can both be configured on the same listener.

### Translation to Consul entries for custom gateway

- Resolved listener SDS is mapped to `APIGatewayListener.TLS.SDS`.
- Listener certificate refs are mapped to `APIGatewayListener.TLS.Certificates`.
- When both are configured, translated listener TLS includes both SDS and Certificates.

### Example: Custom gateway listener SDS via annotations and listener `tls.options`
...
### Custom gateway TLS SDS precedence
...
## Route service SDS overrides for custom gateway (Kubernetes)

`RouteTLSSDSFilter` remains the backend-level SDS override mechanism for HTTP routes with custom gateways.
...
### Example: HTTPRoute backendRef override using RouteTLSSDSFilter with custom gateway
