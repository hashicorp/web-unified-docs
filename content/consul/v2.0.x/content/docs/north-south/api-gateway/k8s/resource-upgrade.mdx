...
2. Verify Consul is deployed in the Cluster
3. Validate if the gateway CRDs under API group `gateway.networking.k8s.io` are pre-installed in the cluster. If pre-installed, set `global.installK8sNetworkingCRDs: false`.  
4. Make sure Gateway-related resources are backed up before the upgrade.
