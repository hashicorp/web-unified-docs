...
This change is necessary if application uses TCPRoute along with API gateway functionality on specific Kubernetes environments, like OpenShift or Gateway CRDs managed by the platform.
...
- Any Kubernetes cluster that uses TCPRoutes and Gateway CRDs are managed by the platform.

    Moving to the new `consul.hashicorp.com` custom API resource group is required.
    
- Any Kubernetes cluster that uses TCPRoutes and Gateway CRDs are not managed by the platform.
...
1. Update your Consul Helm chart values. 
    ...
    openshift:
        ...
        crds:
            ...
            consulapi:
                enabled: true
    ```
    If `openshift version` is less than equal to 4.18
    ...
    If `openshift version` is less than equal to 4.19
    If `openshift.crds.enableTcpRoute` is true, or 
    `openshift.crds.consulapi.enabled` is true, then install all the CRDs under the API group `consul.hashicorp.com`.
