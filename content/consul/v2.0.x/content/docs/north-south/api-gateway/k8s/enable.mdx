...
# Enable stable API gateway for Kubernetes
...
1. The Consul Helm chart deploys the API gateway using the configuration specified in the `values.yaml` file. Refer to [Helm Chart Configuration - `connectInject.apiGateway`](/consul/docs/reference/k8s/helm#apigateway) for information about the Helm chart configuration options. Create a `values.yaml` file for configuring your Consul API gateway deployment and include the following settings:
  ...
  <Tabs>
    ...
    <Tab heading="Kubernetes Clusters with Gateway CRDs managed cluster platforms">
    <CodeBlockConfig filename="values.yaml">
    ```yaml
     global:
       installK8sNetworkingCRDs: false
...
# Enable new custom API gateway for Kubernetes
...
Starting with Consul 2.0, Consul reconciles only API gateways that are controlled by Consul and contain the required labels in the API gateway object. Consul validates the object at two levels: the object must contain the appropriate label and the controller name of the object must match.

## Standard gateway

For the standard Kubernetes Gateway API, use the following labels:
...
For the custom Consul Gateway resource, use the following labels:
