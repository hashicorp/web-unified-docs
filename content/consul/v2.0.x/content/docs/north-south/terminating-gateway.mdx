...
created_at: 2026-03-31T20:45:29.000Z
last_modified: 2026-03-31T20:45:29.000Z
...
field along with the file path fields (`caFile`, `certFile`, `keyFile`) in the terminating gateway configuration entry to enable
TLS via Envoy's Secret Discovery Service (SDS). When SDS is enabled, certificate updates from the referenced Kubernetes secret
are dynamically delivered to Envoy without requiring a gateway restart.
