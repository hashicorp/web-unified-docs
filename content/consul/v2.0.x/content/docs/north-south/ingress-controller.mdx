...
~> If you are looking for a fully supported solution for ingress traffic into Consul Service Mesh, please visit [Consul API Gateway](/consul/docs/north-south/api-gateway) for instruction on how to install Consul API Gateway along with Consul on Kubernetes.
