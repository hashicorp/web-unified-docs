...
created_at: 2026-03-31T20:46:31.000Z
last_modified: 2026-03-31T20:46:31.000Z
# END AUTO GENERATED METADATA
---

# Upgrading Specific Versions

The [upgrading page](/consul/docs/upgrade) covers the details of doing a
...
For step-by-step instructions on upgrading from Consul Enterprise 1.21.x (LTS) to 2.0.x, refer to [Upgrade Consul to v2.0.x](/consul/docs/upgrade/instructions/upgrade-to-2-0-x).

</Note>

### Versioning scheme change

Beginning with Consul 2.0.0, the release versioning format changed from semantic versioning (`X.Y.Z`) to IBM's Version-Modification-Fix (`V.M.F`) model. A new `V` (Version) milestone starts a new support lifecycle, an `M` (Modification) milestone adds features without starting a new lifecycle, and monthly `F` (Fix) releases patch the existing release. This change does not require any configuration updates, but it does affect how you plan future upgrades and read release numbers. For more information, refer to [Consul Enterprise Long Term Support](/consul/docs/upgrade/lts).

### IBM Consul Enterprise license prerequisite

If you use a HashiCorp Consul Enterprise license, you must upgrade to Consul Enterprise 1.21.7 or later and switch to an IBM Consul Enterprise license before upgrading to 2.0.x. Attempting to upgrade directly to 2.0.x with a HashiCorp-issued license causes agents to fail to start. For the complete process, refer to [Upgrade to IBM Consul Enterprise license](/consul/docs/upgrade#upgrade-to-ibm-consul-enterprise-license).

### Multi-port service mesh routing is Enterprise-only

Consul 2.0.0 introduces multi-port routing for service mesh, letting a single sidecar proxy serve a service that exposes multiple named ports. This feature, and the related `proxy.local_service_ports` and `proxy.upstreams[].destination_port` configuration fields, requires Consul Enterprise. Community Edition deployments are unaffected by this change and continue to register single-port services as before. For more information, refer to [multi-port services](/consul/docs/connect/proxy/multiport).

### API gateway Kubernetes resource types changed for OpenShift compatibility

Consul 2.0.0 introduces new API gateway Kubernetes resource types in the `consul.hashicorp.com` API group to support OpenShift Container Platform (OCP) 4.19 and later, which are incompatible with the `v1alpha` Kubernetes Gateway API resources used by previous Consul releases. If you run Consul API Gateway on Kubernetes, and especially on OCP, you must migrate existing `Gateway`, `HTTPRoute`, `TCPRoute`, and `ReferenceGrant` resources to the new resource types as part of your upgrade. For step-by-step instructions, refer to [Upgrade Gateway resource types](/consul/docs/north-south/api-gateway/k8s/resource-upgrade).

### Opt-in telemetry reporting introduced

Consul 2.0.0 adds an opt-in telemetry reporting feature that is **disabled by default**. No action is required to preserve existing behavior. If you want to enable anonymized usage reporting, refer to [Opt-in to telemetry data reporting](/consul/docs/monitor/telemetry#opt-in-to-data-reporting).

## Consul 1.21.x

### License update upgrade procedure <EnterpriseAlert inline />

Complete the following steps to upgrade to Consul 1.21.7+ent or later with the updated license:

1. Upgrade the server agents first, followed by the client agents, to version **1.21.7+ent** or later.

1. Update the license on both the server and client agents to **enterprise-standard**.

1. Restart the **server agents only**, one at a time. The servers are expected to operate normally, and the client agents continue to function using the existing license.

1. Restart the **client agents**. The clients will start successfully with the new license.

## Consul 1.20.x

### Mesh traffic request path normalization enabled by default

As of Consul v1.20.1, inbound traffic to mesh proxies will have Envoy request [path normalization](https://www.envoyproxy.io/docs/envoy/latest/api-v3/extensions/filters/network/http_connection_manager/v3/http_connection_manager.proto#envoy-v3-api-field-extensions-filters-network-http-connection-manager-v3-httpconnectionmanager-normalize-path) applied by default. This should not interfere with the majority of service traffic, but can be disabled if needed by setting `http.incoming.request_normalization.insecure_disable_path_normalization` to `true` in the [global `mesh` configuration entry](/consul/docs/reference/config-entry/mesh#request-normalization). This setting is generally safe to change if not using L7 intentions with path matching.
...
[`connectInject.apiGateway`](/consul/docs/reference/k8s/helm#v-connectinject-apigateway)
...
[API gateway](/consul/docs/reference/config-entry/api-gateway).
...
that did not explicitly specify a namespace or partition incorrectly used service-defaults configurations, such as [default protocol](https://developer.hashicorp.com/consul/docs/reference/config-entry/service-defaults#set-the-default-protocol), from the default partition and namespace instead of the local partition and namespace.
...
When configuring a service defaults configuration entry, the [`UpstreamConfig.Overrides` configuration](/consul/docs/reference/config-entry/service-defaults#upstreamconfig-overrides)
does not apply to peer upstreams unless the [`Peer`](/consul/docs/reference/config-entry/service-defaults#upstreamconfig-overrides-peer) field is explicitly provided.
...
In Consul 1.14.x, `service-defaults` upstream [`overrides`](/consul/docs/reference/config-entry/service-defaults#overrides) apply to both local and peered services as long as the `name` field matches.
Consul 1.15.0 is backward compatible with 1.14 if the [`peer`](/consul/docs/reference/config-entry/service-defaults#peer) is not set in any override.
...
- [Cluster Peering is enabled by default.](/consul/docs/east-west/cluster-peering)
  ...
  To disable cluster peering nonetheless, set [`peering.enabled`](/consul/docs/reference/agent/configuration-file#peering_enabled) to `false`.
- [Consul service mesh is enabled by default.](/consul/docs/connect)
  To disable, set [`connect.enabled`](/consul/docs/reference/agent/configuration-file#connect_enabled) to `false`.
...
**Make configuration changes** if using [`ports.grpc`](/consul/docs/reference/agent/configuration-file/general#grpc_port) in conjunction with any of the following settings that enables encryption:
1. [`tls.grpc`](/consul/docs/reference/agent/configuration-file#tls_grpc)
1. [`tls.defaults`](/consul/docs/reference/agent/configuration-file#tls_defaults)
1. [`auto_encrypt`](/consul/docs/reference/agent/configuration-file#auto_encrypt)
1. [`auto_config`](/consul/docs/reference/agent/configuration-file#auto_config)
...
Consul 1.14 introduces [`ports.grpc_tls`](/consul/docs/reference/agent/configuration-file#grpc_tls_port), a new configuration
for encrypting communication over gRPC. The existing [`ports.grpc`](/consul/docs/reference/agent/configuration-file/general#grpc_port) configuration **no longer supports encryption**. As of version 1.14,
[`ports.grpc_tls`](/consul/docs/reference/agent/configuration-file/general#grpc_tls_port) is the only port that serves encrypted gRPC traffic.
...
+ Change `ports.grpc` to `ports.grpc_tls`. Refer to the [`grpc_tls_port` documentation](/consul/docs/reference/agent/configuration-file#grpc_tls_port) for details.
+ Change `addresses.grpc` to `addresses.grpc_tls`. Refer to the [`grpc_tls` documentation](/consul/docs/reference/agent/configuration-file#grpc_tls) for details.
...
[Cluster peering](/consul/docs/east-west/cluster-peering) was released in Consul 1.13 as an experimental feature.
...
[auto-encrypt](/consul/docs/reference/agent/configuration-file#auto_encrypt) or
[auto-config](/consul/docs/reference/agent/configuration-file#auto_config).
...
[tls.grpc.use_auto_cert](/consul/docs/reference/agent/configuration-file#use_auto_cert)
...
([`ports { https = POSITIVE_INTEGER }`](/consul/docs/reference/agent/configuration-file#https_port))
...
Prior to Consul 1.13, if [`ports { https = POSITIVE_INTEGER }`](/consul/docs/reference/agent/configuration-file#https_port)
...
1. [`tls.grpc`](/consul/docs/reference/agent/configuration-file#tls_grpc), which overrides
1. [`tls.defaults`](/consul/docs/reference/agent/configuration-file#tls_defaults), which overrides
1. [Deprecated TLS options](/consul/docs/reference/agent/configuration-file#tls_deprecated_options) such as
   [`ca_file`](/consul/docs/reference/agent/configuration-file#ca_file-4),
   [`cert_file`](/consul/docs/reference/agent/configuration-file#cert_file-4), and
   [`key_file`](/consul/docs/reference/agent/configuration-file#key_file-4).
...
If [`tls.grpc`](/consul/docs/reference/agent/configuration-file#tls_grpc),
[`tls.defaults`](/consul/docs/reference/agent/configuration-file#tls_defaults),
or the [deprecated TLS options](/consul/docs/reference/agent/configuration-file#tls_deprecated_options)
...
1. Remove TLS material from the Consul agents'
   ...
   [`tls.defaults`](/consul/docs/reference/agent/configuration-file#tls_grpc) and
   [deprecated TLS options](/consul/docs/reference/agent/configuration-file#tls_deprecated_options)
1. Reapply TLS material to the non-gRPC interfaces that need it with the
   ...
   [`tls.https`](/consul/docs/reference/agent/configuration-file#tls_https) and
   [`tls.internal_rpc`](/consul/docs/reference/agent/configuration-file#tls_internal_rpc).
...
- The Consul server agent configuration option [`connect.ca_provider`](/consul/docs/reference/agent/configuration-file#connect_ca_provider) is set to `vault`, or
- The Consul on Kubernetes Helm Chart [`global.secretsBackend.vault.connectCA`](/consul/docs/reference/k8s/helm#v-global-secretsbackend-vault-connectca) value is configured.
...
The [`disable_compat_19`](/consul/docs/reference/agent/configuration-file#telemetry-disable_compat_1.9) telemetry configuration option is now removed.
...
The [`disable_compat_19`](/consul/docs/reference/agent/configuration-file#telemetry-disable_compat_1.9) telemetry configuration option now defaults
...
be replaced with the new [`tls` stanza](/consul/docs/reference/agent/configuration-file#tls-configuration-reference):
...
These config keys are now deprecated:
 ...
 - `audit.sink[].name`
 - [`dns_config.dns_prefer_namespace`](/consul/docs/reference/agent/configuration-file#dns_prefer_namespace)
...
or environment prior to starting. See the [licensing documentation](/consul/docs/enterprise/license)
...
agents require that either the [`start_join`](/consul/docs/reference/agent/configuration-file#start_join) or
[`retry_join`](/consul/docs/reference/agent/configuration-file#retry_join) configurations are set and that they resolve to server
...
For the step by step upgrade procedures see the [Upgrading to 1.10.0](/consul/docs/upgrade/instructions/upgrade-to-1-10-x) documentation.
...
The [`enable_central_service_config`](/consul/docs/reference/agent/configuration-file/general#enable_central_service_config)
...
[intentions](/consul/docs/secure-mesh/intention) into an equivalent set of
...
- `Intention.ID` after migration is stored in the
  [`LegacyID`](/consul/docs/reference/config-entry/service-intentions#legacyid) field.
  After transitioning this field is cleared.

- `Intention.CreatedAt` after migration is stored in the
  [`LegacyCreateTime`](/consul/docs/reference/config-entry/service-intentions#legacycreatetime)
  field. After transitioning this field is cleared.

- `Intention.UpdatedAt` after migration is stored in the
  [`LegacyUpdateTime`](/consul/docs/reference/config-entry/service-intentions#legacyupdatetime)
  field. After transitioning this field is cleared.

- `Intention.Meta` after migration is stored in the
  [`LegacyMeta`](/consul/docs/reference/config-entry/service-intentions#legacymeta)
  ...
  [`Meta`](/consul/docs/reference/config-entry/service-intentions#meta) field. This is
...
The [`acl_enforce_version_8`](/consul/docs/reference/agent/configuration-file#acl_enforce_version_8)
...
Consul has changed the semantics of query counts in its [telemetry](/consul/docs/reference/agent/telemetry).
...
Consul 1.7.0 introduced [limiting of connections per client](/consul/docs/reference/agent/configuration-file#http_max_conns_per_client). The default value
...
Consul 1.6.3 introduced [limiting of connections per client](/consul/docs/reference/agent/configuration-file#http_max_conns_per_client). The default value
...
[removed](/consul/docs/connect/proxy). Before upgrading, you must
migrate any managed proxy usage to [sidecar service
registrations](/consul/docs/connect/proxy/sidecar).
...
block](/consul/docs/reference/agent/configuration-file#acl).
...
In addition, this upgrade will only affect clusters where [service mesh is enabled](/consul/docs/connect) on your servers before the migration.
...
and makes them tunable via the [raft_snapshot_interval](/consul/docs/reference/agent/configuration-file#_raft_snapshot_interval) and
[raft_snapshot_threshold](/consul/docs/reference/agent/configuration-file#_raft_snapshot_threshold) parameters. We recommend
keeping the new defaults. However, operators can go back to the old defaults by changing their
config if they prefer more frequent snapshots. See the documentation for [raft_snapshot_interval](/consul/docs/reference/agent/configuration-file#_raft_snapshot_interval)
and [raft_snapshot_threshold](/consul/docs/reference/agent/configuration-file#_raft_snapshot_threshold) to understand the trade-offs
when tuning these.
...
3](/consul/docs/reference/agent/configuration-file#_raft_protocol) had an issue where performing
...
The [`-raft-protocol`](/consul/commands/agent#_raft_protocol) default has
...
as described on the [Protocol Compatibility Promise](/consul/docs/upgrade/compatibility)
...
[`-node-id`](/consul/commands/agent#_node_id) instead of their IP address
...
[`-config-file`](/consul/commands/agent#_config_file) argument to specify a
file directly.
...
| ------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `-dc`                                                  | [`-datacenter`](/consul/commands/agent#_datacenter)                                                                                                                       |
| `-retry-join-azure-tag-name`                           | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-azure-tag-value`                          | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-ec2-region`                               | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-ec2-tag-key`                              | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-ec2-tag-value`                            | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-gce-credentials-file`                     | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-gce-project-name`                         | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-gce-tag-name`                             | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `-retry-join-gce-zone-pattern`                         | [`-retry-join`](/consul/commands/agent#_retry_join)                                                                                                                       |
| `addresses.rpc`                                        | None, the RPC server for CLI commands is no longer supported.                                                                                                          |
| `advertise_addrs`                                      | [`ports`](/consul/docs/reference/agent/configuration-file#ports) with [`advertise_addr`](/consul/docs/reference/agent/configuration-file#advertise_addr) and/or [`advertise_addr_wan`](/consul/docs/reference/agent/configuration-file#advertise_addr_wan) |
| `dogstatsd_addr`                                       | [`telemetry.dogstatsd_addr`](/consul/docs/reference/agent/configuration-file#telemetry-dogstatsd_addr)                                                                                             |
| `dogstatsd_tags`                                       | [`telemetry.dogstatsd_tags`](/consul/docs/reference/agent/configuration-file#telemetry-dogstatsd_tags)                                                                                             |
| `legacy/http_api_response_headers`                            | [`http_config.response_headers`](/consul/docs/reference/agent/configuration-file#response_headers)                                                                                                 |
| `ports.rpc`                                            | None, the RPC server for CLI commands is no longer supported.                                                                                                          |

| `recursor`                                             | [`recursors`](/consul/docs/reference/agent/configuration-file#recursors)                                                          |
| `retry_join_azure`                                     | [`retry-join`](/consul/docs/reference/agent/configuration-file#retry_join)                                                                                                                       |
| `retry_join_ec2`                                       | [`retry-join`](/consul/docs/reference/agent/configuration-file#retry_join)                                                                                                                       |
| `retry_join_gce`                                       | [`retry-join`](/consul/docs/reference/agent/configuration-file#retry_join)                                                                                                                       |
| `statsd_addr`                                          | [`telemetry.statsd_address`](/consul/docs/reference/agent/configuration-file#telemetry-statsd_address)                            |
| `statsite_addr`                                        | [`telemetry.statsite_address`](/consul/docs/reference/agent/configuration-file#telemetry-statsite_address)                        |
| `statsite_prefix`                                      | [`telemetry.metrics_prefix`](/consul/docs/reference/agent/configuration-file#telemetry-metrics_prefix)                                                                                             |
| `telemetry.statsite_prefix`                            | [`telemetry.metrics_prefix`](/consul/docs/reference/agent/configuration-file#telemetry-metrics_prefix)                                                                                             |
...
[`metrics_prefix`](/consul/docs/reference/agent/configuration-file#telemetry-metrics_prefix).
...
The format for [`-retry-join`](/consul/commands/agent#retry-join) and
[`-retry-join-wan`](/consul/commands/agent#retry-join-wan) values that use
...
A new [`enable_script_checks`](/consul/commands/agent#_enable_script_checks)
...
series and can be enabled with the [`-ui`](/consul/commands/agent#_ui)
...
[`-ui-dir`](/consul/commands/agent#_ui_dir) option is still available for
...
[`acl_datacenter`](/consul/docs/reference/agent/configuration-file#acl_datacenter) must be set on
...
The default for
[`disable_remote_exec`](/consul/docs/reference/agent/configuration-file#disable_remote_exec) was
...
to set the [`-raft-protocol`](/consul/commands/agent#_raft_protocol) option
...
The default for [`max_stale`](/consul/docs/reference/agent/configuration-file#max_stale) has been
...
defaulting [`allow_stale`](/consul/docs/reference/agent/configuration-file#allow_stale) to true for
...
[performance configuration block](/consul/docs/reference/agent/configuration-file#performance). Also,
...
See the [Server Performance](/consul/docs/reference/architecture/server) guide for more details.

#### Leave-Related Configuration Defaults

The default behavior of [`leave_on_terminate`](/consul/docs/reference/agent/configuration-file#leave_on_terminate)
and [`skip_leave_on_interrupt`](/consul/docs/reference/agent/configuration-file#skip_leave_on_interrupt)
...
[HTTP endpoints](/consul/docs/reference/agent/configuration-file#translate_wan_addrs). The servers
...
[`acl_token`](/consul/docs/reference/agent/configuration-file#acl_token) is used, so be sure
...
[Consul Tools page](/consul/docs/integrate/consul-tools).
