...
For step-by-step instructions, example manifests, and connection examples with transparent proxy enabled or disabled, refer to [Configure multi-port services on Kubernetes](/consul/docs/connect/k8s/multiport). For general multi-port concepts and limitations that apply across all platforms, refer to [multi-port services](/consul/docs/connect/proxy/multiport).
