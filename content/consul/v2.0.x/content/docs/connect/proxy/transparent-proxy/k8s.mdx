---
layout: docs
page_title:  Enable transparent proxy mode on Kubernetes
description: >-
...
created_at: 2026-03-31T20:44:38.000Z
last_modified: 2026-03-31T20:44:38.000Z
# END AUTO GENERATED METADATA
---

# Enable transparent proxy mode on Kubernetes
...
Transparent proxy mode supports multi-port services. You do not need to modify the downstream or upstream service deployments to enable multi-port communication. Configure the required configuration entries, such as [service intentions](/consul/docs/reference/config-entry/service-intentions) and [exported services](/consul/docs/reference/config-entry/exported-services) for cross-partition communication, to enable multi-port communication through the service mesh.

Applications in the mesh can address a specific port on a multi-port upstream service using the virtual address format `<port-name>.<service>.virtual.consul`.

~> **Protocol limitation:** Consul does not support a combination of protocols for a multi-port service. All ports of a service must use the same protocol.
