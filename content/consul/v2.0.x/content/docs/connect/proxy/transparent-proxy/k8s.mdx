...
Transparent proxy mode assumes that services have only one port. If you want multi-port services to be reachable from the service mesh, you must use explicit upstreams as well. After the upstreams are configured, workloads can address a named port on the destination service by using the virtual address format `<port-name>.<service>.virtual.consul`.
