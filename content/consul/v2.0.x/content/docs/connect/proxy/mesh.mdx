...
1. Configure any additional proxy behaviors that you want to implement in the `proxy` block. 
1. Specify a port number where other services registered with Consul can discover and connect to the proxies service in the `port` field. For services running on multiple ports, you can specify the `ports` parameter instead. To ensure that services only allow external connections established through the service mesh protocol, you should configure all services to only accept connections on a loopback address.
