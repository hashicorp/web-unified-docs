...
- **URL Path Prefix Rewrite** This release introduces support for rewriting a
  ...
  documentation](/consul/docs/north-south/api-gateway/k8s/reroute) for additional information.  
...
- **Reference Policy Renamed to Reference Grant** In v0.5.0 of the Kubernetes
  ...
  existing `ReferencePolicy` to `ReferenceGrant`. Refer to the [Upgrades](/consul/docs/upgrade/api-gateway)
...
- Envoy proxy support is determined by the Consul version deployed. Refer to
  [Envoy Integration](/consul/docs/reference/proxy/envoy) for details.
