...
created_at: 2026-03-31T20:44:49.000Z
last_modified: 2026-03-31T20:44:49.000Z
...
For more information, refer to the [Consul agents configuration file reference](/consul/docs/reference/agent/configuration-file).
...
We recommend using the ["k8s" cloud auto-join provider](/consul/docs/deploy/server/cloud-auto-join#kubernetes-k8s) to join a cluster running within Kubernetes.
...
The following command has a Consul agent join a cluster running the [official Helm chart](/consul/docs/reference/k8s/helm). This command assumes that there are Consul servers listening on the default [LAN Gossip](consul/docs/concept/gossip) port of 8301.
...
In many hosted Kubernetes environments, you need to explicitly configure your hosting provider to ensure that pod IPs are routable from external VMs.
For more information, refer to hosting provider documentation, such as [Azure AKS CNI](https://docs.microsoft.com/en-us/azure/aks/concepts-network#azure-cni-advanced-networking), [AWS EKS CNI](https://docs.aws.amazon.com/eks/latest/userguide/pod-networking.html) and [GKE VPC-native clusters](https://cloud.google.com/kubernetes-engine/docs/concepts/alias-ips).
...
To join external agents with Consul on Kubernetes deployments installed with default values through the [official Helm chart](/consul/docs/reference/k8s/helm):
  ...
  to the VM's advertised IP on its gossip port (default 8301).
...
If you are unable to use auto-join, try following the instructions in either of the auto-join sections, but instead of using a `provider` key in the `-retry-join` flag, pass the address of at least one Consul server. Example: `-retry-join=$CONSUL_SERVER_IP:$SERVER_SERFLAN_PORT`.
