...
last_modified: 2026-04-02T08:46:50.000Z
...
1. Verify that you have access to the Consul chart:
   ...
   hashicorp/consul	2.0.x        	2.0.x    	Official HashiCorp Consul Chart
   ```
...
Identify the OpenShift version and determine the type of gateway.

<Tabs>
  <Tab heading="OpenShift <= 4.18">
...
```
  ...
  <Tab heading="OpenShift >= 4.19 and No TCP Route - Use Standard Gateway">
...
```
  ...
  <Tab heading="OpenShift >= 4.19 and TCP Route - Use Consul Gateway">
  <CodeBlockConfig filename="values.yaml">
...
| Platform   | Condition                                    | Gateway              | Helm Values                                                                                                                                 |
| ---------- | -------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- |
| OCP < 4.19 | Always                                       | Standard API Gateway | `installK8sNetworkingCrds=true`, `openshift.crds.enableTcpRoute=true`                                            |
| OCP ≥ 4.19 | TCPRoute missing                             | Standard API Gateway | `installK8sNetworkingCrds=false`                                                                                 |
| OCP ≥ 4.19 | TCPRoute present                             | Consul API Gateway   | `installK8sNetworkingCrds=false`, `openshift.crds.enableTcpRoute=false`, `openshift.crds.consulapi.enabled=true` |
| Non-OCP    | Platform does not manage CRDs                | Standard API Gateway | `installK8sNetworkingCrds=true`                                                                                 |
| Non-OCP    | Platform manages CRDs, TCPRoute not required | Standard API Gateway | `installK8sNetworkingCrds=false`                                                                                |
| Non-OCP    | Platform manages CRDs, TCPRoute required     | Consul API Gateway   | `installK8sNetworkingCrds=false`, `crds.enableTcpRoute=false`, `crds.consulapi.enabled=true`                     |
