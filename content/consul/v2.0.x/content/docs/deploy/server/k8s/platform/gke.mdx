...
created_at: 2026-03-31T20:44:43.000Z
last_modified: 2026-03-31T20:44:43.000Z
...
The content of the file differs based on whether you are using Workload Identity Federation (WIF) for GKE or not. If WIF is disabled, the following Helm values are sufficient to deploy Consul on Kubernetes.
...
If you use Workload Identity Federation (WIF) to manage access between your GKE cluster and GCP resources, you must add an annotation to the Consul client service account. Replace `GSA_NAME` and `PROJECT_ID` with your GCP service account name and project ID.

<CodeBlockConfig filename="values.yaml" highlight="28">
...
When using WIF, the `consul-server-acl-init` container requires a custom service account that has the necessary permissions to query Google Compute resources. 

First, generate a GCP IAM service account definition using Helm template and your Consul values file. 
...
Then create your Consul namespace and make sure your namespace name matches the one you will use to deploy Consul. By default, Consul on Kubernetes using the `consul` namespace.
...
Finally, apply the service account definition to the namespace.
