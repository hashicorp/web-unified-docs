...
`bootstrap_expect` alone is not sufficient to form a cluster. Consul servers do not passively wait for peers to connect. Each server actively reaches out to find peers using `retry_join`. Without `retry_join`, a server with `bootstrap_expect = 3` fails immediately because it has no mechanism to discover the other servers. Always configure `retry_join` alongside `bootstrap_expect` when deploying a multi-server cluster. The two parameters are complementary, not mutually exclusive. Do not use `bootstrap` (single-server mode) together with `bootstrap_expect`.
