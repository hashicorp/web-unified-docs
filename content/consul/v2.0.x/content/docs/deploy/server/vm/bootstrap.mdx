...
created_at: 2026-03-31T20:44:47.000Z
last_modified: 2026-03-31T20:44:47.000Z
...
`bootstrap_expect` alone is not sufficient to form a cluster. Consul servers do not passively wait for peers to connect — each server actively reaches out to find peers using `retry_join`. Without `retry_join`, a server with `bootstrap_expect = 3` will fail immediately because it has no mechanism to discover the other servers. Always configure `retry_join` alongside `bootstrap_expect` when deploying a multi-server cluster. The two parameters are complementary, not mutually exclusive. Do not use `bootstrap` (single-server mode) together with `bootstrap_expect`.
...
- [Register service](/consul/docs/register/service/vm)
