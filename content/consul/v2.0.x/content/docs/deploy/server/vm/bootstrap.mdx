...
The following command runs a single Consul server agent. It also instructs it to wait for three Consul server agents in total to be joined, before the Consul cluster is deemed operational and bootstrapped.

```shell-session
$ consul agent -data-dir /tmp/consul -server -bootstrap-expect 3
```

Since `-bootstrap-expect` has been set to three, you need to launch two more instances of Consul server agent, and then [join them](#join-the-servers) for the cluster to be operational.
