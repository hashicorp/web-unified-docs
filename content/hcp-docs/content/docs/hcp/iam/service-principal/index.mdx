...
This capability is especially important for automated workflows using [Workload Identity Federation](/hcp/docs/hcp/iam/service-principal/workload-identity-federation), which requires project-level service principals. By assigning organization-level roles (like `Project Creator` or `Group Manager`) to a project-level service principal, you can automate your entire HCP infrastructure without needing long-lived, organization-level static keys.
