...
You can temporarily suspend your existing SSO connection without deleting existing configurations. This approach is useful for operations such as troubleshooting, identity provider outages, and policy changes. When you suspend SSO, [user invitations](/hcp/docs/hcp/iam/users) automatically activate. That means existing users can invite other users to your HCP organization according to your current [access management settings](/hcp/docs/hcp/iam/access-management). Suspending SSO will **not** delete users connected to the organization through SSO.
...
When you delete an SSO configuration, no SSO user can sign in to HCP. All current SSO users will be **deleted** from the organization.
