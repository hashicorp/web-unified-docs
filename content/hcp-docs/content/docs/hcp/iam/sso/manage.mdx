...
### Direct organization login URLs

Users can bookmark a direct login URL to streamline access to their HCP organization. This URL bypasses the standard login flow where users must first visit the HCP portal, enter their email address, and then get redirected to their identity provider.

The direct login URL format varies by region:

- **US region:** `https://api.cloud.hashicorp.com/auth/cloud-idp/login?sso-org-id=<ORG_ID>`
- **EU region:** `https://api.cloud.eu.hashicorp.com/auth/cloud-idp/login?sso-org-id=<ORG_ID>`

Replace `<ORG_ID>` with your organization's ID. This URL works for both SAML and OIDC SSO configurations.
...
You can temporarily suspend your existing SSO connection without deleting existing configurations. This approach is useful for operations such as troubleshooting, identity provider outages, and policy changes. When you suspend SSO, [user invitations](/hcp/docs/hcp/iam/users) automatically activate. That means existing users can invite other users to your HCP organization according to your current [access management settings](/hcp/docs/hcp/iam/access-management). Additionally, suspending SSO will **prevent SSO login and block access for users who only have SSO configured (no password/Github set up).** Suspending SSO will **not** delete users connected to the org through SSO.
...
When you delete an SSO configuration, no SSO user can sign in to HCP. All current SSO users will be **deleted** from the org.
