...
- Groups: HCP groups that have the same name as a group managed by the identity provider are updated in place by the identity provider group. The group retains the existing HCP/TFC team object ID, and the associated team token remains valid. The identity provider begins managing the group's lifecycle and updates all existing HCP group members with the members from your identity provider group.
