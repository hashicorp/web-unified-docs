...
- Groups:  HCP groups that have the same name as a group managed by the identity provider are not deleted and recreated. They retain the existing HCP/TFC team object ID, and the associated team token remains valid.
