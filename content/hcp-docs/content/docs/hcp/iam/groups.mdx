...
If SCIM provisioning is enabled for your organization, groups are automatically synchronized from your identity provider. Manual group creation is still available through HCP. But if there are any name conflicts between a locally created group and what is in your identity provider, SCIM take over the management of the group and keep it in sync with your identity provider. For more information, refer to [SCIM provisioning](/hcp/docs/hcp/iam/sso/scim).
