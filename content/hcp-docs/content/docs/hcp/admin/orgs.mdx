...
Organizations provide centralized governance through [access control](/hcp/docs/hcp/iam/access-management) and [enterprise constraints](/hcp/docs/hcp/admin/enterprise-constraints). Access control manages who can perform actions, while enterprise constraints let organizations restrict HCP Product usage for cost control and compliance.
...
- **Billing**: Confirms there are no active contracts, unpaid invoices, or billable resources that would block deletion.
  - When you delete an organization, HCP generates a final invoice for any outstanding charges, collects any remaining balance, and sends a receipt to the billing contact email on file.
