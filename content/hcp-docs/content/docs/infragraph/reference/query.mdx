...
A `node` block defines which nodes to match at the current step of the traversal. You must specify at least one of `unifiedTypes` or `sourceTypes`, and can optionally provide a specific `ids` filter.

- Default: None
- This field is required
- Data type: Object containing at least one [`unifiedTypes`](#unifiedtypes) or [`sourceTypes`](#sourcetypes). You can define both.
...
- Default: None
- This field is required unless you define [`sourceTypes`](#sourcetypes). You can define both.
...
- Default: None
- This field is required unless you define [`unifiedTypes`](#unifiedtypes). You can define both.
- Data type: List of strings
- Supports a maximum of 5 elements
...
- **Node Filters:**
  - `unifiedTypes`: Max 5 elements.
  - `sourceTypes`: Max 5 elements.
  - `ids`: Max 10 elements.
  - Only 1 wildcard (`*`) is allowed across all node types in the entire query.
- **Edge Filters:**
  - `edgeTypes`: Max 5 elements.
...
- **Predicates (`where` clauses):**
  ...
  - Max 10 items inside a single `filterGroup`.
