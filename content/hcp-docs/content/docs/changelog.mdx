...
# Changelog
### 2025-12-18
**HCP Access Management:** Project-level service principals can now be assigned organization-level roles, such as `Project Creator` or `Group Manager`. This allows a single principal authenticated with Workload Identity Federation (WIF) to securely automate organization-wide tasks without needing static, org-level keys. For more details, refer to the [service principals documentation](/hcp/docs/hcp/iam/service-principal).

Key Update: 
* You can now grant organization-level permissions to project-level service principals, simplifying secure, large-scale automation.
