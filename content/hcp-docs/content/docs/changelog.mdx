...
# Changelog
### 2026-05-29
**HCP Organizations:** Organization constraints are now available for centralized governance and cost control. Organization administrators can restrict which HCP products (Vault, Boundary, Vault Radar, Packer, Waypoint) can be deployed within their organization, providing preventative controls that apply regardless of individual user permissions. This helps enterprises manage flexible consumption spending, meet security compliance requirements, and enforce organizational standards at scale. For more information, refer to the [organization constraints documentation](/hcp/docs/hcp/admin/orgs-constraints).
