...
**HCP user account deletion:** GitHub and email/password users can now delete their own HCP accounts directly from the portal, without filing a support ticket. The self-service flow includes an automated pre-flight check that blocks deletion if you are the owner of an HCP organization. If you are blocked, you can resolve the dependency by transferring ownership to another user or deleting the organization, then retrying deletion. SSO and SCIM-provisioned users are not eligible for self-service deletion and must contact their organization administrator to be deprovisioned through their identity provider.

For more information, refer to [Delete your account](/hcp/docs/hcp/create-account#delete-your-account).
