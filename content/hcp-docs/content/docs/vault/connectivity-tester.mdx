...
The request body contains `connection_info` array. Each element is a flat object with a `type` enum and the parameters for that test. You can submit one or more tests in a single request.
...
**`HOSTNAME_UNRESOLVED` is returned**
...
**`TIMEOUT` is returned for an `NC` test**
...
**`TIMEOUT` is returned for all tests (host unreachable)**
...
**`TLS_HANDSHAKE_FAILED` is returned for a `CURL` test**

Verify that:
- The service on the target host is TLS-enabled and configured with a valid certificate.
- The port is open and reachable — run a `CONNECTIVITY_COMMAND_TYPE_NC` test first to confirm basic TCP connectivity.
