...
- Azure VPC exists and configured with its address space DNS support and DNS hostnames enabled for the VNet to allow DNS lookups of records in the network.

- A valid connection between the Azure HVN and the target VNet must exist, using [VPC peering connection](/hcp/docs/hcp/network/hvn-azure/hvn-peering). Peering exists between the target VPC and the HVN, configured with the required routes to enable network traffic between the two networks.

- Target VPC's private hosted zone is associated with the VPC connected to the HVN via peering only.

- Security group exists in the target VPC, which allows inbound traffic on port 53 (both UDP and TCP) from the HVN CIDR block to your self-managed DNS server or Azure Private Hosted DNS endpoint.

- If using Azure Private Hosted Zone, ensure an [inbound resolver endpoint exists](https://learn.microsoft.com/azure/dns/dns-private-resolver-endpoints) that allows DNS queries from the HVN to be routed to the target VPC endpoint. Choose the Security Group for which you have configured the inbound rules to allow DNS (TCP and UDP) traffic on port 53 associated with the VNet subnets from the HVN CIDR block for the resolver endpoint.
