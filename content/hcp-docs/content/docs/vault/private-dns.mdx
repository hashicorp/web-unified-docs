...
Private DNS forwarding is available in beta for Amazon Web Services (AWS) and Microsoft Azure.
...
- An AWS HVN created with a unique [address space](/hcp/docs/hcp/network/hvn-aws/hvn-aws)

- An AWS VPC exists and is configured with its address space, with DNS support and
  DNS hostnames enabled to allow DNS lookups of records in the network

- A valid connection between the AWS HVN and the target VPC must exist, using one of the following methods:
  - A [VPC peering connection](/hcp/docs/hcp/network/hvn-aws/hvn-peering) exists between
    the target VPC and the HVN, configured with the required routes to enable
    network traffic between the two networks
  - A [Transit Gateway attachment](/hcp/docs/hcp/network/hvn-aws/tgw-attach) is established between the HVN
...
- The target VPC's private hosted zone is associated with the VPC connected to the HVN via peering or transit gateway

- A security group exists in the target VPC that allows inbound traffic on port 53
  (both UDP and TCP) from the HVN CIDR block to your self-managed DNS server
  or AWS Route 53 VPC endpoint

- If using AWS Route 53, ensure an [inbound resolver endpoint exists](https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver.html)
  that allows DNS queries from the HVN to be routed to the target VPC endpoint
  - Select the Do53 protocol
  - Choose a security group with inbound rules allowing DNS (TCP and UDP) traffic on port 53
    from the HVN CIDR block

- An [HCP service principal user is created](/vault/tutorials/manage-hcp-vault-dedicated/terraform-hcp-provider-vault#create-a-service-principal-and-key)
  with a client ID and secret

</Tab>
<Tab heading="Azure">

1. Access to the [HCP Portal](https://portal.cloud.hashicorp.com/sign-in)

1. Azure HVN created with a unique [address
  space](/hcp/docs/hcp/network/hvn-azure/hvn-azure).

1. Azure VPC exists and configured with its address space DNS support and
  DNS hostnames enabled for the VNet to allow DNS lookups of records in the
  network.

1. A valid connection between the Azure HVN and the target VNet must exist, using [VPC peering connection](/hcp/docs/hcp/network/hvn-azure/hvn-peering). Peering exists between
    the target VPC and the HVN, configured with the required routes to enable
    network traffic between the two networks.

1. Target VPC's private hosted zone is associated with the VPC connected to the HVN via peering only. 

1. Security group exists in the target VPC, which allows inbound traffic on port
  53 (both UDP and TCP) from the HVN CIDR block to your self-managed DNS server
  or Azure Private Hosted DNS endpoint.

1. If using Azure Private Hosted Zone, ensure an [inbound resolver endpoint
  exists](https://learn.microsoft.com/azure/dns/dns-private-resolver-endpoints)
  that allows DNS queries from the HVN to be routed to the target VPC endpoint.
  Choose the Security Group for which you have configured the inbound rules to 
  allow DNS (TCP and UDP) traffic on port 53 associated with the VNet subnets 
  from the HVN CIDR block for the resolver endpoint.

1. [HCP service principal user
...
The HCP Terraform provider supports private DNS forwarding for AWS.
