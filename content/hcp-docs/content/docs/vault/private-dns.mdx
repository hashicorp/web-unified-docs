...
1. AWS HVN created with a unique [address
  space](/hcp/docs/hcp/network/hvn-aws/hvn-aws).

1. AWS VPC exists and configured with its address space DNS support and
  DNS hostnames enabled for the VPC to allow DNS lookups of records in the
  network.

1. A valid connection between the AWS HVN and the target VPC must exist, using one of the following methods:
...
1. Target VPC's private hosted zone is associated with the VPC connected to the HVN via peering or TGW. 
...
1. If using AWS Route 53, ensure an [inbound resolver endpoint
...
1. Use the `dnsforwardings` endpoint to create DNS forwarding. Update
   ...
   - The `inbound_endpoint_ips` is the IP address of your self-managed DNS
     servers, or the AWS Route 53 or the Azure Private Hosted Zone VPC endpoint IP addresses.
   ...
   - Set `connection_type` to `hvn-peering` (Azure only) or `tgw-attachment` based on your connection type.
   ...
   Use this example if you connect your HVN to your VPC using HVN peering (Azure Only)
...
You can also manage private DNS forwarding using the HCP Terraform provider (Currently supported for AWS):
