...
- Azure HVN created with a unique [address
  space](/hcp/docs/hcp/network/hvn-azure/hvn-azure).

- Azure VPC exists and configured with its address space DNS support and
  DNS hostnames enabled for the VNet to allow DNS lookups of records in the
  network.

- A valid connection between the Azure HVN and the target VNet must exist, using [VPC peering connection](/hcp/docs/hcp/network/hvn-azure/hvn-peering). Peering exists between
    the target VPC and the HVN, configured with the required routes to enable
    network traffic between the two networks.
...
- If using Azure Private Hosted Zone, ensure an [inbound resolver endpoint
