...
of a cluster. Snapshots are only retained for essentials, and standard tiers post cluster deletion.
Snapshots can be used to recover a deleted cluster from AWS (Azure restores are currently not supported), including restoring to a
...
Once a cluster is deleted in Azure, all affiliated resources are
deleted with it, including audit logs and snapshots. It is currently not
possible to restore a deleted cluster which was hosted in Azure.
