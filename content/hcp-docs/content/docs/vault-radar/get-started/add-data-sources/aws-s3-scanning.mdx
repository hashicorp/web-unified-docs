...
the Radar agent](/hcp/docs/vault-radar/agent/deploy) (minimum version 0.38.0). Once you deploy the agent,
...
Currently, we do not support short lived AWS credentials through environment variables (`AWS_SESSION_TOKEN`)
...
1. Select **Vault Radar Agent Scan** and click on **AWS S3** in the Object Storage section.
...
1. Enter the details for the selected method and click **Next** to validate and establish the connection.
...
1. Choose the buckets to monitor. You can either choose all the buckets, or individual ones. Click **Finish** once done.
...
1. For versioned buckets, you can also scan the previous versions of files along with the latest version.
