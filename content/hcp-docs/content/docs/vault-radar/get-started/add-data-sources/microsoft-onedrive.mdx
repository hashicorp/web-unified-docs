...
- Access to the Microsoft 365 tenant that contains the OneDrive data you want to scan
...
Before you configure the Microsoft OneDrive data source in HCP Vault Radar, you
...
Microsoft Entra ID so Vault Radar can read the OneDrive content it scans and
sign users in.
...
1. Add the following permissions:
   ...
   - **Microsoft Graph application permissions:**
     - `Files.Read.All` to read all files in OneDrive.
     - `User.Read.All` to list users in the tenant.
...
1. Copy the client secret value and store it as an environment variable on the
   ...
   ```shell-session
   $ export MICROSOFT_ONEDRIVE_CLIENT_SECRET="<client-secret-value>"
   ```
...
1. Copy the **Application (client) ID** and store it as an environment variable on the
   ...
   ```shell-session
   $ export MICROSOFT_ONEDRIVE_CLIENT_ID="<application-client-id>"
   ```
