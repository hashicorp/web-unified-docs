---
page_title: Add a data source - Microsoft Exchange
description: >-
  Add Microsoft Exchange as a data source for Vault Radar to monitor Exchange mailboxes, calendars, and tasks for sensitive data and secrets.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-03-26T00:00:00Z
last_modified: 2026-03-26T00:00:00Z
# END AUTO GENERATED METADATA
---

# Scan Microsoft Exchange for secrets

@include 'beta-feature.mdx'

Connect Microsoft Exchange as a data source to HCP Vault Radar to monitor
Exchange mailboxes, calendars, and tasks for sensitive data and secrets. Mail
attachments are scanned automatically as part of the mail scan.

@include 'vault-radar/radar-quickstart-tutorials.mdx'

## Prerequisites

- HCP IAM user with the HCP owner or admin role
- Access to the Microsoft 365 tenant that contains the Exchange data you want to scan
- Enable [admin consent
  workflow](https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/configure-admin-consent-workflow)
  in the Microsoft 365 tenant (if tenant-wide consent is restricted)
- Permission to register an application in [Microsoft Entra
  ID](https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-register-app)
  (for agent scan)
- Vault Radar agent deployed and running. Refer to the [HCP Vault Radar
  operations
  tutorials](/hcp/tutorials/hcp-vault-radar-operations/install-vault-radar-agent) (for agent scan)

## Add a Microsoft Exchange data source

<Tabs>
<Tab heading="HCP Vault Radar cloud scan">

1. Log into the [HCP Portal](https://portal.cloud.hashicorp.com/sign-in) with an
   HCP IAM user that has the HCP owner or admin role.

1. Click **Vault Radar**.

1. Click **Settings**.

1. Click **Data Sources**.

1. Select **HCP Vault Radar Scan**.

1. Under **Team Collaboration**, click **Microsoft Exchange**.
   ![Select Microsoft Exchange data source](/img/docs/vault-radar/microsoft-exchange-onboarding-saas-flow.png)

1. Click **Connect to Microsoft Exchange**. This starts the Microsoft sign-in and
   consent flow.

1. Sign in with an account of the tenant you want to scan.

1. Review the requested permissions and click **Accept**.

   <Note title="Administrator consent may be required">

   If your organization restricts tenant-wide consent, Microsoft displays a
   message that asks you to request approval from an administrator.

   </Note>

1. HCP Vault Radar displays the message **Authenticated to Microsoft Exchange**.

1. Click **Next**.

1. Select either **All Exchange accounts** or **Select Exchange accounts to
   monitor**.

   For each account in the tenant, you can individually select which data types
   to scan: **Mail**, **Calendar**, and **Tasks**. Mail attachments are included
   automatically when Mail is selected.

1. Click **Finish** to start onboarding and scanning the selected Exchange
   accounts.

</Tab>
<Tab heading="HCP Vault Radar agent scan">

Before you configure the Microsoft Exchange data source in HCP Vault Radar, you
need to register an application in Microsoft Entra ID and configure the required
permissions.

### Register a Microsoft Entra application

For step-by-step instructions on registering an app, see [Quickstart: Register an application](https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-register-app).

1. Sign in to the [Microsoft Entra admin
   center](https://entra.microsoft.com/) with an account that can register
   applications.

1. Navigate to **Entra ID > App registrations**.

1. Click **New registration**.

1. Enter a name for the application, such as `HashiCorp Vault Radar`.

1. For **Supported account types**, select single tenant or multitenant based on your organization's needs.

1. Click **Register**.

### Enable ID token issuance

1. In the Microsoft Entra application, open **Authentication (Preview) > Settings**.

1. Under **Implicit grant and hybrid flows**, select **ID tokens**.

1. Click **Save**.

### Configure Microsoft Graph permissions

Complete the following steps in the Microsoft Entra app registration in
Microsoft Entra ID so Vault Radar can read the Exchange content it scans and
sign users in.

For a guide to adding API permissions to an app, see [Quickstart: Configure app access to web APIs](https://learn.microsoft.com/en-us/entra/identity-platform/quickstart-configure-app-access-web-apis).

1. Navigate to **Entra ID > App registrations**, and then select your client application.

1. Select **API permissions**.

1. Click **Add a permission**.

1. Select **Microsoft Graph**.

1. Add the following permissions:

   - **Microsoft Graph application permissions:**
     - `Mail.Read` to read mailbox messages and attachments.
     - `Calendars.Read` to read calendar events.
     - `Tasks.Read` to read tasks.
     - `Organization.Read.All` to read organization information, such as the tenant name.

   - **Delegated permissions (scopes):**
     - `openid` to sign users in.
     - `profile` to view user's basic profile.
     - `offline_access` to allow the application to maintain access to the Microsoft Graph API when the user is not signed in.

1. Grant admin consent for the tenant after you add the permissions. Refer to the
   [Microsoft documentation for granting tenant-wide admin
   consent](https://learn.microsoft.com/en-us/entra/identity/enterprise-apps/grant-admin-consent).

### Create a client secret and store the values

1. In the Microsoft Entra application, navigate to **Certificates & secrets**.

1. Click **New client secret**.

1. Copy the client secret value and store it as an environment variable on the
   agent host.

   <CodeBlockConfig hideClipboard>

   ```shell-session
   $ export MICROSOFT_EXCHANGE_CLIENT_SECRET="<client-secret-value>"
   ```

   </CodeBlockConfig>

1. Copy the **Application (client) ID** and store it as an environment variable on the
   agent host.

   <CodeBlockConfig hideClipboard>

   ```shell-session
   $ export MICROSOFT_EXCHANGE_CLIENT_ID="<application-client-id>"
   ```

   </CodeBlockConfig>

1. Copy the **Directory (tenant) ID** and store it securely.

   You will enter this value directly in the HCP Portal when you add the
   Microsoft Exchange data source.

### Add a Microsoft Exchange data source

1. Log into the [HCP Portal](https://portal.cloud.hashicorp.com/sign-in) with an
   HCP IAM user that has the HCP owner or admin role.

1. Click **Vault Radar**.

1. Click **Settings**.

1. Click **Data Sources**.

1. Select **HCP Vault Radar Agent Scan**.

1. Under **Team Collaboration**, click **Microsoft Exchange**.
   ![Select Microsoft Exchange data source](/img/docs/vault-radar/microsoft-exchange-onboarding-agent-flow.png)

1. Enter the following information:

   - **Microsoft Entra tenant ID:** The **Directory (tenant) ID** for the
     Microsoft Entra application.

   - **Microsoft Entra client ID:** The environment variable path where you
     stored the client ID on the agent host.

      **Example:**

      <CodeBlockConfig hideClipboard>

      ```plaintext
      env://MICROSOFT_EXCHANGE_CLIENT_ID
      ```

      </CodeBlockConfig>

   - **Microsoft Entra client secret:** The environment variable path where you
     stored the client secret on the agent host.

      **Example:**

      <CodeBlockConfig hideClipboard>

      ```plaintext
      env://MICROSOFT_EXCHANGE_CLIENT_SECRET
      ```

      </CodeBlockConfig>

1. Click **Next**.

1. Select either **All Exchange accounts** or **Select Exchange accounts to
   monitor**.

   For each account in the tenant, you can individually select which data types
   to scan: **Mail**, **Calendar**, and **Tasks**. Mail attachments are included
   automatically when Mail is selected.

1. Click **Finish** to start onboarding and scanning the selected Exchange
   accounts.

</Tab>
</Tabs>

## Assign a group to a resource

@include 'vault-radar/add-group-to-resource.mdx'

## Update data source

@include 'vault-radar/update-data-source.mdx'

## Tutorials

@include 'vault-radar/vault-radar-tutorial-list.mdx'
