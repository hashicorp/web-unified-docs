...
](#onboard-slack) and scan for secrets in channels.
...
1. Click **Connect to Slack** to **install the Slack app**. This initiates
   ...
   Click **Enable secret copying via Vault Radar Agent** to [copy secrets to
   Vault](/hcp/docs/vault-radar/remediate-secrets/copy-secrets).

   <Note title="Vault Radar Agent required for copying secrets">

   Copying secrets to a Vault cluster requires [installing and configuring the
   HCP Vault Radar agent](/hcp/docs/vault-radar/agent/deploy).
