...
| System  | Read license               | `GET`  | [sys/license/status](/vault/api-docs/system/license) |
...
The sample policy below may be used to model your model your authorization policy for Vault Agent.

```shell-session
vault policy write vault-radar-index-vault -<<EOF
# List namespaces
path "sys/namespaces" {
...
# List AppRole roles -- update as needed for your AppRole mounts
path "auth/approle/role" {
  capabilities = ["list"]
}

# List secret-id accessors -- update as needed for your AppRole mounts
path "auth/approle/role/*/secret-id" {
  capabilities = ["list"]
}

# Lookup secret-id accessor metadata -- update as needed for your AppRole mounts
path "auth/approle/role/*/secret-id-accessor/lookup" {
  capabilities = ["update"]
}

# KV secrets -- Read engine configuration -- update as needed for your KV-V2 mounts
path "kv/config" {
  capabilities = ["read"]
}

# List secrets -- update as needed for your KV-V2 mounts
path "kv/metadata/*" {
  capabilities = ["list", "read"]
}

# Read secret versions -- update as needed for your KV-V2 mounts
path "kv/data/*" {
  capabilities = ["read"]
}
