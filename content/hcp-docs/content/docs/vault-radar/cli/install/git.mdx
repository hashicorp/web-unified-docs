...
{
    "fail_severity": "high",
    "pre_commit_skip_activeness": true
}
```
...
**Note: If `fail_severity` configuration value is not defined the default behavior is to not enforce any severity. As a result all risks identified will be allowed.**