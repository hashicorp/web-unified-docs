...
The `scan slack` command scans Slack channel(s) and identifies
messages that contain sensitive secrets. 

## Authentication

Before you can scan Slack channels, the command needs credentials to
make requests to Slack. Generate a user OAuth token
...
- [Request scopes](https://api.slack.com/start/quickstart#scopes) in **OAuth
