...
last_modified: 2026-09-17T17:39:52.000Z
...
- [Q: Does Vault Radar detect Base64 encoded secrets?](#base64-secrets)
...
Yes. Vault Radar detects Base64 encoded secrets in two ways:

- **Dedicated Base64 secret types**: Many providers have a matching secret type with a `_BASE64` suffix, such as `AWS_ACCESS_KEY_ID_BASE64` and `GITHUB_TOKEN_BASE64`. Vault Radar reports these findings under the `_BASE64` type. Refer to the [secret types](/hcp/docs/vault-radar/manage/secret-types) documentation for the full list.

- **Automatic decoding of generic secrets**: When Vault Radar detects a generic secret, it decodes the value if that value is valid standard or URL-safe Base64. If the decoded value matches a known provider pattern, Vault Radar reports the finding under that specific secret type instead of the generic type.

In both cases, Vault Radar reports the Base64 encoded value as it appears in your
source. It does not store or display the decoded value.
