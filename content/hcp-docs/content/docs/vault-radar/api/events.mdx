---
page_title: Public API Documentation - Events
description: >-
  Vault Radar Event API provides endpoints for querying and managing security events detected by HashiCorp Vault Radar.
---

# Public API Documentation - Events

## Overview

The Vault Radar Event API provides endpoints for querying and managing security events detected by HashiCorp Vault
...
> **Note:** For a complete list of all Event fields, see the [Event Fields Reference](#event-fields-reference) section below.

### 2. Group and Aggregate Events

Search for events based on flexible criteria including filtering, sorting, grouping, and pagination.
...
**Response Example:**
Note that the response contains aggregated counts of events per severity and status combination.
...
- **Grouping**: Aggregate events by a specific Event field names

### Search Parameters

- **limit** (int32): Maximum number of results to return (max: 1000)
- **page** (int32): Page number for pagination (first page is 1)
- **filters** (array of FilterV2): Filtering criteria
...
- **Authentication**: [HashiCorp Cloud Platform authentication documentation](/hcp/docs/hcp/api#authenticate-to-hcp)
- **Swagger API Reference**: [Vault Radar Swagger file](https://github.com/hashicorp/hcp-specs/blob/main/specs/cloud-vault-radar/preview/2023-05-01/hcp.swagger.json)
