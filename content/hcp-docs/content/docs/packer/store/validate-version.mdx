...
- **Automatic validation**: Set up and configure the HCP Terraform run task for HCP Packer to check your Terraform configuration references for revoked artifacts.
...
You can also configure how the run task responds to specific scenarios during a Terraform run. Visit the **Run task** settings page to configure the HCP Packer run task.
...
### Configure run task behavior

After you associate the HCP Packer run task with your HCP Terraform workspace, you can configure how HCP Packer responds to certain scenarios during a Terraform run.

<Note>

Configurable run tasks are available in beta on the Standard Pricing Tier. We recommend you test this feature in non-production workspaces first.

</Note>

For example, to block deployments when Terraform references untracked images:

1. Sign into the [HCP portal](https://portal.cloud.hashicorp.com/), then choose your organization.
1. Choose your project, then click **Packer** in the **Services** menu.
1. Click **Settings** in the left navigation menu.
1. If you use the Standard Pricing Tier, click **Run task** under **Settings**.
1. In the **Untracked Image** section, choose **Block**.
1. Click **Save** to apply the setting.

Choose **Block** when you want to enforce deployments that only use images tracked in HCP Packer. Blocking only stops runs when the HCP Packer run task is configured as `Mandatory` in HCP Terraform. Choose **Warn** when you want to report untracked images without interrupting deployments.
