...
- **Automatic validation**: Set up the HCP Terraform run task for HCP Packer to check your Terraform configuration references for revoked artifacts and configure how the run task responds to untracked images. 
...
You can also configure how the run task responds when it detects an untracked image during a Terraform run. Use the **Run task** settings page in HCP Packer to choose whether the run task warns or blocks when it cannot verify the image in your deployment.
...
1. Open the HCP Packer homepage and click **Integrate with HCP Terraform**.
...
### Configure run task behavior for untracked images

After you associate the HCP Packer run task with your HCP Terraform workspace, you can configure how HCP Packer responds when it detects an untracked image during a Terraform run.

<Note>

Untracked image handling is available in beta. We recommend you test this feature in non-production workspaces first.
...
1. Click **Run task**.
1. In the **Untracked Image** section, choose one of the following options:
  - **Warn**: Notify the user but allow the deployment to proceed.
  - **Block**: Block the deployment until a verified image is used.
1. Click **Save**.

Use **Warn** when you want the run task to report untracked images without interrupting deployments. Use **Block** when you want to enforce that deployments only use verified images tracked in HCP Packer.
