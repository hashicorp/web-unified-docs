...
HCP Packer automatically scans SBOMs for known vulnerabilities on a nightly schedule, and generates a report for each bucket.
...
1. Open the artifacts version build overview page.
...
When you associate an SBOM with an artifact build, HCP Packer automatically compares each package version against an internal database of known Common Vulnerabilities and Exposures (CVEs). Since this database receives frequent updates, HCP Packer automatically scans each SBOM in your registry on a nightly schedule.
