...
last_modified: 2026-09-22T20:11:07.000Z
...
When you delete a worker, the controller stops recognizing its stored credentials. However, the storage directory remains on the worker host.
If a worker has stale credentials, it does not generate a new worker auth token. You must clear the storage directory before you can re-register it.

If you start the worker before you clear the directory, it fails to authenticate. The following TLS error appears in the worker logs:
...
To re-register the worker and bring it back online, refer to [Clean up and re-register a worker](/boundary/docs/workers/manage#clean-up-and-re-register-a-worker).
