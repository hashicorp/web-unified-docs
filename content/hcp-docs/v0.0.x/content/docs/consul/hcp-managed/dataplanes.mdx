...
description: >-
   Consul dataplanes remove the need to a run client agents in your deployments. Learn about using Consul Dataplane with HashiCorp-managed servers on HCP Consul.
---

# Deploy Consul dataplanes

This page provides usage information for running Consul dataplanes with HCP Consul. Dataplanes enable communication between a HashiCorp-managed cluster and services running in a user-managed Kubernetes cluster.
...
When using dataplanes with HCP Consul, you run a configured Consul container in your Kubernetes cluster. This Consul instance connects to the external HashiCorp-managed servers and enables Consul to inject dataplanes into sidecar containers.

## Prerequisites

To deploy dataplanes with HCP Consul, you must meet the following minimum version requirements:
...
## Deploy dataplanes with HCP Consul

Complete the following steps to connect services in your Kubernetes cluster to HashiCorp-managed servers:
...
This page describes the process to deploy dataplanes and connect them to a HashiCorp-managed cluster that already exists in your organization. If you have not done so, refer to [Create a HashiCorp-managed cluster](/hcp/docs/consul/hcp-managed/create).

### Prepare your Kubernetes cluster

To automate the process for deploying dataplanes, you must create a Kubernetes Secret containing your [HCP Consul cluster admin token](/hcp/docs/consul/hcp-managed/access#generate-admin-token) in the Kubernetes Namespace where you install Consul. This secret initializes the ACL system in the Consul workload scheduled on your Kubernetes cluster with credentials that enable secure access to the ACL system running on the HashiCorp-managed cluster. 
...
Set the following configurations in the Helm chart or `values.yaml` file:
  ...
  - The datacenter name must match the [cluster ID of your HashiCorp-managed cluster](/hcp/docs/consul/hcp-managed/reference#cluster-configuration-reference).
  - The version of Consul in `global.image` should match the version running on the HashiCorp-managed cluster.
  - The ACL system's `bootstrapToken` must invoke the `consul-bootstrap-token` Secret that contains the HashiCorp-managed cluster's admin token.
  - Use `server.enabled: false` to disable server agent features.
  - Configure an `externalServers` stanza with the [HCP-managed cluster's IP address](/hcp/docs/consul/hcp-managed/access#get-access-url) and your K8s cluster's [API server URL](https://kubernetes.io/docs/tasks/administer-cluster/access-cluster-api/).
