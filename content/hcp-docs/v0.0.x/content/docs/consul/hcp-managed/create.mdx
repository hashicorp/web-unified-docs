---
page_title: Create a HashiCorp-managed cluster
description: |-
  Learn how to create a HashiCorp-managed Consul cluster using the HashiCorp Cloud Platform (HCP) interface.
---

# Create a HashiCorp-managed cluster

This page describes how to create *HashiCorp-managed clusters*, which are one or more Consul server agents that are installed, configured, and managed for you by HashiCorp. To use clusters that you create and host in your own environment, refer to [link self-managed clusters to HCP Consul](/hcp/docs/consul/self-managed). For more information about the differences between the two types of clusters, refer to [cluster management](/hcp/docs/consul/concepts/cluster-management).
...
1. Select **HashiCorp-managed Consul** and then click **Get Started**.
...
1. Enter a name for the cluster in the **Cluster ID** field. The cluster ID is a unique identifier that cannot be used for other active HCP Consul clusters.
...
1. Select a cluster size. Cluster size is measured by the expected number of service instances the cluster supports. For example, a small cluster supports up to 500 service instances. For pricing information for each cluster size, refer to [HCP Consul Pricing](https://www.hashicorp.com/products/consul/pricing).
...
HCP includes an option to generate Terraform code that you can run to create a Consul cluster in AWS. The Terraform code deploys Consul to either a new VPC or an existing VPC. It also deploys a demo application that you can interact with and observe in the HCP Consul UI.
...
1. Select a runtime for your cluster. HCP Consul supports EKS, EC2, and ECS runtimes for HashiCorp-managed clusters.
...
After Terraform starts, the cluster appears in the list of Consul clusters in HCP. You can click the name of your cluster to view details, connect a VPC to your cluster, and access the Consul cluster’s UI.
...
HCP includes an option to generate Terraform code that you can run to create a Consul cluster in Azure. The Terraform code deploys Consul to either a new VNet or an existing VNet. It also deploys a demo application that you can interact with and observe in the HCP Consul UI.
...
1. Select a runtime for your cluster. HCP Consul supports AKS and Azure VM runtimes for HashiCorp-managed clusters.
1. Choose a region from the **HCP Region** menu. This region is where you want to create the Consul server. If you are connecting to an existing VPC, we recommend creating the cluster in the same region as your VPC in order to reduce latency.
...
> **Tutorial:** For step-by-step guidance on deploying HCP Consul on Azure using Terraform automation, refer to the [Deploy HCP Consul with VM using Terraform tutorial](/consul/tutorials/cloud-deploy-automation/consul-end-to-end-azure-vm).
