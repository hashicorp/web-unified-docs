...
description: |-
  This topic provides instructions for registering self-managed workers with HCP
---
...
Note that only HCP KMS workers are listed.

1. Click **New**.

1. (Optional) You can use the new PKI workers page to construct the contents of the `pki-worker.hcl` file, if you did not [create the configuration file](/hcp/docs/boundary/self-managed-workers/install-self-managed-workers#create-the-self-managed-worker-configuration-file) as part of the installation process.
...
Use the following command to create a new PKI worker.
