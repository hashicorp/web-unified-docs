...
description: |-
  This topic provides installation instructions for self-managed workers
---

# Install Self-Managed Workers

HCP Boundary allows organizations to register and manage their own PKI workers.
...
Refer to the [complete configuration example](/boundary/docs/configuration/worker/pki-worker#complete-configuration-example) to view all valid configuration options.
...
1. Create a new file to store the worker configuration.
    ...
    ```shell-session
    $ touch pki-worker.hcl
    ```

1. Open the `pki-worker.hcl` file with a text editor, such as Vi. Paste the
   following configuration information into the `pki-worker.hcl` file:

    <CodeBlockConfig lineNumbers filename="pki-worker.hcl">
...
1. Update the configuration fields in the `pki-worker.hcl` file as necessary.
You can specify the following configuration fields for self-managed workers:
   ...
   - The `public_addr` field should match the public IP or DNS name of your
     self-managed worker instance. In the unlikely event that you deploy the
     Boundary client and worker on the same local machine, you should omit the
     `public_addr` attribute.

      For an example of the Boundary client and worker being deployed on the same local machine, refer to the [Configure the PKI Worker](/boundary/tutorials/hcp-administration/hcp-manage-workers#configure-the-pki-worker) section of the self-managed worker tutorial.

   - The `auth_storage_path` is a local path where the worker stores its PKI
   ...
   - The `initial_upstreams` value indicates the address or addresses a worker
    ...
    in the `worker` stanza as an alternative to the `hcp_boundary_cluster_id`.
      ...
      Make sure to use `hcp_boundary_cluster_id` to connect PKI workers to HCP Boundary.
...
1. Save the `pki-worker.hcl` file.
...
1. Create a new file named `pki-worker.hcl`:

    ```shell-session
    $ touch pki-worker.hcl
    ```

1. Open the `pki-worker.hcl` file with a text editor. Paste the following
   configuration information into the `pki-worker.hcl` file:
...
worker configuration file, for example `/home/pki-worker.hcl`.
...
<Tabs>
<Tab heading="Linux" group="linux">
<CodeBlockConfig lineNumbers highlight="1,12" filename="pki-worker.hcl">

  ```shell-session
  $ ./boundary-worker server -config="/home/myusername/pki-worker.hcl"
  ...
  Worker Auth Registration Request: 
          Worker Auth Storage Path: /home/ubuntu/worker
...
<CodeBlockConfig lineNumbers highlight="1,12" filename="pki-worker.hcl">
  ...
  ```shell-session
  $ ./boundary-worker server -config="/Users/myusername/pki-worker.hcl"
...
<CodeBlockConfig lineNumbers highlight="1,12" filename="pki-worker.hcl">
  ...
  ```shell-session
  $ .\boundary-worker.exe server -config="C:\Users\myusername\pki-worker.hcl"
