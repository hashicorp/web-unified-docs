---
page_title: Configure HCP Vault metric streaming to Splunk
...
1. From the HCP Vault cluster **Overview** page, select the **Metrics** view.
   ![HCP Vault Portal](/img/cloud/hcp-vault-ui-metrics_nav.png)
...
1. From the **Stream Vault metrics** view, select **Splunk** as the provider and click **Next**.
   ![Select Splunk provider](/img/cloud/ui-hcp-vault-metrics-select-splunk.png)

1. Under **Splunk Configuration**, enter your **HTTP Event Collector (HEC)
   Endpoint** URL and event collector **Token**.
   ![Configure Splunk provider](/img/cloud/ui-hcp-vault-metrics-configure-splunk.png)
...
1. HashiCorp has created a [sample HCP Vault Splunk dashboard
...
Splunk app to your Splunk Enterprise or Cloud environment.
   ![Sample Splunk Dashboard](/img/cloud/hcp-vault-ui-metrics_splunk_dashboard.png)
