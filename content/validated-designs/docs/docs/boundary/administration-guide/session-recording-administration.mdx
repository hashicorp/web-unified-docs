---
page_title: Session recording administration
description: Enable and operate Boundary session recording with worker storage, storage providers, storage buckets, and retention lifecycle policies.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-07-03T16:36:39+01:00
last_modified: 2026-07-08T14:53:25.000Z
...
This page covers the operator setup and management of session recording. For how users view recordings, see the User Guide: [Session recording](/validated-designs/boundary/user-guide/session-recording).
...
The worker stores the session recording on the local disk during the recording phase and then moves it to the external object store when the session has terminated. Boundary stores recordings in the BSR (Boundary Session Recording) format. During session establishment, the control plane passes any credentials needed to access the external object store to the worker performing the recording. See [storage](/validated-designs/boundary/administration-guide/session-recording-administration#storage-considerations) for more information on considerations and provider options.
