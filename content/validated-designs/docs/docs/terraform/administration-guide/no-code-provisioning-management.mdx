---
page_title: No-code provisioning management
description: Publish and govern no-code Terraform modules for self-service consumption.
...
Please review the [official documentation](https://developer.hashicorp.com/terraform/tutorials/cloud/no-code-provisioning) for more details on this capability.
...
When publishing a no-code module to the private registry or convert an existing private module as no-code, you must explicitly add the it to the no-code provision allow-list. This can be done using the WebUI, [the API](https://developer.hashicorp.com/terraform/cloud-docs/api-docs/no-code-provisioning), the TFE provider ([tfe_no_code_module](https://registry.terraform.io/providers/hashicorp/tfe/latest/docs/resources/no_code_module)).
...
Consumers of no-code modules must have the [Maintain permission set](https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/permissions#maintain) on the project(s) where the no-code modules are deployed.
...
<Highlight>
HashiCorp recommends the use of <a href='https://developer.hashicorp.com/terraform/cloud-docs/workspaces/dynamic-provider-credentials'>dynamic credentials</a>, although you may also use IAM roles or static credentials (which should be your least favored option).
</Highlight>
...
The no-code feature depends on a properly configured project where the user can create no-code workspaces. The platform team must set up this project with the appropriate permissions for the users ([Maintain permission set](https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/permissions#maintain)).
