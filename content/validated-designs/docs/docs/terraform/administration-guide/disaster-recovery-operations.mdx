...
description: Operate Terraform Enterprise failover and failback procedures.
...
For purposes of clarity, references to `primary` are to your main active operating region, and `secondary` are to your passive, failover region that you replicate your data _to_ in case the `primary` region goes offline. Once the outage has passed, the inference is that you failback _from_ the `secondary` _to_ the `primary`.
...
1. **Declare outage** - Notify end users and stakeholders.
1. **Contact HashiCorp Support** - Even if you have the situation under control, we recommend alerting us to the situation.
1. **Start failover** - Depending on outage scope.
...
1. **Failover actions** (can run in parallel).
...
1. **Terraform Enterprise startup** - Launch a single Terraform Enterprise instance in the failover region (Docker, Podman, Nomad, Kubernetes, OpenShift).
1. **Validate health** - Ensure Terraform Enterprise is operational and accessible. If the cluster is unhealthy, refer to the restore page of this HVD for further assistance.
1. **Scale out** - Once stable, add additional Terraform Enterprise instances as needed.
...
1. **Schedule failback** - Notify end users and stakeholders of a maintenance window for failback.
1. **Drain workloads** - Scale down Terraform Enterprise workloads to zero in the secondary region.
1. **Failback infrastructure**
...
1. **Launch Terraform Enterprise in primary region** - Start a single instance and verify health.
1. **Scale out** - Once stable, add additional Terraform Enterprise instances as needed.
...
None of the commands in the tabs below use exposed, dedicated "last write" APIs. They all work by listing objects and sorting by the LastModified/timestamp metadata. For large buckets with millions of objects, this can be slow. In scaled cases, consider enabling access logging instead. See the *Terraform Installation Guide* [Multiple Regions](/validated-designs/terraform/installation-guide/high-availability-multi-region) page for more information. The commands below aim to widen your toolkit for understanding more about your failover scenario.
...
aws s3api list-objects-v2 \
  ...
  --output json
```
- Click <a href="https://docs.aws.amazon.com/cli/latest/reference/s3api/list-objects-v2.html" target="_blank" rel="noreferrer">here</a> for information on confirming the status of object store replication between regions on AWS.
...
az storage blob list \
  ...
  --output json
```
- Click <a href="https://learn.microsoft.com/en-us/cli/azure/storage/blob?view=azure-cli-latest#az-storage-blob-list" target="_blank" rel="noreferrer">here</a> for information on confirming the status of object store replication between regions on Azure.
...
gcloud storage ls --recursive --long gs://YOUR_BUCKET_NAME/ \
  ...
  | head -1
```
- Click <a href="https://docs.cloud.google.com/sdk/gcloud/reference/storage/ls" target="_blank" rel="noreferrer">here</a> for information on confirming the status of object store replication between regions on GCP.
...
Only AWS <a href="https://aws.amazon.com/blogs/database/working-with-rds-and-aurora-postgresql-logs-part-2/" target="_blank" rel="noreferrer">provides</a> information on the status of cross-region PostgreSQL database replication from logs.
Use the following command against the read replica in the secondary region of any cloud to ascertain the last successful WAL write that occurred.
...
1. Check to find if there are any pending objects that did not replicate to the _secondary_ region. This process depends on your cloud provider, but recommendations regarding this are in the *Terraform Installation Guide* [Multiple Regions](/validated-designs/terraform/installation-guide/high-availability-multi-region) page.
1. The preceding link includes links to cloud-specific metric information on replication lag. In addition, run the following script to find the replication lag time on the _secondary_ PostgreSQL database by connecting directly to the secondary read replica, first updating the `psql*` environment variables to instantiate your database details.
...
1. Promote the read replica in the _secondary_ region to read-write, and wait for it to come up.
1. Turn on the _secondary_ VM scaling group by scaling from zero nodes to `> 0`.
1. Amend your DNS setup. Not every cloud supports global DNS at this time, so the approach differs between cloud vendors and will be different on private cloud `external` operational mode setups.
...
- **AWS**: <a href="https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/AuroraMySQL.Replication.CrossRegion.Promote.html" target="_blank" rel="noreferrer">Promoting a read replica to a DB cluster for Aurora</a> (document cites MySQL but applies to PostgreSQL)
- **Azure**: <a href="https://learn.microsoft.com/azure/postgresql/flexible-server/how-to-read-replicas-portal#stop-replication-to-a-replica" target="_blank" rel="noreferrer">Promote an Azure Database for PostgreSQL read replica</a>
- **GCP**: <a href="https://docs.cloud.google.com/database-migration/docs/postgres/promote-migration" target="_blank" rel="noreferrer">Promote replicas for regional migration or disaster recovery</a>
- **VMware**: Confer with your DBA team. VMware promotes use of <a href="https://github.com/hapostgres/pg_auto_failover" target="_blank" rel="noreferrer">`pg_auto_failover`</a> between leader/follower nodes. To circumvent failures when you lose an entire vSAN cluster, VMware recommends <a href="https://techdocs.broadcom.com/us/en/vmware-cis/live-recovery/live-site-recovery/9-0/using-vsphere-replication-with-vsan-storage.html" target="_blank" rel="noreferrer">vSphere Replication</a>
