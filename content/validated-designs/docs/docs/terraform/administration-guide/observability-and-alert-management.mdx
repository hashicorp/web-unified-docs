...
description: Operate observability, logging, metrics, audit trails, alerting, and notifications for Terraform.
...
HCP Terraform and Terraform Enterprise differ regarding operational logs, which stems from the shared responsibility model associated with HCP Terraform. With HCP Terraform, HashiCorp's SRE team tracks the operational logs as part of the service's operation. With Terraform Enterprise, your SRE team in charge of running the internal infrastructure-as-code service tracks those operation logs.

HCP Terraform and Terraform Enterprise also emit different metics. For information on the metrics you collect from HCP Terraform, refer to [Collect organization metrics](/terraform/cloud-docs/users-teams-organizations/organizations/metrics). For information on Terraform Enterprise metrics, refer to [Enable metrics collection](/terraform/enterprise/deploy/manage/monitor/metrics).
...
You can configure Prometheus to gather metrics from Terraform Enterprise and its underlying components. Terraform Enterprise generates operational metrics that Prometheus can collect. This setup allows you to monitor system metrics, including CPU, memory usage, and request latency. For more information on constructing Prometheus queries, refer to <a href="https://prometheus.io/docs/prometheus/latest/querying/functions/#functions" target="_blank" rel="noreferrer">this documentation</a>.

Also, you can use Grafana to visualize the metrics collected by Prometheus. You can create dashboards that provide real-time insights into the health of your Terraform Enterprise infrastructure, such as resource utilization trends and request rates. You can access the Terraform Enterprise Grafana dashboard <a href="https://grafana.com/grafana/dashboards/15630-terraform-enterprise/" target="_blank" rel="noreferrer">here</a>.
...
Other container and global metrics are in <a href="https://developer.hashicorp.com/terraform/enterprise/flexible-deployments/monitoring/observability/metrics" target="_blank" rel="noreferrer">our public documentation</a>.
...
Health checks are crucial for scaling Terraform Enterprise because they ensure the system's reliability and performance by proactively detecting and addressing issues before they impact availability or efficiency during scaling operations. Learn more on how to monitor the health of application through external health checks in <a href="https://developer.hashicorp.com/terraform/enterprise/flexible-deployments/troubleshooting" target="_blank" rel="noreferrer">our public documentation</a>.
...
Mark variables as sensitive in HCP Terraform when they contain value that must be confidential (such as credentials). This causes the Terraform binary to redact this information in logs and in general take additional measures to prevent accidental data leaks.
...
Terraform Enterprise offers application logs and audit logs. Application logs provide information about the services that make up Terraform Enterprise, while audit logs focus on documented security-related events such as successful logins and logouts through the admin console, failed authentication attempts, invalid or expired sessions, and CSRF violations.

For alerting and notification design, start with the official <a href="https://developer.hashicorp.com/terraform/enterprise/deploy/manage/monitor/logs" target="_blank" rel="noreferrer">Terraform Enterprise logs documentation</a>. It documents:

1. The current audit event types.
1. The standardized audit log fields, including `event_type`, `resource`, `source_ip`, `status_code`, `request_id`, and `actor_id`.
1. Log forwarding and retention considerations for external observability platforms.

This gives security and platform teams a stable reference for building log parsing, routing, dashboards, and alerts without depending on older internal log formats.
