---
page_title: Architecture | Terraform Installation Guide
description: Review the Terraform Enterprise reference architecture.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-07-06T13:22:06+01:00
last_modified: 2026-08-13T14:24:45.000Z
...
- **Availability**: Represents design points that minimize the impact of subsystem failures on the solution uptime. _An example of this is introducing multiple load-balanced application instances._
- **Operational complexity**: From time to time, it is necessary to introduce some complexity during the initial deployment to provide a reduced effort for operations teams to manage the solution. _An example of this is introducing a machine imaging process with Packer that permits immutable operations for scaling up/down and application updates._
- **Scalability**: Some design choices may work at a small scale but introduce overhead as the solution scales. _An example of this is choosing to onboard teams using a manual process in the Terraform Enterprise UI rather than creating an automated onboarding workflow to complete the necessary setup._
- **Security**: Indicates how a decision changes the security posture of the overall solution. _An example of this is taking advantage of workload identity so that workspaces authenticate against their API targets using JWT based workflows rather than using long-lived credentials as workspace variables._
...
- **PostgreSQL database**: Serves as the primary store of Terraform Enterprise's application data such as workspace settings and user settings. You can review the supported versions and requirements on our <a href="https://developer.hashicorp.com/terraform/enterprise/deploy/configuration/storage/connect-database/postgres" target="_blank" rel="noreferrer">Terraform Enterprise PostgreSQL</a> page.
- **Redis cache**: Used for caching and coordination between Terraform Enterprise core's web and background workers. In order to have multiple, simultaneously active application nodes, you require an external cache. We have validated Terraform Enterprise with the native Redis services from AWS, Azure, and GCP. Terraform Enterprise uses Redis internally on single-node VMware-based deployments. You can review the supported versions and requirements on our <a href="https://developer.hashicorp.com/terraform/enterprise/deploy/configuration/storage/connect-redis" target="_blank" rel="noreferrer">Terraform Enterprise Redis</a> page.
...
From <a href="https://developer.hashicorp.com/terraform/enterprise/releases/2023/v202309-1" target="_blank" rel="noreferrer">v202309-1</a>, Terraform Enterprise is compatible with a variety of platforms including Kubernetes. Our official installation documentation for this is <a href="https://developer.hashicorp.com/terraform/enterprise/deploy" target="_blank" rel="noreferrer">here</a>. We recommend using this for all private cloud deployments.
...
![Terraform `disk` operational mode](/img/terraform/solution-design-guides/tfe/architecture-logical-mounted-disk.png)
...
![`external` mode operational mode](/img/terraform/solution-design-guides/tfe/architecture-logical-external-services.png)
...
![`active-active` mode operational mode](/img/terraform/solution-design-guides/tfe/architecture-logical-active-active.png "image_tooltip")
...
Please note that Redis does not have to be external when there is a single node of Terraform Enterprise. When using our Terraform-based deployment modules, they provision a separate Redis instance automatically when the active/active parameter is `true`. For HVD Module deployments onto any managed Kubernetes service, the module also deploys a required Redis instance as an external service.
...
Your choice of operational mode also constrains how you install Terraform Enterprise. Deploying in `active-active` mode makes an automated deployment process a hard requirement, so factor this into your installation approach as you decide on a mode.

<!-- EDITORIAL: renamed from "Automated versus manual installation" and reframed as platform-agnostic guidance. Platform-specific install recommendations (HVD Modules per cloud, EKS/AKS/GKE, Packer) moved to the Deployment pages. -->
## Installation automation

Regardless of the operational mode you select, we recommend that you automate the deployment of Terraform Enterprise. Where your organization already has a configuration management standard — for example Chef, Puppet, Ansible, or Salt — we recommend building on it, because it provides:

- A well-known approach to installation and software management for your organization, and therefore a lower barrier to a successful deployment.
- Adherence to an organizational standard, which also increases the likelihood of regulatory compliance where applicable.

We advise automating the deployment of Terraform Enterprise irrespective of the operational mode used because:

- Manual installation increases the time and human involvement required to upgrade each instance every month. Automation reduces this burden.
- Automated installation lets you version and collaborate on the code used.
- Automated installation lets you rely on the same deployment pattern every time, which increases maintenance confidence for both the architecture teams and the services teams responsible for managing the product in production.

The tooling and modules that implement this automation are platform-specific. See the relevant _Deployment_ page for platform-specific installation recommendations, including the HashiCorp-maintained modules for each target.
