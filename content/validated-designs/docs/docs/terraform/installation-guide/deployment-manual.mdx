...
<!-- EDITORIAL: the vendor-agnostic baseline sizing numbers and component versions that previously lived here were relocated to the detailed design (baseline component sizing / external components) per the page-ownership split. This page now references that baseline and retains only manual, self-managed provisioning guidance. -->
## Component sizing

Terraform Enterprise and its external components follow the [baseline component sizing](/validated-designs/terraform/installation-guide/detailed-design#baseline-component-sizing) defined in the detailed design. In a manual private-cloud deployment you provision the application hosts, PostgreSQL, and Redis yourself, so size each of them to meet or exceed that baseline. The notes below are specific to a self-managed deployment.
...
Each Terraform Enterprise application instance runs as a container. Configure CPU and memory identically across instances, and remember the VM must be sized higher than the Terraform Enterprise application limits to allow for the operating system and any adjacent containers. For ongoing right-sizing and scale-out, see the [Administration Guide](/validated-designs/terraform/administration-guide/observability-and-alert-management).

The disk backing each host must sustain a minimum of 3000 IOPS, and Terraform Enterprise requires at least 40 GB of available space; for Docker deployments this space must be available to `/var/lib/docker`.

Object store consumption depends on two things:
...
1. The size of configuration directories submitted to the API during Terraform runs. Terraform aggregates the configuration directory and sends it to the API, which commits it to encrypted object storage through an internal process called archivist. Limit your configuration repositories to only the files a run needs — a 50 MB `.tgz` file in a configuration directory needlessly adds 50 MB to your object store and wastes hosting cost.

### PostgreSQL and Redis

Provision PostgreSQL and Redis to the [baseline component sizing](/validated-designs/terraform/installation-guide/detailed-design#baseline-component-sizing), at the versions documented in the [detailed design](/validated-designs/terraform/installation-guide/detailed-design#external-components). Because these are self-managed in a private-cloud deployment, confirm your organization has an established pattern for deploying and operating them at the required versions before you begin.

## Observability

Enable the general Terraform Enterprise metrics, logs, and agent telemetry described in [Observability setup](/validated-designs/terraform/installation-guide/observability-setup); that setup is common to all platforms. The guidance below is specific to a manual private-cloud deployment, where you own the underlying hosts and the self-managed PostgreSQL and Redis instances.

Monitor CPU, memory, available disk space, and disk I/O on the application hosts and on the self-managed PostgreSQL and Redis servers using your standard organizational machine telemetry solution. Create alerts for threshold breaches at 50% and 70% utilization; if any parameter exceeds 70% consistently, increase the resource to maintain performance. Report telemetry on an ongoing basis so the team that owns the instance is aware of resource consumption.

Ongoing, day-2 observability and alerting is covered in the [Administration Guide](/validated-designs/terraform/administration-guide/observability-and-alert-management).

## Networking requirements

Terraform Enterprise has a number of ingress and egress requirements.
Please refer to the <a href="https://developer.hashicorp.com/terraform/enterprise/deploy/configuration/network" target="_blank" rel="noreferrer">network requirements page</a> for the latest information.
If a corporate proxy service is filtering outbound Internet traffic, then add the destinations detailed in the preceding link to your allow-list.
