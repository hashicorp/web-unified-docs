---
page_title: Deployment (managed Kubernetes)
description: Deploy Terraform Enterprise on managed Kubernetes.
...
This topic requires a good understanding of Terraform Enterprise and managed Kubernetes services.
Read the <a href="/validated-designs/terraform/installation-guide/architecture" target="_blank">architecture</a> section before this one.
...
- We do not recommend exposing Terraform Enterprise to the public Internet. Users must be on the company network to be able to access the Terraform Enterprise API/UI. However, we recommend allowing Terraform Enterprise to access <a href="https://developer.hashicorp.com/terraform/enterprise/deploy/configuration/network" target="_blank">certain addresses on the Internet</a>:
...
- Terraform Enterprise `v202309-1` or later works with Kubernetes deployments. Review <a href="https://developer.hashicorp.com/terraform/enterprise/deploy" target="_blank">the main public documentation</a> for this architecture if you have not already.
...
- Assess supported versions of Kubernetes from the latest <a href="https://developer.hashicorp.com/terraform/enterprise/releases" target="_blank">Terraform Enterprise Releases</a> document.
...
- The Helm chart that deploys Terraform Enterprise has versions in <a href="https://github.com/hashicorp/terraform-enterprise-helm" target="_blank">this repository</a> and you must read and understand it as part of your preparation for deployment.
...
If you are planning a scaled deployment, ensure your project management team allocates significant resources to engineer a suitable policy-as-code SDLC deployment. It must be ready for UAT at the latest such that users adapt to restrictions _well before_ the production deployment.

## Terraform modules for installation

The primary route to the installation of Terraform Enterprise on Kubernetes is through the use of HVD Modules which require the use of a <a href="https://developer.hashicorp.com/terraform/install" target="_blank">Terraform command-line tool binary</a>. These modules are available in the public HashiCorp Terraform Registry.
...
1. Establish where to store the Terraform state for the deployment. HashiCorp recommends that you store state in HCP Terraform (free access is available or contact your HashiCorp account team for an entitlement for state storage just for Terraform Enterprise installs). If access to HCP Terraform is not possible, we recommend using a secure, cloud-based object store service (S3, Blob Storage, GCS and so on) instead.
1. Select a machine where you execute the Terraform code. This machine needs the <a href="https://developer.hashicorp.com/terraform/downloads" target="_blank">Terraform command-line tool</a>
 available. Use the latest binary version.
1. Ensure that cloud credentials are available on the machine for Terraform execution.
...
- The main repository `README.md` file contains the primary instructions which you must read through _first_ and then follow to deploy Terraform Enterprise.
...
1. Select your managed Kubernetes service below, and then follow the link to the respective public Terraform Registry entry.
1. In the Registry, review the contents, then click the `Source Code` link in the page header to point your browser to the GitHub repository.
1. Read the GitHub repository for the respective Terraform module _in its entirety_. Not doing so may result in a failed deployment. Do not run code you do not understand.
1. Follow the repository `README.md` file step by step, ensuring you have all prerequisites in place _before_ starting the deployment; these may take some time to arrange in your environment and you must account for them in project planning.
...
1. The `README.md` directs you to complete the configuration and deploy Terraform Enterprise using the `terraform init`, `terraform plan` and `terraform apply` commands. If using the example `terraform.tfvars.example` file, remember to remove angled brackets from the resulting `terraform.tfvars` file.
1. Once the Terraform deployment completes, insert the relevant secrets into the cluster. Do this in a way which does not result in secrets storing anywhere else (for example `.bash_history`, VCS and so on). The HVD Modules have a document about Kubernetes secrets management detailing recommendations for different scenarios and lists the secrets you need to use. If your organization has a strategic Kubernetes secrets management approach which avoids writing plain text secrets, we recommend using it. Otherwise, use `read -sp` as per the following command example for each secret in scope to securely instantiate environment variables into your shell and then use these with `kubectl` to ingress them into Kubernetes as the HVD Module instructs.

  ```shell
  read -sp 'TFE_ENCRYPTION_PASSWORD> ' TFE_ENCRYPTION_PASSWORD && export TFE_ENCRYPTION_PASSWORD
  ```

1. The modules contain a `locals_helm_overrides.tf` convenience function which generates a file called `helm/module_generated_helm_overrides.yaml` - this overrides certain default values in the Helm chart for Terraform Enterprise.
1. The instructions then guide you as to the use of <a href="https://helm.sh/" target="_blank">Helm</a> to deploy the Terraform Enterprise container to your Kubernetes cluster.
1. Follow the last link in the README to the information on the HashiCorp public documentation for using the IACT (Initial Admin Console Token) to complete the setup of Terraform Enterprise _within 60 minutes of deployment_.
...
- Ensure that observability tooling is also in place _before_ load testing so that you fully understand CPU, RAM and I/O constraints in your specific context and in terms of connectivity to external services.

### Use of the internal run pipeline versus HCP Terraform Operator

The <a href="https://developer.hashicorp.com/terraform/cloud-docs/integrations/kubernetes" target="_blank">HCP Terraform Operator</a> is more efficient at spreading agent demand by establishing minimum numbers of replicas and preventing thundering herd issues that can occur with the internal <a href="https://developer.hashicorp.com/terraform/enterprise/flexible-deployments/install/configuration#kubernetes-driver-settings" target="_blank">Terraform Kubernetes driver run pipeline</a> where all agents come online at once. However, smaller deployments may find the internal run pipeline sufficient.
...
The default HCP Terraform agent run pipeline configures a pod resource request for every agent at 2048MiB, so if the cluster is not appropriately sized for this reservation, physical memory over-allocation causes run failure. You can adjust this using the `agentWorkerPodTemplate` <a href="https://github.com/hashicorp/terraform-enterprise-helm/blob/main/values.yaml" target="_blank">directive</a> in the Helm `module_generated_helm_overrides.yaml` if you need to adjust the max concurrent workload memory usage. The conservative approach is to size on the limit so you can tune this down if cost efficiency is a priority.
...
In the nominal example, with 30 agents, the maximum RAM requirement would be 60 GB for workspace runs.

For right-sizing of the cluster, at least ten percent overhead is prudent, making the total RAM requirement 66 GB.
Note that this calculation does not include OS requirements or for agents that run in discrete network segments outside of the cluster, which you size separately based on specific graph calculation requirements.
...
These numbers assume, in a scaled environment, HCP Terraform agents run in a dedicated node group/node pool or cluster, as we recommend isolation from the Terraform Enterprise pods.
If running in a shared environment, provide at least 4 GB memory per Terraform Enterprise pod to ensure you apply appropriate resource requests to Terraform Enterprise pods for some protection under load.
...
The HVD Modules all deploy layer 4 load balancers. This is the highest throughput load balancer available.
Ensure that you load the HCP Terraform agent docker image from a performant region local source where possible like ECR, rather than a public Internet-based source.
...
The official Terraform Enterprise deployment module for Elastic Kubernetes Service is available at <a href="https://registry.terraform.io/modules/hashicorp/terraform-enterprise-eks-hvd/aws/latest" target="_blank">this entry in Terraform Registry</a>.

</Tab>
<Tab heading="AKS">

The official Terraform Enterprise deployment module for Azure Kubernetes Service is available at <a href="https://registry.terraform.io/modules/hashicorp/terraform-enterprise-aks-hvd/azurerm/latest" target="_blank">this entry in Terraform Registry</a>
...
Use the following command to identify the version of Kubernetes available for AKS in the region you intend to deploy to, replacing `<region>` with your region (for example `northeurope` or `eastus`). Cross reference this with <a href="https://developer.hashicorp.com/terraform/enterprise/releases" target="_blank">the version of Kubernetes supported by Terraform Enterprise</a> to ensure you deploy a version of Kubernetes which Terraform Enterprise supports.
...
The official Terraform Enterprise deployment module for Google Kubernetes Engine is available at <a href="https://registry.terraform.io/modules/hashicorp/terraform-enterprise-gke-hvd/google/latest" target="_blank">this entry in Terraform Registry</a>
...
The following table maps the [baseline component sizing](/validated-designs/terraform/installation-guide/detailed-design#baseline-component-sizing) to recommended instance types for each managed Kubernetes service.
...
Enable the general Terraform Enterprise metrics, logs, and agent telemetry described in [Observability setup](/validated-designs/terraform/installation-guide/observability-setup); that setup is common to all platforms. On a managed Kubernetes deployment, forward Terraform Enterprise and HCP Terraform agent logs and metrics to the observability service native to your cloud — Amazon CloudWatch on EKS, Azure Monitor and Azure Log Analytics on AKS, and Google Cloud Logging on GKE — and scrape the Terraform Enterprise metrics endpoint with your cluster's metrics stack, such as Prometheus. The HCP Terraform agent exposes telemetry over OpenTelemetry, so route it through an OpenTelemetry collector to your monitoring system.
