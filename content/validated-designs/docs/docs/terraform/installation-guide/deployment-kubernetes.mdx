...
The baseline Terraform Enterprise application, PostgreSQL, and Redis sizing and the external component requirements are covered in the [Detailed design](/validated-designs/terraform/installation-guide/detailed-design) page. The guidance below covers the Kubernetes-specific tuning (pod counts, agent counts, and machine sizing) that builds on that baseline.
