---
title: HCP Terraform Operator for Kubernetes
description: Manage HCP Terraform resources through Kubernetes Custom Resources and auto-scale agent pools.
...
1. Auto-scaling agent pools for HCP Terraform
2. Self-service infrastructure via Kubernetes native consumption
...
![HCP Terraform Operator Agent Pool Scaling](/img/terraform/operating-guides/scale/0050-hcp-k8-agentpool.png)
...
* [HCP Terraform agent connectivity requirements with HCP Terraform](https://developer.hashicorp.com/terraform/cloud-docs/agents/requirements#networking-requirements)
* [HCP Terraform agent connectivity requirements with Terraform Enterprise](https://developer.hashicorp.com/terraform/enterprise/application-administration/agents-on-tfe)
...
1. Using a dedicated Kubernetes cluster or logical node separation to host HCP Terraform agents
2. Use Kubernetes cluster autoscaling where available to manage cluster capacity dynamically. This recommendation is particularly applicable if you are using autoscaling agent pools and there is a high variance between peak and off-peak HCP Terraform run concurrency.
* Considering baseline run concurrency for health checks (drift detection and continuous validation) to set the autoscaling `minReplicas` parameter.
* Scale test your Kubernetes cluster up to your maximum intended concurrency and ensure sufficient capacity and dynamic scaling is handled under load.
...
For detailed AgentPool CRD documentation see link below. [https://github.com/hashicorp/terraform-cloud-operator/blob/main/docs/agentpool.md](https://github.com/hashicorp/terraform-cloud-operator/blob/main/docs/agentpool.md)
...
![Kubernettes Consumption](/img/terraform/operating-guides/scale/0050-hcp-k8-consumption.png)
...
1. Using a single tenant Kubernetes cluster when providing self-service infrastructure with the HCP Terraform Operator.
2. Deploying a single HCP Terraform Operator per Kubernetes cluster and HCP Terraform platform team.
3. Using unique (dedicated) team tokens per HCP Terraform Operator and Kubernetes cluster combination.
4. Leveraging the autoscaling function of Kubernetes cluster, if that function is available (managed services such as EKS Fargate, AKS auto scaler, GKE autopilot do include that function).

<Note>
Though it's preferable adhering to  a single tenant approach, multiple HCP Terraform Operators can be deployed to the same cluster and filters utilized to control namespace access.
</Note>
...
![K8 Operator Installation](/img/terraform/operating-guides/scale/0050-hcp-k8-install.png)

Instructions on how to install the operator can be found on our documentation site: [Set up the HCP Terraform Operator for Kubernetes](https://developer.hashicorp.com/terraform/cloud-docs/integrations/kubernetes/setup). 
...
The HCP Terraform Operator resources utilize a Kubernetes secret to reference a HCP Terraform token (see [Vault Secrets Operator with HCP Terraform Operator](#heading=h.32z4p77wbgsy) to improve your security posture).
...
|-------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| Create and manage agent pools  | [Manage agent pools](https://developer.hashicorp.com/terraform/cloud-docs/users-teams-organizations/permissions#manage-agent-pools) (organization-level permission) |
...
| **Functionality required**                        | **Team Permissions**                                                                                                 |
|---------------------------------------------------|----------------------------------------------------------------------------------------------------------------------|
| Create and manage workspaces within a project     | Team Token <br></br> Project Team Access <br></br> Maintain - Full control of everything in the project, but not the project itself |
...
The [Vault Secrets Operator](https://developer.hashicorp.com/vault/docs/platform/k8s/vso) allows for secrets to be synced between Vault and the Kubernetes secrets into a specified namespace. Within that namespace, applications have access to the secrets. See [https://github.com/hashicorp/vault-secrets-operator](https://github.com/hashicorp/vault-secrets-operator)

The HCP Terraform operator reads in a HCP Terraform token via the CRD from a referenced Kubernetes secret.

![Secrets w/Vault](/img/terraform/operating-guides/scale/0050-hcp-k8-vault.png)

### Observability

The Operator exposes metrics in the Prometheus format for each controller. They are available at the standard `/metrics` path over the HTTPS port 8443. The Operator exposes all metrics provided by the controller-runtime by default. The full list you can find on the [Kubebuilder documentation](https://book.kubebuilder.io/reference/metrics-reference.html).

HCP Terraform Agent Metrics can be collected using OpenTelemetry. When using the HCP Terraform Operator the environment key can be utilized to set TFC_AGENT_OTLP_ADDRESS variable (see [https://developer.hashicorp.com/terraform/cloud-docs/agents/telemetry](https://developer.hashicorp.com/terraform/cloud-docs/agents/telemetry)).
