---
title: Observability
description: Telemetry, logging, audit log forwarding, health probes, and observability for Vault Enterprise on OpenShift.
...
Vault exposes Prometheus-format metrics at the `/v1/sys/metrics` endpoint. For telemetry stanza parameters and recommended defaults, refer to [Vault: Installation Guide - Detailed Design: telemetry stanza](/validated-designs/vault/installation-guide/vms/detailed-design#telemetry-stanza).
...
For critical alert categories, recommended thresholds, and detailed metric guidance for Vault Enterprise, refer to [Vault: validated design guidance - Monitoring and observability: telemetry](/validated-designs/vault/installation-guide/vms/observability-setup#telemetry).
...
For additional guidance on operational log content, refer to [Vault: validated design guidance - Monitoring and observability: operational logs](/validated-designs/vault/administration-guide/monitoring-and-observability#operational-logs).
...
For the full list of observable audit log patterns, refer to [Vault: validated design guidance - Monitoring and observability: usage patterns](/validated-designs/vault/administration-guide/monitoring-and-observability#usage-patterns). For the list of privileged endpoints that require SIEM alerts, refer to [Vault: validated design guidance - Monitoring and observability: privileged endpoints](/validated-designs/vault/administration-guide/monitoring-and-observability#privileged-endpoints).
...
On OpenShift, the Vault Helm chart uses the health endpoint for readiness and liveness probes. The `/v1/sys/health` API returns HTTP status codes that reflect the state of the Vault Pod you query. For status code definitions and alerting guidance, refer to [Vault: validated design guidance - Monitoring and observability: health API](/validated-designs/vault/installation-guide/vms/observability-setup#health-api) and the [Vault /sys/health API documentation](https://developer.hashicorp.com/vault/api-docs/system/health).
