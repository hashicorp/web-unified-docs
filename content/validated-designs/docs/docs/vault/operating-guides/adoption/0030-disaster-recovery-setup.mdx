...
description: Set up disaster recovery replication to protect against catastrophic failures by maintaining a warm standby Vault cluster.
...
- You have reviewed and implemented the HashiCorp [Vault Solution Design Guide HVD](/validated-designs/vault-solution-design-guides-vault-enterprise).
...
You must have permission to call the <code>auth/token/create</code> endpoint in the root namespace for this CLI command. In the <a href="/validated-designs/vault-operating-guides-adoption/authentication-for-people#configure-admin-policy">Configure Admin Policy</a> section, you will configure a policy with super administrators access to call this API in the root namespace.
...
1. Disable load balancing/routing while failover is in progress:
  ...
  1. Disable global load balancer (e.g. F5 GSLB, Amazon Route53 Failover Record, HashiCorp Consul). Recall the requirement of a [failover mechanism](/validated-designs/vault-solution-design-guides-vault-enterprise/architecture#failover-mechanism) in the HVD for Vault Solution Design.
