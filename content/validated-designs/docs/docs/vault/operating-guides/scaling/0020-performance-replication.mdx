...
description: Scale Vault horizontally and reduce latency across regions using performance replication for active secret management in multiple locations.
...
As outlined in the [Vault: Operating Guide for Adoption](https://developer.hashicorp.com/validated-designs/vault-operating-guides-adoption/disaster-recovery-setup), Vault Enterprise supports two types of cluster replication, Disaster Recovery (DR) Replication and Performance Replication (PR).
...
For item number one, refer to the [Vault: Operating Guide for Adoption](https://developer.hashicorp.services/validated-designs/vault-operating-guides-adoption/observability-and-management) to ensure that telemetry has been configured on primary and secondary clusters, and the outlined key metrics are being monitored for general cluster health.
