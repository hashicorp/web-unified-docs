---
page_title: Key lifecycle management administration
description: Configure and administer Vault key management and KMIP secrets engine capabilities.
...
# Key lifecycle management administration

<Note>

This page covers the operator setup and administration of key lifecycle management. For consumer usage workflows, see the User Guide: [Key lifecycle management](/validated-designs/vault/user-guide/key-lifecycle-management).
...
- Review the compatible key types for each KMS Provider ([link](https://developer.hashicorp.com/vault/docs/secrets/key-management#compatibility))
...
Key material will always be securely transferred in accordance with the [key import specification](https://developer.hashicorp.com/vault/docs/secrets/key-management#kms-providers) of the supported KMS providers.  

A typical implementation follows the below diagram

![Implementation Diagram](/img/vault/operating-guides/scaling/image6.png)
...
To permanently delete the key from the secrets engine, the [delete key](https://developer.hashicorp.com/vault/api-docs/secret/key-management#delete-key) API may be invoked.
...
<Note>
If a timeout occurs when distributing a key to a KMS provider, you may need to increase the value of VAULT_CLIENT_TIMEOUT.  Note that rotating a key in the secrets engine will result in a new key version in Vault and in the related KMS provider.<br />
</Note>
...
The [OASIS Key Management Interoperability Protocol (KMIP)](http://docs.oasis-open.org/kmip/spec/v1.4/kmip-spec-v1.4.html) standard is a widely adopted protocol for handling cryptographic workloads and secrets management for enterprise infrastructure such as databases, network storage, and virtual/physical servers. 
...
![KMIP Client Diagram](/img/vault/operating-guides/scaling/image7.png)

<Note>
The KMIP listener will only be enabled for performance standby servers. This means that if performance standby is not used, the load balancer must ensure that the KMIP route is directed only to the leader of the cluster. This can be achieved through <a href="https://developer.hashicorp.com/vault/api-docs/system/health">health checks</a> via the cluster port (8200) to determine the current leader node.
</Note>

### KMIP Certificate Authority
...
The below table outlines the KMIP operations supported by Vault.  When you grant KMIP operations, the argument convention is `operation_<KMIP_operation>` like *operation_activate or operation_revoke*
...
![KMIP Client Flow Chart](/img/vault/operating-guides/scaling/image8.png)
...
| Platform | Reference  |
| :---- | :---- |
| MongoDB | [MongoDB Field Level Encryption with HashiCorp Vault KMIP Secrets Engine](https://www.hashicorp.com/blog/mongodb-field-level-encryption-with-hashicorp-vault-kmip-secrets-engine) [MongoDB Encryption](https://developer.hashicorp.com/vault/tutorials/adp/kmip-engine#kmip-client-configuration)  |
| IBM Storage Scale | [Setup Encryption using HashiCorp Vault KMIP Secrets Engine as the RKM key server](https://www.ibm.com/docs/en/storage-scale/5.1.8?topic=environment-setup-using-hashicorp-vault-kmip-secrets-engine) |
| AWS KMS External Key Store (XKS) | [AWS XKS Integration through PKCS\#11 Provider](https://developer.hashicorp.com/vault/docs/enterprise/pkcs11-provider/aws-xks) |

While the examples above illustrate some Vault KMIP implementations, they do not provide a comprehensive list of compatible platforms. HashiCorp maintains a list of compatible platforms [here](https://developer.hashicorp.com/vault/docs/interoperability-matrix#vault-as-an-external-key-management-system-ekms), which includes all the platforms that have been tested to work with Vault as an external key management system.
