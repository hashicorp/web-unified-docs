...
description: Infrastructure sizing recommendations for Consul control plane on VMs and Kubernetes to handle workloads smoothly.
...
## Sizing guidelines
The Consul server nodes process all read and write operations from the [agents](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/consul-on-aws-ec2#consul-agent-configuration) as well as maintain a consensus among the cluster, and as such are I/O bound for writes and CPU bound for reads. This needs to be taken into consideration and monitoring put in place to adjust as required depending on the type of workload inside of the cluster.
...
As a general rule, we recommend that the maximum size for a single datacenter is 5,000 Consul client agents. This estimate is based on impact of recovery time, write and read requests, and other factors. We recommend deploying read replicas for improved scalability in clusters that are read-heavy. We have customers who have scaled Consul to tens of thousands of [agents](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/consul-on-aws-ec2#consul-agent-configuration) per cluster, but it is highly dependent on the read and write workloads of the cluster. As such, customers must optimize for stability at the [gossip layer](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/appendices#appendix-3:-gossip-protocol) as the cluster scales. The two main factors that affect this with client agent are:
...
HashiCorp recommends [monitoring](/validated-designs/consul-operating-guides-adoption/initial-configuration#configure-platform-monitoring) your production deployment to take data-driven informed decisions to scale your production server resource limits or vertically scale the VM deployments.