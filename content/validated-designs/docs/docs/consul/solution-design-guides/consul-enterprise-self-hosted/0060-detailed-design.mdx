...
description: The Detailed design section will provide architecture recommendations to meet the technical requirements of your organization and deploy consul to meet the requirements.
...
Further, we need to authorize the Consul telemetry collector service to get metrics from other service proxies. We create [service intentions](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/appendices#consul-telemetry-collector-intention) that authorize proxies to push metrics to the collector.
...
HashiCorp Consul supports various Hardware Architectures and Operating Systems. You can find a complete list of supported installation platforms <a href="https://developer.hashicorp.com/consul/downloads" target="_blank" rel="noreferrer">here</a>. For purposes of this Solution Design Guide, while operating Consul Control Plane on VMs, we recommend utilizing a Linux distribution with an AMD64 architecture.
...
##### Secrets
Consul Kubernetes (EKS) deployment requires all the above specified required [secrets](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/appendices#consul-server-helm). If not provided, Helm deployment will generate the secrets and update the Kubernetes secrets. If you specify Vault-as-backend, or provide your own secrets, you need to update the [helm values](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/appendices#consul-server-helm) for the Consul deployment.
