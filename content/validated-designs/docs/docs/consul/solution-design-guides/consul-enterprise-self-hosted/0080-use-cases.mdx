---
title: Use Cases
description: Service discovery, health monitoring, service mesh, gateways, and API gateway examples using HashiCups application.
...
- Using Iaac approach using client agents, service definition files can be placed on disk that Consul can reload during agent start or with `consul reload` command.
...
Because external services by definition don't have a local Consul agent, you can't register them with that agent or use it for health checking. Instead, you must register them directly with the catalog using the `/catalog/register` endpoint. We can use the /catalog/register endpoint for registering external services or nodes but with `skip_node_update` to the API will not modify the node registration and assign specific service and checks to the existing node.
...
![ESM](/img/consul/solution-design-guides/consul-enterprise/0080-esm-1.png)
...
External service can be monitored with <a href="https://developer.hashicorp.com/consul/tutorials/developer-discovery/service-registration-external-services#monitor-the-external-service-with-consul-esm" target="_blank" rel="noreferrer">Consul ESM</a> (External Services Monitor) that runs as a daemon alongside Consul in order to health check external nodes and update their status in the catalog.

Diagram shows how Consul ESM works with Consul to monitor the health of external services:

![ESM2](/img/consul/solution-design-guides/consul-enterprise/0080-esm-2.png)
...
We will register two services and their sidecar proxies in the Consul catalog and then start the services and sidecar proxies. Finally, we will demonstrate that the service-to-service communication is going through the proxies by stopping traffic with an "<a href="https://developer.hashicorp.com/consul/docs/connect/intentions" target="_blank" rel="noreferrer">intention</a>".

![intentions](/img/consul/solution-design-guides/consul-enterprise/0080-intentions.png)
...
![Terminating Gateways](/img/consul/solution-design-guides/consul-enterprise/0080-terminating-gateway.png)
...
![api gateway](/img/consul/solution-design-guides/consul-enterprise/0080-api-gateway.png)
