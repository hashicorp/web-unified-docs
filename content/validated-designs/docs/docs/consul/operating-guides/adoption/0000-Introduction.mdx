---
title: Introduction
description: Learn how to adopt Consul Enterprise for managing  service discovery and multi-platform connectivity.
...
When using Consul Enterprise (Self-managed / Self-Hosted version of HCP Consul), this guide assumes that you have a basic understanding of Consul and have reviewed or implemented the [Consul Enterprise Solution Design Guide](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/).
...
The following is a sample high-level architecture diagram that shows the Consul Enterprise deployment this Operating Guide recommends.  The Consul Enterprise server agents are deployed by the Terraform module from the [Solution Design Guide](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/aws-implementation).  Client workloads running in Amazon EKS are segregated into Consul Enterprise Admin Partitions corresponding to each Kubernetes cluster.
