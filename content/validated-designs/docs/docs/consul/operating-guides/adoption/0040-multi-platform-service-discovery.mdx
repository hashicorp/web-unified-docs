...
description: Service registration, health checking, and using the Consul service catalog for discovery across platforms.
...
1. Deploy one instance of the Consul catalog sync agent per Kubernetes cluster into a dedicated Partition inside your Consul Enterprise deployment using an external server configuration. Refer to our [Solution Design Guide](/validated-designs/consul-solution-design-guides-consul-enterprise-self-hosted/consul-on-kubernetes) for Kubernetes specific deployment considerations.
