...
description: Define producer and consumer roles for operating Consul Enterprise.
...
Operating a networking platform, like Consul for <a href="https://developer.hashicorp.com/consul/docs/concepts/service-discovery" target="_blank" rel="noreferrer">service discovery</a>, requires coordination between different teams to:

- Setup and maintain the platform. This may include the following:
    - Manage access to the platform and defining <a href="https://developer.hashicorp.com/consul/docs/security/acl/acl-policies" target="_blank" rel="noreferrer">policies and roles</a>
...
1. **Producers** ensure that the shared services are readily accessible and efficiently utilized by the Consumers, empowering them to leverage the benefits of the Consul platform effectively. This is usually the platform team. Some of their responsibilities include:
...
- Implementing services queries by using <a href="https://developer.hashicorp.com/consul/docs/services/discovery/dns-overview" target="_blank" rel="noreferrer">Consul DNS</a>.

![Cloud Adoption Framework - Teams](/img/consul/operating-guides/adoption/0020-cof-teams.png)

The platform team is responsible for delivering each layer of a cloud operating model as a
standardized shared service that can be consumed by end-users in the organizations.
...
![COF Dev Teams](/img/consul/operating-guides/adoption/hvd-consul-oga-dev-self-service-path.svg)

From a self-service platform perspective, different roles participate in the workflow:

1. The platform team is responsible for maintaining the shared Consul control plane, the peering settings, and exported services configuration that are required for cross Consul data center workloads.
2. Infrastructure operators and architects deploy and configure the isolated Consul admin partitions depending on the specific team or business unit.
3. Developers, network engineers, or operators will register the service.
...
4. Monitoring services, logs and metrics is a critical task for developers, infrastructure operators and security engineers. This will help them manage and ensure the required availability of services and infrastructure resources, and give some helpful insights about access control (e.g. blocked ACLs).
5. With a centralized service catalog, network operators can use Consul DNS and service discovery to configure load balancers or firewall rules. Developers can use Consul DNS to discover and connect services together in their environments.
