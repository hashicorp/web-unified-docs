...
This guide provides detailed instructions for deploying Nomad Enterprise in on-premises environments, covering both virtual machine and bare metal installations. Before proceeding, ensure you have reviewed the <a href="/validated-designs/nomad-solution-design-guides-nomad-enterprise/architecture" target="_blank">Nomad Architecture</a> and <a href="https://developer.hashicorp.com/nomad/docs/deploy/production/requirements" target="_blank">Deployment Requirements</a>.
...
- If there are more than 3 failure domains (datacenters close to one another (&lt; 10ms latency), independent datacenter rooms or racks, etc), deploy 6 servers and make use of the <a href="/validated-designs/nomad-solution-design-guides-nomad-enterprise/architecture#redundancy-zones" target="_blank">Redundancy Zones</a> feature, which ensures that there are the correct number of voting servers in each failure domain, and increases the fault tolerance and performance of the cluster.
...
Create a standard X.509 certificate for the Nomad servers. Follow your organization’s process for creating a new certificate that matches the DNS record you intend to use for accessing Nomad. This guide assumes the conventions and using self-signed certificates from the <a href="/validated-designs/nomad-solution-design-guides-nomad-enterprise/architecture" target="_blank">Nomad Architecture</a> and <a href="https://developer.hashicorp.com/nomad/docs/secure/traffic" target="_blank">Enable TLS tutorial</a>.
...
- <a href="/validated-designs/nomad-solution-design-guides-nomad-enterprise/architecture#redundancy-zones" target="_blank">Redundancy zones</a>: Nomad uses these values to partition the servers by redundancy zone/failure domain, and aims to keep one voting server per zone. Extra servers in each zone stay as non-voters on standby and Nomad promotes these the respective active voter leaves or dies.
...
For day 2 activities, such as observability and backups, consult the <a href="/validated-designs/nomad-operating-guides-nomad-enterprise" target="_blank">Nomad Operating Guide</a>.
