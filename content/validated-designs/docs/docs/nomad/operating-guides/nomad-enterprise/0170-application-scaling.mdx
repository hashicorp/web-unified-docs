...
description: Scale applications horizontally with Nomad Autoscaler using threshold, target-value, and pass-through strategies.
...
Nomad Enterprise offers multiple scaling mechanisms to address the diverse needs of modern applications.
These include horizontal scaling, which adjusts the number of task instances, and dynamic application sizing, which modifies resource allocations for individual tasks.

## Resource allocation with resource blocks

Resource blocks in Nomad job specifications are crucial for ensuring optimal resource allocation and utilization.
The definition of resource requirements for tasks guarantees the allocations are on nodes that have sufficient capacity to run your applications.

Focus on these primary and associated attributes when configuring required resources for your workload.

- `cpu` & `memory` - [resources{}](https://developer.hashicorp.com/nomad/docs/job-specification/resources)
- `disk` - [emphemeral_disk{}](https://developer.hashicorp.com/nomad/docs/job-specification/ephemeral_disk)
- `device type` - [device{}](https://developer.hashicorp.com/nomad/docs/job-specification/device)
...
- Always specify CPU and memory requirements for each task. Allocate slightly more resources than the default minimum (CPU=100Mhz & Memory=300mb) to account for peak loads and prevent resource contention.
  - Nomad automatically finds a suitable node with the available resources you specify. Ensured this before considering [horizontal scaling](#horizontal-application-scaling-with-nomad-autoscaler).
- Conduct thorough performance testing to determine precise resource requirements for your applications. [Dynamic application sizing](#dynamic-application-sizing) along with correct stress testing can give you an idea of required resources for your application.
- [emphemeral_disk{}](https://developer.hashicorp.com/nomad/docs/job-specification/ephemeral_disk) is only used if you have data within the allocation itself. This does not include any mounted volumes or bind paths outside of the allocation directory. Ephemeral disks are perfect for data that you can rebuild if needed, such as an in-progress cache or a local copy of data.
  - If your application data does not need to be ephemeral, however you need larger allocation disk size, consider using [volumes](https://developer.hashicorp.com/nomad/docs/stateful-workloads/static-host-volumes).
  - If your workload has specific device requirements, consider using the [device{}](https://developer.hashicorp.com/nomad/docs/job-specification/device) block to request access to a GPU, FPGA, or TPU.
- Use [Numa](https://developer.hashicorp.com/nomad/docs/job-specification/numa) settings for your application if needed.
...
1. [**APM (Application Performance Monitoring)**](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/apm): Collects metrics from either Nomad API, Prometheus, or Datadog.
    Nomad Enterprise operators configured this on the client node configuration file and in the Autoscaler job specification. Prometheus is the recommended APM. See the [Observability](/validated-designs/nomad-operating-guides-nomad-enterprise/observability) section.
1. [**Strategy**](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/strategy): Determines how to interpret metrics and make scaling decisions.
1. [**Target**](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/target/nomad): Executes the scaling actions on the Nomad cluster. For application horizontal scaling, this block can be omitted as Nomad will populate them on job submission.

For more details, go to [Nomad Autoscaler concepts](https://developer.hashicorp.com/nomad/tools/autoscaling/concepts) page.

## Horizontal scaling strategies

The Nomad Autoscaler provides several strategies for horizontal application scaling, each designed to address different scaling scenarios and requirements.
This section details four strategies along with their use cases and recommendations to assist you in choosing the right strategy.

### Threshold

The [threshold](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/strategy/threshold) strategy defines a range of metric values with a lower and upper bound, and takes action when the current metric value falls within those bounds. Note that `lower_bound` is inclusive while `upper_bound` is exclusive. For example, a check with `lower_bound = 70` and `upper_bound = 100` with `delta = 1` adds instances when the metric is between 70 and 99.
...
The [Target Value](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/strategy/target-value) strategy aims to maintain a specific metric at a desired target value. Unlike the Threshold strategy, which is reactionary, the target value strategy is proactive. For example, keeping CPU utilization at 70%. Nomad adjusts the number of instances to achieve this target.
...
The Pass-through strategy allows external systems or custom logic to dictate the number of instances.
Nomad Enterprise acts as an executor, scaling the application based on the input it receives, removing computational power from the Nomad Enterprise cluster nodes.
...
- Implement safeguards ([max_scale_up/down](https://developer.hashicorp.com/nomad/tools/autoscaling/plugins/strategy/pass-through#max_scale_up)) to prevent rapid over or under-scaling.
...
[Evaluation interval](https://developer.hashicorp.com/nomad/tools/autoscaling/policy#evaluation_interval) and cool-down period are parameters that impact the responsiveness and stability of your scaling operations.
...
In the preceding definition, Nomad Enterprise adds instances when memory usage is within 400-512 MB (lower_bound inclusive, upper_bound exclusive) or when CPU usage is within 700-1000 MHz, until reaching the defined `max` count. When using multiple checks with the threshold strategy, the `group` parameter ensures all checks work together properly for scale-down operations. Scaling uses the safest check action if there is overlap between the bounds.

Nomad Enterprise sets the `evaluation_interval` to a conservative 5 minutes to respond to changes in traffic and sets the `cooldown` period to 10 minutes to allow the system to stabilize after each scaling action.
...
The [`spread`](https://developer.hashicorp.com/nomad/docs/job-specification/spread) block allows application owners to increase the failure tolerance of their applications. Specify a node attribute to spread allocations over. This allows operators to spread allocations over attributes such as datacenter, availability zone, or even rack in a physical datacenter using metadata.
...
When scaling down (reducing the number of allocations), the Autoscaler does not consider the spread configuration or metadata when choosing which allocations to remove.
Instead, it uses allocation IDs to determine the order of removal.

For example, if you have:
<ul>
<li>Allocations 1, 2, 3 on Node 1</li>
<li>Allocations 4, 5, 6 on Node 2</li>
<li>Allocations 7, 8, 9 on Node 3</li>
</ul>

When scaling down, Nomad Enterprise removes allocations in reverse order (9, 8, 7, and so on) rather than maintaining an even distribution across nodes.
is behavior can potentially impact the availability and balance of your application across the cluster during scale-down operations.
...
Refer to the [Dynamic App Sizing Concept](https://developer.hashicorp.com/nomad/tutorials/autoscaler/dynamic-application-sizing-concepts) and [Dynamic App Sizing](https://developer.hashicorp.com/nomad/tutorials/autoscaler/dynamic-application-sizing) tutorial for an overview of the concepts, how to configure, and recommendations.
...
- If the native Nomad Enterprise UI is not sufficient, then you can utilize [the Nomad API](https://developer.hashicorp.com/nomad/api-docs/recommendations) to retrieve recommendations and display them in a custom dashboard for efficient monitoring and decision-making.
...
When implementing autoscaling for stateful applications in Nomad Enterprise, consider the following factors to ensure data integrity and performance.
Be aware that certain stateful applications may have inherent limitations on how they scale. Be aware that some stateful applications may not be a good fit for autoscaling.

### Storage

The underlying storage is _crucial_ for stateful applications when autoscaling. For container workloads, leverage Nomad Enterprise's Container Storage Interface (CSI) support to dynamically attach storage to instances as your workload scales. This gives you the flexibility to manage storage resources on-the-fly, adapting to your application's changing needs.
...
For clustered applications, utilize [variable locks](https://developer.hashicorp.com/nomad/docs/concepts/variables#locks) to prevent split-brain scenarios and data corruption during scale-up and scale-down events.

### Kill timeout

Ensure your application has proper shutdown procedures to persist state in a safe manner before scaling down. Use  [`kill_timeout`](https://developer.hashicorp.com/nomad/docs/job-specification/task#kill_timeout) to allow sufficient time for graceful shutdowns and consider [`max_kill_timeout`](https://developer.hashicorp.com/nomad/docs/configuration/client#max_kill_timeout) set on the client configuration to ensure job authors do not exceed this amount.

### Service discovery

Implement [Service Discovery](/validated-designs/nomad-operating-guides-nomad-enterprise/service-discovery) or [Service Mesh](https://developer.hashicorp.com/nomad/tutorials/integrate-consul/consul-service-mesh) to dynamically manage network identities and route traffic appropriately.

As always, use a well-tested autoscaling development environment to identify and resolve any issues before implementing in production.
This is _critical_ for stateful applications due to their data persistence requirements.
For additional details not related to autoscaling, view the [Considerations for Stateful Workloads](https://developer.hashicorp.com/nomad/docs/architecture/storage/stateful-workloads) page.

## Standardization

To promote a self service model, consider implementing job templates or utilizing [Nomad Pack](https://developer.hashicorp.com/nomad/tools/nomad-pack) to provide a consistent deployment workflow to include standard metrics across applications while still allowing for customization.
...
Operators can then use features such as [Sentinel](/validated-designs/nomad-operating-guides-nomad-enterprise/sentinel), [resource quotas](/validated-designs/nomad-operating-guides-nomad-enterprise/resource-quotas), and [node pool governance](https://developer.hashicorp.com/nomad/docs/other-specifications/namespace#node_pool_config-parameters) to establish appropriate guardrails. This balanced approach ensures flexibility for individual application needs while maintaining overall system integrity and resource management.
