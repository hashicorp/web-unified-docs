---
page_title: Agent deployment
description: Deploy and scale Vault Radar agents for private network scanning, including prerequisites, resource requirements, and scaling strategies.
...
Deploying agents is an operator task for hybrid scanning of resources that are not publicly accessible by HCP. This page covers the prerequisites, resource requirements, data source connection, and horizontal scaling of Vault Radar agents.

## Prerequisites

- Outbound connectivity to the HCP platform
- An [agent pool in the HCP Portal](https://developer.hashicorp.com/hcp/docs/vault-radar/agent/deploy#create-an-agent-pool-in-the-hcp-portal)
...
For deploying the agent, follow the [Deploy Vault Radar Agent](https://developer.hashicorp.com/hcp/docs/vault-radar/agent/deploy) documentation page.
Kubernetes is currently our recommended platform for deploying the Agent. You are welcome to extrapolate the configuration for other platforms, but we cannot guarantee support for that platform at this time.
...
![Select Vault Radar Agent Scan option](/img/vault-radar/operating-guides/select-scan-type.png)
...
![Configure code server connection settings](/img/vault-radar/operating-guides/configure-code-server.png)
...
For additional information, please check the [Deploy Vault Radar agent page](https://developer.hashicorp.com/hcp/docs/vault-radar/agent/deploy).
