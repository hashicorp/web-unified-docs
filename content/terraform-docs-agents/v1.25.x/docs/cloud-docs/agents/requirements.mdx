...
* `git` - The Git CLI. May be used by the Terraform CLI when downloading modules
  from remote Git servers. Git version 2.3 or greater is required. When using agents for private VCS operations for Stacks, we recommend that the agent includes git version 2.35 or greater.
