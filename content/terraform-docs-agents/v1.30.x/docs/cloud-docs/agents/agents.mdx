...
last_modified: 2026-01-22T16:27:00-06:00
...
* `-otlp-forward-spans`:
    Forward the agent's OpenTelemetry trace context and OTLP exporter
    configuration to the terraform CLI subprocess during plan and apply. When
    enabled, the policy spans terraform emits are exported to the same
    collector and nested under the agent's trace, stitching the agent and core
    traces into one tree. Has no effect unless -otlp-address is also set.

    Default: `false`

    Environment variable: `TFC_AGENT_OTLP_FORWARD_SPANS`
...
* `-accept <job_types>`:
    ...
    Acceptable job types are "plan", "apply", "policy", "assessment", "ingress",
     "stack_prepare", "stack_plan", "stack_apply", "source_bundle", "stack_aggregate_outputs",
    and "test". Do not put whitespace in between entries.

    Default: `plan,apply,policy,assessment,stack_prepare,stack_plan,stack_apply,source_bundle,stack_aggregate_outputs,test`
