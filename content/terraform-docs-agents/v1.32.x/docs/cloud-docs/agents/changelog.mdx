...
* Added support for configurable Vault Transit Secrets Engine mount path for Hold Your Own Key. (#1633)
* Added support for Terraform policy evaluation during workspace Terraform runs. (#1701)

## 1.31.1 (09/01/2026)

BUG FIXES: 
*  Fixed missing description for OPA policies with individually managed and api management modes. (#1641)

## 1.31.0 (08/24/2026)

FEATURES:

* Added support for `no_proxy` settings by leveraging inbuilt `httpproxy` functions . (#1622)

## 1.30.2 (08/18/2026)

IMPROVEMENTS:

* Added direct reads of the cgroup v2 (`cpu.stat`, `memory.current`, `memory.max`, `io.stat`). Container ID detection now happens once at startup; CPU, memory, and I/O profiling fall back to host-level stats when cgroup files are unavailable. (#1631)

BUG FIXES:

* Fixed auto update for Agent versions > 1.30.1. (#1621)
* Fixed query metadata not being reported when a `terraform query` operation fails partway through. (#1569)

SECURITY:

* Upgrade to golang.org/x/mod v0.40.0 to resolve [CVE-2026-56865](https://www.cve.org/CVERecord?id=CVE-2026-56865)

## 1.30.1 (08/05/2026)

SECURITY:

* Upgrade go to 1.26.5 to resolve [CVE-2026-39822](https://avd.aquasec.com/nvd/2026/cve-2026-39822/) (#1572)

## 1.30.0 (07/23/2026)

FEATURES:

* Added support for AWS KMS multi-region keys in HYOK, automatically resolving the AWS region instead of using a fixed region. (#1551)
* Added support for parsing and reporting Terraform action invocation failure counts from apply summaries. (#1557)

BUG FIXES:

* Fixed git ingress clone handling so empty SSH private keys no longer produce invalid temporary identity files and misleading `error in libcrypto` failures. (#1561)
* Replaced the misleading `fork/exec ... file name too long` error, raised when an environment variable value exceeds the operating system's per-variable size limit (128 KB on Linux), with a clear message naming the offending variable and explaining the limit. (#1566)

IMPROVEMENTS:

* Reduced problematic metric cardinality (#1560)
* Updated cloud.google.com/go/kms to v1.33.0. (#1607)

SECURITY:

* Replaced golang.org/x/crypto/openpgp dependency with github.com/ProtonMail/go-crypto/openpgp to address GO-2026-5932 (#1581)

## 1.29.0 (06/23/2026)

IMPROVEMENTS:

* Promoted the git binary cloner to the primary and only implementation for git-type ingress jobs, retiring the `ingress-use-git-binary` feature flag now that it is fully rolled out. (#1528)
* Improve readme-extraction performance using batch processing. README extraction now follows symlinks whose targets stay within the workspace root (e.g. nested READMEs that are symlinked within the repo), while continuing to reject symlinks that escape the workspace. Previously all symlinks were skipped, which could cause legitimate nested READMEs to be missing. (TF-34077)
* Changed OpenTelemetry collector connection failures to log a warning instead of preventing agent startup. (#1541)
* Added a `-otlp-forward-spans` option (environment variable `TFC_AGENT_OTLP_FORWARD_SPANS`) to opt in to forwarding the agent's OpenTelemetry trace context and OTLP exporter configuration to the Terraform CLI subprocess during plan and apply. When enabled (alongside `-otlp-address`), the policy spans Terraform emits are exported to the same collector and nested under the agent's trace. Disabled by default.

## 1.28.12 (06/09/2026)

BUG FIXES:

* Show "Invalid" problem diagnostics raised during the plan step of a Stack as errors on the deployment timeline instead of ignoring them and leaving the deployment hanging in the running state. (#1521)

IMPROVEMENTS:

* Enhanced sanitization for Terraform state and plan artifacts created during HYOK runs. Sanitized state and plan files are now re-scanned for sensitive values which may have been propagated by providers into non-sensitive attributes, causing them to be missed by the previous sanitization passes. (#1499)
...
* Fixed auto update config error.

## 1.28.9 (05/12/26)

BUG FIXES:

* Accept the deprecated `.tfstack.hcl` / `.tfstack.json` extensions when checking for the presence of stack components configuration files, alongside the current `.tfcomponent.hcl` / `.tfcomponent.json` extensions. (#1465)
* Catch incoherent `publish_output` blocks at Stack prepare time and surface them as configuration diagnostics instead of failing later at output aggregation time. (#1474)

## 1.28.8 (04/28/2026)

SECURITY:

* Updated go to 1.26.2 to resolve various CVEs (#1463)

IMPROVEMENTS:

* Added validations to check for the existence of component and deployment files. One of `*.tfcomponent.hcl` or `*.tfcomponent.json` and one of `*.tfdeploy.hcl` or `*.tfdeploy.json` must be present. (#1444)

## 1.28.7 (04/14/2026)

SECURITY:

* Updated aws-sdk-go-v2/config to 1.32.14 (#1441)
* Updated aws-sdk-go-v2/credentials to 1.19.14 (#1441)
* Updated golang.org/x/crypto to 0.50.0 (#1440)
* Updated golang.org/x/net to 0.53.0 (#1440)
* Updated golang.org/x/sys to 0.43.0 (#1440)
* Updated hcp-sdk-go to 0.171.0 (#1440)

IMPROVEMENTS:

* Added a clearer error message when a Stack configuration is missing a `.terraform-version` file. (#1411)

## 1.28.6 (04/01/2026)

SECURITY:

* Updated go-git to 5.17.1 to fix a vulnerability (#1414)

BUG FIXES:

* Fixed an issue where git submodule authentication failed for URLs containing embedded userinfo. The fix ensures credentials are consistently applied using `http.extraHeader` while maintaining backward compatibility with `url.insteadOf`. (#1377)

IMPROVEMENTS:

* Add support for Action Invocation Planning in Stacks (#1304)
* Add support for Deferred Action Invocations in Stacks (#1327)
* Support dynamic module sources, a new feature shipped in Terraform 1.15. (#1405)

## 1.28.5 (03/18/2026)

BUG FIXES:

* Fixed an issue where git submodule authentication failed for URLs containing embedded userinfo. The fix ensures credentials are consistently applied using `http.extraHeader` while maintaining backward compatibility with `url.insteadOf`.

## 1.28.4 (03/4/2026)

IMPROVEMENTS:

* Updated GCP Dynamic Credentials to allow a principal type to be specified. This can be configured using the `TFC_GCP_PRINCIPAL_TYPE` environment variable in HCP Terraform. When set to `workload_pool` the agent will use Workload Identity Federation to assume the role of a Workload Identity Pool without service account impersonation. (#1328)

## 1.28.3 (02/17/2026)

BUG FIXES:

* Removed registry response size limit to support modules with large version lists in stacks source bundle jobs (#1322)

IMPROVEMENTS:

* Added support for overriding the Vault client timeout value for Vault-Backed Dynamic Credentials (#1307)
* Updated go to 1.25.7 to resolve various CVEs (#1323)
* Updated git to 2.53.0 built with OpenSSL to support X509 Policy Constraints extension in certificates (#1332)

## 1.28.2 (02/03/2026)

SECURITY:

* Updated go to 1.25.6 to resolve various CVEs (#1306)
* Added feature-flagged git hardening through restrictions (#1316)

## 1.28.1 (01/27/2026)

BUG FIXES:

* Fixed an issue where opa policy evaluation might fail due to hardcoded binary url check for opa-wrapper. (#1311)
* Fixed an issue where parsing the Terraform plan summary could fail due to ANSI color code. (#1303)
* Fixed git submodule authentication by injecting credentials into submodule URLs to support HTTPS-based git operations (#1291)

## 1.28.0 (01/20/2026)

FEATURES:

* Added support for HashiCorp managed OPA binary (#1279)

BUG FIXES:

* Fixed an issue where potential errors for an edge case involving the `terraform query` command were not properly surfaced in the HCP Terraform UI. (#1282)
* Fixed an issue where module revocation and deprecation notifications failed for modules with source subpaths (#1288)

IMPROVEMENTS:

* Added support for Basic Auth to request forwarding egress proxy (#1287)

## 1.27.0 (01/06/2026)

IMPROVEMENTS:

* stacks: Added support for Terraform Stacks component source addresses in the Terraform Registry client (#1256)

## 1.26.0 (12/09/2025)

FEATURES:

* Added ability to capture Terraform query output and accumulate related metadata (#1229)

BUG FIXES:

* Updated go-slug to 0.18.1 to fix incorrect absolute path errors for some tar formats (#1259)
* Fixed bug causing Hold Your Own Key (HYOK) to interfere with requests from the `terraform-provider-tfe`, causing them to be `unauthorized` (#1908)

IMPROVEMENTS:

* Increased network connection debugging information generated when failing to set up Request Forwarding to make it easier to diagnose where in the network connection issues are occurring (#1252)

## 1.25.2 (11/11/2025)

SECURITY:

* Updated Go to 1.25.3 to resolve various CVEs (#1225)

BUG FIXES:

* Fixed an issue impacting Stacks `source_bundle` operation where `.terraformignore` file entries
for directories with missing trailing `/` would result in file read errors (#1226)
...
* Added query as one of the default workload types the agent can run (#1183)

BUG FIXES:

* Fixed an issue where identical module sources using the FQDN and generic hostname resulted in a 422 error (#1178)

SECURITY:

* Updated Go to 1.25.1 to resolve various CVEs (#1188)

## 1.24.0 (09/16/2025)

FEATURES:

* Added support to run Terraform query agent jobs (#1125)
* Added support for Stacks (#1158)

## 1.23.2 (08/14/2025)

SECURITY:

* Changed Sentinel and OPA command output to log at `trace` level. These
policy engines allow authors to create policies which may inadvertently output
sensitive information, which should never be included in normal operational
logs. (#1139)
...
* Fixed agent to terminate immediately when authentication tokens are revoked during job execution (#1092)

SECURITY:

* Updated Go to 1.24.5 to resolve various CVEs (#1122)
