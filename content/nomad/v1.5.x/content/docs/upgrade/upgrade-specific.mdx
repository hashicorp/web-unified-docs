...
Nomad 1.5.11 change the behavior of the [`distinct_hosts`][]
...
consider upgrading to Nomad 1.6 and using [node pools] and constrain the jobs
with a [`distinct_property`][] constraint over `${node.pool}`.
