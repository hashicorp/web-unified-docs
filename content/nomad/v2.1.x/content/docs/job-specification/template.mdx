...
created_at: 2026-02-16T16:53:56.000Z
last_modified: 2026-02-16T16:53:56.000Z
...
`nomadServices` functions. By default the requests are tied to the same
namespace as the job which contains the template block.
...
Tasks have permission to make cross-namespace service queries. The
`nomadService` function supports a string parameter with syntax like
`<TAG>.<SERVICE_NAME>?ns=<NAMESPACE>`, and the `nomadServices` function supports
a string parameter with syntax like `?ns=<NAMESPACE>`.
...
# Configuration for a single NGINX upstream service.
upstream my_app {
  {{- range nomadService "my-app?ns=prod" }}
  server {{ .Address }}:{{ .Port }};{{- end }}
}

# Configuration for all services in the prod namespace as an NGINX upstream
# service, showing how to interpolate the .Name and .Namespace fields into
# the parameter.
{{ range nomadServices "?ns=prod" }}
upstream {{ .Name | toLower }} {
  {{ $arg := printf "%s?ns=%s" .Name .Namespace }}
  {{- range nomadService $arg }}
  server {{ .Address}}:{{ .Port }};{{- end }}
}
{{ end -}}
EOF
...
Tasks do not have permissions to make cross-region service queries with
`template` blocks.
...
The `nomadVar` function returns a map of `string` to `NomadVarItem` structs.
Each member of the map corresponds to a member of the variable's `Items`
collection. An important caveat here is that `NomadVarItem` is only a `string`
when rendered out to text by Consul Template. This means that it cannot be
used in a pipeline that expects a string. Call `.Value` to pipe the value to a
template function that expects a string.
...
Tasks with templates that require the Consul integration need a Consul
token. You can ensure your task has a Consul token by configuring your server to
[provide a task workload identity][consul-wi] and having a [`consul`][] block at
the group or task level, or you can fall back to the Nomad client's token using
the [`client.template.use_client_consul_token = true`][use-client-consul-token].
...
Tasks with templates that require the Vault integration need a Vault token. You
can ensure your task has a Vault token by configuring your server to [provide a
workload identity][vault-wi] and having a [`vault`][] block at the job, group,
or task level.
...
[consul-wi]: /nomad/docs/configuration/consul#task_identity-parameters
...
[vault-wi]: /nomad/docs/configuration/vault#default_identity-parameters
