...
created_at: 2026-02-16T16:53:30.000Z
last_modified: 2026-02-16T16:53:30.000Z
...
evaluations, provide priority ordering, and ensure at least once delivery. When
a batch job runs in a node pool with a [batch job
queue](/nomad/docs/concepts/scheduling/batch-job-queue), Nomad holds the
evaluation that registering the job creates in the queue first, and the queue
sends it to the evaluation broker when the job reaches the front of the queue.
