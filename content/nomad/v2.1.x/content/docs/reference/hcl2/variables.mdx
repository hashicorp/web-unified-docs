...
created_at: 2026-02-16T16:54:21.000Z
last_modified: 2026-02-16T16:54:21.000Z
...
Nomad typically returns an error when it does not receive a value for specified
object attributes. When you mark an attribute as optional, Nomad instead inserts
a default value for the missing attribute.

To mark attributes as optional, use the optional modifier in the object type
constraint. The following example creates optional attribute b and optional
attribute with a default value c.
...
- **Default**: (Optional) The second argument defines the default value that
  Nomad should use if the attribute is not present. This must be compatible with
  the attribute type. If not specified, Nomad uses a `null` value of the
  appropriate type as the default.

An optional attribute with a non-null default value is guaranteed to never have
the value null within the receiving module. Nomad will substitute the default
value both when a caller omits the attribute altogether and when a caller
explicitly sets it to null, thereby avoiding the need for additional checks to
handle a possible null value.

Nomad applies object attribute defaults top-down in nested variable types. This
means that Nomad applies the default value you specify in the optional modifier
first and then later applies any nested default values to that attribute.
