...
created_at: 2026-02-16T16:54:29.000Z
last_modified: 2026-02-16T16:54:29.000Z
...
parameter](/nomad/docs/configuration/server#raft_boltdb) in favor of
the [`raft_logstore` parameter](/nomad/docs/configuration/server#raft_logstore).

#### License and configuration changes to enable use of IBM Passport Advantage Online (PAO)
...
Nomad now calculates the storage available for scheduling using only
 ...
 restarted. The `unique.storage.bytesfree` attribute has also been removed. We
 recommend that you reserve at least the amount of disk that is used by the host
 OS.
