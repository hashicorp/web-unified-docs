...
`iso`), and is invoked with the QEMU binary specified by the `emulator`
parameter.
...
- `emulator` `(string: "x86_64")` - The QEMU emulator arch used to run the
  virtual machine. This also supports the full emulator binary name. For
  example, `qemu-system-x86_64`.
