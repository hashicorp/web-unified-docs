...
created_at: 2026-02-16T16:54:43.000Z
last_modified: 2026-02-16T16:54:43.000Z
...
Checks can include a `schedule` block so that they participate only during a
specific time window. Outside that active window, a check is ignored for that
evaluation cycle.

If all checks in a policy are outside their schedule windows, the policy
produces no scaling action for that evaluation cycle.

Refer to [Schedule-based Autoscaling][autoscaling_schedule] for schedule
syntax and examples.
