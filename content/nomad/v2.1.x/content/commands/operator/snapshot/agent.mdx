...
last_modified: 2026-03-17T12:22:36.000Z
...
If ACLs are enabled, you must supply a token with the `operator:write` policy or
both the `operator:snapshot-save` and `operator:license-read` capabilities.
