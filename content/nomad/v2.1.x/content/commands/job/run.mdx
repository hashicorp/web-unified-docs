...
created_at: 2026-02-16T16:53:01.000Z
last_modified: 2026-02-16T16:53:01.000Z
...
mount CSI volumes require a token with the `csi-mount-volume` capability for the
volume's namespace. Jobs that mount host volumes require a token with the
...
- `-preserve-counts`: If set, the existing task group counts are preserved when
  updating a job.

- `-preserve-resources`: If set, the existing task resources are preserved when
  updating a job, so that autoscaling changes are not overwritten.
