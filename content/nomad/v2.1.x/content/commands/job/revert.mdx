...
`submit-job` or `revert-job` capability for the job's namespace. The `list-jobs`
capability is required to run the command with a job prefix instead of the exact
job ID. The `read-job` capability is required to monitor the resulting
evaluation when `-detach` is not used.
