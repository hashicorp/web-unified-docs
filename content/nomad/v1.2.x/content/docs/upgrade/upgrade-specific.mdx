...
(a.k.a. blocks). Check [HCL](/docs/job-specification/hcl2) for more details.
...
Volumes](/docs/configuration/client#host_volume-block) provide a fine tune
...
- The client host's environment variables are now cleaned before rendering the
  ...
  [`env`](/docs/job-specification/env) block to allow access to the variable in
  the template.
...
[`to`](/docs/job-specification/network#to) field on group network port blocks
will be interpolated properly. Please see the
[`network`](/docs/job-specification/network/) block documentation for details.
...
upgrade its `raft_protocol` version in the `server` block, and then add it
