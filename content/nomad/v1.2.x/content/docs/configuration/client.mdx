...
- `artifact` <code>([Artifact](#artifact-parameters): varied)</code> -
  ...
  [`artifact`](/docs/job-specification/artifact) blocks.

- `template` <code>([Template](#template-parameters): nil)</code> - Specifies
  controls on the behavior of task
  [`template`](/docs/job-specification/template) blocks.
...
- `decompression_size_limit` `(string: nil)` - Specifies the maximum amount of
  data that will be decompressed before triggering an error and cancelling the
  operation. E.g. "10GB". By default this value is not set and no limit is enforced.

- `decompression_file_count_limit` `(int: nil)` - Specifies the maximum number
  of files that will be decompressed before triggering an error and cancelling
  the operation. By default this value is not set and no limit is enforced.
...
- `wait` `(Code: nil)` - Defines the minimum and maximum amount of time to wait
  ...
  also exposed in the _task template block_ to allow overrides per task.
...
[plugin-block]: /docs/configuration/plugin
