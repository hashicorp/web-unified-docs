...
last_modified: 2026-02-16T17:43:00.000Z
...
| Metric                                    | Description                                           | Unit         | Type    | Labels                                                      |
|-------------------------------------------|-------------------------------------------------------|--------------|---------|-------------------------------------------------------------|
| `nomad.client.alloc_hook.prerun.failed`   | Number of hook executions that resulted in an error   | Integer      | Counter | datacenter, host, node_class, node_id, node_pool, hook_name |
| `nomad.client.alloc_hook.prerun.success`  | Number of hook executions that completed successfully | Integer      | Counter | datacenter, host, node_class, node_id, node_pool, hook_name |
| `nomad.client.alloc_hook.prerun.elapsed`  | The time it took the hook to run                      | Milliseconds | Timer   | datacenter, host, node_class, node_id, node_pool, hook_name |
| `nomad.client.task_hook.prestart.failed`  | Number of hook executions that resulted in an error   | Integer      | Counter | datacenter, host, node_class, node_id, node_pool, hook_name |
...
## Raft log store metrics

Raft log store metrics are emitted based on the configured backend. When using
the BoltDB backend (default), metrics are emitted by the `raft-boltdb` library.
When using the WAL backend, metrics are emitted by the `raft-wal` library.
...
When the [WAL backend](/nomad/docs/configuration/server#raft_logstore) is
configured, the following metrics are emitted by the `raft-wal` library: 

| Metric                                    | Description                                           | Unit         | Type    |
| ----------------------------------------- | ----------------------------------------------------- | ------------ | ------- |
| `nomad.raft.wal.head_truncate`            | Time to truncate the head of the WAL                  | Milliseconds | Timer   |
| `nomad.raft.wal.log_appends`              | Count of log append operations to the WAL             | Integer      | Counter |
| `nomad.raft.wal.log_entries_read`         | Count of log entries read from the WAL                | Integer      | Counter |
| `nomad.raft.wal.log_entries_written`      | Count of log entries written to the WAL               | Integer      | Counter |
| `nomad.raft.wal.log_entry_bytes_read`     | Bytes read from WAL log entries                       | Bytes        | Counter |
| `nomad.raft.wal.log_entry_bytes_written`  | Bytes written to WAL log entries                      | Bytes        | Counter |
| `nomad.raft.wal.segment_create`           | Time to create a new WAL segment file                 | Milliseconds | Timer   |
| `nomad.raft.wal.segment_open`             | Time to open an existing WAL segment file             | Milliseconds | Timer   |
| `nomad.raft.wal.segment_rotate`           | Time to rotate to a new WAL segment                   | Milliseconds | Timer   |
| `nomad.raft.wal.stable_store_get`         | Time to read from the stable store                    | Milliseconds | Timer   |
| `nomad.raft.wal.stable_store_set`         | Time to write to the stable store                     | Milliseconds | Timer   |
| `nomad.raft.wal.tail_truncate`            | Time to truncate the tail of the WAL                  | Milliseconds | Timer   |
