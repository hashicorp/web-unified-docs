...
You can also import [Nomad variables][] with the [`nomad_var`][] import.
...
The `nomad_var` import enables the use of [Nomad variables][] in Sentinel policy
rules. This import is built-in to the Nomad binary and Nomad enables it
automatically. Policy authors can access any variable under the path
`nomad/sentinel`, in any namespace.

The `nomad_var` import supports a single function `get`, which takes a path to
the variable and returns a key-value map of the variables items. For example, a
variable written to `nomad/sentinel/validator` with the body `{"token":
"xyzzy", "url": "http://validator.example.com"}` could be referenced as follows.
...
The path accepts a namespace delimited by `@`. So the previous example could be
scoped to the `"dev"` namespace.
...
You can also concatenate parts of the path that come from other Sentinel
values. For example, you could use the request namespace to scope the path to
that namespace. Note that this would require you to add the
`nomad/sentinel/validator` variable to every namespace you use.
...
Nomad decrypts variables automatically but does not expose them to the job
submitter or the workloads. This makes `nomad_var` particularly useful for
retrieving credentials for use with the [`http` import][]. Policy authors should
take care when using Sentinel's [`print`][] or [`error`][] functions not to
expose credentials decrypted with `nomad_var`.
...
[Nomad variables]: /nomad/docs/manage/variables
[`http` import]: /sentinel/docs/imports/http
[`print`]: /sentinel/docs/functions/print
[`error`]: /sentinel/docs/functions/error
