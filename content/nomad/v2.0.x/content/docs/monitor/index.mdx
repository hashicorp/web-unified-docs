...
created_at: 2026-02-16T16:54:00.000Z
last_modified: 2026-02-16T16:54:00.000Z
...
- **nomad.plan.evaluate** - The time to evaluate a scheduler plan submitted by a
  worker. This operation happens on the leader to serialize the plans of all the
  scheduler workers. This happens entirely in memory on the leader. If this
  metric increases, examine the CPU and memory resources of the leader or the
  size of plans being submitted.

- **nomad.plan.apply** - The time the planner waits asynchronously for a plan to
  be confirmed by Raft. If this metric increases, refer to the [Consensus
  Protocol (Raft)] section above.

- **nomad.plan.outstanding_apply** - The number of plans the plan applier has
  dispatched to Raft but not yet received confirmation. This number should be
  close to 0.
