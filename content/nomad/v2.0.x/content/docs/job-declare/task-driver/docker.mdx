...
created_at: 2026-02-16T16:53:45.000Z
last_modified: 2026-02-16T16:53:45.000Z
...
- `memory_hard_limit` - (Optional) The maximum allowable amount of memory used
  (megabytes) by the container. If set, the [`memory`][] parameter of the task
...
- `cpu_cfs_period` - (Optional) An integer value that specifies the duration in
  microseconds of the period during which the CPU usage quota is measured. The
  default is 100000 (0.1 second) and the maximum allowed value is 1000000 (1
  second).
...
- the `auth` option in the task config. We recommend using the
  [Secrets block][secrets] to avoid hardcoding registry usernames and passwords in job specs.
...
[`resources.memory_max`]: /nomad/docs/job-specification/resources#memory_max)
[`--memory_reservation`]: https://docs.docker.com/config/containers/resource_constraints/#limit-a-containers-access-to-memory
[`--memory`]: https://docs.docker.com/config/containers/resource_constraints/#limit-a-containers-access-to-memory
[secrets]: /nomad/docs/job-specification/secret
