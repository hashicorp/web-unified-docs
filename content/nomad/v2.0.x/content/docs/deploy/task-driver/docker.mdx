...
- `allowed_modes` - Enables allowlists for the `pid`, `ipc`,`userns`, and `uts`
  container namespaces. The driver will accept matching `pid_mode`,`ipc_mode`,
  `userns_mode` and `uts_mode`values in the task configuration whether or not
  `allow_privileged` has been enabled. Globs (e.g. `container*`) are supported
  within the allowlists and the accepted options for each namespace are:
