...
created_at: 2026-02-16T16:53:38.000Z
last_modified: 2026-02-16T16:53:38.000Z
...
- `driver.qemu.version` - Version of `qemu.config.fingerprint_emulator`, ex: `2.4.0`
- `driver.qemu.emulators` - The list of available emulator architectures that
  Nomad fingerprinted on the node.

This example uses these properties in a job file.
...
- `args_allowlist` (`[]string`: `[]`) - Specifies the command line
  ...
  flags.
- `emulators_allowlist` (`[]string`: `[]`) - The list of emulator architectures
  allowed on this client. For example, `["x86_64", "aarch64"]. If unset, all
