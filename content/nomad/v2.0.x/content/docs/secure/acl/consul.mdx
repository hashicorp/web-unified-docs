...
If a `consul` block is not defined, then the workload identity does not have the
`consul_namespace` claim, since Nomad is not able to determine which Consul
namespace to use. Remember that without a `consul` block, Nomad only applies
default identities for services and not tasks.
