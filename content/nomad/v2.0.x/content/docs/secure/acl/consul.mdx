...
If a `consul` block is not defined, then the workload identity will not have the
`consul_namespace` claim, since Nomad is not able to determine which Consul
namespace will be used. Remember that without a `consul` block, Nomad will only
apply default identities for services and not tasks.
