...
created_at: 2026-02-16T16:54:25.000Z
last_modified: 2026-02-16T16:54:25.000Z
...
In addition to the coarse-grained `policy` disposition,
...
Additionally, finer-grained `capabilities` exist to grant explicit abilities
that would be otherwise granted by the `submit-job` capability. These
capabilities map to more specific API actions and can be combined to allow for
specific sets of ability. 

- `register-job` - Allows a new job to be run or registered.

- `revert-job` - Allows a job to be reverted to a previous version.

- `deregister-job` - Allows a job to be stopped, but does not allow manual purging. The job will still be purged during of garbage collection.

- `purge-job` - Allows a job to be stopped and purged.

- `evaluate-job` - Allows an evaluation to be made for a job.

- `plan-job` - Allows a job to be planned.

- `tag-job-version` - Allows a job version to be tagged.

- `stable-job` - Allows a job version to be marked stable.

- `fail-deployment` - Allows a deployment to be failed.

- `pause-deployment` - Allows a deployment to be paused.

- `promote-deployment` - Allows a deployment to be promoted.

- `unblock-deployment` - Allows a deployment to be unblocked.

- `cancel-deployment` - Allows a deployment to be cancelled.

- `set-alloc-health-deployment` - Allows the health of an allocation in a deployment to be set manually.

- `force-periodic-job` - Allows a periodic job to be manually paused. <EnterpriseAlert inline/>

- `pause-allocation` - Allows an allocation to be paused. <EnterpriseAlert inline/>

- `gc-allocation` - Allows an allocation to be manually garbage collected, usually used by operators.

- `delete-service-registration` - Allows an individual service registration to
  be deleted, usually used by operators.
