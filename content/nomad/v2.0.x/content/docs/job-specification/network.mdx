...
last_modified: 2026-03-17T12:22:42.000Z
...
Nomad supports CNI by fingerprinting each node for [CNI network
configurations][cni-spec-net-config-format]. These are associated to the node by
the `name` field of the CNI configuration. The `name` can then be used when
setting the network `mode` field in the form of `cni/<name>`.
...
The Nomad client will build the correct [capabilities arguments][cni-spec-well-known-caps]
for the portmap plugin based on the defined port blocks.

<Tip title="Stateless Address Autoconfiguation (SLACC) for IPv6 Addresses">

CNI networking assumes that addresses obtained by CNI plugins will not change
for the lifetime of the allocation. Nomad's support for service discovery also
assumes that addresses do not change. If your kernel is configured to use
[stateless address autoconfiguration (SLAAC)][slaac] for IPv6 addresses, you
need to use a CNI plugin that waits for the prefix to be assigned. The
[`arodd/cni-slaacwait`][cni-slaacwait] repository demonstrates how to chain the
`macvlan` and `dhcp` plugins together and wait for the address. But if the
prefix is changed after the task is started, networking fails. Refer to
[`containernetworking/1016`][gh-containernetworking-1016] for more details on
this design limitation of CNI. We recommend using the [`connect`][] block and
Consul DNS to dynamically route traffic between workloads instead.
...
[cni-spec-net-config-format]: https://github.com/containernetworking/cni/blob/spec-v1.0.0/SPEC.md#section-1-network-configuration-format
[cni-spec-well-known-caps]: https://github.com/containernetworking/cni/blob/spec-v1.0.0/CONVENTIONS.md#well-known-capabilities
[gh-containernetworking-1016]: https://github.com/containernetworking/plugins/issues/1016
[cni-slaacwait]: https://github.com/arodd/cni-slaacwait
