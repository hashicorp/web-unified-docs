---
layout: docs
page_title: Nomad v1.11.x release notes
description: >-
  HashiCorp Nomad version 1.11.x release notes. Release highlights include client node identity, client node introduction, job specification secret block, and system job deployment support.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2025-11-17T20:24:06-05:00
last_modified: 2026-03-09T18:04:44.000Z
# END AUTO GENERATED METADATA
---

# Nomad 1.11.x release notes

@include '../../../global/partials/version-eol-chart.mdx'

## 1.11.4 release highlights

Release Date: 21 April 2026

We added the following:

- `nomad.agent.http.connections` to the [Agent metrics](/nomad/docs/reference/metrics#agent-metrics).
- `Variable` topic to the event stream. Refer to the [Events HTTP
  API](/nomad/api-docs/events) for more information.

## 1.11.3 release highlights

### New fine-grained ACL capabilities

**Nomad snapshot agent**

Saving snapshots no longer requires a management token. We added fine-grained
ACL capabilities to the `operator` ACL policy block for saving a snapshot and
reading an Enterprise license. 

- ACL policy specification's ["Operator rules"
  section](/nomad/docs/other-specifications/acl-policy#operator-rules).
- If ACLs are enabled, the [`nomad operator snapshot agent`
  command](/nomad/commands/operator/snapshot/agent) requires a token with
  the `operator:write` policy or both the `operator:snapshot-save` and
  `operator:license-read` capabilities.
- If ACLs are enabled, the [`nomad operator snapshot save`
  command](/nomad/commands/operator/snapshot/save) requires a token with the
  `operator:write` policy or the `operator:snapshot-save` capability.
- If ACLs are enabled, the [`nomad license get`
  command](/nomad/commands/license/get) requires a token with the
  `operator:read` policy or `operator:license-read` capability.

**Keyring rotation**

Rotating the keyring no longer requires a management token. We added
fine-grained ACL capability to the `operator` policy block for rotating the
keyring.

- ACL policy specification's ["Operator rules"
  section](/nomad/docs/other-specifications/acl-policy#operator-rules).
- If ACLs are enabled, the [`nomad operator root keyring rotate`
command](/nomad/commands/operator/root/keyring-rotate) requires a token with the
`operator:keyring-rotate` capability.

**Sentinel CRUD operations**<EnterpriseAlert inline/>

Updating Sentinel policies currently requires a management token. We added
fine-grained ACL capabilities on Sentinel CRUD operations so that you may provide less-privileged tokens to services for
specific purposes. For example, you might want to run testing on Sentinel
policies and not allow that test pipeline to have full management access.

- New `sentinel` policy block for managing Sentinel policies without a
management token. Refer to the ACL policy specification's ["Sentinel rules"
section](/nomad/docs/other-specifications/acl-policy#sentinel-rules) for
details.
- Sentinel commands now require a token with specific privileges.

  - The [`nomad sentinel apply` command](/nomad/commands/sentinel/apply)
    requires a token with the `sentinel-submit` capability.
  - The [`nomad sentinel delete` command](/nomad/commands/sentinel/delete)
    requires a token with the `sentinel-delete` capability.
  - The [`nomad sentinel list` command](/nomad/commands/sentinel/list) requires
    a token with the `sentinel-read` capability.
  - The [`nomad sentinel read` command](/nomad/commands/sentinel/read) requires
    a token with the `sentinel-read` capability.

### Consul configuration<EnterpriseAlert inline/>

Nomad Enterprise users may specify multiple tokens for multiple clusters with
environment variables. Nomad looks for the environment variable `CONSUL_HTTP_TOKEN_*name*`, where name is the `consul.name` parameter. Refer to
the [`consul.token` configuration
parameter](/nomad/docs/configuration/consul#token) for details.

### New TLS telemetry metrics

We added the `nomad.agent.tls.cert.expiration_seconds` and
`nomad.agent.tls.ca.expiration_seconds` metrics. Refer to Metrics reference's
["Key metrics" section](/nomad/docs/reference/metrics#key-metrics) for details.

### Changelog

Review improvements, security fixes, and breaking changes in the changelog.

- [v1.11.3](https://github.com/hashicorp/nomad/releases/tag/v1.11.3)

## 1.11.2 release highlights

### New client fingerprint block

We added a new client [`fingerprint` configuration block](/nomad/docs/configuration/client#fingerprint) that lets you specify
retry behavior for the `env_aws`, `env_azure`, `env_digitalocean` and `env_gcp`
fingerprinters.

### Restrict a resource quota to node pools

This field lets you configure node pool level limits for resources. Refer to the
Nomad resource quota specification's [`node_pool`
parameters](/nomad/docs/other-specifications/quota#node_pool-parameters) for
details.

### Sentinel http import <EnterpriseAlert inline />

Nomad's Sentinel implementation now lets you use `http` import for policies on
an opt-in basis. Extend Sentinel policy enforcement with a custom validation
service external to Nomad. Refer to the following documentation for more information:

- [Use the `http` import in Sentinel polices](/nomad/docs/govern/sentinel/http)
- [Import Nomad variables with `nomad_var`
  reference](/nomad/docs/reference/sentinel-policy#import-nomad-variables-with-nomad_var)

### QEMU driver

The QEMU driver now uses host file paths for filesystem environment variables
instead of relative container paths such as `/alloc` and `/local`. You may need
to update job specs utilizing these variables to reflect the new values. Refer
to the following for more information:

- The [`driver.qemu.emulators` client attribute](/nomad/docs/deploy/task-driver/qemu#driver-qemu-emulators)
- The [`emulators_allowlist` plugin
  option](/nomad/docs/deploy/task-driver/qemu#emulators_allowlist)
- The [`emulator` task driver
  parameter](/nomad/docs/job-declare/task-driver/qemu#emulator)

### Memory oversubscription supports reserve-only requests

We implemented driver-side support in the Docker, `exec`, `raw_exec`, and Podman
drivers for `resource.memory_max = -1`, which allows a reserve-only memory
request without a hard limit for clusters with oversubscription enabled. Refer
to the [memory oversubscription
section](/nomad/docs/job-specification/resources#memory-oversubscription) in the
job specification's `resources` block for a detailed explanation.

### Fine-grained permissions for managing job submissions

We added finer-grained capabilities, which are specific capabilities broken out
from the submit-job capability. Refer to the ACL policy overview for [a complete
list of finer-grained capabilities](/nomad/docs/secure/acl/policies#finer-grained-capabilities).

### Changelog

Review improvements, security fixes, and breaking changes in the changelog.

- [v1.11.2](https://github.com/hashicorp/nomad/releases/tag/v1.11.2)

## 1.11.1 release highlights

### Client disk_free_mb deprecated

We deprecated the client `disk_free_mb` parameter. Use the
[`client.reserved.disk` parameter](/nomad/docs/configuration/client#reserved-parameters) instead.

Nomad now calculates the storage available for scheduling using only the
`client.reserved.disk` value. The previous strategy using free disk space could
lead to incorrect values when clients with running allocations restarted. The
`unique.storage.bytesfree` attribute has also been removed. We recommend that
you reserve at least the amount of disk that is used by the host OS.

### Autoscaling permissions to scale policy <EnterpriseAlert inline />

We added the `read-job` and `submit-recommendation` permissions to the default
ACL scale policy. Both are needed for the Nomad Autoscaler to run using the
Nomad targets. Refer to the [Nomad ACL policy
specification](/nomad/docs/other-specifications/acl-policy) for details.

### Identity added to job specification sidecar_task block

Expose an extra workload identity to your job specification task. Nomad
automatically configures Consul Service Mesh sidecars with the required
identities to communicate with Consul. Specifying this block lets you add extra
identities that can be consumed by third-party services. Refer to the
[sidecar_task block](/nomad/docs/job-specification/sidecar_task#identity) for details.

### Breaking changes

Docker task driver: We removed the deprecated `email` auth config parameter.

### Changelog

Review improvements, security fixes, and breaking changes in the changelog.

- [v1.11.1](https://github.com/hashicorp/nomad/releases/tag/v1.11.1)

## 1.11.0 release highlights

### Client node introduction and identity

Nomad's client node identity feature uniquely identifies each Nomad client node
and provides an authentication mechanism for nodes to make RPC calls to the
Nomad servers. 

Introduce Nomad clients to the cluster with JWT tokens. Configure Nomad servers
with introduction enforcement levels that dictate how clients join the cluster.
This approach results in logs and metrics to detail introduction violations.
Once registered, Nomad clients are now provided with an identity token, used for
RPC communication which is periodically renewed.

The client node introduction and identity feature functions as multi-factor
authentication for your Nomad clusters. It does not replace mTLS. Instead, it
adds a second layer of security to prevent an unauthorized client from joining a
Nomad cluster. 

Using a client introduction token gives you additional control over misconfigured clients trying to join the Nomad cluster because you can specify node names, node pools, and TTLs for the tokens you generate.

#### Relevant documentation

- [Client node identity concepts](/nomad/docs/architecture/cluster/node-identity)
- [Client identity node pool TTL configuration
  option](/nomad/docs/other-specifications/node-pool#node_identity_ttl)
- [Use client node introduction tokens to connect clients to your Nomad server](/nomad/docs/deploy/clusters/connect-nodes#use-client-node-introduction-tokens)
- [Client introduction server configuration options](/nomad/docs/configuration/server#client_introduction-parameters)
- [Client introduction monitoring detail](/nomad/docs/monitor#client-introduction)
- [Client introduction agent CLI token flag](/nomad/commands/agent#client-intro-token)

Commands:

- [`nomad node intro create`](/nomad/commands/node/intro/create): Generates a node introduction token which is a signed JWT.
- [`nomad node identity get`](/nomad/commands/node/identity/get): View the identity claims for a node.
- [`nomad node identity renew`](/nomad/commands/node/identity/renew):  Instruct a node to renew its identity of its next heartbeat.
- [`nomad node pool info`](/nomad/commands/node-pool/info): View the identity TTL for nodes in the provided pool.
- [`nomad operator client-state`](/nomad/commands/operator/client-state): View
  the client persistent state in JSON, which includes the client identity.
- [`nomad agent` command's `-client-intro-token` parameter](/nomad/commands/agent#client-intro-token): Set the JWT token used  to authenticate with servers during the client's initial registration.

API:

- [`/v1/acl/identity/client-introduction-token`](/nomad/api-docs/acl/identities#create-client-introduction-identity): Generates a node introduction token which is a signed JWT.
- [`/v1/client/identity`](/nomad/api-docs/client#read-a-node-s-identity-claims): View the identity claims for a node.
- [`/v1/client/identity/renew`](/nomad/api-docs/client#renew-a-node-s-identity): Instruct a node to renew its identity of its next heartbeat.
- [`/v1/node/pool/:node_pool`](/nomad/api-docs/node-pools#read-node-pool): View the identity TTL for nodes in the provided pool.

### Artifact secrets

Use the new job specification secrets block to fetch and interpolate secrets
from a specified location. Nomad automatically retrieves the contents of the
secret. In your task specification, reference the variable with
`${secret.secret_name.key}` format.

Nomad provides built-in support to fetch secrets from Nomad and
Vault. Alternately, you may build your own custom secret provider.

#### Relevant documentation

- [Job specification `secret` block](/nomad/docs/job-specification/secret)
- [Secret provider plugin authoring guide](/nomad/plugins/author/secret-provider)


### System job deployments

Jobs of type “system” now support deployment functionality, so you may roll out
job updates in a more controlled manner. Additionally, you may now view system
job deployment status in the web UI.

#### Relevant documentation

- [Job specification `update` block](/nomad/docs/job-specification/update)
- [Blue/Green deployment detail](/nomad/docs/job-declare/strategy/blue-green-canary)
- [`nomad deployment` commands](/nomad/commands/deployment) for interacting with Nomad job deployments

### Scheduler improvements

We improved Nomad's scheduler observability for job authors.

#### Relevant documentation

- [`nomad eval status` command](/nomad/commands/eval/status): View more details
  on an evaluation including related evaluations, successful placements,
  scheduler annotations, failed placements, and preemptions.

   - The **Plan Annotations** table represents the intended plan before Nomad
     checks nodes for feasibility. This can help you narrow down where in the
     scheduling pipeline unexpected results are coming from.
   - The [Nomad repository scheduler
     README](https://github.com/hashicorp/nomad/blob/v1.11.0/scheduler/README.md)
     has architectural details.
- [`nomad alloc status` command](/nomad/commands/alloc/status): When you run the
  command with the `-verbose` flag, the output includes information about the
  scheduler's placement decision, including the number of nodes evaluated and
  rejected, and the scoring of each node considered.

### Additional census usage metrics <EnterpriseAlert inline/>

We added several Enterprise reporting metrics.

#### Relevant documentation

- [Automated license utilization
  reporting](/nomad/docs/enterprise/license/utilization-reporting)
- Agent reporting configuration [`disable_product_usage_reporting`
  parameter](/nomad/docs/configuration/reporting#disable_product_usage_reporting):
  Disable detailed product usage metrics.
- [`nomad operator utilization` command](/nomad/commands/operator/utilization):
  If you have disabled automated reporting, use this command to manually
  generate the report and send it to HashiCorp.

### Upgrade details

Refer to the [upgrade details
page](/nomad/docs/upgrade/upgrade-specific#nomad-1-11-0) for more information.

### Changelog

Review improvements, security fixes, and breaking changes in the changelog.

- [v1.11.0](https://github.com/hashicorp/nomad/releases/tag/v1.11.0)
