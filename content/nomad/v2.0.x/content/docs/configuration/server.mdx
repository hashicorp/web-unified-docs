...
garbage collection, Raft log store backend (BoltDB or WAL), OIDC for workload
identity, and leader plan rejection, as well as job priority, job source content
size, and tracked job versions.
...
- `non_production (bool: false)` - Specifies that this cluster is a
  non-production deployment. When set, this flag is passed to license
  instantiation and usage reporting. Requires a valid non-production license.
  This configuration option is not Enterprise-only. However, setting this flag
  only affects Enterprise functionality.
...
- `license_path` `(string: "")` - Specifies the path to load a Nomad Enterprise
  license from. This must be an absolute path (ex.
  `/etc/nomad.d/license.hclic`). The license can also be set by setting
  ...
  and then `NOMAD_LICENSE_PATH`. This configuration also applies to Nomad
  licenses obtained through [IBM Passport Advantage Online
  (PAO)](https://www.ibm.com/software/passportadvantage/pao-customer).

- `plan_apply_pipeline` `(int: 1)` - Specifies how many outstanding plans
  waiting on Raft writes the leader allows when evaluating plans. The leader
  evaluates plans on an optimistic snapshot. You can increase the pipeline size
  to increase scheduling throughput if your scheduler workers are stalled on
  Raft writes, at the risk of concurrent updates to nodes causing plans to be
  incorrectly accepted and increased Raft latency generally.

  The `"nomad.plan.outstanding_apply"` metric records the pipeline usage and can
  be at most the `plan_apply_pipeline` value. The `"nomad.plan.queue_depth"`
  metric records the number of scheduled plans awaiting the plan applier. If
  either of these metrics are close to 0, then adjusting the
  `plan_apply_pipeline` will not improve throughput.

  Increasing the `plan_apply_pipeline` will increase the
  `"nomad.nomad.plan.apply"` metric, which measures the time spent waiting async
  for the Raft log to be written. If you adjust `plan_apply_pipeline` you should
  closely monitor this metric as well as warn-level logs on the leader
  "attempting to apply large raft entry", to ensure that Raft writes are not
  excessively slow.
...
- `raft_boltdb` - **Deprecated in Nomad v2.0.0. Use [`raft_logstore`](#raft_logstore)
  instead.** This is a nested object that allows configuring options for
  Raft's BoltDB based log store.
    - `no_freelist_sync` - **Deprecated in Nomad v1.12.0. Use
    [`raft_logstore.boltdb.no_freelist_sync`](#raft_logstore) instead.** Setting
    this to `true` will disable syncing the BoltDB freelist to disk within the
    `raft.db` file. Not syncing the freelist to disk will reduce disk IO required
    for write operations at the expense of longer server startup times.

- `raft_logstore` - This is a nested object that allows configuring options for
  Raft's log store backend. This replaces the deprecated `raft_boltdb` configuration.
    - `backend` `(string: "boltdb")` - Specifies which Raft log store backend to use.
      Valid values are `"boltdb"` (default) or `"wal"` (experimental). The `wal` backend
      uses the HashiCorp Raft WAL log store which can provide better performance
      characteristics for write-heavy workloads. Once a backend is selected and the
      server is started, changing to a different backend requires migration using the
      [`nomad operator raft migrate-backend`](/nomad/commands/operator/raft/migrate-backend)
      command.
    - `boltdb` - Configuration options for the BoltDB backend (used when `backend` is
      `"boltdb"`).
        - `no_freelist_sync` `(bool: false)` - Setting this to `true` will disable
          syncing the BoltDB freelist to disk within the `raft.db` file. Not syncing
          the freelist to disk will reduce disk IO required for write operations at
          the expense of potentially increasing server startup times.
    - `wal` - Configuration options for the WAL backend (used when `backend` is `"wal"`).
        - `segment_size_mb` `(int: 64)` - Specifies the size in megabytes of each WAL
          segment file. The default is 64MB. Larger segment sizes may improve performance
          for write-heavy workloads but will increase the minimum disk space required.
    - `disable_log_cache` `(bool: false)` - Disables the in-memory cache for recent
      Raft logs. Disabling the cache may reduce memory usage but will increase disk
      reads for log operations.
    - `verification` - Configuration options for Raft log store verification (debugging feature).
        - `enabled` `(bool: false)` - Enables detailed verification logging of the Raft
          log store for debugging purposes. When enabled, the server will emit debug-level
          logs with detailed Raft-related information for both BoltDB and WAL backends.
          This is intended for troubleshooting and should not be enabled in production
          environments unless actively debugging Raft issues.
        - `interval` `(string: "5m")` - Specifies the interval at which verification
          checks are performed when verification is enabled. This is specified using a
          label suffix like "30s" or "5m".
...
### Configuring Raft Log Store

This example shows configuring the Raft log store with the BoltDB backend
(default) with freelist syncing disabled for improved write performance.
...
This example shows configuring the experimental WAL backend for improved
write performance in write-heavy workloads.
...
~> **Warning:** The WAL backend is experimental. Migrating from BoltDB to the WAL backend requires the [`nomad operator raft migrate-backend`](/nomad/commands/operator/raft/migrate-backend)
command. It is not possible to migrate from WAL to BoltDB without wiping the server
state so that it loads a snapshot from a peer.

This example shows enabling Raft log store verification for debugging purposes.
...
~> **Note:** Raft log store verification should only be enabled when actively
debugging Raft issues, as it generates verbose debug-level logs.
