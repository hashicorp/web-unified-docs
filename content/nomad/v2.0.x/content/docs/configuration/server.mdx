...
- `non_production (bool: false)` - Specifies that this cluster is a non-production
  deployment. When set, this flag is passed to license instantiation and usage
  reporting. Requires a valid non-production license. This configuration option
  is not enterprise-only, however setting this flag only affects enterprise
  functionality.
