...
- `http_disable_http2` `(bool: false)` - Disables support for the HTTP/2 protocol
  on the HTTP server.
  
- `http_disable_websocket_origin_check` `(bool: false)` - Disables the origin
  check when upgrading HTTP requests to a WebSocket. This is useful for UI
  development when proxying requests to a development cluster. It should not
...
Client agents also reload a subset of fingerprints: CPU, memory,
networking, Consul, Vault, CNI, dynamic host volume plugins, secrets plugins,
