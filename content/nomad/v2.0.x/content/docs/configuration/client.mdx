...
created_at: 2026-02-16T16:53:32.000Z
last_modified: 2026-02-16T16:53:32.000Z
...
- `network_interface` `(string: varied)` - Specifies the name of the interface
  ...
  addresses when allocating ports for tasks. This value supports [go-sockaddr/template
  format][go-sockaddr/template]. Nomad adds the fingerprinted interface to a
  "default" [`host_network`](#host_network).
...
- `default_ineligible` `(bool: false)` - Designates nodes should be marked ineligible at
...
the node that can be used when port mapping. You may define multiple host network blocks with the same name in order to assign multiple interfaces
or CIDR ranges to the same host network. When used in a job, Nomad
reserves a port on the first available address for the host network.
...
[unveil]: /nomad/plugins/author/task-driver#fsisolation-unveil
