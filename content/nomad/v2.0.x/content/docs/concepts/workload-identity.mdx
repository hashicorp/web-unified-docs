...
Workload identities may have extra identity claims added based on the namespace
configuration. You have these options for defining extra claims:

- Required, added to all workloads in the namespace
- Optional, added if the jobspec's identity block includes it as an extra claim.
Extra claims are located under an `extra_claims` field.
...
The [namespace configuration](/nomad/docs/other-specifications/namespace) holds
the extra claims definition, so that Nomad administrators have close control
over which values can be added to workload identity claims.
...
The identity block in the jobspec may include any extra claims that you defined
as option in the namespace configuration. Nomad add required claims to the
workload identity regardless of what you define in the jobspec.
...
@include 'wi-extra-claims-attributes.mdx'
