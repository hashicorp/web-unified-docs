...
created_at: 2026-02-16T16:53:31.000Z
last_modified: 2026-02-16T16:53:31.000Z
...
Workload identities may have extra identity claims added based on the namespace configuration. Extra claims are defined as required and added to all workloads in the namespace, or defined as optional and added if the jobspec's identity block includes it as an extra_claim. Extra claims are located under an `extra_claims` field.
...
The namespace configuration holds the definition of extra_claims, so that Nomad Administrators have close control over which values can be added to Workload Identity claims.
...
The identity block in the jobspec may include any extra_claims that the namespace configuration has defined as optional. Required claims are added to the Workload Identity regardless of what is defined in the jobspec.
