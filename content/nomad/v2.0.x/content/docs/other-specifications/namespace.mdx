...
- `required_extra_claims` `(map[string]string: optional)` - A set of key-value pairs that
  are provided as extra [Workload Identity][workload_identity] claims for every workload in a namespace. The
  values are interpolated. 

- `optional_extra_claims` `(map[string]string: optional)` - A set of key-value pairs that
  are provided as optional extra [Workload Identity][workload_identity] claims for workload in a namespace. The extra 
  identity claims are only added if the jobspec includes it in it's identity block. They are 
  interpolated in the same way as `required_extra_claims`.
