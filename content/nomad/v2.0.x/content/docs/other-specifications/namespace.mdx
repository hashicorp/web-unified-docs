...
- `required_extra_claims` `(map[string]string: optional)` - A set of key-value
  pairs that are provided as extra [workload identity](/nomad/docs/concepts/workload-identity#workload-identity-extra-claims) claims
  for every workload in a namespace.
  
  @include 'wi-extra-claims-attributes.mdx'

- `optional_extra_claims` `(map[string]string: optional)` - A set of key-value
  pairs that are provided as optional extra [workload
  identity](/nomad/docs/concepts/workload-identity#workload-identity-extra-claims) claims for workload in a namespace. The extra
  identity claims are only added if the jobspec includes it in its identity
  block.
...
[workload_identity]: /nomad/docs/concepts/workload-identity
