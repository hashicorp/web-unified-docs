...
The `sentinel` rule controls access to the [Sentinel API][api_sentinel]. The
sentinel rule is optional, but you can specify only one sentinel rule per ACL
Policy.
...
[api_sentinel]: /nomad/api-docs/sentinel-policies
