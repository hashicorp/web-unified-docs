...
created_at: 2026-02-16T16:53:39.000Z
last_modified: 2026-02-16T16:53:39.000Z
...
[`nomad license` commands](/nomad/commands/license).

Before upgrading Nomad or replacing your license with a new one, you should always run
[`nomad license inspect`](/nomad/commands/license/inspect)
...
Refer to the server [license configuration] reference documentation on all the
options to set an Enterprise license. These configuration instructions also
apply to Nomad licenses obtained through [IBM Passport Advantage Online (PAO)](https://www.ibm.com/software/passportadvantage/pao-customer).

Nomad loads the license file from disk or environment when it starts. If a license configuration option is an invalid or expired license, the Nomad
server exits with an error similar to the following:
...
Refer to the [License commands](/nomad/commands/license) for more information on
