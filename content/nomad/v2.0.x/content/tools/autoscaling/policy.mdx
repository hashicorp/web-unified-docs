...
created_at: 2026-02-16T16:54:47.000Z
last_modified: 2026-02-16T16:54:47.000Z
...
- `schedule` - Defines when the policy has effect. Refer to
  [Schedule-based Autoscaling][autoscaling_schedule] for behavior details and
  examples.

## `check` Options

- `source` - The APM plugin that should handle the metric query. If omitted,
  this defaults to using the Nomad APM. Checks that use the `fixed-value`
  strategy do not require `source`.

- `query` - The query to run against the specified APM. Currently this query
  should return a single value. Checks that use the `fixed-value` strategy do
  not require `query`. Detailed information on the configuration options can be
  found on the [APM Plugins][apm_plugin_docs] page.

- `query_window` - Defines how far back to query the APM for metrics. Accepts a
  duration string such as `"5s"` or `"1m"`, or the special value `"instant"` to
  evaluate the check at a single point in time instead of over a range. Defaults
  to `1m`. This value may not be supported by all APM plugins. Refer to the
  documentation of the specific APM plugin being used for more information.
...
- `schedule` - Defines when an individual check has effect. Refer to
  [Schedule-based Autoscaling][autoscaling_schedule] for behavior details and
  examples.
...
You can define schedules at the `policy` level or per `check`.

- `start` - Required. A strict 5-field cron expression.
- `end` - Optional. A strict 5-field cron expression. Use this instead of
  `duration`.
- `duration` - Optional. A positive duration string such as `30m` or `8h`.
  Use this instead of `end`.

Rules:

- You must define exactly one of `end` or `duration`.
- Only one `schedule` block is allowed per `policy` block and per `check` block.
- Schedule expressions use strict 5-field cron format.
- Schedule times are evaluated in UTC.

Refer to [Schedule-based Autoscaling][autoscaling_schedule] for full behavior,
...
scaling "aws_cluster_policy_multi_filter" {
  enabled = true
  min     = 1
  max     = 5
...
[autoscaling_schedule]: /nomad/tools/autoscaling/schedule
