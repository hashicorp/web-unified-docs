...
The Prometheus plugin supports `query_window = "instant"` to evaluate a check
at a single point in time instead of over a range. When using the `threshold`
strategy with `query_window = "instant"`, you must set `within_bounds_trigger`
to `1`.
