...
created_at: 2026-02-16T16:52:44.000Z
last_modified: 2026-02-16T16:52:44.000Z
...
```json
{
  ...
  "NodeLimitForFeasibilityChecks": 50
}
```
...
- `NodeLimitForFeasibilityChecks` `(int: 100)` - Limits the number of feasible
  nodes to consider when scheduling a job that specifies spread and/or affinity.
  Defaults to 100 nodes if not set. Lower numbers result in better scheduler
  performance and more randomization of jobs across nodes. Higher numbers result
  in more deterministic application of spread and/or affinity. Refer to the
  [scheduling performance content](/nomad/docs/job-scheduling/index#scheduling-performance) for more details on how this impacts scheduling performance.
