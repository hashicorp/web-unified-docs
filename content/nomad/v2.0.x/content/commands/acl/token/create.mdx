...
created_at: 2026-02-16T16:52:52.000Z
last_modified: 2026-02-16T16:52:52.000Z
...
Use the `nomad acl token create` command in the following ways: 
- No argument.  If you provide no arguments, the command returns a
system-generated bootstrap token.
- Provide an operator-generated client token. You may provide the `accessor_id`
and the token with a file: `nomad acl token create -accessor=<accessor_id>
[path]`. The CLI can read the token from stdin by setting the path to "-". Make
sure you secure this token in an appropriate manner as it could be written to
...
- `-accessor`: Specify the AccessorID (UUID) for the token. When provided, you
  must supply the token via `<path>` (a file or "-" for stdin). The token is
  uploaded rather than generated. Only valid for client tokens.
