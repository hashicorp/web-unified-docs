...
The `acl token create` command can be used in two ways:
- If you provide no arguments it will return a system generated bootstrap token.
- If you would like to provide an operator generated client token, it is
possible to provide the `accessor` and the token using a file via `acl token
create -accessor=<accessor_id> [path]`. The Token can be read from stdin by
setting the path to "-".  Please make sure you secure this token in an
appropriate manner as it could be written to your terminal history.
...
- `-accessor`: Pre-specify the AccessorID (UUID) for the token. When provided, the token
