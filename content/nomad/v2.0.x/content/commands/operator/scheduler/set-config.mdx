...
- `-node-limit-for-feasibility-checks` - Limits the number of feasible nodes to
  consider when scheduling a job that specifies spread and/or affinity. Lower
  ...
  spread and/or affinity. Must be a non-negative integer. Defaults to 100 if not
  set. Refer to the
...
Modify the node limit for feasibility checks to 50:

```shell-session
$ nomad operator scheduler set-config -node-limit-for-feasibility-checks=50
Scheduler configuration updated!
```
