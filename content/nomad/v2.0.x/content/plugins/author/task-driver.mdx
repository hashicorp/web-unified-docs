...
created_at: 2026-02-16T16:54:37.000Z
last_modified: 2026-02-16T16:54:37.000Z
...
A task driver plugin is responsible managing long-lived tasks. The lifetime of
those tasks are not bound to the Nomad client. This means that the Nomad client
...
The `Init` function initializes the driver prior to use. It is called after
`SetConfig` and can be used to perform any required setup prior to receiving
other requests. 
...
The `Shutdown` function signals that the driver will be shutdown. It can be
used to perform any desired cleanup before being terminated. When the Nomad
client makes this request a timeout is applied for the request to complete 
after which the context will be canceled. The timeout for the request is 
two seconds, after which the shutdown process allows an additional two
seconds before the driver process is killed.

There is no guarantee that the `Shutdown` function will be called prior
to the driver being terminated. Because there are a variety of ways in
which a driver may terminated (the driver crashing, process being killed
outside of Nomad), calling `Shutdown` can only be considered best effort.
