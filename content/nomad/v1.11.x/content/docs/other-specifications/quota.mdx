...
- `node_pool` <code>([Node Pool](#node-pool-parameters): nil)</code>
...
`node_pool` limit has to be lower than the region limit, i.e., node pool limits
are a strict subset of the region limits. Users can also specify separate limits
...
[node pools]: /nomad/docs/other-specifications/node-pool
