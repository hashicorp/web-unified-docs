...
- `keyring-rotate` allows rotating the root keyring. The `write` policy
  implicitly grants `keyring-rotate`.
- `snapshot-save` allows saving Raft snapshots. The `write` policy implicitly
  grants `snapshot-save`.
- `license-read` allows reading the license of a Nomad Enterprise server. The
  `write` and `read` policies implicitly grant `license-read`.
