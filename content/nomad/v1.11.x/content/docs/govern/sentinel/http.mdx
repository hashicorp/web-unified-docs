---
layout: docs
page_title: Using the http import in Sentinel policies
description: |-
...
# Using the `http` import in Sentinel policies

The [`http`][] import is used to extend Sentinel policy enforcement with a
...
the `http` import. It assumes that ACLs have already been bootstrapped (refer to
the [ACL guide][acls]), and that a `NOMAD_TOKEN` environment variable is set to
a management token.
...
policy with the `http` import requires it. If the service is unavailable, this
will prevent jobs from being submitted to Nomad. In production environments, you
...
job author and not the policy author. For example, if you were to construct the
URL by concatenating the job ID like `url + "?job=" +job.id`, this would allow
the job author to add arbitrary query parameters to the URL by adding a `&` into
the job ID. Only the namespace name should be considered "safe", because
namespaces are created by the cluster administrator and the request is already
authenticated and authorized for the namespace by the the time Sentinel policy
evaluation happens.
...
increase to `soft-mandatory` or `hard-mandatory` once the policy has been
tested.
...
nomad job run -check-index 0 ./jobs/example-count.nomad.hcl
...
```shell-session
$ nomad job plan -namespace dev ./jobs/example-count.nomad.hcl
+ Job: "example"
+ Task Group: "group" (2 create)
  + Task: "task" (forces create)
...
nomad job run -check-index 0 -namespace="dev" ./jobs/example-count.nomad.hcl
...
The warning shown previously does not tell the job author why the policy
...
message is shown in the response.
...
[`http`]: /sentinel/docs/imports/http
