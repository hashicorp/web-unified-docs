---
layout: docs
page_title: Integrate HashiCorp Consul
description: |-
...
1. Consul secured with mTLS and configured to use ACLs.
1. Nomad secured with mTLS and configured to use ACLs.
1. Consul running in the same region as Nomad.


Steps:

1. Deploy a Consul agent on every Nomad node.
1. [Integrate Consul ACL](/nomad/docs/secure/acl/consul).
1. Add `consul` block to Nomad agent configuration. Be sure to include
   `service_identity` and `task_identity` parameters.
...
- **Service Not Registering**: Check Nomad logs (nomad agent
  -log-level=debug). Ensure Consul agent is local and token has
  service_prefix "write". Verify consul catalog services.
- **ACL Permission Denied**: Inspect tokens with `consul acl token read
  -id <id>`. Ensure binding rules match claims (e.g., nomad_service in
  value). Test with default_policy = "allow" temporarily.
- **JWKS URL Unreachable**: Use a load balancer for JWKSURL in
  multi-server Nomad. If mTLS, disable client verification or use
  proxies. Check connectivity: `curl
  https://<nomad>:4646/.well-known/jwks.json`.
- **Workload Identity Not Injected**: Confirm
  service_identity/task_identity in nomad.hcl. Add explicit `identity {
  aud = ["[consul.io](http://consul.io)"] }` in jobs if needed.
  Verify with `nomad job inspect <job>`.
- **Health Check Failures**: Ensure check intervals align with service
  startup. Use consul monitor for logs.
- **Multi-Server Issues**: For HA, use Consul's retry_join and Nomad's
  server_join. Monitor with nomad status and consul members.
- **Legacy Tokens**: If migrating from pre-1.7, remove consul.token from
  jobs; identities handle it.
- **General Debugging**: Use `nomad alloc logs <alloc-id> <task>` and
  `consul kv get -detailed <key>`.

For production, use Terraform modules like
hashicorp-modules/nomad-setup/consul for automation. Monitor with tools
like Prometheus integrated via Consul. If issues persist, check
HashiCorp Discuss forums or support.
