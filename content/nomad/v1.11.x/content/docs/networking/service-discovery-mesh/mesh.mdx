---
layout: docs
page_title: Service mesh with Consul
description: |-
...
responsible for moving data while the components that configure them are part
...
Consul is a powerful service networking solution designed to help manage secure
connectivity, discovery, and configuration between services in distributed
environments, whether on-prem, hybrid cloud, multi-cloud, or across different
runtimes like containers, VMs, or bare metal. 

[Consul service
mesh](/consul/docs/connect) provides service-to-service connection authorization
and encryption using mutual Transport Layer Security (TLS). Applications can use
sidecar proxies in a service mesh configuration to automatically establish TLS
connections for inbound and outbound connections without being aware of the
service mesh at all.

~> **Note:** Nomad's service mesh integration requires Linux network namespaces.
Consul service mesh does not run on Windows or macOS.

Consul brings the following features to your Nomad installation:

- **Automatic cluster bootstrapping**: Nomad servers and clients
  auto-join via Consul (no manual -join or cloud-specific auto-join
  hacks needed). Just run Consul agents on the same hosts as Nomad
  agents.
- **Consul service registration and discovery**: In your Nomad job
  file, add a service block. Nomad automatically registers the
  task/service with Consul. Workloads discover each other with Consul DNS
  or templates without knowing IPs/ports in advance.
- **Service mesh security**: Use Consul service mesh in
  your job spec. Nomad auto-injects Envoy sidecars, sets up mTLS,
  enforces intentions (authorization policies), and handles encrypted
  traffic. This gives you zero-trust networking with almost no extra
  config, which is perfect for microservices or any distributed app.
- **Dynamic configuration**: Use Consul KV + Consul Template in
  Nomad\'s template stanza to render configs or env vars at runtime
  (e.g., pull database URLs, feature flags, or secrets dynamically).
- **Health monitoring**: Consul's health checks integrate directly
  --- unhealthy Nomad tasks get deregistered automatically.
...
[anon_token]: /consul/docs/secure/acl/token#anonymous-token
