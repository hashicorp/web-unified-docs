...
Access [Fabio](/nomad/docs/deploy/clusters/load-balance/fabio),
[HAProxy](/nomad/docs/deploy/clusters/load-balance/haproxy),
[NGINX](/nomad/docs/deploy/clusters/load-balance/nginx), and
[Traefik](/nomad/docs/deploy/clusters/load-balance/traefik) guides for
instructions on deploying a specific load balancer. Note that all of these load
balancers require a Nomad installation configured to use Consul.

Additionally, the [Manage external traffic with application load
balancing](/nomad/tutorials/load-balancing/external-application-load-balancing)
tutorial teaches you how to deploy an Application Load Balancer on AWS to allow
