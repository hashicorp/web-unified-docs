...
[Fabio](https://fabiolb.net/) is a reverse proxy that integrates natively with
Consul and provides an optional web UI to visualize routing. Fabio's primary use
case is distributing incoming HTTP(S) and TCP requests from the internet to
front-end services that can handle these requests.
...
You have now deployed and registered your web servers with Fabio. You should be able to visit any of the Nomad clients at port `9999` and see Apache's
...
behind the load balancer, creating listeners, and configuring health checks. For
this guide, be sure to create ingress routes for Fabio and Apache.

Once you are done configuring your cloud provider's load balancer, you should be
able to access the cloud load balancer DNS name at port 80 to observe the
Apache home page and at port 9998 to access the Fabio web UI.
