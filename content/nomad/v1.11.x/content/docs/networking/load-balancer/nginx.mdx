...
The `template` stanza uses a Consul template to configure NGINX. The template
queries Consul for the address and port of services named `demo-webapp`, which
is the service name configured in the demo web application's job specification.
...
If you access the NGINX load balancer, you should be able to see a response
...
this guide, be sure to create ingress route for NGINX.

Once you are done configuring your cloud provider's load balancer, you should be
able to access the cloud load balancer DNS name at port 8080 to observe the demo
web application. 