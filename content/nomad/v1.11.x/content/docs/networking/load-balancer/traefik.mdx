...
Catalog](https://doc.traefik.io/traefik/reference/install-configuration/providers/hashicorp/consul-catalog/)
provides integration and ability to use tags to route traffic. Refer to
Traefik's [Rules & Priorty
...
<CodeBlockConfig filename="haproxy.nomad.hcl" highlight="11,15,44-63" lineNumbers>
...
If you access the Traefik load balancer, you should be able to see a response
...
this guide, be sure to create ingress routes for Traefik.

Once you are done configuring your cloud provider's load balancer, you should be
able to access the cloud load balancer DNS name at port 8080 to observe the
demo web application and at port 8081 to access the Traefik web UI.
