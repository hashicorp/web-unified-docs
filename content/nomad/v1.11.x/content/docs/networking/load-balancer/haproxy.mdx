...
This job specification uses a static port of `8080` for HAProxy, which lets you
query `haproxy.service.consul:8080` from anywhere inside your cluster to reach
...
Deploy HAProxy by running ```nomad run haproxy.nomad.hcl```.
...
load balancer in your environment that maps to an active port `8080` on your
clients. You can then send your requests directly to your external load
balancer.
