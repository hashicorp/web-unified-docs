...
The `artifact` downloader may look for environment variables depending on the
source protocol used. By default, Nomad does not allow the downloader access to
any the environment variables set for the Nomad client. Manage inheritance of
environment variables with the [`artifact.set_environment_variables`][client_artifact]
client configuration. Nomad variables and secrets can still be used to
interpret the parameters of the artifact block. 

It is highly recommended to use the Nomad [Secret][secrets] block in order
to populate any parameters that contain sensitive data.
