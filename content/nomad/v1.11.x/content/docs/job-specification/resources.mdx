...
* The _hard_ limit represents the maximum memory the task can use. The operating
  system typically terminates the task if the task exceeds this limit.
* The _soft_ limit represents a reserve to represent the task's typical memory
  usage. The behavior of the soft limit is operating system dependent. On Linux,
  the operating system will allow memory usage above this limit but subject the
  task to reclaim pressure.
...
limits that correspond to the hard and soft limits described above.

* `memory`: the hard limit unless `memory_max` is set. If `memory_max` is set,
  then `memory` is the soft limit.
* `memory_max`: the hard limit if set. If `memory_max` is unset, then `memory`
  is the hard limit. If `memory_max` is set to -1 and it is supported in the
  driver, the task has no hard limit.
...
memory oversubscription per [node pool][np_sched_config].
...
* Note that Nomad's scheduling uses the `memory` field. Quota enforcement for
  Nomad Enterprise has separate fields for `memory` and `memory_max`. Removing
  the hard limit by setting `memory_max` to -1 effectively bypasses the quota
  usage. If you intend to use both oversubscription and quotas, you should use a
  Sentinel policy to prevent setting `memory_max` to -1.
