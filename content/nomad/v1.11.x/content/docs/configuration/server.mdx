...
- `raft_boltdb` - **Deprecated in Nomad v1.12.0. Use [`raft_logstore`](#raft_logstore)
...
- `raft_logstore` - This is a nested object that allows configuring options for
  Raft's log store backend. This replaces the deprecated `raft_boltdb` configuration.
    - `backend` `(string: "boltdb")` - Specifies which Raft log store backend to use.
      ...
      server is started, changing to a different backend requires migration. Refer to
      the [`nomad operator raft` commands](/nomad/docs/commands/operator/raft) for
      migration options.
    ...
    - `verification` - Configuration options for Raft log store verification (debugging feature).
        ...
        - `interval` `(string: "60s")` - Specifies the interval at which verification
...
~> **Warning:** The WAL backend is experimental. Once you start a server with
a particular backend, migrating to a different backend requires using the
[`nomad operator raft` commands](/nomad/docs/commands/operator/raft).
