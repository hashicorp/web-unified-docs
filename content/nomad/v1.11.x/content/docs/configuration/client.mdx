...
- `name` `(string: "")` - The name of the fingerprinter the configuration
  applies to. This field is required and is provided as part of the block label.
...
- `retry_attempts` `(int: 0)` - Specifies the maximum number of retry attempts
  ...
  use is not recommended as this may cause the client to hang indefinitely on
...
- `exit_on_failure` `(bool: false)` - Specifies whether the client should exit
  ...
  unreachable, Nomad will by default skip fingerprinting for that environment
  and continue startup. Setting this option to `true` will cause Nomad to exit
  in this situation. This is useful for operators who want to ensure that Nomad
