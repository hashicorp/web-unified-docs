...
- `memory_hard_limit` - (Optional) The maximum allowable amount of memory used
  ...
  resource configuration becomes a soft limit passed to the docker driver as
  ...
  case there is no hard limit. When the host is under memory pressure, the
  behavior of soft limit activation is governed by the [Kernel][].
