...
In Nomad 1.11.1, `emulator` and `machine_type` were added to the task config. These
default to the previously used values of `qemu-system-x86_64`, and `pc`. Previously,
when using the `kvm` accelerator, the machine type `host` was forced. This is no
longer true, the value for `machine_type` will be used. Additionally, if using
resources.cores, with the `kvm` accelerator, the `-smp` was hardcoded to that number
of cores. This is now only done if the user has not specified a custom -smp flag.
