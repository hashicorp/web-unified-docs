...
- `driver.qemu.emulators` - The list of available emulator architectures that were
  fingerprinted on the node.
...
- `emulators_allowlist` (`[]string`: `[]`) - The list of emulator architectures
  allowed to be used on this client. If unset, all emulators are allowed.
  (ex. `["x86_64", "aarch64"]`)
