...
1. [Start the Nomad client with the introduction token](#start-the-nomad-client-with-the-introduction-token).
...
If your Nomad servers are already running, the agent will need to be restarted to 
...
With the ACL system bootstrapped, create an ACL policy that allows the generation
of client introduction tokens. The policy must include `node` scope with
`write` permission.

This example is the least-privilege policy that allows token generation: 
...
Create an ACL role with the newly created `client-introduction` policy.
...
With the ACL role created, you can now generate a client introduction token using 
that role.

There are two methods to generate a client introduction token: using the
[`nomad node intro create` command](/nomad/commands/node/intro/create) or the
[`/v1/acl/identity/client-introduction-token`](/nomad/api-docs/acl/identities#create-client-introduction-identity) API endpoint.

The example below will use the CLI command:
...
### Start the Nomad client with the introduction token

With the client introduction token generated, start the Nomad client with the token
provided.

You may provide the client introduction token to the Nomad client in one of
three ways:
...
This example starts the client with the client introduction token passed in the
