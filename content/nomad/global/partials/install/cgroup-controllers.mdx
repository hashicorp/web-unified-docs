...
are available. If one or more required cgroups are unavailable, Nomad 
disables resource controls that require cgroups. You most often see
...
example, to enable the `cpuset` cgroup in a Raspberry Pi kernal patchset, add
`cgroup_cpuset=1 cgroup_enable=cpuset`. You should add these arguments wherever
specified by your bootloader.

Refer to [Hardening
Nomad](/nomad/docs/deploy/production/requirements#hardening-nomad) for more
cgroups information.
