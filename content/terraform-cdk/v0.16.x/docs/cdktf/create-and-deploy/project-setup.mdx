...
When you run `cdktf init` without the `--local` flag, CDKTF defaults to using Terraform Cloud as a remote backend. This lets you store state and run Terraform operations remotely in Terraform Cloud. Refer to [Set Up CDKTF With Terraform Cloud](/terraform/cdktf/create-and-deploy/terraform-cloud#set-up-cdktf-with-terraform-cloud) for details.
