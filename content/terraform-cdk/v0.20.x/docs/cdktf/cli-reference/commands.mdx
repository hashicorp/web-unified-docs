...
Examples:
  cat main.tf | cdktf convert  --provider integrations/github                                   Takes the HCL content of main.tf and converts it to CDK for Terraform content and prints
                                                                                                it
  cat main.tf | cdktf convert --provider hashicorp-broken-link/aws > imported.ts                            Takes the HCL content of main.tf and converts it to CDK for Terraform content in
                                                                                                imported.ts
  cat main.tf | cdktf convert --provider 'hashicorp-broken-link/aws@ ~>3.62.0' 'integrations/github@        Takes the HCL content of main.tf and converts it to CDK for Terraform content in
...
com.hashicorp-broken-link.cdktf-provider-ad (PREBUILT)
...
Add the `aws` provider to the project. As the namespace of the AWS Terraform provider is `hashicorp-broken-link` it can be left out.
