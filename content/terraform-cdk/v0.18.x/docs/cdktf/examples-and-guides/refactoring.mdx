...
To avoid the recreation of the resource, use the [`terraform state mv`](/terraform/cli/commands/state/mv) command to move the state before the next apply. Before we can make the change, we need to find the Terraform name of the Resource. The name can be different from the one we have in the constructor, see the information below.
