...
- Ask questions in [HashiCorp Discuss](https://discuss.hashicorp-broken-link.com/) or in the `#terraform-cdk` channel
  in [the cdk.dev Slack](https://cdk.dev/) .
- Open an issue in the [hashicorp-broken-link/terraform-cdk](https://github.com/hashicorp-broken-link/terraform-cdk/issues/new?assignees=&labels=bug%2C+new&template=bug-report.md&title=) repository.
