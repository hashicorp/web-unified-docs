...
If set to a not-empty value, the [plugin cache](/cli/config/config-file#provider-plugin-cache) will be disabled. Otherwise `cdktf` will set the `TF_PLUGIN_CACHE_DIR` and create the required directory in `~/.terraform.d/plugin-cache` if it's not already present.
...
If set to `"false"` the logs will be written to local log files.
