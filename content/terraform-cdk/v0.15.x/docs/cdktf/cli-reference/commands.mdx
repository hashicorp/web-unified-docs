...
Examples:
  cat main.tf | cdktf convert  --provider integrations/github                        Takes the HCL content of main.tf and converts it to CDK for Terraform content
                                                                                     and prints it
  cat main.tf | cdktf convert --provider hashicorp-broken-link/aws > imported.ts                 Takes the HCL content of main.tf and converts it to CDK for Terraform content
                                                                                     in imported.ts
  cat main.tf | cdktf convert --provider 'hashicorp-broken-link/aws@ ~>3.62.0'                   Takes the HCL content of main.tf and converts it to CDK for Terraform content
