...
When using the File builder, ensure that source identifies a readable file on the machine running Packer.
The path is opened during the build; if it cannot be opened, Packer returns the underlying operating-system error.
