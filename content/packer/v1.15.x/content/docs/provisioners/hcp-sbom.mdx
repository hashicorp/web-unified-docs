...
When you enable `auto_generate` without specifying `scanner_url`, Packer automatically downloads the latest Syft v1.x release for compatibility and stability. If you need to use a specific version of Syft, you can provide a direct URL to that version's binary using the `scanner_url` parameter.

-> **Note:** Packer downloads Syft v1.x releases to ensure compatibility and avoid breaking changes that may occur in major version updates.
