...
- `-skip-enforcement` - Skip injection of HCP Packer enforced provisioners. Enforced provisioners are versioned provisioner definitions linked to an HCP Packer bucket, and Packer applies them automatically during the build. Requires admin privileges. Refer to [Enforced provisioners](/packer/docs/provisioners/enforced-provisioners) for details.