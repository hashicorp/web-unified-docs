...
func (p *ExampleCloudProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
  resp.ActionResourceData = &http.Client{/* ... */}
}
```
...
1. Send any [progress messages](#sending-progress-messages) while invoking the action.
...
For actions that can have longer invocation times, the provider can send progress updates back to Terraform that will be displayed to the practitioner.
...
{/* TODO: Create a good example for waiting for an HTTP request, have a start below */}
...
func (e *DoThingAction) Invoke(ctx context.Context, req action.InvokeRequest, resp *action.InvokeResponse) {
    ...
    go func() {
        ...
        if err != nil {
            resp.Diagnostics.AddError(
                "HTTP GET Error",
                "Error retrieving data. Please report this issue to the provider developers.",
            )
            done <- true
            return
        }
    ...
    for {
        select {
        case <-done:
            // Check for diagnostics, add to resp.Diagnostics
            return
        case <-ticker.C:
...
The [`action.ActionResourceWithConfigValidators` interface](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/action#ActionResourceWithConfigValidators) follows a similar pattern to attribute validation and allows for a more declarative approach. This enables consistent validation logic across multiple actions. Each validator intended for this interface must implement the [`action.ConfigValidator` interface](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/action#ConfigValidator).
...
{/* TODO: Skeleton
    ...
    Updates to framework docs
        ...
        - Implementation (start with one page, then move to multiple if needed)
            - ModifyPlan


        - Updates to make to other pages (maybe in a way that we don't have to constantly update for new schemas?)
            - Provider configuration page
            - Adjust type/schema/attribute/block shared docs
            - Common validation page
