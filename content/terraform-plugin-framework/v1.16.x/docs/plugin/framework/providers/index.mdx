...
description: >-
  Learn how to implement a provider in the Terraform plugin framework.
  Providers, wrapped by a provider server, are plugins that allow Terraform to
  interact with APIs.
---
...
- Configure implementations like [resources](/terraform/plugin/framework/resources/configure) or [data sources](/terraform/plugin/framework/data-sources/configure) with provider-level data types or clients.
