...
func (f *EchoFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Diagnostics.Append(req.Arguments.Get(ctx, &input)...)

    // Set the result to the same data
    resp.Diagnostics.Append(resp.Result.Set(ctx, &input)...)
}
```
...
If the logic needs to return [warning or error diagnostics](/terraform/plugin/framework/diagnostics), they can be added into the [`function.RunResponse.Diagnostics` field](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/function#RunResponse.Diagnostics).
...
The first option is using the [`(function.ArgumentsData).Get()` method](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/function#ArgumentsData.Get) to read all arguments at once. The framework will return errors if the number and types of target variables does not match the argument data.
...
func (f *ExampleFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Diagnostics.Append(req.Arguments.Get(ctx, &boolArg, &stringArg)...)

    // ... other logic ...
}
```

The second option is using [`(function.ArgumentsData).GetArgument()` method](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/function#ArgumentsData.GetArgument) to read individual arguments. The framework will return errors if the argument position does not exist or if the type of the target variable does not match the argument data.
...
func (f *ExampleFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
    ...
    resp.Diagnostics.Append(req.Arguments.GetArgument(ctx, 0, &boolArg)...)
