...
concurrently. Refer to [muxing](/terraform/plugin/framework/migrating/mux) for
more information.
...
When muxing SDKv2 and framework to migrate, the SDKv2 version of your provider
definition uses an SDKv2 provider schema, and your framework provider definition
uses the framework version. Once you have completed the migration, remove the
SDKv2 provider schema along with the provider definition and related mux code in
the provider server.
