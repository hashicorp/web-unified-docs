...
framework. In the framework, you will implement resource import with a
`ImportState()` function on your resource type.
...
Refer to [Resources
- Import](/terraform/plugin/framework/resources/import) in the framework
...
`schema.ResourceData`. This slice might be as simple as returning the
`ResourceData` that was passed into the function, or it may involve multiple fan
out to multiple resources, such as AWS security groups.
...
- In both SDKv2 and framework, there is no access to the configuration, state,
  or plan during import. The import functions can only access the value (e.g.,
  the resource's `ID`) supplied to the `terraform import` command or import block.
