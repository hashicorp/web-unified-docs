...
`schema.ResourceData`. This slice might return the
`ResourceData` that was passed into the function or involve fanning ou to multiple resources, such as AWS security groups.
...
- Neither SDKv2 or framework provide access to the configuration, state,
  or plan during import. The import functions can only access the value supplied to the `terraform import` command or import block, such as the resource's `ID`.
