...
# Migration using muxing

Muxing enables multiple underlying provider implementations to exist within the
same logical provider server via the [terraform-plugin-mux Go
module](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-mux). Each
underlying provider implementation serves different managed resources and data
sources. Refer to the [Combining and Translating
documentation](/terraform/plugin/mux) for more details about muxing
configuration.
...
- You are migrating an existing terraform-plugin-sdk based provider to the
  framework.
...
Otherwise, for simplicity, we recommend that you migrate directly to the
framework without temporarily introducing muxing.

## Requirements

- Ensure `github.com/hashicorp/terraform-plugin-sdk/v2` is upgraded to the
  latest version. For example, running the `go get
  github.com/hashicorp/terraform-plugin-sdk/v2@latest` command.
- Ensure existing acceptance tests pass. Acceptance testing can be used to
  verify the muxing implementation works before you release a new version of
  your provider.

## Implementation

To implement muxing on your provider:
...
1. Implement the `provider.Provider` type's
   ...
   methods so it is compatible for muxing. The schema and configuration handling
   must exactly match between all underlying providers in the provider server.
   Refer to the [provider schema
   section](/terraform/plugin/framework/migrating/providers#provider-schema) of
   the migration guide for additional details.
1. Introduce a mux server using `terraform-plugin-mux` functionality. This code
   eventually must be referenced by your provider's `main()` function, which is
   responsible for starting the provider server. Refer to the [Mux Server
...
1. Introduce an acceptance test for the mux server implementation. Refer to the
   [Testing Examples section](#testing-examples) for additional details.
...
1. Migrate your provider to the framework. Refer to the [Workflow - Migrating
   with
   muxing](/terraform/plugin/framework/migrating/workflow#migrating-with-muxing)
   section of this guide for additional details.
1. Once the migration is complete, remove the muxing configuration from your
   provider, as well as any remaining references to SDKv2 libraries.
1. Verify that all of your tests continue to pass.
1. Release a new version of your provider with muxing and SDKv2 libraries
   removed.

## Mux server examples

The following examples demonstrate adding muxing to your provider in various
scenarios.

### Terraform 0.12 compatibility

If your provider maintains compatability with Terraform 0.12, implement muxing
with [protocol version
5](/terraform/plugin/terraform-plugin-protocol#protocol-version-5).

The following `main.go` example shows how to set up muxing for a provider that
uses protocol version 5 to maintain compatibility with Terraform 0.12 and later.
...
The mux server can be setup to break compatibility with Terraform 0.12 through
1.1.6, but enable [protocol version
6](/terraform/plugin/terraform-plugin-protocol#protocol-version-6) capabilities
in the framework provider, such as nested attributes. Remember to only introduce
...
The following `main.go` example shows how to set up muxing for a provider that
upgrades the `terraform-plugin-sdk` based provider to protocol version 6 to
support those new features in the framework provider.
...
The following examples demonstrate adding tests to verify that the muxing
...
that defines the provider code to verify the muxing setup using protocol version
5:
...
The following acceptance test example would be included in the same Go package
that defines the provider code to verify the muxing setup:
...
- Only acceptance tests for migrated managed resources and data sources require
  testing code updates as noted in the
  [testing](/terraform/plugin/framework/migrating/testing) page of the migration
  guide.

## Troubleshooting muxing
