...
module](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-mux) to help you migrate to the plugin framework over two or more release cycles. The module allows you to multiplex, also called **mux**, data from providers built on SDKv2 and the plugin framework. Each underlying provider implementation serves different managed resources and data sources. Refer to the [Combining and Translating documentation](/terraform/plugin/mux) for more details about muxing configuration.
...
When possible, we recommend migrating directly to the
framework without temporarily muxing provider resources.
...
Complete the following steps to mux your provider:
...
1. Introduce a mux server using `terraform-plugin-mux` functionality. Your provider's `main()` function must eventually reference this code. The `main()` function is responsible for starting the provider server. Refer to the [Mux Server
...
The following examples demonstrate how to implement the `terraform-plugin-mux` module in your provider for various
scenarios.
...
1.1.6 and still enable [protocol version
6](/terraform/plugin/terraform-plugin-protocol#protocol-version-6) capabilities, such as nested attributes, 
in the framework provider. You should only introduce
...
The following `main.go` example shows how to use muxing to upgrade a provider based on `terraform-plugin-sdk` so that it uses protocol version 6 and supports features in the framework provider.
...
- You only need to update acceptance test code for resources and data sources managed by Terraform. Refer to [Test your migration](/terraform/plugin/framework/migrating/testing) for more information.

## Troubleshoot the `terraform-plugin-mux` implementation
