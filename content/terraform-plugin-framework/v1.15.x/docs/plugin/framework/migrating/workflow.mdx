...
you can follow the [workflow without muxing](#migrate-without-muxing) to migrate
your provider in a single release cycle. For complex providers that support a
number of resources and other Terraform features, we recommend that you follow
the [workflow with muxing](#migrate-with-muxing) to incrementally migrate your
provider to the framework across several release cycles. Migration to the
framework should not require any changes to configuration that uses your
provider. Remember to only introduce breaking changes to your provider in [major
version
...
1. Ensure all
   [tests](/terraform/plugin/framework/migrating/testing#testing-migration)
   pass.
...
To migrate your provider incrementally across multiple release cycles, follow
the steps below.
...
1. [Serve your
   provider](/terraform/plugin/framework/migrating/providers#serving-the-provider)
   via the framework.
1. Implement [muxing](/terraform/plugin/framework/migrating/mux) for your provider.
1. Update the [provider
   definition](/terraform/plugin/framework/migrating/providers#provider-definition)
   to use the framework.
1. Update the [provider
...
1. During migration, follow your normal release process to fix bugs and add
   features to your provider. By relying on muxing and test-driven development, the
   migration should not affect users of existing configuration.
1. Remove the muxing configuration from your provider, as well as any remaining references to SDKv2 libraries.
1. Verify that all of your tests continue to pass.
1. Once you have completed the migration process and verified that your provider
   works as expected, release a new version of your provider.
