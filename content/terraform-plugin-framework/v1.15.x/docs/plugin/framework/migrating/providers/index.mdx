...
provider. Once migration is complete, remove the SDKv2 versions of your provider
...
You can use the `terraform-plugin-mux` module to help you iteratively migrate individual resources and data sources to the
framework across multiple release cycles. The module multiplexes, or **muxes**, data using SDKv2 and framework concurrently. Refer to
[muxing](/terraform/plugin/framework/migrating/mux) for more information..

### Migrate in a single release cycle

If you are migrating your provider in a single release cycle, update your provider's `main()` function to serve the framework provider
...
When muxing SDKv2 and framework to migrate, the SDKv2 version of your provider definition 
uses an SDKv2 provider schema, and your framework provider definition uses
the framework version. Once you have completed the migration, remove the SDKv2
provider schema along with the provider definition and related mux code in
the provider server.
...
In SDKv2, `schema.Schema` is a struct that defines attributes and behaviors, such as `Type` or `Optional`. In the framework
