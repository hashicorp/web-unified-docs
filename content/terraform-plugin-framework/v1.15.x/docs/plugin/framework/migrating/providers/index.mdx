...
server, provider definition, and provider schema. When you migrate using muxing,
your provider server will serve both the SDKv2 and framework versions of your
provider. Once mgiration is complete, remove the SDKv2 versions of your provider
server, definition, and schema.
...
### Provider server with muxing

Use muxing to iteratively migrate individual resources and data sources to the
framework across multiple release cycles. Refer to
[muxing](/terraform/plugin/framework/migrating/mux) to implement a provider
server with muxing.

### Provider server without muxing

If you are migrating your provider in a single release cycle without using
muxing, update your provider's `main()` function to serve the framework provider
instead of the SDKv2 version.
...
When migrating using muxing, you will implement both the SDKv2 and framework
versions of your provider definition during the migration. Once you have
completed the migration, remove the SDKv2 provider definition and related muxing
code in the provider server.
...
A provider schema defines the attributes and behaviors of the provider itself.
...
When migrating using muxing, the SDKv2 version of your provider definition will
use an SDKv2 provider schema, and your framework provider definition will use
the framework version. Once you have compelted the migration, remove the SDKv2
provider schema along with the provider definition and related muxing code in
the provider server.
...
method is part of the `provider.Provider` interface that your provider must
implement. `Schema` returns a struct containing fields for `Attributes` and
`Blocks`. These `Attributes` and `Blocks` contain `map[string]schema.Attribute`
and `map[string]schema.Block`, respectively. Refer to [Providers -
...
pages in this migration guide to learn how to migrate those fields to the
framework.
...
Remember the following differences between SDKv2 and the framework when you migrate your provider schema.

- In SDKv2, `schema.Schema` is a struct that defines attributes and behaviors
(e.g., `Type`, `Optional`). In the framework `schema.Schema` is a struct that
includes attributes and blocks.
