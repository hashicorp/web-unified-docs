...
itself. You must also update your existing tests to use a provider
factory to test your the provider's framework version.
...
<Tip>
  ...
  If you are [muxing](/terraform/plugin/framework/migrating/mux) SDKv2 and framework to incrementally migrate, update the test code for each resource and data source
...
Each migration test runs `terraform plan` to generate state with the SDKv2 version of your provider. Next, it generates a plan with the framework version of the provider. This step also verifies that the plan is non-operational and confirms that migrating to the framework will not alter the behavior of existing configurations that use your provider.
...
Terraform refreshes data sources during every plan operation, but it does not use prior state
during the operation. As a result, you should use a separate managed resource or output value to ensure that there are no differences between a data source implemented with SDKv2 and the same data source after migration to the framework.
...
In many cases, existing tests do not require significant updates. The only necessary change is to the provider factories that
