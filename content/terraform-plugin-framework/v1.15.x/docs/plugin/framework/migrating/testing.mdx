...
itself. You will also need to update your existing tests to use a provider
factory to test the framework version of your provider.
...
<Tip>
  ...
  If you are using [muxing](/terraform/plugin/framework/migrating/mux) during
  migration, update the testing code for each resource and data source
  immediately before migrating them. You do not have to update tests for
  resources and data sources that you are not yet migrating to the framework.
...
For each migration test, the first test step will apply a plan and generate
state with the SDKv2 version of your provider. The second test step will
generate a plan with the framework version of the provider and verify that the
plan is a no-op, confirming that migrating to the framework will not alter the
...
- The second `TestStep` uses `ProtoV6ProviderFactories` so that the test uses
  the provider code contained within the provider repository. Then it uses
  `ConfigPlanChecks` to verify that an empty (no-op) plan is generated,
  indicating that configuring using the SDKv2 version of your provider will
  continue to work the same way after you migrate your provider to the
  framework.

### Testing data sources

Since data sources are refreshed every Terraform plan and do not use prior state
during planning, use a separate managed resource or output value to ensure there
are no differences between a data source implemented with SDKv2 and the same
data source after migration to the framework.
...
<!-- Suggested example: data source test using null_resource -->

## Provider factories

Existing tests should require minimal updates when migrating from SDKv2 to the
framework. The only necessary change relates to the provider factories that
create the provider during the test case. Refer to [Acceptance Tests - Specify
...
`provider.Provider`, depending on the [Terraform Plugin
