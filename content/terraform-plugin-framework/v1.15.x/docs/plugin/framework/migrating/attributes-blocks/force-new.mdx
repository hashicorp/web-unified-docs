...
attribute changes. In SDKv2, this is accomplished via the `ForceNew` field. In
the framework, implement the same behavior via a `RequiresReplace` plan
...
In SDKv2, setting the `ForceNew` field on an attribute's `schema.Schema`
triggers a replace (i.e., a destroy-create cycle) whenever the attribute's value
is changed.

In the framework, you implement the same behavior by using the
...
for an example, but bear in mind that each implementation requires different
