...
All identity attributes provided in configuration do not need to be exactly retained.

If identity data is present in the request, the provider is expected to ignore anything in the import_id field (which Core should set to ""). To maintain compatibility with the terraform import CLI command and the import block with id field, providers must continue to support importing via import ID if the identity data is not present.

For new resources, providers may choose not to support an import ID at all. In this case, if the user supplies an import ID (via the terraform import CLI command or in an import block), Core will send an ImportResourceState request including a non-blank import_id field, and the provider must return diagnostics with the ImportResourceState response saying that it is not supported.