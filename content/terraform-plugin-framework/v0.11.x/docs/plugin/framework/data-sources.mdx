...
interface](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/provider#DataSourceType)
...
`NewDataSource` returns a new instance of that data source type. It needs to instantiate a new [`datasource.DataSource`
implementation](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/datasource#DataSource)
that expects to operate on data with the structure defined in `GetSchema`.

The `NewDataSource` method is passed a [`provider.Provider`
implementation](https://pkg.go.dev/github.com/hashicorp/terraform-plugin-framework/provider#Provider).
