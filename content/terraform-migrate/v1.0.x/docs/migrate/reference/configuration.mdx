...
This page describes the parameters you can configure for the `tf-migrate` CLI.
...
Specifies a list of paths to directories you want `tf-migrate` to skip. This parameter is identical to using the [`-skip-dir` command-line flag](/terraform/migrate/reference/prepare#available-options). By default,  `tf-migrate` processes all child directories containing Terraform configuration files.
