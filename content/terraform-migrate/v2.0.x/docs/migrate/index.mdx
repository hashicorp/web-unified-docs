...
The `tf-migrate` CLI can migrate Terraform configuration and state from one or more HCP Terraform workspace to a Stack. Refer to [Migrate workspaces to a Stack](/terraform/migrate/stacks) for more information.
