...
last_modified: 2026-04-01T17:54:02.000Z
...
- Your `terraform.tfstate` state file, including `terraform.tfstate.*` backup state files. The state file can contain secrets and other sensitive information, and must be locked so that only one instance of Terraform modifies it at a time.
