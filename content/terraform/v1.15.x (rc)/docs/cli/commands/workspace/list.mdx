...
The current workspace is indicated using an asterisk (`*`) marker.
...
### Example: machine-readable, JSON output
...
an error prior to beginning the list task that will thus not be subject
...
The output includes a `format_version` key, which as of Terraform 1.15.0 has
...
- `workspaces` (array of objects): A JSON array of nested objects that each
...
- `is_current` (boolean): Indicates whether this workspace is the current workspace.
  This will only be present in the current workspace JSON object.
