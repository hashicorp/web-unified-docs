...
refer to [Manage sensitive data](/terraform/language/manage-sensitive-data).
...
However, Terraform completely omits any `ephemeral` values, even if you specify an output by name. Ephemeral values are never stored in state or included in Terraform plans. For more information, refer to [Manage sensitive data](/terraform/language/manage-sensitive-data).
...
The `terraform output` command only displays outputs from the root module. 
To display outputs from child modules, you must define a new output in the 
root module with a value of the child submodule output.
