...
The `depends_on` argument specifies an explicit dependency on another upstream resource for an `output` block. Terraform must complete all operations on the upstream resource before it computes the value of an `output` block that depends on it.
