...
Assertion arguments in Terraform `run` blocks help your [validate your configuration](/terraform/language/validate). Each `run` block contains a [`condition` argument](/terraform/language/expressions/conditionals) and an [`error_message` argument](/terraform/language/validate#error-messages).
...
- They do not share the same state file. The state file is determined by the state key or the module source when the state key is not set. If two `run` blocks do share the same module configuration, they must specify different state keys to be executed in parallel.
