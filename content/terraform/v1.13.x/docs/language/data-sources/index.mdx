...
The `filter` blocks, and the arguments within them, and the `most_recent` argument are specific to the `aws_ami` provider. Refer to the [AWS provider documentation](https://registry.terraform.io/providers/hashicorp/aws/latest) in the Terraform registry for details.