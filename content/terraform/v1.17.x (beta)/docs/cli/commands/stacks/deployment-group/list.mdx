...
last_modified: 2026-09-24T22:32:39.000Z
...
- **Failed deployment groups**: Appears when one or more deployment groups have a `failed` or `abandoned` status. The following example shows how Terraform lists each affected group and provides a `terraform stacks deployment-run list` command for inspecting its runs:
