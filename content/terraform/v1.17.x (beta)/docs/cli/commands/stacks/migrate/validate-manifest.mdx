...
- Each dynamic provider credential configuration appears only once per
  deployment.
- All deployments use the same dynamic provider credential families, alias
  tags, and audiences.
...
- Each `state.tfstate` path points to a readable Terraform state file created
  with Terraform v1.13.0 or later.
- All state files record the same Terraform version.
