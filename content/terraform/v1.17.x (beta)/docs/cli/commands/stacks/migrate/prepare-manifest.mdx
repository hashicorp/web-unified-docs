...
The command detects dynamic provider credentials configured on the source
workspaces and records supported OpenID Connect (OIDC) configurations in the
manifest. Every deployment must use the same provider families, alias tags, and
audiences because a Stack defines identity tokens once for all deployments.

The command stops when a workspace uses credentials that Terraform Stacks
cannot represent. Unsupported configurations include Vault, Vault-backed, and
HCP Vault Secrets-backed dynamic credentials, and plan-specific or
apply-specific identities. Configure a single run identity or migrate the
workspace manually before running the command again.
