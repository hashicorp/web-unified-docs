...
last_modified: 2026-09-14T19:41:22.000Z
...
- `-invoke=action.<TYPE>.<LABEL>`: Specifies an action to create a plan for. When set, Terraform excludes all other configurations from the plan. Refer to [Invoke an action](/terraform/language/invoke-actions) for more information about actions.
...
- `-minimal-refresh` - Reduces the number of remote API requests by refreshing only resource instances that Terraform plans to change. Terraform first compares the current configuration
  to the prior state; if there are no planned changes, the resource skips refreshing its current state. If there are planned changes, Terraform refreshes the resource and plans it again with the refreshed state.
  Each resource instance independently determines whether it needs to be refreshed, so refreshing one resource does not cause other resources that depend on it to be refreshed.

  Similar to `-refresh=false`, this option causes Terraform to ignore external changes to resources that have no planned changes, which can result in an incomplete or incorrect plan.
  
  
  You cannot use `-minimal-refresh` in refresh-only planning mode, or in combination with the `-refresh=false` option.

  -> **Note:** Terraform refreshes a resource regardless of the plan result if the resource schema version changes. Avoid using `-minimal-refresh` or `-refresh=false` when upgrading provider versions so that Terraform can populate state with any schema changes or additions.
...
When Terraform runs a plan operation, it displays every action it will take on the resources in your configuration. Terraform uses the following symbols to indicate if it plans to create, destroy, or change a resource:

| Symbol | Action | Description |
| ------ | ------ | ----------- |
| `+`    | Create | This resource does not currently exist. Terraform will create it. |
| `-`    | Destroy | Terraform will destroy this resource. |
| `~`    | In-place update | Terraform will update this resource without destroying and recreating it. |
| `-/+`  | Replace | Terraform will destroy this resource and then recreate it. |

The following example shows a plan that creates, deletes, updates, and recreates resources:
