...
indicated using an asterisk (`*`) marker, or by its JSON key `is_current` set to _True_.

The command-line flags are all optional. The following flags are available:

* `-json` - Produce output in a machine-readable JSON format. Always disables
  color.
...
In this example, the active workspace is `development`, and the output is in human readable form.
...
In this example, the active workspace is `development`, and the output is in JSON form.
...
When you use the `-json` option, Terraform will produce workspace list results in JSON format.

As with all JSON output options, it's possible that Terraform will encounter an error prior to beginning the list task that will thus not be subject to the JSON output setting. For that reason, external software consuming Terraform's output should be prepared to find data on stdout that _isn't_ valid JSON, which it should then treat as a generic error case.

The output includes a `format_version` key, which as of Terraform 1.15.0 has value `"1.0"`. The semantics of this version are:
...
We will introduce new major versions only within the bounds of [the Terraform 1.0 Compatibility Promises](/terraform/language/v1-compatibility-promises).
...
- `workspaces` (array of objects): A JSON array of nested objects that each describe a workspace.

- `diagnostics` (array of objects): A JSON array of nested objects that each describe an error or warning from Terraform.

The nested objects in `workspaces` have the following properties:

- `name` (string): The name of the workspace.

- `is_current` (boolean): Indicates whether this workspace is the current workspace. This will only be present in the current workspace JSON object.
...
- `snippet` (object): An optional object including an excerpt of the configuration source code that the diagnostic message relates to.
  ...
  - `context` (string): An optional summary of the root context of the diagnostic. For example, this might be the resource block containing the expression that triggered the diagnostic. For some diagnostics, this information is not available, and then this property will be `null`.
