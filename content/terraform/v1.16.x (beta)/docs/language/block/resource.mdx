...
last_modified: 2025-11-24T15:36:07Z
...
- [`resource "<TYPE>" "<LABEL>"`](#resource) &nbsp; block
   ...
   - [`lifecycle`](#lifecycle) &nbsp; block
      - [`action_trigger`](#action_trigger) &nbsp; block
        - [`events`](#action_trigger) &nbsp;  list | required to invoke an action
        - [`condition`](#action_trigger) &nbsp; expression
        - [`actions`](#action_trigger) &nbsp;  list | required to invoke an action
        - [`on_failure`](#action_trigger) &nbsp; keyword
...
The `action_trigger` block specifies a set of arguments that determine which events trigger one or more provider actions, under which conditions the action runs, and which actions Terraform invokes. When an action is called from an `action_trigger`, a special `caller` symbol can be used within the action config. The `caller` symbol evaluates to the value of the resource invoking the action from its `action_trigger`
...
The optional `on_failure` argument specifies how Terraform behaves when an action fails. You can set one of the following values:

- `halt`: Stops the apply operation and reports an error. The resource is not tainted. This is the default.
- `continue`: Logs the error as a warning and continues the apply operation.
- `taint`: Stops the apply operation, reports an error, and marks the resource as tainted so that Terraform replaces it on the next apply.

`action_trigger` is a directive available in the `lifecycle` **meta-argument**. Meta-arguments are built into the Terraform language and control how Terraform creates resources. Refer to the [`lifecycle` reference](/terraform/language/meta-arguments/lifecycle) for details about how the argument works.
...
`create_before_destroy` is a directive available in the `lifecycle` **meta-argument**. Meta-arguments are built into the Terraform language and control how Terraform creates resources. Refer to the [`lifecycle` reference](/terraform/language/meta-arguments/lifecycle) for details about how the argument works.
...
`prevent_destroy` is a directive available in the `lifecycle` **meta-argument**. Meta-arguments are built into the Terraform language and control how Terraform creates resources. Refer to the [`lifecycle` reference](/terraform/language/meta-arguments/lifecycle) for details about how the argument works.
...
`ignore_changes` is a directive available in the `lifecycle` **meta-argument**. Meta-arguments are built into the Terraform language and control how Terraform creates resources. Refer to the [`lifecycle` reference](/terraform/language/meta-arguments/lifecycle) for details about how the argument works.
...
`replace_triggered_by` is a directive available in the `lifecycle` **meta-argument**. Meta-arguments are built into the Terraform language and control how Terraform creates resources. Refer to the [`lifecycle` reference](/terraform/language/meta-arguments/lifecycle) for details about how the argument works.
...
We recommend using configuration management tools or other means to perform actions on the local or remote machine instead of using provisioners. Refer to [Perform post-apply action](/terraform/language/provisioners) for more information.
...
We recommend using configuration management tools or other means to perform actions on the local or remote machine instead of using the `provisioner` block. Refer to [Perform post-apply action](/terraform/language/provisioners) for more information.
...
- `TYPE`: Specifies the type of provisioner to use. You must specify one of the following types:
  - `file`: Copies files or directories from the machine where Terraform is running to the new resource using the chosen connection `type` (`ssh` or `winrm`).
