...
use the [`terraform_remote_state` data source](/terraform/language/reference/remote-state-data).
...
For optional backend attributes, such as `region`, you can set the value using one of the following environment variable formats:
...
> ⚠️ **Note**:  Required attributes like `bucket` and `namespace` cannot be configured through environment variables. They must be explicitly specified in the backend configuration block.
