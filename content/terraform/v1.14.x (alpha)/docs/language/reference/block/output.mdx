...
Any valid expression can be an output `value.` Refer to [Expressions](/terraform/language/reference/expressions/) to learn more.
...
Terraform also records sensitive values in state, so anyone who can access your state data can access your sensitive values. For more information about safely storing sensitive data, refer to [Manage sensitive data](/terraform/language/manage-sensitive-data).
...
- A [write-only argument](/terraform/language/manage-sensitive-data/write-only)
- [Variables with the `ephemeral` argument](/terraform/language/reference/block/variable#ephemeral)
- [An `ephemeral` resource block](/terraform/language/reference/block/ephemeral)
...
| :---- | :---- | :---- | :---- |
| `condition` | Expression that must return `true` for Terraform to proceed with an operation. | A valid [Conditional expression](/terraform/language/reference/expressions/conditionals). | Required |
...
Refer to [Validate your configuration](/terraform/language/validate#preconditions) for more details about preconditions.
