...
- In the [`locals` block](/terraform/language/reference/block/locals)
- In [`variable` blocks with the `ephemeral` argument set to `true`](/terraform/language/reference/block/variable#ephemeral)
- In [child module `output` blocks with the `ephemeral` argument set to `true`](/terraform/language/reference/block/output#ephemeral)
- Configuring [providers in the `provider` block](/terraform/language/reference/functions/terraform-applying)
...
- `LABEL`: Specifies a name for the ephemeral resource. Terraform uses this label to track the resource during the current operation. The label does not affect settings on the actual infrastructure resource. Refer to [References to Named Values](/terraform/language/reference/expressions/references) and [Resource naming](/terraform/language/style#resource-naming) for label syntax and recommendations.
