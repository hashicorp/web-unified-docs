...
[`regexall`](/terraform/language/reference/functions/regexall) and test that the result has length greater than
zero.
...
- [`regexall`](/terraform/language/reference/functions/regexall) searches for potentially multiple matches of a given pattern in a string.
- [`replace`](/terraform/language/reference/functions/replace) replaces a substring of a string with another string, optionally matching using the same regular expression syntax as `regex`.
