...
* [`filebase64`](/terraform/language/reference/functions/filebase64) also reads the contents of a given file,
  but returns the raw bytes in that file Base64-encoded, rather than
  interpreting the contents as UTF-8 text.
* [`fileexists`](/terraform/language/reference/functions/fileexists) determines whether a file exists
  at a given path.
* [`templatefile`](/terraform/language/reference/functions/templatefile) renders using a file from disk as a
  template.
