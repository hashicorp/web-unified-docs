...
The `filesha512` is a variant of [`sha512`](/terraform/language/reference/functions/sha512)
that hashes the contents of a given file rather than a literal string.

This is similar to `sha512(file(filename))`, but
because [`file`](/terraform/language/reference/functions/file) accepts only UTF-8 text it cannot be used to
...
- [`sha512`](/terraform/language/reference/functions/sha512) computes the SHA-512 hash of a given string and encodes it with hexadecimal digits.
- [`filesha256`](/terraform/language/reference/functions/filesha256) computes the SHA-1 hash of a given file and encodes it with hexadecimal digits.
