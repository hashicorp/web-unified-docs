...
accepted in an
[input variable declaration](/terraform/language/reference/block/variable#basic-variable-declaration).

The `.tfvars` format is specific to Terraform and so we do not recommend using
it as a general serialization format.
Use [`jsonencode`](/terraform/language/reference/functions/jsonencode) or
[`yamlencode`](/terraform/language/reference/functions/yamlencode) instead to produce
...
* [`decode_tfvars`](/terraform/language/reference/functions/terraform-decode_tfvars)
  performs the opposite operation: parsing `.tfvars` content to obtain
  the variable values declared inside.
* [`encode_expr`](/terraform/language/reference/functions/terraform-encode_expr)
