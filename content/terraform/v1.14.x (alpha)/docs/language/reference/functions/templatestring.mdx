...
For information about the syntax you can use for the variables arguments, refer to [Strings and Templates](/terraform/language/reference/expressions/strings).
...
For more examples of how to use templates, refer to the documentation for [the `templatefile` function](/terraform/language/reference/functions/templatefile#Examples).

## Dynamic template construction

You can write an expression that builds a template dynamically and then assigns it to a [local value](/terraform/language/reference/block/locals). You can then use a reference to that local value as the first argument to the `templatestring` function.
...
* [`templatefile`](/terraform/language/reference/functions/templatefile) reads a file from disk and renders its content as a template.
