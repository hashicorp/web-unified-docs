...
The `filesha1` is a variant of [`sha1`](/terraform/language/reference/functions/sha1) that hashes the contents of a given file rather than a literal string. 

Use the `filesha1` function instead of wrapping the `file` function in a `sha1` function, for example `sha1(file(filename))`, because [`file`](/terraform/language/reference/functions/file) accepts only UTF-8 text. As a result, you cannot use `sha1(file(filename))` to create hashes for binary files.
...
- [`sha1`](/terraform/language/reference/functions/sha1) computes the SHA-1 hash of a given string and encodes it with hexadecimal digits.
- [`filesha256`](/terraform/language/reference/functions/filesha256) computes the SHA-256 hash of a given file and encodes it with hexadecimal digits.
- [`filesha512`](/terraform/language/reference/functions/filesha512) computes the SHA-512 hash of a given file and encodes it with hexadecimal digits.
