...
[`try`](/terraform/language/reference/functions/try) instead, because it allows for more concise definition of
...
[variable validation](/terraform/language/reference/block/variable#validation).
For example:
...
prefer to use [`try`](/terraform/language/reference/functions/try).
...
* [`try`](/terraform/language/reference/functions/try), which tries evaluating a sequence of expressions and
