...
[attribute notation](/terraform/language/reference/expressions/types#indices-and-attributes) for elements of object values, they are not
...
* If the resource doesn't use `count` or `for_each`, the reference's value is an
  ...
  access them using [dot or square bracket notation](/terraform/language/reference/expressions/types#indices-and-attributes).
...
`var.<NAME>` is the value of the [input variable](/terraform/language/reference/block/variable) of the given name.
...
`local.<NAME>` is the value of the [local value](/terraform/language/reference/block/locals) of the given name.
...
[output values](/terraform/language/reference/block/output), use `module.<MODULE NAME>.<OUTPUT NAME>`.
...
variables](/terraform/language/reference/block/variable); they are just arbitrary names
that temporarily represent a value.

The names in this section relate to top-level configuration blocks only.
If you use [`dynamic` blocks](/terraform/language/reference/expressions/dynamic-blocks) to dynamically generate
...
* The arguments of the `ebs_block_device` nested blocks can be accessed using
  a [splat expression](/terraform/language/reference/expressions/splat). For example, to obtain a list of
...
* Sometimes nested blocks are defined as taking a logical key to identify each
  ...
  block types, use a [`for` expression](/terraform/language/reference/expressions/for):
...
either [splat expressions](/terraform/language/reference/expressions/splat) or index syntax:
...
When a resource has the
[`for_each`](/terraform/language/reference/block/resource#for_each)
...
be accessed using a [`for` expression](/terraform/language/reference/expressions/for).
...
[an input variable declared as sensitive](/terraform/language/reference/block/variable#sensitive),
...
[Cases where Terraform may disclose a sensitive variable](/terraform/language/reference/block/variable#cases-where-terraform-may-disclose-a-sensitive-variable).

If you use a sensitive value from a resource attribute as part of an
[output value](/terraform/language/reference/block/output) then Terraform will require
...
values in cleartext. For more information, refer to [Manage sensitive data](/terraform/language/manage-sensitive-data).
