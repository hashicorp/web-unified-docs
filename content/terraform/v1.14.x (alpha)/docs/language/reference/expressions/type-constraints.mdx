...
[expressions](/terraform/language/reference/expressions), but are a special syntax. Within the
Terraform language, they are only valid in the `type` argument of an
[input variable](/terraform/language/reference/block/variable).
...
[Expressions: Literal Expressions](/terraform/language/reference/expressions/types#literal-expressions) as
