...
[`for_each`](/terraform/language/reference/block/resource#for_each) in the `resource` block reference.
...
The above example uses a [`dynamic` block](/terraform/language/reference/expressions/dynamic-blocks), which
