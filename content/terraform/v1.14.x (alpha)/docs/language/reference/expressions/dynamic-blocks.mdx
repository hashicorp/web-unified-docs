...
A `dynamic` block acts much like a [`for` expression](/terraform/language/reference/expressions/for), but produces
...
[`flatten`](/terraform/language/reference/functions/flatten)
and
[`setproduct`](/terraform/language/reference/functions/setproduct)
functions.
