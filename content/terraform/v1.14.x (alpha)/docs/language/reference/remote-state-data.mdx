...
[the `jsonencode` function](/terraform/language/reference/functions/jsonencode)
and
[the `jsondecode` function](/terraform/language/reference/functions/jsondecode) respectively
...
* (v0.12+) `outputs` - An object containing every root-level
  [output](/terraform/language/reference/block/output) in the remote state.
* (<= v0.11) `<OUTPUT NAME>` - Each root-level [output](/terraform/language/reference/block/output)
