---
page_title: Pass temporary values to a resource with write-only arguments
...
# Pass temporary values to a resource with write-only arguments
...
Write-only arguments complement [other ephemeral values](/terraform/language/reference/block/ephemeral#reference-ephemeral-resources) in Terraform, letting you securely pass sensitive data throughout your configuration without ever storing it in Terraform's artifacts. For example, you can generate a random password using an `ephemeral` resource then pass it to a write-only argument on another `resource` block. The provider uses the write-only argument value to configure the resource, then Terraform discards the value without storing it.
...
The following examples demonstrate how to use write-only arguments with different cloud providers.
