---
page_title: Omit values from state and plan files
description: Use ephemeral values to keep sensitive data out of Terraform state and plan files entirely.
---
...
Because Terraform does not store ephemeral values, you must capture any generated values you want to preserve in another resource or output in your configuration.

To learn about ephemeral resources, refer to the [`ephemeral` block reference](/terraform/language/reference/block/ephemeral).
...
To learn more about this example and using write-only arguments, refer to the [Use write-only 
arguments](/terraform/language/manage-sensitive-data/write-only).
