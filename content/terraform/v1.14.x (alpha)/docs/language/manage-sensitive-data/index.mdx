...
- In [`variable` blocks with the `ephemeral` argument](/terraform/language/reference/block/variable#ephemeral)
- In [child module `output` blocks with the `ephemeral` argument](/terraform/language/reference/block/output#ephemeral)
- In a managed resource [write-only argument](/terraform/language/manage-sensitive-data/write-only)
- The [`ephemeral` block](/terraform/language/reference/block/ephemeral)
