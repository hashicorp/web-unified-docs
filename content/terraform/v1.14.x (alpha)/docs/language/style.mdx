...
1. [Validation blocks](/terraform/language/reference/block/variable#validation)
...
Local values let you reference an [expression](/terraform/language/reference/expressions) or value multiple times. Use local values sparingly, as overuse can make your code harder to understand.
...
For more information, refer to the [`locals` block](/terraform/language/reference/block/locals) and the [Simplify Terraform configuration with locals](/terraform/tutorials/configuration-language/locals) tutorial.
...
A common practice to conditionally create resources is to use the `count` meta-argument with a [conditional expression](/terraform/language/reference/expressions/conditionals). In the following example, Terraform will only create the `aws_instance` if `var.enable_metrics` is `true`.
