...
regions) before they can be used. The [`provider` block reference](/terraform/language/reference/block/provider) documents how
...
1. Define the provider's source, local name, and version in the [`required_providers` block](/terraform/language/reference/block/terraform#required_providers) inside your top-level `terraform` block.
...
If you are running Terraform locally, your plan and apply operations use the provider versions specified in the dependency lock file. You can upgrade a provider version in your lock file by changing the version in the `required_providers` block and re-running `terraform init`.
...
[`terraform` block](/terraform/language/reference/block/terraform) (which can also contain other settings).
...
name when [configuring the provider](/terraform/language/reference/block/provider#using-an-alternate-provider-configuration):
...
declare should have a [version constraint](/terraform/language/reference/expressions/version-constraints) given in
...
[the `terraform_remote_state` data source](/terraform/language/reference/remote-state-data).
