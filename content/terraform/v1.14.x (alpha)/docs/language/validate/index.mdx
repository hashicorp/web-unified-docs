...
To learn more about variable validations, refer to the [input variable block](/terraform/language/reference/block/variable#validation).
...
The precondition detects if the caller accidentally selected an AMI for a different architecture, which may not be able to run the software this instance hosts. Terraform evaluates the precondition while it builds its plan, and if the precondition fails Terraform throws an error with the `error_message` argument and stops the current operation. Refer to the [resource configuration reference](/terraform/language/reference/block/resource) for more examples of using a precondition block.
...
If the `precondition` fails, Terraform throws an error with the `error_message` and stops the current operation. Refer to the [`output` block reference](/terraform/language/reference/block/output) for more details.
...
For more details, refer to the [`check` configuration reference](/terraform/language/reference/block/check).
...
This includes literal strings, heredocs, and template expressions. You can use the [`format` function](/terraform/language/reference/functions/format) to convert items of `null`, `list`, or `map` types into a formatted string. Multi-line error messages are supported, and Terraform does not wrap lines with leading whitespace.
...
To learn more about the conditions you can use in validations, refer to [Conditional Expressions](/terraform/language/reference/expressions/conditionals).
