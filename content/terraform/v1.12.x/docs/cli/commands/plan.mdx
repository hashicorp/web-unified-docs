...
- `-var-file=FILENAME` - Sets values for potentially many
  ...
  [`.tfvars` file](/terraform/language/parameterize/configure#variable-definition-files).
  Use this option multiple times to include values from more than one file.
There are several other ways to set values for input variables in the root
module, aside from the `-var` and `-var-file` options. Refer to
[Assign values to input variables](/terraform/language/parameterize/configure#assign-values-to-input-variables) for more information.
...
input variables, see
[Assign values to input variables](/terraform/language/parameterize/configure#assign-values-to-input-variables).
