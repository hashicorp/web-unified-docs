...
Meta-arguments are a class of arguments built into the Terraform configuration language that control how Terraform creates resources. This page provides an overview of the meta-arguments included with Terraform.

## Introduction

You can use meta-arguments in any type of `resource`  block, including resources created with `data` blocks. You can also use most meta-arguments in `module` blocks.
...
### Guidance 
 ...
 - [Set dependencies](/terraform/language/modules/configuration) in your module configuration
...
### Guidance 
 ...
 - [Create multiple instances](/terraform/language/modules#create-multiple-copies-of-modules) when calling modules
...
By default, a Terraform configures one infrastructure object for each `resource` block. Terraform also creates single instances of a module from per `module` block. You can add the `for_each` block to your `resource` and `module` blocks to create and manage several similar objects, such as a fixed pool of compute instances, without writing a separate block for each instance.
...
By default, child modules inherit the default provider configurations of their parent module, but you can specify an alternate provider configuration in the `module` block. The `providers` argument instructs Terraform to use the reference provider configuration to create the module resources. 
...
1. Creates resources that exist in the configuration but are not associated with a real infrastructure object in the state.
...
1. Destroys and re-create resources whose arguments have changed but that cannot be updated in-place due to remote API limitations.
...
You can apply the following rules to all `resource` blocks:
...
- [Custom conditions for data sources](/terraform/language/data-sources#custom-condition-checks)
