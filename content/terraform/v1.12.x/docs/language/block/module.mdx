...
You must run [`terraform init`](/terraform/cli/commands/init) after adding, removing, or modifying a `module` block. To upgrade an already installed module, use `terraform init -upgrade`. 
...
The location is a local file or directory or a remote module source, such as the public Terraform registry. You must specify a literal string for the `source` value. This argument does not support template sequences or arbitrary expressions. Refer to [Module Sources](/terraform/language/modules/sources) for more information.
