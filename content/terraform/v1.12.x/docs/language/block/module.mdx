...
A module is a collection of multiple resources that Terraform uses together. Refer to the following topics for more information:
<!-- TODO: update links when the content is published -->
- [Modules overview](/terraform/language/module) 
- [Configure modules](#)
- [Write modules](#)
- [Publish modules to the registry](#)
- [Develop modules](#)

You must run [`terraform init`](/terraform/cli/commands/init) after adding, removing, or modifying a `module` block. Note that the command does not upgrade an already-installed module. You must also include the `-upgrade` option to upgrade to the newest availableversion.
...
The `LABEL` is a local name for the module. When the child module you are calling into the local module exposes resource attributes in an `output block`, you can use the `module.<label>.<output>` syntax to reference the data in the child module. Refer to the following topics for more information:

- [Use modules](TBD)
- [Develop modules](/terraform/docs/write-configuration/modules) 
...
The `source` argument specifies a location where Terraform can retrieve module source code. 
...
The location is a local file or directory or a remote module source, such as the public Terraform registry. You must specify a literal string for the `source` value. This argument does not support template sequences and arbitrary expressions are not supported. Refer to [Module Sources](/terraform/language/modules/sources) for more information.
...
Place any query parameters, such as the `ref` argument supported for the version control sources, after the sub-directory segment. For example:
...
```
```hcl
source = "hg::http://[user[:pass]@]host[:port]/[path][#revision]
```
```hcl
source = "hg::https://[user[:pass]@]host[:port]/[path][#revision]"
```
```hcl
...
By default, Terraform applies the default provider based on the module resource type, but you can create multiple provider configurations and use a non-default configuration for specific modules. 
