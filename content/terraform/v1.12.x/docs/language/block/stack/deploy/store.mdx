...
By default, all variables retrieved from variable sets in a Stack are ephemeral, and are not persisted in the state file of your deployments. To avoid this behavior you can use the `stable` keyword. To learn more, refer to [Accessing variables with stable](#store-variable-values-in-deployment-state).
...
### Store variable values in deployment state
...
HCP Terraform stores the value of `consul_license_key` in the state of the `production` deployment, but does not store the value of `gcp_service_account_key` because it is not marked as `stable`.
