...
Use the `deployment` block to define how many times you want to deploy your Stack's infrastructure. You can define `deployment` blocks in a `<NAME>.fdeploy.hcl` file, which is the deployment configuration file for your Stack.
...
The `deployment` block defines how to deploy your Stack's infrastructure. Each deployment represents a separate instance of your Stack infrastructure. Each deployment runs in its own isolated agent, wholly separated from other Stack deployments.
...
You can define the following configuration blocks in a `tfdeploy.hcl` file:
- Use the [`deployment_group`](/terraform/language/block/stack/deploy/deployment-auto-group) block to create a group of deployments and apply rules to those deployments.
- Use the [`deployment_auto_approve`](/terraform/language/block/stack/deploy/deployment-auto-group) block to define rules that automatically approve deployment plans based on specific conditions.
- Use the [`identity_token`](/terraform/language/block/stack/deploy/identity_token) block to authenticate a deployment with a provider configuration using OIDC.
- Use the [`store`](/terraform/language/block/stack/deploy/store) block to define key-value secrets in your deployment configuration and use existing variable sets in HCP Terraform.
- Use the [`locals`](/terraform/language/block/stack/deploy/locals) block to define local values that you can reuse within your deployment configuration.
- Use the [`publish_output`](/terraform/language/block/stack/deploy/publish_output) block to output values that you want to be available to other Stacks.
- Use the [`upstream_input`](/terraform/language/block/stack/deploy/upstream_input) block to source inputs from another Stack.
...
The keys in the `inputs` map must correspond to the names of the variables defined in your Stack configuration, and the values must be valid HCL literals that meet the type constraints of those variables.
...
The matching Stack configuration for this deployment, in a `tfcomponent.hcl` file, must include the same variable names and types as those defined in the `inputs` map.
...
When you run this configuration, HCP Terraform creates two separate deployments that each use the same Stack configuration. The `staging` deployment uses the `us-east-1` region and has one instance, while the `production` deployment uses the `us-west-1` region and has three instances.
...
To learn more about referencing variable sets from HCP Terraform in your Stack deployments, refer to the [`store` block](/terraform/language/block/stack/deploy/store).
