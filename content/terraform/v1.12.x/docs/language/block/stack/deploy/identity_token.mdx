...
| `sub` (subject) | Fully qualified path to a Stack deployment, followed by the operation type:<br/>`organization:<ORG_NAME>:project:<PROJECT_NAME>:stack:<STACK_NAME>:deployment:<DEPLOYMENT_NAME>:operation:<OPERATION_TYPE>`<br/><br/>For example:<br/>`organization:My_Org_name:project:My_Project:stack:My_Stack:deployment:staging:operation:apply` |
...
In the following example, the `aws` identity token configures an audience for AWS workload identity. The `production` deployment passes the token that the `identity_token` block generates, `identity_token.aws.jwt`, to the Stack configuration along with an IAM role ARN:
...
In your Stack configuration, you can use the generated JWT token to configure your AWS provider:
