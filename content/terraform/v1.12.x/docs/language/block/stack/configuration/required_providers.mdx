---
page_title: required_providers block reference for Terraform Stack configurations
...
Terraform relies on plugins called providers to interact with cloud providers, SaaS providers, and other APIs. Use the `required_providers` block in your Stack configuration to declare which providers your Stack requires.

## Background

In Stack configurations, you must define the `required_providers` block at the top level of your `tfcomponent.hcl` file.
...
The Terraform CLI uses the `required_providers` block to download the listed providers and compute provider lock hashes. To learn more and view examples, refer to [Declare providers](/terraform/language/stacks/create/declare-providers).
...
To learn more about using the `provider` block to further configure your Stack's providers, refer to [Declare providers](/terraform/language/stacks/create/declare-providers).
