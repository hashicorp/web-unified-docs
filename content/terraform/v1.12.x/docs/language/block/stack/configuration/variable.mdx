...
description: Use the `variable` block to declare input variables for your Stack configuration, making your Stack dynamic and reusable.
---

# `variable` block reference for Stack configurations

Use the `variable` block to declare input variables for your Stack configuration, making your components dynamic and reusable.
...
Use the `variable` block in Stack configurations, `<NAME>.tfcomponent.hcl` files, to declare values that can change between deployments.

Deployments, in [deployment configuration files](/terraform/language/stacks/deploy/config), pass values to Stack variables through the `inputs` argument in the deployment configuration. The separation between Stack configuration and deployment configuration lets you maintain a single Stack definition while creating multiple deployments with different inputs.
...
The label after the `variable` keyword is a name for the variable, which must be unique among all variables in the same Stack configuration. The name of a variable can be any valid [identifier](/terraform/language/syntax/configuration#identifiers).
...
The `type` argument constrains the type of value that deployments can assign to this variable. Stack configurations require you to explicitly define the type for each variable.
...
The following examples demonstrate common use cases for `variable` blocks in Stack configurations.

## Using variables in Stack configurations
...
The `production` and `staging` deployments can each input different values for `aws_region` and `instance_count` while building the same Stack configuration. The names of the variables in the deployment's `inputs` block must match the names declared in the Stack configuration `variable` blocks.
