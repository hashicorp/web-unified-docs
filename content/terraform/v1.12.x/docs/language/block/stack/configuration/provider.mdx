---
page_title: provider block reference for Terraform Stack configurations
description: Use the `provider` block in Stack configurations to configure providers for your Stack components.
---

# `provider` block reference for Stacks

Terraform relies on plugins called providers to interact with cloud providers, SaaS providers, and other APIs. Use the `provider` block in Stack configurations to configure your Stack's declared providers.

## Background

In Stack configurations, you must declare a provider in the [`required_providers`](/terraform/language/block/stack/configuration/required_providers) block at the top level of your `tfcomponent.hcl` file before you can use that provider.

After your provider declaration, use the `provider` block to further configure your providers. Unlike traditional Terraform configurations, `provider` blocks in Stack configurations support the following:
...
After configuring your providers, pass them to your Stack's [`component` blocks using the `providers` argument](/terraform/language/block/stack/configuration/component#providers). Passing configurations directly to components ensures that every component in your Stack uses the same provider configuration. To learn more and view examples, refer to [Declare providers](/terraform/language/stacks/create/declare-providers).
...
Stack configurations require all provider arguments to be nested within a `config` block.
...
To learn more about authenticating with cloud providers within Stacks, refer to [Authenticate providers](/terraform/language/stacks/create/authenticate-providers).
