...
# `output` block reference for Stack configurations
...
In a Stack configuration file, the `output` block supports the following configuration.

### `output "<LABEL>"`

The label after the `output` keyword is a name for the output, which must be unique among all outputs in the same Stack configuration. The name of an output can be any valid [identifier](/terraform/language/syntax/configuration#identifiers).
...
| `ephemeral` | Specifies whether to exclude this value from plans and state. | Boolean | Optional |

### `type`

The `type` argument specifies the type constraint for the output value. Stack configurations require you to explicitly define the type of each output.
...
In Stack configurations, outputs commonly reference component outputs and other Stack resources, such as `component.<COMPONENT_NAME>.<OUTPUT_NAME>`.
...
The following examples demonstrate common use cases for `output` blocks in Stack configurations.
