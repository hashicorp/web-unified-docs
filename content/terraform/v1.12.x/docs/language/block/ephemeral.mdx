...
To learn more about the `ephemeral` block and its unique lifecycle and provisioning order, refer to [Sensitive data in State](/terraform/language/state/sensitive-data).

## Refer to ephemeral resources

You can only refer to ephemeral resources in specific ephemeral contexts or Terraform throws an error. The following are valid contexts for referring to ephemeral resources:

You can only refer to values with the `ephemeral` property in specific contexts or Terraform throws an error. The following are valid ephemeral contexts:
...
You must set the following arguments for every `ephemeral` block:
...
- `LABEL`: Specifies a name for the ephemeral resource. Terraform uses this label to track the resource during the current operation. The label does not affect settings on the actual infrastructure resource. Refer to [References to Named Values](/terraform/language/expressions/references) and [Resource naming](/terraform/language/style#resource-naming) for expression syntax and label recommendations.
...
In blocks where you set `count`, Terraform exposes an additional `count` object. You can reference the object to modify the configuration of each instance. The `count` object has an `index` attribute starting from `0`.

To refer to an individual instance of an ephemeral resource created using the `count` meta-argument, use the `ephemeral.<TYPE>.<NAME>[INDEX]` syntax. For example, `ephemeral.random_password.db_password[0]` refers to the first instance of the `random_password` ephemeral resource named `db_password`.
...
The `depends_on` meta-argument specifies an upstream resource that the ephemeral resource depends on. Terraform must complete all operations on the upstream resource before performing operations on the ephemeral resource containing the `depends_on` argument.
...
The `depends_on` argument is a meta-argument, which is built into Terraform and controls the way that Terraform creates resources. Refer to [Meta-arguments](/terraform/language/meta-arguments/depends_on) for more information.

#### Summary

- Data type: List
- Default: None
- Example: [Specify dependencies for ephemeral resources](#specify-dependencies-for-ephemeral-resources)
...
You cannot use sensitive values, such as [sensitive input variables](/terraform/language/values/variables#sensitive), [sensitive outputs](/terraform/language/values/outputs#sensitive), or [sensitive resource attributes](/terraform/language/expressions/references#sensitive-resource-attributes), as arguments in `for_each`. Terraform uses the value in `for_each` to identify the ephemeral resource instance and always discloses it in UI output, so sensitive values are not allowed.
...
Use the `ephemeral.<TYPE>.<NAME>[<KEY>]` syntax to access an instance of an ephemeral resource created using `for_each`. For example, `ephemeral.random_password.passwords["admin"]` refers to an instance of the `random_password` ephemeral resource named `passwords` created from the `admin` key.

The `for_each` argument is a meta-argument, which is built into Terraform and controls the way that Terraform creates resources. Refer to [Meta-arguments](/terraform/language/meta-arguments/for_each) for more information.
...
The `provider` argument is a meta-argument, which is built into Terraform and controls the way that Terraform creates resources. Refer to [Meta-arguments](/terraform/language/meta-arguments/resource-provider) for more information.
...
- [`precondition`](#precondition): Specifies a condition that Terraform evaluates before creating the ephemeral resource. Refer to [Test and validate](/terraform/language/test-and-validate/validate) for more information.
- [`postcondition`](#postcondition): Specifies a condition that Terraform evaluates after creating the ephemeral resource. Refer to [Test and validate](/terraform/language/test-and-validate/validate) for more information.

The `lifecycle` block is a meta-argument. Meta-arguments are built-in arguments that control how Terraform creates resources. Refer to [Meta-arguments](/terraform/language/meta-arguments/lifecycle) for more information.
...
Refer to [Test and validate](/terraform/language/test-and-validate/validate) for information about adding validations to your Terraform configuration.
...
The following example configures the `postgresql` provider with credentials from an ephemeral resource:
...
### Configure providers with ephemeral resources
