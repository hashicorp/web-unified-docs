...
You can add the `sensitive` argument to `variable` or `output` blocks to make Terraform redact those values from CLI output and in the HCP Terraform UI. Terraform also treats any expressions that reference a sensitive variable or output as inherently sensitive. Learn more [about sensitive variables and outputs](#hide-sensitive-variables-and-outputs).
...
Terraform provides four ways to define ephemeral values in your configuration:

- The [`ephemeral` argument on variables](#add-the-ephemeral-argument-to-a-variable-or-output)
- The [`ephemeral` argument on child module outputs](#add-the-ephemeral-argument-to-a-variable-or-output)
- The [`ephemeral` block](#use-the-ephemeral-block)
- A [write-only argument](#use-write-only-arguments) on a managed resource

Ephemeral values do restrict where you can reference them in your configuration. You can only refer to ephemeral values in the following contexts, otherwise Terraform throws an error:
...
You can reference a `variable` block with the `ephemeral` argument in other ephemeral contexts. Refer to [ephemeral contexts](#omit-values-from-state-and-plan-files) to learn where you can reference a variable or output with the `ephemeral` argument.

For example, you can use the `api_token` variable in a `provider` block to configure a provider:
