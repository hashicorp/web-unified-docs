...
As described in [`provider` block reference](/terraform/language/block/provider), you can optionally
...
[a `<PROVIDER>.<ALIAS>` reference](/terraform/language/block/provider#using-an-alternate-provider-configuration),
