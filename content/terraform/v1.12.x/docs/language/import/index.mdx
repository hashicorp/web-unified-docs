...
description: Learn how to import existing resources into Terraform state so that you can manage it as code 
---
...
When you import existing infrastructure into state, you must also define the Terraform configuration corresponding to that resource to manage the rest of its lifecycle. You can manually write all of the configuration or use the Terraform CLI to generate some of the Terraform import code. Terraform pulls all of the resource's attributes into the state file, but you do not need to define all of them in the `resource` block. 
...
The following example resource is the destination for an AWS instance with a state address of `aws_instance.example`:
...
The `to` argument is the destination resource address, which is formed from the inline type and label arguments. 
