...
local values](/terraform/language/block/locals), [`ephemeral`
variables](/terraform/language/block/variable#ephemeral), or [`ephemeral`
output
values](/terraform/language/block/output#ephemeral).
...
[`sensitive` variables](/terraform/language/block/variable#sensitive)
or [`sensitive` output values](/terraform/language/block/output#sensitive).
...
~> **Note**: A resource's destroy-time provisioners do not run if you enable [`create_before_destroy`](/terraform/language/meta-arguments#lifecycle) on that resource.
