...
As a workaround, you can use a Terraform provisioner to run commands on the machine where Terraform or on a remote resource. Refer to [Configure a provisioner](#configure-a-provisioner) for instructions on adding provisioners to the resource configuration.
...
Refer to the [`provisioner` block reference](/terraform/language/resource#provisioner) for details about the available arguments.

### Commands on a remote resource 

To run commands on a remote resource, add the `remote-exec` provisioner type. This provisioner lets you run the CLI for your target system in order to create, update, or otherwise interact with remote objects in that system. You must include [a `connection` block](/terraform/language/resources/provisioners#connect-to-remote-resources) for Terraform to connect to the server. 
...
For details about each provisioner type and its arguments, refer to the [`provisioner` block reference](/terraform/language/blocks/resource#provisioner). 
...
In the following example, the `provisioner "file"` block connects to the resource as the root user over SSH and copies the `/etc` directory to the resource. A second `provisioner "file"` block connects to the resource as an administrator user using WinRM to copy a file to the resource's `C:/App` directory.
...
Configure the following arguments in your `connection` block to establish an indirect connection with a bastion host over SSH. Refer to the [`connection` block reference](//terraform/language/blocks/resource#connection) for additional details:
...
Configure the following arguments in your `connection` block to establish a connection through an HTTP or SOCKS5 proxy over SSH. Refer to the [`connection` block reference](//terraform/language/blocks/resource#connection) for additional details:
...
You can use sensitive values in `provisioner` blocks. Terraform suppresses sensitive values in all log output. Refer to the following topics for more information about using sensitive values in your configurations:
...
- [`ephemeral` resources](/terraform/language/resources/ephemeral)
- [`ephemeral` local values](/terraform/language/values/locals#ephemeral-values)
- [`ephemeral` variables](/terraform/language/values/variables#ephemeral)
- [`ephemeral` output values](/terraform/language/values/outputs#ephemeral-avoid-storing-values-in-state-or-plan-files)
...
You can change the default tainting behavior by setting the `on_failure` attribute to `continue`. Refer to [`on_failure` argument reference](/terraform/language/resource#on_failure) for more information.
...
Enabling the [`create_before_destroy` argument](/terraform/language/resource#create_before_destroy) on the resource prevents destroy-time provisioners from running.

#### Remove a `resource` block containing a destroy-time provisioner

Destroy-time provisioners can only run if they remain in the configuration at the time a resource is destroyed. If a `resource` block with a destroy-time provisioner is removed entirely from the configuration, its provisioner configurations are removed along with it. The destroy provisioner won't run as a result. To work around this, configure a multi-step process to safely
...
You can specify multiple provisioners within a `resource` block. Terraform executes provisioners
