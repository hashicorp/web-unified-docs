...
- Declare a resource and specify its type and a name.
...
You must also specify a local name for the resource. Terraform uses the resource type and name as the resource's address in the [state file](/terraform/language/state). 
...
The `terraform_data` resource type implements the standard resource lifecycle, but does not directly take any other actions. Refer to [`terraform_data` resource reference](/terraform/language/resources/terraform_data) for more information.
...
- If you hard-coded values into your configuration, you can use dynamic language features to [make your module configurable](/terraform/language/parameterize/configure).
...
- When you no longer need a resource, learn how to [remove a resource from state](/terraform/language/resources/remove) or [destroy a resource](/terraform/language/resources/destroy).   
