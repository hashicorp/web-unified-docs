...
[the `provider` block reference](/terraform/language/block/provider) for additional information.
...
you must use [the `provider` meta-argument](/terraform/language/meta-arguments#provider)
...
- [`provisioner`, for taking extra actions after resource creation](/terraform/language/resources/provisioners#use-a-provisioner)
=======
- [`depends_on`, for specifying hidden dependencies](/terraform/language/meta-arguments#depends_on)
- [`count`, for creating multiple resource instances according to a count](/terraform/language/meta-arguments#count)
- [`for_each`, to create multiple instances according to a map, or set of strings](/terraform/language/meta-arguments#for_each)
- [`provider`, for selecting a non-default provider configuration](/terraform/language/meta-arguments#provider)
- [`lifecycle`, for lifecycle customizations](/terraform/language/meta-arguments#lifecycle)
...
A `removed` block may also contain a [Destroy-Time Provisioner](/terraform/language/resources/provisioners#destroy-time-provisioners), so that the provisioner can remain in the configuration even though the `resource` block has been removed.
...
You can write preconditions for resources in your configuration to help capture assumptions to help future maintainers understand your configuration design and intent. Validations return useful information about errors with context, helping consumers to diagnose issues in their configuration. Refer to [Validate your configuration](/terraform/language/validate#preconditions-and-postconditions) for more details.
