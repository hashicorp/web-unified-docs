...
This page describes how to integrate modules into your configuration so that you can reuse existing collections of resources. For information about the module workflow and key concepts, refer to [Modules overview](/terraform/language/modules). For instruction on how to create reusable child modules, refer to [Develop modules](/terraform/language/modules/develop).
...
Refer to the [`ref` parameter](/terraform/language/block/module#ref) in the `module` reference for more information.

### Specify input values

Module authors expose inputs that you can configure as attributes in the `module` block. Inputs let you customize the module's behavior when you call it in a parent module without modifying the module's source code. Refer to the module documentation for inputs that the module expects. In the following example, the module expects an integer value for the `servers` input:
