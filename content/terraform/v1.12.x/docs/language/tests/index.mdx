...
Assertion arguments in Terraform `run` blocks help your [validate your configuration](/terraform/language/test-and-validate/validate). Each `run` block contains a [`condition` argument](/terraform/language/expressions/conditionals) and an [`error_message` argument](/terraform/language/test-and-validate/validate#error-messages).
...
You can specify values for variables across all tests with the [Command line](/terraform/language/parameterize/configure#command-line-variables) and with [Variable definition files](/terraform/language/parameterize/configure#variable-definition-files).
...
By default, if any [validation](/terraform/language/test-and-validate/validate), including `check` block assertions, fails during the execution of a Terraform test file then the overall command reports the test as a failure.
