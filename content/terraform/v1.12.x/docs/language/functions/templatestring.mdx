...
last_modified: 2026-01-16T10:48:18+01:00
...
- The second argument is an object that specifies variables to use for rendering the template.
...
You can write an expression that builds a template dynamically and then assigns it to a [local value](/terraform/language/block/locals). You can then use a reference to that local value as the first argument to the `templatestring` function.
