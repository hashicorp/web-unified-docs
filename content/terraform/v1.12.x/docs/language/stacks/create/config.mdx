...
You can also define the following configuration blocks in a `tfcomponent.hcl` Stack configuration file:
- The [`component` block](/terraform/language/block/stack/configuration/component) lets you define reusable infrastructure modules in your Stack configuration.
- The [`required_providers` block](/terraform/language/block/stack/configuration/required_providers) lets you declare the providers your Stack configuration requires so that Terraform can install and use them.
- The [`provider` block](/terraform/language/block/stack/configuration/provider) lets you configure providers for your Stack components.
- The [`removed` block](/terraform/language/block/stack/configuration/removed) lets you define resources that you want to remove from your Stack.
- The [`import` block](/terraform/language/block/stack/configuration/import) lets you import existing resources into your Stack.
- The [`moved` block](/terraform/language/block/stack/configuration/moved) lets you move resources from one Stack to another.
- The [`variable` block](/terraform/language/block/stack/configuration/variable) lets you define input variables for your Stack.
- The [`locals` block](/terraform/language/block/stack/configuration/locals) lets you define local values that you can use within your Stack configuration.
- The [`output` block](/terraform/language/block/stack/configuration/output) lets you define outputs that you want to refer to in the HCP Terraform UI. # declare type
