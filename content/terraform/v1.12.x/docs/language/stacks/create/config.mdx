...
- The [`component` block](/terraform/language/block/stack/component) lets you define reusable infrastructure modules in your Stack configuration.
- The [`required_providers` block](/terraform/language/block/stack/required_providers) lets you declare the providers your Stack configuration requires so that Terraform can install and use them.
- The [`provider` block](/terraform/language/block/provider) lets you configure providers for your Stack components.
- The [`variable` block](/terraform/language/block/variable) lets you define input variables for your Stack.
- The [`output` block](/terraform/language/block/output) lets you define outputs that you want to refer to in the HCP Terraform UI.
- The [`locals` block](/terraform/language/block/locals) lets you define local values that you can use within your Stack configuration.
- The [`removed` block](/terraform/language/block/stack/removed) lets you define resources that you want to remove from your Stack.
- The [`import` block](/terraform/language/block/stack/import) lets you import existing resources into your Stack.
- The [`moved` block](/terraform/language/block/stack/moved) lets you move resources from one Stack to another.
