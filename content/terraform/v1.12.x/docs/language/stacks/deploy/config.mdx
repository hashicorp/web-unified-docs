...
You can define the following configuration blocks in a `tfdeploy.hcl` deployment configuration file:
- The [`deployment` block](/terraform/language/block/stack/deployment) lets you define how many times you want to deploy your Stack's infrastructure.
- The [`deployment_auto_approve` block](/terraform/language/block/stack/deployment_auto_approve) lets you define rules that automatically approve deployment plans based on specific conditions.
- The [`identity_token` block](/terraform/language/block/stack/identity_token) lets you define JSON Web Tokens (JWT) that Terraform generates for OIDC authentication with cloud providers.
- The [`store` block](/terraform/language/block/stack/store) lets you define access to external secret stores and variable sets from your deployment configuration.
- The [`publish_output` block](/terraform/language/block/stack/publish_output) lets you export values from your Stack that other Stacks in the same project can consume.
- The [`upstream_input` block](/terraform/language/block/stack/upstream_input) lets you consume outputs from another Stack in the same project and use them as inputs in your current Stack.
- The [`locals` block](/terraform/language/block/locals) lets you define local values that you can use within your deployment configuration.
