...
Adding the `ephemeral` argument to an output omits that value from state and plan files, but it adds restrictions to the values you can assign that output. To learn more about the `ephemeral` argument and handling and outputting sensitive data in your configuration, refer to [Sensitive data in state](/terraform/language/state/sensitive-data).
