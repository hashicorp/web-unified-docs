---
page_title: Make modules configurable
...
Add `variable` blocks in your configuration to let your module consumers assign values to those variables, letting users tweak module behavior without altering a module's source code. Variables define the interface of your module by specifying what values your module accepts as arguments.

Defining variables in your root module lets consumers pass in values at run time. Defining a `variable` block in a child module lets a parent module pass values to that child at run time. Learn more about [passing values to child modules](/terraform/language/modules/syntax#calling-a-child-module).
...
Defining variables gives your module consumer the flexibility to change values at run time, add `variable` blocks for each input you want to define for your module. To let your module consumers customize the instance type, subnet, and deployment environment of your web server at run time, add `variable` blocks for each input:
...
The `environment` variable uses an optional `validation` block to ensure the value a consumer assigns meets module requirements. Learn more about [validating variable values in your configuration](/terraform/language/test-and-validate/validate).
...
Module consumers can decide to change the environment, subnet, and instance type of the web server by inputting new values when they run this configuration, or they can leave the default values. If a variable does not have a default value, like the `subnet_id` variable, then Terraform prompts the user to assign a value before it generates a plan.
...
Terraform still stores the values of sensitive variables in your state. Adding the `ephemeral` argument omits a variable from state and plan files all together, but adds restrictions to the values you can assign that variable. To learn more about the `ephemeral` argument and handling sensitive data in your configuration, refer to [Sensitive data in state](/terraform/language/state/sensitive-data).
...
HCP Terraform and command-line assigned values take precedence over other ways of assigning variable values, and a variable's `default` argument is at the lowest level of precedence.

#### HCP Terraform

HCP Terraform lets you [assign values to variables in your configuration](/terraform/cloud-docs/workspaces/variables#terraform-variables) through workspaces. You can also group variables into sets that you can apply to multiple workspaces, and use access control to limit who can view existing variables or create new ones.
...
For readability, and to avoid shell escaping, we recommend setting complex variable values with [variable definition files](#variable-definition-files). For more details on shell quoting and the Windows command prompt, refer to [Input variables on the command line](/cli/commands/plan#input-variables-on-the-command-line).

#### Variable definition files

Variable definition files are ideal for managing different environment configurations, and simplify version control of your variable values. Create a variable definition file to assign multiple variable values in one file.

You can assign values directly to variable names in `tfvars` files. For example, the following file assigns variable values for a production environment:

<CodeBlockConfig filename="production.auto.tfvars">
...
Define a `locals` block when you want to reuse an expression throughout your configuration. You can define the `locals` block in any module, and the value you assign can be any valid Terraform expression, and can reference the following:
...
To reference the values from the `locals` block in your configuration, use `local.<NAME>` syntax. The `locals` block defines local values, and you reference those values on an object named `local`, which is singular.
...
Local values help avoid repetition and give a meaningful name to the value of an expression. However, they can make configuration harder to read because they obscure where values originate. Use local values in situations where you reuse a single value in many places to let you change a value in a single place, or when the value is the result of a complex expression.
...
Outputs let you expose information about your infrastructure on the command line, in HCP Terraform, and in other Terraform configurations. The `output` block serves four main purposes in Terraform:
...
Adding the `ephemeral` argument to an output omits that value from state and plan files all together, but adds restrictions to the values you can assign that output. To learn more about the `ephemeral` argument and handling and outputting sensitive data in your configuration, refer to [Sensitive data in state](/terraform/language/state/sensitive-data).
...
- [Test your Validation](/terraform/language/test-and-validate/validate) - Validate your configuration to improve your module consumer's troubleshooting, make your runs more predictable, and help your maintainers understand your configuration's intent.
- [Sensitive data in state](/terraform/language/state/sensitive-data) - Learn how to handle sensitive and ephemeral data in your configuration and state.
