...
For example, the following resource block manages an EC2 instance at the `address aws_instance.example`:

resource "aws_instance" "example" {
  # Resource arguments
}

To remove this resource from Terraform state without destroying the underlying infrastructure, replace the `resource` block with a `removed` block that uses the same resource address:
