...
For information on all of the ways to validation configuration in Terraform, refer to [Validate your configuration](/terraform/language/validate#input-variable-validation).
...
#### Validate Azure storage account settings

The following example contains two assertions that verify that the `azurerm_storage_account.example` resource uses TLS 1.2 and prevents nested items from becoming public:

```hcl
check "storage_account_security" {
 assert {
   condition     = azurerm_storage_account.example.min_tls_version == "TLS1_2"
   error_message = "The storage account must use TLS 1.2."
 }
 assert {
   condition     = azurerm_storage_account.example.allow_nested_items_to_be_public == false
   error_message = "The storage account must not allow nested items to become public."
 }
}
```
