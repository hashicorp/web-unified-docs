...
In the following example, Terraform queries for `azurerm_resource_group` resources using the AzureRM provider configuration with the `alternate` alias.
...
provider "azurerm" {
 alias = "alternate"
 features {}
 subscription_id = var.alternate_subscription_id
}
list "azurerm_resource_group" "alternate" {
