...
The following example declares the AzureRM provider and configures it with an Azure subscription ID:
...
variable "subscription_id" {
 description = "The Azure subscription ID used by the provider configuration."
 type        = string
}
provider "azurerm" {
 features {}
...
### Multiple AzureRM provider configurations

The following example defines a default AzureRM provider configuration and an additional configuration with the `alternate` alias:
...
Resources use the default AzureRM provider configuration unless you explicitly select the `azurerm.alternate` configuration.
...
### Select an alternate AzureRM provider configuration

The following example creates one Azure resource group with the alternate provider configuration and another with the default provider configuration:

```hcl
resource "azurerm_resource_group" "alternate" {
 provider = azurerm.alternate
 name     = "provider-block-alternate-rg"
 location = "West US 2"
}
resource "azurerm_resource_group" "default" {
 name     = "provider-block-default-rg"
 location = "East US"
}
```
...
### Use an alternate AzureRM provider configuration in a child module

The following example passes the `azurerm.alternate` provider configuration from the root module to a child module:
...
module "alternate_resource_group" {
 source = "./modules/resource-group"
 providers = {
   azurerm.alternate = azurerm.alternate
 }
 name     = "provider-block-module-rg"
 location = "West US 2"
}
```

The child module declares the provider alias using `configuration_aliases`:
