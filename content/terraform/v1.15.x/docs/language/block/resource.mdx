...
The following example uses the `azurerm` provider to create an Azure resource group and virtual network. Each `resource` block declares an Azure infrastructure object that Terraform manages.

The `azurerm_resource_group` resource creates a resource group in the `East US` region. The resource type is azurerm_resource_group and the resource label is example, so other resources can reference it as azurerm_resource_group.example.

```hcl
resource "azurerm_resource_group" "example" {
  name = "resource-block-example-rg"
  location = "East US"
}
```

The azurerm_virtual_network resource creates a virtual network in the resource group. It references attributes from azurerm_resource_group.example to use the resource group’s location and name. Terraform uses these references to determine that it must create the resource group before creating the virtual network.

```hcl
resource "azurerm_virtual_network" "example" {
 name = "resource-block-example-vnet"
