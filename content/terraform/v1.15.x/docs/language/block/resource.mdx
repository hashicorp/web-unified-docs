...
| `agent_identity` | When authenticating with an SSH agent, prefer this identity first. Set it to a path to a public or private key file (Terraform also tries `path.pub`), or to the agent key comment. Terraform matches that key among agent-loaded identities and tries it before the others, which helps when the agent holds more keys than the host will allow attempts. | string | `ssh` | none |
