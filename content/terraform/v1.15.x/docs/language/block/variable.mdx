...
### Sensitive variables with Azure

Set `sensitive` to `true` to prevent Terraform from displaying the variable's value in CLI output.
> **Note:** Terraform can still store sensitive values in state.

The following example defines a sensitive value for an Azure Key Vault secret:
...
Terraform redacts `secret_value` from CLI output while allowing the value to be used elsewhere in the configuration.
...
### Ephemeral variables with Azure

Set `ephemeral` to `true` to make a variable available during the current Terraform operation without storing its value in state or plan files.
Ephemeral variables are available in Terraform v1.10 and later.

The following example uses an ephemeral variable to configure the AzureRM provider with an Azure subscription ID:
...
```
Terraform uses the subscription ID during the operation without persisting the value in state or plan files.
