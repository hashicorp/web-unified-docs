...
Assertion arguments in Terraform `run` blocks help you [validate your configuration](/terraform/language/validate). Each `assert` block contains a [`condition` argument](/terraform/language/expressions/conditionals) and an [`error_message` argument](/terraform/language/validate#error-messages).
