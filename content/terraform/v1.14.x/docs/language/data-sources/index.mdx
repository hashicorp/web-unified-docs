...
Configure arguments in the body of the `data` block to define query constraints. Refer to your provider's documentation for available arguments. The following example instructs Terraform to query the [`aws_ami`](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/ami) data source. The query is constrained to the most recent AMI owned by the current user that has a `Name` tag key set to `app-server` and a `Tested` tag key set to `true`.
