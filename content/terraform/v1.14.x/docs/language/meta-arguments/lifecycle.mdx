...
In the following examples, Terraform ignores changes to `tags` so that a management agent can update them based on a rule set managed elsewhere. The Azure example uses a resource group to demonstrate the same behavior for Azure tags.
