...
- `-auto-approve` - Skips interactive approval of the plan before applying. Terraform ignores this
  ...
  Terraform interprets the act of passing the plan file as the approval. Note that this includes
  destructive operations such as deleting resources. If you use this option in automation,
  we recommend you first perform a plan operation and review the proposed changes before you apply them.
...
- `-invoke=action.<TYPE>.<LABEL>`: Specifies a provider action in the configuration file to invoke. Refer to [Invoke an action](/terraform/language/invoke-actions) for more information.
...
## Errors during an apply operation

When Terraform or a Terraform provider encounters an error during an apply step, it performs the following steps:

1. Logs the error and reports it to the console.
1. Updates the state file with any changes to your resources.
1. Unlocks the state file.
1. Exits the current operation.

Your infrastructure may be in an invalid state after a Terraform apply operation fails. Terraform does not automatically roll back a partially-completed apply. After you resolve the error, you must apply your configuration again to update your infrastructure to the desired state.
