...
| `deployments[].oidc` | Optional dynamic provider credential configuration detected on the source workspace. Omit this field when the workspace does not use dynamic provider credentials. |
| `deployments[].oidc.providers` | Dynamic provider credential configurations used by the deployment. Every deployment must use the same set of configurations. |
| `deployments[].oidc.providers[].family` | Provider family, such as `aws`, `azure`, or `gcp`. |
| `deployments[].oidc.providers[].tag` | Optional tag that associates a dynamic credential configuration with a provider alias. |
| `deployments[].oidc.providers[].audience` | Optional workload identity audience. When omitted, the provider's default audience applies. |
