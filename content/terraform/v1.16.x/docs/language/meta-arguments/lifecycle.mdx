...
| `on_failure` | Optional keyword specifying how Terraform behaves when an action fails. Valid values: `halt` (default — stops the apply operation and reports an error; the resource is not tainted), `continue` (logs the error and continues the apply operation), `taint` (stops the apply operation, reports an error, and marks the resource as tainted so Terraform replaces it on the next apply). | keyword | optional |
...
Use this rule as protection against accidentally replacing objects that may be costly to reproduce, such as database instances, storage, or other stateful resources. Enabling `prevent_destroy`, however, makes certain configuration changes impossible to apply
and prevents the `terraform destroy` command from operating once such objects are created. Use `prevent_destroy` sparingly.

You can use `prevent_destroy` in [`resource`](/terraform/language/block/resource) blocks. 
...
You can use `postcondition` in the following Terraform configuration blocks:
