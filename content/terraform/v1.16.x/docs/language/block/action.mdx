...
last_modified: 2026-01-12T14:57:22-05:00
...
```
The `config` block may contain arguments, nested blocks, or a combination of both. Refer to the provider documentation for arguments you can configure in the `config` block for the type of action you want to invoke. Within the `config` block a special symbol named `caller` is available, which represents the value of a resource instance invoking the action from an `action_trigger`.
