...
- Azure CLI authentication defaults to disabled. Enable it explicitly with `use_cli = true` or the selected `ARM_USE_CLI` variable. A missing backend credential does not fall back to an ambient CLI session.
