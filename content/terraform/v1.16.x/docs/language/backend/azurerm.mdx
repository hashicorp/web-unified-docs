...
Configure a federated identity credential on each identity for the same GitHub repository and workflow ref, with the `api://AzureADTokenExchange` audience. Set these GitHub Actions repository variables to the corresponding identity values:

| Repository variables | Identity |
| --- | --- |
...
The following `main.tf` persists the selector, disables backend CLI authentication, and creates a resource group in the AzAPI provider's subscription. Replace `<STATE_STORAGE_ACCOUNT_NAME>` with your existing state storage account name.
...
terraform {
  ...
  backend "azurerm" {
    environment_variable_suffix = "_BACKEND"
    use_cli                     = false
...
The following workflow initializes the backend and generates a plan. Unsuffixed `ARM_*` variables select the AzAPI identity. Suffixed `ARM_*_BACKEND` variables select the state identity. The PowerShell step copies GitHub's broker URL and bearer token into the backend namespace before running Terraform; it does not store them in backend configuration.
...
terraform {
  backend "azurerm" {
    use_oidc                           = true
    ado_pipeline_service_connection_id = "<SERVICE_CONNECTION_ID>"
