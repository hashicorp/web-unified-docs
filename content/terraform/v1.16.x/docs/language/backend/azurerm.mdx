...
- Terraform ignores native continuous integration (CI) fallbacks such as `ACTIONS_ID_TOKEN_REQUEST_*`, `SYSTEM_OIDCREQUESTURI`, `SYSTEM_ACCESSTOKEN`, and `AZURESUBSCRIPTION_SERVICE_CONNECTION_ID`, including suffixed variants of those names. Explicitly map broker values into the selected `ARM_OIDC_REQUEST_URL` and `ARM_OIDC_REQUEST_TOKEN` variables.
...
For suffix mode with `_BACKEND`, persist `environment_variable_suffix = "_BACKEND"` in the backend configuration and supply the state identity through suffixed variables. Set `ARM_ADO_PIPELINE_SERVICE_CONNECTION_ID_BACKEND` or its lower-priority alias `ARM_OIDC_AZURE_SERVICE_CONNECTION_ID_BACKEND` to the state identity's service connection ID. Neither the provider's unsuffixed service connection nor `AZURESUBSCRIPTION_SERVICE_CONNECTION_ID` is a fallback.

Make `System.AccessToken` available to the task as `SYSTEM_ACCESSTOKEN`. In the same PowerShell step that runs Terraform, map the pipeline broker values explicitly:

```powershell
$env:ARM_OIDC_REQUEST_URL_BACKEND = $env:SYSTEM_OIDCREQUESTURI
$env:ARM_OIDC_REQUEST_TOKEN_BACKEND = $env:SYSTEM_ACCESSTOKEN
```
...
The Azure CLI tasks reference both service connections and expose their identity metadata. The first task passes only non-secret backend metadata to the next task. Terraform authenticates through OIDC broker requests, not the tasks' CLI sessions: both `ARM_USE_CLI` flags are `false`.
...
steps:
  ...
  - task: AzureCLI@2
    displayName: "[Terraform] Read backend identity metadata"
    inputs:
  ...
  - task: AzureCLI@2
    ...
    inputs:
      ...
      inlineScript: |
        ...
        $env:ARM_OIDC_REQUEST_TOKEN = $env:SYSTEM_ACCESSTOKEN
        $env:ARM_OIDC_REQUEST_URL_BACKEND = $env:SYSTEM_OIDCREQUESTURI
        $env:ARM_OIDC_REQUEST_TOKEN_BACKEND = $env:SYSTEM_ACCESSTOKEN
        terraform init -input=false `
          "-backend-config=environment_variable_suffix=_BACKEND" `
    ...
    env:
      ...
      ARM_ADO_PIPELINE_SERVICE_CONNECTION_ID_BACKEND: $(STATE_SERVICE_CONNECTION_ID)
      ARM_USE_OIDC_BACKEND: "true"
      ARM_USE_AZUREAD_BACKEND: "true"
      ARM_USE_CLI_BACKEND: "false"
```

The provider and backend share the job's broker URL and bearer token but request assertions for their distinct service connection IDs. Both flows support token refresh. The backend does not use native `SYSTEM_*` or `AZURESUBSCRIPTION_SERVICE_CONNECTION_ID` fallbacks: the pipeline maps its broker inputs and service connection ID explicitly into suffixed `ARM_*` variables.
...
In default, unsuffixed mode, Azure DevOps token endpoint variables are automatically found. Supply `ado_pipeline_service_connection_id` or one of its environment fallbacks. Refer to [Example Configuration for Azure DevOps](#example-configuration-for-azure-devops) for service connection precedence, explicit broker mapping in suffix mode, and direct assertions without a service connection.
...
* `oidc_request_url` - (Optional) The URL of the OIDC broker from which to request an ID token. In unsuffixed mode, the backend uses the first non-empty value from `ARM_OIDC_REQUEST_URL`, `ACTIONS_ID_TOKEN_REQUEST_URL`, or `SYSTEM_OIDCREQUESTURI`. Suffix mode uses only the selected `ARM_OIDC_REQUEST_URL` variable; map the CI-native URL into it explicitly.

* `oidc_request_token` - (Optional) The bearer token for the request to the OIDC broker. In unsuffixed mode, the backend uses the first non-empty value from `ARM_OIDC_REQUEST_TOKEN`, `ACTIONS_ID_TOKEN_REQUEST_TOKEN`, or `SYSTEM_ACCESSTOKEN`. Suffix mode uses only the selected `ARM_OIDC_REQUEST_TOKEN` variable; map the CI-native bearer token into it explicitly.
