...
Persist the non-secret selector in your backend block or a non-secret `-backend-config` file. Terraform saves explicit backend configuration in `.terraform` and plan files, including an explicitly empty selector. It does not persist an environment-derived selector or environment-derived backend credentials. Supply fresh credentials independently to each plan and apply job. If you set the selector only through its control environment variable, supply it again for apply; omitting it restores unsuffixed behavior.
...
The following workflow initializes the backend and generates a plan. It passes only non-secret backend settings through `-backend-config`, persisting the suffix selector for later commands. Unsuffixed `ARM_*` variables configure AzAPI authentication; suffixed `ARM_*_BACKEND` variables configure backend authentication. The PowerShell step copies GitHub's broker URL and bearer token into the backend namespace before running Terraform; it does not store them in backend configuration.
      ...
      - name: Initialize and plan
        ...
        run: |
          ...
          terraform init -input=false `
            "-backend-config=environment_variable_suffix=_BACKEND" `
...
Both identities use GitHub's refreshable OIDC request flow, but exchange assertions in their respective tenants. An apply job must supply its own identity environment variables and repeat the broker mapping in the step that runs `terraform apply`. Do not reuse credentials from the plan job or pass short-lived tokens through `-backend-config`.
...
The first Azure CLI task initializes the backend with the state service connection and passes its non-secret identity metadata to the next task. The second task runs the plan with the provider service connection, while the backend continues to use the state connection. Both tasks run in the same job and working directory, so the plan reuses the initialized partial backend configuration.
...
An apply job must repeat the service connection setup and environment mappings with its own `System.AccessToken`. Keep credentials out of `-backend-config` and saved pipeline artifacts; only the non-secret backend settings belong in partial configuration.
