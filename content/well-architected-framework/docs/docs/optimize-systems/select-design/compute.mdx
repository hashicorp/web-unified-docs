...
Consul lets you define health checks that send HTTP requests, establish TCP connections, run local scripts, and more. The following example defines a health check that sends an HTTP GET request to the `/health` endpoint every 15 seconds. If the service returns an HTTP 200 response code, Consul considers the service healthy. 
