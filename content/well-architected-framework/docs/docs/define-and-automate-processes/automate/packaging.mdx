...
description: Package applications using containers and machine images. Automate builds with Packer for consistent deployments across all environments.
---
...
This template packages your application by copying the built application files from the `dist/` directory into the container, installing Node.js and production dependencies, creating a non-root user for security, and tagging the result with version numbers. Running `packer build` produces a complete Docker image containing your application that you can push to a container registry and deploy to Kubernetes or Nomad.

For a beginner-friendly introduction to Packer's Docker builder, visit the [Packer Docker get started tutorial](/packer/tutorials/docker-get-started/docker-get-started-build-image).

## Use Packer for machine image packaging
...
- Implement [semi-automated deployments](/well-architected-framework/define-and-automate-processes/process-automation/semi-automated) with Packer image builds
- Implement [fully-automated deployments](/well-architected-framework/define-and-automate-processes/process-automation/fully-automated) with CI/CD and Packer
- Read Packer [documentation](/packer/docs) and learn Packer with [tutorials](/packer/tutorials)
- Learn how to use [HCL templates](/packer/docs/templates/hcl_templates)
- Read the Packer Docker [builder documentation](/packer/plugins/builders/docker)
