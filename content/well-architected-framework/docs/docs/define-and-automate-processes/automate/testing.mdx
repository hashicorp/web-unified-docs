...
description: Automate testing for applications and infrastructure to catch issues early. Use Sentinel for policy enforcement and Terraform test for infrastructure validation.
---

# Automate testing

Testing automation validates that your applications and infrastructure work correctly before deployment, catching issues in development rather than production. Manual testing processes create errors, consume time, and become difficult to maintain as your infrastructure grows. Automate testing at multiple levels to ensure consistent quality, enforce compliance policies, and accelerate release cycles.
...
This Sentinel policy validates that all EC2 instances have encrypted root volumes and use approved instance types. When you run `terraform plan` in HCP Terraform, Sentinel evaluates this policy against the plan. If the policy fails, HCP Terraform blocks the apply operation and displays the policy violations, ensuring security requirements are enforced before any infrastructure changes occur. This prevents developers from accidentally deploying non-compliant resources.
...
The following example shows a Terraform test file that validates the EC2 instance configuration from the [deployments document](/well-architected-framework/define-and-automate-processes/automate/deployments):
...
- **Container image scanning:** Scan Docker images for security vulnerabilities using tools like Trivy or Anchore before pushing to your registry. Fail the build if critical vulnerabilities are found.
...
Integrate testing throughout your automation workflow to catch issues at each stage:

1. **Policy validation:** Before running `terraform plan`, Sentinel validates that your configuration meets security and compliance policies. Failed policies prevent the plan from proceeding.

1. **Infrastructure testing:** After Sentinel approval, Terraform test deploys your configuration to a test environment, validates that resources are created correctly, and automatically destroys the test infrastructure. Failed tests prevent promotion to production.

1. **Image scanning:** After Packer builds your application images, security scanners check for vulnerabilities before pushing images to registries. Images with critical vulnerabilities are rejected.

1. **Deployment validation:** After Terraform deploys your application, smoke tests validate that endpoints respond correctly and health checks pass. Failed smoke tests trigger automatic rollbacks.

This multi-layered approach ensures that issues are caught at the earliest possible stage, reducing production failures and accelerating your release cycles.

HashiCorp resources:

- [Sentinel documentation](/sentinel/docs) - Learn about policy-as-code and the Sentinel language
- [Sentinel policy examples](https://github.com/hashicorp/terraform-sentinel-policies) - Review sample policies for common use cases
- [Write Sentinel policies for Terraform](/terraform/tutorials/policy/sentinel-policy) - Tutorial on creating and testing Sentinel policies
- [Terraform test tutorial](/terraform/tutorials/configuration-language/test) - Complete guide to using Terraform's built-in testing framework
- [Terraform test documentation](/terraform/language/tests) - Reference for test file syntax and assertions
- [Testing Terraform modules](/terraform/language/modules/testing-experiment) - Best practices for module testing
- [HCP Terraform policy enforcement](/terraform/cloud-docs/policy-enforcement) - Configure policy sets in HCP Terraform
- [CI/CD integration examples](/terraform/tutorials/automation/github-actions) - Automate testing in GitHub Actions
...
- [Create CI/CD pipelines](/well-architected-framework/define-and-automate-processes/automate/cicd) that integrate testing at every stage
- Implement [zero-downtime deployments](/well-architected-framework/define-and-automate-processes/deploy/zero-downtime-deployments) with progressive testing strategies