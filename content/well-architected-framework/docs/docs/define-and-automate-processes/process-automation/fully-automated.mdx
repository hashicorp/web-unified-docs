...
Fully automated systems build on the best practices of [semi-automated systems](/well-architected-framework/define-and-automate-processes/process-automation/semi-automated) (VCS, automation tools, audit). Before implementing fully-automated deployments, ensure you have version control in place, scripted deployments, and immutable infrastructure patterns established through semi-automation.
...
- **Eliminate remaining manual bottlenecks:** Semi-automated deployments still require someone to execute scripts, coordinate timing, and manually trigger infrastructure changes. These manual steps create deployment bottlenecks, limit deployment frequency, and introduce timing errors when multiple teams coordinate changes. Fully-automated CI/CD eliminates these bottlenecks by triggering deployments automatically on code commits, enabling continuous delivery without human intervention.

- **Enable self-service infrastructure for development teams:** Semi-automated deployments require operations teams to execute scripts and provision infrastructure, creating dependencies and wait times for developers. Development velocity suffers when developers submit tickets, wait for approvals, and coordinate with operations for infrastructure access. Self-service platforms like HCP Waypoint let developers provision infrastructure independently through standardized workflows, eliminating operations bottlenecks while maintaining governance and security controls.

- **Detect and prevent issues before production:** Semi-automated deployments lack comprehensive automated testing and validation in the deployment pipeline. Teams catch issues after deployment to production, causing incidents, rollbacks, and service disruptions. Fully-automated pipelines integrate automated testing, security scanning, policy validation, and progressive deployment strategies that catch issues before they reach production, dramatically reducing mean time between failures (MTBF).

- **Scale deployment frequency without increasing risk:** Semi-automated deployments limit teams to weekly or monthly releases because each deployment requires coordination, manual validation, and risk acceptance. Organizations cannot deploy multiple times daily with manual processes. Fully-automated deployments with comprehensive monitoring, automated rollback, and progressive deployment strategies enable teams to deploy continuously with lower risk than infrequent manual deployments, accelerating feature delivery and bug fixes.
...
```python
# sentinel.hcl - Policy enforcement for automated deployments
...
These Sentinel policies automatically validate every Terraform plan in your CI/CD pipeline. When a developer tries to create a public S3 bucket, deploy an oversized instance, or skip required tags, Sentinel blocks the change and explains the violation. Approved changes that meet all policies proceed automatically to deployment without manual review. This automation enables rapid deployments while maintaining security and compliance standards. HCP Terraform enforces these policies before applying changes, preventing policy violations from reaching your infrastructure.
...
The following example shows an HCP Waypoint application template that enables developer self-service:

```hcl
# waypoint.hcl - Application template for developer self-service
...
# Custom actions for day 2 operations
action "restart-service" {
  use "exec" {
    command = ["nomad", "job", "restart", "api-service"]
  }
}

action "scale-up" {
  use "exec" {
    command = ["nomad", "job", "scale", "api-service", "5"]
  }
}
```

This HCP Waypoint template defines a complete application deployment workflow. Developers run `waypoint up` to build their application, push the container to AWS ECR, deploy it with Nomad, and register it with Consul—all without writing Terraform or understanding infrastructure details. Platform teams maintain the template, ensuring deployments follow organizational standards for resource limits, health checking, and service registration. The custom actions enable developers to restart services or scale capacity through HCP Waypoint's UI or CLI, performing day 2 operations without infrastructure access.

Once developers deploy the templates to create infrastructure, they must maintain their application. HCP Waypoint actions enable developers to perform day 2 operations to maintain their application, such as rotating secrets, rebuilding a database index, or flushing an application's cache. 
...
- [Implement semi-automated deployments](/well-architected-framework/define-and-automate-processes/process-automation/semi-automated) - Review foundational automation practices

Related resources for implementing fully-automated practices:

HashiCorp resources:
- Introduction to [Terraform](/terraform/intro), [Packer](/packer/docs/intro), [Vault](/vault/docs/what-is-vault), [Consul](/consul/docs/intro), [Nomad](/nomad/intro), and [Sentinel](/sentinel/intro)
...
- [Integrate Vault with CI/CD pipelines](/vault/tutorials/app-integration/github-actions)
...
- [Deploy applications with Nomad](/nomad/tutorials/get-started/get-started-install)
