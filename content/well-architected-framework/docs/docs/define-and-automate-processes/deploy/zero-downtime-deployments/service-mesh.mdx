...
- **Reduce blast radius with progressive rollouts:** Deploying to all users simultaneously increases the impact of malfunctions, in case of issues or bugs. Service mesh enables progressive traffic shifting (5%, 10%, 25%, 50%, 100%), allowing you to validate each stage before proceeding and limiting the impact of deployment issues to a small user subset.
...
This configuration routes 90% of traffic to version 1 (stable) and 10% to version 2 (canary). To gradually shift traffic, update the weights and apply the changes. Consul immediately adjusts traffic distribution without downtime.
