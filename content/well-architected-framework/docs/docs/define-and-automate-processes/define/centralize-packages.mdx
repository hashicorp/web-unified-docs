...
**Package and dependency management** (Artifactory, Nexus, Azure Artifacts):
- Stores application dependencies (npm packages, Maven JARs, Python wheels, Go modules)
- Acts as a proxy cache for public package registries
- Manages build artifacts from CI/CD pipelines
- See [Manage artifact repositories as code](/well-architected-framework/define-and-automate-processes/define/as-code/artifact-management) for configuration details

**Machine image management** (HCP Packer):
- Tracks and versions machine images (AMIs, Docker images, VM templates)
- Manages image promotion across environments (dev → staging → production)
...
Choose an artifact manager based on your needs:
- **JFrog Artifactory**: Universal repository supporting all major package formats with advanced features
- **Sonatype Nexus**: Open-source option with strong Maven support and enterprise add-ons
- **Cloud-native options**: Azure Artifacts for Azure DevOps, AWS CodeArtifact for AWS environments

Manually configuring artifact managers across environments can lead to configuration drift and inconsistencies. You can [define artifact managers as code](/well-architected-framework/define-and-automate-processes/define/as-code/artifact-management) with Terraform to ensure consistent deployment across development, testing, and production environments. Refer to that document for detailed configuration guidance for each platform.
...
Before using HCP Packer with Terraform, build and register images with HCP Packer. When Packer builds an image with HCP Packer integration enabled, it automatically registers the artifact metadata with HCP Packer, creating the bucket and channel structure referenced in Terraform configurations.
...
- [Package applications](/well-architected-framework/define-and-automate-processes/automate/packaging) to create artifacts for centralized storage
...
Get started with artifact management:
