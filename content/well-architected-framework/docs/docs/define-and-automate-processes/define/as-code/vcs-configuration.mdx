...
## Choose a VCS platform approach

When managing VCS configuration as code, you can manage GitHub, GitLab, or Bitbucket configurations:

**Use the GitHub provider when:**
- Your organization uses GitHub or GitHub Enterprise
- You need to manage repository settings, branch protections, and team permissions
- You want to standardize repository configuration across your organization

**Use the GitLab provider when:**
- Your organization uses GitLab (self-hosted or GitLab.com)
- You need to manage projects, groups, and CI/CD pipeline configurations
- You require approval rules and merge request settings as code

The examples below demonstrate both GitHub and GitLab configurations. The same principles apply to Bitbucket and other VCS platforms with Terraform providers.
...
When managing existing repositories, use `terraform import` to bring them under Terraform management without recreating them. For example: `terraform import github_repository.infrastructure infrastructure-production`. After importing, Terraform manages the existing repository configuration going forward.
...
variable "gitlab_token" {
  type        = string
  description = "GitLab access token used by the GitLab provider"
  sensitive   = true
}
...
The configuration retrieves the GitHub token from Vault's secret store, eliminating the need to store credentials in Terraform variables or environment variables. Before using this pattern, store your VCS access token in Vault using the Vault CLI (`vault kv put secret/github/terraform token=ghp_yourtoken`) or API. Vault's audit logs track all credential access, providing visibility into who accessed VCS credentials and when. When you rotate VCS tokens in Vault, all Terraform runs automatically use the updated credentials without modifying Terraform configurations.
