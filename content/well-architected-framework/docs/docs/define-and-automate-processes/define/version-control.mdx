...
description: Implement version control for infrastructure configuration to enable collaboration, change tracking, rollback capabilities, audit trails, and compliance requirements.
---
...
Storing infrastructure configuration in version control enables collaboration through code reviews, provides rollback capabilities when changes cause problems, creates audit trails for compliance requirements, and documents infrastructure evolution over time. Version control transforms infrastructure management from a manual, error-prone process into a collaborative, repeatable workflow. After implementing version control for your infrastructure code, you can [manage your VCS platform configuration as code](/well-architected-framework/define-and-automate-processes/define/as-code/vcs-configuration) to ensure consistent repository settings and security policies.
...
## Configure version control for infrastructure code

Infrastructure code requires specific version control configuration to prevent sensitive data exposure and ensure clean commit history. The following sections show essential configuration patterns for Terraform projects.

### Exclude sensitive files with .gitignore

Terraform generates files containing sensitive data and local state that should never be committed to version control. The following .gitignore file protects your repository:

<CodeBlockConfig filename=".gitignore">
...
# Terraform state files (contain sensitive data and resource IDs)
*.tfstate
*.tfstate.*
...
# CLI configuration files (contain credentials)
.terraformrc
terraform.rc
...
This .gitignore configuration prevents accidentally committing Terraform state files (which contain resource IDs and potentially sensitive outputs), variable files that might include secrets, and local Terraform directories. Always use remote state storage like HCP Terraform or S3 with state locking instead of committing state files.

### Implement branch protection strategy

Version control workflows for infrastructure code typically use a branch protection strategy that requires code review before changes reach production. The following branch strategy balances safety with productivity:

**Main branch protection:**
- Require pull request reviews before merging (minimum 2 approvals for production infrastructure)
- Require status checks to pass (Terraform plan, automated tests, policy checks)
- Prevent force pushes and branch deletion
- Require linear history (no merge commits)

**Feature branch workflow:**
1. Create feature branch from main: `git checkout -b feature/add-monitoring`
1. Make infrastructure changes and commit with descriptive messages
1. Push branch and create pull request with Terraform plan output
1. Address review comments and policy violations
1. Merge to main after approvals, triggering deployment

**Environment promotion pattern:**
- Development infrastructure in `dev` branch (auto-deploys on merge)
- Staging infrastructure in `staging` branch (requires approval)
- Production infrastructure in `main` branch (requires multiple approvals)

### Use descriptive commit messages

Infrastructure changes require context explaining what changed and why. Use the following commit message format for infrastructure code:
...
```text
feat: add monitoring and alerting for production database

- Configure CloudWatch alarms for CPU, memory, and connection count
- Set up SNS topic for alert notifications to on-call team
- Add dashboard showing key database metrics

Implements incident response requirement from security audit.
Prevents database performance issues from going undetected.
```

```text
fix: increase RDS connection pool size for application servers

- Update max_connections parameter from 100 to 200
- Adjust connection pool timeout to 30 seconds

Resolves connection exhaustion errors during peak traffic.
Application logs showed connection timeouts at ~95 connections.
```

Descriptive commit messages create an audit trail explaining infrastructure evolution and help team members understand the reasoning behind configuration decisions.
