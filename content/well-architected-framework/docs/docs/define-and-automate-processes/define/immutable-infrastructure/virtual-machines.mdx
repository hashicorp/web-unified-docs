...
description: Create immutable virtual machines by building images with Packer and deploying them with Terraform.
---
...
- **Prevent manual change errors:** Manual updates through SSH introduce human error—typos in commands, forgotten steps, or changes applied to wrong servers cause outages. Immutable virtual machines remove the need for manual server access, applying all changes through tested, version-controlled machine images.
...
You can improve the workflow include the following:
...
- Learn Packer with the Packer [documentation](/packer/docs) and [tutorials](/packer/tutorials)
...
- Use the [AWS AMI data source](/terraform/providers/aws/latest/docs/data-sources/ami) to find Packer images
- Learn about [Auto Scaling Groups](/terraform/providers/aws/latest/docs/resources/autoscaling_group) for immutable deployments
...
- Overview of [immutable infrastructure concepts](/well-architected-framework/define-and-automate-processes/define/immutable-infrastructure) - Overview of immutable infrastructure concepts
