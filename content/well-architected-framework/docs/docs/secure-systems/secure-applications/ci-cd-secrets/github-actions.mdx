...
- Vault policies that grant GitHub Actions the necessary read permissions for your secrets. Each policy should follow the principle of least privilege by specifying exact secret paths rather than using wildcards. Policies must be attached to JWT auth method roles that bind to specific GitHub claims like repository name, organization, workflow, or branch patterns.
...
The following best practices help you secure and operationalize Vault integration with GitHub Actions:

- **Configure JWT roles with bound claims:** When using OIDC authentication, configure Vault JWT auth method roles with bound claims that restrict authentication to specific repositories, organizations, workflows, or branch patterns. Use `bound_claims` to match GitHub's repository name, organization, workflow name, or branch to ensure only authorized workflows can authenticate. This prevents workflows from other repositories or organizations from accessing your secrets.

- **Set appropriate token TTLs:** Configure Vault token time-to-live (TTL) values that match your workflow execution duration. Short TTLs (10-30 minutes) work well for most workflows and reduce the risk window if tokens are compromised, while longer TTLs may be needed for extended deployment pipelines that run for hours.

- **Implement least-privilege policies per workflow:** Write Vault policies that grant each workflow or repository the minimum permissions required for its operation. Grant read access only to the specific secret paths each workflow needs, avoiding wildcard permissions or broad path access. Create separate policies for different workflow types (testing, deployment, release) with appropriate access levels.

- **Use OIDC instead of long-lived tokens:** Prefer GitHub's OIDC provider over storing long-lived Vault tokens in GitHub Secrets. OIDC authentication eliminates the need to manage, rotate, and secure static credentials while providing automatic token rotation for each workflow run. Only use AppRole or token-based authentication when OIDC is not available.

- **Secure workflow permissions:** Configure minimal GitHub Actions permissions in your workflow files. Use `id-token: write` permission only for steps that need to authenticate to Vault, and combine with `contents: read` or other specific permissions rather than granting broad `write` access. This follows the principle of least privilege at the workflow level.

- **Use dynamic secrets where possible:** Prefer dynamic secrets over static secrets for database credentials, cloud provider credentials, and other infrastructure access. Dynamic secrets provide automatic rotation, time-limited credentials, and better audit trails without requiring manual rotation workflows. Use the HashiCorp Vault Action with secrets engines like `aws/creds`, `database/creds`, or `pki/issue` to retrieve dynamic credentials.

- **Audit workflow secret access regularly:** Enable Vault audit logging and regularly review which workflows accessed which secrets. Monitor for unexpected access patterns, failed authentication attempts, or secrets accessed by workflows that should not need them for security investigations and compliance reporting. Use Vault's audit logs to track the repository, workflow name, and commit SHA for each secret access.

- **Validate Vault connectivity in workflows:** Include error handling in your workflows that validates Vault authentication and secret retrieval succeeded before proceeding with deployment steps. Check for empty secret values, authentication failures, or network errors. Fail the workflow early with clear error messages rather than attempting deployments with missing or invalid credentials.
...
- Read [Vault integration with GitHub Actions](https://www.hashicorp.com/en/blog/vault-integration-with-github-actions) for GitHub OIDC setup guide
