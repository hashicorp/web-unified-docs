...
- **Set short token TTLs:** Configure Vault token TTL values that match your build execution duration — typically 15-30 minutes for most builds. Short TTLs reduce the risk window if a token is leaked. The TeamCity Vault plugin automatically manages token renewal during build execution.
...
- Read [AppRole recommended pattern](/vault/docs/auth/approle/approle-pattern) for CI/CD-specific patterns including wrapped SecretID delivery and CIDR restrictions
