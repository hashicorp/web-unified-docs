...
**Secret sprawl across projects and variable groups:** Teams often duplicate secrets across multiple Azure DevOps projects, variable groups, and service connections. When secrets need rotation, administrators must update each copy manually. If a single project is compromised, attackers gain access to secrets duplicated across other projects.

**Limited variable group protection:** Azure DevOps variable groups provide basic secret storage with masking in logs, but secrets stored in variable groups remain valid indefinitely. Variable groups do not support automatic rotation, dynamic secret generation, or centralized policy management across multiple Azure DevOps organizations.

**Compliance and audit requirements:** Organizations need detailed audit trails showing which pipelines accessed which secrets and when. Azure DevOps logs pipeline runs and variable group access, but these logs do not capture individual secret reads, secret field access patterns, or failed authentication attempts required for SOC 2, PCI DSS, and HIPAA compliance.

**No dynamic credential support:** Static secrets stored in Azure DevOps variables remain valid indefinitely. If these secrets leak through logs, error messages, or compromised pipeline agents, they remain exploitable until manually rotated. Dynamic credentials expire automatically—typically after 1 hour to 24 hours—limiting exposure if leaked.
...
**Vault CLI with AppRole:** Install the Vault CLI on pipeline agents and authenticate using the [AppRole auth method](/vault/docs/auth/approle). AppRole works with any CI/CD platform and supports custom authentication workflows, but requires managing Role IDs and Secret IDs. Use AppRole when your environment does not support OIDC or managed identities.
