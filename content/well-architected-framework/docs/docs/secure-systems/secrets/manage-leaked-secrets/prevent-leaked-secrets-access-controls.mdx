...
description: Reduce secret exposure through separation of duties, least-privilege access controls, single sign-on authentication, and zero trust network access principles.
---

# Prevent leaked secrets with access controls

Organizations face constant risk from leaked secrets and exposed credentials. Access control strategies limit who can view, modify, and use secrets by implementing organizational policies and identity-based security controls.
...
**Insider threats cause 60% of data breaches:** Organizations that grant broad secret access to users and teams face significant risk from both malicious insiders and accidental exposure. Without access controls, a single compromised account can expose all organizational secrets.

**Compliance mandates access segregation:** Regulations like SOX, PCI-DSS, and HIPAA require separation of duties and least-privilege access to sensitive data. Organizations must prove they limit secret access to authorized personnel only.

**Shared credentials amplify breach impact:** When teams share service accounts and API keys, you cannot trace secret usage to specific users or revoke access granularly. This practice violates audit requirements and makes incident response difficult.

**VPN-based access violates least privilege:** Traditional remote access solutions grant broad network access once users authenticate. Attackers who compromise VPN credentials can pivot freely across your environment to discover and exfiltrate secrets.
...
- Creating role-based access policies that grant access to specific secret paths
- Using namespaces to isolate secrets between teams and projects
- Requiring approval workflows for access to production secrets
- Regularly reviewing and revoking unnecessary secret access
- Auditing all secret access attempts and policy changes
...
The policy demonstrates separation by environment (development vs. production), grants only necessary capabilities for each role, and explicitly denies access to administrative secrets. This approach ensures teams access only the secrets required for their work while preventing accidental or unauthorized access to sensitive credentials.
...
- Eliminating shared credentials for system access
- Providing session-specific credentials that expire automatically
- Creating audit trails that connect access to specific user identities
- Preventing lateral movement if credentials become compromised
- Supporting just-in-time access provisioning and deprovisioning

## HashiCorp resources

Get started with access controls:
...
- Follow the [Boundary OIDC authentication with Okta tutorial](/boundary/tutorials/identity-management/oidc-okta) for SSO integration
- Follow the [Vault OIDC authentication with Okta tutorial](/vault/tutorials/auth-methods/vault-oidc-okta) for identity provider integration
- Read the [HCP SSO documentation](/hcp/docs/hcp/iam/sso) for platform-wide authentication
- Read the [HCP Terraform SSO documentation](/terraform/cloud-docs/users-teams-organizations/single-sign-on) for team management

Advanced access features:

- Learn about [Vault as an OIDC identity provider](/vault/docs/secrets/identity/oidc-provider) for federated authentication
- Use the [Vault OIDC/JWT authentication method](/vault/docs/auth/jwt) for workload identity
- Learn about [Boundary host management](/boundary/tutorials/host-management) for dynamic infrastructure
- Learn about [Boundary credential management](/boundary/tutorials/credential-management) for session-based access
- Learn how to [connect to Kubernetes using Boundary](/boundary/tutorials/kubernetes-connect) for container access
