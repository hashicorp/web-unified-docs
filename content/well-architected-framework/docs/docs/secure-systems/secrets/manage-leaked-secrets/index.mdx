...
**Prevent financial damage:** Leaked secrets cost organizations $4.45 million on average according to IBM's 2024 Cost of a Data Breach Report. Data breaches resulting from exposed credentials impose massive financial burdens through incident response, forensics, legal fees, regulatory fines, and customer notification.
...
1. Code repositories are a common sources of secret exposure. Developers accidentally commit API keys, passwords, and tokens directly into source code. Once secrets are in code and pushed to repositories, either public or private, they become accessible to anyone with repository access and remain in the commit history until someone removes them.
...
1. Process friction is when the ability to securely access a secret is so difficult, people find a way to bypass the process to retrieve the secret. Process friction harms operational excellence at organizations, effectively lowering the security posture.
   ...
   Mitigate process friction using a centralizing secret management systems like Vault, creating  documentation and training for creating and accessing secrets, and reducing the risk of human error by automating the use of secrets through CI/CD systems.
