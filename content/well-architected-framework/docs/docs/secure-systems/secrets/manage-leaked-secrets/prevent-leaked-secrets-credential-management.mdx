...
Choose credential management strategies based on your security maturity and infrastructure:

- **Use dynamic credentials** when you need short-lived access to databases, cloud providers, or HCP Terraform without storing static credentials.
- **Use workload identity federation** when you need to eliminate bootstrap credentials for CI/CD pipelines, Kubernetes workloads, or cloud deployments.
- **Use centralized secret management** when you need to consolidate secrets from scattered locations into an auditable, encrypted platform with access controls.

Most organizations implement all three strategies together. They centralize existing static secrets, replace them progressively with dynamic credentials, and eliminate bootstrap credentials through workload identity.

## Why use dynamic credentials and centralized management

**Exposes credentials in version control history:** Long-lived API keys, database passwords, and service account credentials often end up committed to Git repositories, configuration files, and CI/CD systems. Once committed, these secrets remain accessible even after rotation.

**Prevents credential rotation at scale:** Organizations managing hundreds of applications and services cannot reliably rotate static credentials manually. Forgotten credentials remain active for months or years, increasing the window of exposure if leaked.

**Creates security visibility gaps:** When secrets exist in multiple locations, such as environment variables, configuration management systems, CI/CD platforms, and developer workstations, security teams lose visibility into what secrets exist and where they're used.

**Creates permanent unauthorized access:** Traditional approaches provision long-lived service accounts for application authentication. These credentials rarely rotate and provide permanent access if compromised, making them high-value targets for attackers.

Implementing credential management strategies eliminates long-lived static secrets. These strategies include dynamic credentials with workload identity federation and centralized secret storage. Together, they provide centralized visibility and control over all organizational credentials.
...
The workload identity federation workflow includes the following steps:
...
- Follow the [HCP Vault Dedicated quickstart](/vault/tutorials/get-started-hcp-vault-dedicated) for managed deployment
- [Sign up for HCP Vault Dedicated](https://portal.cloud.hashicorp.com/sign-in)

Dynamic credentials for platforms and services:
...
- Follow the tutorial to [enable self-service workflows with Vault-backed dynamic credentials](/terraform/tutorials/cloud/dynamic-credentials-no-code) for no-code provisioning
- Follow the tutorial to [inject secrets into Terraform using the Vault provider](/terraform/tutorials/secrets/secrets-vault) for runtime retrieval

Workload identity federation:

- Follow the tutorial to [manage federated workload identities with Vault](/vault/tutorials/enterprise/plugin-workoad-identity-federation) for trust-based authentication
