...
- [Generate dynamic database credentials with Vault](/vault/tutorials/db-credentials) for time-limited access
- [Store and version secrets with Vault's KV engine](/vault/tutorials/secrets-management/versioned-kv) for static secrets
- [Securely store secrets for CI/CD pipelines](/well-architected-framework/secure-systems/secure-applications/ci-cd-secrets) in automated workflows
- Read the [Vault Key/Value (KV) secrets engine documentation](/vault/docs/secrets/kv/kv-v2) for configuration details
- Learn how to [enable transparent data encryption for Microsoft SQL with Vault](/vault/docs/platform/mssql) for database security
