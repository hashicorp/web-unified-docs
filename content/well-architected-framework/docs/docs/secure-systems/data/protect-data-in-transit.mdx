...
- [Vault's Transit secrets engine documentation](/vault/docs/secrets/transit)
- [Encrypt data in transit using Vault's Transit engine](/vault/tutorials/encryption-as-a-service/eaas-transit)
- [Explore Vault's encryption capabilities](/vault/tutorials/encryption-as-a-service)
- [Transform sensitive data with format-preserving encryption](/vault/tutorials/adp/transform)
- [Update Consul agents to securely communicate with TLS](/consul/tutorials/archive/tls-encryption-secure-existing-datacenter)
- [Enable TLS on Consul ingress gateways](/consul/docs/north-south/ingress-gateway#custom-tls-certificates-via-secret-discovery-service-sds)
- [Enable TLS on Consul API gateway listeners](/consul/docs/api-gateway/configuration/gateway#listeners-tls)
