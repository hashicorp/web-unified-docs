...
| Custom tool metrics | <li>`mcp_tool_calls_total`</li><li>`mcp_tool_errors_total`</li><li>`mcp_tool_duration_seconds`</li></ul> |  The server uses MCP hooks to call the `BeforeCallTool` and `AfterCallTool` tools, which emit the metrics.  |

### Important scope note

You must use the streamable HTTP server path to collect metrics. Streaming metrics is not supported on the default stdio path.
