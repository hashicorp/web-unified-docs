...
created_at: 2026-04-01T22:23:45-06:00
last_modified: 2026-04-08T00:18:29.000Z
...
1. Verify [Kiro CLI](https://kiro.dev/docs/cli/) or [Kiro IDE](https://kiro.dev/downloads/) is installed.
1. Verify MCP support is configured, refer to the [Kiro CLI MCP documentation](https://kiro.dev/docs/cli/mcp/) or [Kiro IDE MCP documentation](https://kiro.dev/docs/mcp/) for more information. 

    For Kiro IDE, enable MCP support by pressing the `Cmd+,` or `Ctrl+,` keys to open the **Settings** page, then search for MCP.
1. Create or open either the global `mcp.json` file at `~/.kiro/settings/mcp.json` or the local file at `<project-root>/.kiro/settings/mcp.json`. 
1. Add following configuration to your `mcp.json` file:
...
Terraform MCP server generates the metrics and exports them to the configured OTLP endpoint. A remote instance of the server can then either receive or forward the telemetry to Datadog through its own collector or forwarding setup. This repo produces OTel metrics, but the remote repo is responsible for routing them to Datadog.
