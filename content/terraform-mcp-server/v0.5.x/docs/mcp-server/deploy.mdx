...
Terraform MCP server uses OpenTelemetry to collect telemetry metrics. Set the `OTEL_METRICS_ENABLED` environment variable to `true` to enable metrics. In HTTP mode, the server implements the following lifecycle:
...
Refer to the [telemetry-metrics reference](/terraform-mcp-server/v0.5.x/docs/mcp-server/reference.mdx#telemetry-metrics) for more information
