...
Terraform MCP server generates the metrics and exports them to the configured OTLP endpoint. A remote instance of the server can then either receive or forward the telemetry to Datadog through its own collector or forwarding setup. This repository produces OTel metrics, but the remote repo is responsible for routing them to Datadog.
Refer to [Telemetry metrics](/terraform/docs/tools/mcp-server/reference#telemetry-metrics) for more information.
