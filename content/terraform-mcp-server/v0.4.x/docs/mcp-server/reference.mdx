...
The server has 3 toolsets:

| Toolset | What it contains |
| `registry` | Tools for accessing the public registry |
| `registry-private` | Tools for accessing private registries |
| `terraform` | Tools for interacting with TFC/TFE |

These tools can be enabled or disabled using the `--toolsets` argument supplied to the server binary. 
...
| `get_token_permissions` | Get the permissions for the TFE_TOKEN | A list of actions the user can take |
