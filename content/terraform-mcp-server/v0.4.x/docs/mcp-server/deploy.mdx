...
1. Create or edit the `mcp.json` file at `~/.kiro/settings/mcp.json` (global) or `<project-root>/.kiro/settings/mcp.json` (local) with the following configuration:
...
1. Verify [Kiro CLI](https://kiro.dev/docs/cli/) or [Kiro IDE](https://kiro.dev/downloads/) is installed.
1. Verify MCP support is configured, refer to the [Kiro CLI MCP documentation](https://kiro.dev/docs/cli/mcp/) or [Kiro IDE MCP documentation](https://kiro.dev/docs/mcp/) for more information. For Kiro IDE, enable MCP support via **Settings** (`Cmd+,` / `Ctrl+,`) and search for "MCP".
