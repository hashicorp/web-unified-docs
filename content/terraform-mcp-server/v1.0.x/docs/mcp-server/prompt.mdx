...
Your agent client may use specific characters to invoke tools from connected MCP servers. For example, use `#terraform` in the Visual Studio Code chat interface to direct your connected model to use the Terraform MCP server. Refer to the documentation for your client or model for instructions on how to trigger specific behaviors.  
