...
- HCP Terraform and Terraform Enterprise support: Full workspace management, list organizations and projects, and private registry access.
...
1. List organizations and workspaces in HCP Terraform and Terraform Enterprise.
1. Run operations that manage workspaces, variables, tags, and variable sets in HCP Terraform or Terraform Enterprise.
...
- MCP host: AI application or environment in a language model that performs AI-driven tasks that operate the MCP client, such as Claude Desktop.
...
- MCP transport: Handles the underlying communication of how the MCP client sends and receives messages over the JSON-RPC 2.0 protocol. The stdio transport allows the MCP server to invoke tools directly using the standard input/output pipe. The streamable HTTP transport exposes server communication over HTTP or HTTPS to receive and respond to MCP tool calls.
