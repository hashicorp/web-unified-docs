...
1. If you intend to connect the server to your HCP Terraform or Terraform Enterprise account, create an API token for authentication.  
...
The requirements depend on how your installation environment is configured. 
...
Follow security best practices any time you generate a token for accessing your Terraform web UI. Refer to [Secure configuration](/terraform/mcp-server/deploy#secure-configuration) for additional information. 
...
1. Verify MCP support is enabled. Refer to the [VS Code MCP documentation](https://code.visualstudio.com/docs/copilot/chat/mcp-servers) for more information.
...
1. Verify MCP support is enabled. Refer to [Cursor's MCP documentation](https://docs.cursor.com/context/model-context-protocol#configuring-mcp-servers) for more information.    
1. To use the MCP server in all workspaces, add the following configuration to your user settings JSON file:
...
1. Verify MCP support is enabled. Refer to [Claude Desktop's MCP documentation](https://modelcontextprotocol.io/quickstart/user) for more information.    
...
1. Verify MCP support is enabled. Refer to [Amazon Q's MCP documentation](https://docs.aws.amazon.com/amazonq/latest/qdeveloper-ug/qdev-mcp.html) for more information.    
...
1. Verify MCP support is enabled. Refer to [Bob's MCP documentation](https://bob.ibm.com/docs/ide) for more information.
...
Terraform MCP server uses OpenTelemetry to collect telemetry metrics. Set the `OTEL_METRICS_ENABLED` environment variable to `true` to enable metrics. You must use the streamable HTTP server path to collect metrics. Streaming metrics is not supported on the default stdio path. 
