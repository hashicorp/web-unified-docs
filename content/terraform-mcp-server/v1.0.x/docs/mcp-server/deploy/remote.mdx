...
Choose the environment for your deployment. You can deploy the server into several types of environments, such as running the compiled binary in a cloud instance or running the containerized image in Docker, Fargate, or other container. 
...
You must configure the server with a baseline API token for authentication by setting the `TFE_TOKEN` variable and the HCP Terraform or Terraform Enteprise instance by setting the `TFE_ADDRESS` variable. Note that the server supports presenting multiple tokens using HTTP headers. Refer to [Configure local agents](#configure-local-agents) for more information.
...
Instead of using the baseline token the server is configured to present, you can also configure your client to pass your user API token to the remote server for authentication with HCP Terraform or Terraform Enterprise. This lets different clients access resources based on their RBAC permissions so that mutliple users with different access levels can access data they need using a centrally managed MCP server.  
