...
Instead of using the baseline token the server is configured to present, you can also configure your client to pass your API token to the remote server for authentication with HCP Terraform or Terraform Enterprise. This lets different clients access resources based on their RBAC permissions so that multiple users with different access levels can access data they need using a centrally managed MCP server.  
