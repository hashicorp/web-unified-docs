...
You must configure the server with a baseline API token for authentication by setting the `TFE_TOKEN` variable to your API token. Note that the server supports presenting multiple tokens using HTTP headers. Refer to [Configure local agents](#configure-local-agents) for more information.
