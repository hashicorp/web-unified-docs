...
### Note on UI token renewal

The Vault UI does not provide a configurable session timeout. Instead, the GUI uses
a built-in timeout based on your token's time-to-live (TTL) or lease duration (your
authentication method type determines whether Vault sets TTL or lease duration).

The Vault UI automatically renews your token at half of its TTL or lease duration,
whichever Vault sets. After three minutes of inactivity, which starts after your last
request to the Vault API, the GUI stops renewing your token.
The GUI then lets your token expire when it reaches its TTL or lease duration.
Auto-renewal resumes when you make another request to the Vault API.

For example, assume your token TTL is 10 minutes. If you become inactive at minute 1,
the Vault UI stops renewing your token at minute 4. Your token expires at minute 10, and the
session times out. You cannot interact with the GUI until you re-authenticate.
If you make another request to the API, such as navigating to a new page, the GUI resumes
auto-renewal and resets the three-minute inactivity timer.
...
Vault does not provide an explicit user-configurable session timeout. If this feature
would benefit your organization, contact your account team.
