...
<Tip title="License location recommendation">
  A config file approach to licenses may be less disruptive as
  changes can be made dynamically without the need for a restart.

  Using Environment variables will typically require a restart of
  the Vault processes or its pods.
