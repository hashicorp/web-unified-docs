...
Vault Enterprise automatically checks the following locations, in order,
and stops once it finds a valid license.
...
Only the first applicable option will be selected and the rest are ignored.

<Tip title="License location recommendation">
  A config file approach to licenses may be less disruptive as you can make
  changes dynamically without the need for a restart. If you use environment
  variables, changing the value typically requires a restart of the Vault
  processes or its pods.
