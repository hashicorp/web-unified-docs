...
last_modified: 2026-06-25T17:23:42.000Z
...
- **Set `deny_slash_in_templated_paths`**. We recommend setting
  [`deny_slash_in_templated_paths`](/vault/docs/configuration#deny_slash_in_templated_paths)
  to `true` in your Vault configuration. This prevents unintended access by
  ensuring that Vault returns a permission denied response when slashes are
