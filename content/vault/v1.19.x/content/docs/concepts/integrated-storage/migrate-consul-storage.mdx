...
the window before replication reaches it. Vault does not guarantee read-after-write consistency, even across requests to the same standby. Read-after-write inconsistency does not exist with Consul storage, it is a specific consequence of using integrated storage with performance standbys.
