...
last_modified: 2026-06-25T17:23:40.000Z
...
Using wildcards (`+`) and globs (`*`) in the rendered output of identity
templates will result in a "permission denied" response from Vault.

The [`deny_slash_in_templated_paths`](/vault/docs/configuration#deny_slash_in_templated_paths)
configuration option controls whether using slashes (`/`) in the rendered
output of identity templates will result in a "permission denied" response from
Vault.
