...
| Change      | Affected version | Fixed version
| ----------- | ---------------- | -------------
| Known issue | 1.19.x+ent before 1.19.11+ent | 1.19.11+ent

Vault interprets `rotation_schedule`s relative to the local time zone of the machine when initially set.
After a restart or leadership change, Vault will interpret `rotation_schedule`s relative to UTC. This may result
in a shift in rotation time depending on the time zone offset of the Vault instance.

In version 1.19.11+ent or later, Vault interprets `rotation_schedule`s relative to UTC in all cases.
