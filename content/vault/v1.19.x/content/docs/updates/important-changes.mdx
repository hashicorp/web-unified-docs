...
As a result, Vault blocks secrets sync operations to private IP ranges for all
...
In Vault 1.19.x and newer the [disable_strict_networking](https://developer.hashicorp.com/vault/api-docs/system/secrets-sync#disable_strict_networking) configuration parameter has been added, which allows disabling both the IP address and port number restrictions used by the SSRF-safe sync clients.
