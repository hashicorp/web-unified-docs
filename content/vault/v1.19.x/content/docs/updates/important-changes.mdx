...
When you set a `rotation_schedule`, Vault interprets it relative to the local time zone
of the machine. After a restart or leadership change, Vault interprets the `rotation schedule` relative
to UTC, which may result in shifted rotation times depending on the time zone offset of the Vault instance.

As of Vault Enterprise v1.19.11, Vault interprets `rotation_schedule`s relative to UTC in all cases.
