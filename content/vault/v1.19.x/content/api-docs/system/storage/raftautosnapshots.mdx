...
- `aws_access_key_id` `(string)` - AWS access key ID. Make sure that this account has 
  the following permissions in S3, so that Vault can store and manage the snapshots:
  - `s3:ListBucket`
  - `s3:PutObject`
  - `s3:DeleteObject`
