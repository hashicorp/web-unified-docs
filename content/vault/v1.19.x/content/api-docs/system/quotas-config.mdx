...
- `absolute_rate_limit_exempt_paths` `([]string: [])` - Specifies the set of
  absolute paths exempted from all rate limit quotas. Leave
  `absolute_rate_limit_exempt_paths` empty to ensure Vault applies rate limits
  to all paths. You must call the endpoint from the root namespace to apply or
  modify rate limit exemptions.
  and can only be set from the root namespace.
