...
last_modified: 2026-02-18T02:31:57.000Z
...
- `provider_config` `(map: <optional>)` - Configuration options for provider-specific handling. Providers with specific handling include: Azure, Google, SecureAuth, IBM ISAM. The options are described in each provider's section in [OIDC Provider Setup](/vault/docs/auth/jwt/oidc-providers). You can include the following optional parameter in the `provider_config` object:

   - `fetch_groups` `(bool: false)` - *Applicable to Azure (Entra ID) with the [JWT login](/vault/api-docs/auth/jwt#jwt-login) flow only.* When set to `true`, Vault fetches groups directly from the Microsoft Graph API [`me/getMemberObjects`](https://learn.microsoft.com/en-us/graph/api/directoryobject-getmemberobjects) endpoint instead of relying on the groups claim in the token. This bypasses both the 200-group token limit and the per-application 1,000 group assignment limit in Microsoft Entra ID. Requires a `distributed_claim_access_token` during [JWT login](/vault/api-docs/auth/jwt#jwt-login) so that Vault can authenticate to the Microsoft Graph API.
...
- `distributed_claim_access_token` `(string: <optional>)` - An access token that Vault uses to fetch group
  memberships specified by the distributed claim source in the JWT. This parameter only applies to the
  Azure (Entra ID) provider, where Vault uses the token to retrieve additional groups from the Microsoft
  Graph API.
