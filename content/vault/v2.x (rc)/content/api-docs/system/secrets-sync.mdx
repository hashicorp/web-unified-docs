...
- `secret_access_key` `(string: "")` - Secret access key to authenticate against the AWS Secrets Manager. Must be provided with `access_key_id`.
  If omitted, authentication falls back to the AWS credentials provider chain.
...
- `region` `(string: "")` - Region where to manage the Secrets Manager entries. If omitted, configuration falls back to
...
#### Sync secrets to AWS Secrets Manager with static credentials:
```json
{
    "access_key_id": "AKI***",
...
#### Sync secrets to AWS Secrets Manager with workload identity federation (WIF):
```json
{
