...
- `client_secret` `(string: "")` - Client secret of an Azure app registration. Required for client secret authentication. Mutually exclusive with `identity_token_audience`. If omitted, authentication falls back to workload identity federation (WIF) when `identity_token_audience` is set, or Azure managed identity when neither is provided.
