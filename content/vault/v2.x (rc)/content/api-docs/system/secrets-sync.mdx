...
- `identity_token_audience` `(string: "")` - Audience claim value for plugin identity tokens used in workload identity federation (WIF) authentication. The value must match the allowed audiences configured for the federated identity credential in Azure. Required for WIF authentication. Mutually exclusive with `client_secret`.

- `identity_token_key` `(string: "")` - Name of a configured [identity token key](/vault/api-docs/secret/identity/tokens) to use for signing plugin identity tokens for WIF authentication. If omitted, the default plugin identity token signing key is used.

- `identity_token_ttl` `(duration: "1h")` - Time-to-live for generated plugin identity tokens used in WIF authentication. Accepts duration format strings such as "30m" or "1h". Must be non-negative.
