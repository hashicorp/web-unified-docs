...
## Read OpenID configuration for secret sync identity token issuer <EnterpriseAlert product="vault" inline />

Use the `.well-known` endpoint to retrieve an
[OpenID Provider Configuration Response](https://openid.net/specs/openid-connect-discovery-1_0.html#ProviderConfigurationResponse)
with a set of claims about the secret sync identity token issuer.

| Method | Path                                             |
| :----- | :----------------------------------------------- |
| `GET`  | `identity/oidc/secrets-sync/.well-known/openid-configuration` |
...
$ curl \
    --request GET \
    ${VAULT_ADDR}/v1/identity/oidc/secrets-sync/.well-known/openid-configuration
```
...
```json
{
  "issuer": "https://example.com/v1/identity/oidc/secrets-sync",
  "jwks_uri": "https://example.com/v1/identity/oidc/secrets-sync/.well-known/keys",
  "response_types_supported": [
    "id_token"
  ],
  "subject_types_supported": [
...
## Read the public JWKS  from a secrets-sync identity token issuer <EnterpriseAlert product="vault" inline />

Query the `.well-known/keys` endpoint to retrieve the public portion of named keys. Clients can use the endpoint to validate the authenticity of an identity token.
...
$ curl \
    --request GET \
    http://127.0.0.1:8200/v1/identity/oidc/secrets-sync/.well-known/keys
```
