...
This endpoint returns high-quality randomm bytes of the specified length. Up
to 10MB may be fetched from the platform source or if using the DRBG option.
128KB may be fetched from seal/all sources using entropy augmentation to
avoid depleting HSM entropy.
...
- `drbg` `(string: "")` - When set to "auto" or "hmacdrbg", seeds a
  cryptographically secure random generator with a seed from the chosen
  source in order to efficiently generate a larger volume of random bytes.
