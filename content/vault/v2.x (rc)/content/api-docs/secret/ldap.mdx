...
- `self_managed` `(bool: false)` - <EnterpriseAlert inline="true" />
  Configures the plugin mount as self-managed. Self-managed mounts do not support
  automated plugin features like dynamic roles. You must manage static roles
  manually with self-managed mounts and you cannot change `self_managed` after
  creation. 
...
- `password` `(string: "")` - The current password of the LDAP entry. You must
  ...
  Vault uses `password` to rotate the role password in the LDAP system. For
  non-self-managed configurations, Vault returns `password` as the role password
  until it rotates the associated credentials. 
...
Vault uses the same rotation manager to manage static role rotation and
automated root rotation.

When you first unseal Vault after an upgrade, it automatically begins migrating
any existing static roles from a plugin-managed rotation system to the rotation
manager.
...
The `GET` endpoint returns statistics about in-progress migration attempts.

You can manually trigger a migration process with the `POST` endpoint if you
have static roles that have not migrated yet. The endpoint returns a warning if
Vault is already in the process of migrating the role.
