...
- `self_managed` `(bool: false)` - <EnterpriseAlert inline="true" />
  Configures the plugin mount as self-managed. Self-managed mounts will only support
...
- `include_role_names` `(boolean: <required>)` – Includes lists of migrated role names and unmigrated role names when reading
  the migration status. `include_role_names` is required to be set to either true or false and has no default value.

The `GET` endpoint returns statistics about in-progress migration attempts or nothing if no migration is active. Include
`include_role_names` to list all role names that are finished migrating or still need to be migrated. Be aware that this
can result in a very large response if there are many static roles.
...
You can manually trigger a migration process with the `POST` endpoint if you have static roles that have not
migrated yet. The endpoint returns a warning if Vault is already in the process of migrating roles, or if
