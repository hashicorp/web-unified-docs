...
last_modified: 2026-02-18T02:32:18.000Z
...
- `binddn` `(string: "")` - Distinguished name (DN) of object to bind for managing user entries.
  For example, `cn=vault,ou=Users,dc=hashicorp,dc=com`. You must set `binddn` for non-self-managed plugin
  configurations.
...
### Sample non-self-managed payload
...
- `password` `(string: "")` - The current password of the LDAP entry. You must
  set `password` for self-managed configurations. For self-managed configurations,
  Vault uses `password` to rotate the password for the role in the LDAP system. For
  non-self-managed configurations, Vault returns the value set in `password` as a starting password
...
- `rotation_policy` `(string: "")` – <EnterpriseAlert product="vault" inline /> 
  The name of the rotation policy to apply when the rotation manager is rotating
  the credential. The rotation manager uses the values defined in the rotation
  ...
  [rotation policy documentation](/vault/docs/enterprise/rotation-management-system/rotation-policies) for more information.
