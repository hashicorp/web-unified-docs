...
- `password` `(string: "")` - The current password of the LDAP entry. You must
  ...
  Vault uses `password` to rotate the role's password in the LDAP system. For
  non-self-managed configurations, Vault returns the set `password` as the role's password
  until it is rotated.
