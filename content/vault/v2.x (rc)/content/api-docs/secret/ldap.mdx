...
- `bindpass` `(string: <optional>)` - Password to use along with `binddn` for managing user entries. Bindpass
  cannot be set for self-managed plugin configurations.
- `self_managed` `(bool: false)` - <EnterpriseAlert inline="true" /> Set to true to configure the plugin mount as
  self-managed. A bindpass will not be required when enabled. **NOTE:** Only self-managed static roles will be
  supported under this mount and other plugin features, such as dynamic roles, will be unavailable. This field
...
- `username` `(string: <required>)` - The username of the existing LDAP entry to manage
  ...
  **Example:** `"bob"`
- `password` `(string: <optional>)` - The current password of the LDAP entry. This field is required for
  self-managed configurations, but optional for non-self-managed configurations. For self-managed configurations,
  this password will be used for the role to rotate its own password in the LDAP system. When set for non-self-managed
  configurations, Vault will use this as the initial password and return it when credentials are requested up until
  the first rotation occurs. 
...
Starting with Vault version 2.0.0 and above, static role rotation is managed by Vault Enterprise's Rotation
Manager system, the same system already used for automated root rotation. When Vault is first unsealed after an upgrade,
it will automatically begin a migration process for all existing static roles to move from a plugin-managed rotation
system to Rotation Manager. This process runs in the background and will not prevent ordinary operation of the plugin during its execution. Individual static role rotations will be paused until they get migrated, at which point they will
automatically resume their normal rotation schedule.

The `static-migration` endpoint provides some visibility into the migration process and is intended for operational
awareness and troubleshooting purposes if intervention is required.
...
The `GET` endpoint returns several statistics of the migration process currently running or if there is no migration
in progress. The `POST` endpoint starts the migration process if there are any static roles that haven't been migrated
yet or reports that there is already a migration in progress.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    http://127.0.0.1:8200/v1/ldap/static-migration
```
...
The `POST` endpoint to the `/stop` path interrupts the active migration process if one is running.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    http://127.0.0.1:8200/v1/ldap/static-migration/stop
```
