...
last_modified: 2026-02-18T02:23:52.000Z
...
- `enable_unauthenticated_access` `(string array: [])` - The list of endpoint family names that are normally authenticated
  which should be made unauthenticated.  These were originally unauthenticated in older Vault versions, and this configuration
  allows for maintaining the old behavior when necessary. This can be updated on a running Vault process with a SIGHUP signal.
  Valid values that can be in the list: "rekey", "generate-root", and "generate-operation-token".
