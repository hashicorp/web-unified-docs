...
The `reporting` stanza specifies various parameters for tuning reporting related values.
...
Defaults to the license start timestamp. **Note: If using an IBM PAO license, the billing start timestamp will be set to
the start time of the entitlement selected by the `license_entitlement` stanza, and this parameter will be ignored.**
