...
billing start date configured. For clusters using an IBM PAO license, the billing start date is always the start time of the entitlement selected by the [license_entitlement](/vault/docs/configuration/license-entitlement) stanza regardless of any explicitly configured billing dates.
