...
billing start date configured. For clusters using an IBM PAO license, the billing start date will be set to the start time of the entitlement selected by the [license_entitlement](/vault/docs/configuration/license-entitlement) stanza regardless of if a billing start date is explicitly configured or not.
