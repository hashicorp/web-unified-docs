...
1. Configure the OIDC auth method in Vault by setting `"provider_config"` to Azure.
   ...
   You can include the following optional parameter in the `provider_config` object:

   - `fetch_groups` `(bool: false)` - If set to `true`, Vault fetches groups directly
     from the Microsoft Graph API through `GET me/getMemberObjects` instead of relying on the
     groups claim in the token. This is useful for retrieving group memberships
     regardless of the 200-group limit in the token. It also bypasses the
     per-application 1,000 group assignment limit in Microsoft Entra ID, which can
     be a constraint when your organization assigns a large number of groups to a
     single application. When you enable `fetch_groups`, you must pass a
     `distributed_claim_access_token` during [JWT login](/vault/api-docs/auth/jwt#jwt-login)
     so that Vault can authenticate to the Microsoft Graph API.
