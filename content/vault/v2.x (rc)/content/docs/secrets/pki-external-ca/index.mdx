...
2. Check the order status, waiting for the order to be ready for challenge fulfillment (status `awaiting_challenge_fulfillment`):
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/status
```

3. Fetch the challenge token and auth key for each identifier from Vault:
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/challenges \
    identifier="www.example.com" challenge_type="http-01"
```

4. Fulfill challenges (place HTTP tokens or DNS records) as ACME server expects.

5. Notify Vault that challenges are ready
```shell-session
...
6. Wait for certificate issuance to be completed (status `completed`)
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/status
```

7. Retrieve certificate when ready
```shell-session
...
2. Create order with CSR
```shell-session
$ vault write pki-external-ca/role/web-servers/new-order \
    csr=@server.csr
```

3. Check the order status, waiting for the order to be ready for challenge fulfillment (status `awaiting_challenge_fulfillment`):
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/status
```

4. Fetch the challenge token and auth key for each identifier from Vault:
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/challenges \
identifier="www.example.com" challenge_type="http-01"
```

5. Fulfill challenges (place HTTP tokens or DNS records) as ACME server expects.

6. Notify Vault that challenges are ready
```shell-session
$ vault write pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/fulfilled-challenge \
identifier="www.example.com" \
challenge_type="http-01"
...
7. Wait for certificate issuance to be completed (status `completed`)
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/status
```

8. Retrieve certificate when ready
```shell-session
$ vault read pki-external-ca/role/web-servers/order/01936d8e-7c3a-7890-b123-456789abcdef/fetch-cert
```
...
you call the `new-order` endpooint.
