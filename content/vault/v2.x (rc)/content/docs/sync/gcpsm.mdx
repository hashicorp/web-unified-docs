...
1. Configure a sync destination using the below approaches

  - Using the Service Account JSON credentials created in the previous step. See docs for
  [alternative ways](/vault/docs/secrets/gcp#authentication) to pass in the `credentials` parameter or using the workload identity federation credentials.
  ...
  ```shell-session
  $ vault write sys/sync/destinations/gcp-sm/my-dest \
      identity_token_audience='<identity token audience value>'
      identity_token_key='<identity token key name>'
      identity_token_ttl='<identity token ttl value>'
      replication_locations='us-east1,us-west1'
  ```
  
  - **Application Default Credentials**: If you run Vault on GCP infrastructure, or have setup credentials externally. They will be derived from the environment with minimal
     configuration.
