...
1. Configure authentication credentials for Azure Key Vault. You can use one of the following methods:
   ...
   - **Client secret authentication**: Create a service principal with a client ID and client secret. This [guide](https://learn.microsoft.com/en-us/azure/active-directory/develop/howto-create-service-principal-portal) walks you through creating the service principal.
   
   - **Workload identity federation (WIF)**: Configure a trust relationship between Vault and Azure. This method eliminates the need to manage long-lived client secrets. See the [WIF setup](#workload-identity-federation-wif) section for details.
   
   - **Managed identity**: If running Vault on Azure infrastructure, you can use Azure managed identity. This method requires minimal configuration as authentication is inferred from the environment.
...
Workload identity federation (WIF) lets the Azure Key Vault sync destination authenticate to Azure without managing long-lived client secrets. Instead, Vault exchanges short-lived identity tokens for Azure access tokens.

To configure WIF for Azure Key Vault sync:

1. Ensure that Vault [openid-configuration](/vault/api-docs/secret/identity/tokens#read-secret-sync-identity-token-issuer-s-openid-configuration)
...
1. Configure a
   ...
   - The issuer URL **must** point at your [Vault identity token issuer](/vault/api-docs/secret/identity/tokens#read-secret-sync-identity-token-issuer-s-openid-configuration) with the
     `/.well-known/openid-configuration` suffix removed. For example:
     `https://host:port/v1/identity/oidc/secrets-sync`.
   
   - The subject identifier **must** match the unique `sub` claim issued by identity tokens.
     The subject identifier should have the form `secrets-sync:<NAMESPACE_PATH>:<STORE_TYPE>:<STORE_NAME>`.
