...
edition of Vault. This system is not supported by HashiCorp. Vault
contains two fully-supported MFA systems that are significantly more complete and
flexible and which can be used throughout Vault's API. See the [Vault
Enterprise MFA](/vault/docs/enterprise/mfa) or [Vault OSS MFA (Login MFA)](/vault/docs/auth/login-mfa) pages for more information.
