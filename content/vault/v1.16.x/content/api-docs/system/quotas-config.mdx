...
- `absolute_rate_limit_exempt_paths` `([]string: [])` - Specifies the set of
  exempt paths used for all rate limit quotas. Any request path that exists in
  this set is exempt from rate limiting. When `absolute_rate_limit_exempt_paths`
  is empty, no paths are exempt. The paths specified here are absolute paths,
