...
description: This guide provides guidance in creating a multi-tenant environment.
...
- [Namespaces](/vault/docs/enterprise/namespaces)
- [Streamline Secrets Management with Vault Agent and Vault 0.11](https://youtu.be/zDnIqSB4tyA)
- [Vault Deployment Reference Architecture](/guides/operations/reference-architecture)
...
self-serving manner. This means that a company needs to implement a **_Vault as
a Service_** model allowing each organization (tenant) to manage their own
...
Groups](/guides/identity/identity) guide if you need the full details.
...
(`token_policies`); however, he inherited the `edu-admin` policy from the `Bob Smith` entity (`identity_policies`).
...
Notice that the user, `bob` only has `default` policy attached to his token
(`token_policies`); however, he inherited the `edu-admin` policy from the `Bob Smith` entity (`identity_policies`). Also, `training-admin` policy is listed
...
Refer to the [Sentinel Policies](/guides/identity/sentinel) guide if you
