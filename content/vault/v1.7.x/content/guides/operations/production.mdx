...
last_modified: 2025-07-25T13:26:43-07:00
...
model](/vault/docs/internals/security) and focus on defense in depth.
...
- **Disable Swap**. Vault encrypts data in transit and at rest, however it must
  ...
  automatically](/vault/docs/configuration#disable_mlock), but disabling
...
- **Avoid Root Tokens**. Vault provides a root token when it is first
  ...
  [generated when needed](/guides/operations/generate-root), and should be
...
- **Enable Auditing**. Vault supports several auditing backends. Enabling
  ...
  hash](/vault/docs/audit) any sensitive data, but access should still be
...
- **Restrict Storage Access**. Vault encrypts all data at rest, regardless of
  ...
  with arbitrary control](/vault/docs/internals/security) can cause data
...
- **Disable Shell Command History**. You may want the `vault` command itself to
  not appear in history at all. Refer to [additional methods](/guides/secret-mgmt/static-secrets#additional-discussion)
  for guidance.
...
- **Docker Containers**. To leverage the ["memory lock"](/vault/docs/configuration#disable_mlock)
