...
This guide introduces **_Consul Template_** and **_Envconsul_** to help you
...
Both **_Consul Template_** and **_Envconsul_** provide first-class support for
...
- A [Vault environment](/intro/getting-started/install)
...
Secrets](/guides/secret-mgmt/dynamic-secrets) guide first if you are not
...
> example. Be sure to use the correct [connection URL](/vault/api-docs/secret/databases/postgresql#connection_url) to
> match your environment.

~> Refer to the [PostgreSQL Database Secrets
Engine](/vault/docs/secrets/databases/postgresql) documentation or [Secret as a
Service: Dynamic Secrets](/guides/secret-mgmt/dynamic-secrets) guide if you
...
The main difference here is that the `app.sh` is reading **_environment
variables_** to set `username` and `password` values; therefore, no templating
is involved.
...
Authentication_](/guides/identity/authentication) guide to learn about the
