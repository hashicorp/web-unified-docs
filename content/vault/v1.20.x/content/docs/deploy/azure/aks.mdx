...
# Deploy Vault on Microsoft Azure Kubernetes Service (AKS)
...
For a client of the Vault server to read the secret data defined in the [Set a secret in Vault](#set-a-secret-in-vault) step requires that the read capability be granted for the path `secret/data/devwebapp/config`.
