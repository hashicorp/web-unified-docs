...
There are currently 4 different integrations to help Kubernetes workloads seamlessly consume secrets from Vault, without the need to modify the application to interact directly with Vault. Each integration addresses slightly different use-cases. 
The following is a brief overview of the strengths of each integration. Refer to the [comparisons](/vault/docs/platform/k8s/comparisons) page for more details.

#### Vault Secrets Operator

([Vault Secrets Operator docs](/vault/docs/platform/k8s/vso))
...
- Supports templating with [SecretTransformations](/vault/docs/platform/k8s/vso/secret-transformation)
  that deliver secrets in the desired format.

#### Vault Secrets Operator CSI Driver
([Vault Secrets Operator CSI Driver docs](/vault/docs/platform/k8s/vso/vso-csi))

- Writes all secrets to ephemeral volumes to avoid the Kubernetes Secrets
  requirements for locked-down API server permissions and `etcd` encryption.
- Reduced load on Vault compared to Agent Injector, as the number of connections to Vault corresponds to the number of nodes, not the number of consuming pods.
- Supports templating with [SecretTransformations](/vault/docs/platform/k8s/vso/secret-transformation)
  that deliver secrets in the desired format.

#### Secrets Store CSI Provider for Vault
([Secrets Store CSI Provider for Vault docs](/vault/docs/platform/k8s/csi))
...
([Vault Agent Injector docs](/vault/docs/platform/k8s/injector))
