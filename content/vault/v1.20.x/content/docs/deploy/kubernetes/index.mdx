...
#### Agent injector
([docs](/vault/docs/platform/k8s/injector))
...
([docs](/vault/docs/platform/k8s/vso))
...
- Ability to use templating through [SecretTransformations](/vault/docs/platform/k8s/vso/secret-transformation) to deliver secrets in the desired format

#### Vault Secrets Operator CSI driver
([docs](/vault/docs/platform/k8s/vso/vso-csi))

- All secrets are written only to ephemeral volumes, for users who wish to avoid the locked-down API server permissions and etcd encryption requirements of Kubernetes Secrets.
...
#### Vault CSI provider for Secrets Store CSI driver
([docs](/vault/docs/platform/k8s/csi))
