...
| Feature                             | Vault Secrets Operator                                             | Vault Secrets Store CSI provider   | Vault Agent Injector
	...
	| Supported auth methods              | K8s, AppRole, GCP, AWS, JWT                                      | K8s, JWT             | K8s, others
	...
	| Requires Vault for autoscaling      | No                                                               | Yes                  | Yes
...
Vault Enterprise users can also configure it to run a [CSI driver]((/vault/docs/platform/k8s/vso/vso-csi)), which mounts secrets directly from Vault to application containers as volumes, bypassing Kubernetes Secrets. 
...
<Tabs>
	...
	- **Vault Agent Injector** - Highest load due to per-pod connections and sidecar
	   pattern.
	...
	- **Vault Agent Injector** - High consumption due to the number of sidecar containers
	  per pod.
