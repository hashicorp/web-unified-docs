...
Kubernetes Secrets, the Vault Secrets
Operator (CSI)[https://github.com/container-storage-interface/spec/blob/master/spec.md] driver hooks into the pod volume lifecycle to fetch secrets from
Vault, bypass Kubernetes Secrets, and mount Vault secrets directly to containers as volumes.
...
The Vault Secrets Operator CSI Driver currently does not support the [instant updates](/vault/docs/deploy/kubernetes/vso/sources/vault/instant-updates) feature. 
Application containers must self-restart to receive updated secret information after a secret is updated in Vault.
