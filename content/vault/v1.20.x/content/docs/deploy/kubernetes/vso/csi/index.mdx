---
layout: docs
page_title: Use direct secrets in Kubernetes
description: >-
  Mount secrets from Vault directly into application containers with the Vault
  Secrets Operator CSI driver.
---

# Use direct secrets in Kubernetes with CSI

@include 'alerts/enterprise-only.mdx'

While the Vault Secrets Operator lets you store Vault secrets in clusters as native
Kubernetes Secrets, you must install the Vault Secrets Operator [CSI driver](https://github.com/container-storage-interface/spec/blob/master/spec.md) to use "direct secrets" functionality.
...
Refer to the [Vault Secrets Operator CSI driver setup guide](/vault/docs/platform/k8s/vso/csi/setup) to use direct secrets with your Vault Secrets Operator instance.
...
In the CSI driver workflow, the driver does not cache secrets as Kubernetes Secrets, so secret data is never stored in `etcd`.
