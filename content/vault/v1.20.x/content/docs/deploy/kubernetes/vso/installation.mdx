...
The Vault Secrets Operator Helm chart can also be used to deploy a DaemonSet of the [Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/vso-csi). 
To do so, add the flag `"csi.enabled=true"`.
...
For more information about this operating mode, see the [Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/vso-csi) documentation.
