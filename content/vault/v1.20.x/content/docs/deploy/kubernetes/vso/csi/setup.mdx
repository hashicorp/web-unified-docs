...
<Warning>
The examples on this page are meant for simple demonstration, and may contain examples of Kubernetes RBAC configuration
that is not production-ready. Refer to the Vault Secrets Operator [threat model](https://github.com/hashicorp/vault-secrets-operator/tree/main/docs/threat-model) 
as you configure your authentication, to understand the implications on cluster security.
</Warning>


## Step 1: Install driver

Installing the Vault Secrets Operator Helm chart with the `"csi.enabled=true"` flag deploys the CSI driver as a DaemonSet running on every node.
...
## Step 2: Add restarts for containers when a secret has been changed
...
## Step 3: Set up authentication to Vault
...
After you've written your policy to Vault, you can create a role that uses that policy 
in any auth method supported by Vault Secrets Operator.

Here is an example for the [Kubernetes](/vault/docs/auth/kubernetes) auth method:
```
...
For example, the following resource definition creates a single `CSISecrets` resource that:
- Fetches Vault secrets from a `kv` v1 secrets engine mounted at `kv1`, and a `kv` v2 secrets engine mounted at `kv2`
- Generates a secret ID from an `approle` auth method mounted at `auth/approle`, for a role configured with the name `my-app`.
- Uses regular expression patterns in the `accessControl` field to define which pods are authorized to use this CSISecrets resource
...
Reference your CSISecrets resource in the `volumeMounts` field of your application's Deployment spec, and 
ensure that your application pod's name, namespace, etc. satisfy the patterns you defined in the `accessControl` stanza before.

The CSI driver pod on your application pod's node responds to the request, ensuring that the 
kubelet publishes a volume containing all the secrets from the CSISecrets resource.
