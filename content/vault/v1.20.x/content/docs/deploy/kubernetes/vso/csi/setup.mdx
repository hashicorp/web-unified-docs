...
All other secrets sync whenever a requesting container starts or restarts, or when a CSISecrets resource is modified.
...
- Review the [Vault Secrets Operator threat model](https://github.com/hashicorp/vault-secrets-operator/tree/main/docs/threat-model).
  The simplified Kubernetes RBAC configuration examples below are for informational purposes only and not intended to be
  production-ready.
...
- The CSI driver does not support [instant updates](/vault/docs/deploy/kubernetes/vso/sources/vault/instant-updates). 
  Application deployments should consider whether, when, and how to handle secrets updates.
...
If desired, you can reference existing auth resources from Vault Secrets Operator.
...
- Define a [regular expression](https://pkg.go.dev/regexp/syntax) pattern in the `accessControl` stanza of your `CSISecrets` resource, that specifies which Pods should have access to the secret data.
