...
Vault Enterprise users can deploy the Vault Secrets Operator alongside a CSI
driver to mount secrets directly to pods without using Kubernetes Secrets.

Refer to the
[Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/csi)
documentation to learn more about the CSI driver option.
