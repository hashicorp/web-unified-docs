...
The Vault Secrets Operator (VSO) allows Pods to consume Vault secrets natively from Kubernetes Secrets. It can 
alternatively be used to mount Vault secrets directly to containers with its [CSI driver](/vault/docs/platform/k8s/vso/vso-csi) option. 
...
For Vault Enterprise users, the Vault Secrets Operator can be alternatively configured to use a CSI driver to mount secrets directly to pods, 
without making use of Kubernetes Secrets. The rest of this page covers the traditional operation of Vault Secrets Operator. 
You can learn more about the CSI driver option on the [Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/vso-csi) documentation page.
