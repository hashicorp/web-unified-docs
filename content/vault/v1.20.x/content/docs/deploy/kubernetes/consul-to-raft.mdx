...
When you use integrated storage, the default configuration of the Vault Helm
chart retains any PVCs created. Vault does not use PVCs when you use Consul
storage, so you must remove the PVCs before re-attempting the migration from
Consul to integrated storage.
