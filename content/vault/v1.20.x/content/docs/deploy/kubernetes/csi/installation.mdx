---
layout: docs
page_title: Install the Vault Secrets Store CSI provider
description: Install the Vault Secrets Store CSI provider using Vault Helm.
---
...
install and configure the Vault Secrets Store CSI provider in Kubernetes.

To install a new instance of Vault and the Vault Secrets Store CSI provider, first add the
...
Then, patch the `DaemonSet` for the Vault Secrets Store CSI provider to
...
The Secrets Store CSI driver and Vault Secrets Store CSI provider need `hostPath` mount access.
...
Add the service account for the Vault Secrets Store CSI provider to the `privileged`
...
You need to give additional access to the application retrieving secrets with the Vault Secrets Store CSI provider.
