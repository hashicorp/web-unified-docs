---
layout: docs
page_title: Install the Secrets Store CSI Provider for Vault
description: Install the Secrets Store CSI Provider for Vault using Vault Helm.
---

# Install the Secrets Store CSI Provider for Vault
...
install and configure the Secrets Store CSI Provider for Vault in Kubernetes.

To install a new instance of Vault and the Secrets Store CSI Provider for Vault, first add the
HashiCorp helm repository and ensure you have access to the chart:

~> **Note:** Secrets Store CSI Provider for Vault Helm installation requires Vault Helm 0.10.0+.
...
chart include limiting the namespaces the Secrets Store CSI Provider for Vault runs in, TLS options and
more.
...
Then, patch the `DaemonSet` for the Secrets Store CSI Provider for Vault to
...
The Secrets Store CSI driver and Secrets Store CSI Provider for Vault need `hostPath` mount access.
...
Add the service account for the Secrets Store CSI Provider for Vault to the `privileged`
...
You need to give additional access to the application retrieving secrets with the Secrets Store CSI Provider for Vault.
