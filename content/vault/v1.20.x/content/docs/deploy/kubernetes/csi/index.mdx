...
is `vault`. The Vault Secrets Store CSI Provider then uses the specified `SecretProviderClass` and the service account of the pod to retrieve
...
Vault Secrets Store CSI Provider supports the following features:
...
The Vault Secrets Store CSI Provider authenticates with Vault as the service account of the
