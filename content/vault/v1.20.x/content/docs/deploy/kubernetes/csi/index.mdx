---
layout: docs
page_title: Secrets Store CSI Provider for Vault
description: >-
  Learn how the Vault provider for the Secrets Store CSI Driver allows pods to consume Vault secrets using CSI volumes.
---

# Secrets Store CSI Provider for Vault

The Secrets Store CSI Provider for Vault allows pods to consume Vault secrets using
...
The Secrets Store CSI Provider for Vault is a fully supported component of HashiCorp Vault.

~> The Secrets Store CSI Provider for Vault requires the [CSI Secret Store](https://github.com/kubernetes-sigs/secrets-store-csi-driver)
...
are created, the Secrets Store CSI driver will send the request to the Secrets Store CSI Provider for Vault if the provider
is `vault`. The Secrets Store CSI Provider for Vault will then use Secret Provider Class specified and the pod's service account to retrieve
...
The following features are supported by the Secrets Store CSI Provider for Vault:
...
The Secrets Store CSI Provider for Vault will authenticate with Vault as the service account of the
...
Refer to the [Secrets Store CSI Provider for Vault](/vault/tutorials/kubernetes/kubernetes-secret-store-driver)
