---
layout: docs
page_title: Vault Secrets Store CSI Provider
description: >-
  Learn how to consume Vault secrets using CSI volumes with the Vault provider
  for the Secrets Store CSI Driver.
---

# Vault Secrets Store CSI Provider

The Vault Secrets Store CSI Provider allows pods to consume Vault secrets using
[Secrets Store CSI](https://github.com/kubernetes-sigs/secrets-store-csi-driver) volumes. 
Alternatively, you can deploy HashiCorp's
[Vault Secrets Operator CSI Driver](/vault/docs/platform/k8s/vso/vso-csi) as part of your
Vault Secrets Operator installation.

The Vault Secrets Store CSI Provider is a fully supported component of HashiCorp Vault.

~> The Vault Secrets Store CSI Provider requires the [CSI Secret Store](https://github.com/kubernetes-sigs/secrets-store-csi-driver)
Driver to be installed.

## Overview

At a high level, the Secrets Store CSI driver allows users to create `SecretProviderClass` objects.
This object defines which secret provider to use and what secrets to retrieve. When pods requesting CSI volumes
are created, the Secrets Store CSI driver will send the request to the Vault Secrets Store CSI Provider if the provider
is `vault`. The Vault Secrets Store CSI Provider will then use Secret Provider Class specified and the pod's service account to retrieve
...
The following features are supported by the Vault Secrets Store CSI Provider:
...
The Vault Secrets Store CSI Provider will authenticate with Vault as the service account of the
...
Refer to the [Vault Secrets Store CSI Provider](/vault/tutorials/kubernetes/kubernetes-secret-store-driver)
