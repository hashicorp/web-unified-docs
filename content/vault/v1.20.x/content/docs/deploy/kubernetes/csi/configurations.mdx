---
layout: docs
page_title: Secrets Store CSI Provider for Vault configurations
description: >-
   Learn about the command line arguments and the secret provider class parameters supported by the Secrets Store CSI Provider for Vault.
---

# Secrets Store CSI Provider for Vault configurations

The following command line arguments are supported by the Secrets Store CSI Provider for Vault.
...
- `-vault-addr` `(string: "https://127.0.0.1:8200")` - Default address
  ...
  to the Secrets Store CSI Provider for Vault for caching and renewals, but setting `-vault-addr`
  here will cause the Secrets Store CSI Provider for Vault to bypass the Agent's cache.
...
- `vaultAddress` `(string: "")` - The address of the Vault server. **Note:** It is
  ...
  The helm chart will install Vault Agent as a sidecar to the Secrets Store CSI Provider for Vault
  for caching and renewals, but setting `vaultAddress` here will cause the Secrets Store CSI Provider for Vault to bypass the Agent's cache.
