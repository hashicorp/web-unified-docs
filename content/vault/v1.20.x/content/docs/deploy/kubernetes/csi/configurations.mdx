...
- `vaultAddress` `(string: "")` - The address of the Vault server. **Note:** It is
  ...
  The helm chart installs Vault Agent as a sidecar to the Vault Secrets Store CSI Provider
	for caching and renewals. Setting `vaultAddress` here causes the Vault Secrets Store CSI Provider to bypass the Vault Agent cache.
