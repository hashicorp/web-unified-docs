...
- `-vault-addr` `(string: "https://127.0.0.1:8200")` - Default address
  ...
  to the Vault Secrets Store CSI Provider for caching and renewals, but setting `-vault-addr`
  here will cause the Vault Secrets Store CSI Provider to bypass the Agent's cache.
