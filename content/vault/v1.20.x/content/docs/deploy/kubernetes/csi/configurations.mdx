---
layout: docs
page_title: Vault Secrets Store CSI Provider configurations
description: >-
   Learn about the command line arguments and the secret provider class parameters supported by the Vault Secrets Store CSI Provider.
---

# Vault Secrets Store CSI Provider configurations

The following command line arguments are supported by the Vault Secrets Store CSI Provider.
...
- `-vault-addr` `(string: "https://127.0.0.1:8200")` - Default address
  ...
  installing the helm chart. The helm chart installs Vault Agent as a sidecar to the 
  Vault Secrets Store CSI Provider for caching and renewals. 
  Setting `vaultAddress` here causes the Vault Secrets Store CSI Provider to bypass the Vault Agent cache.
...
- `vaultAddress` `(string: "")` - The address of the Vault server. **Note:** It is
  ...
  The helm chart will install Vault Agent as a sidecar to the Vault Secrets Store CSI Provider
  for caching and renewals, but setting `vaultAddress` here will cause the Vault Secrets Store CSI Provider to bypass the Agent's cache.
