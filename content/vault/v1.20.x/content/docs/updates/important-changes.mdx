...
If you use private endpoints to sync secrets, set the
[`disable_strict_networking`](/vault/api-docs/system/secrets-sync#disable_strict_networking)
secret sync configuration parameter to `true` to disable IP address and port number restrictions
used by the SSRF-safe sync clients.
