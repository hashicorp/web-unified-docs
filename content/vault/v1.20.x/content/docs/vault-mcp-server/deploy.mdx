...
The Vault MCP server is a specialized service that provides AI models with access to Vault APIs. You can deploy the server to the following environments:
...
You can use the `vault-mcp-server` CLI and specify the transport protocol you want to use to start the server. Refer to the [transport protocols reference](/vault/docs/vault-mcp-server/reference#transport-protocols) for more information.
...
Instead of setting values manually, you can also use the supported environment variables. Refer to the [environment variables reference](/vault/docs/vault-mcp-server/reference#environment-variables) for details.

## Next steps

- Begin prompting your AI model about Vault operations. Refer to [Prompt an AI model](/vault/docs/vault-mcp-server/prompt) for guidance on effective prompting techniques.
