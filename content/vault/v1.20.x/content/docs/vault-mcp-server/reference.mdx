...
Example prompt: "How do I configure an AWS credentials and store this securely?"
...
| `create_mount` | Create a new mount in Vault                 | Confirmation of the mount creation              |
| `delete_mount` | Delete a specific mount from Vault          | Confirmation of the mount deletion              |

Example prompt: "Create an KV mount named Azure to store keys"
...
| `VAULT_TOKEN`            | Vault authentication token (required)                        | —                         | `s.xxxxxxxx`                   | Any valid Vault token                   |
...
| `MCP_ALLOWED_ORIGINS`    | Comma-separated list of allowed origins for CORS             | `""` (none)               | `http://vault.example.com:8200`| Comma-separated list of origins or `""` |
