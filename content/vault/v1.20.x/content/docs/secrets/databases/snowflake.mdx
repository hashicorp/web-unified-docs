...
1.  Configure Vault with keypair authentication:
    ...
    ~> **Note:** When using key-pair authentication, do not provide any templated
    or hard-coded `username:password` information as part of the connection URL.
    The plugin will appropriately construct the full DSN using the provided `username`
    and `private_key` to authenticate to Snowflake.

    When you configure the database, you must provide a `connection_url` in the
    following format (including any optional query parameters):
    ```shell-session
