...
- `connection_url` `(string: <required>)` - Specifies the Snowflake DSN. This field
  ...
  parameters in the format `{{field_name}}` only when using password authentication.
  A templated connection URL is required when using root credential rotation for passwords.
  See the [Snowflake secrets engine docs](/vault/docs/secrets/databases/snowflake#setup)
  for more information on the connection URL format for different authentication methods.
