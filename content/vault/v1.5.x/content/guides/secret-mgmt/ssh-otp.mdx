...
certificate](/vault/docs/secrets/ssh/signed-ssh-certificates) and one-time SSH
...
**_Vault helper_** which validates the OTP with the Vault server. The Vault
...
On the **_Vault server_**, you must enable the SSH secrets engine before you can
...
Read the [Signed SSH
Certificates](/vault/docs/secrets/ssh/signed-ssh-certificates) documentation if
