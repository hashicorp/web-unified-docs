...
This endpoint creates or updates static role definitions. Static roles are used to
manage pre-existing service principals in Azure.
...
- `application_object_id` (`string: <required>`) -  Application Object ID for an existing service principal that will
be managed by this static role.
...
This endpoint retrieves the named static role definition.
...
This endpoint list all existing static roles in the Azure secrets engine.
...
This endpoint deletes an existing static role by the given name.
...
- `name` `(string: <required>)` – Specifies the name of the static role to rotate the password for.
