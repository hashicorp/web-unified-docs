...
- `client_secret` (`string: ""`) - The plaintext secret value of the credential you want to import.
   If omitted, Vault requires a rotation before the credential can be used.
- `expiration` (`string: ""`) - The expiration time of the imported credential, in RFC3339 format.
   Must be a future time if provided.
- `skip_import_rotation` (`bool: false`) – If set to true, Vault stores the imported credential values
   without automatically rotating them. You must rotate the role before credentials can be read.
