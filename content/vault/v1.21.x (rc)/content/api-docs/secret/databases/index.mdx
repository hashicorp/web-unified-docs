...
| Method    | Path                           |
|:----------|:-------------------------------|
| `RECOVER` | `/database/static-roles/:name` |
...
- `X-Vault-Recover-Snapshot-Id` `(string: <required>)` - The ID of a snapshot previously loaded into Vault that contains the role at the provided path.
- `X-Vault-Recover-Source-Path` `(string: <optional>)` - The source path in the snapshot to recover from. If not provided, defaults to the request path.
...
$ curl \
    ...
    --header "X-Vault-Recover-Source-Path: database/static-roles/my-old-role" \
    https://127.0.0.1:8200/v1/database/static-roles/my-role
```
