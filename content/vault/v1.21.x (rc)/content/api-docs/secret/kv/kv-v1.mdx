...
- `read_snapshot_id` `(string: <required>)` - Query parameter specifying the ID of a snapshot previously loaded into Vault that contains secrets at the provided path.
...
- `recover_source_path` `(string: <optional>)` - The source path in the snapshot to recover from. If not provided, defaults to the value of `path`.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --get --data-urlencode "recover_source_path=secret/my-old-secret" \
