...
- `enable_self_enrollment` `(bool: false)` - <EnterpriseAlert product="vault" inline /> When set to true,
  users can self-enroll by generating their TOTP secret (QR code) during login if they do not already have one. Supported from Vault 1.21.0.
...
This endpoint allows a user to self-enroll in TOTP MFA by generating their TOTP
secret (QR code) during login if they do not already have one. This endpoint is
only available in Vault Enterprise if the `enable_self_enrollment` parameter is
set to true on the TOTP MFA method.
...
- `mfa_request_id` `(string: <required>)` - A unique identification of an MFA
  restricted login request. This can be found in the MFA requirement included in
  the auth response of the login request.
