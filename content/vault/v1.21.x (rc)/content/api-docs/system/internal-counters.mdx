...
endpoint from all namespaces and the cumulative total for each unique client
includes activity in the provided namespace plus all decedent namespaces. 

To query client counts for deleted namespaces, you must call the endpoint from
`root` since the namespace path is unknown to all other namespaces. As a result,
deleted namespaces do not contribute to the cumulative counts of any namespace
other than `root`. 

<Tip title="Related endpoint">

For client counts isolated to a specific namespace, grouped by month, or to see
...
uses to the billing period associated with `start_time`.
