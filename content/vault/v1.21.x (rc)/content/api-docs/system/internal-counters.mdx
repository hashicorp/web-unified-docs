...
<Note title="Tech preview">
  ...
  **We strongly discourage using tech preview features in production deployments
  of Vault**

</Note>

The cumulative client count endpoint returns a by-namespace breakdown of client
activity for the given billing period. You can call the cumulative client count
endpoint from all namespaces and the cumulative total for each namespace includes
client activity in said namespace plus all descendant namespaces.

To query client counts for deleted namespaces, you must call the endpoint from
`root` since the original namespace path is unknown. As a result,
deleted namespaces do not contribute to the cumulative counts of any namespace
other than `root`. 

<Tip title="Related endpoint">

For client counts isolated to each namespace, grouped by month, or to see
mount-level attribution, use `sys/internal/counters/activity`.
...
If the number of namespaces with client activity during the requested billing
period approaches or exceeds the [default maximum](/vault/docs/internals/limits#namespace-limits),
the request may take several seconds to complete. To prevent impacting the
performance of the Vault cluster, avoid making another request to this endpoint
...
- `end_time` `(string, optional)` - An RFC3339 timestamp or Unix epoch time
  specifying the end of the client count query period. Leave `end_time`
  unset to use the end f the current month as the end time.
...
- `by_namespace` - maps the namespace path to an object containing the namespace
  ID and cumulative client counts for that namespace and all its children. Vault
...
- the `root` namespace has zero direct clients.
- all other namespaces have 50 direct clients.
