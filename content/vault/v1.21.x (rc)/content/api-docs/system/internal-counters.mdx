...
This endpoint returns a by-namespace breakdown of client activity for a given billing period. The response
gives _cumulative_ namespace client counts: the total number of unique clients that had activity not just in each namespace
but also in its children, grandchildren, etc. This is in contrast to the `sys/internal/counters/activity` endpoint's
`by_namespace` breakdown which returns only the client activity that occurred directly within each namespace.

<Note title="Tech preview">

This endpoint is currently in tech preview status. The API call and response format
...
- The response contains the total activity for the requested billing period, represented by the `start_time` and `end_time`
  parameters. Only entire billing periods can be queried. If the requested time parameters do not align with a complete billing period,
  Vault will adjust the request to the full billing period associated with the provided times. If both start and end times are provided
  and they do not fall within the same billing period, the billing period associated with the `start_time` will be used.
  The response will detail the actual time period covered, along with a warning if the requested times were adjusted.

- Unlike the `sys/internal/counters/activity` endpoint, this endpoint does not return a breakdown of activity by month,
  nor does it include mount level attributions.

- This endpoint can be called from all namespaces. The requested namespace will act as a filter. The response will include activity for the requested
  namespace and all of its children.

- If a namespace has subsequently been deleted, its path will be listed as `deleted namespace :ID:`. Deleted
  namespaces are reported only for queries made from the root namespace because information about the namespace path is unknown.
  For this same reason, deleted namespaces do not contribute to the cumulative counts of their parent namespaces,
  only to the root namespace. In other words, deleted namespaces are treated as direct children of the root namespace.

<Warning>

If the number of namespaces with client activity during the requested billing period approaches or exceeds the [default
maximum](/vault/docs/internals/limits#namespace-limits), the request may take several seconds to complete. To prevent
impacting the performance of the Vault cluster, avoid making another request to this endpoint until the previous one
has completed.
...
- `by_namespace` - a map from namespace path to an object containing the cumulative client counts for that namespace
  and all of its children, and the namespace ID.

- `total` - the total cumulative client counts for the requested billing period, across all included namespaces.

- `start_time` - the start time used for the query, aligns with the start of a billing period.

- `end_time` - the end time used for the query, aligns with the end of a billing period.

### Sample response

For the cluster that generated the response below, each namespace has 50 direct clients, except for the root namespace
which has zero direct clients.
...
admin,admin,3f210722-7210-98e8-1f0d-e6a39ffb29c6,entity,false,root,,auth_userpass_a005db73,auth/userpass/,userpass,2022-07-10T09:33:51Z,2024-08-03T12:00:00Z,admin@example.com,san_francisco,west,admin@example.com,746fbaf9-ffeb-62b9-7f0b-42d79ca0883f,secret-read,secret-list,secret-write,secret-delete
...
jdoe,jdoe,665a54bf-8652-c0c5-8121-40adf0d9786a,entity,false,wOsmr,ns1/a,auth_ldap_a005db73,auth/ldap/,ldap,2022-07-08T11:47:57Z,2024-08-03T12:00:00Z,jdoe@example.com,new_york,east,,9f18cd4a-4e64-a2b2-b001-7b6b0dfb1270,secret-read,,,
