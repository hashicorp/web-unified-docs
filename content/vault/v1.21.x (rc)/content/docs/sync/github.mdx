...
GitHub Enterprise Server (self-hosted GitHub) destinations are configured similarly to standard github.com
destinations, but require an extra field on the configuration to specify the URL of the
server instance.

Using repository secrets as an example:
