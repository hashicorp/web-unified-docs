...
In general, the GCP key sections create a service agent identity for the
`secretsmanager` API and grant service agents the correct IAM policy bindings
with the `cloudkms.cryptoKeyEncrypterDecrypter` role. Be aware that you must
grant the cloud KMS role to the service agent for each key and replication
