...
GCP key names are expected in the format of the entire resource name, e.g. `projects/<project_id>/locations/<location_name>/keyRings/<key_ring_name>/cryptoKeys/<key_name>`. Be aware that this is **case-sensitive**, with special attention on the `keyRings` and `cryptoKeys` portions.
See the [API examples](/vault/api-docs/system/secrets-sync#sync-secrets-to-gcp-with-a-global-kms-key) for more details.

Refer to the [GCP documentation](https://cloud.google.com/secret-manager/docs/cmek) for more details on setting this up. For brevity, the key parts are to create a service agent identity for
the secretsmanager API and grant the service agent the correct IAM policy bindings with the
`cloudkms.cryptoKeyEncrypterDecrypter` role. Be aware that this role must be granted to the service
agent for each key and replication location being used.
