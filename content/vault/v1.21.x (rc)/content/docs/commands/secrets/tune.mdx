...
> **Note:** As of Vault 1.21, you can tune the TTL in the UI for the **keymgmt** secrets engine only. Support for other engines may be added in future releases.

To tune a Keymgmt mount in the GUI:

1. Navigate to the **Secrets Engines** page in the Vault UI.
1. Select a mounted **keymgmt** secrets engine.
1. Click the **Manage** dropdown menu and select **Configure** (with a gear icon).
1. Adjust any of the following settings as needed:
   - **Default lease TTL** - The default length of time a token or lease is valid for
   - **Maximum lease TTL** - The maximum allowed lifetime of tokens and leases
   - **Description** - A description of this secrets engine mount
1. Click **Save changes** to apply your changes.

![Tune Keymgmt Secret Engine](/img/ui-tune-ttl-placeholder.png)
