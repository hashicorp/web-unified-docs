...
#### Configure the TOTP Method

**Option 1: Self-enrollment setup**

Configure the TOTP method with `enable_self_enrollment=true` so the Vault web UI prompts users with a QR code when they do not have TOTP configured.
