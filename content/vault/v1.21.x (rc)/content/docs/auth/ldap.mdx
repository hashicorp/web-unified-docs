...
Use [`rotation_url`](/vault/api-docs/auth/ldap#rotation_url) to provide the specific LDAP
server URL that the bind account belongs to for rotating its credential. Use this in cases
such as configuring an LDAP forest where the bind account belongs to only a single domain
behind a load-balancer. Defaults to the same value as `url`.
