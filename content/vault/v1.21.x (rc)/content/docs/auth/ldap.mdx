...
- `rotation_url` `(string: "")` – The LDAP server to perform a root config password
  rotation on if it is different from `url` parameter. 
...
Use [`rotation_url`](/vault/api-docs/auth/ldap#rotation_url) to provide a specific LDAP
server URL that the bind account belongs to for rotating the credential. Use this in cases
such as configuring an LDAP forest where the bind account belongs to only a single domain,
or when using a load-balancer between LDAP domains. Defaults to the same value as `url`.
