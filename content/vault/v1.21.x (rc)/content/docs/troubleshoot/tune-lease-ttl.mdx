...
> **Note:** Vault 1.21 supports configuration updates (tune) for mounted **keymgmt** secrets engine in the GUI. Support for other engines may be added in future releases.
...
1. Click **Manage** in the top navigation bar, then select **Configure**.

1. Set the **Time-to-live (TTL)** or **Maximum Time-to-live (TTL)** values.
