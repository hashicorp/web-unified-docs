...
- **Time-based One-time Password (TOTP)** - If configured and enabled on a path,
  ...
  TOTP key present in the identity of the caller in Vault. This method supports [self-enrollment](#self-enroll-mfa-totp).
...
The TOTP method supports self-enrollment so admins do not have to generate a QR code for the
end user.
