...
- Metadata import: Provide only the `secret_id`, and optionally `expiration`.
  Vault records the metadata but blocks reads until you explicitly rotate the role
  to generate a valid secret.
- Full import: Provide the `secret_id`, `client_secret`, and optionally an `expiration`.
  This makes the credential immediately usable in Vault.
- Rotate on import: Provide the `secret_id` and do not set `skip_import_rotation`.
  Vault will rotate the credential during import, ensuring a fresh, valid secret.

Vault validates imported credentials to prevent incomplete or invalid inputs:
