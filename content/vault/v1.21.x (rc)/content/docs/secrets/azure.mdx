...
In addition to dynamic roles, the Azure secrets engine also supports static roles.

A static role allows Vault to manage a long-lived client secret for an Azure application.
Unlike dynamic roles, which generate new credentials on each request, a static role always returns
the same client secret until it is rotated. Rotation can be triggered manually by calling the
[`rotate-role`](/vault/api-docs/secret/azure#manually-rotate-static-role-password) endpoint. When it occurs, Vault
will generate a new client secret for the application and revoke the previous one.

Please see [API documentation](/vault/api-docs/secret/azure#create-update-static-role) for more details on this feature
