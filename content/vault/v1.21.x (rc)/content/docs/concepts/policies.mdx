...
- `allowed_parameters` - A list of keys and values that are
  ...
  - When a request includes a parameter that matches an entry in `allowed_parameters`
  and its value is a list, Vault allows the request if all elements in the list are
  allowed, or if the list as a whole is allowed. In Vault 1.20 and earlier, Vault
...
- `denied_parameters` - A list of keys and values that are not permitted on the given
  ...
  - When a request includes a parameter that matches an entry in `denied_parameters`
  and its value is a list, Vault denies the request if any element in the list is
  denied, or if the list as a whole is denied. In Vault 1.20 and earlier, Vault only
  denied the request when the entire list matched a denied value. During the
  deprecation period, you can restore the old behavior by setting the
  `VAULT_LEGACY_EXACT_MATCHING_ON_LIST` environment variable on the server.
