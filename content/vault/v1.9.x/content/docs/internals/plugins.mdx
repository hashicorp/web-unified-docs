...
token](/vault/docs/concepts/response-wrapping) to the
...
plugin executable in your [plugins directory](/vault/docs/internals/plugins#plugin-directory) must be
given the ability to use the `mlock` syscall.

### Plugin Upgrades

Plugins may be updated by registering and reloading them. More details on the
upgrade procedure can be found in [Upgrading Vault Plugins](/vault/docs/upgrading/plugins).
