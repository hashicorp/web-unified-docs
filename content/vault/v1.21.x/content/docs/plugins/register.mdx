...
An external plugin is any authentication or secrets plugin built from external
code. Vault pre-registers commonly used official plugins (like `kv`, but you
...
plugin register API. You can set `download` to `true` when registering an 
Official HashiCorp plugin to have Vault fetch the plugin binary from the
[HashiCorp release page](https://releases.hashicorp.com). Refer to the
[Plugin Catalog API reference](/vault/api-docs/system/plugins-catalog#download)
for more details.


## Before you start

- **Enterprise Registration**: To register official enterprise plugins (ENT),
  you must have Vault v1.16.21+, 1.17.17+, 1.18.10+, 1.19.4+, or 1.20.x+.
- **Community Registration**: To register official community plugins (CE) with
  an extracted `.zip` file, you must have Vault v1.16.21+, 1.17.17+, 1.18.10+,
  1.19.5+, or 1.20.x+.
...
The registration flow differs depending on whether you are using a standalone
binary (Community) or an official extracted artifact directory (Enterprise).
