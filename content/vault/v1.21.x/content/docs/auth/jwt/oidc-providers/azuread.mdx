...
1. Under **Token configurations**, add a groups claim. For "Customize token properties by type" in the claim, ensure at least **ID** is checked.

1. Under **API permissions**, ensure that there are permissions to read groups and users. It is depending on how AD is set up.
