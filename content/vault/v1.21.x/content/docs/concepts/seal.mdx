...
When you enable DR or PR replication in Vault Enterprise, [performance standby](/vault/docs/enterprise/performance-standby) nodes on the DR cluster seal themselves, so you must restart them to unseal them.
