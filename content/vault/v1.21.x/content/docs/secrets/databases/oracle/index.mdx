...
For the Enterprise Oracle database plugin, the following table lists the Oracle Instant
Client version each release was built and shipped against:

| Plugin release | Instant Client built against | Architecture | Qualification status |
| -------------- | ---------------------------- | ------------ | -------------------- |
| 0.14.1+ent     | 19.26                        | linux/amd64  | Not yet qualified    |
| 0.14.0+ent     | 19.26                        | linux/amd64  | Not yet qualified    |
| 0.13.0+ent     | 19.26                        | linux/amd64  | Not yet qualified    |
| 0.12.3+ent     | 19.26                        | linux/amd64  | Not yet qualified    |
| 0.11.0+ent     | 19.26                        | linux/amd64  | Not yet qualified    |

~> Instant Client 19.26 is no longer listed on Oracle's download page. The current 19.x
release is 19.31. Refer to the Instant Client family support table below before choosing a
version.

### Instant Client family support

Plugin releases 0.11.0+ent through 0.14.1+ent are dynamically linked against
`libclntsh.so.19.1`. Every 19.x Instant Client provides that library name, so any 19.x
release satisfies the plugin's load-time dependency. The 21.x and 23.x families provide
`libclntsh.so.21.1` and `libclntsh.so.23.1` instead, so the plugin cannot start against
them.

| Instant Client family | Provides             | Loads with 0.11.0+ent - 0.14.1+ent | Tested by HashiCorp    | Minimum Oracle database server |
| --------------------- | -------------------- | ---------------------------------- | ---------------------- | ------------------------------ |
| 19.x                  | `libclntsh.so.19.1`  | Yes                                | Built against 19.26 only; 19.31 not yet qualified | 11.2 |
| 21.x                  | `libclntsh.so.21.1`  | No                                 | Untested               | 12.1                           |
| 23.x                  | `libclntsh.so.23.1`  | No                                 | Untested               | 19c                            |
...
~> Do not create a symlink from `libclntsh.so.19.1` to a 21.x or 23.x library. Doing so
silences the loader without providing any ABI guarantee, and failures surface later as
symbol resolution errors or memory corruption rather than a clear startup error.
