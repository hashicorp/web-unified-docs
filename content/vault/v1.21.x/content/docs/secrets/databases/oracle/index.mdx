...
last_modified: 2025-12-19T15:06:54-08:00
...
For `linux/amd64`, download prebuilt binaries from the [releases page](https://releases.hashicorp.com/vault-plugin-database-oracle/).

Refer to [Oracle database plugin release notes](/vault/docs/secrets/databases/oracle/release-notes).
...
The following matrix distinguishes the build-time Oracle Instant Client software
development kit (SDK) from the exact runtime client and Vault Enterprise versions
tested with the Oracle database plugin. A build SDK version alone does not
establish runtime coverage.

| Plugin release | Build SDK | Runtime Instant Client | Vault `1.19.21+ent` | Vault `1.21.10+ent` | Vault `2.0.4+ent` | Vault `2.1.0+ent` |
| -------------- | --------- | ---------------------- | ------------------ | ------------------ | ---------------- | ---------------- |
| `0.14.1+ent` | `19.26` | `19.32.0.0.0` | Tested | Tested | Tested | Tested (1) |
| `0.14.1+ent` | `19.26` | `19.26.0.0.0` | Tested (3) | Tested | Tested | Tested (2) |
| `0.14.1+ent` | `19.26` | `21.19.0.0.0` | Tested | Tested | Tested | Tested |
| `0.14.1+ent` | `19.26` | `23.9.0.25.07` | Tested | Tested | Tested | Tested |
| `0.14.0+ent` | `19.26` | `19.26.0.0.0` | Tested | Tested | Tested | Tested |
| `0.13.0+ent` | `19.26` | `19.26.0.0.0` | Tested | Tested | Tested | Tested |
| `0.12.3+ent` | `19.26` | `19.26.0.0.0` | Untested | Untested | Untested | Untested |
| `0.11.0+ent` | `19.26` | `19.26.0.0.0` | Untested | Untested | Untested | Untested |

**Tested** means the exact combination passes plugin registration and connection
verification, dynamic credential creation and authentication, lease renewal,
revocation with and without an open session, administrator credential rotation,
and managed and self-managed static credential retrieval and rotation. Static
rotation checks cover manual and automatic 60-second period-based rotation.
The 24 tested configurations have 192 passing final functional-check results.

Test observations:

1. Initial dynamic-role setup omitted the `CONNECT` grant. After correction, the
   affected checks passed. Teardown required explicitly terminating one lingering
   self-managed test session after `ORA-01940`; its underlying cause is unresolved.
2. An image-build retry removed leftover client metadata before functional testing.
3. A complete rerun on fresh storage passed after an interrupted test session.
   The results retain the incomplete attempt separately.

The totals exclude retries, the interrupted attempt, and earlier loader-only or
smoke tests.

Tests use unchanged released `linux/amd64` plugin binaries on Ubuntu 22.04 under
Colima/QEMU emulation on Apple Silicon, with Oracle Database XE `21.3.0.0.0`.
Runtime packages are Instant Client Basic Linux x64 ZIPs, except for `21.19.0.0.0`,
which uses the complete EL8 RPM payload extracted into Ubuntu without running RPM
installation scripts.

**Untested** means these functional checks have not been run for that combination;
it does not mean incompatible. Vault versions and plugin/client pairs not listed
as tested remain untested. For `0.13.0+ent` and `0.14.0+ent`, this includes
client `19.32.0.0.0` and all 21.x and 23.x clients.
Results do not extend to other platforms, database editions, package formats,
rotation scheduling modes, high-availability deployments, or upgrade paths.

These results identify tested versions, not minimum supported Vault versions.
They do not replace [plugin release requirements](/vault/docs/secrets/databases/oracle/release-notes)
or support lifecycle policy. Supplementary source tests and release/security
review are separate from these functional checks.

### Instant Client family support

The tested `0.13.0+ent`, `0.14.0+ent`, and `0.14.1+ent` binaries are dynamically
linked against `libclntsh.so.19.1`. That shared-library name does not require the exact `19.26`
patch used to build the plugin. The tested 21.19 and 23.9 packages include
Oracle-supplied compatibility links that resolve this name to their own client
libraries. Loader checks and the connected plugin's memory mappings confirm use
of the selected newer libraries.

| Instant Client family | Library in tested packages | Resolution of `libclntsh.so.19.1` | Minimum Oracle database server family |
| --------------------- | -------------------------- | -------------------------------- | ------------------------------------- |
| 19.x | `libclntsh.so.19.1` | Direct library | 11.2 |
| 21.x | `libclntsh.so.21.1` | Oracle-supplied compatibility link | 12.1 |
| 23.x | `libclntsh.so.23.1` | Oracle-supplied compatibility link | 19c |

The minimum database server column matters when selecting a client. A 23.x client cannot
connect to an Oracle database server older than 19c, so moving to a newer Instant Client
family is not universally safe.

~> Use a complete Oracle Instant Client package and preserve its vendor-supplied
compatibility links. Do not mix libraries from different client installations or
create arbitrary replacement links. If a required link is absent or broken,
reinstall the selected package and verify the plugin's library search path.
