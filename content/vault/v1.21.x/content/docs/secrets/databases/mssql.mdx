...
<Warning>
  ...
  For operations such as `ALTER ROLE ... ADD MEMBER ...`, a schema modification
  lock (`SCH-M`) lock is acquired. This will block even read operations, since
  read operations aquire a schema stability lock (`SCH-S`). As a result,
