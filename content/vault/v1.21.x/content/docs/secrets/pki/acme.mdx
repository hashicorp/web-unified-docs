...
Based on the challenge type used the ACME client will need to provision a specific resource on the domain it
is trying to get a certificate for, such as a specific HTTP endpoint for HTTP-01 or a specific DNS record for DNS-01.
Once the client has successfully proven control over the domain, it can request a certificate issuance from the ACME CA.
...
that will be used for a given directory path and `default_directory_policy` config.
...
mount to serve ACME clients. Each of which can be broken down within three main
categories.
...
which will need to be specified by [mount tuning](/vault/api-docs/system/mounts#tune-mount-configuration)
...
In order to use ACME, a [cluster path](/vault/api-docs/secret/pki#set-cluster-configuration) must
be configured for the PKI mount. The cluster path is used to supply the ACME base URL to
...
The ACME configuration is used to specify the default behavior of the top level ACME directory,
restrict ACME to certain issuers and roles along with the External Account Binding (EAB) policy.

A basic ACME configuration can be set with the following command. This is not recommended for production
use as it will allow any role and issuer on the mount to be used for ACME issuance along with any client
being able to register an account without an EAB token:
