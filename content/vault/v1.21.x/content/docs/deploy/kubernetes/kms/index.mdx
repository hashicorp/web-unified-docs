...
The Vault Kubernetes KMS plugin is a Kubernetes KMS v2 plugin that lets Kubernetes use
Vault Enterprise for encryption at rest. The Kubernetes API server uses a DEK seed to generate
data encryption keys (DEKs) for encrypting and decrypting cluster data. The API server encrypts
...
For details on configuring a KMS plugin and the KMS v2 API, refer to the
...
control plane can reach the KMS plugin's gRPC endpoint.
