...
created_at: 2026-03-11T04:47:29.000Z
last_modified: 2026-03-11T04:47:29.000Z
# END AUTO GENERATED METADATA
---

# Install Vault Kubernetes KMS
...
- You must have access, and the ability to make changes, to your Kubernetes
  control plane host.
- You must have permisison to modify the Kubernetes API server configuration and
...
The Vault Kubernetes KMS process, `vault-kube-kms`, communicates over a Unix
socket to encrypt/decrypt data before writing to, or reading from, `etcd`. As a
...
1. Update the manifest to add `--encryption-provider-config` and set it to your
   `encryption-config.yaml` file. For example:
