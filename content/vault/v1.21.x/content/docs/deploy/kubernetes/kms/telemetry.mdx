...
description: Telemetry for Vault Kubernetes KMS.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-01-12T16:15:37-05:00
last_modified: 2026-03-11T04:47:30.000Z
# END AUTO GENERATED METADATA
---

# Telemetry

If you enable metrics, Vault Kubernetes KMS exposes Prometheus-compatible
metrics on the `/metrics` endpoint. You can configure the endpoint using the
[metrics and telemetry parameters](/vault/docs/deploy/kubernetes/kms/configuration#metrics)
for Vault Kubernetes KMS.

<Note title="Vault Kubernetes KMS metrics are not stored in Vault">

  You can only access Vault Kubernetes KMS metrics through the `/metrics` endpoint
  of the Kubernetes plugin. You Vault server does not have access to that
  information.
...
|-------------|---------------|-------------|
| histogram   | milliseconds  | Measures the duration of inbound gRPC requests handled by Vault Kubernetes KMS. |

Vault Kubernetes KMS emits this metric per gRPC method and status code.
...
| Metric type | Value       | Description |
|-------------|-------------|-------------|
| counter     | operations  | Counts successful Vault client operations performed by Vault Kubernetes KMS. |
...
| Metric type | Value      | Description |
|-------------|------------|-------------|
| counter     | errors     | Counts failed Vault client operations performed by Vault Kubernetes KMS. |
...
| Metric type | Value    | Description |
|-------------|----------|-------------|
| histogram   | seconds  | Measures the duration of Vault client operations performed by Vault Kubernetes KMS. |
