...
description: Telemetry for Vault Kubernetes Key Management.
...
<Note title="Vault Kubernetes Key Management metrics are not stored in Vault">

  You can only access Vault Kubernetes Key Management metrics through the `/metrics` endpoint
  of the `vault-kube-kms` process. You Vault server does not have access to that
  information.
