...
The Unix domain socket address where the plugin listens for requests from the
Kubernetes API server. The kube-apiserver must have filesystem access to this
socket in order to communicate with the plugin.

This value must be a non-empty string.

- `vault-address` `(string: <required>)` –
The HTTP(S) API address of the Vault server. The plugin connects to this
...
The Vault namespace in which the plugin operates. If unset, the plugin uses
the root namespace.

This parameter is required when using Vault Enterprise with namespaces enabled.

- `transit-mount` `(string: <required>)` –
The mount path of the Vault Transit secrets engine used by the plugin to
...
The name of the Vault Transit key used by the plugin to encrypt and decrypt
...
The Vault authentication method used by the plugin. Supported values are `approle`.
---
...
Configures the plugin’s logging behavior using Zap logging levels.

Supported values (in order of descending detail) are `debug`, `info`, `warn`, and `error`

The plugin uses Zap for structured logging. Setting a more verbose level
...
The local TCP port where the plugin exposes a Prometheus-compatible
