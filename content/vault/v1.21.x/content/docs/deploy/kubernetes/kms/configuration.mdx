...
- `listen-address` `(string: <required>)` – The Unix domain socket address
  where the plugin listens for requests from the Kubernetes API server. The
  kube-apiserver must have filesystem access to this socket in order to
...
- `vault-namespace` `(string: nil)` – The Vault namespace in which the plugin operates.
...
- `transit-mount` `(string: <required>)` – The mount path of the Vault Transit
...
- `transit-key-name` `(string: <required>)` – The name of the Vault Transit key used
...
- `auth-method` `(string: <required>)` – The Vault authentication method used by the plugin.
...
- `approle-role-id` `(string: <required>)` – The AppRole Role ID used to authenticate
  to Vault. This value is not sensitive and can be provided directly as a flag.

- `approle-secret-id-path` `(string: <required>)` – The filesystem path to a file
...
- `log-level` `(string: "info")` – Log verbosity level.
  Supported values (in order of descending detail) are `trace`, `debug-extended`, `debug`, `info`, and `error`

---

## Metrics and telemetry parameters

- `metrics-port` `(int: nil)` – The local TCP port where the plugin exposes a
