...
1. The Vault administrator ties authentication (AppRole) to the policy and
   ...
   - **The secret ID** is a sensitive credential stored on the Kubernetes control plane node and read by Vault Kubernetes KMS from a filesystem path.

1. The Kubernetes administrator uses the Vault server address, appRole ID, and
   secret ID to configure Vault Kubernetes KMS.
...
1. Vault Kubernetes KMS reports the encrypted seed to the Kubernetes
