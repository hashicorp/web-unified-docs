...
1. Create volume for Vault configuration files.
...
1. Access config volume.

  ```shell-session
  $ docker run --rm -it -v vault-config:/vault alpine sh
  ```

1. Create configuration file for Leader and follower vault.
  ...
  Exposes Vault on port 8200 for API and UI traffic.
