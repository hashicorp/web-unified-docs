...
You need to have opencryptoki installed, and the Crypto Express adapter enabled for LinuxOne or Linux on IBM Z instances to use EP11 tokens. 
You also need to install opencryptoki which can then talk to EP11 token. 
Refer to the [IBM EP11 Token Documentation](https://www.ibm.com/docs/en/linux-on-systems?topic=support-exploiting-enterprise-pkcs-11-using-opencryptoki) for more details. 
