...
- `leaf_not_after_behavior` `(string: "err")` - Behavior of a leaf's
  ...
  - `permit` to allow this issuance to succeed with a `NotAfter` value
    ...
    ~> **Note**: Prior to Vault 1.17.x, the default behavior for `leaf_not_after_behavior` was `permit` for intermediate CAs.
       Starting in Vault 1.17.x, the default behavior for intermediate CAs is `truncate` when `leaf_not_after_behavior` is set to `err` (the default).
       This ensures that intermediate certificates are not issued with a validity period extending beyond the signing CA.
       To restore the previous behavior, explicitly set `leaf_not_after_behavior` to `permit`.
