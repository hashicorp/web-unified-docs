...
result, the `start_time` and `end_time` values in API responses are normalized to
align with full calendar months. Please see below behavior differences between
Enterprise and Community editions for more details.

Normalization occurs whether you provide explicitly times in the request or use
the defaults:
