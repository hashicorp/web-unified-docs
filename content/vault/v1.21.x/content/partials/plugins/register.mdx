...
<Tab heading="Enterprise (extracted .zip folder)" group="ent">

For official Enterprise plugins, `-sha256` and `-command` are **not** used.
Vault automatically discovers the binary within the extracted directory. You
**must** provide the `-version` flag, and it must match the version string in
the directory name (e.g., `0.18.0+ent`).
...
1. Identify the version from your extracted plugin directory:
    ```shell-session
    $ ls ~/dev/plugins/
    vault-plugin-secrets-keymgmt_0.18.0+ent_darwin_arm64/
    ```

1. Register using the `-version` flag and the official plugin name:
    ```shell-session
    $ vault plugin register \
        -version="0.18.0+ent" \
        secret \
...
<Tab heading="Community Edition (Binary)" group="ce">

For CE or third-party plugins, you must provide the SHA256 checksum and the
specific command Vault should use to execute the binary.
