...
- **Log file management**. Due to volume of Vault logs generated, consider rotating
  and compressing these logs to disk, or send them to a central location or service.
...
- **Least privilege and bias towards simple policies**. We recommend enforcing the
  principle of least privilege when creating policies, and ensuring the policies
  are as simple and as explicit as possible, and resorting to the use of globs,
  wildcards, templates only when necessary.
...
- **Consider a separate partition for Vault's Integrated Storage**. This will ensure
  graceful degradation in the event Vault consumes all storage available.

- **Use an administrative namespace for day-to-day admin operations**. [Administrative Namespace](/vault/docs/namespaces/create-admin-namespace)
  should be used to enforce least-privilege for administration operation, and the `root`
  namespaces should be used only when strictly necessary.

- **Authenticated Reverse proxy**. While Vault is meant to defend against unauthenticated
  attackers, an authenticated reverse proxy may provide defense-in-depth, and guard against
  scenarios where Vault is not running the latest version and may be affected by known vulnerabilities.

- **Use an outbound network proxy**. Vault can be configured to access a wide
  range of network services and endpoints. In addition to network segmentation,
  the use of an outbound proxy to further restrict and observe Vault traffic
  is recommended.