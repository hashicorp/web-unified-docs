...
reflects the latest committed state. Vault provides the same behavior when you use Consul with `consistency_mode = "strong"`.
...
standby presents a stale view of the data. The potential for stale data means that Vault cannot guarantee read-after-write consistency, even across requests to the same performance standby.

The risk of read-after-write inconsistency is specific to integrated storage with
performance standby nodes.

Vault provides mitigations for read-after-write inconsistency. Refer to [Eventual consistency in
...
| Consistency model   | Eventual consistency by default; `strong` mode available | Strongly consistent without performance standbys; eventual consistency possible with performance standbys (**Vault Enterprise**) |
