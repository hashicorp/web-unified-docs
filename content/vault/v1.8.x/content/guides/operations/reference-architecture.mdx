...
backend](/vault/docs/internals/architecture) since that is the recommended
...
Eight Nodes with [Consul Storage Backend](/vault/docs/configuration/storage/consul)
...
or load balancer;
[X-Forwarded-For Headers](/vault/docs/configuration/listener/tcp#x_forwarded_for_authorized_addrs)
and [PROXY v1](/vault/docs/configuration/listener/tcp#proxy_protocol_authorized_addrs). Both require a trusted load balancer and require IP address whitelisting to
...
and **disaster recovery**. The [Vault documentation](/vault/docs/enterprise/replication) provides more detailed information on the replication capabilities within Vault Enterprise.
...
- Vault [architecture](/vault/docs/internals/architecture) documentation explains
  each Vault component
- To integrate Vault with existing LDAP server, refer to
  [LDAP Auth Method](/vault/docs/auth/ldap) documentation
