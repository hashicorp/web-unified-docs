...
method**](/vault/docs/auth) to acquire a token. This token has policies attached so
...
[**LDAP**](/vault/docs/auth/ldap) auth method enables user authentication using
an existing LDAP server while [**AppRole**](/vault/docs/auth/approle) auth
...
enabling the [**AppRole**](/vault/docs/auth/approle) auth method.

## Reference Material

- [AppRole Auth Method](/vault/docs/auth/approle)
- [AppRole Auth Method (API)](/vault/api-docs/auth/approle)
...
[AppRole](/vault/docs/auth/approle) is an authentication mechanism within Vault
...
parameters](/vault/api-docs/system/auth#enable-auth-method) of the method.
...
> [parameters](/vault/api-docs/auth/approle#create-update-approle) that you can set
...
[parameters](/vault/api-docs/auth/approle#generate-new-secret-id) such as
`cidr_list`.
...
You can pass
[parameters](/vault/api-docs/auth/approle#generate-new-secret-id) in the request
...
wrapping**](/vault/docs/concepts/response-wrapping) to transmit the _reference_
