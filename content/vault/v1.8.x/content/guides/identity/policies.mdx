...
[**`root`**](/vault/docs/concepts/policies#root-policy) policy gets created by
...
[**`default`**](/vault/docs/concepts/policies#default-policy), is created. The
...
Enterprise](/vault/docs/enterprise). Since Sentinel is an enterprise-only
...
- [Policies](/vault/docs/concepts/policies#default-policy) documentation
- [Policy API](/vault/api-docs/system/policy) documentation
...
Define one or more [capabilities](/vault/docs/concepts/policies#capabilities) on each path to control operations that are
permitted.
...
[`sys/policies/acl`](/vault/api-docs/system/policies#create-update-acl-policy)
endpoint:
...
[`sys/policy`](/vault/api-docs/system/policy) endpoint as well.
