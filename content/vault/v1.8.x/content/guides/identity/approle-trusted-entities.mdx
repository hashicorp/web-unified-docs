...
In the [AppRole Pull
Authentication](/guides/identity/authentication#advanced-features) guide,
...
method](/vault/docs/auth/approle), along with Terraform and Chef, to address the
...
[response-wrapped](/guides/secret-mgmt/cubbyhole) `SecretID` and deliver it
...
- **Option 2:** Continue onto [Step 3: AppRole Setup](#step-3-approle-setup) to
  set up the demo environment **_manually_**.
...
Refer to the [online documentation for initializing and unsealing](/intro/getting-started/deploy#initializing-the-vault) Vault for more details.
...
**_only_** retrieve `SecretID` from Vault which is not much of a use without a
