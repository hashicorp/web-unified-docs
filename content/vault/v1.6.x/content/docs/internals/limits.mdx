...
sidebar_title: Limits and Maximums
...
[`sys/auth`](/vault/api-docs/system/audit) and
[`sys/mounts`](/vault/api-docs/system/mounts) endpoints, which contain all mount points in all namespaces.
...
point limit by the larger of of the number of auth mounts per namespace
...
Instead, consider using [external groups](/vault/docs/secrets/identity#external-vs-internal-groups) or split the group up into multiple sub-groups.
...
the `maximum_request_size` option in the [listener stanza](/vault/docs/configuration/listener/tcp). It defaults to 32 MiB.  This value, minus the overhead of
