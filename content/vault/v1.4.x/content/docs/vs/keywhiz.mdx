...
secrets. Vault supports [mTLS authentication](/vault/docs/auth/cert) along with
many [other mechanisms](/vault/docs/auth). The goal is to make it easy
...
system](/vault/docs/concepts/policies) is used to protect secrets and gate
access, similarly to the Keywhiz ACL system. With Vault, all auditing is done
server side using [audit devices](/vault/docs/audit).
