...
therefore, only the **_trusted entities_** should have an access to your
secrets.
...
of that first secret. Getting the first secret to the consumer, is the **_secure
introduction_** challenge.
...
[Tokens](/vault/docs/concepts/tokens) are the core method for authentication
...
- [AppRole Auth Method](/vault/docs/auth/approle)
  - [AppRole Pull Authentication](/guides/identity/authentication)
  - [AppRole with Terraform and Chef Demo](/guides/identity/approle-trusted-entities)
- [TLS Certificates Auth Method](/vault/docs/auth/cert)
- [Token Auth Method](/vault/docs/auth/token)
  - [Cubbyhole Response Wrapping](/guides/secret-mgmt/cubbyhole)
...
- Automatically authenticates to Vault for those [supported auth methods](/vault/docs/agent/autoauth/methods)
...
To leverage this feature, run the vault binary in agent mode (`vault agent -config=<config_file>`) on the client. The agent configuration file must specify
the auth method and [sink](/vault/docs/agent/autoauth/sinks) locations
...
- [Vault Agent documentation](/vault/docs/agent)
- [Auto-Auth documentation](/vault/docs/agent/autoauth)
