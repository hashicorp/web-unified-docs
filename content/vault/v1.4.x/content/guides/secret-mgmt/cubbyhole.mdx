...
using the [Vault policy](/vault/docs/concepts/policies) and attach that to
...
by the [`default`](/vault/docs/concepts/policies#default-policy) policy.
...
Also, refer to [Cubbyhole Secret Engine (API)](/vault/api-docs/secret/cubbyhole).
