...
Vault supports custom HTTP response headers for the root path (`/`) and API
endpoints (`/v1/*`) based on the returned status code. For example, you can
define a list of custom response headers for the `200` status code, and another
list of custom response headers for the `307` status code.

Use the [`/sys/config/ui/headers` endpoint](/vault/api-docs/system/config-ui-headers)
to set `UI` specific custom headers. You cannot configure the same header in a
configuration file and the `/sys/config/ui/headers` endpoint. To modify or
remove custom header values in your Vault configuration file, you need to update
the file accordingly and send a `SIGHUP` signal to restart the Vault process.

Vault rejects any custom header also used by internal Vault processes. For
example, Vault rejects any custom header with the `X-Vault-` prefix. If you
define a custom header called `X-Vault-My-Header`, Vault rejects the header on
startup and logs the fact that it rejected the custom header.

### Order of precedence

If a header definition exists in your Vault configuration file and under the
`/sys/config/ui/headers` endpoint, Vault defers to the header in the
configuration file. For example, if you define a default value for
`"Content-Security-Policy"` in the `/sys/config/ui/headers` endpoint and the
same header exists in the configuration file, Vault sets the configuration file
the response headers instead of the default value in `/sys/config/ui/headers`.
