...
When DR or PR replication is enabled in Vault Enterprise, [Performance Standby](/vault/docs/enterprise/performance-standby) nodes on the Vault DR or PR secondary cluster will seal themselves, so they must be restarted to be unsealed.
