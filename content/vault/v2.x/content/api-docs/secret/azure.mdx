...
- `seamless_rotation` (`bool`) - If set to `true`, Vault enables seamless rotation for the role, or disables it when `false`. If unset during role creation, Vault uses the `seamless_rotation` value from the
