...
last_modified: 2026-09-04T16:03:52.000Z
...
- `seamless_rotation` (`bool: true`) - <EnterpriseAlert product="vault" inline /> Enable or disable seamless rotation for newly created static roles.
...
<Note title="Seamless rotation not available">
Vault does not support seamless rotation for root credentials.
</Note>
...
- `metadata` (`map[string]string: {}`) - A map of key/value pairs associated with the role.
  You can read metadata values from both the `/azure/roles/:name` and `/azure/creds/:name`
  endpoints. Vault merges role metadata with any metadata provided in the request
  when returning credentials. If metadata keys overlap, the role metadata always
  takes precedence.
...
- `metadata` (`map: {}`) - Key/value pairs attached to the
  credential request. Vault merges request metadata with any
  metadata stored for the role, with role metadata taking
  precedence when keys overlap.  Key/value pairs appear in the
  response as strings separated by the `=` symbol (`key=value`).
  When using the Vault CLI, you can provide multiple tags in the role
...
- `ttl` (`string: ""`) - **Deprecated**. Alias for `rotation_period`. Kept for
  backward compatibility.
- `rotation_period` (`string: ""`) - Timespan of 1 month (`720h`) or more during
...
- `defer_initial_creds` (`bool`, `false`) - If set to `true`,
  Vault defers initial credential generation until the first time
  a client reads credentials from the role. Defaults to `false`.
- `secret_id` (`string: ""`) - **Deprecated - Use the [Import endpoint](/vault/api-docs/secret/azure#import-static-role)
   to import existing credentials** - The secret ID of the Azure password credential you want to import.
   Required if you provide `client_secret` or `expiration`.
- `client_secret` (`string: ""`) - **Deprecated - Use the [Import endpoint](/vault/api-docs/secret/azure#import-static-role)
   to import existing credentials** - The plaintext secret value of the credential you want to import.
   If unset, you must rotate the imported credential before you can use it with Vault.
- `expiration` (`string: ""`) - **Deprecated** - A future expiration time for the imported
   credential, in RFC3339 format.
- `skip_import_rotation` (`bool`, `false`) – **Deprecated** - Alias for `defer_initial_creds`, kept for backward compatibility.
   Controls whether Vault creates initial credentials during static role
   creation or import. Use `defer_initial_creds` instead. For importing existing credentials, use the [Import endpoint](/vault/api-docs/secret/azure#import-static-role)
- `seamless_rotation` (`bool`) - Enable or disable seamless rotation for the role.
  Leave `seamless_rotation` unset to use the `seamless_rotation` value from the
  plugin configuration at `/azure/config`.
- `rotation_grace_period` `(string/integer: 300)` - The duration of lifetime overlap
  between two credentials when `seamless_rotation` is enabled. The overlap allows
  the staged credential to activate prior to the expiration of the active credential.
  The grace period cannot exceed `(rotation_period - 1h) / 2`. Set to `-1` to disable
  the overlap or `0` to reset to the default value.
...
Import existing Azure credentials to create a static role. Use
the Create/Update static role endpoint for subsequent updates to
the role.

| Method | Path                               |
| :----- | :----------------------------------|
| `POST` | `/azure/static-roles/:name/import` |
...
- `application_object_id` (`string: <required>`) -  Application object ID for
an existing service principal currently managed by the static role.
- `ttl` (`string: ""`) - **Deprecated**. Alias for `rotation_period`. Kept for
  backward compatibility.
- `rotation_period` (`string: ""`) - Maximum timespan during which the role
  credentials are valid. You can specify timespans of 1 month
  (`720h`) or more as time suffixed strings (`"1h"`) or an
  integer number of seconds. Leave `ttl` unset or set to `0` to
  use the default value of 2 years (`17520h`).
- `metadata` (`map[string]string: {}`) - A map of key/value pairs associated with the static role.
You can read metadata values from both the `/azure/static-roles/:name` and `/azure/static-creds/:name`
endpoints. When Vault fetches credentials it merges role metadata with any metadata provided in the request,
and role metadata always takes precedence if keys overlap.
- `secret_id` (`string: <required>`) - The secret ID of the Azure password credential you want to import.
- `client_secret` (`string: ""`) - The plaintext secret value of the credential you want to import.
If unset, you must rotate the imported credential before you can use the client secret with Vault.
- `skip_import_rotation` (`bool`, `false`) - Determines whether Vault should skip rotating the imported credential
upon role creation. Set `skip_import_rotation` to `true` to import the provided credential without performing rotation.
If you skip rotation, you can rotate the credential manually by rotating the role using `/rotate-role` endpoint.
If you omit `skip_import_rotation` or set it to `false`, Vault rotates the imported credential during role creation.
- `seamless_rotation` (`bool`) - Enable or disable seamless rotation for the role.
  If not specified, defaults to the value of `seamless_rotation` stored within the
  plugin's configuration at `/azure/config`.
- `rotation_grace_period` `(string/integer: 300)` - The duration of lifetime overlap
  between two credentials when `seamless_rotation` is enabled, which allows the staged
  credential to activate prior to the expiration of the active credential. Must not
  exceed `(rotation_period - 1h) / 2`. Set to `-1` to disable overlap or `0` to use
  the default value of `5m`.
...
{
  "application_object_id": "408bf248-dd4e-4be5-919a-7f6207a307ab",
  "secret_id": "43193751-2cd7-4d03-9472-fb1024ldkmnw",
  "ttl": "8760h",
  "metadata": {
    "team": "engineering",
    "environment": "development"
...
You must upgrade to Vault 2.2.0 or later to generate OAuth2 access tokens.

</Note>

Generate an OAuth2 access token for the named static role.

Vault uses the `client_credentials` grant type to call the Azure AD token
endpoint and exchange the `client_id` and `client_secret` associated with the
role for an access token. The caller supplies the scope and Vault returns a
token directly from the Azure endpoint.

Vault does not create a lease for the returned token and you cannot revoke the
access token once Vault issues it. The Azure access tokens expire naturally based
on the TTL configured in Azure (~1 hour by default).


The token endpoint returns a `400` error if Vault has never rotated the targeted
static role. For example, if you create a role with `defer_initial_creds=true`
and have not yet rotated the credential, Vault returns an error because it cannot
find an associated `client_secret` to use.

We recommend always rotating the role with the [`/azure/rotate-role/:name`](#manually-rotate-static-role-password)
endpoint before calling the token endpoint for the first time.


| Method | Path                                    |
| :----- | :-------------------------------------- |
| `POST` | `/azure/static-creds/:name/token`       |
...
$ curl \
    ...
    ${VAULT_ADDR}/v1/azure/static-creds/my-static-role/token
```


### Sample response

Vault wraps the Azure token response with a `data` envelope:
...
<Warning title="Synchronization known issue">
  State propagation delays may result in orphaned credentials.
  Refer to the
  [Azure rotate static credentials cannot run in quick succession](/vault/docs/updates/important-changes#azure-sync-issue)
  entry in the Important Changes doc for more details.
</Warning>
