...
revoked or renewed once issued; they expire naturally (~1 hour by default, configurable in
Azure).
...
been rotated), because Vault has no stored `client_secret` to use. Rotate the role with the
...
$ curl \
    ...
    http://127.0.0.1:8200/v1/azure/static-creds/my-static-role/token
```
...
```shell-session
$ vault write azure/static-creds/my-static-role/token \
    scope="https://graph.microsoft.com/.default"
```
