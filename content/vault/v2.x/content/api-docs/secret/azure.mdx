...
- `ttl` (`string: ""`) - **Deprecated in Vault 2.2.0**. Alias for `rotation_period`. Kept for
  backward compatibility.
- `rotation_period` (`string: ""`) - **Requires Vault 2.2.0 or later**. Timespan of 1 month (`720h`) or more during
   which the role credentials are valid. You can specify timespans
   as time suffixed strings (`"1h"`) or an integer number of
   seconds. Leave `rotation_period` unset or set to `0` to use the default value
   of 2 years (`17520h`).
...
- `seamless_rotation` (`bool`) - **Requires Vault 2.2.0 or later**. If set to `true`, Vault enables seamless rotation for the role, or disables it when `false`. If unset during role creation, Vault uses the `seamless_rotation` value from the
  plugin configuration at `/azure/config`.
- `rotation_grace_period` `(string/integer: 300)` - **Requires Vault 2.2.0 or later**. The duration of lifetime overlap
...
- `ttl` (`string: ""`) - **Deprecated in Vault 2.2.0**. Alias for `rotation_period`. Kept for
  backward compatibility.
- `rotation_period` (`string: ""`) - **Requires Vault 2.2.0 or later**. Maximum timespan during which the role
  credentials are valid. You can specify timespans of 1 month
  (`720h`) or more as time suffixed strings (`"1h"`) or an
  integer number of seconds. Leave `rotation_period` unset or set to `0` to
