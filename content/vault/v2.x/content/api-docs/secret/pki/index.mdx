...
This endpoint requires a **sudo token**. Standard policy `sudo` capability
grants satisfy this requirement.
...
- `public_key` `(string: <required>)` - PEM-encoded public key used to wrap
  ...
  key types: RSA-2048, RSA-3072, RSA-4096, RSA-8192, EC P-256/P-384/P-521,
...
Exactly one of `pem_bundle` or the `wrapped_key` + `export_key_hmac` pair must
be provided.
