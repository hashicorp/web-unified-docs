...
- [Managing Keys and Issuers](#managing-keys-and-issuers)
  ...
  - [Export CA Key <EnterpriseAlert inline="true" />](#export-ca-key)
...
~> **Note**: If the private key associated with this issuer has been exported
   via [Export CA key](#export-ca-key), Vault will include a warning message in
   the `warnings` array of this response.
...
~> **Warning**: Deleting an export key is irreversible. If the corresponding
   wrapped CA key blob has not yet been imported, it will be permanently
   unrecoverable.
...
### Export CA key <EnterpriseAlert inline="true" />
...
~> **Important**: This endpoint requires a **sudo token**. Standard policy
   `sudo` capability grants satisfy this requirement.

~> **Note**: This endpoint is not supported for managed (HSM-backed) keys.
   See [Managed Keys](#managed-keys) for details.
...
- `ca_key_uuid` `(string: <required>)` - UUID of the CA key to export.
  This parameter is part of the request URL.
...
- `public_key` `(string: <required>)` - PEM-encoded public key used to wrap
  ...
  key types: RSA (any modulus size), EC P-256/P-384/P-521, ML-KEM-768,
  ML-KEM-1024.
...
~> **Note**: Exactly one of `pem_bundle` or the `wrapped_key` + `export_key_hmac` pair must
  be provided.
...
#### Sample response (wrapped key) <EnterpriseAlert inline="true" />
