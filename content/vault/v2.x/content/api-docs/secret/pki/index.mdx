...
- `private_key_format` `(string: "der")` - Specifies the format for marshaling
  the private key within the `private_key` response field. Defaults to `der`
  which returns either base64-encoded DER or PEM-encoded DER, depending on the
  value of `format`. Vault does not return `private_key` as a separate field when format 
  is `pkcs12_bundle` or `jks_bundle`. The other option is `pkcs8` which will return 
