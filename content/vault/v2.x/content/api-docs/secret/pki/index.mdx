...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  `pem`, `der`, `pem_bundle`, `pkcs12_bundle`, or `jks_bundle`; defaults to `pem`. 
  If `der`, `pkcs12_bundle` or `jks_bundle` the output is base64 encoded. 
  If `pem_bundle`, the `certificate` field will contain the private key and certificate, 
  concatenated; if the issuing CA is not a Vault-derived self-signed root, this will be
  included as well. If `pkcs12_bundle` or `jks_bundle`, the PKCS#12 archive or Java keystore 
  (JKS) contains the private key, certificate, and CA chain.

  ~> **Note** The `jks_bundle` format is provided only for compatibility with legacy 
  systems and should be avoided for new usage. Prefer `pkcs12_bundle`.
...
- `private_key_format` `(string: "der")` - Specifies the format for marshaling
  ...
  PEM-encoded PKCS8. `private-key` is not returned as a separate field when format 
...
- `pkcs12_password` `(string: "changeit")` - Password for encrypting the PKCS#12 
  archive when format is set to `pkcs12_bundle`. If not provided, defaults to 
  `changeit`. It is recommended to use the default password and protect the file 
  using other means or use a high-entropy password. 

- `jks_password` `(string: "changeit")` - Password for encrypting the Java keystore 
  when format is set to `jks_bundle`. If not provided, defaults to `changeit`. It is 
  recommended to use the default password and protect the file using other means or 
  use a high-entropy password. 

- `jks_private_key_alias` `(string: "1")` - The entry alias in the Java keystore (JKS) when 
  format is set to `jks_bundle` and bundle contains a single PrivateKeyEntry. This 
  field is case-sensitive, but relying on case-only differences for unique aliases 
  is not recommended. Defaults to `1`. This parameter is ignored by endpoints that 
  return TrustedCertificateEntry values (trust stores), and instead entries are 
  assigned incrementing numeric string aliases starting at `"1"`. 
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  `pem`, `der`, `pem_bundle`, `pkcs12_bundle`, or `jks_bundle`; defaults to `pem`. 
  If `der`, `pkcs12_bundle` or `jks_bundle` the output is base64 encoded. 
  If `pem_bundle`, the `certificate` field will contain the certificate and, 
  if the issuing CA is not a Vault-derived self-signed root, it will be 
  concatenated with the certificate. If `pkcs12_bundle` or `jks_bundle`, 
  the PKCS#12 archive or Java keystore (JKS) contains the certificate and CA chain 
  (no private key).
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  concatenated; if the issuing CA is not a Vault-derived self-signed root, this will be 
  included as well. If `pkcs12_bundle` or `jks_bundle`, the PKCS#12 archive or Java keystore 
  (JKS) contains the certificate and CA chain (no private key).
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  concatenated with the certificate. If `pkcs12_bundle` or `jks_bundle`, 
  the PKCS#12 archive or Java keystore (JKS) contains the certificate and CA chain 
  without a private key for signed certificates.
...
- `pkcs12_encoder` `(string: "modern2026")` - Encoder profile to use for PKCS#12 
  archives when format is set to `pkcs12_bundle`. Valid values are `modern2026` and 
  `modern2023`. Defaults to `modern2026`, which uses the newer PKCS#12 integrity 
  format (PBMAC1). Only applicable when `format=pkcs12_bundle`.
...
- `format` `(string: "pem")` - Specifies the format for returned data. Can be
  ...
  will contain the private key (if exported) and certificate, concatenated; if the 
  issuing CA is not a Vault-derived self-signed root, this will be included as well. 
  If `pkcs12_bundle` or `jks_bundle`, the output is a base64-encoded PKCS#12 archive 
  or Java keystore (JKS) containing the certificate and CA chain. For `type=exported`, 
  the private key is included; for other types (`internal`, `existing`, `kms`), only 
  the certificate and CA chain are included (trust store).
...
- `jks_password` `(string: "changeit")` - Password for encrypting the Java keystore 
  when format is set to `jks_bundle`. If not provided, defaults to `changeit`. It is 
  recommended to use the default password and protect the file using other means or 
  use a high-entropy password. Only applicable when `format=jks_bundle`.

- `jks_private_key_alias` `(string: "1")` - The entry alias in the Java keystore (JKS) when 
  format is set to `jks_bundle`. This field is case-sensitive, but relying on 
  case-only differences for unique aliases is not recommended. Defaults to `1`. 
  For `type=exported`, this is used for the PrivateKeyEntry; for other types, this 
  parameter is ignored and entries are assigned incrementing numeric string aliases 
  starting at `1`. Only applicable when `format=jks_bundle`.
