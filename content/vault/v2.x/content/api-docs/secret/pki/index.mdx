...
- `key_type` `(string: "rsa")` - Specifies the desired key type; must be `rsa`, `ed25519`,
  `ec`, or <EnterpriseAlert inline="true" /> `ml-dsa`.
...
- `key_bits` `(int: 0)` - Specifies the number of bits to use for the
  ...
  384, or 521; ignored with `key_type=ed25519` or `key_type=ml-dsa`.

- `parameter_set` `(string: "44")` - <EnterpriseAlert inline="true" /> For ML-DSA keys, specifies the parameter set for it;
  must be `"44"`, `"65"`, or `"87"`.
...
~> **Note**: ECDSA, Ed25519, and ML-DSA issuers do not follow configuration of the
...
- `key_type` `(string: "rsa")` - Specifies the desired key type; must be `rsa`, `ed25519`, `ec`
  or <EnterpriseAlert inline="true" /> `ml-dsa`.
...
~> **Note**: For ML-DSA keys, the response also includes a `parameter_set` field indicating
   the parameter set used (e.g., `"44"`, `"65"`, or `"87"`).
...
- `key_type` `(string: "rsa")` - Specifies the desired key type; must be `rsa`, `ed25519`, `ec` or <EnterpriseAlert inline="true"/> `ml-dsa`. 
  Not suitable for `type=existing` requests.
...
- `key_bits` `(int: 0)` - Specifies the number of bits to use for the
  ...
  `key_type=rsa`, allowed values are: 2048 (default), 3072, 4096, or 8192;
  with `key_type=ec`, allowed values are: 224, 256 (default),
  384, or 521; ignored with `key_type=ed25519` or `key_type=ml-dsa`. Not suitable for
...
This endpoint allows an operator to import a single pem encoded `rsa`, `ec`, `ed25519`, or
<EnterpriseAlert inline="true" /> `ml-dsa` key.
...
- `key_type` `(string: "rsa")` - Specifies the type of key to generate for
  ...
  Currently, `rsa`, `ec`, `ed25519`, and <EnterpriseAlert inline="true"/> `ml-dsa` are supported, or when signing
...
- `key_bits` `(int: 0)` - Specifies the number of bits to use for the
  ...
  `key_type=rsa`, allowed values are: 2048 (default), 3072, 4096 or 8192;
  with `key_type=ec`, allowed values are: 224, 256 (default),
  384, or 521; ignored with `key_type=ed25519` or `key_type=ml-dsa` or in signing operations
  when `key_type=any`.
