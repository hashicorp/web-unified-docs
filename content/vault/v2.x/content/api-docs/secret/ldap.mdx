...
- `auto_unlock` `(bool: false)` - <EnterpriseAlert inline="true" /> When `true`,
  ...
  Applies to all root-managed static roles on the mount unless overridden during
  [role creation](#static-roles). Requires the Active Directory schema
  (`schema=ad`). The setting also applies to the initial import rotation when you
  create a role. Refer to the
...
Vault reports `rotated_on_read` as `true` when the read triggered a rotation and
`false` when the credential was still within the cooldown window.
...
## Unlock static role account

@include 'alerts/enterprise-and-hcp.mdx'

The `static-role/unlock` endpoint clears a login-failure lockout on the Active
Directory account associated with a static role. It does not enable or disable
the account. Unlocking an already unlocked account is a no-op success.
...
- Only supports the Active Directory schema (`schema=ad`). Vault returns an error for requests using the `openldap` or `racf` schemas.
- The unlock enpoint only supports root-managed roles. Vault does not support
  unlocking self-managed roles (`self_managed=true` at the mount or role level)
  because it does not use the administrator bind DN to manage those accounts.
  You can unlock self-managed accounts directly in Active Directory.
