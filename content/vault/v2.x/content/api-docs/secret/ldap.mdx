...
- `skip_static_role_import_rotation` `(bool: false)` - The default value to use for `skip_import_rotation` when
  ...
  When `true`, Vault rotates the static role credential on each read from
  `/{mount_path}/static-cred/{role_name}` as long as the read happens outside
  the configured cooldown period. The roate on read configuration applies to all
  static roles on the mount, but you can ooverride the setting for individual roles.
...
- `rotate_on_read` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read` setting for the role. When `true`,
  Vault rotates the credential on each read as long as the read happens outside
  the configured cooldown period.
- `rotate_on_read_cooldown` `(duration: "1m")` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read_cooldown` configuration for the role.
  Set to `0` to rotate on every read.
