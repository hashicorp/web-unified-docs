...
- If `password_policy` is set on a role, passwords will be generated from the associated
  [password policy](/vault/docs/concepts/password-policies). Role-level password policies take precedence over mount-level
  policies. If a role does not have a password policy set, the mount-level password policy (if configured) will be used.
  The password policy must exist at the time of role creation or update, otherwise validation will fail.
