...
The `/unlock` endpoint clears a login-failure lockout on the Active
...
| Method | Path                                    |
| :----- | :----------------------------------------- |
| `PUT`  | <nobr>`/ldap/static-role/:role_name/unlock`</nobr> |
...
- The unlock endpoint only supports root-managed roles. Vault does not support
...
$ curl \
    ...
    --request PUT \
    ${VAULT_ADDR}/v1/ldap/static-role/hashicorp/unlock
```
