...
last_modified: 2026-07-22T14:13:51.000Z
...
- `binddn` `(string: "")` - Distinguished name (DN) of object to bind for managing user entries.
  For example, `cn=vault,ou=Users,dc=hashicorp,dc=com`. You must set `binddn` for root-managed plugin
  configurations.
- `bindpass` `(string: "")` - Password to use along with `binddn` for
  managing user entries. You must leave `bindpass` unset for self-managed plugin
  configurations.
- `self_managed` `(bool: false)` - <EnterpriseAlert inline="true" />
  Configures the plugin mount as self-managed. Self-managed mounts only support
  the self-managed static roles feature. You cannot change `self_managed` after
  creation. 
...
- `userdn` `(string: "")` - The base DN under which to perform user search in
...
- `userattr` `(string: "")` – The attribute field name used to perform user search
...
- `insecure_tls` `(bool: <optional>)` - If true, skips LDAP server SSL certificate verification - insecure,
  use with caution!
- `certificate` `(string: "")` - CA certificate to use when verifying LDAP server certificate, must be
  x509 PEM encoded.
...
- `rotate_on_read` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  When `true`, Vault rotates the static role credential each time it reads the
  credential from `/{mount_path}/static-cred/{role_name}` outside the cooldown
  period. The setting applies to all static roles on the mount, but you can
  override the configuration for individual roles.
- `rotate_on_read_cooldown` `(duration: "1m")` - <EnterpriseAlert product="vault" inline />
  Minimum time allowed between rotate-on-read rotations. Vault returns the
  existing credential For requests that occur within the cooldown window. Set
  `rotate_on_read_cooldown` to `0` to disable the cooldown and rotate on every
  read. You can override the cooldown period for individual role.
- `auto_unlock` `(bool: false)` - <EnterpriseAlert inline="true" /> When `true`,
  Vault automatically unlocks the account after a successful password rotation.
  Applies to all root-managed static roles on the mount unless a role overrides
  the value. Requires the Active Directory schema (`schema=ad`). Individual roles
  can override this value during [role creation](#static-roles). The setting also
  applies to the initial import rotation when you create a role. See the
  [Account unlock section](/vault/docs/secrets/ldap#account-unlock) for more
...
- `username` `(string: <required>)` - The username of the existing LDAP entry to manage
  ...
  when `dn` isn't used for login purposes (such as SSH). Cannot be modified after creation. **Example:** `"bob"`
- `password` `(string: "")` - <EnterpriseAlert product="vault" inline /> 
  The current password of the LDAP entry. You must
  set `password` for self-managed configurations. Vault uses `password` to rotate the password for self-managed roles
  in the LDAP system. For root-managed configurations, Vault returns the value set in `password` as a starting password
  until the credential rotates.
- `dn` `(string: "")` - Distinguished name (DN) of the existing LDAP entry to manage
  password rotation for. If given, it will take precedence over `username` for the LDAP
  search performed during password rotation. You must set `dn` for self-managed
  configurations. Cannot be modified after creation. **Example:** `cn=bob,ou=Users,dc=hashicorp,dc=com`
- `skip_import_rotation` `(boolean: false)` - When set on a role creation request, Vault will not rotate the
  pre-existing password of the associated LDAP entry. Reading credentials for the role returns the 
  value set on the `password` field until the password rotates (either automatically or manually by `rotate-role`).
  The `skip_import_rotation` field overrides `skip_static_role_import_rotation` from the ldap config if both are set.
- `rotation_policy` `(string: "")` – <EnterpriseAlert product="vault" inline />
  The name of the rotation policy to apply when Vault Enterprise is rotating
  the credential. Vault uses the values defined in the rotation
  policy to manage backoff and retries when automatically rotating credentials. If
  you do not set a rotation policy, Vault applies the default rotation policy. Refer to
  [rotation policy documentation](/vault/docs/enterprise/automated-credential-rotation/rotation-policies) for more information.
- `rotate_on_read` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read` setting for the role. Set
  `rotate_on_read` to `true` to enable read-rotation explicitly for the
  associated credential.
- `rotate_on_read_cooldown` `(duration: "1m")` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read_cooldown` for the role. Set
  `rotate_on_read_cooldown` to `0` to rotate the credential on every read.
- `password_policy` `(string: "")` - <EnterpriseAlert product="vault" inline />
  The name of the [password policy](/vault/docs/concepts/password-policies) Vault
  should use to generate passwords for the targeted role. Role-level password
  polices take precedence over any configured mount-level policies. The password
  policy must exist when you create or update the role or the request fails
  validation.
- `auto_unlock` `(bool: <unset>)` - <EnterpriseAlert inline="true" /> Overrides the mount-level `auto_unlock`
  setting for this role. When unset, the role inherits the mount-level value.
  Applies to root-managed roles that use the Active Directory schema
  (`schema=ad`). See the
  [Account unlock section](/vault/docs/secrets/ldap#account-unlock) for the
  resolution precedence and restrictions.

@include 'rotationfields_role.mdx'
...
```json
{
  ...
  "rotation_period": 86400,
  "rotated_on_read": true,
  "ttl": 86072,
  "username": "hashicorp"
}
```

Vault reports `rotated_on_read` as `true` when the read triggered a rotation and`false` when
the credential was still within the cooldown window.
...
## Unlock static role account <EnterpriseAlert inline="true" />

The `static-role/unlock` endpoint clears a login-failure lockout on the Active
Directory account associated with a static role. For Active Directory, unlock
sets the `lockoutTime` attribute to `0`, which matches the **Unlock Account**
operation. Unlock only clears a login-failure lockout and does not enable or
disable the account. Unlocking an account that is already unlocked is a no-op
success.

Before you use the endpoint:

- Configure the bind DN with permission to write the
  [`lockoutTime`](https://learn.microsoft.com/en-us/windows/win32/adschema/a-lockouttime)
  attribute on the target user object.
- Grant trusted identities explicit `update` permission for
  `ldap/static-role/unlock/:role_name` in a
  [Vault policy](/vault/docs/concepts/policies). Vault ACL policies control this
  endpoint in the same way they control rotation endpoints.

| Method | Path                                    |
| :----- | :----------------------------------------- |
| `PUT`  | <nobr>`/ldap/static-role/unlock/:role_name`</nobr> |

### Limitations

- Supports only the Active Directory schema (`schema=ad`). The `openldap` and
  `racf` schemas return an error.
- Supports only root-managed roles. You cannot use this endpoint for
  self-managed roles (`self_managed=true` at the mount or role level) because
  Vault does not use the administrator bind DN to manage those accounts. Unlock
  self-managed accounts directly in Active Directory.
- Repeated requests to unlock the same Active Directory account can return
  `LDAP Result Code 51 "Busy"` when Active Directory is processing another
  write to the account. Retry the request after the other write completes.

### Parameters

- `role_name` `(string: <required>)` – URL parameter specifying the name of the
  static role. The request body is empty.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request PUT \
    http://127.0.0.1:8200/v1/ldap/static-role/unlock/hashicorp
```

## Static role migration

@include 'alerts/enterprise-and-hcp.mdx'

The `static-migration` endpoint provides visibility into the migration process
for operational awareness and troubleshooting purposes if the migration requires
manual intervention.

| Method   | Path                          |
| :------- | :---------------------------- |
| `GET`    | `/ldap/static-migration`      |
| `POST`   | `/ldap/static-migration`      |

### Parameters

- `include_role_names` `(boolean: <required>)` – Includes lists of migrated role names and unmigrated role names when reading
  the migration status. Vault does not set a default value for `include_role_names`. You must set `include_role_names` explicitly.

The `GET` endpoint returns statistics about in-progress migration attempts. If
Vault does not have an active migration, the endpoint returns nothing. To include
completed and pending migrations in the result, set the `include_role_names`
parameter. Be aware that including all migrations can result in a very large
response if you have many static roles.

### Sample GET payload

```json
{
  "include_role_names": true
}
```
...
$ curl \
    ...
    --request GET \
    --data @payload.json \
    ${VAULT_ADDR}/v1/ldap/static-migration
```
...
```json
{
  "data": {
    ...
    "total_role_count": 5,
    "total_successfully_migrated": 3,
    "total_remaining": 2
  }
}
```

You can manually trigger a migration process with the `POST` endpoint if you have
unmigrated static roles. The endpoint returns a warning if Vault is already in
the process of migrating roles, or the migration already completed.
...
For successfully started migration:
...
```json
{
  "data": null,
  "warnings": [
    "static role migration is already complete, all roles have been migrated"
  ]
}
```

## Stop static role migration

@include 'alerts/enterprise-and-hcp.mdx'

| Method   | Path                          |
| :------- | :---------------------------- |
| `POST`   | `/ldap/static-migration/stop` |

The `/stop` endpoint interrupts a currently active migration process. The
endpoint does not return data on success.
...
$ curl \
    --header "X-Vault-Token: ${VAULT_TOKEN}" \
    --request POST \
    ${VAULT_ADDR}/v1/ldap/static-migration/stop
```
