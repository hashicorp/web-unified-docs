...
- `rotate_on_read` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  When `true`, Vault rotates the static role credential each time it is read from
  `/{mount_path}/static-cred/{role_name}`, subject to the cooldown period. Applies
  to all static roles on the mount. Can be overridden per role.
- `rotate_on_read_cooldown` `(duration: "1m")` - <EnterpriseAlert product="vault" inline />
  Minimum time between rotate-on-read rotations. Vault returns the existing
  credential for reads within the cooldown window. Set to `0` to disable the
  cooldown and rotate on every read. Can be overridden per role.
...
- `rotate_on_read` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read` setting for this role. When `true`,
  Vault rotates the credential each time it is read, subject to the cooldown period.
- `rotate_on_read_cooldown` `(duration: "1m")` - <EnterpriseAlert product="vault" inline />
  Overrides the mount-level `rotate_on_read_cooldown` for this role. Minimum time
  between rotate-on-read rotations. Set to `0` to rotate on every read.
...
`rotated_on_read` is `true` when the read triggered a rotation, or `false` when
