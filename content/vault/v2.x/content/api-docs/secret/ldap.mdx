...
- `password_policy` `(string: "")` - <EnterpriseAlert product="vault" inline />
  The name of the [password policy](/vault/docs/concepts/password-policies) to use to
  generate passwords for this role. A role-level `password_policy` takes precedence over the mount-level
  `password_policy` (if configured). The password policy must exist when you create or update the role; otherwise the
