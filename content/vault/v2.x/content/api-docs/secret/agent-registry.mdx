...
last_modified: 2026-09-15T11:54:05.000Z
...
- `entity_id` `(string: <required>)` – Entity ID representing the agent. The
  ...
  Agent Registry record per namespace and locality.
...
- `local` `(bool: false)` - When set to `true`, Vault stores the the agent
  registration in local storage for the cluster and does not replicate registration
  details to performance secondaries. You cannot update the `local` field after
  agent registration. 
