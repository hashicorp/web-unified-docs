...
| Method | Path                                                     |
|:-------|:---------------------------------------------------------|
| `GET`  | `/:secret-mount-path/data/:path?version=:version` |
...
- `version` `(int: 0)` - Specifies the version to return. If not set the latest
  version is returned.
- `read_snapshot_id` `(string: "")` - <EnterpriseAlert inline="true" /> Specifies a
  loaded snapshot ID. When set, Vault reads the secret data from the snapshot
  instead of live cluster storage.
...
## Recover secret from snapshot <EnterpriseAlert inline="true" />

Reads the secret at the specified path from a loaded snapshot and writes it to
live cluster storage as a new version. The request body is always ignored —
data is sourced exclusively from the snapshot. If `cas_required` is set to
`true` on the key or engine configuration, the recover operation will fail.

| Method    | Path                             |
|:----------|:---------------------------------|
| `RECOVER` | `/:secret-mount-path/data/:path` |

### Parameters

- `secret-mount-path` `(string: <required>)` - The path to the KV mount, such
  as `secret`. This is specified as part of the URL.
- `path` `(string: <required>)` – The destination path for the recovered secret.
  This is specified as part of the URL.

### Headers

- `X-Vault-Recover-Snapshot-Id` `(string: <required>)` – The ID of a snapshot
  previously loaded into Vault that contains the secret at the provided path.
- `X-Vault-Recover-Source-Path` `(string: "")` – Optional header specifying the
  original `data/` path of the secret in the snapshot. Must be on the same mount
...
To recover to a different path within the same mount:

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --header "X-Vault-Recover-Snapshot-Id: 9465df92-8236-4af9-8cc8-b7460d882e41" \
    --header "X-Vault-Recover-Source-Path: secret/data/my-old-secret" \
    --request RECOVER \
    https://127.0.0.1:8200/v1/secret/data/my-new-secret
```
...
After a successful recovery, reading the secret at the same path returns the
restored data as the latest version:
...
- `exclude_deleted` `(bool: false)` – Only return non-deleted entries.
...
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://${VAULT_ADDR}/v1/secret/metadata/my-secret?exclude_deleted=false
```
