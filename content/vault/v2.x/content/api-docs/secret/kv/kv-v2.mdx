...
live cluster storage as a new version. Vault only sources secret data from the
loaded snapshot. The request fails if `cas_required` is set to
`true` on the key or engine configuration.
| Method    | Path                             |
|:----------|:---------------------------------|
...
$ curl \
    ...
    --request RECOVER \
    ${VAULT_ADDR}/v1/secret/data/my-new-secret
```
...
After a successful recovery, you can read the secret as the latest version for the restored path:
