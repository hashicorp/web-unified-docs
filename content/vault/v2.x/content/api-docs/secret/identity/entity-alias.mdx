...
- `issuer` `(string: "")` - Issuer URL of the external identity provider
  bound to the alias. You cannot change `issuer` after creation. Setting
  `mount_accessor` is optional when you set `issuer`.

<<<<<<< Updated upstream
- `external_id` `(string: <optional>)` - Subject identifier for the alias from the
  external identity provider. Must be unique with `issuer` within a namespace.
  Cannot be changed after creation.

<Note>

For aliases bound to an OAuth resource-server configuration
(Enterprise), when only the `issuer` field is provided during creation, the `mount_accessor` will be an automatically generated synthetic value with the format
`oauth-resource-server_<namespace_id>_<config_id>`, where `<config_id>` is the
UUID of the matching configuration.

</Note>
=======
- `external_id` `(string: "")` - Subject identifier for the alias from the
  external identity provider. The ID must be unique with `issuer` within a
  namespace. You cannot change `external_id` after creation.
>>>>>>> Stashed changes
...
```json
{
  ...
  "external_id": "svc-agent@example.com"
}
```
...
- `issuer` `(string: "")` - Issuer URL bound to the alias. The provided issuer
  must match the existing `issuer` value for the alias.

- `external_id` `(string: "")` - Subject identifier bound to the alias. The
  provided ID must match the existing `external_id` value for the alias.
