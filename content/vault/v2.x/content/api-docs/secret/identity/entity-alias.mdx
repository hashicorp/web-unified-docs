...
- `mount_accessor` `(string: <required>)` - Accessor of the mount to which the
  alias should belong to. Can be omitted when `issuer` is set (see the note below).
...
- `issuer` `(string: <optional>)` - Issuer URL of the external JWT/OIDC identity
  provider bound to this alias. When set without a `mount_accessor`, Vault
  generates the synthetic `mount_accessor` from the matching OAuth
  resource-server configuration. Cannot be changed after creation.

- `external_id` `(string: <optional>)` - Subject identifier for the alias from
  the external identity provider. The `issuer` and `external_id` pair must be
  unique within a namespace. Cannot be changed after creation.

~> **NOTE:** For aliases bound to an OAuth resource-server configuration
(Enterprise), `mount_accessor` is a synthetic value with the format
`oauth-resource-server_<namespace_id>_<config_id>`, where `<config_id>` is the
UUID of the configuration. Provide `issuer` instead and Vault generates it for you.
...
### Sample payload (OAuth resource-server alias)

```json
{
  "name": "svc-agent",
  "canonical_id": "404e57bc-a0b1-a80f-0a73-b6e92e8a52d3",
  "issuer": "https://dev-84756123.okta.com/oauth2/default",
  "external_id": "00ub0oNGTSWTBKOLGLNR"
}
```
...
```json
{
  "data": {
    ...
    "metadata": null,
    "mount_accessor": "oauth-resource-server_root_6ced393c-4d49-72f2-5af6-320fe46309b4",
    "name": "svc-agent",
    "namespace_id": "root"
  }
}
...
- `issuer` `(string: "")` - Issuer URL bound to the alias. If provided, must
  match the alias's existing `issuer`.

- `external_id` `(string: "")` - Subject identifier bound to the alias. If
  provided, must match the alias's existing `external_id`.
