...
If you provision entities through the
[SCIM API](/vault/api-docs/secret/identity/scim), use the SCIM endpoints to
manage those resources. Vault restricts changes to SCIM-managed fields on the
standard identity entity endpoints.
...
- `external_id` `(string: <optional>)` – External identifier for the entity from
  an external identity provider, such as one provisioned through SCIM. If the
  provided ID matches an existing entity, Vault updates the matching entity
...
We recommend sending merge requests only to the leader node of your primary
cluster.
...
Calling this endpoint requires a token with `sudo` capability on the `/identity/entity/merge` path.

</Note>

<Note>

Vault enforces SCIM ownership rules on entity merges. Requests that would
modify SCIM-managed entities outside the owning SCIM client fail.
