...
- `external_id` `(string: <optional>)` – External identifier for the entity from
  an external identity provider, for example an entity provisioned through SCIM.
  If the `external_id` matches an existing entity, Vault updates that entity
  instead of the one named in the request.
