...
Callers must have `update` permissions on the `/identity/entity/merge` path **and** `update` permissions on each of the entity paths (i.e., `/identity/entity/id/:id`) for all entities involved in the merge — both the source entities in `from_entity_ids` and the destination entity in `to_entity_id`.
