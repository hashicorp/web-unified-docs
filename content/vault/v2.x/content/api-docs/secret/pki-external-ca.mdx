...
- `within` `(string: "1h")` - Duration to look back for recent orders (e.g., `180m`, `1h`, `24h`). Defaults to 1 hour, which is also the minimum. The maximum duration is 1 week (168h).
...
The response is limited to a maximum of 500 results for performance reasons. The queried duration must be between 1 hour and 1 week (168 hours). Orders are sorted by creation time rather than last update time.
