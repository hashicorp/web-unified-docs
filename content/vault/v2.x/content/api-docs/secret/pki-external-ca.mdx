...
- `default_nameserver` `(string: "")` - Address of a DNS nameserver (`host` or
  `host:port`) to append to the resolver list when verifying DNS-01 challenge
  propagation for DNS providers that do not specify a nameserver. Leaving `default_nameserver` empty tells Vault not to append additional resolvers beyond the authoritative primary NS. 
...
- `trusted_ca` `(string: "")` - PEM-encoded trusted CA certificates for the
  ACME server.

- `default_nameserver` `(string: "")` - Address of a DNS nameserver (`host` or
  `host:port`) to use when verifying DNS-01 challenge propagation for DNS
  providers that do not specify a nameserver. Set to an empty string to clear
  a previously configured nameserver.
...
- `nameserver` `(string: "")` - DNS server address (`host` or `host:port`) to
  use when verifying DNS-01 challenge propagation for the configured provider. When empty,
  Vault falls back to the `default_nameserver` value from the ACME account (if set) and the authoritative primary NS.
...
- `nameserver` `(string: "")` - DNS server address (`host` or `host:port`) to
  use when verifying DNS-01 challenge propagation for this provider. When empty,
  Vault falls back to the `default_nameserver` value from the ACME account (if set) and the authoritative primary NS.
...
- `nameserver` `(string: <required>)` - DNS server address in `host` or `host:port` format (e.g., `192.168.1.1:53` or `192.168.1.1`). RFC2136 defaults to port 53 when you do not specify a port.
...
The ACME server controls `challenge_status`, which does not map directly to `order_status`.
When `order_status` is `error`, check `last_error` for the actual failure reason.
[Check order status](/vault/docs/secrets/pki-external-ca#check-order-status)
details how `order_status` and `challenge_status` relate.

- `order_status` `(string)` - Where the order sits in the internal workflow in Vault. 

- `challenge_status` `(string)` - The state the upstream ACME server last reported
  for the challenge. Possible values per [RFC 8555](https://datatracker.ietf.org/doc/html/rfc8555#section-7.1.6):

  - `pending` - Initial state. Vault has not notified the ACME server to verify
    the challenge, and the status will not update until Vault does so.
  - `processing` - The ACME server received the notification and is actively verifying.
  - `valid` - The ACME server successfully verified the challenge.
  - `invalid` - The ACME server was unable to verify the challenge.

- `last_error` `(string)` - An empty string or the reason the order failed when
  `order_status` is `error`.
...
List orders created within a specified time window, sorted by creation time (most recent first). 
Vault limits the response to a maximum 500 results for performance reasons. 
The queried duration must be between 1 hour and 1 week (168 hours).
