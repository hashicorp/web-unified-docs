...
- `nameserver` `(string: "")` - DNS server address (`host` or `host:port`) to
  use when verifying DNS-01 challenge propagation for this provider. When empty,
  Vault falls back to the ACME account's `default_nameserver` (if set) and the
  authoritative primary NS.
...
- `nameserver` `(string: <required>)` - DNS server address in `host` or `host:port` format (e.g., `192.168.1.1:53` or `192.168.1.1`). RFC2136 defaults to port 53 when no port is specified.
