...
- `order_status` `(string)` - Vault's internal order state. See [Check order status](/vault/docs/secrets/pki-external-ca#check-order-status)
  for a description of each value.

- `challenge_status` `(string)` - The challenge state the upstream ACME server last
  reported for this challenge (`pending`, `valid`, or `invalid`). The ACME server
  controls this value independently of `order_status`. A `pending` value does not
  mean the ACME server is still working. (Vault may never have asked the ACME server
  to verify the challenge.) When `order_status` is `error`, check `last_error` for
  the actual failure reason.

- `last_error` `(string)` - The reason the order failed when `order_status` is
  `error`. Empty string otherwise.
...
- `order_status` `(string)` - Vault's internal order state. See [Check order status](/content/vault/v2.x/content/docs/secrets/pki-external-ca/index.mdx)
  for a description of each value.
