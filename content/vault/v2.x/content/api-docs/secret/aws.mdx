...
Both `/aws/creds` and `/aws/sts` accept `GET` and `POST`. Use `GET` for reads
with no request body. Use `POST` when passing parameters in a JSON body (e.g.,
`role_arn`, `ttl`, `role_session_name`).
...
`/aws/creds` and `/aws/sts` are functionally identical for modern roles. For
legacy roles created with the `arn` or `policy` parameters, Vault restricts
credential retrieval to specific types:

- `/aws/sts` — limited to `assumed_role` or `federation_token` credential types.
- `/aws/creds` — limited to the `iam_user` credential type.

We offer `/aws/sts` as a legacy alias for backwards compatibility.
