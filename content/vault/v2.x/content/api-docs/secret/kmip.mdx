...
@include '../../../global/partials/alerts/status/experimental.mdx'
...
$ curl \
    --header "X-Vault-Token: ${VAULT_TOKEN}" \
    --request GET \
    ${VAULT_ADDR}/v1/kmip/template/create
```
...
- `scope` (`string: <required>`) - Name of scope.
- `role` (`string: <required>`) - Name of role.


#### Request parameters

- `operations` (`string: ""`) - Slash-separated list of KMIP operation names
  (e.g. `locate/encrypt`). You must specify the operation of every batch item
  in the request.
- `template` (`string: ""`) - Either the name of a built-in template (e.g.
  ...
  If you omit `template`, Vault selects a built-in template based on the `operations` URL
  segment.
- `params` (`map<string,string>`) - Arbitrary key/value replacement parameters
  for `PARAMETER_<name>` placeholders in the XML template.

Vault also accepts some named parameters as top-level fields.
Providing the following parameters has the same result as setting the
same key in `params`:
...
#### Sample request
 ...
 The following example creates a symmetric key using a built-in template.
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    ${VAULT_ADDR}/v1/kmip/scope/myscope/role/myrole/client/create
```
...
The following example locates and encrypts a KMIP key using a built-in template.
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    ${VAULT_ADDR}/v1/kmip/scope/myscope/role/myrole/client/locate/encrypt
```
...
The following example uses a custom XML template.
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    ${VAULT_ADDR}/v1/kmip/scope/myscope/role/myrole/client/get
```
