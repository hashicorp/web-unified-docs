---
layout: api
page_title: KMIP - Secrets Engines - HTTP API
description: This is the API documentation for the Vault KMIP secrets engine.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-04-15T00:13:54.000Z
last_modified: 2026-04-15T00:13:54.000Z
# END AUTO GENERATED METADATA
---

# KMIP secrets engine (API)

@include 'x509-sha1-deprecation.mdx'

This is the API documentation for the Vault KMIP secrets engine. For general
information about the usage and operation of
the KMIP secrets engine, please see [these docs](/vault/docs/secrets/kmip).

This documentation assumes the KMIP secrets engine is enabled at the `/kmip` path
in Vault. Since it is possible to mount secrets engines at any path, please
update your API calls accordingly.

## Configuration management

### Write config

| Method | Path           |
| :----- | :------------- |
| `POST` | `/kmip/config` |

This endpoint configures shared information for the secrets engine. After writing
to it the KMIP engine will generate a CA and start listening for KMIP requests.
If the server was already running and any non-client settings are changed, the
server will be restarted using the new settings. All generated CAs will use
entropy augmentation to generate their certificates if entropy augmentation
is enabled.

#### Parameters

- `listen_addrs` (`list: ["127.0.0.1:5696"] || string`) - Address and port the
  KMIP server should listen on. Can be given as a JSON list or a
  comma-separated string list. If multiple values are given, all will be
  listened on.

- `server_hostnames` (`list: ["localhost"] || string`) - Hostnames to include in
  the server's TLS certificate as SAN DNS names. The first will be used as the
  common name (CN).

- `server_ips` (`list: [] || string`) - IPs to include in the server's TLS
  certificate as SAN IP addresses. Localhost (IPv4 and IPv6) will be automatically
  included.
- `tls_ca_key_type` (`string: "ec"`) - CA key type, `rsa` or `ec`.

- `tls_ca_key_bits` (`int: 521`) - CA key bits, valid values depend on key type.

- `tls_min_version` (`string: "tls12"`) - Minimum TLS version to accept.

- `default_tls_client_key_type` (`string: "ec"`): - Client certificate key type,
  `rsa` or `ec`.

- `default_tls_client_key_bits` (`int: 521`): - Client certificate key bits, valid
  values depend on key type.

- `default_tls_client_ttl` (`int: 86400 || string:"24h"`) – Client certificate
  TTL in either an integer number of seconds (10) or an integer time unit (10s).

#### Sample payload

```json
{
  "listen_addrs": "127.0.0.1:5696,192.168.1.2:9000",
  "server_hostnames": "myhostname1,myhostname2",
  "server_ips": "192.168.1.2",
  "tls_ca_key_type": "ec",
  "tls_ca_key_bits": 521,
  "tls_min_version": "tls11",
  "default_tls_client_key_type": "ec",
  "default_tls_client_key_bits": 224,
  "default_tls_client_ttl": 86400
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/config
```

### Read config

| Method | Path           |
| :----- | :------------- |
| `GET`  | `/kmip/config` |

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/config
```

#### Sample response

```json
{
  "data": {
    "listen_addrs": ["127.0.0.1:5696", "192.168.1.2:9000"],
    "server_hostnames": ["myhostname1", "myhostname2"],
    "server_ips": ["192.168.1.2"],
    "tls_ca_key_type": "ec",
    "tls_ca_key_bits": 521,
    "tls_min_version": "tls11",
    "default_tls_client_key_type": "ec",
    "default_tls_client_key_bits": 224,
    "default_tls_client_ttl": 86400
  }
}
```

### Read CA

| Method | Path       |
| :----- | :--------- |
| `GET`  | `/kmip/ca` |

Returns the CA certificates in PEM format. Returns an error if config has never
been written.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/ca
```

#### Sample response

```json
{
  "data": {
    "ca_pem": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
  }
}
```

## Certificate Authority management

### Generate CA

| Method | Path                           |
| :----- | :----------------------------- |
| `POST` | `/kmip/ca/:name/generate` |

Generate a new Certificate Authority with the specified name. This CA can be used
to generate server certificates for listeners or to sign client certificates.

#### Parameters

- `name` (`string: <required>`) - Name of the CA. This is part of the request URL.
- `key_type` (`string: <required>`) - CA key type, `rsa` or `ec`.
- `key_bits` (`int: <required>`) - CA key bits. Valid values depend on key type.
  For RSA: 2048, 3072, 4096. For EC: 224, 256, 384, 521.
- `ttl` (`int or string: "365d"`) - CA TTL in either an integer number of seconds
  (31536000) or an integer time unit (8760h).

#### Sample payload

```json
{
  "key_type": "rsa",
  "key_bits": 2048,
  "ttl": "8760h"
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/ca/my-ca/generate
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----",
    "serial_number": "123456789"
  }
}
```

### Import CA

| Method | Path                       |
| :----- | :------------------------- |
| `POST` | `/kmip/ca/:name/import` |

Import an external Certificate Authority. Imported CAs can only be used to
verify client certificates, not to generate client or server certificates. You
must specify how to extract scope and role information from client certificates.

#### Parameters

- `name` (`string: <required>`) - Name of the CA. This is part of the request URL.
- `ca_pem` (`string: <required>`) - CA certificate in PEM format.
- `scope_name` (`string`) - Fixed scope name to associate with this CA. Mutually
  exclusive with `scope_field`.
- `scope_field` (`string`) - Certificate field to use for scope (CN, O, OU, or UID).
  Mutually exclusive with `scope_name`.
- `role_name` (`string`) - Fixed role name to associate with this CA. Mutually
  exclusive with `role_field`.
- `role_field` (`string`) - Certificate field to use for role (CN, O, OU, or UID).
  Mutually exclusive with `role_name`.

~> **Note**: You must specify either `scope_name` or `scope_field`, and either
`role_name` or `role_field`.

#### Sample payload (with fixed scope and role)

```json
{
  "ca_pem": "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----",
  "scope_name": "my-service",
  "role_name": "admin"
}
```

#### Sample payload (with certificate fields)

```json
{
  "ca_pem": "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----",
  "scope_field": "OU",
  "role_field": "CN"
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/ca/my-imported-ca/import
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----",
    "imported": true,
    "scope_field": "OU",
    "role_field": "CN"
  }
}
```

### Read CA

| Method | Path            |
| :----- | :-------------- |
| `GET`  | `/kmip/ca/:name` |

Read information about a specific CA.

#### Parameters

- `name` (`string: <required>`) - Name of the CA. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/ca/my-ca
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\n...\n-----END CERTIFICATE-----",
    "key_type": "rsa",
    "key_bits": 2048,
    "imported": false,
    "serial_number": "123456789"
  }
}
```

### Update CA

| Method | Path            |
| :----- | :-------------- |
| `POST` | `/kmip/ca/:name` |

Update configuration for an imported CA. Only scope and role settings can be updated.
Generated CAs cannot be updated.

#### Parameters

- `name` (`string: <required>`) - Name of the CA. This is part of the request URL.
- `scope_name` (`string`) - Fixed scope name to associate with this CA.
- `scope_field` (`string`) - Certificate field to use for scope (CN, O, OU, or UID).
- `role_name` (`string`) - Fixed role name to associate with this CA.
- `role_field` (`string`) - Certificate field to use for role (CN, O, OU, or UID).

#### Sample payload

```json
{
  "scope_field": "O",
  "role_field": "UID"
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/ca/my-imported-ca
```

### List CAs

| Method | Path        |
| :----- | :---------- |
| `LIST` | `/kmip/ca` |

List all configured CAs.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/ca
```

#### Sample response

```json
{
  "data": {
    "keys": ["my-ca", "my-imported-ca"]
  }
}
```

### Delete CA

| Method   | Path            |
| :------- | :-------------- |
| `DELETE` | `/kmip/ca/:name` |

Delete a CA by name. The CA cannot be deleted if it is currently in use by any
listener or role.

#### Parameters

- `name` (`string: <required>`) - Name of the CA. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request DELETE \
    https://127.0.0.1:8200/v1/kmip/ca/my-ca
```

## Listener management

### Write listener

| Method | Path                      |
| :----- | :------------------------ |
| `POST` | `/kmip/listener/:name` |

Create or update a named KMIP listener. Each listener is associated with a CA for
generating server certificates and can trust additional CAs for client verification.

#### Parameters

- `name` (`string: <required>`) - Listener name. This is part of the request URL.
- `ca` (`string: <required>`) - Name of the CA to use for generating the server
  certificate. Must be a generated CA, not an imported one.
- `address` (`string: <required>`) - Host:port address to listen on.
- `server_hostnames` (`list: [] || string`) - DNS SANs to include in the server
  certificate.
- `server_ips` (`list: ["127.0.0.1", "::1"] || string`) - IP SANs to include in
  the server certificate. Localhost addresses are included by default.
- `additional_client_cas` (`list: [] || string`) - Names of additional CAs to trust
  for client certificate verification.
- `also_use_legacy_ca` (`bool: false`) - Whether to also trust the legacy CA for
  client certificate verification.
- `tls_min_version` (`string: "tls12"`) - Minimum TLS version. One of `tls12` or `tls13`.
- `tls_max_version` (`string`) - Maximum TLS version. One of `tls12` or `tls13`.
- `tls_cipher_suites` (`string`) - Comma-separated list of TLS cipher suites to allow.
  Only applies to TLS 1.2.

#### Sample payload

```json
{
  "ca": "my-ca",
  "address": "0.0.0.0:5697",
  "server_hostnames": ["kmip.example.com", "kmip2.example.com"],
  "server_ips": ["192.168.1.100"],
  "additional_client_cas": ["my-imported-ca"],
  "tls_min_version": "tls12"
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/listener/my-listener
```

#### Sample response

```json
{
  "data": {
    "ca": "my-ca",
    "address": "0.0.0.0:5697",
    "server_hostnames": ["kmip.example.com", "kmip2.example.com"],
    "server_ips": ["192.168.1.100"],
    "additional_client_cas": ["my-imported-ca"],
    "also_use_legacy_ca": false,
    "tls_min_version": "tls12",
    "tls_max_version": "",
    "tls_cipher_suites": ""
  }
}
```

### Read listener

| Method | Path                      |
| :----- | :------------------------ |
| `GET`  | `/kmip/listener/:name` |

Read configuration for a specific listener.

#### Parameters

- `name` (`string: <required>`) - Listener name. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/listener/my-listener
```

#### Sample response

```json
{
  "data": {
    "ca": "my-ca",
    "address": "0.0.0.0:5697",
    "server_hostnames": ["kmip.example.com"],
    "server_ips": ["192.168.1.100"],
    "additional_client_cas": ["my-imported-ca"],
    "also_use_legacy_ca": false,
    "tls_min_version": "tls12",
    "tls_max_version": "",
    "tls_cipher_suites": ""
  }
}
```

### List listeners

| Method | Path              |
| :----- | :---------------- |
| `LIST` | `/kmip/listener` |

List all configured listeners.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/listener
```

#### Sample response

```json
{
  "data": {
    "keys": ["my-listener", "another-listener"]
  }
}
```

### Delete listener

| Method   | Path                      |
| :------- | :------------------------ |
| `DELETE` | `/kmip/listener/:name` |

Delete a listener by name. The listener will be stopped and removed from configuration.

#### Parameters

- `name` (`string: <required>`) - Listener name. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request DELETE \
    https://127.0.0.1:8200/v1/kmip/listener/my-listener
```

## Scope management

### Write scope

| Method | Path                 |
| :----- | :------------------- |
| `POST` | `/kmip/scope/:scope` |

Creates a new scope with the given name.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    https://127.0.0.1:8200/v1/kmip/scope/myscope
```

### List scopes

| Method | Path          |
| :----- | :------------ |
| `LIST` | `/kmip/scope` |

List existing scopes.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/scope
```

#### Sample response

```json
{
  "data": {
    "keys": ["myscope"]
  }
}
```

### Delete scope

| Method   | Path                 |
| :------- | :------------------- |
| `DELETE` | `/kmip/scope/:scope` |

Delete a scope by name.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `force` (`bool: false`) - Force scope deletion. If KMIP managed objects have
  been created within the scope this param must be provided or the deletion will
  fail. This value should be supplied as a query parameter, or as an argument in
  the CLI.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request DELETE \
    https://127.0.0.1:8200/v1/kmip/scope/myscope?force=false
```

## Scope management

### Write role

| Method | Path                            |
| :----- | :------------------------------ |
| `POST` | `/kmip/scope/:scope/role/:role` |

Creates or updates a role.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `ca` (`string`) - Name of the CA to use for signing client certificates generated
  for this role. If not specified, the legacy CA will be used.
- `tls_client_key_type` (`string`): - Client certificate key type,
  `rsa` or `ec`. Overrides engine-wide default managed in `config` endpoint.
- `tls_client_key_bits` (`int`): - Client certificate key bits, valid
  values depend on key type. Overrides engine-wide default managed in `config`
  endpoint.
- `tls_client_ttl` (`int or string`) – Client certificate
  TTL in either an integer number of seconds (10) or an integer time unit (10s).
  Overrides engine-wide default managed in `config` endpoint.
- `operation_none` (`bool: false`) - Remove all permissions
  from this role. May not be specified with any other
  `operation_` params.
- `operation_all` (`bool: false`) - Grant all permissions
  to this role. May not be specified with any other
  `operation_` params.
- `operation_activate` (`bool: false`) - Grant permission to use the KMIP
  `Activate` operation.
- `operation_add_attribute` (`bool: false`) - Grant permission to use the KMIP
  `Add Attribute` operation.
- `operation_create` (`bool: false`) - Grant permission to use the KMIP
  `Create` operation.
- `operation_decrypt` (`bool: false`) - Grant permission to use the KMIP
  `Decrypt` operation.
- `operation_destroy` (`bool: false`) - Grant permission to use the KMIP
  `Destroy` operation.
- `operation_discover_versions` (`bool: false`) - Grant permission to use the KMIP
  `Discover Versions` operation.
- `operation_encrypt` (`bool: false`) - Grant permission to use the KMIP
  `Encrypt` operation.
- `operation_get` (`bool: false`) - Grant permission to use the KMIP
  `Get` operation.
- `operation_get_attribute_list` (`bool: false`) - Grant permission to use the KMIP
  `Get Attribute List` operation.
- `operation_get_attributes` (`bool: false`) - Grant permission to use the KMIP
  `Get Attributes` operation.
- `operation_import` (`bool: false`) - Grant permission to use the KMIP
  `Import` operation.
- `operation_locate` (`bool: false`) - Grant permission to use the KMIP
  `Locate` operation.
- `operation_query` (`bool: false`) - Grant permission to use the KMIP
  `Query` operation.
- `operation_register` (`bool: false`) - Grant permission to use the KMIP
  `Register` operation.
- `operation_rekey` (`bool: false`) - Grant permission to use the KMIP
  `Rekey` operation.
- `operation_revoke` (`bool: false`) - Grant permission to use the KMIP
  `Revoke` operation.

#### Sample payload

```json
{
  "operation_activate": true,
  "operation_add_attribute": true,
  "operation_create": true,
  "operation_decrypt": true,
  "operation_destroy": true,
  "operation_discover_versions": true,
  "operation_encrypt": true,
  "operation_get": true,
  "operation_get_attribute_list": true,
  "operation_get_attributes": true,
  "operation_import": true,
  "operation_locate": true,
  "operation_query": true,
  "operation_register": true,
  "operation_rekey": true,
  "operation_revoke": true
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole
```

### Read role

| Method | Path                            |
| :----- | :------------------------------ |
| `GET`  | `/kmip/scope/:scope/role/:role` |

Read a role.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole
```

#### Sample response

```json
{
  "data": {
    "operation_activate": true,
    "operation_add_attribute": true,
    "operation_create": true,
    "operation_decrypt": true,
    "operation_destroy": true,
    "operation_discover_versions": true,
    "operation_encrypt": true,
    "operation_get": true,
    "operation_get_attribute_list": true,
    "operation_get_attributes": true,
    "operation_import": true,
    "operation_locate": true,
    "operation_query": true,
    "operation_register": true,
    "operation_rekey": true,
    "operation_revoke": true
  }
}
```

### List roles

| Method | Path                      |
| :----- | :------------------------ |
| `LIST` | `/kmip/scope/:scope/role` |

List roles with a scope.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role
```

#### Sample response

```json
{
  "data": {
    "keys": ["myrole"]
  }
}
```

### Delete role

| Method   | Path                            |
| :------- | :------------------------------ |
| `DELETE` | `/kmip/scope/:scope/role/:role` |

Delete a role by name.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request DELETE \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole
```

## Client credentials

### Generate credential

| Method | Path                                                |
| :----- | :-------------------------------------------------- |
| `POST` | `/kmip/scope/:scope/role/:role/credential/generate` |

Create a new client certificate tied to the given role and scope.
This endpoint uses entropy augmentation to generate the client certificate
if entropy augmentation is enabled.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `format` (`string: "pem_bundle"`) - Format to return the certificate, private key,
  and CA chain in. One of `pem`, `pem_bundle`, or `der`.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/credential/generate
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
    "private_key": "-----BEGIN EC PRIVATE KEY-----\\n-----END EC PRIVATE KEY-----",
    "serial_number": "728181095563584845125173905844944137943705466376"
  }
}
```

### Sign CSR

| Method | Path                                            |
| :----- | :---------------------------------------------- |
| `POST` | `/kmip/scope/:scope/role/:role/credential/sign` |

Create a new client certificate tied to the given role and scope,
based on a Certificate Signing Request (CSR) provided as input.
The key type and key bits used in the CSR must match those of the role.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `format` (`string: "pem_bundle"`) - Format to return the certificate, private key,
  and CA chain in. One of `pem`, `pem_bundle`, or `der`.
- `csr` (`string`) - CSR in PEM format.

#### Sample request

```
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data '{"csr": "-----BEGIN CERTIFICATE REQUEST-----\\nb/t+UQOjhlb0X2YdjPGHjFd+spZQ6u0a\n-----END CERTIFICATE REQUEST-----"}'
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/credential/sign
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
    "serial_number": "728181095563584845125173905844944137943705466376"
  }
}
```

### Lookup credential

| Method | Path                                              |
| :----- | :------------------------------------------------ |
| `GET`  | `/kmip/scope/:scope/role/:role/credential/lookup` |

Read a certificate by serial number. The private key cannot be obtained except
at generation time.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `serial_number` (`string: <required>`) - Serial number of certificate to revoke.
- `format` (`string: "pem_bundle"`) - Format to return the certificate, private key,
  and CA chain in. One of `pem`, `pem_bundle`, or `der`.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/credential/lookup?serial_number=728181095563584845125173905844944137943705466376
```

#### Sample response

```json
{
  "data": {
    "ca_chain": [
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
      "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----"
    ],
    "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
    "serial_number": "728181095563584845125173905844944137943705466376"
  }
}
```

### List credential serial numbers

| Method | Path                                       |
| :----- | :----------------------------------------- |
| `LIST` | `/kmip/scope/:scope/role/:role/credential` |

List the serial numbers of all certificates within a role.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/credential
```

#### Sample response

```json
{
  "data": {
    "keys": ["728181095563584845125173905844944137943705466376"]
  }
}
```

### Revoke credential

| Method | Path                                              |
| :----- | :------------------------------------------------ |
| `POST` | `/kmip/scope/:scope/role/:role/credential/revoke` |

Delete a certificate, thereby revoking it.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `serial_number` (`string: ""`) - Serial number of certificate to revoke.
  Exactly one of `serial_number` or `certificate` must be provided.
- `certificate` (`string: """`) - Certificate to revoke, in PEM format.
  Exactly one of `serial_number` or `certificate` must be provided.

#### Sample payload

```json
{
  "serial_number": "728181095563584845125173905844944137943705466376"
}
```

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/credential/revoke
```

## Client API (alpha)

~> **Important:** The client API is experimental and may have backwards incompatible
changes introduced in subsequent versions of Vault, or be removed altogether. The client
API is disabled by default and should **NOT** be used in production.

The client API endpoints can be enabled via the `experiment:kmip.client_api.alpha1`
[experiment option](/vault/docs/configuration#experiments) in the Vault
configuration or on the command-line:

```shell-session
$ vault server -experiment kmip.client_api.alpha1
```

The client API lets you send KMIP operations directly to the Vault KMIP secrets
engine through the Vault HTTP API, without needing a native KMIP client
library. Requests are expressed as XML documents following the [KMIP XML
encoding specification](https://docs.oasis-open.org/kmip/profiles/v1.4/os/kmip-profiles-v1.4-os.html#_Toc491431438).

Vault ships a library of built-in XML request templates for the most common
operations.

### List templates

| Method | Path              |
| :----- | :---------------- |
| `LIST` | `/kmip/template` |

List the names of the built-in XML request templates.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request LIST \
    https://127.0.0.1:8200/v1/kmip/template
```

#### Sample response

```json
{
  "data": {
    "keys": [
      "addattribute",
      "create",
      "destroy",
      "get",
      "getattribute",
      "getattributelist",
      "getattributes",
      "locate",
      "locate_addattribute",
      "locate_by_name",
      "locate_decrypt",
      "locate_encrypt",
      "locate_get",
      "locate_getattribute",
      "locate_getattributelist",
      "locate_getattributes",
      "locate_modifyattribute",
      "locate_revoke",
      "locate_revoke_destroy",
      "modifyattribute",
      "revoke",
      "revoke_destroy"
    ]
  }
}
```

### Read template

| Method | Path                    |
| :----- | :---------------------- |
| `GET`  | `/kmip/template/:name` |

Read the XML content of a built-in request template and the list of parameter
placeholders it contains.

#### Parameters

- `name` (`string: <required>`) - Name of the template. This is part of the
  request URL.

#### Sample request

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request GET \
    https://127.0.0.1:8200/v1/kmip/template/create
```

#### Sample response

```json
{
  "data": {
    "template": "...",
    "parameters": ["algorithm", "length", "name", "usage_mask"]
  }
}
```

### Execute KMIP request

| Method | Path                                                            |
| :----- | :-------------------------------------------------------------- |
| `POST` | `/kmip/scope/:scope/role/:role/client[/:operations]` |

Execute one or more KMIP operations on behalf of the given scope and role.
The request is expressed as an XML document. You can provide either:

- A **built-in template name** (e.g. `create`, `locate_encrypt`) and its
  parameters, **or**
- A **custom XML template** string with `PARAMETER_<name>` placeholders and the
  matching parameter values, **or**
- The **operations** path segment (e.g. `.../client/locate/encrypt`) combined
  with parameters so that Vault selects the matching built-in template
  automatically.

#### Parameters

- `scope` (`string: <required>`) - Name of scope. This is part of the request URL.
- `role` (`string: <required>`) - Name of role. This is part of the request URL.
- `operations` (`string: ""`) - Slash-separated list of KMIP operation names
  (e.g. `locate/encrypt`). The operation of every batch item in the request must
  be specified.
- `template` (`string: ""`) - Either the name of a built-in template (e.g.
  `"create"`) or a raw XML template string with `PARAMETER_<name>` placeholders.
  If omitted, Vault selects a built-in template based on the `operations` URL
  segment.
- `params` (`map<string,string>`) - Arbitrary key/value replacement parameters
  for `PARAMETER_<name>` placeholders in the XML template.

The following named parameters are also accepted as top-level fields and are
equivalent to setting the same key in `params`:

| Parameter               | Description |
| :---------------------- | :---------- |
| `algorithm`             | Cryptographic algorithm |
| `attr_name`             | Name of the attribute |
| `attr_type`             | Type of the attribute (Boolean, DateTime, Enumeration, Integer, Interval, TextString) |
| `attr_value`            | Value of the attribute |
| `block_cipher_mode`     | Block cipher mode (CBC, GCM, etc.) |
| `data`                  | Data byte string (e.g. `"001a2b34"`) |
| `iv_counter_nonce`      | IV, counter, or nonce |
| `length`                | Cryptographic length |
| `name`                  | Name attribute value |
| `padding_method`        | Padding method (e.g. PKCS5) |
| `revocation_reason_code`| Revocation reason (Unspecified, KeyCompromise, Superseded, etc.) |
| `unique_identifier`     | UniqueIdentifier |
| `usage_mask`            | Cryptographic usage mask |

#### Sample request — create a symmetric key using a built-in template

```shell-session
$ vault write kmip/scope/myscope/role/myrole/client/create \
    name="mykey" \
    algorithm=AES \
    length=256 \
    usage_mask="Encrypt Decrypt"
```

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/client/create
```

Where `payload.json` contains:

```json
{
  "name": "mykey",
  "algorithm": "AES",
  "length": "256",
  "usage_mask": "Encrypt Decrypt"
}
```

#### Sample request — locate and encrypt using a built-in template

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/client/locate/encrypt
```

Where `payload.json` contains:

```json
{
  "name": "mykey",
  "block_cipher_mode": "GCM",
  "padding_method": "PKCS5",
  "data": "001a2b34",
  "iv_counter_nonce": "aabb1122"
}
```

#### Sample request — custom XML template

```shell-session
$ vault write kmip/scope/myscope/role/myrole/client/locate \
    template=@find.mxl \
    params=altname="kek"
```

```shell-session
$ curl \
    --header "X-Vault-Token: ..." \
    --request POST \
    --data @payload.json \
    https://127.0.0.1:8200/v1/kmip/scope/myscope/role/myrole/client/get
```

Where `payload.json` contains:

```json
{
  "template": "<KMIP><RequestMessage>...</RequestMessage></KMIP>",
  "params": {
    "altname": "kek"
  }
}
```

#### Sample response

```json
{
  "data": {
    "response": "<KMIP><ResponseMessage>...</ResponseMessage></KMIP>"
  }
}
```

#### Built-in templates

The following templates are built into the KMIP secrets engine. Use
[`LIST /kmip/template`](#list-templates) to retrieve the full list at runtime,
or [`GET /kmip/template/:name`](#read-template) to inspect a specific template
and its required parameters.

| Template                 | Operations               | Description |
| :----------------------- | :----------------------- | :---------- |
| `addattribute`           | AddAttribute             | Add an attribute |
| `create`                 | Create                   | Create a symmetric key |
| `destroy`                | Destroy                  | Destroy a managed object |
| `get`                    | Get                      | Get a managed object |
| `getattribute`           | GetAttributes            | Get an attribute by name |
| `getattributelist`       | GetAttributeList         | List attribute names of a managed object |
| `locate`                 | Locate                   | Locate all managed objects |
| `modifyattribute`        | ModifyAttribute          | Modify an attribute by name |
| `revoke`                 | Revoke                   | Revoke a managed object |
| `revoke_destroy`         | Revoke, Destroy          | Revoke and destroy a managed object |
| `locate_addattribute`    | Locate, AddAttribute     | Locate by name and add an attribute |
| `locate_by_name`         | Locate                   | Locate by name |
| `locate_decrypt`         | Locate, Decrypt          | Locate by name and decrypt |
| `locate_encrypt`         | Locate, Encrypt          | Locate by name and encrypt |
| `locate_get`             | Locate, Get              | Locate by name and get the object |
| `locate_getattributelist`| Locate, GetAttributeList | Locate by name and list attribute names |
| `locate_getattribute`    | Locate, GetAttributes    | Locate by name and get an attribute by name |
| `locate_getattributes`   | Locate, GetAttributes    | Locate by name and get all attributes |
| `locate_modifyattribute` | Locate, ModifyAttribute  | Locate by name and modify an attribute |
| `locate_revoke`          | Locate, Revoke           | Locate by name and revoke |
| `locate_revoke_destroy`  | Locate, Revoke, Destroy  | Locate by name, revoke, and destroy |
