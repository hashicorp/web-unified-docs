...
The SPIFFE secrets engine exposes two standard OIDC discovery endpoints that
allow OIDC providers to validate JWTs minted by the engine.

### OIDC discovery document

Returns the OpenID Connect discovery document for the SPIFFE secrets engine.

| Method | Path                                       |
|:-------|:-------------------------------------------|
| `GET`  | `/spiffe/.well-known/openid-configuration` |
...
```json
{
  "issuer": "https://vault.example.com/v1/spiffe",
  "jwks_uri": "https://vault.example.com/v1/spiffe/.well-known/keys",
...
Returns the JSON Web Key Set (JWKS) containing the public keys that OIDC
providers can use to verify the signature of JWTs minted by the SPIFFE secrets
engine.

| Method | Path                       |
|:-------|:---------------------------|
| `GET`  | `/spiffe/.well-known/keys` |
