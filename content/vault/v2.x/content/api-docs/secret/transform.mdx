...
created_at: 2026-04-15T00:13:57.000Z
last_modified: 2026-04-15T00:13:57.000Z
...
~> **Deprecation Warning!**
As of 2.1.0, FF3-1 is deprecated, and all new transformations created through this
endpoint will use FF1. Data encrypted under an FF3-1 transformation should be
migrated to an FF1 transformation using the [Rewrap](#rewrap) endpoint.
...
- `max_tweak_len` `(int: 0)` - The maximum tweak size allowed for this transformation.
...
- `tweak` `(string)` –
  ...
  transformations with `supplied` as the tweak source. For FF3-1 transformations,
  the tweak must be a 7-byte value. For FF1 transformations, the tweak's size must
  not exceed the transformation's `max_tweak_len`.
...
- `tweak` `(string)` – Specifies the **base64 encoded** tweak to use. Only
  ...
  source. For FF3-1 transformations, the tweak must be a 7-byte value. For FF1
  transformations, the tweak's size must not exceed the transformation's `max_tweak_len`.
...
This endpoint decodes a ciphertext under one FPE transformation and re-encodes
it under another FPE transformation. Both transformations must be FPE
transformations and must be accessible by the given role. This is the
recommended way to migrate data encrypted under a deprecated FF3-1
transformation to a new FF1 transformation.

| Method | Path                           |
| :----- | :----------------------------- |
| `POST` | `/transform/rewrap/:role_name` |
...
- `value` `(string: <required>)` –
  Specifies the encoded value to rewrap. Ignored when `batch_input` is set.

- `transformation` `(string: <required>)` –
  Specifies the destination FPE transformation to use to re-encode the value.
  Ignored when `batch_input` is set.

- `decode_transformation` `(string: <required>)` –
  Specifies the source FPE transformation to use to decode the value.
  Ignored when `batch_input` is set.

- `tweak` `(string)` –
  Specifies the **base64 encoded** tweak value to use when re-encoding with the
  destination transformation. Required when the destination transformation's
  `tweak_source` is `supplied`. Ignored for `generated` or `internal` tweak
  sources. Ignored when `batch_input` is set.

- `decode_tweak` `(string)` –
  Specifies the **base64 encoded** tweak value that was used when the value was
  originally encoded under the source transformation. Required when the source
  transformation's `tweak_source` is `supplied` or `generated`. Ignored when
  `batch_input` is set.

- `batch_input` `(array<object>: nil)` -
  Specifies a list of items to be rewrapped in a single batch. When this
  parameter is set, the `value`, `transformation`, `decode_transformation`,
  `tweak`, and `decode_tweak` parameters are ignored. Instead, those parameters
  should be provided within each object in the list.
  ...
  [
    {
      "value": "5682-4613-6822-8064",
      "transformation": "ccn-ff1",
      "decode_transformation": "ccn-fpe"
    }
  ]
  ```
...
$ curl \
    ...
    --request POST \
    --data @payload.json \
    http://127.0.0.1:8200/v1/transform/rewrap/example-role
```
...
{
  "data": {
    "encoded_value": "2314-8892-0171-4321"
  }
}
```
...
```json
{
  "data": {
    "batch_results": [
      ...
      {
        "encoded_value": "1234-5678-9012-3456"
      }
    ]
  }
}
