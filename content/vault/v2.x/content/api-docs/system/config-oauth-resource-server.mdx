...
last_modified: 2026-09-23T13:56:50.000Z
...
- `public_keys` `(list of objects: [])` – List of static public keys for
  ...
  - `key_id` `(string)` – The JWT Key ID (`kid`) for this key. Must be unique
    within the profile.
  - `pem` `(string)` – PEM-encoded public key. Vault accepts RSA, ECDSA, and
    Ed25519 keys.
...
- `supported_algorithms` `(list of strings: [])` – Signing algorithms to
  accept (for example, `RS256`, `ES256`, `EdDSA`). When empty, Vault accepts any
