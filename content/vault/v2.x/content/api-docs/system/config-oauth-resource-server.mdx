...
- `local` `(bool: false)` – When set to `true`, Vault stores the profile in
  cluster-local storage. Local profiles are never replicated to other clusters
  and can be written directly on a performance secondary without forwarding the
  request to the primary. This field is immutable after the profile is created;
  to change locality, delete and recreate the profile.
