...
last_modified: 2026-08-24T18:26:37.000Z
...
- `name` `(string: <required>)` – Name of the profile. The name must be unique
  across all profiles in the
  [target namespace](/vault/docs/enterprise/namespaces#vault-api-and-namespaces),
  regardless of locality. A local profile and a global profile cannot share a name.
...
- `issuer_id` `(string: <required>)` – The issuer identifier (`iss` claim) to
  ...
  lowercasing. The `issuer_id` must be unique within its locality (local or
  global) in the namespace. A local profile and a global profile may share an
  `issuer_id`. You cannot modify `issuer_id` during updates.

- `local` `(bool: false)` – When set to `true`, Vault stores the profile in
  cluster-local storage and never replicates it to other clusters. You can write
  local profiles directly on a performance secondary without forwarding the
  request to the primary. The `local` field is immutable after creation. Delete
  and recreate the profile to change locality.
...
- You cannot change `local` after creating the profile. Omit `local` on updates,
  or pass the value already stored. Vault returns an error if you attempt to
  change `local` on updates.
