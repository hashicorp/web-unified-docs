...
last_modified: 2026-08-21T22:35:44.000Z
...
- `local` `(bool: false)` – When set to `true`, Vault stores the profile in
  ...
  forwarding the request to the primary. The `local` field is immutable once you create
  the profile. You must delete and recreate the profile to change locality.
...
- `issuer_id`, `local`, and `unique_id_claim` cannot be changed after creating
