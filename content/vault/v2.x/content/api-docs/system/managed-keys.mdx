...
- `rsa_oaep_hash` `(string: "")` - The hash algorithm to use for RSA-OAEP encryption with
  `CKM_RSA_PKCS_OAEP`. Requires mechanism `CKM_RSA_PKCS_OAEP`. The supported values are
    - `sha1`
    - `sha224`
    - `sha256`
    - `sha384`
...
- `encryption_mode` `(string: "envelope")`: The encryption mode for the key. `envelope` for AES-GCM envelope encryption
  or `raw` to encrypt directly with the KMS key.
