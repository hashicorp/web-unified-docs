...
- `aws_access_key_id` `(string)` - AWS access key ID. Provide together with
  `aws_secret_access_key` to authenticate using static credentials, or omit both
  to use the AWS SDK default credential chain, which supports IAM Roles for
  Service Accounts (IRSA) on EKS. The AWS identity Vault uses must have the
