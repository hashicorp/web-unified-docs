...
- `aws_access_key_id` `(string)` - AWS access key ID. Provide together with
  `aws_secret_access_key` to authenticate using static credentials. For IAM roles
  for service accounts (IRSA) on EKS, leave `aws_access_key_id` and
  `aws_secret_access_key` unset to use the AWS SDK default credential chain. The
  AWS identity Vault uses must have the
  following S3 permissions so that Vault can store and manage the snapshots:
  - `s3:ListBucket`
  - `s3:PutObject`
  - `s3:DeleteObject`
