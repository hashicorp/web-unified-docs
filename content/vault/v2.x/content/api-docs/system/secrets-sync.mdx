...
fallbacks on the AWS credentials provider chain and tries to infer authentication from the environment. Mutually exclusive with
...
- `identity_token_audience` `(string: "")` - Audience claim value for identity tokens used in workload identity federation (WIF) authentication. The value must match the allowed audiences configured for the IAM OIDC identity provider in AWS. Required for WIF authentication. Mutually exclusive with `access_key_id`.

- `identity_token_key` `(string: "")` - Name of a configured identity token key to use for signing identity tokens for WIF authentication. [Create the key](/vault/api-docs/secret/identity/tokens#create-a-named-key) before configuring the identity token key. Leave `identity_token_key` unset to use the [default](/vault/docs/concepts/oidc-provider#keys) identity token signing key.

- `identity_token_ttl` `(string/int: 3600)` - Time-to-live for generated identity tokens used in WIF authentication. Accepts [duration format strings](/vault/docs/concepts/duration-format).
...
- `kms_key_id` `(string: "")` - ARN of the customer-managed AWS KMS key Vault uses to encrypt synced secrets
  at rest in the primary region.

- `replica_regions` `(map<string|string>: nil)` - Map of region names to the AWS KMS key (ARN) Vault uses to encrypt
  secrets replicated to each region. AWS requires the KMS key for a region to reside in that same region. The KMS key
  value for a region is optional and may be an empty string to replicate to that region with the AWS-managed key.
...
#### Sync secrets to AWS with a customer-managed KMS key:
```json
{
    "access_key_id": "AKI***",
    "secret_access_key": "ktri****",
...
#### Sync secrets to AWS with multi-region replica encryption:
```json
{
    "access_key_id": "AKI***",
    "secret_access_key": "ktri****",
    ...
    "kms_key_id": "arn:aws:kms:us-east-1:123456789012:key/12345678-1234-1234-1234-123456789012",
    "replica_regions": {
        "us-east-2": "arn:aws:kms:us-east-2:123456789012:key/22222222-2222-2222-2222-222222222222",
        "us-west-1": ""
    }
}
```
...
- `client_secret` `(string: "")` - Client secret of an Azure app registration.
  Required for client secret authentication and mutually exclusive with
  `identity_token_audience`. If you omit `client_secret`, Vault falls back to
  alternative authentication methods:
  - Set `identity_token_audience` to fall back to workload identity federation
    (WIF).
  - Leave `client_secret` and `identity_token_audience` unset to fall back to
    Azure managed identity.

- `tenant_id` `(string: <required>)` - ID of the target Azure tenant.

- `identity_token_audience` `(string: "")` - Audience claim value for identity tokens used in workload identity federation (WIF) authentication. The value must match the allowed audiences configured for the federated identity credential in Azure. Required for WIF authentication. Mutually exclusive with `client_secret`.

- `identity_token_key` `(string: "")` - Name of a configured identity token key to use for signing identity tokens for WIF authentication. [Create the key](/vault/api-docs/secret/identity/tokens#create-a-named-key) before configuring the identity token key. Leave `identity_token_key` unset to use the [default](/vault/docs/concepts/oidc-provider#keys) identity token signing key.

- `identity_token_ttl` `(duration: "1h")` - Time-to-live for generated identity tokens used in WIF authentication. Accepts duration format strings such as "30m" or "1h". Must be non-negative.
...
#### Sync secrets to Azure Key Vault with client secret authentication:
```json
{
...
#### Sync secrets to Azure Key Vault with workload identity federation (WIF):
```json
{
    "key_vault_uri": "https://keyvault-1234abcd.vault.azure.net",
    "tenant_id": "uuid",
...
$ curl \
    ...
    --request POST \
    --data @payload.json
    http://127.0.0.1:8200/v1/sys/sync/destinations/azure-kv/my-store-1
```
...
- `credentials` `(string: <required>)` - JSON credentials (either file contents or '@path/to/file')
    Required for static credentials authentication and mutually exclusive with
  `identity_token_audience`. If you omit `credentials`, Vault falls back to
  alternative authentication methods:
  - Set `identity_token_audience` to fall back to workload identity federation
    (WIF).
  - Leave `client_secret` and `identity_token_audience` unset to fall back to
...
- `replication_locations` `(list: nil)` - **DEPRECATED** Use `replica_regions` instead. A list of GCP location names the destination can use to
  store replicated secrets. Note that secrets remain globally readable regardless of the selected locations.

- `locational_kms_keys` `(map<string|string>: nil)` - **DEPRECATED** Use `replica_regions` instead. A map of location names to KMS key names to leverage customer-managed encryption keys for
  encryption at rest. Each pair follows the format `location_name=encryption_key_resource_name`.

- `global_kms_key` `(string: "")` - **DEPRECATED** Use `kms_key_id` instead. The KMS key ID Vault should use to encrypt secrets at rest in GCP Secrets Manager.

- `kms_key_id` `(string: "")` - Resource name of the customer-managed KMS key Vault uses to encrypt synced secrets with
  GCP automatic replication, for example
  `projects/{project}/locations/global/keyRings/{keyring}/cryptoKeys/{key}`. GCP requires that the key reside in the
  `global` multi-region. Mutually exclusive with `replica_regions`.

- `replica_regions` `(map<string|string>: nil)` - Map of GCP location names to the KMS key (key resource name) Vault
  uses when replicating secrets with user-managed replication. Each key must reside in the location it encrypts. To use
  customer-managed encryption, provide a key for every location. GCP requires that either all locations, or none of the locations have a key . Leave every value empty to replicate to the listed locations with Google-managed encryption. Mutually exclusive
  with `kms_key_id`. Refer to the [sample payloads](#sample-payloads-2) for more details.
...
- `identity_token_audience` `(string: "")` - Audience claim value for identity tokens used in workload identity federation (WIF) authentication. The value must match the allowed audiences configured for the workload identity provider in GCP. Required for WIF authentication. Mutually exclusive with `credentials`.

- `service_account_email` `(string: "")` - Email of the service account to be impersonated in the workload identity federation flow. This value is used along with `identity_token_audience`. 
...
#### Sync secrets to GCP with WIF Credentials:
```json
{
    "identity_token_audience": "<audience value>",
    "identity_token_ttl": "1h",
    "service_account_email": "<service account email value>"
    "replication_locations": [
        "us-east1",
        "us-west1"
    ],
...
{
    "credentials": "<private key string>",
    "kms_key_id": "projects/my-project/locations/global/keyRings/my-global-keyring/cryptoKeys/my-global-key"
}
```

#### Sync secrets to GCP with user-managed replication and Google-managed encryption:
```json
{
    "credentials": "<private key string>",
    "replica_regions": {
...
#### Sync secrets to GCP with user-managed replication and customer-managed keys:
```json
{
    "credentials": "<private key string>",
    "replica_regions": {
