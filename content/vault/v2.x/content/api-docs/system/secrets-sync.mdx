...
- `kms_key_id` `(string: "")` - ARN or alias of the customer-managed AWS KMS key Vault uses to encrypt synced secrets
...
- `kms_key_id` `(string: "")` - Resource name of the customer-managed KMS key Vault uses to encrypt synced secrets with
  ...
  `projects/{project}/locations/global/keyRings/{keyring}/cryptoKeys/{key}`. GCP requires this key to reside in the
...
- `replica_regions` `(map<string|string>: nil)` - Map of GCP location names to the KMS key (key resource name) Vault
  ...
  customer-managed encryption, provide a key for every location; GCP requires all locations to have a key or none of
  them. Leave every value empty to replicate to the listed locations with Google-managed encryption. Mutually exclusive
