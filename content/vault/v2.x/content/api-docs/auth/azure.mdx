...
- `auth_type` `(string: "")` - The credential mechanism Vault should use to
  authenticate to Azure APIs. Leave `auth_type` empty to uses the existing,
  ...
  - `root_creds` - Uses the static `client_id` and `client_secret` configured in
    the auth mount. You must set `client_secret` to use `root_creds`.

  - `plugin_wif` - <EnterpriseAlert product="vault" inline />
    Uses a Vault-issued OIDC plugin identity token. You must set
    `identity_token_audience` to use `plugin_wif` and configure your Azure tenant
    to trust tokens issued by the Vault plugin identity token issuer using
    workload identity federation.

  - `msi` — Uses the Azure Managed Identity attached to the host resource where
    Vault is running.

  - `aks_wif` — Uses
    [AKS Workload Identity Federation](/vault/docs/auth/azure#aks-workload-identity-federation-aks-wif).
    AKS injects a projected Kubernetes service account token into the Vault pod,
    which Vault exchanges for a short-lived Microsoft Entra access token scoped
    to the configured `tenant_id`. Using a token scoped to the configured tenant avoids `InvalidAuthenticationTokenTenant`
    errors caused by using tokens from the wrong tenant in cross-tenant deployments.
    See Microsoft's
