...
- `auth_type` `(string: "")` - The credential mechanism Vault should use to
  ...
  - `aks_wif` — Uses
    [AKS Workload Identity Federation](/vault/docs/auth/azure#aks-workload-identity-federation-aks-wi).
    ...
    to the configured `tenant_id`. 
    [Microsoft AKS WIF](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview)
    `InvalidAuthenticationTokenTenant` errors in cross-tenant deployments. See 
