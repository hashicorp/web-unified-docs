...
identity federation (WIF), or aks workload identity federation (AKS WIF).
...
- `auth_type` `(string: "")` - The credential mechanism Vault uses to authenticate
  ...
  - `aks_wif` — Uses [AKS Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview)
