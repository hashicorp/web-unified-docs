...
- `auth_type` `(string: "auto")` - Specifies how Vault authenticates to Azure when
  querying resource metadata. Defaults to `auto`, which preserves the existing
  credential discovery behavior for backward compatibility. See
  [authentication types](/vault/docs/auth/azure#authentication-types) for details.
  Valid values are:
  - `auto` - Automatically selects the best available authentication method based on
    the configuration and runtime environment (the existing discovery logic).
  - `root_creds` - Uses a static client ID and client secret. Requires `client_secret`.
  - `plugin_wif` - <EnterpriseAlert product="vault" inline /> Uses Vault
    [plugin workload identity federation](/vault/docs/auth/azure#plugin-workload-identity-federation-wif).
    Requires `identity_token_audience`.
  - `aks_wi` - Uses [AKS Workload Identity federation](/vault/docs/auth/azure#aks-workload-identity-federation-aks-wi)
    for secretless, cross-tenant authentication.
