...
to Azure. These credentials are used to query Azure resource metadata during
authentication.

You can configure the auth engine with account credentials, plugin workload
identity federation (WIF), or AKS Workload Identity (`aks_wi`).
...
- `auth_type` `(string: "")` - Specifies the credential mechanism Vault uses to authenticate
  to Azure APIs. If omitted or set to an empty string, Vault uses its existing backward-compatible
  credential discovery logic: it checks for `client_secret` → `identity_token_audience` →
  `AZURE_FEDERATED_TOKEN_FILE` environment variable → Managed Identity, in that order. Accepted
  values when set:
  - `root_creds` — Uses the static `client_id` and `client_secret` configured in the auth mount.
    Requires `client_secret` to be set.
  - `plugin_wif` — <EnterpriseAlert product="vault" inline /> Uses a Vault-issued OIDC plugin
    identity token. Requires `identity_token_audience` to be set. Azure must trust tokens issued
    by the Vault plugin identity token issuer via workload identity federation.
  - `msi` — Uses the Azure Managed Identity attached to the host resource where Vault is running.
  - `aks_wi` — Uses [AKS Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview)
    federation. AKS injects a projected Kubernetes service account token into the Vault pod;
    Vault exchanges it for a short-lived Microsoft Entra access token scoped to the configured
    `tenant_id`. This resolves `InvalidAuthenticationTokenTenant` errors in cross-tenant
    deployments. See [AKS Workload Identity Federation](/vault/docs/auth/azure#aks-workload-identity-federation-aks-wi)
