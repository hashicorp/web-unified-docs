...
created_at: 2026-04-15T00:13:44.000Z
last_modified: 2026-04-15T00:13:44.000Z
...
- `discoverdn` `(bool: false)` – Use anonymous bind to discover the bind DN of a
  user.
- `deny_null_bind` `(bool: true)` – **DEPRECATED**. As of v1.21.1, Vault
  enforces non-empty passwords on login regardless of the value of `deny_null_bind`.
...
- `rotation_url` `(string: "")` – <EnterpriseAlert product="vault" inline /> The LDAP
  server to connect to for password rotation if it is different from `url` parameter.
...
- `schema` `(string: "openldap")` - The LDAP schema to use when storing entry passwords.
  Valid schemas include `openldap` and `ad`. Active Directory users must set `schema` to `ad`.
...
$ curl \
    --request POST \
    --header "X-Vault-Token: ..." \
    http://127.0.0.1:8200/v1/auth/ldap/config/rotate-root
```
