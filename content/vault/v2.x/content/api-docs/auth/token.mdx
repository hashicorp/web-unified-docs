...
last_modified: 2026-06-10T15:59:21.000Z
...
In Vault Enterprise, child tokens inherit the parent token's direct policies
when Vault creates the child token in the same namespace as the parent token and
no explicit policies are requested. When Vault creates the child token in a
different namespace than the parent token, Vault does not automatically inherit
the parent token's direct policies from the parent namespace. If a child token
is created in a different namespace to the parent, it receives only explicitly requested policies for the target namespace from its parent, plus `default`
when applicable. Parent namespace identity-derived policies may appear as
...
- `policies` `(array: "")` – A list of policies for the token. This must be a
  ...
  If not specified, Vault applies the policy inheritance rules for the namespace
  where the child token is created.
