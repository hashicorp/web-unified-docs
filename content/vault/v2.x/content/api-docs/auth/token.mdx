...
last_modified: 2026-06-10T21:14:44.000Z
...
If you create a child token under the same namespace as the parent, the token
receives all polices associated with the parent token unless you override
inheritance with explicit policy assignments.

If you create a child token under a different namespace from the parent, the
token receives any parent policies tied to the target namespace and the
`default` policy  (when applicable) unless you override inheritance with
explicit policy assignments.

Vault always returns identity-derived policies from the parent namespace as
...
- `policies` `(array: "")` – A list of policies for the token. This must be a
  ...
  where you create the child token.
