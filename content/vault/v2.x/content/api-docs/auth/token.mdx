...
the parent token's direct policies from the parent namespace. The child token
receives explicitly requested policies for the target namespace, plus `default`
