...
In Vault Enterprise, child tokens inherit all of the parent token's policies when
Vault creates the child token in the same namespace as the parent token and no
explicit policies are requested. Child tokens created in a different namespace
receive only explicitly requested policies for the target namespace from the
parent, plus `default` when applicable. Parent namespace identity-derived
policies are returned as `external_namespace_policies`.
