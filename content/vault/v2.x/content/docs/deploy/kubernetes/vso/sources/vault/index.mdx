...
last_modified: 2026-09-23T09:44:44.000Z
...
|---|---|---|
| KV v1 & v2 | `kv` | Used by `VaultStaticSecret`. |
| Database | `database` | Static roles and dynamic credentials via `VaultDynamicSecret`. Needs VSO 1.6.0+ |
| LDAP | `ldap` | Static roles and dynamic credentials via `VaultDynamicSecret`. Needs VSO 1.6.0+ |
| Lease lifecycle (all lease-based engines) | `lease` | Revoke/expire events for `VaultDynamicSecret` dynamic credentials. Needs VSO 1.6.0+ |
| Azure | `azure` | Static roles via `VaultDynamicSecret`. Needs VSO 1.6.0+ |
