...
last_modified: 2026-09-24T17:40:20.000Z
...
- [Instant updates](/vault/docs/platform/k8s/vso/sources/vault#instant-updates)
  for VaultStaticSecret's with Vault Enterprise 1.13+.
...
`VaultDynamicSecret` covers two credential patterns:

- **Auto or manually rotated credentials for static roles** (For example database, and LDAP static roles)
  set `spec.allowStaticCreds: true`. Vault rotates the credentials on a
  schedule; VSO reads the updated value on each rotation event.
- **Dynamic lease based credentials**  leave `spec.allowStaticCreds: false`
  (the default). Vault issues a new set of credentials on every request and
  assigns them a TTL; VSO renews the lease, or requests newly
  issued credentials when the lease expires or is revoked.

Both patterns support [instant updates](#instant-updates).
...
due to the use of [Vault Event Notifications][vault-events]. The minimum
version depends on the engine and credential type, refer to
[Supported engines](#supported-engines) below for details.

</Note>

### Supported engines

VSO resolves the engine type at runtime via a live `sys/mounts/<mount>` lookup
and subscribes to `/v1/sys/events/subscribe/<event type>*`. The event router has
specific handling for `kv` (routes by secret path) and `lease` (routes by lease
ID, skips renewal events). All other engine types — `database`, `ldap`,
`azure`, and any future engine — share a generic `default` routing path that
matches by mount and role name.

| Engine | Vault event type | Notes |
|---|---|---|
| KV v1 & v2 | `kv` | Used by `VaultStaticSecret`.<br />Needs Vault Enterprise v1.13+ |
| Database | `database` | Static roles via `VaultDynamicSecret`.<br />Needs VSO v1.6.0+ and Vault Enterprise v1.16+ |
| LDAP | `ldap` | Static roles via `VaultDynamicSecret`.<br />Needs VSO v1.6.0+ and Vault Enterprise v1.21+ |
| Lease lifecycle (all lease-based engines) | `lease` | Events for `VaultDynamicSecret` dynamic credentials.<br />Needs VSO v1.6.0+ and Vault Enterprise v2.0+ |
| Azure | `azure` | Static roles via `VaultDynamicSecret`.<br />Needs VSO v1.6.0+ and Vault Enterprise v2.0+ |

### Supported secret types

Enable instant updates by setting `spec.syncConfig.instantUpdates: true` on the
relevant custom resource.

| Secret type | Custom resource | `spec` fields |
|---|---|---|
| KV v1 / v2 static secrets | `VaultStaticSecret` | `syncConfig.instantUpdates: true` |
| Auto/Manually rotated credentials for static-roles (DB / LDAP / Azure static roles) | `VaultDynamicSecret` | `allowStaticCreds: true`<br />`syncConfig.instantUpdates: true` |
| Dynamic lease-based credentials | `VaultDynamicSecret` | `syncConfig.instantUpdates: true` |

`spec.allowStaticCreds` controls the distinction between static-role and dynamic lease credentials. When `true`, VSO subscribes to the engine WebSocket
only — the rotation event is sufficient. When `false` (the default), VSO also
subscribes to a `lease` WebSocket to detect revoke and expire events and
re-issue credentials immediately.

The [Instant updates](/vault/docs/platform/k8s/vso/sources/vault/instant-updates) guide
will walk you through the steps to enable instant updates for each secret type.
