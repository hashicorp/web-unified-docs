...
last_modified: 2026-09-24T11:13:27.000Z
...
- **Auto/Manually rotated credentials for static-roles** (For example database, and LDAP static roles) —
...
- **Dynamic lease-based credentials** — leave `spec.allowStaticCreds: false`
  ...
  assigns them a TTL; VSO renews the lease when possible, or requests newly
...
|---|---|---|
| KV v1 & v2 | `kv` | Used by `VaultStaticSecret`.<br />Needs Vault Enterprise v1.16.3+ |
| Database | `database` | Static roles via `VaultDynamicSecret`.<br />Needs VSO v1.6.0+ and Vault Enterprise v1.16.3+ |
| LDAP | `ldap` | Static roles via `VaultDynamicSecret`.<br />Needs VSO v1.6.0+ and Vault Enterprise v1.16.3+ |
