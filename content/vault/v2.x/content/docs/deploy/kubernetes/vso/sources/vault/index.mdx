...
last_modified: 2026-09-23T11:25:57.000Z
...
| Engine | Vault event type | Notes | Vault Enterprise version |
|---|---|---|---|
| KV v1 & v2 | `kv` | Used by `VaultStaticSecret`. | 1.16.3+ |
| Database | `database` | Static roles and dynamic credentials via `VaultDynamicSecret`. Needs VSO v1.6.0+ | 1.16.3+ (static roles) / 2.0+ (dynamic/lease) |
| LDAP | `ldap` | Static roles and dynamic credentials via `VaultDynamicSecret`. Needs VSO v1.6.0+ | 1.16.3+ (static roles) / 2.0+ (dynamic/lease) |
| Lease lifecycle (all lease-based engines) | `lease` | Revoke/expire events for `VaultDynamicSecret` dynamic credentials. Needs VSO v1.6.0+ | 2.0+ |
| Azure | `azure` | Static roles via `VaultDynamicSecret`. Needs VSO v1.6.0+ | 2.0+ |
