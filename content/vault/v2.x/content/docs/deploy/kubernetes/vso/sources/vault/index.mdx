...
last_modified: 2026-07-31T08:52:30.000Z
...
`VaultStaticSecret` is designed for KV v1 and KV v2 secrets only. If you need
instant updates for secrets from other engines (such as database or LDAP
static-role credentials), use the
[VaultDynamicSecret](#vaultdynamicsecret-custom-resource) custom resource with
`spec.allowStaticCreds: true`.
...
- **Auto-rotated static credentials** (e.g. database and LDAP static roles) —
...
- **Dynamic lease-based credentials** — leave `spec.allowStaticCreds: false`
  ...
  assigns them a TTL; VSO renews the lease and re-issues when it expires or is
  revoked.
...
|---|---|---|
| KV v1 & v2 | `kv` | Confirmed. Used by `VaultStaticSecret`. |
| Database | `database` | Confirmed. Static roles and dynamic credentials via `VaultDynamicSecret`. |
| LDAP | `ldap` | Confirmed. Static roles and dynamic credentials via `VaultDynamicSecret`. |
| Lease lifecycle (all lease-based engines) | `lease` | Confirmed. Revoke/expire events for `VaultDynamicSecret` dynamic credentials. |
| Azure | `azure` | Depends on Vault emitting rotation events for [Azure static roles](https://developer.hashicorp.com/vault/api-docs/secret/azure#create-update-static-role). VSO subscribes generically if the mount type resolves. |
...
| Auto-rotated static credentials (DB / LDAP / Azure static roles) | `VaultDynamicSecret` | `allowStaticCreds: true`<br />`syncConfig.instantUpdates: true` |
| Dynamic lease-based credentials | `VaultDynamicSecret` | `syncConfig.instantUpdates: true` |

The distinction between auto-rotated and dynamic lease credentials is controlled
by `spec.allowStaticCreds`. When `true`, VSO subscribes to the engine WebSocket
