...
created_at: 2026-04-15T00:15:13.000Z
last_modified: 2026-09-25T05:30:29.000Z
...
hashicorp/vault-secrets-operator	1.6.0       	1.6.0     	Official HashiCorp Vault Secrets Operator Chart
```

Then install the Operator:

```shell-session
$ helm install --version 1.6.0 --create-namespace --namespace vault-secrets-operator vault-secrets-operator hashicorp/vault-secrets-operator
```

## CSI driver option

You can also use the Vault Secrets Operator Helm chart to deploy a DaemonSet of the
[Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/csi) with
the `"csi.enabled"` flag:
...
Refer to the [Vault Secrets Operator CSI driver](/vault/docs/platform/k8s/vso/csi)
documentation to learn how to use the CSI driver to mount secrets directly to application pods.
...
<Note title="Removing deprecated HCP Vault Secrets resources prior to upgrading">

 If you are upgrading from a version of VSO that supported HCP Vault Secrets (HVS), remove all `HCPVaultSecretsApp` and `HCPAuth` custom resources **before** upgrading the operator. `HCPVaultSecretsApp` resources use finalizers and deleting the operator before removing the custom resources may cause them to remain in a `Terminating` state. After upgrading, delete the obsolete HVS CRDs if they remain:

  ```shell-session
  $ kubectl delete crd hcpauths.secrets.hashicorp.com hcpvaultsecretsapps.secrets.hashicorp.com
  ```
...
For example, if you are upgrading to VSO 1.6.0:
```shell-session
$ helm upgrade --version 1.6.0 --namespace vault-secrets-operator vault-secrets-operator hashicorp/vault-secrets-operator
```
...
```shell-session
$ wget -q https://github.com/hashicorp/vault-secrets-operator/archive/refs/tags/v1.6.0.tar.gz
$ tar -zxf v1.6.0.tar.gz
$ cd vault-secrets-operator-1.6.0/
```
