...
<Note title="Removing deprecated HCP Vault Secrets resources prior to upgrading">
  ...
  If upgrading from a version of VSO that supported HCP Vault Secrets (HVS), ensure all `HCPVaultSecretsApp` and `HCPAuth` custom resources are deleted **before** upgrading the operator. Because `HCPVaultSecretsApp` resources use finalizers, deleting the operator before removing the custom resources may cause them to remain in a `Terminating` state. After upgrading, delete the obsolete HVS CRDs if they remain:
