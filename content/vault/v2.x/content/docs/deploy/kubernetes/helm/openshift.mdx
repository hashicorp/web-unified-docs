...
last_modified: 2026-09-26T18:43:05.000Z
...
Then install Vault Enterprise on OpenShift. Setting `server.enterpriseLicense.secretName` causes the chart to automatically use `hashicorp/vault-enterprise` for the Vault server and for the Vault Agent sidecar images injected by the Agent Injector and used by the CSI Provider — no manual image configuration required. The Agent Injector's and CSI Provider's own images are unaffected:
...
Append additional parameters (HA, Raft, and dev mode) as `--set` flags or via a values file as needed. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for full details, including pinning a specific release tag or using a custom registry.
