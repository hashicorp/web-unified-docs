...
last_modified: 2026-09-26T17:42:11.000Z
...
If you have a Vault Enterprise license, create the `vault` namespace and a
Kubernetes secret using the contents of your license file before installing.
The secret must exist in the same namespace as the Helm release
(`vault` in this example), so create the namespace first and target it
explicitly when creating the secret:

```shell-session
$ kubectl create namespace vault
$ secret=$(cat 1931d1f4-bdfd-6881-f3f5-19349374841f.hclic)
$ kubectl create secret generic vault-ent-license --namespace vault --from-literal="license=${secret}"
```

Then install Vault Enterprise on OpenShift. When `server.enterpriseLicense.secretName` is set, the chart automatically uses `hashicorp/vault-enterprise` for the Vault server and for the Vault Agent sidecar images injected by the Agent Injector and used by the CSI Provider — no manual image configuration required. The Agent Injector's and CSI Provider's own images are unaffected:
...
Additional parameters (HA, Raft, and dev mode) can be appended as `--set` flags or via a values file as needed. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for full details, including pinning a specific release tag or using a custom registry.

#### Vault Community Edition

To install the community edition, use `helm install` without a license secret:
