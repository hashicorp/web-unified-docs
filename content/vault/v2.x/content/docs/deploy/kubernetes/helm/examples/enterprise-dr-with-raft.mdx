...
last_modified: 2026-09-26T17:42:08.000Z
...
-> **Enterprise image auto-selection:** Setting `server.enterpriseLicense.secretName` causes the chart to automatically select the `hashicorp/vault-enterprise` image for the Vault server — you do not need to manually set `server.image.repository` or `server.image.tag`. For full license configuration options, refer to [Running Vault Enterprise](/vault/docs/platform/k8s/helm/enterprise).

## Prerequisites

The `helm install` commands below set `server.enterpriseLicense.secretName=vault-ent-license`,
which requires a Kubernetes secret with that name to already exist in the
target namespace. Before installing the primary cluster, create the secret
from the contents of your license file:
...
-> **Note:** If you cannot find your `.hclic` file, please contact your sales
team or Technical Account Manager. If the secondary cluster is installed in a
different namespace than the primary, repeat this step in that namespace
before installing the secondary cluster.
