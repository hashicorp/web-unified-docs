...
- `injector` - Values that configure running a Vault Agent Injector Admission Webhook Controller within Kubernetes.
  ...
  - `image` - Values that configure the Vault Agent Injector Docker image.
    ...
    - `tag` (`string: "1.7.2"`) - The tag of the Docker image for the Vault Agent Injector. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your admission controller.
  ...
  - `agentImage` - Values that configure the Vault Agent sidecar image.
    ...
    - `tag` (`string: "2.0.2"`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar. **Vault 1.3.1+ is required by the admission controller**.
...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    - `tag` (`string: "2.0.2"`) - The tag of the Docker image for the containers running Vault. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your admission controller.
  ...
  - `ha` - This configures `ha` mode for the Vault server.
    ...
    - `raft` - This configures `raft` integrated storage mode for the Vault server.
      ...
      - `redundancyZones` - Vault Enterprise redundancy zones configuration for autopilot. Requires Kubernetes 1.35+ with PodTopologyLabelsAdmission and nodes labeled with `topology.kubernetes.io/zone`.

        - `enabled` (`boolean: false`) - Enable automatic zone detection for Vault Enterprise autopilot redundancy zones. When enabled:
          - The zone label from the node is exposed as `VAULT_REDUNDANCY_ZONE` environment variable
          - The `VAULT_REDUNDANCY_ZONE` placeholder in `server.ha.raft.config` is automatically substituted with the actual zone value
          - The PDB `maxUnavailable` defaults to 1 (instead of ⌊(n-1)/2⌋) to prevent quorum loss
          - Requires `server.ha.enabled=true`, `server.ha.raft.enabled=true`, and the `autopilot_redundancy_zone = "VAULT_REDUNDANCY_ZONE"` placeholder in the Raft configuration
          - See the [redundancy zones example](/vault/docs/platform/k8s/helm/examples/enterprise-with-raft#redundancy-zones) for configuration details
...
- `csi` - Values that configure running the Vault CSI Provider.
  ...
  - `image` - Values that configure the Vault CSI Provider Docker image.
    ...
    - `tag` (`string: "1.7.0"`) - The tag of the Docker image for the Vault CSI Provider.. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your CSI provider.
  ...
  - `agent` - Configures the Vault Agent sidecar for the CSI Provider
    ...
    - `image` - Values that configure the Vault Agent sidecar image for the CSI Provider.
      ...
      - `tag` (`string: "2.0.2"`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar.
