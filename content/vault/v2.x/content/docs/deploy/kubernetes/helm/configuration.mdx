...
last_modified: 2026-09-26T18:42:58.000Z
...
- `injector` - Values that configure running a Vault Agent Injector Admission Webhook Controller within Kubernetes.
  ...
  - `image` - Values that configure the Vault Agent Injector Docker image. This image does not change based on [`server.enterpriseLicense`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense).

    - `repository` (`string: "hashicorp/vault-k8s"`) - The name of the Docker image for Vault Agent Injector.

    - `tag` (`string: "1.7.6"`) - The tag of the Docker image for the Vault Agent Injector. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your admission controller.
  ...
  - `agentImage` - Values that configure the Vault Agent sidecar image that the injector adds to annotated application pods.

    - `repository` (`string: "hashicorp/vault"`) - The name of the Docker image for the Vault Agent sidecar. This should be set to the official Vault Docker image. Setting [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) and leaving this field at its default causes the chart to resolve it to `hashicorp/vault-enterprise` instead. Refer to [Automatic image selection](/vault/docs/platform/k8s/helm/enterprise#automatic-image-selection) for details.

    - `tag` (`string: ""`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar. **Vault 1.3.1+ is required by the admission controller**. When empty, the chart resolves this to the chart's `AppVersion`; configuring an Enterprise license changes this to `AppVersion-ent`.
...
- `server` - Values that configure running a Vault server within Kubernetes.
  ...
  - `enterpriseLicense` - This value refers to a Kubernetes secret that you have created that contains your enterprise license. If you are not using an enterprise image or if you plan to introduce the license key via another route, then leave secretName blank ("") or set it to null. Requires Vault Enterprise 1.8 or later. Setting `secretName` also drives automatic image selection for `server.image`, `injector.agentImage`, and `csi.agent.image`. Refer to [Automatic image selection](/vault/docs/platform/k8s/helm/enterprise#automatic-image-selection) for details.
  ...
  - `serviceCA` - OpenShift service-ca operator integration. Automates TLS certificate generation via the service-ca operator.

    - `enabled` (`boolean: false`) - Enable service-ca operator annotations and ConfigMap creation. Requires: global.openshift=true

    - `secretName` (`string: "vault-tls"`) - Secret name where service-ca will store the generated certificate. IMPORTANT: Must match the secret name in `server.volumes` below.

    - `configMapName` (`string: "service-ca-bundle"`) - ConfigMap name for service-ca CA bundle injection. IMPORTANT: Must match the ConfigMap name in `server.volumes` below.
  ...
  - `image` - Values that configure the Vault Docker image.
    ...
    - `repository` (`string: "hashicorp/vault"`) - The name of the Docker image for the containers running Vault. Setting [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) and leaving this field at its default causes the chart to resolve it to `hashicorp/vault-enterprise` instead. Refer to [Automatic image selection](/vault/docs/platform/k8s/helm/enterprise#automatic-image-selection) for details.

    - `tag` (`string: ""`) - The tag of the Docker image for the containers
      running Vault. We recommend pinning the tag to a specific version when
      running in production to avoid accidentally upgrading your admission
      controller due to other chart changes. When empty, the chart resolves this
      to the chart's `AppVersion`; configuring an Enterprise license changes this
      to `AppVersion-ent`.
  ...
  - `httproute` - Values that configure HTTPRoute services for Vault using the Kubernetes Gateway API.

    ~> HTTPRoute requires the [Gateway API CRDs](https://gateway-api.sigs.k8s.io/guides/getting-started/introduction/) to be installed on your cluster. <br/> <br/>
    If [`ha`](#ha) is enabled the HTTPRoute will point to the active vault server via the `active` Service by default. This requires vault 1.4+ and [service_registration](/vault/docs/configuration/service-registration/kubernetes) to be set in the vault config.

    - `enabled` (`boolean: false`) - When set to `true`, an [HTTPRoute](https://gateway-api.sigs.k8s.io/guides/user-guides/http-routing/) resource will be created.

    - `labels` (`dictionary: {}`) - Labels for the HTTPRoute resource.

    - `annotations` (`dictionary: {}`) - This value defines additional annotations to add to the HTTPRoute resource. This can either be YAML or a YAML-formatted multi-line templated string.
    ...
    - `activeService` (`boolean: true`) - When HA mode is enabled and K8s service registration is being used, configure the HTTPRoute to point to the Vault active service.

    - `parentRefs` (`array: []`) - **Required when HTTPRoute is enabled.** Configures the parent Gateway(s) that this HTTPRoute should attach to. At least one parent reference must be specified.
    ...
    - `hostnames` (`array: []`) - Configures the hostnames that should be matched for this HTTPRoute.
    ...
    - `matches` (`array`) - Configures the request matching rules for the HTTPRoute. Defaults to matching all requests with a path prefix of `/`.
    ...
    - `filters` (`array: []`) - Configures filters that are applied to requests matching this HTTPRoute. Filters can modify requests or responses.
    ...
    - `additionalRules` (`array: []`) - Configures additional custom routing rules that will be prepended to the default rule. This allows for advanced routing scenarios.
  ...
  - `ha` - This configures `ha` mode for the Vault server.
    ...
    - `raft` - This configures `raft` integrated storage mode for the Vault server.
      ...
      - `redundancyZones` - Autopilot redundancy zone definitions. Requires
        Kubernetes 1.35+ with PodTopologyLabelsAdmission and nodes labeled with
        `topology.kubernetes.io/zone`.

        - `enabled` (`boolean: false`) - Enable automatic zone detection for the
          autopilot redundancy zones. To use automatic zone detection, you must
          set `server.ha.enabled=true`, `server.ha.raft.enabled=true`, and
          `autopilot_redundancy_zone = "VAULT_REDUNDANCY_ZONE"` in the Raft
          configuration.
          
          Enabling automatic zone detection:

          - Exposes the zone label from the node as `VAULT_REDUNDANCY_ZONE`.
          - Vault automatically replaces `VAULT_REDUNDANCY_ZONE` in
            `server.ha.raft.config` with the actual zone value.
          - The PDB `maxUnavailable` defaults to 1 (instead of ⌊(n-1)/2⌋) to
            prevent quorum loss.
...
- `csi` - Values that configure running the Vault CSI Provider.
  ...
  - `image` - Values that configure the Vault CSI Provider Docker image. This image does not change based on [`server.enterpriseLicense`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense).

    - `repository` (`string: "hashicorp/vault-csi-provider"`) - The name of the Docker image for the Vault CSI Provider.

    - `tag` (`string: "1.7.4"`) - The tag of the Docker image for the Vault CSI Provider.. **This should be pinned to a specific version when running in production.** Otherwise, other changes to the chart may inadvertently upgrade your CSI provider.
  ...
  - `agent` - Configures the Vault Agent sidecar for the CSI Provider
    ...
    - `image` - Values that configure the Vault Agent sidecar image for the CSI Provider.
      ...
      - `tag` (`string: ""`) - The tag of the Vault Docker image to use for the Vault Agent Sidecar. When empty, the chart resolves this to the chart's `AppVersion`; configuring an Enterprise license changes this to `AppVersion-ent`.
...
- `serverTelemetry` - Values the configure metrics and telemetry. Enabling these features requires setting
  ...
  - `serviceMonitor` - Values that configure monitoring the Vault server
    ...
    - `matchLabels` (`dictionary: {}`) - Service selector labels for the ServiceMonitor.
