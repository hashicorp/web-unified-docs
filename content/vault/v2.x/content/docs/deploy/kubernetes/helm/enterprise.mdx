...
If you need to use a custom registry or a different enterprise image, you can still set `server.image.repository` explicitly:

```yaml
# config.yaml — explicit image override
server:
  image:
    repository: my-registry.example.com/vault-enterprise
    tag: 2.0.3-ent
  enterpriseLicense:
    secretName: vault-ent-license
```
