...
last_modified: 2026-09-26T18:43:02.000Z
...
When you provide a Vault Enterprise license with [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense), the Helm chart automatically resolves the following images to their Enterprise equivalents, using the chart's `AppVersion` suffixed with `-ent` as the tag:

- **Vault server** ([`server.image`](/vault/docs/platform/k8s/helm/configuration#image-1)) defaults to `hashicorp/vault-enterprise` with tag `AppVersion-ent` instead of `hashicorp/vault`.
- **Injected Vault Agent sidecar** ([`injector.agentImage`](/vault/docs/platform/k8s/helm/configuration#agentimage)) — the Agent image the injector adds to annotated application pods — defaults to `hashicorp/vault-enterprise` with tag `AppVersion-ent` instead of `hashicorp/vault`.
- **CSI provider's Vault Agent sidecar** ([`csi.agent.image`](/vault/docs/platform/k8s/helm/configuration#image-3), used when `csi.agent.enabled` is `true`) — the Agent image the CSI provider uses to authenticate to Vault — defaults to `hashicorp/vault-enterprise` with tag `AppVersion-ent` instead of `hashicorp/vault`.

This automatic resolution does not apply to the Agent Injector's own image (`injector.image`, default `hashicorp/vault-k8s`) or the CSI Provider's own image (`csi.image`, default `hashicorp/vault-csi-provider`). Those images stay the same regardless of license configuration.

You do not need to manually set `server.image.repository` to `hashicorp/vault-enterprise` after configuring a license secret. If you set an explicit `repository` or `tag` for any of the three auto-resolved fields, the chart always uses your value — it only fills in a value when one isn't already provided.
...
Set the name of the secret you just created in [`server.enterpriseLicense`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense). The chart automatically uses the `hashicorp/vault-enterprise` image, so it doesn't require any other overrides:
...
By default, when you leave [`server.image.tag`](/vault/docs/platform/k8s/helm/configuration#image-1) unset, the chart resolves it to the chart's `AppVersion` suffixed with `-ent` (for example, `2.0.4-ent`) due to the configured license. If you need a specific Enterprise release tag instead, set `server.image.tag` explicitly — the chart always uses your value verbatim, without modification:
