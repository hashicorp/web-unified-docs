...
last_modified: 2026-09-26T18:43:07.000Z
...
For Vault Enterprise deployments on Kubernetes 1.35+, you can enable [redundancy zones](/vault/docs/platform/k8s/helm/examples/enterprise-with-raft#redundancy-zones) for improved availability and fault tolerance across availability zones.
...
purposes, the example below uses `vault:123.456` for Vault Community Edition.
...
-> **Enterprise upgrades:** If you are running Vault Enterprise and have [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) configured, the chart automatically targets the `hashicorp/vault-enterprise` image. You only need to set `server.image.tag` to the desired enterprise release tag (for example, `2.0.3-ent`). You do not need to explicitly set `server.image.repository` unless you are using a custom registry.
...
For Vault Enterprise, omit `server.image.repository` so the chart continues to
automatically select the `hashicorp/vault-enterprise` image:

```shell-session
$ helm upgrade vault hashicorp/vault --version=0.34.1 \
    --set='server.image.tag=2.0.3-ent' \
    --dry-run
```
