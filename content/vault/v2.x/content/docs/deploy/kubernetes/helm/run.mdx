...
For Vault Enterprise deployments on Kubernetes 1.35+, you can enable [redundancy zones](/vault/docs/platform/k8s/helm/enterprise#redundancy-zones) for improved availability and fault tolerance across availability zones. See the [Enterprise with Raft example](/vault/docs/platform/k8s/helm/examples/enterprise-with-raft#redundancy-zones) for configuration details.
...
```shell-session
$ helm upgrade vault hashicorp/vault --version=0.33.0 \
