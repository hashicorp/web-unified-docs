...
last_modified: 2026-08-31T05:51:44.000Z
...
-> **Enterprise image auto-selection:** When [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) is set, the chart automatically uses `hashicorp/vault-enterprise` for the Vault server, and selects enterprise-compatible images for the Vault Agent Injector and Vault CSI Provider. You do not need to manually set `server.image.repository` for an enterprise deployment. See [Vault Enterprise license management](/vault/docs/deploy/kubernetes/helm/enterprise) for details.
...
-> **Enterprise upgrades:** If you are running Vault Enterprise and have [`server.enterpriseLicense.secretName`](/vault/docs/platform/k8s/helm/configuration#enterpriselicense) configured, the chart automatically targets the `hashicorp/vault-enterprise` image. You only need to set `server.image.tag` to the desired enterprise release tag (e.g., `2.0.3-ent`). You do not need to explicitly set `server.image.repository` unless you are using a custom registry.
