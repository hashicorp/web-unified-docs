...
1. Create a file named `helm-vault-raft-values.yml` with the following contents:
   ...
   If you are using Prometheus for monitoring and alerting, HashiCorp recommends to set the `cluster_name` in the HCL configuration.  use the `config` parameter in the Vault Helm chart.
