...
1. Create a new Vault cluster. HashiCorp recommends a minimum of three nodes and
   ...
   Autopilot manages the underlying infrastructure on your behalf. You can start with a smaller set of nodes but autopilot provisions more as needed.

Once Google Cloud creates, deploys, and confirms the health of the cluster it modifies your `kubectl` configuration to perform any commands you issue against
the cluster.
