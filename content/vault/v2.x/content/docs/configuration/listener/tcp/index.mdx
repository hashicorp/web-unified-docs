...
last_modified: 2026-07-21T20:46:36.000Z
...
Opaque Vault tokens are fixed-size and well under 8 KB, so the default is sufficient 
for standard deployments. Raise `max_token_header_size` when you use enterprise JWT 
tokens or OIDC tokens that carry large `authorization_details` payloads and token header sizes
...
`max_token_header_size` applies to the API listener only. Vault does not apply
the token header size check to forwarded requests between cluster nodes (HA peers,
performance replication secondaries), so a request accepted by the originating
node is never rejected downstream. No additional configuration is required for
HA or performance replication setups.
...
- `max_token_header_size` `(int: 8192)` – Specifies the maximum allowed size, in
  ...
  `Authorization: Bearer`. The max token size is a defense-in-depth guardrail
  ...
  header size` for requests that exceed the configured limit return. Refer to
  [Authentication token header size](/vault/docs/internals/limits#authentication-token-header-size)
  for guidance on when to change this value.
