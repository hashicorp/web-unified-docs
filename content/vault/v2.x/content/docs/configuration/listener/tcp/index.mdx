...
- `max_token_header_size` `(int: 8192)` – Specifies the maximum allowed size, in
  ...
  | `-1` (or any negative integer) | Removes the 8 KB application-level check. Requests are still bounded by the Go stdlib `MaxHeaderBytes` backstop (~1 MB), which returns `431 (Request Header Fields Too Large)`. |
