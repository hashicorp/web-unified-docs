...
last_modified: 2026-07-20T22:24:02.000Z
...
- `max_token_header_size` `(int: 8192)` – Specifies the maximum allowed size, in
  ...
  `Authorization: Bearer`. The max token size is a defence-in-depth guardrail
  that protects against memory exhaustion from oversized variable length tokens
  with large `authorization_details` claims. Vault returns a `431` status code
  and error message including `authentication token exceeds maximum allowed
  header size` for requests that exceed the configured limit return.

  | Value | Behaviour |
  | ----- | --------- |
  | `0` or unset | Uses the default limit of 8192 bytes (8 KB). |
  | Positive integer | Sets the limit to that many bytes. |
  | `-1` (or any negative integer) | Removes the 8 KB application-level check. Vault still enforces an upper bound on requests based on the Go stdlib `MaxHeaderBytes` backstop (~1 MB) and returns `431 (Request Header Fields Too Large)` for requests that exceed that limit. |

  Requests that exceed the configured limit return a `431` status code, 
  with an error message including `authentication token exceeds maximum allowed header size`.

  **When to raise this value:** opaque Vault tokens are fixed-size and well
  under 8 KB, so the default is sufficient for standard deployments. Raise
  `max_token_header_size` when you use enterprise JWT tokens or OIDC tokens
  that carry large `authorization_details` payloads and token header sizes
  exceed 8 KB. Evaluate your largest token sizes and set the limit accordingly.
  ...
  **Cluster listener behaviour:** the limit applies to the API listener only.
  Forwarded requests between cluster nodes (HA peers, performance replication
  secondaries) skip this check so that a request accepted by the originating
  node is never re-rejected downstream. No additional configuration is required
  for HA or performance replication setups.
