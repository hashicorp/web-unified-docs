...
last_modified: 2026-07-20T19:47:04.000Z
...
- `max_token_header_size` `(int: 8192)` – Specifies the maximum allowed size, in
  ...
  | `-1` | Removes the 8 KB application-level check. Requests are still bounded by the Go stdlib `MaxHeaderBytes` backstop (~1 MB), which returns `431 (Request Header Fields Too Large)`. |
