...
- `vault-version` `(string: "2.0.2")` – Vault version is a string that will not change during a pod's lifecycle.
...
```shell-session
$ helm upgrade vault --set='server.image.tag=2.0.2'
