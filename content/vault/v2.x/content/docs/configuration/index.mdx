...
- `plugin_tmpdir` `(string: "")` - A directory Vault uses for temporary files
  ...
  Has no effect on non-containerized plugins. Set `plugin_tmpdir` to a path accessible to both Vault and
  the plugin container when the default OS temporary directory is not shared between them. 
