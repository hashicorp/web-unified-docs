...
last_modified: 2026-07-17T20:20:04.000Z
...
- `plugin_tmpdir` `(string: "")` - A directory Vault uses for temporary files
  when starting [containerized external plugins](/vault/docs/plugins/containerized-plugins).
  Vault does not use `plugin_tmpdir` with non containerized plugins.
  Set `plugin_tmpdir` to a path accessible to both Vault and the plugin container
  when the default OS temporary directory is not shared between them. 
  Set `VAULT_PLUGIN_TMPDIR` to configure the directory with an environment variable.
...
- `detect_deadlocks` `(string: "")` - A comma separated string indicating which
  internal mutex locks Vault should monitor for potential deadlocks. Vault logs
  a `POTENTIAL DEADLOCK:` warning for the configured value whenever an attempt
  at a core state lock appears to be deadlocked. Enabling `detect_deadlocks`
  can negatively impact performance due to the tracking of each lock attempt.
...
- `ui_settings` `([UISettings][ui-settings]: <none>)` – Configures options for the
  Vault web UI, such as UI telemetry.
...
- `disable_goroutine_trace_dump` `(bool: false)` - When set to `true`, suppresses
  ...
  The dump writes a file named `goroutine` that includes a `pprof` goroutine
  profile readable by `go tool pprof` to a randomly named directory inside
  [`os.TempDir()`](https://pkg.go.dev/os#TempDir). For example, on Linux the 
  dump writes to `/tmp/vault-goroutine-dump-2738451092`. If you want to redirect
  the `goroutine` file to a different location, set `VAULT_STACKTRACE_FILE_PATH`
  to an existing directory path.

  <Note>

  You can trigger a separate goroutine dump on demand by sending SIGUSR2 to
  the Vault process if you set the `VAULT_STACKTRACE_WRITE_TO_FILE` environment
  to any non-empty value before Vault starts. You cannot use the
  `disable_goroutine_trace_dump` configuration parameter to affect the SIGUSR2
  dump.
...
- `deny_slash_in_templated_paths` `(bool: false)` - Controls whether Vault returns
  "permission denied" when
  [templated policy paths](/vault/docs/concepts/policies#templated-policies).
  include slashes (`/`). You can also set the environment variable,
  `VAULT_DENY_SLASH_IN_TEMPLATED_PATHS`.
...
[ui-settings]: /vault/docs/configuration/ui
