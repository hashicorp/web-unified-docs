...
created_at: 2026-04-15T00:14:59.000Z
last_modified: 2026-04-15T00:14:59.000Z
...
- `shared_creds_filename` `(string: "")`: Relative path to a shared credentials
  file.

- `shared_creds_profile` `(string: "default")`: Profile to use from the shared
  credentials file. Leave `shared_creds_profile` unset to use the `AWS_PROFILE`
  environment variable. Leave both unset to use the default.

- `role_arn` `(string: "")`: The role ARN for a web identity role provider with
  STS. You must set `web_identity_token_file` to use `role_arn`.

- `web_identity_token_file` `(string: "")`: Token file for a web identity role
  provider with STS. You must set `role_arn` to use `web_identity_token_file`.

- `role_session_name` `(string: "")`: The role session to use for the web
