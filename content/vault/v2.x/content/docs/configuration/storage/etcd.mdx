...
created_at: 2026-04-15T00:15:03.000Z
last_modified: 2026-04-15T00:15:03.000Z
...
- `dial_keepalive_time` `(string: "10s")` – Specifies how long a connection to
  an etcd member may be idle before sending a gRPC keepalive ping. The
  connection will be closed if the member doesn't respond in
  `dial_keepalive_timeout`. Must not be below etcd's
  `--grpc-keepalive-min-time`. Set to `"0"` to disable keepalive.

- `dial_keepalive_timeout` `(string: "3s")` – Specifies how long Vault will
  wait for a response from the keepalive ping before closing the connection.
  Only applies when `dial_keepalive_time` is non-zero. Sum of
  `dial_keepalive_timeout` and `dial_keepalive_time` should stay below
  `lock_timeout` to prevent active Vault node from losing leadersip during the
  window that is needed to evict unresponsive etcd node. On linux this value is
  also applied as connection's `TCP_USER_TIMEOUT`.
