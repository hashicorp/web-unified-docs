...
model is to provide [confidentiality, integrity, availability, accountability, and authentication](https://en.wikipedia.org/wiki/Information_security).
...
- Access to data or controls without authentication or authorization. All requests
  must be preceded by the applicable security policies.
...
Vault architecture comprises of three distinct systems:
...
concrete or brute force the encryption keys, it would be prohibitively time-consuming. Opening the bank vault requires two-factors: the key and the
combination. Similarly, Vault requires multiple shares be provided to
reconstruct the root key. Once unsealed, each security deposit box still
