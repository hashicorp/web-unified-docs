...
last_modified: 2026-07-20T19:47:06.000Z
...
### Authentication token header size

The maximum size of the `X-Vault-Token` or `Authorization: Bearer` header is
controlled by the
[`max_token_header_size`](/vault/docs/configuration/listener/tcp#max_token_header_size)
option in the listener stanza. The default is 8 KB (8192 bytes). This limit is a
defence-in-depth guardrail for variable length enterprise tokens with `authorization_details` claims. 
Requests that exceed the limit receive HTTP `431` before token validation occurs.
