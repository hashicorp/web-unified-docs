...
The `vault.wal.write_controller.reject_fraction` metric is a gauge value between 0 and 1 that represents the estimated fraction of write requests being rejected to maintain cluster stability. Monitor this metric to detect capacity issues. Sustained non-zero values indicate write load pressure and may require investigating your workload patterns, checking storage backend performance, or scaling your infrastructure. For more information, refer to the [Adaptive overload protection](/vault/docs/concepts/adaptive-overload-protection) documentation.
