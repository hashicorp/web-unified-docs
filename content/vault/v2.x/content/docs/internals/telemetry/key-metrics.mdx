...
created_at: 2026-04-15T00:15:27.000Z
last_modified: 2026-04-15T00:15:27.000Z
...
The lifecycle of leases is managed by the expiration manager, which handles the revocation of a lease when the time to live (TTL) value associated with the lease is reached. Irrevocable leases can occur due to backend misconfiguration, network connectivity issues, or persistent failures in the target system, among other causes. These leases are tracked by the vault.expire.num_irrevocable_leases metric. It is important to monitor this metric, as a rising count may indicate operational or security risks that require investigation. Refer to the [Troubleshoot irrevocable leases tutorial](/vault/tutorials/monitoring/troubleshoot-irrevocable-leases) for guidance on identifying and resolving irrevocable leases.
...
### Monitor write overload protection
...
The adaptive write overload protection feature helps maintain cluster stability
during periods of high write load. When write requests arrive faster than the
storage backend can process them, write-ahead logs (WALs) can accumulate rapidly,
potentially overwhelm the storage backend, and degrade overall cluster
performance. The write controller monitors WAL buildup and automatically rejects
a fraction of incoming write requests when necessary to prevent overload conditions.

#### Alerting:

The `vault.wal.write_controller.reject_fraction` metric is a gauge value between
0 and 1 that represents the estimated fraction of write requests being rejected
to maintain cluster stability.

We recommend monitoring the reject fraction metric to detect capacity issues as
sustained non-zero values indicate write load pressure. You may need to
investigate your workload patterns, check storage backend performance, or scale
your infrastructure if you continue to address the load pressure. For more
information, refer to the
[Adaptive overload protection](/vault/docs/concepts/adaptive-overload-protection) documentation.
