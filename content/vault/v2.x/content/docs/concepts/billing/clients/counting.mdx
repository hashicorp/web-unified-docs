...
In Vault agentic workflows, each agent is counted as a standard entity client on its 
**first successful API action**. In a delegated workflow where an agent 
acts on behalf of a subject, both the agent and the subject are recorded 
as separate entity clients. 
