...
last_modified: 2026-07-01T19:21:26.000Z
...
- **Enable audit device logs**. Vault supports several [audit
  ...
  unintended information disclosure. Consider disabling hashing of certain
  fields such as token accessors for quicker log correlation.

- **Log file management**. Due to volume of logs Vault generates, consider:
  1. Rotating and compressing the logs to disk
  1. Sending the logs to central location or service.
...
- **Least privilege and bias towards simple policies**. We recommend enforcing
  the principle of least privilege when creating policies. Limit the use of
  globs, wildcards, and templates. Instead, we recommend creating policies that
  are as simple and as explicit as possible.
...
- **Consider using a separate partition for integrated storage**. Using a
  separate patrition ensures graceful degradation in the event Vault consumes
  all the available storage.

- **Use an administrative namespace for day-to-day admin operations**.
  [Administrative namespaces](/vault/docs/namespaces/create-admin-namespace) help
  enforce least-privilege for administration operation. You should use the `root`
  namespace unless only when strictly necessary. For example, if a given API
  endpoint requires it.

- **Authenticated reverse proxy**. Running older versions of Vault can leave you
  susceptible to known vulnerabilities. If you do not upgrade regularly, we
  recommend using an authenticated reverse proxy to provide defense-in-depth.

- **Use an outbound network proxy**. You can configure Vault to access a wide
  range of network services and endpoints. In addition to network segmentation,
  we recommend using an outbound proxy to further restrict and observe Vault
  traffic.

- **Set `deny_slash_in_templated_paths`**. To prevent unintended access, we
  recommend setting
  [`deny_slash_in_templated_paths`](/vault/docs/configuration#deny_slash_in_templated_paths)
  to `true` in your Vault configuration. Denying slashes ensures that Vault
  returns a permission denied response when templated policy paths include
  slashes, preventing unintended sub-path access.
