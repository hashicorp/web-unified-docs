...
created_at: 2026-04-15T00:14:52.000Z
last_modified: 2026-04-15T00:14:52.000Z
...
## Eventual consistency with performance standby nodes

@include 'alerts/enterprise-and-hcp.mdx'

Without performance standby nodes, Integrated Storage is strongly consistent.
Regular standby nodes forward all requests to the active node, so you do not
need to take any additional steps to preserve read-after-write consistency
after migration.

If you use performance standby nodes, the consistency model changes after
migration. Performance standbys serve read-only requests locally from their own
copy of the Raft log. Because writes are applied on the active node and shipped
to other nodes asynchronously, a performance standby can return stale data in
the window before replication reaches it. Read-after-write consistency is not
guaranteed — even across requests to the same standby. This behavior does not
exist with Consul storage and is introduced specifically by Integrated Storage
with performance standbys.

Before sending production traffic to the migrated cluster, review the available
mitigations:

- **`X-Vault-Forward: active-node`** — unconditionally forwards a request to
  the active node, guaranteeing a fresh read. Use selectively, because routing
  all requests to the active node reduces the value of performance standbys.
- **`X-Vault-Index` with `X-Vault-Inconsistent: forward-active-node`** —
  conditionally forwards a request to the active node only when the standby
  does not yet have the required state.
- **`X-Vault-Index` alone** — causes the node to return HTTP 412 when the
  required state is not present. The client retries the request. The Vault Go
  SDK retries 412 responses automatically. This approach also works across
  performance replication clusters.
- **Vault Proxy `enforce_consistency` and `when_inconsistent`** — configure
  consistency handling transparently in the Proxy `api_proxy` stanza, so
  individual clients do not need modification.

Refer to [Eventual consistency in Vault
replication](/vault/docs/enterprise/consistency) for a full explanation of
each mitigation and the header-forwarding configuration required to enable them.
