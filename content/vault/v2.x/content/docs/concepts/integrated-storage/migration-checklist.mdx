...
Regular standby nodes forward all requests to the active node, so every read
reflects the latest committed state — the same behavior as using Consul with its
`consistent` mode enforced.

If you plan to use **performance standby nodes** (Vault Enterprise), the
consistency model changes. Performance standbys serve read-only requests
locally from their own copy of the Raft log. Writes go to the active node,
which then ships updates to other nodes asynchronously. In the window between
a write landing on the active node and that update reaching a standby, the
standby presents a stale view of the data. This means read-after-write
consistency is not guaranteed — even across requests to the same performance
standby.

This consistency concern does not exist with Consul storage. It is introduced
specifically by using Integrated Storage with performance standby nodes.

Vault provides several mitigations, including request forwarding headers,
automatic 412 retry behavior in the Go SDK, and Vault Proxy consistency
configuration. Refer to [Eventual consistency in Vault
replication](/vault/docs/enterprise/consistency) for a full explanation of the
consistency model and available mitigations.
...
| Consistency model   | Eventual consistency possible if Consul agents are not using consistent reads | Strongly consistent without performance standbys; eventual consistency possible with performance standbys (**Vault Enterprise**) |

## Self-check questions

- [ ] Do you plan to use performance standby nodes (Vault Enterprise)?
- [ ] Do any clients or automation assume read-after-write consistency on Vault reads?
