...
Vault provides mitigations for this behavior. Refer to [Eventual consistency in
Vault replication](/vault/docs/enterprise/consistency) for details.

### Inspect Vault data

Inspection of Vault data differs considerably from the `consul kv` commands used
to inspect Consul's KV store.
Refer to the [Inspect Data in Integrated Storage](/vault/tutorials/monitoring/inspect-data-integrated-storage)
