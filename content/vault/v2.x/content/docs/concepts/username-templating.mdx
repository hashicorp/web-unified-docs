...
created_at: 2026-04-15T00:14:55.000Z
last_modified: 2026-04-15T00:14:55.000Z
...
-> `DisplayName` is sanitized to `[a-zA-Z0-9_-]` — any character outside that set (including `@`, `.`, spaces, and `'`) 
is replaced with `-`. This means an OIDC identity like `alice@corp.com` enters the template as `alice-corp-com`.
