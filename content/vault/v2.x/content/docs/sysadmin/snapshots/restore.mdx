...
created_at: 2026-04-15T00:15:56.000Z
last_modified: 2026-04-15T00:15:56.000Z
...
network environment. Isolation is critical to prevent both unwanted
...
@include '../../../global/partials/upgrade/isolate-vault.mdx'
...
**External plugin artifacts are not included in Raft snapshots.** When you
restore a snapshot, catalog entries for external plugins are restored, but
the plugin binary or `.zip` file in `plugin_directory` is not. If the
required artifact is absent from `plugin_directory` on the restored node,
Vault's plugin verification will prevent the node from becoming active.

Before restoring a snapshot, copy the correct version of every external
plugin artifact to `plugin_directory` on every target node.
...
@include 'sysadmin/snapshots/snapshot-restore-behavior.mdx'
