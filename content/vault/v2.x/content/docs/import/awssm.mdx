...
<Warning title="Only allow trusted private ranges">
    ...
    the import is in the same or a peered VPC as the AWS PrivateLink endpoint.
    Use the narrowest possible CIDR range, such as the endpoint's private IP
    address with a `/32` prefix, rather than an entire RFC 1918 range.
</Warning>
