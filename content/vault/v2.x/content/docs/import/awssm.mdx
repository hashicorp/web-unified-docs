...
last_modified: 2026-09-08T07:19:59.000Z
...
- `allowed_ipv4_cidrs` `([]string: [])` - Optional. Private IPv4 CIDR ranges,
such as `10.0.0.0/8`, to allow in addition to public addresses. By default,
Vault blocks RFC 1918 private addresses to help prevent server-side request
forgery (SSRF). You should only set CIDR ranges when your AWS Secrets Manager
endpoint requires private connectivity, such as through an AWS PrivateLink
VPC endpoint.

<Warning title="Only allow trusted private ranges">
    Setting `allowed_ipv4_cidrs` bypasses SSRF private-address blocking for the
    specified ranges. You should only use CIDR ranges when the AWS PrivateLink
    endpoint and the importing Vault node live in the same, or a peered, VPC. 
    Always use the narrowest possible CIDR range. We recommend using the private
    endpoint IP with a `/32` prefix rather than an entire RFC 1918 range.
</Warning>

For example, to allow a source to reach an AWS Secrets Manager PrivateLink
VPC endpoint use the following importer configuration:

```hcl
source_aws {
  name                = "my-aws-source-1"
  credentials_profile = "my-cred-profile-name"
  allowed_ipv4_cidrs  = ["10.0.5.10/32"] # PrivateLink VPC endpoint address
}
```