...
last_modified: 2026-09-08T07:20:00.000Z
...
- `credentials_file` `(string: "")` - Path to a file on your Azure credential
  ...
  credentials.
- `allowed_ipv4_cidrs` `([]string: [])` - Private IPv4 CIDR ranges, such as
`10.0.0.0/8`, to allow in addition to public addresses. By default, Vault
blocks RFC 1918 private addresses to help prevent server-side request
forgery (SSRF). You should only set CIDR ranges when your Azure Key Vault
endpoint requires private connectivity, such as through an Azure Private Link
endpoint.

<Warning title="Only allow trusted private ranges">
    Setting `allowed_ipv4_cidrs` bypasses SSRF private-address blocking for the
    specified ranges. You should only use CIDR ranges when the Azure Private Link
    endpoint and the importing Vault node live in the same, or a peered, virtual
    network.  Always use the narrowest possible CIDR range. We recommend using
    the private endpoint IP with a `/32` prefix rather than an entire RFC 1918
    range.
</Warning>

For example, to allow a source to reach an Azure Key Vault Private Link
endpoint, use the following import configuration:
...
source_azure {
  ...
  client_id          = "<your client id>"
  credentials_file   = "/path/to/client-secret"
  allowed_ipv4_cidrs = ["10.0.5.10/32"] # Private Link endpoint address
}
```