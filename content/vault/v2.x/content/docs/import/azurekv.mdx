...
<Warning title="Only allow trusted private ranges">
    ...
    specified ranges. Only use this parameter when the Vault node performing
    the import is in the same or a peered virtual network as the Azure Private
    Link endpoint. Use the narrowest possible CIDR range, such as the endpoint's
    private IP address with a `/32` prefix, rather than an entire RFC 1918 range.
</Warning>
