...
created_at: 2026-04-15T00:14:45.000Z
last_modified: 2026-04-15T00:14:45.000Z
...
- `VAULT_STACKTRACE_WRITE_TO_FILE` `(non-empty string)` - (environment variable)
  ...
  named `goroutine` in addition to logging goroutine stacks to the log. The
  `VAULT_STACKTRACE_WRITE_TO_FILE` value has no effect on the automatic shutdown
  dump. You must set `VAULT_STACKTRACE_WRITE_TO_FILE` before Vault starts and you
  cannot reload it when Vault is running.

- `VAULT_STACKTRACE_FILE_PATH` `(directory path)` - (environment variable)
  Overrides the output directory for goroutine dump files. The stacktrace path
  applies to both the SIGUSR2 dump (when `VAULT_STACKTRACE_WRITE_TO_FILE` is set)
  and the automatic shutdown dump. The path must point to an existing directory.
  If the path is invalid or the directory does not exist, Vault logs a warning
  and skips the dump. When unset, Vault creates a randomly named temporary
  ...
  allocs, threadcreate, and CPU profile) to `$TMPDIR/vault-pprof/`. Vault does
  not use `VAULT_STACKTRACE_FILE_PATH` or `VAULT_STACKTRACE_WRITE_TO_FILE` to
  determine where to write the bundle data.
