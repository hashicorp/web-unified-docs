...
description: |-
  ...
  parent token's direct policies unless you explicitly assign policies.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-04-14T17:13:42-07:00
last_modified: 2026-06-10T15:59:22.000Z
...
the currently authenticated token, the child token inherits the parent token's
direct policies unless you explicitly assign policies with `-policy`.

In Vault Enterprise, when Vault creates the child token in a different namespace,
the child token does not automatically inherit direct policies from the parent
token's namespace. Vault assigns explicitly requested policies for the target
namespace, plus `default` when applicable.
