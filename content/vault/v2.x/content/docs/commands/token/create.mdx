...
description: |-
  ...
  authentication. Vault creates this token as a child of the currently
  authenticated token. In the same namespace, the generated token inherits the
  parent token's policies and permissions unless you explicitly assign policies.
# START AUTO GENERATED METADATA, DO NOT EDIT
created_at: 2026-04-14T17:13:42-07:00
last_modified: 2026-06-10T21:14:46.000Z
...
The `token create` command creates a new token that can be used for
authentication. Vault creates this token as a child of the currently
authenticated token. When Vault creates the child token in the same namespace as
the currently authenticated token, the child token inherits all of the parent
token's policies and permissions unless you explicitly assign policies with `-policy`.

In Vault Enterprise, when Vault creates the child token in a different namespace,
the child token does not automatically inherit all policies and permissions from
the parent token's namespace. Vault assigns explicitly requested policies for the
target namespace, plus `default` when applicable.
