...
description: |-
  ...
  authentication as a child of the currently authenticated token.
...
The `token create` command creates a new token that can be used for
authentication. Vault creates the new token as a child of the currently
authenticated token. When Vault creates the child token in the same namespace as
the currently authenticated token, the child token inherits all policies and
permissions associated with the parent token unless you explicitly assign
different policies with `-policy`.

When you create a child token in a different namespace, Vault does not
automatically assign all policies and permissions from the parent token. Instead,
Vault assigns explicitly requested policies for the target namespace and
