...
To authenticate using a TPM-bound certificate:

```shell-session
$ vault login -method=tpm -tpm-state-dir=/etc/vault/tpm role_name=my-role
...
- `-tpm-state-dir` `(string: ".")` - Directory containing the TPM certificate
  and key blobs written by `vault tpm attest`. Defaults to the current working
  directory.

- `-tpm-device-path` `(string: "")` - Path to the TPM device (e.g. `/dev/tpm0`
  or a Unix socket path). Auto-detected when omitted.
