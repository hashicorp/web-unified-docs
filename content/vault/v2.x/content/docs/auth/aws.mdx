...
AWS can enforce IMDSv2 at the account or individual instance level, disabling IMDSv1
access entirely. The Vault AWS SDK automatically uses IMDSv2 when it is available. The
PKCS#7 signature fetch commands in the [Authentication](#authentication) section below
use the IMDSv2 token flow and work in all environments, including those where IMDSv1 is
disabled. For more information, refer to the
...
This two-step approach works on all EC2 instances and is required when IMDSv1 access is disabled.
...
curl -X POST "http://127.0.0.1:8200/v1/auth/aws/login" \
  -d '{"role":"dev-role","pkcs7":"'"$SIGNATURE"'","nonce":"5defbf9e-a8f9-3063-bdfc-54b7a42a1f95"}'
