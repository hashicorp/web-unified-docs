...
1. (Optional for public clients) Under **Certificates & secrets**,
   [add a client secret](https://docs.microsoft.com/en-us/azure/active-directory/develop/quickstart-register-app#add-a-client-secret)
   Record the secret's value as you will need it as the `oidc_client_secret` for Vault.
   For public clients (mobile apps, SPAs, desktop applications), you can skip the
   secret lookup and use PKCE instead.

1. Under **Token configurations**, add a groups claim. For "Customize token
   properties by type" in the claim, one of the selected options must be **ID**.

1. Confirm the permissions under **API permissions** include permission to read
   groups and users.
...
1. Configure the OIDC auth method with the `oidc_client_id` (application ID) and
   `oidc_discovery_url` (endpoint URL) you recorded from Azure. Include `oidc_client_secret`
   (client secret) for confidential clients, or omit it for public clients using PKCE:
...
When Vault runs on Kubernetes with
[Azure Workload Identity](https://azure.github.io/azure-workload-identity/) enabled,
you can authenticate the OIDC client to Microsoft Entra ID with a federated token
instead of a static `oidc_client_secret`. Vault presents the projected Kubernetes
service account token as a client assertion (`private_key_jwt`) during the
authorization code exchange. This is opt-in; existing `oidc_client_secret`
configurations are unaffected.

1. Enable Azure Workload Identity on your cluster and add a
   [federated credential](https://learn.microsoft.com/en-us/entra/workload-id/workload-identity-federation-create-trust)
   to the Vault application registration. Use the **Kubernetes accessing Azure
   resources** scenario with:

   - **Cluster issuer URL**: the OIDC issuer URL of your Kubernetes cluster.
   - **Namespace**: the namespace where Vault runs.
   - **Service account**: the name of Vault's service account.
   - The federated credential audience should be `api://AzureADTokenExchange`.

1. Label the Vault pods and annotate the Vault service account so the Azure
   Workload Identity webhook projects the token and sets the
   `AZURE_FEDERATED_TOKEN_FILE` environment variable in the Vault container.
   ...
   With the official [Vault Helm chart](https://github.com/hashicorp/vault-helm),
   set the pod label via `server.extraLabels` and the service account annotation
   via `server.serviceAccount.annotations`. Note that Azure Workload Identity
   requires the `azure.workload.identity/use` label on the **pod** (unlike AWS IRSA
   or GCP Workload Identity, which need only a service account annotation).

1. Configure the OIDC auth method with `use_workload_identity` set to `true` in
   `provider_config`, and omit `oidc_client_secret`:
   ...
   When `use_workload_identity` is `true`, `oidc_client_id` is required and
   `oidc_client_secret` must not be set.
