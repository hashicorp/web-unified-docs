...
If you are using the Azure provider with [`fetch_groups`](/vault/api-docs/auth/jwt#fetch_groups) enabled to fetch group memberships directly from the Microsoft Graph API instead of relying on the groups claim in the JWT, include the `distributed_claim_access_token` parameter:
