...
`pem_keys` only replaces local signature verification; it does not skip
TokenReview.

`kubernetes_host` must accept Kubernetes Service Account tokens. Some
hosted control-plane endpoints authenticate callers with cloud IAM
instead. GKE's DNS-based endpoint is one example: when Kubernetes tokens
via DNS are disabled, TokenReview returns HTTP 401 or 403 and every
login fails with permission denied.

In that topology, use [JWT auth][k8s-jwt-auth] with a publicly reachable
OIDC issuer. For GKE, that is the cluster's OIDC issuer on
`container.googleapis.com`, not the API server's `/.well-known` document,
whose `jwks_uri` is often a private address unreachable from Vault.
