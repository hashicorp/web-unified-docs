...
created_at: 2026-04-15T00:14:27.000Z
last_modified: 2026-04-15T00:14:27.000Z
...
different path, use that value instead of `kubernetes`.
...
<Warning title="kubernetes_host must accept Service Account tokens">

Kubernetes auth calls TokenReview on every login. Omitting
`token_reviewer_jwt` uses the login JWT as the TokenReview bearer.
Setting `pem_keys` does not skip TokenReview during authentication. Kubernetes
only uses the PEM keys for local signature verification.

</Warning>

Some hosted control-plane endpoints authenticate callers with cloud IAM.
That TokenReview call can fail when Kubernetes tokens through DNS are disabled,
because Vault cannot present a token the endpoint accepts. For example, the
DNS-based endpoint for GKE returns HTTP 401 or 403 and every login fails with
permission denied.

For cloud IAM configurations, use [JWT auth][k8s-jwt-auth] with a publicly
reachable OIDC issuer. For GKE, use the cluster OIDC issuer on
`container.googleapis.com` instead of the `/.well-known` document from the API
server. Vault cannot reach the private address set for `jwks_uri`.
...
You can manage Kubernetes auth resources programmatically with the Vault Terraform provider. Refer to the Terraform Registry documentation for more details:

- [Kubernetes auth method config resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/kubernetes_auth_backend_config)
- [Kubernetes auth method role resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/kubernetes_auth_backend_role)
