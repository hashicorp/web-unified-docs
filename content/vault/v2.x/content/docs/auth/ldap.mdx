...
- `schema` - (string, optional) - Defines the LDAP schema used to store entry passwords. Supported values are `openldap` and `ad`. The `schema` parameter is optional for OpenLDAP, but required for Active Directory.
...
- `rotation_url` `(string: "")` – The LDAP server to perform a root config password
...
You can manage LDAP auth resources programmatically with the Vault Terraform provider. Refer to the Terraform Registry documentation for more details:

- [LDAP auth method backend resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/ldap_auth_backend)
- [LDAP auth method group resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/ldap_auth_backend_group)
- [LDAP auth method user resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/ldap_auth_backend_user)
