...
Each [SPIFFE auth method instance](/vault/docs/auth/spiffe) has a single trust
domain and an associated trust bundle. Role-level configuration maps
...
- The `aud` claim embedded in the JWT-SVID when the workload requests a SVID from a
  SPIFFE-compliant service, such as SPIRE.

- Vault's `audience` allow-list on `auth/spiffe/config`.
...
1. Configure the SPIFFE auth method. Replace the example parameters with the
   values for your environment. `endpoint_spiffe_id` must match the SPIFFE ID
   your federation endpoint actually presents — confirm the real value for your
   deployment rather than reusing the example values in this step.
...
refreshes on the trust bundle on behalf of all nodes in the cluster. Active
nodes on Performance replica clusters fetch and refresh the trust bundle
independently.

To force an immediate refresh, call the
[configuration endpoint](/vault/api-docs/auth/spiffe#create-configuration)
with an empty payload. To view details about a fetched trust bundle, including
the last refresh time and calculated refresh interval, call the
...
following four conditions must all be true:
...
1. The frontend performs full TLS verification of the client certificate —
   an unverified certificate must never pass through.
1. The connection between the frontend and Vault is secured with mutual TLS.
...
The SPIFFE auth method has a full HTTP API. Refer to the
[SPIFFE auth method API reference](/vault/api-docs/auth/spiffe) for endpoint
details and request parameters.
...
Refer to [Authenticate to Vault using the SPIFFE auth method with
