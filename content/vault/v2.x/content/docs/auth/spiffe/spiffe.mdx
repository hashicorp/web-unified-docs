...
1. Configure the SPIFFE auth method. Replace the example parameters with the
   ...
   #### need help validating from here
   ...
   To validate JWT-SVIDs, configure a JWKS-format bundle by using the `static`
   profile with a JWKS `bundle`, or the `https_web_bundle` /
   `https_spiffe_bundle` profiles. 
   
   `https_web_pem` fetches a PEM certificate and does not support JWT-SVIDs.
   
   `audience` is mount-wide, not per-role. Every role on this mount shares
   this same JWT audience allow-list.
