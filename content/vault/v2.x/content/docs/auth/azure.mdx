...
`auth_type` empty to uses the existing, backward-compatible credential
discovery logic, which checks the following places, in order: `client_secret`,
`identity_token_audience`, `AZURE_FEDERATED_TOKEN_FILE` (environment variable),
Managed Identity. 
...
1. Configure the Azure auth method:
   ...
   - [Plugin Workload Identity Federation](#plugin-workload-identity-federation-wif) (WIF),
     ...
     Azure tenent to trust using workload identity federation.
   - [AKS Workload Identity](#aks-workload-identity-federation-aks-wi), where Vault uses the Kubernetes-projected service account token on the AKS cluster it runs on.
