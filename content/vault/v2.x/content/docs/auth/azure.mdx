...
1. Create or identify a Microsoft Entra App Registration for Vault. Use
   ...
   - `appId`: The application's **Application (client) ID**. Use this value for
     `<app_client_id>` in the service account annotation, Vault configuration,
     and cross-tenant admin consent URL.
   - `id`: The app registration's **Object ID**. Use this value for
     `<app_registration_object_id>` when creating the federated identity credential.
     This is different from the Application (client) ID and a service principal's
     Object ID.
   ...
   **Object ID** on its **Overview** page. For cross-tenant deployments, look up
   these values in tenant A. The Application (client) ID remains the same when
...
Resource Manager APIs in Tenant B with that token produces an
...
Before starting the tenant B steps, configure the federated identity credential
on the App Registration in tenant A using the
[`az ad app federated-credential create` command in the prerequisites](#prerequisites-1).
Run that command while signed in to tenant A. Use the app registration's Object
...
1. Grant the Azure **Reader** role to the application's service principal in
   **tenant B**, scoped to the resource group containing the workloads. This lets
   Vault read Azure resource metadata when validating workload identities.
   Admin consent does not grant this Azure resource access. Use an account with
   permission to create role assignments at this scope:
