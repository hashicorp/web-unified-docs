...
| `aks_wif` | Uses [AKS Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview) federation. Vault exchanges the Kubernetes-projected service account token for a short-lived Microsoft Entra access token. Supports both single-tenant and cross-tenant deployments. |

Leave `auth_type` empty to uses the existing,
backward-compatible credential discovery logic, which checks the following
places, in order: `client_secret`, `identity_token_audience`,
`AZURE_FEDERATED_TOKEN_FILE` (environment variable), Managed Identity.
Set `auth_type` explicitly to skip discovery and pin the mount to one credential mechanism,
which makes the configuration easier to audit and troubleshoot.
...
1. Configure the Azure auth method:
   ...
   - [Plugin Workload Identity Federation](#plugin-workload-identity-federation-wif) (WIF),
     where Vault issues its own OIDC identity token that you configure your
     Azure tenant to trust using workload identity federation.
   - [AKS Workload Identity Federation](#aks-workload-identity-federation-aks-wif), where Vault uses the Kubernetes-projected service account token on the AKS cluster it runs on.
...
1. Create a role:
   ...
   Each role must specify either `bound_service_principal_ids` or `bound_group_ids` to restrict which Azure identities (service principals or group members) can authenticate to the role.
   The following example uses `bound_group_ids` instead:
...
## AKS Workload Identity Federation (AKS WIF)

The Azure auth method supports
[AKS Workload Identity](https://learn.microsoft.com/en-us/azure/aks/workload-identity-overview)
as a credential mechanism for Vault to authenticate to Azure APIs. When `auth_type=aks_wif`
is configured, AKS injects a projected Kubernetes service account token into the Vault pod.
Vault presents the projected Kubernetes service account token to Microsoft Entra, which validates it against a federated
identity credential and returns a short-lived access token scoped to the configured
as a credential mechanism for Vault to authenticate to Azure APIs. When you 
configure AKS WIF, AKS injects a projected Kubernetes service account token into
the Vault pod.

Vault presents the Kubernetes token to Microsoft Entra, which validates it
against a federated identity credential and returns a short-lived access token
scoped to the configured tenant ID.
...
Using the `aks_wif` authentication type also resolves the
`InvalidAuthenticationTokenTenant` error that occurs in cross-tenant deployments
using `msi`.

Setting the workload identity credential pins the tenant ID and client
ID from the Azure plugin configuration, which scopes the issued access token to
the target workload tenant, not the home tenant of your Vault deployment.
...
Complete the following steps before setting `auth_type=aks_wif` in Vault:
...
1. Retrieve the AKS OIDC issuer URL. You need the issuer URL to create the federated
   identity credential later:
...
1. Create or identify a Microsoft Entra App Registration for Vault. Use
   ...
   cross-tenant deployments. For cross-tenant deployments, create the app
   registration in the tenant hosting Vault (tenant A):

   <Tabs>
   <Tab heading="Single-tenant">
   ...
   Record the following values from the `az ad app create` output for use in the
   remaining steps:

   - `appId`: The application's **Application (client) ID**. Use the Application (client) ID for
     `<app_client_id>` in the service account annotation, Vault configuration,
     and cross-tenant admin consent URL.
   - `id`: The app registration's **Object ID**. Use the app registration Object ID for
     `<app_registration_object_id>` when creating the federated identity credential.
     The app registration Object ID is different from the Application (client) ID and a service principal's
     Object ID.

   For an existing app registration, open **Microsoft Entra ID > App registrations**
   in the Azure portal, select the app, and find **Application (client) ID** and
   **Object ID** on its **Overview** page. For cross-tenant deployments, look up
   the Application (client) ID and Object ID in tenant A. The Application (client) ID remains the same when
   configuring Vault to authenticate to tenant B.

1. Configure a federated identity credential on the App Registration to trust
   projected service account tokens from the Vault pod using the OIDC issuer URL
   ...
   $ az ad app federated-credential create \
       ...
       --parameters '{
         ...
         "subject": "system:serviceaccount:<vault_namespace_name>:<vault_service_account_name>",
         "audiences": ["api://AzureADTokenExchange"]
       }'
   ```

   You only need to configure the WIF credential once in Azure. Microsoft Entra
   uses the credential to validate the `iss` claim on the projected service
   account token at every login.  Vault never reads or sends the credential
   directly.

1. Annotate the Vault Kubernetes service account with the Azure client ID:

   ```shell-session
   $ kubectl annotate serviceaccount <vault_service_account_name> \
       -n <vault_namespace_name> \
...
If you have Vault and the workloads it authenticates in the same Microsoft Entra tenant, configuring WIF is straightforward:

1. Enable the Azure auth method if it is not already enabled:

   ```shell-session
   $ vault auth enable azure
   ```

1. Configure the Azure auth method to use AKS WIF with the tenant ID shared by
   Vault and the workloads it authenticates:
...
1. Create a Vault Azure auth role that defines which workload identities can
   authenticate and attaches the `dev` policy to the Vault tokens issued on
   successful login:
   ...
   $ vault write auth/azure/role/dev-role \
       ...
       bound_service_principal_ids=<service_principal_object_id>
   ```

### Cross-tenant configuration

When you have Vault running in one Microsoft Entra tenant (Tenant A) and
the workloads it authenticates running in a separate tenant (Tenant B), you
must provide cross-tenant permissions as part of the configuration.

The `msi` fallback issues tokens scoped to the Vault tenant (Tenant A). Calling Azure
Resource Manager APIs in Tenant B with a token scoped to Tenant A produces an
`InvalidAuthenticationTokenTenant` error. With `aks_wif`, `WorkloadIdentityCredential`
pins the `tenant_id` and `client_id` from Vault configuration, so Entra issues a token
scoped to Tenant B instead.

Before starting the tenant B steps, sign in to tenant A and follow the
[federated identity credential setup instructions](#prerequisites-1) to configure
the credential on Vault's app registration. Use the app registration's Object
ID and the issuer URL, Kubernetes namespace, and service account of the AKS
deployment hosting Vault. Keep the audience set to `api://AzureADTokenExchange`.
The projected service account token comes from the AKS cluster hosting Vault,
even when Vault authenticates workloads in tenant B.

1. Grant admin consent in **tenant B**. Open the following URL, sign in as a
   tenant B **Global Administrator**, and select **Accept** to provision a service
   principal in tenant B for the app registered in tenant A. Use the Application
   (client) ID recorded in the prerequisites:

   ```text
   https://login.microsoftonline.com/<tenant_b_id>/adminconsent?client_id=<app_client_id>
   ```

1. Use the Azure CLI to log into **tenant B** and select the subscription containing
...
1. Grant the Azure **Reader** role to the application's service principal in
   **tenant B**, scoped to the resource group containing the workloads. The **Reader** role lets
   Vault read Azure resource metadata when validating workload identities.
   Admin consent does not grant Azure resource access. Use an account with
   permission to create role assignments at the resource group scope:

   ```shell-session
   $ az role assignment create \
       --role "Reader" \
       --assignee "<app_client_id>" \
       --scope "/subscriptions/<tenant_b_subscription_id>/resourceGroups/<tenant_b_resource_group_name>"
   ```

   The `--assignee` value is the Application (client) ID from tenant A; the role
   assignment applies to its service principal in tenant B. See
   [Role assignments](#role-assignments) for the login parameters that require
...
1. Create a Vault Azure auth role bound to resources in tenant B. The role
   attaches the `default` policy to the Vault tokens issued on successful login:

   ```shell-session
   $ vault write auth/azure/role/cross-tenant-role \
       policies="default" \
       bound_subscription_ids=<tenant_b_subscription_id> \
       bound_resource_groups=<tenant_b_resource_group_name> \
       ttl=1h
   ```

### Migrating to AKS Workload Identity
...
1. Update the Azure auth configuration to use `aks_wif` by following the
   [single-tenant configuration](#single-tenant-configuration) or
   [cross-tenant configuration](#cross-tenant-configuration) steps for your
   deployment. Reuse the existing auth mount and roles. You do not need to enable
   the auth method again or recreate roles.
...
You do not need to update any existing Azure authentication roles, Vault policies, or workload login configurations.
...
modification. During the propagation window, token acquisition fails with `AADSTS70021`. When
you use AKS WIF, Vault proactively verifies the credential during login and
returns a clear error instead of a generic `500`. Wait 60 seconds after configuring the
federated credential, then retry.

Refer to the [API documentation](/vault/api-docs/auth/azure#configure)
