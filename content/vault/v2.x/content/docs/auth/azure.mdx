...
the workloads it authenticates running in a separate tenant (Tenant B), the you
...
1. As a **tenant B admin**, open the following URL in a browser and grant admin
   consent to provision a service principal in tenant B for the app registered
   in tenant A. Use the Application (client) ID recorded in the prerequisites:

   ```text
   https://login.microsoftonline.com/<tenant_b_id>/adminconsent?client_id=<app_client_id>
   ```

1. Use the Azure CLI to log into **tenant B**:
...
To configure the federated identity credential, sign in to tenant A with
`az login --tenant <tenant_a_id>` and run the
[`az ad app federated-credential create` command in the prerequisites](#prerequisites-1).
Use the object ID of the App Registration in tenant A, the OIDC issuer URL of the
AKS cluster hosting Vault, and the Vault pod's Kubernetes namespace and service
account names. Keep the audience set to `api://AzureADTokenExchange`. The projected
service account token is always issued by the AKS cluster hosting Vault,
regardless of the target workload tenant.
