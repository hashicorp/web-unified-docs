...
1. Configure the Azure auth method:
   ...
   - [Plugin Workload Identity Federation](#plugin-workload-identity-federation-wif) (WIF), where Vault issues its own OIDC identity token that Azure is configured to trust via workload identity federation.
...
~> **Note:** `aks_wif` also resolves the `InvalidAuthenticationTokenTenant` error that
...
1. Enable the Azure auth method and configure it with `auth_type=aks_wif`:
...
The `msi` fallback issues tokens scoped to Vault's home tenant (Tenant A). Calling Azure
...
`auth_type=aks_wif` is set, Vault proactively verifies the credential during login and
