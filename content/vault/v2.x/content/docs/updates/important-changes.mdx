...
### AWS credential precedence change for AWS auth method and AWS secrets engine ((#aws-credential-precedence))

| Change       | Affected version | Vault edition
| ------------ | ---------------- | -------------
| Breaking     | 2.2.0+           | All

When both Workload Identity Federation (WIF) credentials and AWS environment variables are configured,
the [AWS auth method](/vault/docs/auth/aws) and the [AWS secrets engine](/vault/docs/secrets/aws)
now always use the explicitly configured WIF credentials and environment variables are ignored.
In previous versions, environment variables took precedence over explicitly
If you have Workload Identity Federation (WIF) credentials and AWS environment
variables configured, the [AWS auth method](/vault/docs/auth/aws) and the
[AWS secrets engine](/vault/docs/secrets/aws) now always use the explicitly
configured WIF credentials.

In previous versions, environment variables took precedence over explicitly
configured WIF credentials during credential resolution. But Vault now ignores
and environment variables in favor of WIF credentials.

#### Recommendation

Review any deployments where you configured WIF alongside AWS environment variables
to confirm your intended credential precedence. If your workflow relied on environment
variables taking precedence, update your configuration prior to upgrading to Vault 2.2.0.
