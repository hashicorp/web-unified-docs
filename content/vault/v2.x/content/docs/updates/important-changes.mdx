...
**This change has no impact on existing deployments** that use only opaque Vault
tokens. The limit becomes customer-visible when [External JWT
Tokens](/vault/docs/auth/jwt) are in use, because JWT tokens (especially those
with rich `authorization_details` claims) can be significantly larger than opaque
tokens.
