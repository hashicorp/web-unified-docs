...
Workflows such as the supported SDK and Vault CLI, as well as your own
software, can use the data key set generation
to generate keys to use with [envelope encryption](/vault/docs/secrets/transit/envelope-encryption).
...
- `ml-dsa` - ML-DSA; supports signing and signature verification <EnterpriseAlert inline="true" />
- `hybrid` - combination of two signature algorithms; supports signing and signature verification <EnterpriseAlert inline="true" />
...
You can manage Transit secret resources programmatically with the Vault Terraform provider. Refer to the Terraform Registry documentation for more details:

- [Transit secrets engine cache config resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/transit_secret_backend_cache_config)
- [Transit secrets engine key resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/transit_secret_backend_key)
