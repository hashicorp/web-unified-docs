...
created_at: 2026-04-15T00:15:41.000Z
last_modified: 2026-04-15T00:15:41.000Z
...
1. Ensure that Vault
   [openid-configuration](/vault/api-docs/secret/identity/tokens#read-plugin-identity-token-issuer-s-openid-configuration)
   and [public
   JWKS](/vault/api-docs/secret/identity/tokens#read-plugin-identity-token-issuer-s-public-jwks)
   APIs are network-reachable by GCP. If you need to limit Vault API exposure or use an API proxy,
   gateway, or configure the [`--jwk-json-path="JWK-FILE-PATH"`
   option](https://docs.cloud.google.com/iam/docs/workload-identity-federation-with-other-providers#expandable-3)
   to configure a static JWKS file for the plugin identity token issuer.
...
impersonate the service account. `sub` claims have the form: `plugin-identity:<NAMESPACE_ID>:secret:<GCP_SECRETS_MOUNT_ACCESSOR>`.
...
You can manage GCP secret resources programmatically with the Vault Terraform provider. Refer to the Terraform Registry documentation for more details:

- [GCP secrets engine backend resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/gcp_secret_backend)
- [GCP secrets engine roleset resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/gcp_secret_roleset)
- [GCP secrets engine static account resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/gcp_secret_static_account)
- [GCP secrets engine impersonated account resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/gcp_secret_impersonated_account)
