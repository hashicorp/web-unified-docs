...
1. Ensure that Vault
   ...
   APIs are network-reachable by GCP. If you need to limit Vault API exposure use an API proxy,
   gateway, or configure the [`--jwk-json-path="JWK-FILE-PATH"` option](https://docs.cloud.google.com/iam/docs/workload-identity-federation-with-other-providers#expandable-3)
   to use a static JWKS file.
