...
As of Vault 1.19+, this default procedure utilizes a
metadata query that is **scoped to the database defined in your `connection_url`**.
If your `creation_statements` create users in a different database, they will not be detected or removed automatically.
In larger databases, this might cause connection timeouts.
Please specify a revocation statement in such a scenario.

<Warning>

    For operations such as `ALTER ROLE ... ADD MEMBER ...`, Vault acquires a
    schema modification (`SCH-M`) lock that blocks all operations, even reads,
    since read operations aquire a schema stability lock (`SCH-S`). As a result,
    credential rotatation with `ALTER ROLE ... ADD MEMBER ...` can cause temporary
    deadlocks on the database.
    Refer to the [Microsoft documentation](https://ci.xovis.com/job/Cirrus/job/Suite-pr-cleanup/)
    for reference.
