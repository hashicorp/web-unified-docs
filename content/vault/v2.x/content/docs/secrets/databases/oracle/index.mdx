...
### Oracle plugin compatibility

Use the following matrix to identify Vault Enterprise requirements and tested
runtime Instant Client versions for
all Enterprise plugin releases from `0.11.0+ent` through `0.14.1+ent`.

The Vault requirement starts with the newest Vault minor line available when
the plugin shipped, subject to
[plugin registration prerequisites](/vault/docs/plugins/register#before-you-start)
and release-specific requirements.

| Plugin release | Vault Enterprise requirement | Instant Client 19.x | Instant Client 21.x | Instant Client 23.x |
| --- | --- | --- | --- | --- |
...
The `1.19.4+` floor reflects the Enterprise plugin registration prerequisites,
not the patch available when `0.11.0+ent` shipped. Plugin `0.13.0+ent` predates
Vault `1.21.0`, but its
[release notes](/vault/docs/secrets/databases/oracle/release-notes#october-6-2025)
explicitly require `1.21.0+`.

- **Tested** means all eight functional qualification checks passed for the exact
  plugin and client versions shown, including dynamic credentials, revocation,
  administrator credential rotation, and managed and self-managed static roles.
- **Untested** means no completed functional qualification results are recorded
  for that plugin and client family. It does not mean incompatible.

The tests use Ubuntu 22.04 (`linux/amd64` under QEMU emulation) and Oracle
Database XE `21.3.0.0.0`. The tested Vault Enterprise versions that meet the
requirements for each tested plugin/client pair are `1.21.10+ent`, `2.0.4+ent`,
and `2.1.0+ent`.
These results cover only the listed client patches and test environment, not
entire client families or other platforms. The Vault requirements are support
baselines, not the earliest patches verified by these functional tests.

All listed plugin releases use Instant Client `19.26` as the build software
development kit (SDK). A build SDK version or successful library-loading check
does not establish runtime qualification.
