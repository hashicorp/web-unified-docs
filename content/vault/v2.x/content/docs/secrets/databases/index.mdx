...
created_at: 2026-04-15T00:15:39.000Z
last_modified: 2026-04-15T00:15:39.000Z
...
By default, static role credentials rotate on a fixed period using
`rotation_period`. As an alternative, you can use a cron-style
`rotation_schedule` to control exactly when rotation occurs with an optional
`rotation_window` that limits the time Vault has to complete the rotation.
...
Vault logs rotations with reference to the `name` of the role and `error` if the
rotation failed. The logs also indicate if the rotation was part of a `periodic function`.

### Retry behavior on failure

When a scheduled rotation attempt fails, Vault retries automatically in the
background. Vault checks the rotation queue for pending rotations every 5
seconds, and retries a failed rotation after a fixed 10-second backoff. Vault
does not cap the number of retries or apply an exponential backoff. Vault keeps
retrying the rotation on a fixed schedule until the rotation succeeds or the
`rotation_window` closes, whichever comes first.

If the `rotation_window` closes before the rotation succeeds, Vault stops
retrying and schedules the next attempt for the next occurrence of
`rotation_schedule` (for example, the following Saturday at 00:00). Vault
does not invalidate or lock out the account just because a rotation attempt
failed and the credential keeps its last successfully rotated value. 

To rotate the credential immediately, rather than waiting for the next scheduled window,
use the [Rotate root credentials endpoint](/vault/api-docs/secret/databases#rotate-root-credentials)
for root credentials, or the
[Rotate static role credentials](/vault/api-docs/secret/databases#rotate-static-role-credentials)
endpoint for static roles.
