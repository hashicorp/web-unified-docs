...
created_at: 2026-04-15T00:15:54.000Z
last_modified: 2026-04-15T00:15:54.000Z
...
[FF3-1](https://csrc.nist.gov/publications/detail/sp/800-38g/rev-1/draft) (deprecated)
and [FF1](https://csrc.nist.gov/pubs/sp/800/38/g/r1/2pd), but
...
1. Create a transformation:
   ...
   ~> ** Note **
   As of Vault 2.1.0, FF3-1 is deprecated. All new FPE transformations
   created using Vault 2.1.0 or higher will use FF1.
...
transformation to encode input values while maintaining its data
format and length.

~> **Deprecation Warning!**
As of the latest [NIST publication](https://csrc.nist.gov/pubs/sp/800/38/g/r1/2pd)
for FPE, FF3-1 is no longer approved due to a vulnerability in the tweak schedule.
Beginning with 2.1.0, FF3-1 is deprecated, and newly created
transformations will use FF1 instead. Existing FF3-1 transformations
will support encoding and decoding, but creation of new FF3-1
transformations is no longer supported.

#### Tweak and tweak source

FPE uses a non-confidential parameter called the tweak along with the
ciphertext when performing encryption and decryption operations. For FF3-1,
the tweak is precisely a 7-byte value. For FF1, the `max_tweak_len` parameter
on the transformation determines valid tweak lengths for that transformation.
A value of `0` allows tweaks of any length.

The secret engine consumes a base64 encoded string of this value for its
encode and decode operation whenever this input is required.
...
- `supplied`: This is the default behavior for FPE transformations. The tweak
  value must be generated externally, and supplied into the encode and decode
  operations.
...
FPE algorithms specify both minimum and maximum limits on the length of an input.
...
For FF3-1, using an alphabet of length A, an input length L is valid if:
...
For FF1, using an alphabet of length A, an input length L is valid if:
