...
created_at: 2026-04-15T00:15:44.000Z
last_modified: 2026-04-15T00:15:44.000Z
...
In addition to the legacy CA, you can now create and manage named CAs that can be
used for:

- **Server certificates**: Generate TLS certificates for KMIP listener endpoints
- **Client certificates**: Verify client certificates for mTLS authentication
...
To import an external CA for client verification:
...
Or use certificate fields to extract scope and role:
...
You can list all configured CAs:
...
The KMIP secrets engine now supports named listeners, which allow you to configure
multiple KMIP server endpoints with different settings. Each listener is associated
with a specific CA for generating server certificates and can trust additional CAs
for client certificate verification.

To create a named listener:
...
Parameters:
- `ca` (required): Name of the CA to use for generating the server certificate
- `address` (required): Host:port address for the listener
- `server_hostnames`: DNS SANs to include in the server certificate
- `server_ips`: IP SANs to include in the server certificate
- `additional_client_cas`: Additional CAs to trust for client certificate verification
- `also_use_legacy_ca`: Whether to also trust the legacy CA for client certificates
- `tls_min_version`: Minimum TLS version (tls12 or tls13)
- `tls_max_version`: Maximum TLS version (tls12 or tls13)
- `tls_cipher_suites`: Allowed TLS cipher suites (for TLS 1.2 only)
...
~> **Note**: Named listeners provide more flexibility than the legacy `listen_addrs`
configuration in `/kmip/config`. You can use named listeners alongside or instead of
...
1.  Create a role within the scope, specifying the set of operations to allow or
    deny. Optionally, specify a named CA to use for signing client certificates.
    ...
    To use a specific named CA for client certificates:
