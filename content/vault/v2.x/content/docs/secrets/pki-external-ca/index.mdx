...
The response contains two distinct status fields: `order_status` and
`challenge_status` (nested inside `challenges`). These come from different
sources and change independently of each other.

#### `order_status` — managed by Vault

`order_status` reflects where the order sits in Vault's internal workflow.
Vault owns this value and updates it as the order moves through each
processing stage.
...
The upstream ACME server (such as Let's Encrypt) sets `challenge_status` for
each individual challenge. Vault stores this value when it fetches challenge
details from the ACME server but does not control it. Common values are
`pending`, `valid`, and `invalid`, as defined by
[RFC 8555](https://datatracker.ietf.org/doc/html/rfc8555/#section-7.1.6).

#### Why a failed order may show `challenge_status: pending`

Vault does not keep these two statuses in sync. If an order fails before
Vault notifies the ACME server, for example, because Vault's internal DNS
propagation check timed out, the ACME server never verifies the challenge.
The challenge remains in whatever state Vault last fetched from the ACME
server, which is typically `pending`.
