...
external systems to fulfill. They retrieve challenge tokens from Vault and manually create DNS records or HTTP tokens.

1. **Automatic DNS Fulfillment**: Vault automatically manages DNS records using configured provider
credentials. When DNS providers are configured and match all the requested identifiers within an order, Vault handles
the entire ACME certificate request process.

1. **Hybrid Fulfillment**: Some challenges are fulfilled manually while others are automatically fulfilled by Vault.
This is useful when you have DNS providers configured for some domains but not others.

### DNS provider configuration

You manage DNS providers independently `config/dns/<provider-type>/<name>` and specify supported identifiers
using glob patterns. During ACME challenge evaluation for any order from any ACME account, the system will:

1. Select one or more matching DNS providers based on the order's identifiers
1. Challenges with no matching provider require manual fulfillment
1. Matching challenges will be automatically fulfilled using the DNS provider
1. In hybrid scenarios, manual challenges must be fulfilled first, then Vault performs automatic DNS updates
...
Before using a DNS provider within an ACME workflow, a test API endpoint
allows you to validate the DNS provider configuration can be used without requesting any certificates.
...
1. **If DNS providers are configured**: Vault automatically handles DNS-01 challenges for matching identifiers. Skip to step 5.

1. **If no DNS providers are configured or identifiers don't match**: Fetch the challenge token and auth key for each identifier from Vault:
...
- `fetching_certificate` - Retrieving issued certificate, the ACME server has accepted the challenges
