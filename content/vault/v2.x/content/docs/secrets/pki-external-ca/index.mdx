...
last_modified: 2026-04-15T00:15:50.000Z
...
The PKI External CA secrets engine is verified to work with
- [GlobalSign](https://www.hashicorp.com/en/partners/tech/globalsign#all)
- [Sectigo](https://www.hashicorp.com/en/partners/tech/sectigo#all)
- [DigiCert](https://www.hashicorp.com/en/partners/tech/digicert#all)
...
If you know the role name:

```shell-session
$ vault read pki-external-ca/role/<role_name>/order/<order_id>/status
```

If you only have the order ID:

```shell-session
$ vault read pki-external-ca/lookup/order/<order_id>
```

The response contains two distinct status fields under `challenges`:
`order_status` and `challenge_status`. The status come from different sources
and change independently of each other.

#### `order_status` — managed by Vault

`order_status` reflects where the order sits in the internal workflow for Vault.
Vault owns the order status and updates it as the order moves through each
processing stage.

| Status | Meaning |
| --- | --- |
| `new` | Vault created the order but has not yet submitted it to the ACME server |
| `submitted` | Vault submitted the order to the ACME server |
| `awaiting-challenge-fulfillment` | Vault is waiting for the client to fulfill challenges |
| `vault-challenge-fulfillment` | Vault is fulfilling DNS challenges |
| `vault-challenge-propagating` | Vault is waiting for internal validation of the DNS challenges it made in `vault-challenge-fulfillment` |
| `notify-acme-server-challenges-completed` | Vault is ready to notify the ACME server |
| `processing-challenge` | The ACME server is validating challenges |
| `fetching-certificate` | Vault is retrieving the issued certificate |
| `completed` | Vault issued the certificate successfully |
| `expired` | The order expired before Vault completed it |
| `revoked` | Vault revoked the certificate |
| `error` | The order failed. Check the `last_error` field for details. |

#### `challenge_status` — reported by the ACME server

Vault does not own or control the `challenge_status` field. Upstream ACME servers,
like Let's Encrypt, set `challenge_status` for each individual challenge. Vault
stores the challenge status when it fetches challenge details from the ACME
server. [RFC 8555](https://datatracker.ietf.org/doc/html/rfc8555/#section-7.1.6)
defines the common status values: `pending`, `valid`, and `invalid`.

#### Why a failed order may show `challenge_status: pending`

Vault does not keep `order_status` and `challenge_status` in sync. If an order
fails before Vault notifies the ACME server, the ACME server never verifies the
challenge and the status remains in whatever state Vault last fetched from the
ACME server (typically `pending`). For example, if the internal DNS propagation
check times out for Vault, the order fails, but Vault does not update the
challenge status.

A `pending` challenge status does **not** indicate that the ACME server is
still working on the challenge. It indicates that Vault never asked the ACME
server to proceed. The definitive indicator of failure is `order_status:
error`, not `challenge_status`.

When an order is in the `error` state, check `last_error` in the order
response for the specific reason the order failed.
