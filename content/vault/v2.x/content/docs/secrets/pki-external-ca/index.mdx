...
| `vault-challenge-propagating` | Vault waits for internal validation of the DNS challenges it made in vault_challenge_fulfillment |
...
| `error` | An error occurred; a new order must be created |
| `expired` | Order expired before completion; a new order must be created |
