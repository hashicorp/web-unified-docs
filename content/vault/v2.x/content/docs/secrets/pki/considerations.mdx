...
creating and signing CSRs for intermediate CAs. The private key can be exported
at generation time using the command path (so the ability to do so can be
controlled with ACL policies). After generation, the private key can only be
exported with sudo privileges.

On Vault Enterprise, it is also possible to export any existing CA private key
for migration to a new Vault mount or instance. Unlike rotating the root, this
preserves the existing trust anchor, which means clients that already trust the
CA do not need to be updated.

To migrate a CA, export the CA key on the source mount using a root token or a
token with the `sudo` capability on the export path
([Export CA key](/vault/api-docs/secret/pki#export-ca-key)), then import the
wrapped key on the destination mount using Vault Enterprise
([Import key](/vault/api-docs/secret/pki#import-key)). Setting up the
destination mount first requires creating an export keypair
([Export Key Management](/vault/api-docs/secret/pki#export-key-management) — Vault Enterprise only).
Managed (HSM-backed) keys are not supported by this feature.
...
The `key_usage` and `ext_key_usage` fields are used to limit the usage of the certificate
to specific purposes. For example, a certificate can be limited to only be used
for client authentication, server authentication, or code signing.
Vault implements [RFC #5280](https://www.rfc-editor.org/rfc/rfc5280#section-4.2.1.3),
which means the generated key usage extension is marked as critical and cannot be ignored.
