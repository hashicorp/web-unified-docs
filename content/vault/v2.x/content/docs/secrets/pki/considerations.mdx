...
key will be slower than issuing from an ECDSA, Ed25519, or ML-DSA key. Key generation
...
P-256 -> P-521), signature times also increases. This is also true for more complex
parameter sets with ML-DSA (ML-DSA-44 -> ML-DSA-65 -> ML-DSA-87).
...
some considerations to be aware of:
 ...
 - RSA key generation is much slower and highly variable than EC key
   generation or key generation with ML-DSA. If performance and throughput 
   are a necessity, consider using EC keys (including NIST P-curves and Ed25519) 
   or ML-DSA instead of RSA.
 ...
 - The CA's key type matters as well: using a RSA CA will result in a RSA
   signature and takes longer than a ECDSA, Ed25519, or ML-DSA CA.
...
Some suggested keys to un-HMAC for requests are as follows:
 ...
 - Various issuance-related overriding parameters, such as:
   ...
   - `parameter_set` - parameter set for ML-DSA keys,
