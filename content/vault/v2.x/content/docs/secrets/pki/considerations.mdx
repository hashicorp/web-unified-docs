...
**Vault Enterprise v2.2.0+** provides a secure key export mechanism that allows
migrating an existing CA private key to a new Vault mount or instance, even if
the key was not marked exportable at creation time. This is preferable to
rotating the root in migration scenarios because it preserves the existing trust
anchor and clients that already trust the CA do not need to be updated.

To migrate a CA, create an export keypair on the destination mount
([Export Key Management](/vault/api-docs/secret/pki#export-key-management)),
export the CA key on the source mount using a sudo token
([Export CA key](/vault/api-docs/secret/pki#export-ca-key)), then import the
wrapped key on the destination mount
([Import key](/vault/api-docs/secret/pki#import-key)). Managed (HSM-backed)
keys are not supported by this feature.
