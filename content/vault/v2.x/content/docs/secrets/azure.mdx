...
If you enable seamless rotation for a static role, the plugin pre-provisions
(stages) the next credential within Azure with an explicit `startDateTime`. You
can configure the start datetime to overlap with the end of life for the
...
credential to become active just before the current credential expires.
...
- The plugin provisions `sec-2` within Azure, setting its start time 30 minutes prior to the expiration of `sec-1`.
