...
This allows platform teams to grant token access (`azure/static-creds/+/token`) separately
...
- **No Vault lease** is created. Access tokens cannot be revoked or renewed once issued
...
- **The static role must have been rotated at least once.** Roles created with 
  `defer_initial_creds=true` have no stored `client_secret` until first rotation, and will return a `400` error.
