...
last_modified: 2026-09-11T17:09:50.000Z
...
For WIF configuration, select **Workload Identity Federation** for ""Access Type"
...
The Azure secrets engine supports dynamic and static roles.

Vault uses static roles to manage long-lived client secrets for Azure
application. Unlike dynamic roles, which generate new credentials on each
request, static roles always return the same client secret until you explicitly
rotate the value. Once the secret rotates, Vault generates a new client secret
for the application and revokes the previous secret.

You can manually rotate secrets by calling the [`rotate-role`](/vault/api-docs/secret/azure#manually-rotate-static-role-password) endpoint.

In addition to generating new secrets, static roles also support importing
existing Azure credentials. This allows Vault to take over management of
credentials that were created outside of Vault and bring them under Vault’s
lifecycle controls.
...
- Rotate on import - Provide the `secret_id` and leave `skip_import_rotation`
  unset.  Vault rotates the credential during import to ensure a fresh, valid
  secret.
...
You must have Vault 2.2.0+ to use seamless rotation.

</Note>

Seamless rotation provides automated credential rotation for static roles. The
automated rotation eliminates the operational burden of manual rotation and
mitigates the availability risks that come from the consistent nature of Azure.

Vault does not automatically apply seamless rotation to existing mounts. You can
opt-in or opt-out by setting `seamless_rotation` on the relevant
[plugin configuration](/vault/api-docs/secret/azure#configure-access) or by
[configuring the static role](/vault/api-docs/secret/azure#create-update-static-role):

- To opt-in existing mounts, set `seamless_rotation=true`.
- To opt-out new mounts on creation, set `seamless_rotation=false`.

If you enable seamless rotaiton for a static role, the plugin pre-provisions
(stages) the next credential within Azure with an explicit `startDateTime`. You
can configured the start datetime to overlap with the end of life for the
current credential, which Vault calls the rotation grace period
(`rotation_grace_period`).

The default grace period is five minutes (`"5m"`), which allows a staged
credential to becomes active just before the current credential expires.

Once a staged credential activates, Vault always returns the client secret from
the staged credential when reading static credentials. The existing credential
remains valid until the rotation manager promotes the staged credential.

The chart below demonstrates a static role with seamless rotation enabled, using a fictitious `rotation_period` (or `ttl`)
of 2 hours, with a `rotation_grace_period` of 30 minutes.

![Diagram showing the lifecycle of a static role with seamless rotation enabled](/img/azure-seamless-rotation-lifecycle.png)
_Legend: A = Active, I = Inactive/Replicating, GP = Grace Period (Overlap)_

- The plugin stages provisions `sec-2` within Azure, setting its start time 30 minutes prior to the expiration of `sec-1`.
- When the grace period starts at `01:30`, `sec-1` remains valid, but Vault returns the new secret `sec-2`.
- Rotation manager then performs automated rotation at `02:00`, which revokes `sec-1` and stages the next credential (`sec-3`).

<Warning>

Even with seamless rotation, you can still use the manual rotation endpoint, but
calling the endpoint fully re-provisions credentials and may result in brief
availability issues while the new credentials propagate across Azure.
...
grant flow themselves. When a caller requests a token, Vault uses the `client_id`
and `client_secret` associated with the static role to request a bearer token from
Azure AD then wraps and returns the token response from Azure without modification.

Wrapping the token response allows platform teams to grant token access (`azure/static-creds/+/token`) separately
...
- **Vault does not create new leases**. Access tokens cannot be revoked or renewed once issued
  and expire naturally. By default, tokens expire approximately 1 hour after minting,
  but you can
  [configure an alternative default token expiration in Azure](https://learn.microsoft.com/en-us/entra/identity-platform/configure-token-lifetimes).
- **Access tokens are immediately consistent.** Access tokens never suffer the propagation delay that can
  affect rotated static role credentials.
- **Scope is caller-supplied.** Vault passes the `scope` parameter to Azure
...
- **The static role must have been rotated at least once.** Roles created with
  `defer_initial_creds=true` have no stored `client_secret` until first rotation,
  and Vault returns a `400` error if you try to mint a token without a client secret.
  Use the [`rotate-role`](/vault/api-docs/secret/azure#manually-rotate-static-role-password)
  endpoint first.

### ACL policy example

The following example grants token access to developers while denying direct credential access:

```hcl
# Allow developers to obtain access tokens for any role
path "azure/static-creds/+/token" {
...
You can manage Azure secret resources programmatically with the Vault Terraform provider. Refer to the Terraform Registry documentation for more details:

- [Azure secrets engine backend resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/azure_secret_backend)
- [Azure secrets engine role resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/azure_secret_backend_role)
- [Azure secrets engine static role resource](https://registry.terraform.io/providers/hashicorp/vault/latest/docs/resources/azure_secret_backend_static_role)
