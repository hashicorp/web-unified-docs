...
This feature requires Vault 2.2 or later.
</Note>

Seamless rotation is a feature which provides automated credential rotation for static roles,
eliminating the operational burden of manual rotation and availability risks that arise from
the eventually consistent nature of Azure. Existing mounts must opt-in by setting `seamless_rotation=true`
on the [plugin's configuration](/vault/api-docs/secret/azure#configure-access) or by
[configuring the static role](/vault/api-docs/secret/azure#create-update-static-role).
New mounts can opt-out in the same manner.

When enabled for a static role, the plugin will pre-provision (or stage) the next credential within Azure.
The credential staged by the plugin has an explicit `startDateTime` within Azure, and can be
configured to overlap the end of the current credential's life (referred to as `rotation_grace_period`).
By default, the `rotation_grace_period` is five minutes (`"5m"`), resulting in a staged
credential that becomes active _just before_ the current credential expires.

Once a staged credential activates, Vault always returns the staged credential's client secret
when reading static credentials. The existing credentials remain valid until rotation manager
promotes the staged credential, whichever occurs first.

**Note:** the manual rotation endpoint can still be used, but will fully reprovision credentials
and may result in brief availability issues while the new credentials propagate across Azure.
## Access tokens
