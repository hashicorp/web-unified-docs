...
last_modified: 2026-09-16T11:18:06.000Z
...
| `vault.agentregistry.agent.registrations.local.count` | Total number of local agent registrations for this cluster, across all namespaces. |
...
| `vault.auth.method.azure.auth_type.aks_wif.count` | Number of Azure auth method mounts configured to use AKS Workload Identity Federation (aks_wif) across all namespaces. |
| `vault.auth.method.azure.auth_type.auto.count` | Number of Azure auth method mounts configured to use automatic authentication (auto) across all namespaces. |
| `vault.auth.method.azure.auth_type.msi.count` | Number of Azure auth method mounts configured to use Managed Service Identity (msi) across all namespaces. |
| `vault.auth.method.azure.auth_type.plugin_wif.count` | Number of Azure auth method mounts configured to use plugin Workload Identity Federation (plugin_wif) across all namespaces. |
| `vault.auth.method.azure.auth_type.root_creds.count` | Number of Azure auth method mounts configured to use root credentials (root_creds) across all namespaces. |
...
| `vault.oauth.resource.server.config.local.count` | Total number of local OAuth resource server configurations for this cluster, across all namespaces. |
...
| `vault.secret.engine.alicloud.dynamic.role.count` | Number of AliCloud secret engine dynamic roles across all namespaces. |
...
| `vault.secret.engine.gcpkms.key.count` | Number of GCP KMS secret engine keys across all namespaces. |
...
| `vault.secret.engine.mongodbatlas.dynamic.role.count` | Number of MongoDB Atlas secret engine dynamic roles across all namespaces. |
...
| `vault.tpm.records.count` | Total number of TPM records registered in identity store |
| `vault.tpm.groups.count` | Total number of TPM groups in identity store |
| `vault.tpm.auth_methods.count` | Total number of TPM auth methods enabled |
| `vault.tpm.auth_roles.count` | Total number of TPM auth roles configured |
| `vault.tpm.machines.assigned_roles.count` | Total number of distinct TPM machines assigned to roles, including via group membership |
