- `rotation_period` `(string/integer: 0)` – <EnterpriseAlert product="vault" inline />
  ...
  Vault should wait before rotating the credential. A zero value tells Vault
  not to rotate the credential. The minimum rotation period is 10 seconds. **You must
...
- `rotation_schedule` `(string: "")` – <EnterpriseAlert product="vault" inline />
  ...
  defining the schedule on which Vault should rotate the credential. Standard
  ...
  Vault to rotate the credential every Saturday at 00:00. In 1.20.5 or later, Vault interprets the schedule in UTC.
...
- `rotation_window` `(string/integer: 0)` – <EnterpriseAlert product="vault" inline />
  ...
  a rotation when a scheduled credential rotation occurs. If Vault cannot rotate the
  credential within the window (for example, due to a failure), Vault must wait to
...
- `disable_automated_rotation` `(bool: false)` - <EnterpriseAlert product="vault" inline />
  Cancels all upcoming rotations of the credential until unset.