...
1. Use the Vault CLI in your interactive session to write out a policy that lets
   ...
   path "creds/data/devwebapp/config" {
      capabilities = ["read"]
   }
   EOF
   ```

1. Use the Vault CLI in your interactive session to create a corresponding
...
The authentication role connects the Kubernetes service account named in `bound_service_account_names` to the Vault policy named in `policies`.