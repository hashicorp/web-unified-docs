...
Choose to use either an authorization code or client credentials grant type
when defining your authorization server details.

<Tip>

You can use the workflow shown here to set up Okta for either authorization
code or client credentials grant types.
</Tip>

## Step 1: Create the OIDC Service App

You must first configure Okta with a Service App and custom authorization
server. You then define an authorization server policy and default scope
for reading Vault secrets.
...
1. Click **Web Application**.
   ...
   ![Application general setttings](/img/agentic-ai/screenshot-okta-app001.png)
...
1. Click **Token Exchange**.
   ...
   ![Application general setttings](/img/agentic-ai/screenshot-okta-app002.png)

1. Set the **Sign-in redirect URIs** to that of the `oauth2c` default value:
   `http://localhost:9876/callback`

   ![Redirect URI setttings](/img/agentic-ai/screenshot-okta-app003.png)

1. Click **Allow everyone in your organization to access**.

1. Click **Enable immediate access with Federation Broker Mode**.

   ![Client access setttings](/img/agentic-ai/screenshot-okta-app004.png)
...
Now that you have configured Okta for agent access through an authorization
code or client credentials, you can
[configure Vault Enterprise to use Okta as an authorization server](/vault/docs/ai/iam/setup) for your AI agents.

Vault requires the following information for setup:

Variable         | Purpose
---------------- | -------------------------------------------------------------
`ISSUER`         | The issuer retrieved from the Well known Open ID configuration of the JWT issuer. This is the `OKTA_URL` value.
`SUBJECT_EXT_ID` | Client ID of the **subject** user from Okta; this is the  `OKTA_CLIENT_ID` from the steps in this workflow.
`ACTOR_EXT_ID`   | Client ID of the **actor** user from Okta; this is the  `OKTA_CLIENT_ID` from the steps in this workflow.
