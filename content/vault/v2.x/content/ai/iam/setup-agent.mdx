...
last_modified: 2026-09-01T23:38:52.000Z
...
1. Write a secret to the `kv` plugin as yourself:
    ...
    ```shell-session
    $ vault write secret-v2/data/shared-secret/test - <<EOF
      {
        "data": {
...
1. Try to use the original minted token to write a `kv` secret:
    ...
    ```shell-session
    $ VAULT_TOKEN=${AGENT_TOKEN} vault write secret-v2/data/shared-secret/test - <<EOF
      {
        "data": {
          "secret_author": "I am an AI agent"
      }
    }
    EOF
