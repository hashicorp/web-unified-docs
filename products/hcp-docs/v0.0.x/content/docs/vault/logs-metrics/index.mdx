---
page_title: HCP Vault logs and metrics overview
sidebar_title: Overview
description: |-
  HCP Vault logs and metrics overview
---

# HCP Vault logs and metrics overview

Audit log and metrics observability is essential for ensuring the performance and security of
your HCP Vault cluster. It's also useful for business operations, like
understanding client-related usage. HCP Vault metrics provide operational
insights into:
...
HCP Vault metrics include critical Vault performance and usage metrics from the
[Vault telemetry](/vault/docs/internals/telemetry)
endpoint, as well as host performance metrics. To reduce noise, the metrics
available for HCP Vault are scoped to best practice metrics that are actionable
to users in a managed service context. This document details the metrics
available to HCP Vault production clusters, and provides guidance on detecting
...
For detailed instructions on how to configure HCP Vault audit log, and metrics streaming, refer to the
specific provider documentation in the left navigation menu. Unless
otherwise noted, any HCP Vault sample dashboards average all gauge metrics and
...
The HCP Vault sample dashboards will display "Unsealed" if at least one node is
...
These metrics represent system level CPU measurements. In the HCP Vault sample
...
The following metrics represent host memory measurements. In the HCP Vault
...
These metrics represent host disk measurements. In the HCP Vault sample
...
In the HCP Vault sample dashboards this metric is broken down by auth method and
TTL.
...
In the HCP Vault sample dashboards this metric is displayed by mount.
...
In the HCP Vault sample dashboards this metric is grouped by namespace.

## Metrics streaming configuration

For detailed instructions on how to configure HCP Vault audit log or metrics streaming to
your preferred provider, refer to the following documentation:

### Audit logs

- [Configure HCP Vault Metrics Streaming to CloudWatch](/hcp/docs/vault/logs-metrics/cloudwatch/logs)
- [Configure HCP Vault Metrics Streaming to Datadog](/hcp/docs/vault/logs-metrics/datadog/logs)
- [Configure HCP Vault Metrics Streaming to Elasticsearch](/hcp/docs/vault/logs-metrics/elasticsearch/logs)
- [Configure HCP Vault Metrics Streaming to Grafana Cloud](/hcp/docs/vault/logs-metrics/grafana/logs)
- [Configure HCP Vault Metrics Streaming to HTTP sink](/hcp/docs/vault/logs-metrics/http/logs)
- [Configure HCP Vault Metrics Streaming to New Relic](/hcp/docs/vault/logs-metrics/new-relic/logs)
- [Configure HCP Vault Metrics Streaming to Splunk](/hcp/docs/vault/logs-metrics/splunk/logs)

### Metrics

- [Configure HCP Vault Metrics Streaming to CloudWatch](/hcp/docs/vault/logs-metrics/cloudwatch/metrics)
- [Configure HCP Vault Metrics Streaming to Datadog](/hcp/docs/vault/logs-metrics/datadog/metrics)
- [Configure HCP Vault Metrics Streaming to Elasticsearch](/hcp/docs/vault/logs-metrics/elasticsearch/metrics)
- [Configure HCP Vault Metrics Streaming to Grafana Cloud](/hcp/docs/vault/logs-metrics/grafana/metrics)
- [Configure HCP Vault Metrics Streaming to HTTP sink](/hcp/docs/vault/logs-metrics/http/metrics)
- [Configure HCP Vault Metrics Streaming to New Relic](/hcp/docs/vault/logs-metrics/new-relic/metrics)
- [Configure HCP Vault Metrics Streaming to Splunk](/hcp/docs/vault/logs-metrics/splunk/metrics)
