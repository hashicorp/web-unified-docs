---
page_title: Configure HCP Vault metric streaming to generic HTTP sink
sidebar_title: generic HTTP sink
description: |-
  Configure HCP Vault metric streaming to generic HTTP sink
---
...
The generic HTTP sink is an optional metric streaming configuration that can be used when native integrations
...
If you are using an existing HCP Vault cluster, metrics will be publicly available.

</Highlight>

1. Open a web browser and navigate to https://webhook.site/

1. Make note of the URL generated.
...
1. Metrics will start to appear after a few minutes, though the process to enable metric streaming in the HCP Portal
   may take up to 20 minutes.

   <Note>

   At this time, HCP Vault only supports metric streaming to
