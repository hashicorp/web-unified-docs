...
description: Front-load descriptions with target keywords and keyword phrases so that users can easily search.
---
...
The background block contains information that may not directly be related to the topic but is necessary for helping readers understand the topic. The background block always has an H2 (##) heading with "Background" as the text.

Some topics are abstract or provide a narrow view into the product's overall usability. Use background blocks to fill in gaps that help readers form a mental model about why the information is relevant.
