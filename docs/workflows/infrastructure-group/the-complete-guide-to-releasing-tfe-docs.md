# The complete guide to releasing TFE docs

This guide is the single comprehensive reference for how Terraform Enterprise
(TFE) documentation gets from `content/terraform-docs-common` to a published
TFE release. It combines the release process (who does what, and when) with
the engineering detail of the automation that does it (the
`copy-cloud-docs-for-tfe` action, the four GitHub Actions workflows that call
it, and the changelog scripts that generate release notes). It's intended for
software engineers who maintain this automation and technical writers and
release engineers who run it.

Two narrower companion documents cover parts of this same material in more
focused form: [Terraform Enterprise releases](publish-tfe-docs.md) for the
human release process on its own, and [Copy cloud docs for TFE
action](copy-cloud-docs-for-tfe-action.md) for the action and workflow
engineering detail on its own. This guide supersedes neither — read it when
...
change gets a new version folder and a full release cycle (covered in
[The release workflows](#the-release-workflows)), while a FIXES change is
published directly against `main` through **Create TFE Patch Release Notes**,
with no new version folder.

## The docs pipeline at a glance
...
mirrored byte-for-byte into `hashicorp/web-unified-docs-internal`. The two
release-notes workflows only function when run from that internal
repository — refer to [The release workflows](#the-release-workflows) for the
...
- A draft PR named `TFE Release <version>.<release>.x` (**PR 1**), from
  `tfe-release/<version>.<release>.x` into `main`. This is the PR merged on
  release day to publish the docs.
- A draft PR named `HCP TF changes for TFE release <version>.<release>.x`
  (**PR 2**), from `HCPTF-diff/<version>.<release>.x` into
  `tfe-release/<version>.<release>.x`.
- A draft PR also named `TFE Release <version>.<release>.x` (**PR 3**), from a
...
the file, then transforms its frontmatter and body before writing it to the
TFE target:
...
TFC:only -->` HTML comment tags described in [Exclusion tag
syntax](#exclusion-tag-syntax). Those tags exclude content at render time, not
...
merged into `content/terraform-docs-common` on `main` after this run won't be
copied into the version folder until Sync Cloud Docs For TFE runs again.
...
shares the same triggers and `version` input as the Copy workflow, but is
meant to be run repeatedly against branches the Copy workflow already
created. Its single job (`sync-docs`) does the following:
...
one last time, and generates the release notes.
...
1. **`release-notes`** (needs `sync-docs`) checks out `tfe-release/<version>` and runs the changelog scripts in
   ...
   false) and edits PR 1's body again, this time adding links to both PR 2
   and PR 3.
...
`VERSION.RELEASE.FIXES` described in [Release
versioning](#release-versioning). It takes the same inputs as Create TFE
Release Notes, but differs in three ways:
...
can implement new content in the correct place, and populates the new folder
with a copy of the HCP Terraform docs on `main` from the public repo. For
this reason, don't run it too early in the cycle — content merged to `main`
after it runs won't be copied to the upcoming enterprise folder until Sync
Cloud Docs For TFE runs again. Running it too late in the cycle creates a
bottleneck of content changes.
...
1. Create an external docs plan: draft changes in a Word doc to streamline the review and approval process, and copy the content to the appropriate files after app deadline.
...
1. Author content under the current version folder. After app deadline, manually port changes to the folder for the upcoming version and roll back changes in the current folder. This approach works best when all or most content is confined to new `.mdx` files.

### Exclusion tag syntax

Most content in the Terraform Enterprise documentation is sourced from the
`terraform-docs-common` folder shared with HCP Terraform, but some features,
such as operating HCP Terraform in Europe and tiered pricing, are specific to
the SaaS offering. There is also sometimes a lag between when a feature
...
Enterprise from leaking into HCP Terraform's docs.

You can exclude page-level content as well as entire MDX files.

#### Exclude content on a page

Use HTML comment tags and add the `BEGIN: TFC:only` and `END: TFC:only`
exclusion directives to exclude content from displaying in the Terraform
Enterprise docs:
...
Conversely, exclude content from displaying on the HCP Terraform docs using
`BEGIN: TFEnterprise:only` and `END: TFEnterprise:only` exclusion directives:
...
Except for the `BEGIN:` and `END:` directives, the content of each tag must
be identical, otherwise the platform recognizes them as different directives
and returns an error. The `name` attribute is optional, but it is especially
helpful for staying organized when the page contains several exclusions.

You can exclude MDX components, such as callouts, but there must be a line
break between the components and the exclusion directives:
...
You can also exclude content mid-sentence, but pay extra attention to your
...
the page's frontmatter. For example:
...
This is the only exclusion mechanism the `copy-cloud-docs-for-tfe` action
itself enforces — refer to [File filtering](#file-filtering).
...
and also prepares the release notes section of the docs. Refer to [Release
notes guidance](#release-notes-guidance) for assistance.
...
1. Content authors add TFE-only content directly to `tfe-release/<version>`, using one of the options in [Prepare for app deadline](#prepare-for-app-deadline).
1. As needed before the deadline, the release engineer runs **Sync Cloud Docs For TFE** to refresh `HCPTF-diff/<version>` with the latest `content/terraform-docs-common` changes.
...
- [Terraform Enterprise releases](publish-tfe-docs.md): A narrower reference
  covering the human release process on its own.
- [Copy cloud docs for TFE action](copy-cloud-docs-for-tfe-action.md): A
  narrower reference covering the action and workflow engineering detail on
  its own.
