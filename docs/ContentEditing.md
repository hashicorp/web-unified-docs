# Editing documentation in UDR

## Table of Contents

- [Editing documentation in UDR](#editing-documentation-in-udr)
  ...
  - [Embargoed Content](#embargoed-content)
...
The website can be run locally through [Docker](https://www.docker.com/get-started).
...
The docker image is pre-built with all the website dependencies installed, which is what makes it so quick and simple, but also means if you need to change dependencies and test the changes within Docker, you'll need a new image. If this is something you need to do, you can run `make clean` to remove the local Docker image, then `make` again to generate a new local Docker image with updated dependencies.
...
The structure of the sidebars are controlled by files in the [`/data` directory](data) of the desired product. For example, [content/terraform-plugin-sdk/v2.36.x/data/plugin-sdkv2-nav-data.json](data/docs-nav-data.json) controls the **docs** sidebar for version `v2.36.x` of `terraform-plugin-sdk`. Within the `data` folder, any file with `-nav-data` after it controls the navigation for the given section.
...
This website is hosted on Vercel and configured to automatically deploy anytime you push code to the `main` branch. Any time a pull request is submitted that changes files a deployment preview will appear in the github checks which can be used to validate the way docs changes will look live. Deployments from `main` will look and behave the same way as deployment previews.
